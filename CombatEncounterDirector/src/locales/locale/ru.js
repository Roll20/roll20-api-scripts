const TRANSLATION = {
  ...
  errors: {
    ...
    reinforceActionHint:
      "Допустимые действия: дублировать, перечислять, показывать.",
    noReinforcementsToReveal:
      "Никаких недавних подкреплений, о которых можно было бы узнать.",
    noReinforcementsToRevealHint:
      "Используйте !director сначала для усиления дубликата.",
    ...
    unknownEncounterAction:
      "Неизвестное действие при столкновении: «{action}».",
    encounterActionHint:
      "Допустимые действия: сохранить, загрузить, удалить, составить список.",
    encounterNameRequired: "Требуется имя встречи.",
    encounterNameRequiredHint:
      "Пример: !director встреча сохранить засаду гоблинов",
    ...
    reportActionHint:
      "Допустимые действия: обновить, выбрать, изменить, очистить.",
    ...
    deckViewHint:
      "Допустимые виды: все, масштабирование, позиционирование, администрирование.",
    ...
    invalidPartySize:
      'Размер группы должен быть числом от 1 до 30 (получено "{value}").',
    invalidHpPercent:
      'Процент HP должен быть между 1 и 1000 (получено "{value}"). Пример: 150',
    invalidAcModifier:
  ...
  confirm: {
    scalingPresetPending:
      "Выберите токены и нажмите «Применить масштабирование».",
    journalsRebuilt: "Командная колода и журнал состояния были восстановлены.",
    deckUpdated:
      "Командная колода была воссоздана с использованием представления {view}.",
