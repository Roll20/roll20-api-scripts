    ...
    if (command == 'co'){
        ...
        function Skill(userid, targetid, obj, triggertime) { //haha END ME
            ...
            function skillMain() {
                ...
                if (obj.u_stat_target != "CurrHPU" && obj.u_stat_target != "CurrHPE" && obj.u_stat_target != "none"){
                    if (StattargetmodU > 0){
                        queue.push([StattargetU, "decrement", 1, 0, "command"])
                        log([StattargetU, "decrement", 1, 0, "command"])
                        log("Pushed to queue!")
                    } else {
                        queue.push([StattargetU, "increment", 1, 0, "command"])
                        log([StattargetU, "increment", 1, 0, "command"])
                        log("Pushed to queue!")
                    }
                    //check queue for repeated buff/debuffs
                    for (var i in queue){
                        if ((queue[i][0] == StattargetU) && (queue[i][4] == "command") && (queue[i] != queue[queue.length - 1])){ //the last element should be immune since it just got pushed
                            queue.shift();
                            i--;
                            StattargetU.setWithWorker({
                                current: currvlU
                ...
                if (obj.e_stat_target != "CurrHPU" && obj.e_stat_target != "CurrHPE" && obj.e_stat_target != "none"){
                    if (StattargetmodE> 0){
                        queue.push([StattargetE, "decrement", 1, 0, "command"])
                        log([StattargetE, "decrement", 1, 0, "command"])
                        log("Pushed to queue!")
                    } else {
                        queue.push([StattargetE, "increment", 1, 0, "command"])
                        log([StattargetE, "increment", 1, 0, "command"])
                        log("Pushed to queue!")
                    }
                    //check queue for repeated debuffs
                    for (var i in queue){
                        if ((queue[i][0] == StattargetE) && (queue[i][4] == "command") && (queue[i] != queue[queue.length - 1])){ //change the checked string for each different queuetype
                            queue.shift();
                            i--;
                            StattargetE.setWithWorker({
                                current: currvlE
                ...
                if (obj.radius != 0) {
                    ...
                    for (var i in tokenInRadius) {
                        ...
                        for (var i in effect) {
                          ...
                          if (target[i] != HPcurrC) {
                            if (parseInt(effect[i]) > 0){
                                queue.push([target[i], "decrement", 1, 0, "command-r"])
                                log([target[i], "decrement", 1, 0, "command-r"])
                                log("Pushed to queue!")
                            } else {
                                queue.push([target[i], "increment", 1, 0, "command-r"])
                                log([target[i], "increment", 1, 0, "command-r"])
                                log("Pushed to queue!")
                            }
                            for (var j in queue){
                                if ((queue[j][0] == target[i]) && (queue[j][4] == "command-r") && (j != queue.length - 1)){ //the last element should be immune since it just got pushed
