...
/* --------------------------------------------------- Table Management --------------------------------------------- */
	...
	var getTable = function(character,tableObj,name,tableDef,attrDef,col,defaultVal,caseSensitive) {
//        log('getTable table'+name+' attrDef='+attrDef);
        let rowName;
        col = _.isNull(col) || _.isUndefined(col) ? '' : col;
	    if (_.isNull(col) || _.isUndefined(col) || (tableDef && !_.isNull(tableDef) && !tableDef[1] && col && col==1)) {col = '';}
	    if (tableDef && !_.isNull(tableDef)) {
            rowName = tableDef[0]+col+'_$0_'+attrDef[0]+col;
	    } else {
            rowName = attrDef[0];
	    }
		if (_.isNull(defaultVal) || _.isUndefined(defaultVal)) {
//		    log('getTable table '+name+' defaulting defaultVal to '+attrDef[2]);
...
//		log('getTable defaultVal for table '+name+' has been set to '+tableObj[name].defaultVal);
...
//            log('getTable initialising attrs '+(_.isUndefined(tableObj.sortKeys)?'and sortKeys':''));
	...
	var tableLookup = function( tableObj, name, index, defVal, retObj ) {
...
//		    log('tableLookup found tableObj '+tableObj.table[0]+' name '+name+' with a standard default value of '+tableObj[name].defaultVal);
			let property = (retObj === true) ? null : ((retObj === false) ? tableObj[name].property :  retObj);
			defVal = (defVal===false) ? (undefined) : ((defVal===true) ? tableObj[name].defaultVal : defVal);
			...
			if (index>=0) {
				...
				if (index<sortOrderKeys.length && _.has(attrs,sortOrderKeys[index])) {
					if (_.isUndefined(property) || _.isNull(property) || !property || !property[1] || _.isUndefined(attrs[sortOrderKeys[index]])) {
					    log('tableLookup table '+name+' undefined property or object')
...
//		log('handleSecondWeapon buildCall:'+buildCall);
	...
	**/
	...
	var handleInitRW = function( charType, charCS, args ) {
		...
		speedMult = Math.max(parseFloat(attrLookup( charCS, fields.initMultiplier ) || 1), 1);
		attackNum = (attrLookup( charCS, fields.RW_noAttks, fields.RW_table, refIndex ) || 1);
		weapSpecial = attrLookup( charCS, fields.WP_specialist, fields.WP_table, (rowIndex-2) );
    ...
    */
	...
	var handleInitMIBag = function( charCS, args ) {
		...
		itemName = attrLookup( charCS, fields.MI_name, fields.MI_table, rowIndex );
		itemSpeed = attrLookup( charCS, fields.MI_speed, fields.MI_table, rowIndex );
	...
	**/
	...
	var handleAllWeapons = function( senderId, charCS, args, base, onlyDancing ) {
		...
		WeaponTable = getTable( charCS, {}, 'MW_name', fields.MW_table, fields.MW_name );
		WeaponTable = getTable( charCS, WeaponTable, 'MW_speed', fields.MW_table, fields.MW_speed );
		WeaponTable = getTable( charCS, WeaponTable, 'MW_noAttks', fields.MW_table, fields.MW_noAttks );
		WeaponTable = getTable( charCS, WeaponTable, 'MW_dancing', fields.MW_table, fields.MW_dancing );
		
		log('handleAllWeapons about to loop on Melee Weapons');
			
		do {
			weapon = tableLookup( WeaponTable, 'MW_name', entry, false );
			dancing = tableLookup( WeaponTable, 'MW_dancing', entry );
    		log('handleAllWeapons found weapon '+weapon+', dancing='+dancing);
			
			if (_.isUndefined(weapon)) {break;}
			if (weapon != '-' && (!onlyDancing || dancing != 0)) {
				weapons.push(weapon);
				speed = parseInt(tableLookup( WeaponTable, 'MW_speed', entry, '0' ));
				actionNum = tableLookup( WeaponTable, 'MW_noAttks', entry, '1' );
		...
		WeaponTable = getTable( charCS, {}, 'RW_name', fields.RW_table, fields.RW_name );
		WeaponTable = getTable( charCS, WeaponTable, 'RW_speed', fields.RW_table, fields.RW_speed );
		WeaponTable = getTable( charCS, WeaponTable, 'RW_noAttks', fields.RW_table, fields.RW_noAttks );
		WeaponTable = getTable( charCS, WeaponTable, 'RW_dancing', fields.RW_table, fields.RW_dancing );
		entry = fields.RW_table[1];
		
		log('handleAllWeapons about to start Ranged Weapon loop');

		do {
			weapon = tableLookup( WeaponTable, 'RW_name', entry, false );
			dancing = tableLookup( WeaponTable, 'RW_dancing', entry );
    		log('handleAllWeapons found weapon '+weapon+', dancing='+dancing);
			if (_.isUndefined(weapon)) {break;}
			if (weapon != '-' && !weapons.includes(weapon) && (!onlyDancing || dancing != 0)) {

				speed = parseInt(tableLookup( WeaponTable, 'RW_speed', entry, '0' ));
				actionNum = tableLookup( WeaponTable, 'RW_noAttks', entry, '1' );
				actions = eval(actionNum+'*2*'+speedMult);
				initiative = base+speed+init_Mod;
		log('handleAllWeapons pushing init:'+initiative+', action: with their '+(dancing ? 'dancing ' : '')+weapon+', msg: rate '+actionNum+', speed '+speed+', modifier '+init_Mod);
				attacks.push({init:initiative,ignore:0,action:('with their '+(dancing ? 'dancing ' : '')+weapon),msg:(' rate '+actionNum+', speed '+speed+', modifier '+init_Mod)});
...
//			}
		...
		base = actions[0].init;
//		log('handleInitSubmit base is '+base);

		content += ' --addtotracker '+tokenName+'|'+tokenID+'|'+base+'|'+actions[0].ignore+'|'+actions[0].action+'|'+actions[0].msg;
		actions.shift();
		_.each( actions, function(act) {
		    if (_.isUndefined(act.init)) {return;}
			content += ' --addtotracker '+tokenName+'|'+tokenID+'|'+(act.init-base)+'|'+act.ignore+'|'+act.action+'|'+act.msg;
		});
//		log('handleInitSubmit actions.length+1 is '+(actions.length+1));
		content += ' --removefromtracker '+tokenName+'|'+tokenID+'|'+(actions.length+1);
...
*/
	...
	var checkForMIs = function( charCS ) {
		...
		var MagicItems = getTable( charCS, {}, 'MI_name', fields.MI_table, fields.MI_name ),
			i = fields.MI_table[1],
			item;

		while (!_.isUndefined(item = tableLookup( MagicItems, 'MI_name', i++ ))) {
	...
	**/
	...
	var makeWeaponButtons = function( tokenID, charButton, submitted, MWcmd, RWcmd, show2H, showDancing, showInHand ) {
		...
		var charCS = getCharacter( tokenID ),
			...
			WeaponTable = getTable( charCS, {}, 'MW_name', fields.MW_table, fields.MW_name );
			WeaponTable = getTable( charCS, WeaponTable, 'MW_twoHanded', fields.MW_table, fields.MW_twoHanded );
			WeaponTable = getTable( charCS, WeaponTable, 'MW_dancing', fields.MW_table, fields.MW_dancing );
		
		a = fields.MW_table[1];
		for (i = a; i < (fields.MWrows + a); i++) {
			w = (1 - (a * 2)) + (i * 2);
			weapName = tableLookup( WeaponTable, 'MW_name', i, false );
...
//			log('makeWeaponButtons name='+weapName+', 2H='+tableLookup( WeaponTable, 'MW_twoHanded', i )+', dancing='+tableLookup(WeaponTable, 'MW_dancing', i ));
			twoHanded = tableLookup( WeaponTable, 'MW_twoHanded', i ) != 0;
			dancing = tableLookup(WeaponTable, 'MW_dancing', i ) != 0;
...
//				log('makeWeaponButtons adding '+weapName+' to dancingWeapons');
		...
		WeaponTable = getTable( charCS, {}, 'RW_name', fields.RW_table, fields.RW_name );
		WeaponTable = getTable( charCS, WeaponTable, 'RW_twoHanded', fields.RW_table, fields.RW_twoHanded, '', 1 );
		WeaponTable = getTable( charCS, WeaponTable, 'RW_dancing', fields.RW_table, fields.RW_dancing, '', 0 );

		a = fields.RW_table[1];
		for (i = a; i < (fields.RWrows + a); i++) {
			w = (2 - (a * 2)) + (i * 2);
			weapName = tableLookup( WeaponTable, 'RW_name', i );
			if (_.isUndefined(weapName)) {break;}
//			log('makeWeaponButtons name='+weapName+', 2H='+tableLookup( WeaponTable, 'RW_twoHanded', i )+', dancing='+tableLookup(WeaponTable, 'RW_dancing', i ));
			twoHanded = tableLookup( WeaponTable, 'RW_twoHanded', i ) != 0;
			dancing = tableLookup(WeaponTable, 'RW_dancing', i ) != 0;
			if (showInHand && weapName != '-' && (show2H || !twoHanded) && !dancing) {
			    if (header) {
    ...
    */
	...
	var makeSecondWeaponMenu = function(charCS,submitted,args) {
        ...
        var menu = args[0],
			...
			rowCount,
			WeaponTable = getTable( charCS, {}, 'MW_name', fields.MW_table, fields.MW_name );
			WeaponTable = getTable( charCS, WeaponTable, 'MW_twoHanded', fields.MW_table, fields.MW_twoHanded, '', 0 );
			WeaponTable = getTable( charCS, WeaponTable, 'MW_dancing', fields.MW_table, fields.MW_dancing, '', 0 );
		...
		for (i = a; i < (fields.MWrows + a); i++) {
			w = (1 - (a * 2)) + (i * 2);
			weapName = tableLookup( WeaponTable, 'MW_name', i );
			if (_.isUndefined(weapName)) {break;}
			twoHanded = tableLookup( WeaponTable, 'MW_twoHanded', i ) != 0;
			dancing = tableLookup(WeaponTable, 'MW_dancing', i ) != 0;
...
//				content += attrLookup( charCS, [ammoPointer + fields.Ammo_qty[0], fields.Ammo_qty[1]] ) + ' x ' + weapName;
    ...
    */
	...
	var makeMIBagMenu = function( charCS, submitted, args ) {
		...
		rowAdj = fields.MI_table[1];
		for (r = rowAdj; r < (fields.MIRows + rowAdj); r++) {
			miName = attrLookup( charCS, fields.MI_name, fields.MI_table, r );
