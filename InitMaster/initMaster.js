...
var initMaster = (function() {
	...
	InitMaster_Help:	{name:'InitiativeMaster Help',
						 ...
						 bio:'<div style="font-weight: bold; text-align: center; border-bottom: 2px solid black;">'
							+'<span style="font-weight: bold; font-size: 125%">InitiativeMaster Help v2.03</span>'
							+'</div>'
...
/*		// build the in-hand Magic Item Powers list
		...
		for (let r = miTable.table[1]; !_.isUndefined(miName = miTable.tableLookup( fields.Magic_name, r, false )); r++) {
			if (miName != '-') {
				if (!inHandTitle) {
				...
				content += miName;
				content += (((buttonID == charButton) || submitted) ? '</span>' : '](!init --button ' + cmd + '|' + tokenID + '|' + buttonID + '|' + r + ')');
			}
			buttonID++;
		}
		if (inHandTitle) {
...
//		weaponButtons = makeWeaponButtons( tokenID, -1, false, BT.MELEE, BT.RANGED, false, false );
		...
		[inHandMIs,buttonID] = inHandMIbuttons( tokenID, charCS, 0, (isMIattk ? charButton : -1), submitted, BT.MI_ATTACK );
		content += inHandMIs;
    ...
    */
	...
	var makeMIBagMenu = function( charCS, submitted, args, senderId ) {
		...
		[inHandMIs,buttonID] = inHandMIbuttons( tokenID, charCS, buttonID, charButton, submitted, BT.MI_POWER );
		content += inHandMIs;
