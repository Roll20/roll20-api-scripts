 ...
 * v1.016  18/02/2021  Changed spell intiative menu to display the correct number of spells
		...
		for (l = 1; l <= (isMU ? 9 : 7); l++) {
			...
			while (levelSpells[l].spells > 0) {
				...
				for (w = 1; (w <= fields.SpellsCols) && (levelSpells[l].spells > 0); w++) {
					...
					content += (((buttonID == spellButton) || submitted) ? '</span>' : '](!init --button ' + (isMU ? BT.MU_SPELL : BT.PR_SPELL) + '|' + tokenID + '|' + buttonID + '|' + r + '|' + c + ')');
					buttonID++;
					c++;
