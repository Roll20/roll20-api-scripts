...
var initMaster = (function() {
	...
	InitMaster_Help:	{name:'InitiativeMaster Help',
						 ...
						 bio:'<div style="font-weight: bold; text-align: center; border-bottom: 2px solid black;">'
							...
							+'<h4>and later</h4>'
							+'<p><span style='+design.selected_button+'>New:</span> Faster initiatives with pre-selected attacks</p><br>'
...
//				}
				if (!_.isUndefined(ref)) return [((ref%2) > 0 ? ((ref-(1+(fields.MW_table[1]*2)))/2) : ((ref-(2+(fields.RW_table[1]*2)))/2)),ref];
...
//							log('getRef: found name '+miName+' at ranged row '+index+', ref '+ref);
			...
			if (weapCount.melee + weapCount.ranged + (shieldStyle ? weapCount.shield : 0)) {
				[primeHand,primeRef] = getRef(charCS,0);
				[offHand,offRef] = getRef(charCS,1);
				[bothHands,bothRef] = getRef(charCS,2);
			}
//			log('makeWeaponMenu: getRef has returned offHand = '+offHand+', and offRef = '+offRef);
