 ...
 *                     sheet. 
 * v4.0.2  01/04/2025  Fixed error with makeCheckInitMenu() when dealing with an item with no specs.
 * v5.0.0  28/05/2025  Initiative [All Weapons] is now available for a mix of creature innate and melee
 ...
 * v5.4.0  31/05/2026  Added called shot initiative options, including parrying.
 * v5.4.1  17/06/2026  Added the --surprise command and rolls
 * v5.4.2  19/07/2026  Added one-off conditional attack modifiers. Replaced use of attrLookup() for
 *                     table lookup with table management functions to support attribute cache.
 */
...
const initMaster = (function() {		// eslint-disable-line no-unused-vars
	...
	const fetchTickMods = (...a) => libRPGMaster.fetchTickMods(...a);
	const assessItem = (...a) => libRPGMaster.assessItem(...a);
	...
	InitMaster_Help:	{name:'InitiativeMaster Help',
						 ...
						 bio:'<div style="font-weight: bold; text-align: center; border-bottom: 2px solid black;">'
							+'<span style="font-weight: bold; font-size: 125%">InitiativeMaster Help v2.10</span>'
							+'</div>'
							...
							+'<ul>'
							+'<li><mark style="color:green">New</mark> Initiative for Called Shots.</li>'
							+'<li><mark style="color:green">New</mark> Surprise rolls and modifiers.</li>'
							+'<li><mark style="color:green">New</mark> --turn command to support multi-AC creatures and Called Shots.</li>'
							+'</ul>'
							...
							+'<p>"Standard" initiative just requires a "Party" initiative dice roll and a "Foe" initiative dice roll to be entered, and the Turn Order entries are set appropriately.  For "Group" initiative, the same rolls are entered but, in addition, each character / NPC / creature (each token) taking part specifies what actions they are going to perform that round and the speed of that action is added to the relevant group dice roll to create the Turn Order priority for that token.  For "Individual" initiative, each character / NPC / creature makes its own individual dice roll as well as specifying their action, with the individual dice roll and speed of action being combined to give the Turn Order priority.</p>'
							+'<p>Alternatively, standard Roll20 functionality can be used to "right click" on a token and choose the option to add it to the Turn Order, and the <b>--maint</b> command can be used to set the "Stop Melee" button, thus stopping the Turn Order from being cleared each round, and then the Initiative will just cycle round the party members.</p>'
							...
							+'<p>The system can take into account various modifiers applied by spells and/or magic items (e.g. Haste and Slow spells), and the spell, power & magic item macros provided with the <b>MagicMaster API</b> use this functionality when used in conjunction with <b>RoundMaster</b> <i>Effects</i>. Mods set, amended or cleared with the provided commands will be stored on the character sheet and can be examined by the player alongside a tag describing each initiative mod using the [Check Initiative Modifiers] button on the activities dialog (group & individual initiative only) or the <b>!init --checkinit</b> command.</p>'
							
							+'<h3><mark style="color:green">New</mark> Surprise</h3>'
							+'<p>Before undertaking Initiative rolls, surprise by one group or individual or another should be considered.  Each group in an encounter should roll 1d10, and if they roll a 3 or less they are surprised by the other group. Certain races, classes and creatures might be better or worse at surprise, sometimes only under certain circumstances. The most common example is a character\'s or NPC\'s <i>Reaction Adjustment</i>.</p>'
							+'<p>InitiativeMaster provides the means to specify the two groups of tokens. Shift-select a number of tokens or drag a selection box to identify a group - or don\'t have any tokens selected and the party named on the initiative dialog (using the <b>!init --init</b> command) will automatically be used. Then use the DM\'s [Surprise] button or the <b>!init --surprise</b> command. When the dialog appears select the other group (or none to auto-select the party if not done before) and choose who is doing the surprising first (the reverse will be possible later). The optional modifiers will then be presented and can be selected or deselected. Then surprise can be rolled for. Each member of each group will be assessed: in the case of the <i>surprised</i> group, the very best member of the group will be chosen (assumes they shout out or otherwise warn the rest of the group). In the case of the group attempting surprise, the worst member is selected (assumes they are most likely to give the surprise away). If surprised, dexterity AC bonuses will be ignored for one round for all members of the surprised group.</p>'
							+'<p>Once this cycle is complete for one group surprising the other (successful or otherwise), a button will be presented to do the check the other way round.</p>'

							+'<h3><mark style="color:green">New</mark> Called Shots</h3>'
							+'<p>When using "Group" or "Individual" initiative and specifying an Attack initiative option, a new button will be shown in the weapon dialog labeled "Called Shot [Select Shot]". When using "Standard" initiative, this button appears when using the DM\'s or a token\'s <i>Initiative Menu</i>. Clicking this button asks for another NPC/creature to be targeted, then presents a new dialog with a choice of Called Shots relevant for that targeted character / NPC / creature. Selecting an option can apply adjustments to the Initiative Roll (Group & Individual initiative only) and the Attack Roll, defaulting to -1 and -4 respectively. The options will always include <i>Other</i> and <i>None</i>. <i>None</i> will cancel any previously selected called shot. <i>Other</i> will prompt for the name of a bespoke called shot (agree with the DM before using). If using <I>The Complete Fighter\'s Handbook</i> Parrying rules (see below), <i>Parry</i> (which does not suffer penalties) will prompt for a parrying maneuver when the targeted creature has their turn (this can be ignored if so desired).</p>'
							+'<p>If the target can use weapons or hold items, two additional options appear - <i>Disarm</i> and <i>Smash Held Item</i>. Other creatures might have other possible called shots. The possible called shots might each have different armour classes (AC). If specified in the creature definition, the respective AC will be used when a called shot is enacted. Even if not using a targeted attack, the AC of the creature will be adjusted <i>for this attack only</i> to be the appropriate AC for the body part. Even if not calling a shot, a random chance can be specified in the creature definition that a vulnerable area might be hit. See the <i>Race & Creature Database Help</i> handout for details of specifying called shots and body part AC for NPCs and Creatures.</p>'
							+'<p>Three Initiative API ommands exist to support called shots - if using all RPGMaster API facilities these commands may not be needed as API calls are managed internally by the APIs. --call-shot will set a specific called shot for one token against another. --turn is executed each time the turn moves to a new token. --isRound is used to signify the end of the round.  If not using RoundMaster for turn order management, these commands must be used manually for called shots to work.</p>'
							
							+'<h3><mark style="color:green">New</mark> Parrying</h3>'
							+'<p>The RPGMaster APIs support Parrying using either the rules in the DMG, page 61, or alternatively using the rules in <i>The Complete Fighter\'s Handbook</i>, by selecting the appropriate Parry option in the RPGM configuration dialog using the GM Config macro button (or the <b>!attk --config</b> command).</p>'
							+'<p>If using the DMG rules, a [Parry] option button will appear on the Initiative dialog when undertaking <i>Group</i> or <i>Individual</i> initiative. If using <i>Standard</i> initiative Parry is presented as an option in the [Called Shot] list. In either case the creature\'s AC will be adjusted appropriately for the next round.</p>'
							+'<p>If using <i>The Complete Fighter\'s Handbook</i> rules, Parry appears as a Called Shot option on the Attack initiative dialog. Selecting a Called Shot will first prompt for the token to parry against, then select to Parry (or other Called Shot), then the weapon to parry with as the attack initiative. During the turn, the player will be prompted when to perform the parry manouver.</p>'
							...
							+'<pre><mark style="color:green">New</mark> --surprise<br>'
							+'--weapon [token-id]<br>'
							+'--monster [token-id]<br>'
							+'--complex [token-id]<br>'
							+'--muspell [token-id]<br>'
							...
							+'--set-mods [token-id]|cmd|name|[=][+/-]mod|[=][+/-]mult|[SILENT]<br>'
							+'<mark style="color:green">New</mark> --call-shot token-id|target-id|[shot name]</pre>'
							+'<h3>4. Maintain the Turn Order and Rounds</h3>'
							+'<pre>--maint<br>'
							+'--check-tracker<br>'
							+'<mark style="color:green">New</mark> --turn prev-id|next-id<br>'
							+'<mark style="color:green">New</mark> --isRound round-no|[TRUE/FALSE]<br>'
							...
							+'<h4><mark style="color:green">New</mark> 3.1 Perform Surprise Rolls</h4>'
							+'<pre>--surprise</pre>'
							+'<p>Does not take any arguments but optionally select none, one or more tokens before executing the command as the first group, then once the dialog is displayed select none, one or more tokens as the second group before clicking the button to choose who is attempting to surprise who. In either case, if no tokens are selected the whole party as defined by the Initiative Dialog (using the <b>!init --init</b> command) will be used as one or other group.</p>'
							+'<p>Displays the first dialog in a sequence to specify two groups of tokens and the modifiers appropriate to the represented characters, NPCs and creatures, then roll the dice and mark as surprised as appropriate. See the <i>Class Database Help, Race & Creature Database Help, Item Database Help</i> and the <i>AttackMaster Help</i> handout section on the <b>--set-mods</b> command for details on specifying the optional surprise condition modifiers that are displayed.</p>'
							...
							+'<h4>3.11 Add, change or remove modifiers</h4>'
							...
							+'<h4><mark style="color:green">New</mark> 3.12 Set a Called Shot</h4>'
							+'<pre>--call-shot token-id|target-id|[shot name]</pre>'
							+'<p>Takes a mandatory token ID of the attacker, a mandatory targeted token ID for the victim, and an optional shot name.</p>'
							+'<p>The shot name can be one of the standard and given called shots for the targeted creature, or an invented called shot, or blank in which case a dialog will appear with the possible named shots available. <i>Parry, Other</i> and <i>None</i> are always available. To clear any existing called shot, use the appropriate token and target IDs and the shot name of <i>None</i>.</p>'
							+'<p>If the specified shot name exists as a named body part for the target, with a stated AC for that body part, the AC will be set when the token makes an attack (even if not targeted).</p>'
							+'<br>'
							+'<h3>4. Maintain the Turn Order and Rounds</h3>'
							...
							+'<h4><mark style="color:green">New</mark> 4.3 Declair turn moving to next token</h4>'
							+'<pre>--turn [prev-id]|[next-id]</pre>'
							+'<p>Takes an optional token ID for the token who\'s turn has just ended, and an optional token ID for the token with the next turn.</p>'
							+'<p>Notifies the InitiativeMaster API of an advancing turn to reset previous called shots, set up any upcoming called shot, and calculate random chances of the next attack hitting a vulnerable spot if a specific shot has not been called. See the <i>Race & Creature Database</i> help handout for more details of random vulnerable shots.</p>'
							+'<p>This command is issued automatically by the RoundMaster API if installed and used with InitiativeMaster.</p>'
							+'<h4><mark style="color:green">New</mark> 4.4 New round</h4>'
							+'<pre>--isRound round-number|[TRUE/FALSE]</pre>'
							+'<p>Takes a mandatory round number and an optional <i>new round</i> flag.</p>'
							+'<p>Signals to InitiativeMaster that the last round has ended and a new round is starting. This resets all called shot variables. A new round number can be specified.</p>'
							+'<p>This command is issued automatically by the RoundMaster API if installed and used with InitiativeMaster.</p>'
							+'<h4>4.5 Change the list of characters in the Party</h4>'
	...
	const BT = Object.freeze({
		...
		ROLL_SURPRISE:	'ROLL_SURPRISE',
		SURPRISE_12:	'SURPRISE_12',
		SURPRISE_21:	'SURPRISE_21',
		SWITCH_SURPRISE:'SWITCH_SURPRISE',
	});
	...
	const Init_Messages = Object.freeze({
		...
		stdInit: '&{template:'+fields.warningTemplate+'} {{name=^^tname^^\'s\nInitiative}}{{desc=Currently, the game is running on Standard AD&D Initiative rules, so it is a Party initiative roll.  You do not need to select an action but you can make a [Called Shot](!init --call-shot ^^tid^^|&#64;{target|Select Target|token_id}||initMenu) for the selected token.}}',
	...
	const init = function() {
		try {
			...
			if (_.isUndefined(state.initMaster.shots))
				{state.initMaster.shots = [];}
			if (_.isUndefined(state.initMaster.randomShots))
				{state.initMaster.randomShots = [];}
			if (_.isUndefined(state.initMaster.parrying))
				{state.initMaster.parrying = [];}
	...
	/**
	 * Get the currently selected token ID, either from args[] or
	 * from the selected array.
	 */
	 
	const getTokenID = function( args, selected, n ) {
		if (!args) args = [];
		if (!args[n] && selected && selected.length) {
			args[n] = selected[n]._id;
		} else if (!args[n]) {
 ...
 // -------------------------------------------- utility functions ----------------------------------------------
	...
	var getPlayerCharList = function( page=false, monster=false ) {
		...
		nameList =  _.chain(filterObjs(function(obj) {
				...
				.sortBy('name')
				.value();
		measureTime('filterObjs (getPlayerCharList)',start);
		return nameList;
	}
	
	/*
	 * Get a list of ids for members of the Party
	 * as defined by !init --init
	 */
	
	const getParty = function( ) {
		let pageID = Campaign().get('playerpageid'),
			charList = state.initMaster.playerChars.map(c => c.name.dbName()),
			charCS, charID;
		const start = Date.now();
		let list = filterObjs( obj => {
			if (obj.get('_type') !== 'graphic' || obj.get('_subtype') !== 'token') return false;
			if (obj.get('_pageid') !== pageID) return false;
			charID = obj.get('represents');
			if (!charID) return false;
			charCS = getObj('character',charID);
			if (!charCS) return false;
			return charList.includes(charCS.get('name').dbName());
		});
		measureTime('filterObjs',start);
	...
	**/
	...
	const buildWeaponArgs = function( tokenID, charCS, cmd, weaponRef ) {
		...
		let attackNum = (WeaponTables.tableLookup( attrs.noAttks, weaponRef ) || 1),
			shot = attrLookup( charCS, [fields.ShotCalled[0]+tokenID,fields.ShotCalled[1],fields.ShotCalled[2]] ),
			attackCount, attacks;
			
		attackNum = (styleNum && styleNum != '0') ? '(('+attackNum+')+('+styleNum+'))' : attackNum;
		shot = String(shot).split(':');
		const calledShot = (!shot || !shot.length || !shot[0].length) ? '' : (shot[0].toLowerCase() !== 'parry' ? ('doing a Called Shot of '+shot[0]) : 'Parrying');
		...
		return {name:weaponName, speed:weapSpeed, mult:speedMult, attkNum:attackNum, preInit:preInit, attks:attacks, twoHanded:twoHanded, type:weaponType, special:weapSpecial, shot:calledShot};
	}	
	...
	const checkForPowers = function( charCS ) {
		let PowersTable = [],
			item = {},
			r,c;
		PowersTable[0] = getTableField( charCS, {}, fields.Powers_table, fields.Powers_name, fields.PowersBaseCol );
		for (r = 0; !_.isUndefined(item); r++) {
			for (c = 0; c<fields.PowersCols && !_.isUndefined(item); c++) {
				if (_.isUndefined(PowersTable[c])) PowersTable[c] = getTableField( charCS, {}, fields.Powers_table, fields.Powers_name, fields.PowersBaseCol+c );
				item = PowersTable[c].tableLookup( fields.Powers_name, r, false );
...
//			monAttk3 = attrLookup( charCS, fields.Monster_dmg3 ),
	...
	**/
	...
	const handleInitMW = function( charType, charCS, args, senderId ) {
		...
		const weapArgs = buildWeaponArgs( tokenID, charCS, BT.MELEE, refIndex ),	
			  buildCall = '!init --buildMenu ' + senderId 
						+ '|' + (charType == CharSheet.MONSTER ? MenuType.COMPLEX : MenuType.WEAPON)
						+ '|' + tokenID
						+ '|' + rowIndex
						+ '|' + weapArgs.shot +' with their ' + weapArgs.name
						+ '|[[' + weapArgs.speed + ']]'
	...
	**/
	...
	const handlePrimeWeapon = function( charCS, args, senderId ) {
		...
		if (rowIndex > 0) {
			const weapArgs = buildWeaponArgs( tokenID, charCS, command, refIndex );	
	...
	**/
	...
	const handleSecondWeapon = function( charCS, args, senderId ) {
		...
		const weapArgs = buildWeaponArgs( tokenID, charCS, command, weaponRef ),	
			  buildCall = '!init --buildMenu ' + senderId 
						+ '|' + MenuType.MW_SECOND
						+ '|' + tokenID
						+ '|' + weapon
	...
	**/
	...
	const handleTwoWeapons = function( charCS, args, senderId ) {
		...
		const weapArgs = buildWeaponArgs( tokenID, charCS, ((rowIndex%2) > 0 ? BT.MELEE : BT.RANGED), refIndex );
		setInitVars( charCS, [MenuType.MW_PRIME,tokenID,rowIndex,(weapArgs.shot + ' with their '+weapArgs.name),evalAttr(weapArgs.speed,charCS),(weapArgs.mult+'*'+weapArgs.attkNum),weapArgs.preInit,'1','0',weapArgs.attks], 'current');
	...
	**/
	...
	const handleInitRW = function( charType, charCS, args, senderId ) {
		...
		let weapArgs = buildWeaponArgs( tokenID, charCS, BT.RANGED, refIndex ),
			  buildCall = '!init --buildMenu ' + senderId 
						+ '|' + (charType == CharSheet.MONSTER ? MenuType.COMPLEX : MenuType.WEAPON)
						+ '|' + tokenID
						+ '|' + rowIndex
	...
	**/
	...
	const handleInitSpell = function( spellCasterType, charCS, args, senderId ) {
		...
		let SpellsTable = getTableField( charCS, {}, fields.Spells_table, fields.Spells_name, colIndex );
			SpellsTable = getTableField( charCS, SpellsTable, fields.Spells_table, fields.Spells_speed, colIndex );
		const spellName = SpellsTable.tableLookup( fields.Spells_name, rowIndex ),
			  spellCastTime = (spellSpeedOverride || SpellsTable.tableLookup( fields.Spells_speed, rowIndex )),
			  buildCall = '!init --buildMenu ' + senderId 
    ...
    */
	...
	const handleInitPower = function( charCS, args, senderId ) {
		...
		let PowersTable = getTableField( charCS, {}, fields.Powers_table, fields.Powers_name, colIndex );
			PowersTable = getTableField( charCS, PowersTable, fields.Powers_table, fields.Powers_speed, colIndex );
		const powerName = PowersTable.tableLookup( fields.Powers_name, rowIndex ),
			  powerCastTime = PowersTable.tableLookup( fields.Powers_speed, rowIndex ),
			  buildCall = '!init --buildMenu ' + senderId 
						+ '|' + MenuType.POWER
						+ '|' + tokenID
						+ '|' + charButton
    ...
    */
	...
	const handleInitMIpower = function( charCS, args, senderId ) {
		...
		let MagicTable = getTableField( charCS, {}, fields.Magic_table, fields.Magic_name );
			MagicTable = getTableField( charCS, MagicTable, fields.Magic_table, fields.Magic_speed );
		const powerName = MagicTable.tableLookup( fields.Magic_name, rowIndex ),
			  powerSpeed = MagicTable.tableLookup( fields.Magic_speed, rowIndex ),
			  buildCall = '!init --buildMenu ' + senderId 
	...
	**/
	...
	const handleOtherActions = function( charCS, args, senderId ) {
		...
		sendAPI( buildCall, senderId );
		if ((otherAction || '').toLowerCase().includes('parry')) setAttr( charCS, [fields.ShotCalled[0]+tokenID,fields.ShotCalled[1],fields.ShotCalled[2]], 'Parry:0:0' );
		return;
	}
	...
	**/
	...
	const handleInitSubmit = function( senderId, charCS, args ) {
		...
		const initSubmit = function( senderId, charCS, args ) {
			...
			let shot = String(attrLookup( charCS, [fields.ShotCalled[0]+tokenID,fields.ShotCalled[1],fields.ShotCalled[2]] )),
				shotData = attrLookup( charCS, fields.CalledShots ).split('/').map(cs => cs.split(':')),
				diceVal = shotData.reduce((a,b) => a+parseInt(b[4] || 0),0);
				
			if (diceVal > 0) state.initMaster.randomShots.push(tokenID);
			shot = shot.split(':');
			if (shot && shot[0].toLowerCase() === 'parry') {
				state.initMaster.parrying.push([tokenID,attrLookup(charCS,[fields.CalledTarget[0]+tokenID,fields.CalledTarget[1],fields.CalledTarget[2]])]);
				if (state.attackMaster.parry){
					const classObj = classObjects(charCS,senderId),
						  warrior = classObj.some( c => c.base === 'warrior' ),
						  acAdj = Math.ceil((characterLevel(charCS)+(!warrior ? 0 : 1))/2); 
					sendAPI( (fields.attackMaster+' --set-mods '+tokenID+'|add|Parry|Action|ac+:'+acAdj+'||0|silent'), senderId );
					state.initMaster.shots.push(tokenID);
				};
				setAttr( charCS, fields.ACnotes, ('[Parry (' + (state.attackMaster.parry ? 'DMG' : 'CFH') + ')] ' + attrLookup( charCS, fields.ACnotes )));
			};
			...
			if ((rowIndex == 0 || rowIndex == -2) && (initMenu == MenuType.COMPLEX || initMenu == MenuType.SIMPLE || initMenu == MenuType.WEAPON)) {
				...
				const monAttk1 = (attrLookup( charCS, fields.Monster_dmg1 ) || '').split(','),
					  ...
					  calledShot = (!shot || !shot.length || !shot[0].length) ? '' : (shot[0].toLowerCase() !== 'parry' ? ('doing a Called Shot of '+shot[0]+' ') : 'Parrying ');
				...
				if (monAttk1[0].length && (rowIndex == -2 || rowIndex2 == 0 || rowIndex2 == 1)) {
					...
					if (init_attacks >= 1) actions.push({init:(base+Math.round(monSpeed1)+monMod),ignore:0,action:(calledShot+'with their '+monDmg1),msg:(' rate '+init_Mult+', speed '+Math.round(monSpeed1)+' modifier '+monMod)});
				...
				if (monAttk2[0].length && (rowIndex == -2 || rowIndex2 == 0 || rowIndex2 == 2)) {
					init_attacks = init_attacks + init_Mult - Math.floor(init_attacks);
					if (init_attacks >= 1) actions.push({init:(base+Math.round(monSpeed2)+monMod),ignore:0,action:(calledShot+'with their '+monDmg2),msg:(' rate '+init_Mult+', speed '+Math.round(monSpeed2)+' modifier '+monMod)});
				...
				if (monAttk3[0].length && (rowIndex == -2 || rowIndex2 == 0 || rowIndex2 == 3)) {
					init_attacks = init_attacks + init_Mult - Math.floor(init_attacks);
					if (init_attacks >= 1) actions.push({init:(base+Math.round(monSpeed3)+monMod),ignore:0,action:(calledShot+'with their '+monDmg3),msg:(' rate '+init_Mult+', speed '+Math.round(monSpeed3)+' modifier '+monMod)});
	...
	/*
	 * Select a called shot for this initiative roll.
	 */
	
	const handleCallShot = function( args, selected, senderId ) {
		
		const tokenID = args[1],
			  targetID = args[2],
			  shot = args[3] || '',
			  menu = (args[4] || 'weap').toLowerCase(),
			  charCS = getCharacter( tokenID ),
			  targetCS = getCharacter( targetID ),
			  isGM = playerIsGM( senderId );
			  
		if (!charCS || !targetCS || tokenID === targetID) {
			sendFeedback( '&{template:'+fields.warningTemplate+'}{{title=Invalid token selected}}{{desc=An invalid token was selected either to make the Called Shot, or to be the target for the Called Shot}}' );
			return;
		}
		if (!shot || !shot.length) {
			state.initMaster.parrying = state.initMaster.parrying.filter(p => (p[0] !== tokenID));
			
			let shotList = attrLookup( targetCS, fields.SavedShots, {def:''} ),
				content = '&{template:'+fields.defaultTemplate+'}{{title=Select a Called Shot}}'
						+ '{{desc=Choose from the following available Called Shots:\n';
						
			if (csVer(targetCS) < 5.4 && (!shotList || !shotList.length)) {
				setAttr( targetCS, fields.msVersion, '5.4' );
				let race = attrLookup( targetCS, fields.Race );
				shotList = resolveData( race, fields.RaceDB, reClassRaceData, targetCS, {cattr:reClassSpecs.cattr} ).attrs.shots;
				if (shotList && shotList.length) {
					setAttr( targetCS, fields.SavedShots, shotList );
					setAttr( targetCS, fields.CalledShots, shotList );
				}
			} else {
				shotList = attrLookup( targetCS, fields.SavedShots );
			};
			
			shotList = shotList.split('/').map(cs => cs.split(':')[0]);

			for (const aShot of shotList) {
				if (!aShot.length || (!isGM && aShot.toLowerCase().startsWith('ac'))) continue;
				content += '['+aShot+'](!init --button '+BT.CALLED_SHOT+'|'+tokenID+'|'+targetID+'|'+aShot+'|'+menu+')';
			};
			content += ((state.attackMaster.parry && (state.initMaster.initType !== 'standard')) ? '' : '[Parry](!init --button '+BT.CALLED_SHOT+'|'+tokenID+'|'+targetID+'|Parry|'+menu+')')
					+  '[Other](!init --button '+BT.CALLED_SHOT+'|'+tokenID+'|'+targetID+'|?{State Called Shot name}|'+menu+')'
					+  '[None](!init --button '+BT.CALLED_SHOT+'|'+tokenID+'|'+targetID+'|None|'+menu+')}}'
					+  '{{hide1=These choices are those defined for this foe, defaulting to those stated in the DMG.'
					+  ' Selecting Other will ask for a name for the Called Shot.  Default and Other Called Shots '
					+  'incur a penalty of 1 to initiative and a penalty of 4 to To Hit roll. According to the '
					+  'DMG section on *Called Shots*, against a creature a called shot will only cause the normal '
					+  'amount of damage allowed the weapon. Attempts to blind, cripple, or maim will not succeed.\n'
					+  'If the RPGM Configuration option *Parry* is set to *CFH Rules* then use the Parry button '
					+  'to parry attacks during the round.}}';
			sendResponse( charCS, content, senderId );
			return;
		};
		
		let shotCalled = attrLookup( charCS, [fields.ShotCalled[0]+tokenID,fields.ShotCalled[1],fields.ShotCalled[2]] ).split(':');
		state.initMaster.parrying = state.initMaster.parrying.filter( p => p[0] !== tokenID );
		if (shotCalled[0].toLowerCase() === 'parry' && state.attackMaster.parry) sendAPI(fields.attackMaster+' --set-mods '+tokenID+'|DELSPELL|Parry|Action',senderId);
		if (shot === 'None') {
			if (menu !== 'weap') doTurnActions(['',tokenID],senderId);
			setAttr( charCS, [fields.ShotCalled[0]+tokenID,fields.ShotCalled[1],fields.ShotCalled[2]], '' );
			setAttr( charCS, [fields.CalledTarget[0]+tokenID,fields.CalledTarget[1],fields.CalledTarget[2]], '' );
			if (shotCalled[0] && shotCalled[0].length) {
				sendAPI(fields.initMaster+' --set-mods '+tokenID+'|DEL|Called Shot '+shotCalled[0]+'|||silent',senderId);
			};
		} else {
			let shotData = attrLookup( targetCS, fields.SavedShots ).split('/').map(cs => cs.split(':')).find(cs => cs[0] === shot);
			if (!shotData || !shotData.length) {
				shotData = (shot !== 'Parry') ? [shot,-1,-4] : [shot,0,0];
			}
			if (shot === 'Parry') {
				if (state.initMaster.initType === 'standard') {
					state.initMaster.parrying.push([tokenID,targetID]);
					if (state.attackMaster.parry) {
						const classObj = classObjects(charCS,senderId),
							  warrior = classObj.some( c => c.base === 'warrior' ),
							  acAdj = Math.ceil((characterLevel(charCS)+(!warrior ? 0 : 1))/2); 
			...
			setAttr( charCS, [fields.ShotCalled[0]+tokenID,fields.ShotCalled[1],fields.ShotCalled[2]], shotData.join(':') );
			setAttr( charCS, [fields.CalledTarget[0]+tokenID,fields.CalledTarget[1],fields.CalledTarget[2]], targetID );
			if (menu !== 'weap') {
				doTurnActions([tokenID,tokenID],senderId);
			} else if (state.initMaster.initType !== 'standard') {
				sendAPI(fields.initMaster+' --set-mods '+tokenID+'|DEL|Called Shot '+shotCalled[0]+'|||silent --set-mods '+tokenID+'|MOD|Called Shot '+shot+'|'+shotData[1]+'||silent',senderId);
			};
		};
		
		if (menu === 'attk') {
			sendAPI(fields.attackMaster+' --attk-hit '+tokenID);
		} else {;
			state.initMaster.shots = state.initMaster.shots.filter( s => s !== tokenID );
			if (shot !== 'None') state.initMaster.shots.push(tokenID);

			if (menu !== 'diceroll' && menu !== 'initmenu') {
				sendAPI(fields.initMaster+' --button ' + BT.ONEWEAPON + '|' + tokenID + '|-1|-1',senderId);
			} else if (menu !== 'initmenu') {
//				sendResponse(charCS,'&{template:'+fields.menuTemplate+'}{{title=Called Shot}}{{desc='+getObj('graphic',tokenID).get('name')+' has declaired a *Called Shot* of '+shot+'}}',senderId);
//			} else {
				doInitDiceRoll([],(getObj('graphic',tokenID).get('name')+' has declaired a ***Called Shot*** of '+shot),senderId);
			};
		};
		return;
	};
	...
	/*
	 * Handle a check for a token parrying an attack
	 */
	 
	const handleParryMsg = function( args, senderId ) {
		
		if (state.attackMaster.parry) return;
		
		const attackingID = args[1],
			  attkToken = getObj( 'graphic', attackingID ),
			  attkName = attkToken.get('name'),
			  parrying = state.initMaster.parrying.filter( p => !(p[1] !== attackingID));
			  
		for (let parryID of parrying) {
			let parryToken = getObj( 'graphic', parryID[0] );
			if (parryToken) {
				let parryName = parryToken.get('name');
				sendResponse( getCharacter(state.initMaster.parrying[0]), '&{template:'+fields.warningTemplate+'}{{title='+parryName+' Performs a Parry}}{{desc='+parryName+' has seleted to parry an attack by '+attkName+' as their move. Do this by attacking '+attkName+' now. If you succeed then you have parried the attack and take no damage}}');
				sendFeedback( '&{template:'+fields.messageTemplate+'}{{title='+parryName+' is Parrying}}{{desc='+parryName+' is performing a parry as their called shot against '+attkName+'}}');
			};
		};
	};
...
// ---------------------------------- build menus to display --------------------------------------------------------	
	...
	const otherActions = function( initMenu, tokenID, charButton, submitted ) {
		const dmgParry = state.attackMaster.parry,
			  content = (charButton == 101 ? '<span style=' + design.selected_button + '>' : (submitted ? '<span style=' + design.grey_button + '>' : '['))
					  + 'Move'
					  + ((charButton == 101 || submitted) ? '</span>' : '](!init --button ' + BT.OTHER + '|' + tokenID + '|101|' + initMenu + '|while moving|0)')
					  + (dmgParry ? (charButton == 105 ? '<span style=' + design.selected_button + '>' : (submitted ? '<span style=' + design.grey_button + '>' : '[')) : '')
					  + (dmgParry ? 'Parry' : '')
					  + (dmgParry ? ((charButton == 105 || submitted) ? '</span>' : '](!init --button ' + BT.OTHER + '|' + tokenID + '|105|' + initMenu + '|while parrying|0)') : '')
						...
						for (let i = a; i < (fields.MWrows + a); i++) {
							...
							if (showInHand && (weapName != '-') && (show2H || !twoHanded) && !dancing) {
								...
								let miName = WeaponTable.tableLookup( fields.MW_miName, i ) || '',
									InHand = getTableField( charCS, {}, fields.InHand_table, fields.InHand_index ),
									itemIndex = InHand.tableLookup( fields.InHand_index, WeaponTable.tableLookup( fields.MW_hand, i )),
    ...
    */
	...
	async function makeWeaponMenu(charCS,submitted,args,senderId) { 
		...
		try {
			...
			let shotCalled  = (attrLookup( charCS, [fields.ShotCalled[0]+tokenID,fields.ShotCalled[1],fields.ShotCalled[2]] ).split(':') || [''])[0];
			content += '{{Called Shot='
					+  (submitted ? ('<span style=' + design.grey_button + '>') : '[')
					+  (shotCalled || 'Select shot')
					+  (submitted ? '</span>' : ('](!init --button ' + BT.CALLED_SHOT + '|' + tokenID + '|&#64;{target|Select Target|token_id} )'))
					+  '}}';
			...
			for (l = 1; l < levelSpells.length; l++) {
				...
				for (let s = levelSpells[l].spells; s > 0; s--) {
					...
					for (w = 1; (w <= fields.SpellsCols) && (s > 0); w++) {
						if (_.isUndefined(SpellTable[w])) {
							SpellTable[w] = getTableField( charCS, {}, fields.Spells_table, fields.Spells_name, c );
							SpellTable[w] = getTableField( charCS, SpellTable[w], fields.Spells_table, fields.Spells_castValue, c );
						};
						spellName = SpellTable[w].tableLookup( fields.Spells_name, r, false );
						if (_.isUndefined(spellName)) {
							s = 0;
							break;
						}
						qty = parseInt( SpellTable[w].tableLookup( fields.Spells_castValue, r ) || 0);
		...
		try {
			...
			for (r = 0; r < fields.PowerRows; r++) {
				...
				for (w = 1; w <= fields.PowersCols; w++) {
					if (!_.isUndefined(PowersTable[w])) {
						PowersTable[w] = getTableField( charCS, {}, fields.Powers_table, fields.Powers_name, c );
						PowersTable[w] = getTableField( charCS, PowersTable[w], fields.Poers_table, fields.Powers_astValue, c );
					}
					qty = PowersTable[w].tableLookup( fields.Spells_castValue, r );
					powerName = PowersTable[w].tableLookup( fields.Spells_name, r, false );
    ...
    */
	...
	var makeCheckInitMenu = function( tokenID, charCS, senderId, silent=false, msg='', menu='' ) { //specs
		...
		content += '{{Totals=Mod of **'+(totalMod > 0 ? '+' : '')+totalMod+'** and Mult of **x'+totalMult+'**}}'
				...
				+  (menu && menu.length ? ('{{desc1=[Return to menu](!init --'+menu+' '+tokenID+')}}') : '');
	...
	/*
	 * Make a dialog to prompt the player to choose possible modifiers 
	 * to a surprise roll.
	 */
	
	const makeSurpriseMenu = function( args = [] ) {		// bestSurpriser
		
		const tokenID = args[0],
			  modWho = (args[1] || '').toLowerCase(),
			  modName = args[2] || '',
			  modToggle = args[3] != 'false';
		
		const tickBox = (who, title, toggle) => ('<a style= "width: 16px; height: 16px; border: none; background: none" title="'+title+'" href="!init --button '+BT.SURPRISE_MOD+'|'+theSurprisers[0]+'|'+who+'|'+title+'|'+!toggle+'">'+(toggle ? '\u2705' : '\u2B1C')+'</a>');
		...
		const table57mods = function() {
			surpriseMe = matchSurprise( surpriseMe, ['Fleeing',-2,false,'all'] );
			surpriseMe = matchSurprise( surpriseMe, ['In poor light',-1,false,'all'] );
			surpriseMe = matchSurprise( surpriseMe, ['In darkness',-4,false,'all'] );
			surpriseMe = matchSurprise( surpriseMe, ['Panicked',-2,false,'all'] );
			surpriseMe = matchSurprise( surpriseMe, ['Anticipating Attack',2,false,'all'] );
			surpriseMe = matchSurprise( surpriseMe, ['Suspicious',2,false,'all'] );
			surpriseMe = matchSurprise( surpriseMe, ['Raining',-1,false,'all'] );
			surpriseMe = matchSurprise( surpriseMe, ['Heavy Fog',-2,false,'all'] );
			surpriseMe = matchSurprise( surpriseMe, ['Extremely still',2,false,'all'] );

			surpriseYou = matchSurprise( surpriseYou, ['Distinctive Smell',-2,false,'all'] );
			surpriseYou = matchSurprise( surpriseYou, ['Lightly Camouflaged',1,false,'all'] );
			surpriseYou = matchSurprise( surpriseYou, ['Moderately Camouflaged',2,false,'all'] );
			surpriseYou = matchSurprise( surpriseYou, ['Heavily Camouflaged',3,false,'all'] );

			return;
		};
		
		const listMods = function( surprises ) {
			return _.chain( surprises )
					.groupBy( s => s[0] )
					.mapObject( s => s[0][2] )
					.pairs()
					.value();
		...
		const addTickBoxes = function( mods = [], who ) {
			let boxes = '';
			listMods(mods).forEach( mod => boxes += '<tr><td>'+mod[0]+'</td><td>'+tickBox(who,mod[0],mod[1])+'</td></tr>' );
			return boxes;
		};

		const getBest = function( ids, surprises, max=true ) {
			if (!surprises || !surprises.length) return {id:ids[0],val:0,mods:[],adv:0};
			let groupedMods = _.groupBy( surprises, s => s[3] ),
				mappedMods = _.mapObject( groupedMods, (s,k) => s.reduce((a,b) => a+(b[2] ? (parseInt(b[1]) || 0) : 0),0)),
				best;

			_.each( mappedMods, (s,k) => {
				if (k === 'all') return;
				s = s + mappedMods.all;
				let advantage = parseInt(((attrLookup( getCharacter(k), [fields.Magical_advantages[0]+k,fields.Magical_advantages[1]] )+'///////').split('/')[6]) || 0);
				s = s + (advantage > 0 ? 5 : (advantage < 0 ? -5 : 0));
				best = (_.isUndefined(best) || (max ? (best.val < s) : (best.val > s))) ? {id:k,val:s,adv:advantage} : best;
			});
			if (!best || !best.id) best = {id:ids[0],val:0,mods:[],adv:0};
			
			if (_.isUndefined(groupedMods[best.id])) groupedMods[best.id] = [];
			if (_.isUndefined(groupedMods.all)) groupedMods.all = [];
			if (!_.isUndefined(mappedMods.all)) {
				best.mods = (groupedMods[best.id] || []).concat(groupedMods.all).filter( m => !!m[2] );
			} else {
				best.mods = groupedMods[best.id].filter( m => !!m[2] );
			};
			best.val = best.val - (best.adv > 0 ? 5 : (best.adv < 0 ? -5 : 0));
			return best;
		};
		
		const makeSurpriseRoll = function(ability, roll, roll2) {
			
			const mod 		= (bestSurprised.val - worstSurpriser.val),
				  advantage = bestSurprised.adv - worstSurpriser.adv,
				  fudge 	= bestSurprised.mods.some( s => s[0].dbName() === 'fudgefactor' ),
				  impact	= advantage > 0 ? 'kh1' : (advantage < 0 ? 'kl1' : ''),
				  adTxt 	= advantage > 0 ? withAdv : (advantage < 0 ? withDis : ''),
				  surpriseRoll = impact ? ` ([[ { [[${roll}]],[[${roll2}]] }${impact}]][Dice Roll ${adTxt}])` : '([['+roll+']][Dice Roll])';
				  
			bestSurprised.mods = bestSurprised.mods.filter( s => s[0].dbName() !== 'fudgefactor' );
				  
			let content = '&{template:'+fields.menuTemplate+'}{{title=Surprise}}'
						+ '{{Roll=[[ '+surpriseRoll+' + ([['+mod+']][Modifier]) '+(!fudge ? '' : '+ ([[?{GM Adjustment - +ve benefits surprised group}]][Fudge Factor])')+' ]] surprised if 3 or less}}'
						+ '{{Result=Roll<=3}}{{Success=They are surprised}}{{Failure=They are not surprised}}';

			if (bestSurprised.mods.length || worstSurpriser.mods.length) {
				content += '{{desc=Applicable modifiers were}}';
				if (bestSurprised.mods.length) {
					let name = getObj('graphic',bestSurprised.id).get('name');
					content += '{{desc1=**From those being surprised most alert is &lt;span style="color:green"&gt;'+name+'&lt;/span&gt;**&lt;table&gt;';
					bestSurprised.mods.forEach( s => content += '&lt;tr&gt;&lt;td&gt;'+s[0]+'&lt;/td&gt;&lt;td&gt;'+s[1]+'&lt;/td&gt;&lt;/tr&gt;' );
					content += '&lt;/table&gt;}}';
				};
				if (worstSurpriser.mods.length) {
					let name = getObj('graphic',worstSurpriser.id).get('name');
					content += '{{desc2=**From the surprising token(s) least stealthy is &lt;span style="color:red"&gt;'+name+'&lt;/span&gt;**&lt;table&gt;';
					worstSurpriser.mods.forEach( s => content += '&lt;tr&gt;&lt;td&gt;'+s[0]+'&lt;/td&gt;&lt;td&gt;'+(0-s[1])+'&lt;/td&gt;&lt;/tr&gt;' );
					content += '&lt;/table&gt;}}';
				};
			} else {
				content += '}}'
			...
			const advContent = !impact ? (bestSurprised.adv > 0 ? 'Advantage on surprise on both sides' : ((bestSurprised.adv < 0) ? 'Disadvantage on surprise on both sides' : '')) : ('**'+((advantage > 0 ? 'Advantage' : 'Disadvantage')+'** surprise dice were $[[0]] & $[[1]]'));
			
			content += '{{successcmd='+fields.initMaster+' --nowaitmsg --button '+BT.ROLL_SURPRISE+'|'+worstSurpriser.id+'}}'
					+  '{{desc3='+advContent+'}}'
					+  '{{desc9=[Surprise Other Way Round](!init --nowaitmsg --button '+BT.SWITCH_SURPRISE+'|'+bestSurprised.id+')}}';
			
			setAbility( bestCS, 'Do-Not-Use-Surprise-'+ability, content );
			return;
		};
		...
		switch (modWho) {
		case 'list':
			surprisedNames = theSurprised.map( s => getObj('graphic',s).get('name') );
			surpriserNames = theSurprisers.map( s => getObj('graphic',s).get('name') );
			break;
		case 'table57':
			table57mods();
			break;
		case 'me':
			surpriseMe = surpriseMe.map( s => ((s[0].dbName() !== modName.dbName()) ? s : [s[0],s[1],(!!modToggle),s[3]] ));
			break;
		case 'you':
			surpriseYou = surpriseYou.map( s => ((s[0].dbName() !== modName.dbName()) ? s : [s[0],s[1],(!!modToggle),s[3]] ));
			break;
		default:
			surpriseMe = undefined;
			surpriseYou = undefined;
			theSurprisers.forEach( t => surpriseYou = fetchTickMods( t, 'surpriseyou', surpriseYou  ));
			theSurprised.forEach( t => surpriseMe = fetchTickMods( t, 'surpriseme', surpriseMe ));
			surpriseMe = matchSurprise( surpriseMe, ['Fudge Factor',0,false,'all'] );
			surpriseYou = matchSurprise( surpriseYou, ['Group Size',(0-Math.floor(theSurprisers.length/10)),true,'all'] );
		};
		
		const bestSurprised = getBest( theSurprised, surpriseMe, true ),
			  worstSurpriser = getBest( theSurprisers, surpriseYou, false ),
			  bestCS = getCharacter(bestSurprised.id);
			  
		makeSurpriseRoll( 'PC-Rolls', '1d10', '1d10' );
		makeSurpriseRoll( 'You-Roll', '?{Roll for Surprise|1d10}', '?{Roll again for Surprise|1d10}' );
		
		let content = '&{template:'+fields.dialogTemplate+'}{{title=Determine Surprise}}'
					+ '{{Section=Tick or untick each modifier that applies to this partiular surprise situation. _Show Groups_(!init --button '+BT.SURPRISE_MOD+'|'+bestSurprised.id+'|list)}}';
					
		content += '{{Section1=<u>Surprised Tokens</u>\n';
		if (modWho === 'list') content += '**'+surprisedNames.join(', ')+'**\n';
		content += '<table>'+addTickBoxes( surpriseMe, 'me' )+'</table>}}'
				+  '{{Section3=<u>Tokens Causing Surprise</u>\n'
		if (modWho === 'list') content += '**'+surpriserNames.join(', ')+'**\n';
		content += '<table>'+ addTickBoxes( surpriseYou, 'you' )+'</table>}}';
		
		content += '{{Section4=_Show DMG Table57 Surprise Mods_(!init --button SURPRISE_MOD|'+bestSurprised.id+'|table57)}}';

		content += '{{Section5=[PC Rolls](~'+bestCS.id+'|Do-Not-Use-Surprise-PC-Rolls) or [You Roll](~'+bestCS.id+'|Do-Not-Use-Surprise-You-Roll)}}';
...
//------------------------------------- do commands --------------------------------------------
	...
	*/
	...
	var doInitMenu = function( args, selected, initMenu, senderId ) {
		...
		if (!initSelection) initSelection = [];
		if (!initSelection[senderId]) initSelection[senderId] = [];
		if ((!initMenu || initMenu === MenuType.MENU) && !initSelection[senderId].length && !!selected && selected.length) initSelection[senderId] = Array.from(selected);
...
//			  monsterBaseAC = attrLookup( charCS, fields.Monster_baseAC, {def:false} ),
//			  shot = attrLookup( charCS, [fields.ShotCalled[0]+tokenID,fields.ShotCalled[1],fields.ShotCalled[2]] ).split(':')[0];
		...
		sendAPI(fields.attackMaster+' --check-ac '+tokenID+'|quiet',senderId);
		return;
	...
	/*
	 * Assess who the individuals or groups are that are
	 * involved in a surprise roll
	 */
	 
	const doSurprise = function( args = [], selection = [] ) {
		const tokenID = args[0],
//			  noPartyMsg = 'Party names not specified. Use GM\'s [Initiative Menu](!init --init) to add names to party list',
			  gotParty = !selection || !selection.length;

		theSurprised = !gotParty ? selection.map( s => s._id ) : getParty();
		
		let content = '&{template:'+fields.dialogTemplate+'}{{title=Surprise}}'
					+ '{{Section1=**Choose who is involved in surprise**\n'
					+ (gotParty ? 'The party has been selected as group 1.' : 'The previous selection is group 1.')
					+ ' Now '+(!gotParty ? 'either ' : '')+'select one or more tokens to be in group 2'+(!gotParty ? ' or leave none selected to make the party group 2' : '')+'.}}'
					+ '{{Section2=[G1 surprises G2]('+fields.initMaster+' --button '+BT.SURPRISE_12+'|'+theSurprised[0]+') or [G2 surprises G1]('+fields.initMaster+' --button '+BT.SURPRISE_21+'|'+theSurprised[0]+')}}'
					+ '{{desc=How to use surprise:}}{{hide1=Before doing surprise, select one or more tokens to be in group 1 (or if none selected defaults to the party).'
					+ ' Then do surprise using the GM macro button or the **!init --surprise** command. When you see this dialog, select one or more tokens to be in group 2 (or have none seleted for the party)'
					+ ' then click the button that defines which group attempts to surprise the other.}}';
		sendFeedback(content);
	};
	
	/*
	 * Make the lists of token IDs for those involved in surprise 
	 * and define who is attempting to surprise whom. Make sure
	 * no one is doing both!
	 */
	
	const doAssessSurprise = function( args = [], selection = [] ) {	
		const cmd = args[0],
			  tokenID = args[1],
			  gotParty = !selection || !selection.length,
			  noPartyMsg = 'Party not on current player map page, or Party names not specified. To specify Party members, use GM\'s [Initiative Menu](!init --init). To test surprise for tokens not on the current player map page, select one or more tokens for each group.';
			  
		if (cmd === BT.SWITCH_SURPRISE) {
			let theSwitch = theSurprised;
			theSurprised = theSurprisers;
			theSurprisers = theSwitch;
		} else {
			if (cmd !== BT.SURPRISE_21) {
				theSurprisers = Array.from(theSurprised);
				theSurprised = !gotParty ? selection.map( s => s._id ) : getParty();
			} else {
				theSurprisers = !gotParty ? selection.map( s => s._id ) : getParty();
			};
			if (!theSurprised || !theSurprised.length || !theSurprisers || !theSurprisers.length) {
				sendError(noPartyMsg);
				return;
			} else if (_.intersection(theSurprisers,theSurprised).length) {
				sendError('The surprised and the surprisers must be different. At least one token is both surprised and surpriser!');
				return;
			};
		};
		makeSurpriseMenu( args, selection );
		return;
	};
	...
	/*
	 * Set a surprise mod.  Surprise is short term and only ever
	 * lasts one round.
	 */
	
	const doMarkSurprised = function( args, senderId ) {
		
		let surpriserID = args[0],
			charCS;
			
		theSurprised.forEach( s => {
			charCS = getCharacter( s );
			log('doMarkSurprised: '+charCS.get('name')+' has dexterity ACbonus of '+attrLookup(charCS,fields.Dex_acBonus));
			sendAPI(fields.roundMaster + ' --addtargetstatus '+s+'|Surprised|1|-1|Frozen by surprise, lose AC & Save Dexterity bonus|interdiction|silent');
		});
	};

	/*
	 * If using Standard initiative, call here before the dice roll
	 * dialog, to reset any called shots.
	 */
	 
	const doResetStdInitShots = function() {		// senderId
		
		let charCS, shot, shotObj, shotData, acNotes, diceVal, monsterBaseAC;
		
		for (let shotID of state.initMaster.shots) {
			charCS = getCharacter(shotID);
			if (!charCS) continue;
			if (state.attackMaster.parry) {
				sendAPI(fields.attackMaster + ' --check-ac '+shotID+'|quiet');
			}
			acNotes = attrLookup( charCS, fields.ACnotes );
			if (acNotes.trim().startsWith('[Parry (')) setAttr( charCS, fields.ACnotes, acNotes.trim().slice(13) );
			shotObj = attrLookup( charCS, [fields.ShotCalled[0]+shotID,null] );
			if (!shotObj) continue;
			shot = (shotObj.get(fields.ShotCalled[1]) || '').split(':')[0];
			if (shot && shot.length) sendAPI('!init --set-mods '+shotID+'|del|Called Shot '+shot+'|||silent');
			shotObj.set({current:'',max:''});
			monsterBaseAC = attrLookup( charCS, fields.Monster_baseAC, {def:false} );
			if (monsterBaseAC && monsterBaseAC.length) {
				setAttr( charCS, fields.MonsterAC, monsterBaseAC );
				setAttr( charCS, fields.Monster_baseAC, '' );
			};
		};
		state.initMaster.shots = [];
		state.initMaster.randomShots = [];
		...
		if (state.initMaster.initType !== 'standard') return;
		
		const page = Campaign().get('playerpageid');
		filterObjs( obj => {
			if (obj.get('_type') !== 'graphic' || obj.get('_subtype') !== 'token' || obj.get('_pageid') !== page) return false;
			if (obj.get('isdrawing')) return false;
			charCS = getCharacter(obj.id);
			if (!charCS) return false;
			shotData = attrLookup( charCS, fields.CalledShots ).split('/').map(cs => cs.split(':'));
			diceVal = shotData.reduce((a,b) => a+parseInt(b[4] || 0),0);
			if (diceVal > 0) state.initMaster.randomShots.push(obj.id);
			return false;
		});
		return;
	};
	...
	const doInitDiceRoll = function( args, msg='', senderId ) {
		...
		let	content = '&{template:'+fields.menuTemplate+'}'
					...
					+ '}}{{section2=Choose Attack Initiative detail level\n'
					...
					+ '}}';
		if (state.initMaster.initType !== 'standard') {
			content +='{{Section3=Check that all Characters have specified what they are doing\n'
					+ '[Check Tracker Complete](!init --check-tracker roll|'+argStr+')}}';
		} else {
			content +='{{Section4=Enter a [Called Shot](!init --call-shot &#64;{selected|token_id}|&#64;{target|Select Target|token_id}||DiceRoll) for the\nselected token}}';
		};
		if (state.initMaster.initType !== 'individual') {
			state.initMaster.playerRoll = playerRoll;
			state.initMaster.dmRoll = dmRoll;
			args[2] = '';
			content +='{{Section5=Ask a Player to roll 1d10 and also roll 1d10 as DM, then enter the values below\n'
		...
		content += '{{Section9=['+(state.initMaster.dispRollOnInit ? ('<span style='+design.selected_button+'>Auto-displaying</span>') : 'Auto-display')+'](!init --init '+args[0]+'|'+args[1]+'|dispToggle) on new round}}';
	...
	/*
	 * Manage any actions required when the turn for a token occurs
	 */
	 
	var doTurnActions = function( args, senderId ) {
		
		const nextID = args[0],
			  prevID = args[1],
			  nextCS = getCharacter(nextID),
			  prevCS = getCharacter(prevID);
			  
		let	targetID, targetCS, targetBaseAC, targetAC,
			shot, shotData;
		
		if (prevCS) {
			targetID = attrLookup( prevCS, [fields.CalledTarget[0]+prevID,fields.CalledTarget[1],fields.CalledTarget[2]] );
			targetCS = getCharacter(targetID);
			if (targetCS) {
				targetBaseAC = attrLookup( targetCS, fields.Monster_baseAC );
				if (targetBaseAC && targetBaseAC.length) {
					setAttr( targetCS, fields.MonsterAC, targetBaseAC );
					setAttr( targetCS, fields.Monster_baseAC, '' );
					if (nextID !== prevID) sendAPI(fields.attackMaster+' --checkac '+targetID+'|quiet',senderId);
				};
			};
		};
		for (let shotID of state.initMaster.randomShots) {
			let shotCS = getCharacter(shotID);
			if (!shotCS) continue;
			targetAC = attrLookup( shotCS, fields.MonsterAC, {def:false} );
			if (_.isUndefined(targetAC) || !targetAC.length) continue;
			shotData = attrLookup( shotCS, fields.CalledShots ).split('/').map(cs => cs.split(':'));
			let diceVal = shotData.reduce((a,b) => a+parseInt(b[4] || 0),0);
			if (diceVal > 0) {
				let roll = randomInteger(diceVal);
				let row = shotData.find(cs => {roll = roll - (cs[4] || 0); return roll <= 0});
				if (!_.isUndefined(row) && !_.isUndefined(row[3]) && !isNaN(parseInt(row[3]))) {
					targetAC = targetAC.split('[');
					targetAC[0] = row[3];
					targetAC[1] = row[0]+']';
					setAttr( shotCS, fields.MonsterAC, targetAC.join('[') );
					sendAPI(fields.attackMaster+' --checkac '+shotID+'|quiet',senderId);
				};
			};
		};
		if (nextCS) {
			targetID = attrLookup( nextCS, [fields.CalledTarget[0]+nextID,fields.CalledTarget[1],fields.CalledTarget[2]] );
			targetCS = getCharacter(targetID);
			let shotData = attrLookup( nextCS, [fields.ShotCalled[0]+nextID,fields.ShotCalled[1],fields.ShotCalled[2]] ).split(':'),
				shot = shotData[0];
			if (shot && shot.length) {
				sendResponse( nextCS, '&{template:'+fields.menuTemplate+'}{{title=Called Shot}}{{desc='+getObj('graphic',nextID).get('name')+' has declaired a *Called Shot* of '+shot+' as their action}}', senderId);
				if (!_.isUndefined(shotData[3]) && String(shotData[3]).length) {
					let shotAC = shotData[3] || '',
						targetAC = attrLookup( targetCS, fields.MonsterAC, {def:false} );
					if (String(shotAC).length && !_.isUndefined(targetAC) && String(targetAC).length) {
						targetBaseAC = attrLookup( targetCS, fields.Monster_baseAC, {def:false} );
						if (_.isUndefined(targetBaseAC) || !targetBaseAC.length) setAttr( targetCS, fields.Monster_baseAC, targetAC );
						targetAC = targetAC.split('[');
						targetAC[0] = shotAC;
						targetAC[1] = shot+']';
						setAttr( targetCS, fields.MonsterAC, targetAC.join('[') );
						sendAPI(fields.attackMaster+' --checkac '+targetID+'|quiet',senderId);
					};
				};
			};
		};
...
//		}
		switch (handler.toUpperCase()) {
			...
			case BT.CALLED_SHOT :
				...
				// Handle selection of a called shot
				
				handleCallShot( args, selected, senderId );
				break;
				
			case BT.PARRY_MSG :
			
				// Handle detection of a parry
				
				handleParryMsg( args, senderId );
				break;
			...
			case BT.SURPRISE_MOD :
			
				// Handle the confirmation or otherwise of a surprise modifier
				
				args.shift();
				makeSurpriseMenu( args );
				break;
				
			case BT.ROLL_SURPRISE :
			
				// Roll the dice to assess if a group is surprised
				
				args.shift();
				doMarkSurprised( args, senderId );
				break;
				
			case BT.SURPRISE_12 :
			case BT.SURPRISE_21 :
			case BT.SWITCH_SURPRISE :
			
				// For two distinct selected groups determine which
				// is trying to surprise the other
				
				doAssessSurprise( args, selected );
				break;
...
// ------------------------------------- process messages from chat ------------------------------------	
	...
	const handleChatMessage = function(msg) {
		...
		const doInitCmd = function( e, selected, senderId ) {
			...
			try {
				if (!sendGMquery( 'init', arg, senderId )) {
					...
					case 'maint':
						...
						break;
					case 'surprise':
						if (isGM) doSurprise(arg,selected);
						break;
					case 'init-level':
						if (isGM) state.initMaster.weapInit = (arg[0].toLowerCase() == 'weapon');
					case 'init':
					...
					case 'isround':
						...
						break;
					case 'turn':
						sendWait(senderId,0,'Init --turn');
						doTurnActions(arg,senderId);
						break;
					case 'end-of-day':
			...
			if (!!state.initMaster.debug) reportTimes('!init --'+e);
		}
