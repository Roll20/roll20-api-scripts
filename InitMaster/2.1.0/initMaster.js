...
var initMaster = (function() {
	...
	InitMaster_Help:	{name:'InitiativeMaster Help',
						 ...
						 bio:'<div style="font-weight: bold; text-align: center; border-bottom: 2px solid black;">'
							+'<span style="font-weight: bold; font-size: 125%">InitiativeMaster Help v2.04</span>'
							+'</div>'
...
//----------------------------------- button press handlers ------------------------------------------	
    ...
    */
	...
	var handleInitMIpower = function( charCS, args, senderId ) {
		...
		if (_.isUndefined(rowIndex)) {
			sendDebug( 'handleInitMIpower: row index undefined' );
			sendError( 'Invalid button' );
			return;
		}
...
//		}, 10, initMenu, charCS, selected, args, senderId );
		return;
	}
	...
	var inHandMIbuttons = function( tokenID, charCS, buttonID, charButton, submitted, cmd ) {
	...
	async function makeMonAttkButtons( tokenID, charCS, charButton, monButton, submitted ) {
		...
		var makeAttkButtons = function() {
			...
			//			content += 'Monster '+monAttk3[0];
					...
					log('InitMaster makeMonAttkButtons: JavaScript '+e.name+': '+e.message+' while creating creature attack initiative buttons');
					sendDebug('InitMaster makeMonAttkButtons: JavaScript '+e.name+': '+e.message+' while creating creature attack initiative buttons');
					sendError('InitMaster JavaScript '+e.name+': '+e.message);
				} finally {
					setTimeout(() => {
						resolveMon(content);
	 ...
	 */
	async function makeWeaponButtons( tokenID, charButton, submitted, MWcmd, RWcmd, show2H, showDancing, showInHand ) {
		...
		content += await meleeWeaps();
		content += await rangedWeaps();
    ...
    */
	...
	async function makeMonsterMenu(complex,charCS,submitted,args,senderId) {
		...
		content += await makeMonAttkButtons( tokenID, charCS, charButton, monButton, submitted );

		if (complex) {
			content += '\n'+await makeWeaponButtons( tokenID, charButton, submitted, BT.MON_MELEE, BT.MON_RANGED );
...
//		weaponButtons = makeWeaponButtons( tokenID, -1, false, BT.MELEE, BT.RANGED, false, false );
		...
		[inHandMIs,buttonID] = await inHandMIbuttons( tokenID, charCS, 0, (isMIattk ? charButton : -1), submitted, BT.MI_ATTACK );
		content += inHandMIs;
		...
		content += await makeWeaponButtons( tokenID, charButton, submitted, BT.MELEE, BT.RANGED );
    ...
    */
	...
	async function makePrimeWeaponMenu(charCS,submitted,args,senderId) {
		...
		content += '{{desc1=' + await makeWeaponButtons( tokenID, -1, submitted, BT.MW_PRIME, BT.RW_PRIME, false );
...
//				highlight = submitted ? design.grey_button : ((charButton == w) ? design.green_button : design.selected_button);
    ...
    */
	...
	async function makeSpellMenu( spellCasterType, charCS, submitted, args, senderId ) {
		...
		dancers =  await makeWeaponButtons( tokenID, -1, submitted, '', '', true, true, false );
    ...
    */
	...
	async function makeMIBagMenu( charCS, submitted, args, senderId ) {
		...
		[inHandMIs,buttonID] = await inHandMIbuttons( tokenID, charCS, buttonID, charButton, submitted, BT.MI_POWER );
		content += inHandMIs;
		
/*		miTable = getTableField( charCS, {}, fields.Magic_table, fields.Magic_name );
		for (r = miTable.table[1]; !_.isUndefined(miName = miTable.tableLookup( fields.Magic_name, r, false )); r++) {
			if (miName != '-') {
				if (!inHandTitle) {
					content += '{{Section1=Magic Items in-hand\n';
					inHandTitle = true;
				}
				content += (buttonID == charButton ? '<span style=' + design.selected_button + '>' : (submitted ? '<span style=' + design.grey_button + '>' : '['));
				content += miName;
				content += (((buttonID == charButton) || submitted) ? '</span>' : '](!init --button ' + BT.MI_POWER + '|' + tokenID + '|' + buttonID + '|' + r + ')');
			}
			buttonID++;
		}
		if (inHandTitle) {
		...
		_.each(args, function(e) {
			setTimeout( () => {
				...
				try {
					...
					sendError('initiativeMaster JavaScript '+err.name+': '+err.message);
				}
			}, ((isGM ? 1 : 10)*t++), e);
     	});
	};
