	...
	**/
	...
	var handleInitSubmit = function( senderId, charCS, args ) {
		...
		var	charName = charCS.get('name'),
			...
			init_Mod = (attrLookup( charCS, fields.initMod ) || 0),
			...
			init_speed = attrLookup( charCS, fields.Init_speed ),
		...
		if (initMenu == MenuType.TWOWEAPONS) {
			...
			var init_speed2 = attrLookup( charCS, fields.Init_2ndSpeed ),
...
// ---------------------------------- build menus to display --------------------------------------------------------	
	...
	**/
	...
	var makeWeaponButtons = function( tokenID, charButton, submitted, MWcmd, RWcmd, show2H = true ) {
		...
		a = fields.MW_table[1] ? 0 : -1;
		for (i = a; i < (fields.MWrows + a); i++) {
			w = (1 - (a * 2)) + (i * 2);
			weapName = attrLookup( charCS, fields.MW_name, fields.MW_table, i );
			if (_.isUndefined(weapName)) {break;}
			twoHanded = parseInt(attrLookup( charCS, fields.MW_twoHanded, fields.MW_table, i ) || 0);
			if (weapName != '-' && (show2H || !twoHanded)) {
		...
		content += '\n**Ammo x Ranged weapons**\n';

		// build the character Ranged Weapons list
		
		a = fields.RW_table[1] ? 0 : -1;
		for (i = a; i < (fields.RWrows + a); i++) {
			w = (2 - (a * 2)) + (i * 2);
			weapName = attrLookup( charCS, fields.RW_name, fields.RW_table, i );
			if (_.isUndefined(weapName)) {break;}
			twoHanded = parseInt(attrLookup( charCS, fields.RW_twoHanded, fields.RW_table, i ) || 1);
			if (weapName != '-' && (show2H || !twoHanded)) {
				content += (w == charButton ? '<span style=' + design.selected_button + '>' : (submitted ? '<span style=' + design.grey_button + '>' : '['));
				ammoPointer = attrLookup( charCS, [fields.Ammo_indirect[0] + (i+1-a) + '-', fields.Ammo_indirect[1]]);
				if (_.isUndefined(ammoPointer)) {
					ammoPointer = (fields.Ammo_table[1] || i >= 0) ? fields.Ammo_table[0]+'_$'+i+'_' : '';
				}
    ...
    */
	...
	var makeMonsterMenu = function(complex,charCS,submitted,args) {
		...
		if (complex) {
			...
			content += makeWeaponButtons( tokenID, charButton, submitted, BT.MON_MELEE, BT.MON_RANGED );
    ...
    */
	...
	var makeWeaponMenu = function(charCS,submitted,args) {
		...
		content += '{{desc=' + makeWeaponButtons( tokenID, charButton, submitted, BT.MELEE, BT.RANGED );
    ...
    */
	...
	var makeMIBagMenu = function( charCS, submitted, args ) {
		...
		content += '}}{{desc1=' + otherActions( MenuType.MIBAG, tokenID, charButton, submitted ) + '}}'
				+ '{{desc2=Select action above, then '
				+ (((charButton < 0) || submitted) ? '<span style=' + design.grey_button + '>' : '[')
				+ 'Submit'
    ...
    */
	...
	var makePowersMenu = function( charCS, submitted, args ) {
		...
		content += '}}{{desc1=' + otherActions( MenuType.POWER, tokenID, charButton, submitted ) + '}}'
				+ '{{desc2=Select action above, then '
				+ (((charButton < 0) || submitted) ? '<span style=' + design.grey_button + '>' : '[')
				+ 'Submit'
    ...
    */
	...
	var makeThiefMenu = function( charCS, submitted, args ) {
		...
		content += '}}{{desc1=' + otherActions( MenuType.THIEF, tokenID, charButton, submitted ) + '}}'
				+ '{{desc2=Select action above, then '
				+ (((charButton < 0) || submitted) ? '<span style=' + design.grey_button + '>' : '[')
				+ 'Submit'
