...
//----------------------------------- button press handlers ------------------------------------------	
	...
	**/
	...
	var handlePrimeWeapon = function( charCS, sheetTypes, args ) {
		...
		if (rowIndex > 0) {
			if (command == BT.MW_PRIME) {
				repeating_index = (fields.MWfirstRowNamed && rowIndex == 1) ? '' : fields.MW_table[0] + '_$' + refIndex + '_';
				weaponName = (attrLookup( charCS, repeating_index + fields.MW_name[0], fields.MW_name[1]) || '');
				weapSpeed = (attrLookup( charCS, repeating_index + fields.MW_speed[0], fields.MW_speed[1]) || 0);
				speedMult = Math.max(parseFloat(attrLookup( charCS, fields.initMultiplier[0], fields.initMultiplier[1]) || 1), 1);
				attackNum = (attrLookup( charCS, repeating_index + fields.MW_noAttks[0], fields.MW_noAttks[1]) || 1);
			} else {
				repeating_index = (fields.RWfirstRowNamed && rowIndex == 1) ? '' : fields.RW_table[0] + '_$' + refIndex + '_';
	...
	**/
	...
	var handleInitCarry = function( tokenID, charCS, sheetTypes, initMenu ) {
		...
		setAttr( charCS, 'init_done', 'current', 0 );
		setAttr( charCS, 'init_submitVal', 'current', 1 );
	...
	**/
	...
	var handleInitSubmit = function( senderId, charCS, sheetTypes, args ) {
		...
		var submitVal = attrLookup( charCS, 'init_submitVal', 'current' );
		...
		var	charName = charCS.get('name'),
			...
			init_Done = parseInt(attrLookup( charCS, 'init_done', 'current' ), 10),
		...
		setAttr( charCS, 'init_done', 'current', -1 );
		setAttr( charCS, 'init_submitVal', 'current', 0 );
...
// ---------------------------------- build menus to display --------------------------------------------------------	
	...
	var makeMonsterMenu = function(complex,charCS,sheetTypes,submitted,args) {
		...
		if (complex) {
			...
			if (fields.MWfirstRowNamed) {
			    rowCount--;
				weapName = attrLookup( charCS, fields.MW_name[0], fields.MW_name[1]);
				if (weapName != '-') {
			...
			for (let i = 0; i < rowCount; i++) {
				w = 3 + (i * 2);
				weapName = attrLookup( charCS, fields.MW_table[0] + '_$' + i + '_' + fields.MW_name[0], fields.MW_name[1]);
				if (weapName != '-') {
			...
			if (fields.RWfirstRowNamed) {
			    rowCount--;
			    ammoRowAdjust++;
				weapName = attrLookup( charCS, fields.RW_name[0], fields.RW_name[1]);
				if (weapName != '-') {
			...
			for (let i = 0; i < rowCount; i++) {
				w = 4 + (i * 2);
				weapName = attrLookup( charCS, fields.RW_table[0] + '_$' + i + '_' + fields.RW_name[0], fields.RW_name[1]);
				if (weapName != '-') {
					content += (w == charButton ? '<span style=' + design.selected_button + '>' : (submitted ? '<span style=' + design.grey_button + '>' : '['));
	...
	var makeWeaponMenu = function(charCS,sheetTypes,submitted,args) {
		...
		if (fields.MWfirstRowNamed) {
			...
			if (weapName != '-') {
				content += (1 == charButton ? '<span style=' + design.selected_button + '>' : (submitted ? '<span style=' + design.grey_button + '>' : '['))
						+  weapName
						+  (((1 == charButton) || submitted) ? '</span>' : '](!init --button ' + BT.MELEE + '|' + tokenID + '|1|-1)');
			}
		}
		...
		for (let i = 0; i < rowCount; i++) {
			...
			if (weapName != '-') {
				content += (w == charButton ? '<span style=' + design.selected_button + '>' : (submitted ? '<span style=' + design.grey_button + '>' : '['))
						+  weapName
						+  (((w == charButton) || submitted) ? '</span>' : '](!init --button ' + BT.MELEE + '|' + tokenID + '|' + w + '|' + i + ')');
			}
		};
		...
		if (fields.RWfirstRowNamed) {
			...
			if (weapName != '-') {
				ammoPointer = attrLookup( charCS, fields.Ammo_indirect[0] + '1-', fields.Ammo_indirect[1]);
				ammoQty = parseInt(attrLookup( charCS, ammoPointer + fields.Ammo_qty[0], fields.Ammo_qty[1]));
				content += (2 == charButton ? '<span style=' + design.selected_button + '>' : ((submitted || !ammoQty) ? '<span style=' + design.grey_button + '>' : '['));
				content += ammoQty + ' x '+ weapName;
				content += (((2 == charButton) || submitted || !ammoQty) ? '</span>' : '](!init --button ' + BT.RANGED + '|' + tokenID + '|2|-1)');
			}
		}
		...
		for (let i = 0; i < rowCount; i++) {
			...
			if (weapName != '-') {
				ammoPointer = attrLookup( charCS, fields.Ammo_indirect[0] + (i+ammoRowAdjust) + '-', fields.Ammo_indirect[1]);
				ammoQty = parseInt(attrLookup( charCS, ammoPointer + fields.Ammo_qty[0], fields.Ammo_qty[1]));
				content += (w == charButton ? '<span style=' + design.selected_button + '>' : ((submitted || !ammoQty) ? '<span style=' + design.grey_button + '>' : '['));
				content += ammoQty + ' x '+ weapName;
				content += (((w == charButton) || submitted || !ammoQty) ? '</span>' : '](!init --button ' + BT.RANGED + '|' + tokenID + '|' + w + '|' + i + ')');
			}
		};
    ...
    */
	...
	var makePrimeWeaponMenu = function(charCS,sheetTypes,submitted,args) {
		...
		if (fields.MWfirstRowNamed) {
			...
			weapName = (attrLookup( charCS, fields.MW_name[0], fields.MW_name[1]) || '-');
			if (!twoHanded && weapName != '-') {
				content += '[' + weapName + '](!init --button ' + BT.MW_PRIME + '|' + tokenID + '|1|-1|-1|-1)';
			}
		}
		...
		for (let i = 0; i < rowCount; i++) {
			...
			weapName = (attrLookup( charCS, fields.MW_table[0] + '_$' + i + '_' + fields.MW_name[0], fields.MW_name[1]) || '-');
			if (!twoHanded && weapName != '-') {
				content += '[' + weapName + '](!init --button ' + BT.MW_PRIME + '|' + tokenID + '|' + w + '|-1|' + i + '|-1)';
			}
		};
		...
		if (fields.RWfirstRowNamed) {
			...
			if (!twoHanded && weapName != '-') {
				...
				ammoPointer = attrLookup( charCS, fields.Ammo_indirect[0] + '1-', fields.Ammo_indirect[1]);
				content += attrLookup( charCS, ammoPointer + fields.Ammo_qty[0], fields.Ammo_qty[1]) + ' x '+ weapName;
				content += '](!init --button ' + BT.RW_PRIME + '|' + tokenID + '|2|-1|-1|-1)';
			}
		}
		...
		for (let i = 0; i < rowCount; i++) {
			...
			if (!twoHanded && weapName != '-') {
				...
				content += weapName + '](!init --button ' + BT.RW_PRIME + '|' + tokenID + '|' + w + '|-1|' + i + '|-1)';
			}
		};
	...
	var makeSecondWeaponMenu = function(charCS,sheetTypes,submitted,args) {
		...
		for (let i = 0; i < rowCount; i++) {
			...
			if (!twoHanded && weapName != '-') {
				...
				content += (!submitted) ? ('](!init --button ' + handler + '|' + tokenID + '|' + charButton + '|' + w + '|' + ((charButton-4)/2) + '|' + i + ')') : '';
			}
		}
    ...
    */
	...
	var makeSpellMenu = function( spellCasterType, charCS, sheetTypes, submitted, args ) {
		...
		for (l = 1; l <= (isMU ? 9 : 7); l++) {
			...
			while (levelSpells[l].spells > 0) {
				...
				for (w = 1; (w <= fields.SpellsCols) && (levelSpells[l].spells > 0); w++) {
					...
					qty = parseInt(attrLookup( charCS, rep+fields.Spells_castValue[0]+col, fields.Spells_castValue[1] ));
					content += (buttonID == spellButton ? '<span style=' + design.selected_button + '>' : (submitted || qty == 0 ? '<span style=' + design.grey_button + '>' : '['));
					content += attrLookup( charCS, rep+fields.Spells_name[0]+col, fields.Spells_name[1]);
		...
		content += MIandPowers( tokenID, sheetTypes, submitted ) + '}}'
				+ '{{desc1=' + otherActions( (isMU ? MenuType.MUSPELL : MenuType.PRSPELL), tokenID, sheetTypes, spellButton, submitted ) + '}}'
				+ '{{desc2=Select action above, then '
    ...
    */
	...
	var makeMIBagMenu = function( charCS, sheetTypes, submitted, args ) {
		...
		if (!fields.MIFirstRowNum) {
			miName = attrLookup( charCS, fields.MI_name[0], fields.MI_name[1] );
			if (miName != '-') {
		...
		for (r = 0; r < itemRows; r++) {
			miName = attrLookup( charCS, fields.MI_table[0] + '_$' + r + '_' + fields.MI_name[0], fields.MI_name[1]);
			if (miName != '-') {
				content += (buttonID == charButton ? '<span style=' + design.selected_button + '>' : (submitted ? '<span style=' + design.grey_button + '>' : '['));
				content += miName;
				content += (((buttonID == charButton) || submitted) ? '</span>' : '](!init --button ' + BT.MI_BAG + '|' + tokenID + '|' + buttonID + '|' + r + ')');
	...
	var makePowersMenu = function( charCS, sheetTypes, submitted, args ) {
		...
		for (r = 0; r < powerRows; r++) {
			...
			for (w = 1; w <= fields.PowersCols; w++) {
				...
				qty = attrLookup( charCS, rep + fields.Spells_castValue[0] + col, fields.Spells_castValue[1] );
				powerName = attrLookup( charCS, rep + fields.Powers_name[0] + col, fields.Powers_name[1] );
				if (powerName != '-') {
