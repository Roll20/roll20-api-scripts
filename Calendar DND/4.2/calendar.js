	...
	(function () {
		...
		var version = '4.2',
				...
				if (playerIsGM(msg.playerid)) {
					switch (args[0]) {
						...
						case '!advtime':
							...
							if (
								((args[1].toLowerCase() == 'short rest' || args[1].toLowerCase() == 'hours') && Number(args[2]) >= 8) ||
								args[1].toLowerCase() == 'long rest' ||
								args[1].toLowerCase() == 'months' ||
								args[1].toLowerCase() == 'days' ||
								args[1].toLowerCase() == 'years'
							) {
								weather();
							}
							calmenu();
			...
			calmenu = function () {
				...
				sendChat(
					...
					'/w gm <div ' +
						...
						'" href="!setmonth,' +
						day +
						',?{Month|Hammer|Midwinter|Alturiak|Ches|Tarsakh|Greengrass|Mirtul|Kythorn|Flamerule|Midsummer|Eleasias|Eleint|Highharvestide|Marpenoth|Uktar|Feast of the Moon|Nightal}">' +
						month +
						'</a></td></tr>' + //--
						'<tr><td>Year: </td><td><a ' +
						astyle1 +
						'" href="!setyear,?{Year?|1486}">' +
						state.Calendar.now.year +
						'</a></td></tr>' + //--
						'<tr><td>Hour: </td><td><a ' +
						astyle1 +
						'" href="!sethour,?{Hour?|5}">' +
						state.Calendar.now.hour +
						'</a></td></tr>' + //--
						'<tr><td>Minute: </td><td><a ' +
						astyle1 +
						'" href="!setminute,?{Minute?|5}">' +
						state.Calendar.now.minute +
						'</a></td></tr>' + //--
						'</table>' + //--
						'<br>Weather: ' +
						...
						astyle2 +
						'" href="!advtime,?{Format of Time?|Short Rest|Long Rest|Days|Months|Years},?{Amount?|5}">Advance the Time</a></div>' + //--
						'<div style="text-align:center;"><a ' +
						astyle2 +
						'" href="!weather">Roll Weather</a></div>' + //--
						'<div style="text-align:center;"><a ' +
						astyle2 +
						'" href="!setalarm,?{Alarmnumber?|Alarm1|Alarm2|Alarm3|Alarm4|Alarm5|Alarm6|Alarm7|Alarm8|Alarm9|Alarm10},?{Day?|5},?{Month?|Hammer|Alturiak|Ches|Tarsakh|Mirtul|Kythorn|Flamerule|Eleasias|Eleint|Marpenoth|Uktar|Nightal},?{Year?|1486},?{Hour?|5},?{Minute?|5},?{Title?|1}">Set an Alarm</a></div>' + //--
						'<div style="text-align:center;"><a ' +
						astyle2 +
						'" href="!playercal">Show to Players</a></div>' + //--
						'<div style="text-align:center;"><a ' +
						astyle2 +
						'" href="!enc,?{Location?|City|Wilderness}">Random Encounter</a></div>' + //--
						'</div>'
				);
			},
			...
			setalarm = function (anum, tday, tmonth, tyear, thour, tminute, ttitle) {
				...
				sendChat(
					'Calendar',
					`/w gm Your Alarm >>${title}<< has been set for Day ${day} of ${month} of the Year ${year} at ${hour}:${minute}`
				);
				if (day > maxday) {
