...
var PAGENAVIGATOR= PAGENAVIGATOR|| (function(){
        ...
        switch(state.PAGENAVIGATOR.dmarker) {
            case 'name':
                dests = _.filter(graphics, function(d){
                    return (_.contains(state.PAGENAVIGATOR.pagenames, d.get('name').replace(/\[[^\[\]]\]/,'').trim())===true && (d.get('layer')==='map'||d.get('layer')==='gmlayer'));
                });
            ...
            default:
                dests = _.filter(graphics, function(d){
                    return (_.contains(state.PAGENAVIGATOR.pagenames, d.get('name').replace(/\[[^\[\]]\]/,'').trim())===true && (d.get('layer')==='map'||d.get('layer')==='gmlayer') && d.get('statusmarkers').indexOf(state.PAGENAVIGATOR.dmarker)>=0);
                });
                state.PAGENAVIGATOR.destinationids = _.map(dests,function(d){
                    return d.get('id');
                });
    ...
    */
    getPages = function() {
        state.PAGENAVIGATOR.folders = state.PAGENAVIGATOR.folders || {};
        allPages = findObjs({
            type: 'page',
            archived: false
        });
        var folders;
        _.each(allPages,(p)=>{
            folders = p.get('name').match(/(?:\[([^\[\]]+)\])/g);
            _.each(folders,(f)=>{
                state.PAGENAVIGATOR.folders[f.replace('[','').replace(']','')] = state.PAGENAVIGATOR.folders[f.replace('[','').replace(']','')] || 'gm';
                allFolders[f.replace('[','').replace(']','')] = allFolders[f.replace('[','').replace(']','')] || [];
                allFolders[f.replace('[','').replace(']','')].push(p);
            });
        });
        switch(state.PAGENAVIGATOR.access){
            case 'restricted':
                ...
                break;
            case 'open':
                playerPages = allPages;
        };
        state.PAGENAVIGATOR.pagenames = _.map(allPages, function(p) {
            return p.get('name');
        });
        getDestinations();
    },
    ...
    loadObjects = function(){
        var playerKeys = _.keys(state.PAGENAVIGATOR.players),
            character,token,jsonTok;
        state.PAGENAVIGATOR.folders = state.PAGENAVIGATOR.folders || {};
        getPages();
        destTokens = findObjs({
            type: 'graphic',
        ...
        _.each(playerKeys,(p)=>{
            playerTokens[p]=[];
            character = _.map(state.PAGENAVIGATOR.players[p],(id)=>{
                return getObj('character',id);
            });
            _.each(character,(c)=>{
                token = new Promise(function(resolve,reject){
                    c.get('defaulttoken',(t)=>{
                        try{
                            jsonTok = JSON.parse(decodeURIComponent(t).split(/<[/]?.+?>/g).join(''));
                            resolve(jsonTok);
                        }catch(err){
                            sendChat('PageNavigator Startup Error','/w gm The default token for character '+c.get('name')+' contains problem characters (most likely in its gmnotes field. This character will not be able to teleport until this is resolved. This is usually caused by copying and pasting formatted content into the gmnotes field.')
                        }
                    });
                });
    ...
    setDefaultTokens = function(playerId,characters){
        state.PAGENAVIGATOR.players[playerId]=characters;
    },
    ...
    outputConfig = function() {
        ...
        if(state.PAGENAVIGATOR.teleport === 'on'){
            ...
            if(state.PAGENAVIGATOR.removal === true){
                removalButton = makeButton(
                    '!nav-config removeoff',
                    'Token Removal On','green','black'
                );
            }else{
                removalButton = makeButton(
                    '!nav-config removeon',
                    'Token Removal Off','red','black'
                );
            }
            teleportMsg+=teleportButton+'<br><br>'+removalButton+'<br><br>';
            _.each(players,function(p){
    ...
    //                                      array, returns the array
        ...
        landingPt;
        surround = {
            left : locUse.left - locUse.width/2 + 35 - (newToken.width - 70)/2,
            top : locUse.top - locUse.height/2 + 35 - (newToken.height - 70)/2,
            width : 0,
            height : 0
        }
        ...
        while(!created && ((_.now() - start)/1000)<25){
            ...
            landingPt = [locUse.left,locUse.top,1];
            wallSegments = PathMath.toSegments(wallPaths);
            ...
            || (~~(surround.left/70 - 0.5) + 1)>page.get('width')){
                error=1;
            }
            ...
            if(!created){
                switch(inOut){
                    case true:
                        if(surround.width<(locUse.width- 70)){
                            surround.width += 70;
                            if(!hflip){
                                surround.left += 70;
                            }else{
                        ...
                        }else if(surround.height<(locUse.height - 70)){
                            surround.height += 70;
                            surround.width = 0;
                            hflip= !hflip;
                            if(!vflip){
                    ...
                    case false:
                        switch(true){
                            case (surround.width<(locUse.width - 70)):
                                surround.width+=70;
                                if(!hflip){
                                    surround.left+=70;
                                }else{
                                ...
                                break;
                            case (surround.height<(locUse.height - 70)):
                                surround.height+=70;
                                if(!vflip){
                                    surround.top+=70;
                                }else{
    ...
    createTokenLanding = function(landingToken, playerId){
        ...
        _.each(playerId,function(id){
            var stored = _.clone(playerTokens[id]);
        ...
        surr = 'initialized',
        i=0;
        if(state.PAGENAVIGATOR.removal){
            _.each(characters,(c)=>{
                _.each(findObjs({type:'graphic', represents:c.represents}),(g)=>{
                    g.remove();
                });
            });
        }
        while(i<characters.length && surr){
            ...
            if(surr){
                ...
                sendChat('Page Navigator','/w gm A space for all characters could not be found before risking an infinite loop. Please manually drag and drop any missing '
    ...
    findEntrance = function(origin, originGM, destination, playerId){
        ...
        _.each(landings, function(obj){
                try{
                    ...
                    if(destGM['location']===originGM['linkLocation'] && obj.get('name')===origin.get('name')){
                        createTokenLanding(obj,playerId);
                    };
                }catch(err){
                    sendChat('Page Navigator','/w gm There was a problem with the GM notes of the '+obj.get('name')+' destination token. Please '
                    +'review the formatting guidelines in the script forum post.');
                }
        });
    },
    ...
    */
    movePlayer = function(destToken, destination, playerid){
        ...
        try{
            ...
            sendChat('Page Navigator','/w gm There was a problem with the GM notes of the '+dToken.get('name')+' origin token. Please '
            +'review the formatting guidelines in the script forum post.');
        }
    },
    ...
    pagesTeleport = function(speakerid, destinationid, playerid,ret){
    ...
    pagesReturn = function(whoToMove, access, speaker, speakerid){
        ...
        if(access==='player' || whoToMove === 'allow'){
                if(ribbon.get('name').indexOf(state.PAGENAVIGATOR.pageMarker)>=0 || state.PAGENAVIGATOR.access === 'open' || whoToMove === 'allow'){
                        ...
                        Campaign().set({playerspecificpages: pp});
                        if(state.PAGENAVIGATOR.teleport === 'on'){
                            //             speakerid,destinationid,playerid(s)
                            pagesTeleport(speakerid,Campaign().get('playerpageid'),[speakerid],'return');
                        }
                }else{
                    msg += makeButton(
            ...
            if(whoToMove){
                control = _.map(whoToMove, function(obj){
                    if(obj.get('represents')){
                        ...
                        if(!obj.get('controlledby')){
                            sendChat('Page Navigation', '/w ' + speaker + ' <div style="border: 1px solid black; background-color: white; padding: 3px 3px;">'
                                +'<div style="font-weight: bold; border-bottom: 1px solid black;font-size: 100%;">'
                                +'ERROR CODE:RETURN NULLTOKEN - '+obj.get('name')+' is not controlled by anyone.</div>'
                            );
                        }else{
                            return obj.get('controlledby').split(',');
                        }
                ...
                if(state.PAGENAVIGATOR.teleport === 'on'){
                    //             speakerid,destinationid,playerid(s)
                    pagesTeleport(access,Campaign().get('playerpageid'),control,'return');
                }
            }else{
                Campaign().set({playerspecificpages: false});
                if(state.PAGENAVIGATOR.teleport === 'on'){
                    //             speakerid,destinationid,playerid(s)
                    pagesTeleport(access,Campaign().get('playerpageid'),_.map(findObjs({type:'player'}),(p)=>{return p.get('id')}),'return');
                }
            }
                            ...
                            case 'return':
                                if(access==='gm'){
                                    if(args[3]){
                                        ...
                                        tokens = _.map(msg.selected,(sel)=>{
                                            return getObj('graphic',sel._id);
                                        });
                                        pagesReturn(tokens, access, speaker, msg.playerid);
                                    };
                                }else if(access === 'player'){
                ...
                switch (args[1]){
                    ...
                    case 'tokens':
                        ...
                        if(args[2] && args[3]){
                            ...
                            charObj = _.reject(charObj,(o)=>{return _.isUndefined(o);});
                            setDefaultTokens(args[2],charObj);
                        };
                        break;
                    case 'removeon':
                        state.PAGENAVIGATOR.removal=true;
                        outputConfig();
                        break;
                    case 'removeoff':
                        state.PAGENAVIGATOR.removal=false;
                        outputConfig();
                }
                break;
        }
