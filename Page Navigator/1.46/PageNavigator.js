...
var PAGENAVIGATOR= PAGENAVIGATOR|| (function(){
    ...
    */
    ...
    //                                      array, returns the array
        log('initial loc: '+loc.left+' X '+loc.top+' |loc wxh: '+loc.width+' X '+loc.height+' <||> newToken wXh: '+newToken.width+' X '+newToken.height);
        var map = [],
        locUse = _.clone(loc),
        surround,
        squareNum = 0,
        ...
        while(!created && ((_.now() - start)/1000)<25){
            log('surround:'+surround.left+'X'+surround.top+' | Relative to loc:'+(surround.left-loc.left)+'X'+(surround.top-loc.top) + ' ||WidthXHeight|| Surround:'+surround.width+'X'+surround.height+' | LocUse:'+locUse.width+'X'+locUse.height+' | vFlip:'+vflip+' hFlip:'+hflip);
            error = null;
