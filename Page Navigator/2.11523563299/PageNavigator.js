...
*/
    var PAGENAVIGATOR= PAGENAVIGATOR|| (function(){
        ...
        var version = '2.11523563299',
            lastUpdate = 1523563299,
            schemaVersion = 2.11523563299,
            defaults = {
                css: {
                    button: {
        ...
        */
        ...
        moveToPage = function(id,players,pageId,destination){
            ...
            folders.push('All');
            _.each(_.range(folders.length),(f)=>{
        ...
        moveHandler = function(cmdDetails,id,selected,who){
            ...
            if(cmdDetails.players.length===0 && selected){
            ...
            }else if(cmdDetails.players.length>0){
                let players = [];
                _.each(cmdDetails.players,(s)=>{
                    character = getObj('character',s);
                    if(character){
                        let control = character.get('controlledby');
                        _.each(control.split(','),(c)=>{
                            players.push(c);
                        });
                    }else{
                        players.push(s);
                ...
                players = _.reject(players,(p)=>{return _.isUndefined(p);});
                cmdDetails.players = players;
            }
