...
var PAGENAVIGATOR= PAGENAVIGATOR|| (function(){
    ...
    */
    ...
    //                                      array, returns the array
        ...
        log(squareTest.length);
        wallSegments = PathMath.toSegments(wallPaths);
        while(!created && ((_.now() - start)/1000)<20){
            error = null;
    ...
    createTokenLanding = function(landingToken, playerId){
        ...
        _.each(characters, function(obj){
            surr = null;
            surr = mapSurrounding(landingLocation,{width:obj.width,height:obj.height},start);
            createObj('graphic',_.defaults(surr,obj));
        });
    ...
    */
    movePlayer = function(destToken, destination, playerid){
        ...
        _.each(playerid, function(id){
            var currentMap;
            if(pp[id]){
                currentMap.push(getObj('page',pp[id]).get('name'));
            }else{
                currentMap.push(getObj('page',Campaign().get('playerpageid')).get('name'));
            };
            pp[id] = destination;
        });
        if(state.PAGENAVIGATOR.teleport = 'on'){    
            findEntrance(getObj('page',dToken.get('pageid').get('name')), gm, destination, playerid);
        };
    ...
    moveCurrentParty = function(destToken, destination){
        ...
        if(state.PAGENAVIGATOR.teleport = 'on'){
            ...
            findEntrance(getObj('page',dToken.get('pageid').get('name')), gm, destination, party);
        };
    },
