...
var RoundMaster = (function() {
	...
	RoundMaster_Help:	{name:'RoundMaster Help',
						 ...
						 bio:'<div style="font-weight: bold; text-align: center; border-bottom: 2px solid black;">'
							...
							+'<p>If an Effects database of effect macros exists within the campaign (a character sheet with the name Effects-DB, with Ability Macros named the same as the status parameter set for markers) the Effects database will be searched in three ways: when a status marker is set, any Ability Macro with the name status-start (where status is the status name specified in the command) is run.  Each round when it is the turn of a token with the status marker set, the Ability Macro with the name status-turn is run.  And when the status ends (duration reaches 0) or the status is removed using --removestatus, or the token has the Dead marker set or is deleted, an Ability Macro with the name status-end is run.  See the Effects database documentation for full information on effect macros and the options and parameters that can be used in them.</p>'
...
//		args = args.join('|');
	...
	var doAddStatus = function(args,selection) {
		...
		_.each(selection,function(e) {
			...
			log('doAddStatus: checking for effectsLib = '+state.roundMaster.effectsLib);
			if (state.roundMaster.effectsLib) {
				log('doAddStatus: running -start effect');
