...
**/
	 ...
	 * Build turn display
	 */ 
	var makeTurnDisplay = function(curToken,msg) {
		if (!curToken) 
			{return;}
	...
	var handleAdvanceTurn = function(turnorder,priororder) {
		...
		if (currentTurn) {
			...
			&& isTracker(currentTurn)) {
                ...
                // RED: v1.207 chack to see that ChatSetAttr and Initiative are defined via flags
                if (flags.canSetAttr && flags.canSetRoundCounter) {
    				roundCtrCmd = '!setattr --silent --name Initiative --round-counter|' + rounds;
		...
		_.each(selection, function(e) {
			...
			effects = _.reject(effects,function(elem) {
				if (args.includes(elem.name.toLowerCase()) || args.includes('all')) {
				    // RED: v2.003 changed '==='' comparason of strings to 'includes()' comparison
...
*/		
	...
	**/
	...
	var sendPublic = function(msg) {
		...
		sendChat('',content,null,(flags.archive ? null:{noarchive:true}));
	};
	...
	**/
	var sendAPImacro = function(curToken,msg,effect,macro) {
		...
		if (effectsLib && journal) {
			var cname = journal.get('name'),
			    ac = getAttrByName( cid, 'ac' ),
			    thac0 = getAttrByName( cid, 'thac0' ),
			    hp = getAttrByName( cid, 'hp' ),
			...
			if (effectMacro.length > 0) {
		        ...
		        sendChat("character|"+cid,macroBody,null,(flags.archive ? null:{noarchive:true}));
    ...
    var sendRmAPI = function(msg) {
		...
		sendChat('',msg,null,(flags.archive ? null:{noarchive:true}));
    };
	...
	*/
	var sendFeedback = function(msg) {
		...
		sendChat(fields.feedbackName,content,null,(flags.archive ? null:{noarchive:true}));
	};
	...
	var sendDebug = function(msg) {
	    if (!!state.roundMaster.debug) {
		    ...
		    if (!msg)
		        {msg = 'No debug msg';}
    		sendChat('TJ Debug',to + '<span style="color: red; font-weight: bold;">'+msg+'</span>',null,(flags.archive ? null:{noarchive:true})); 
	    };
	}; 
				...
				if (effectsLib) {
					...
					if (charCS) {
						var cname = charCS.get('name'),
							ac = getAttrByName( oldRepresents, 'ac' ),
							thac0 = getAttrByName( oldRepresents, 'thac0' ),
							hp = getAttrByName( oldRepresents, 'hp' ),
						...
						if (effectMacro.length > 0) {
							...
							sendChat("character|"+oldRepresents,macroBody,null,(flags.archive ? null:{noarchive:true}));
