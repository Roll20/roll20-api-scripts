...
var Gaslight = Gaslight || (() => {
    ...
    /**
     * Feature-detect the Latest-sandbox (Mod Script Sandbox v1.5) graphic.createCopy(properties) method.
    ...
    const stageTokenToPages = (token, targetPageIds) => {
        ...
        pagesToCloneTo.forEach(function(targetPageId) {
            ...
            if (supportsCreateCopy(token)) {
                // Latest sandbox (v1.5): createCopy duplicates ALL properties
    ...
    const doQuick = (msg, args) => {
        ...
        if (!supportsCreateCopy(probe)) {
            reply(msg, 'Error', '<b>' + CMD + ' quick</b> requires the Latest sandbox (Mod Script Sandbox v1.5; needs <code>graphic.createCopy</code>). Switch your game to the Latest sandbox in Mod Scripts settings, or duplicate pages manually and use <code>' + CMD + ' setup</code>.');
            return;
        }
    ...
    const registerWithScriptKit = () => {
            ...
            help: {
                ...
                changelog: [
                    ...
                    { version: '2.3.0', date: '2026-08-31', changes: [
                        '`!gaslight quick [group] [players...]` — configure + split in one step, cloning the master onto page copies (and reusable ' + SCRATCH_NAME + ' pages) via graphic.createCopy (Latest sandbox v1.5)',
        ...
        const gsManualSetup = [
            { prompt: '💡 **Tip:** Gaslight has a one-command setup — `!gaslight quick` — that clones your page and splits automatically, no manual page duplication needed. It requires the **Latest sandbox (Mod Script Sandbox v1.5)**, which your game isn\'t using right now. You can switch to it in your game\'s Mod Scripts settings if you\'d like the easier flow; otherwise, continue with the manual steps below.' },
