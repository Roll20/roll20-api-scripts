...
Updates		  : 1.0.14 - Merged 1.0.12a and 1.0.12b changes into OneClick
						Added !eh config as an alias to !eh configmenu

				1.0.13 - OneClick version that didn't include 1.0.12a and 1.0.12b changes

				1.0.12b - Added the "include_graphics" config setting and adjusted enocunter creation and update routines to work with Paths as well as tokens. 
				...
				case "!eh":
					...
					case "reset":
						if (args.length > 0) {
							...
							//sendChat(translate(APINAME), `!eh show ${encName}`); 
							//sendChat(translate(APINAME), `!eh hide ${encName}`); 
						}
						break;
					...
					case "debuginfo":
						log(state[APINAME]);
						log(useroptions);
						var currentPageGraphics = findObjs({_pageid: Campaign().get("playerpageid"), _type: "graphic",});
						_.each(currentPageGraphics, function(obj) {    
						    log(`Object: ${obj.get("name")} - ${obj.get("_id")} (${obj.get("left")},${obj.get("top")}) `);
						    if (obj.get("name") == "Encounter Token") {
						        sendChat(translate(APINAME), `/w gm Found Encounter Token (${obj.get("_id")}) at (${obj.get("left")}, ${obj.get("top")})`);
						    }
						});
						break;
