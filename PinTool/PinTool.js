    ...
    function handlePurge(msg, args)
    {
        ...
        if(confirmed)
        {
            ...
            if(mode === "pins" && handoutId)
            {
                ...
                sendChat(
                    "PinTool",
                    `/w gm ✅ Deleted ${targets.length} pin(s) linked to "${_.escape(handoutName)}".`
                );
            }
...
*/
    ...
    function handleConvert(msg, tokens)
    {
        ...
        h.get("notes", function(notes)
        {
            ...
            {
                const headerText = _.unescape(match[1]).trim();
