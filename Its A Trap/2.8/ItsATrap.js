...
var ItsATrapCreationWizard = (() => {
  'use strict';
  const DISPLAY_CORE_WIZARD_CMD = '!ItsATrap_trapCreationWizard_coreOptions';
  const DISPLAY_THEME_WIZARD_CMD = '!ItsATrap_trapCreationWizard_themeOptions';
  const MODIFY_TRAP_CMD = '!ItsATrap_trapCreationWizard_modifyTrap';
  ...
  function displayWizard(who, playerId, trapToken) {
    ...
    let properties = getWizardProperties(trapToken);
    _.each(properties, prop => {
      ...
      _.each(paramProperties, item => {
        ...
        params.push(`${item.id}: ?{${item.name}: ${item.desc} ${options}}`);
      });

      row.append('td', `[${prop.name}](${MODIFY_TRAP_CMD} ${prop.id} ${params.join('&&')})`);
      row.append('td', `${prop.value}`);
    });

    content.append('div', '[Theme-specific options](' + DISPLAY_THEME_WIZARD_CMD + ' ?{Save Area of Effect: name})');

    let menu = _showMenuPanel('Trap core properties', content);
  ...
  function getWizardProperties(trapToken) {
    let anonVictim = {
      get: () => {
        return undefined;
      }
    };
    let trapEffect = ItsATrap.getTrapEffect(anonVictim, trapToken);
    ...
    return [
      ...
      },
      {
        id: 'api',
        name: 'API Command',
        desc: 'An API command which the trap runs when it is activated.',
        values: trapEffect.api
      },
      {
        id: 'disabled',
        name: 'Disabled',
        desc: 'Disable this trap from activating?',
        value: trapEffect.disabled ? 'yes' : 'no',
        options: ['yes', 'no']
      },
      {
        id: 'effectDistance'
      ...
      },
      {
        id: 'flying',
        name: 'Flying',
        desc: 'Should this trap affect flying tokens (fluffy-wing status is set)?',
        ...
        properties: [
          {
            id: 'name',
            name: 'FX Name',
            desc: 'The name or custom JSON definition for the special FX.'
          },
          {
            id: 'offset',
            name: 'FX Offset',
            desc: 'The offset (in units) of the special FX from the trap\'s center. Format: [X,Y]'
          },
          {
            id: 'direction',
            name: 'FX Direction',
            desc: 'The directional vector for the special FX (Leave blank to direct it towards characters). Format: [X,Y]'
          }
        ]
      },
      {
      ...
      },
      {
        ...
        value: trapEffect.searchDist || trapToken.get('aura2_radius')
      },
      {
        id: 'stopAt',
