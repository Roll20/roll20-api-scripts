...
var PCMHelper = PCMHelper || (function() {
	...
	var PCMHelper_Version = "1.0.5";
	var PCMHelper_LastUpdated = "2019-02-03";
    ...
    var pc_templates = `Basics: --tokenid|~0! --target_list|~1! --emote|~S-CN$ ~2! against ~T-CN$ --format|~S-CN$ --name|~3! --leftsub|~4! --rightsub|~5!<br>
...
--template|NPCAttack|~@{selected|rtype}$ 
  ...
  --replaceattrs|S-|@{selected|character_id} 
  --replaceattrs|T-|@{target|character_id} 
  --template|Basics|@{selected|token_id};@{target|token_id};uses ~PCA-NAME$;~PCA-NAME$;~PCA-ATYPE$;~PCA-RANGE$ 
...
--template|NPCAttack|~@{selected|rtype}$ 
  ...
  --template|Basics|@{selected|token_id};@{target|token_id};casts ~SP-NAME$;~SP-NAME$;Cantrip;~SP-RANGE$
  --template|CantripAttack|~@{selected|rtype}$
}}`;
  ...
  --replaceattrs|S-|@{selected|character_id} --replaceattrs|T-|@{target|character_id} --replacement|Advantage
  --template|Basics|@{selected|token_id};@{target|token_id};casts ~SP-NAME$;~SP-NAME$;Save DC ~S-SSDC$;~SP-RANGE$
  --template|SpellCast|~@{selected|rtype}$
}}`;
  ...
  --template|SpellCast|~@{selected|rtype}$
		...
		switch (msg.content.split(" ", 1)[0].toLowerCase()) {
			...
			case "!pcmeverybody":
				...
				_.each(chars, function(obj) {
				    log("Processing character: " + obj.get("_id"));
					processCharacter(obj.get("_id"));
					processNPC(obj.get("_id"));
					processSpells(obj.get("_id"));
				});
				break;
			case "!pcm_version":
	...
	function processQueue() {
		if (state.PCMQueue !== "") {
			var queueList = state.PCMQueue.split("|");
			for (var x=0; x<queueList.length; x++) {
				if (queueList[x].replace(/\|/g,"") !== "") {
					processCharacter(queueList[x]);
					processNPC(queueList[x]);
					processSpells(queueList[x]);
				}
			}
		}
		state.PCMQueue = "";
	...
	function AddQueueItem(charID) {
		if (!IsInQueue(charID)) {
			state.PCMQueue += "|" + charID + "|";
			log("PCMHelper: Adding " + charID + " to PCM Processing Queue");
			setTimeout(function(){processQueue();},5000);
		}
	}
	
	function IsInQueue(charID) {
		if (state.PCMQueue == undefined) { state.PCMQueue = ""; return false; }
		return (state.PCMQueue.indexOf("|" + charID + "|") !== -1);
	}
