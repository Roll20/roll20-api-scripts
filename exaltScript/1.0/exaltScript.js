...
var exaltScript = exaltScript || {
        ...
        _.each(graphic, function (graphic) {
            ...
            if (character != undefined){
                ...
                if (comitted < maxValue){
                    ...
                    toAdd = exaltScript.findMaxMotes(character.id,"personal-equation");;
                    toAdd = toAdd.toString();
                    exaltScript.setAttribute("personal-essence",character.id, toAdd) 
                } else {
                    exaltScript.setAttribute("peripheral-essence",character.id, "0");
                    comitted = comitted - maxValue;
                    maxValue = exaltScript.findMaxMotes(character.id,"personal-equation");
                    if (comitted < maxValue){
                        toAdd = maxValue - comitted;
                        toAdd = toAdd.toString();
                        exaltScript.setAttribute("personal-essence",character.id, toAdd);
    ...
    addMotes: function (motes, characterID){                                                //Generic algorithm for adding motes.
        ...
        if (getAttrByName(characterID, "peripheral-essence") == undefined
            || isNaN(getAttrByName(characterID, "peripheral-essence")) == true)                  //In the event that the attribute is set to an undefined value...
        ...
        if (getAttrByName(characterID, "personal-essence") == undefined 
            || getAttrByName(characterID, "personal-essence") == NaN)
    ...
    changePenaltyStatus: function(object, previous){                         //Spend five or more peripheral motes, get more anima.
        var graphic = findObjs({                              
            _represents: object.get("characterid"),                              
            _type: "graphic",
            _pageid: Campaign().get("playerpageid"),
        ...
        var graphic = graphic[0];
        if (graphic == undefined ) return;
        if (object.get("current") >= 0 || object.get("current") < -9) graphic.set("status_red",false);
            else (graphic.set("status_red",-object.get("current")));
    },
    ...
    updateStatuses: function(obj, lastPage){
        var oldGraphic = findObjs({
            _pageid: lastPage.playerpageid,
            _type: "graphic",
            layer: "objects",
        });

        var newGraphic = findObjs({
            _pageid: Campaign().get("playerpageid"),
            _type: "graphic",
            layer: "objects",
        });

        _.each(oldGraphic, function (oldGraphic) {
            _.each(newGraphic,function(newGraphic){
                if(oldGraphic.get("represents") == newGraphic.get("represents") && oldGraphic.get("represents") != ""){
                    newGraphic.set("status_aura",oldGraphic.get("status_aura"));
                    newGraphic.set("status_red",oldGraphic.get("status_red"));
                    newGraphic.set("status_purple",oldGraphic.get("status_purple")); 
                }
            });
        });
    },
    ...
    ToggleStatusmigration: function(){
        if(state.doStatusSwitch == false) state.doStatusSwitch = true;
        else state.doStatusSwitch = false;
        exaltScript.NotifyChange("Status Migration",state.doStatusSwitch);
    },

    ToggleWoundtracker: function(){
        if(state.doWoundtracker == false) state.doWoundtracker = true;
        else state.doWoundtracker = false;
        exaltScript.NotifyChange("Wound Tracker",state.doWoundtracker);
    },

    DisplayInterface: function(target,condensed){
        ...
        if (state.doStatusSwitch == false){
            migrationColor = "#FF0000";
        } else migrationColor = "#6B8E23";
        if (state.doWoundtracker == false){
            woundpenaltyColor = "#FF0000";
        } else woundpenaltyColor = "#6B8E23";
        if(condensed == false)
            sendChat('exaltScript','/w '
                        ...
                        +'<a href="!exaltScript settings migration" style="background-color:' 
                        +migrationColor
                        +'; margin: 2px;">Status Migration</a>'
                        +'<a href="!exaltScript settings woundpenalty" style="background-color:' 
                        +woundpenaltyColor
                        +'; margin: 2px;">Wound Penalty Tracker</a>'
    ...
    ChangeSettings: function(msg){
        switch(msg){
            ...
            case "migration":
                exaltScript.ToggleStatusmigration();
                break;
            
            case "woundpenalty":
                exaltScript.ToggleWoundtracker();
                break;
        }
    },
    ...
    HandleInput: function(tokens, player){      
        ...
        switch (command)
        {
            ...
            case 'UI':
                if(tokens[1] == "condensed") exaltScript.DisplayInterface(player,true);
                else exaltScript.DisplayInterface(player,false);
                break;
        }
    },
    ...
    RegisterEventHandlers: function(){
        ...
        on("change:attribute:current", function(obj, prev){   
            if(obj.get("name") == "peripheral-essence" && state.doAnima == true) exaltScript.increaseAnima(obj,prev);
            if(obj.get("name") == "wound-penalty" && state.doWoundtracker == true) exaltScript.changePenaltyStatus(obj,prev);
        });

        on("change:campaign:playerpageid", function(obj, prev){
            if(state.doStatusSwitch == true) exaltScript.updateStatuses(obj, prev);
        });
    }
};
