...
// -------------------------------------------- utility functions ----------------------------------------------
    ...
    **/
	...
	var castingLevel = function( charCS, casterType ) {
		switch (casterType) {
		case 'MU':
...
//        sendChat( fields.feedbackName, content );
	...
	var makeManageSpellsMenu = function( args, msg ) {
		...
		if (isPower) {
			...
			editCmd = BT.EDIT_POWER;
			reviewCmd = BT.REVIEW_POWER;
			memCmd = BT.MEM_POWER;
		...
		content = '&{template:2Edefault}{{name=Select Slot to Use in '+tokenName+'\'s '+tableType+'s}}'
				+ (isPower) ? '' : '{{subtitle=Level '+level+' spells}}';
		...
		while (levelSpells[level].spells > 0) {
			...
			for (w = 1; (w <= fields.SpellsCols) && (levelSpells[level].spells > 0); w++) {
				...
				rep = fields.Spells_table[0] + col + '_$' + r + '_' + fields.Spells_name[0] + col;
				content += (selectedSlot ? ('<span style=' + design.selected_button + '>') : '[');
				content += attrLookup( charCS, rep , fields.Spells_name[1]);
				content += (selectedSlot ? '</span>' : '](!magic --button ' + editCmd + '|' + tokenID + '|' + level + '|' + r + '|' + c + '|' + spellToMemorise + ')');
				c++;
		...
		content += '}}{{desc2=...Then\n'
				...
				+		 (!selectedBoth ? '</span>' : ('](!magic --button '+memCmd+'|'+tokenID+'|'+level+'|'+spellRow+'|'+spellCol+'|'+spellToMemorise+')'))+' the '+magicWord+'\n'
				...
				+  'Remove '+slotSpell
				+  (!selectedSlot ? '</span>' : ('](!magic --button '+memCmd+'|'+tokenID+'|'+level+'|'+spellRow+'|'+spellCol+'|-)') )+' the '+magicWord+'}}';
		...
		for (l = 1; l <= (isMU ? 9 : 7); l++) {
			...
			while (levelSpells[l].spells > 0) {
				...
				for (w = 1; (w <= fields.SpellsCols) && (levelSpells[l].spells > 0); w++) {
					...
					content += (buttonID == spellButton ? '<span style=' + design.selected_button + '>' : ((submitted || spellName == '-') ? '<span style=' + design.grey_button + '>' : '['));
					content += spellName;
					content += (((buttonID == spellButton) || submitted || (spellName == '-')) ? '</span>' : '](!magic --button ' + (isMU ? BT.MU_SPELL : BT.PR_SPELL) + '|' + tokenID + '|' + buttonID + '|' + r + '|' + c + ')');
					buttonID++;
					c++;
	...
	var makeViewMemSpells = function( args ) {
		...
		content = '&{template:2Edefault}{{name=View '+getObj('graphic',tokenID).get('name')+'\'s currently memorised '+magicWord+'s}}'
				+ '{{subtitle=' + tableType + '}}'
				+ '{{desc=';
		...
		for (l = 1; l <= levelLimit; l++) {
			...
			while (levelSpells[l].spells > 0) {
				...
				for (w = 1; (w <= fields.SpellsCols) && (levelSpells[l].spells > 0); w++) {
					...
					rep = fields.Spells_table[0] + col + '_$' + r + '_' + fields.Spells_name[0] + col;
					spellName= attrLookup( charCS, rep , fields.Spells_name[1]);
					content += (buttonID == spellButton ? '<span style=' + design.selected_button + '>' : '[')
							+  spellName
							+  (buttonID == spellButton ? '</span>' : '](!magic --button ' + viewCmd + '|' + tokenID + '|' + buttonID + '|' + r + '|' + c + '|' + spellName
							+  '\n&#37;{' + magicDB + '|'+spellName+'})');
					buttonID++;
					c++;
					levelSpells[l].spells--;
	...
	var makeUsePowerMenu = function( args, submitted ) {
		...
		while (levelSpells[l].spells > 0) {
			...
			for (w = 1; (w <= fields.SpellsCols) && (levelSpells[l].spells > 0); w++) {
				...
				spellName = attrLookup( charCS, fields.Spells_table[0] + col + '_$' + r + '_' + fields.Spells_name[0] + col, fields.Spells_name[1]) || '-';
				content += (buttonID == spellButton ? '<span style=' + design.selected_button + '>' : ((submitted || spellName == '-') ? '<span style=' + design.grey_button + '>' : '['));
				content += spellName;
				content += (((buttonID == spellButton) || submitted || (spellName == '-')) ? '</span>' : '](!magic --button '+BT.POWER+'|' + tokenID + '|' + buttonID + '|' + r + '|' + c + ')');
				buttonID++;
				c++;
				levelSpells[l].spells--;
			}
...
/*	
    ...
    */
	...
	var makeMUSpellsMenu = function( args ) {
		...
		var level = castingLevel( charCS, 'MU' );
	...
	var makePRSpellsMenu = function( args ) {
		...
		var level = castingLevel( charCS, 'PR' );
...
// ------------------------------------------------------------ Menu Button Press Handlers --------------------------------------------
	...
	**/
	...
	var handleChooseSpell = function( args ) {
		...
		col = (fields.SpellsFirstColNum || colIndex != 1) ? colIndex : '';
		rep = fields.Spells_table[0] + col + '_$' + rowIndex + '_';

		spellName = attrLookup( charCS, rep + fields.Spells_name[0] + col, fields.Spells_name[1] ) || '-';
		spellCost = attrLookup( charCS, rep + fields.Spells_cost[0] + col, fields.Spells_cost[1] ) || 0;
	...
	var handleCastSpell = function( args ) {
		...
		if (spellValue > 0) {
		    spellValue--;
			setAttr( charCS, spellRef+fields.Spells_castValue[0]+(colNum?colIndex:''), fields.Spells_castValue[1], spellValue );
		}
		if (spellValue == 0) {
	...
	var handleMemoriseSpell = function( args ) {
		...
		setAttr( charCS, tableRef + fields.Spells_castValue[0] + col, fields.Spells_castValue[1], 1 );
		setAttr( charCS, tableRef + fields.Spells_castMax[0] + col, fields.Spells_castMax[1], 1 );
...
// ------------------------------------------------------------- Command Action Functions ---------------------------------------------
	...
	var doCastSpell = function( args ) {
		...
		if (!level || level <= 0) {
			level = castingLevel( charCS, (isPower ? 'POWER' : (isMU ? 'MI' : 'PR')) );
		}
	...
	var doSpellsMenu = function( args ) {
		...
		var sheetTypes = getSheetTypes( charCS ),
			muLevel = castingLevel( charCS, 'MU' ),
			prLevel = castingLevel( charCS, 'PR' );
