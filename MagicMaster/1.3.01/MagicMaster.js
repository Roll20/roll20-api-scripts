...
var MagicMaster = (function() {
	...
	MagicMaster_Help:	{name:'MagicMaster Help',
						 ...
						 bio:'<div style="font-weight: bold; text-align: center; border-bottom: 2px solid black;">'
							...
							+'--mem-spell (MI-MU/MI-PR)|[token_id]|[mi-name]<br>'
							+'--view-spell (MI-MU/MI-PR/MI-POWER)|[token_id]|[mi-name]<br>'
							...
							+'<pre>Class, Item, Spell and Power Databases<br>'
							+'Classes<br>'
							+'Spells and Powers<br>'
							+'Types of Item Provided<br>'
							+'Adding Items to the Character<br>'
							+'Adding Spells & Powers to the Character<br>'
							+'Using Items<br>'
							+'Casting Spells and Using Powers<br>'
							+'Dynamic Lighting for Tokens<br>'
							+'DM Tools</pre>'
							+'<br>'
							...
							+'<pre>--mi-power token_id|power_name|[type-]mi_name|[casting-level]</pre>'
							...
							+'<pre>--mem-spell (MI-MU/MI-PR)|[token_id]|[mi-name]</pre>'
							+'<p>Takes a mandatory spell type, an optional Token ID for the character, and an optional magic item name.  If token ID is not provided, it uses the selected token, and if the magic item name is not specified, the last used magic item is assumed.</p>'
							+'<p>MI-MU and MI-PR mem-spell types are used to cast memorised spells into a spell-storing magic item, such as a Ring of Spell Storing.  Magic Item spells are stored in an unused spell level of the Character Sheet (by default Wizard Level 15 spells).  This command displays both all the character\'s memorised spells and the spell-storing magic item spell slots in the specified magic item (or the last one used if not specified), and allows a memorised spell to be selected, a slot (for the same spell name - limiting the item to only store certain defined spells) to be selected, and the spell cast from one to the other.</p>'
							...
							+'<h3>6.2 Tidy one or more character sheets</h3>'
							...
							+'<h3>6.3 Configure API behavior</h3>'
							...
							+'</table>'
							+'<h3>6.4 Check database completeness & integrity (GM only)</h3>'
							...
							+'<h3>6.5 Extract database for Editing</h3>'
							...
							+'<h3>6.6 Handshake with other APIs</h3>'
							...
							+'<h3>6.7 Switch on or off Debug mode</h3>'
							...
							+'<br>'
							+'<h2>7. How MagicMaster works</h2>'
							+'<h3>Class, Item, Spell and Power databases</h3>'
							...
							+'<h3>Classes</h3>'
							+'<p>The definitions for character Classes held in the Class-DB database include a description of the class and its capabilities, the powers/non-weapon proficiencies that it comes with, any restrictions on weapons, armour and spells that it is subject to, and other class-specific aspects such as alignments and races.  As you might expect, these are not just descriptions, but restrict the player character to the characteristics defined (alterable by using the <b>!magic --config</b> command).  The Class Database Help handout provides information on the structure of the class specifications and how the GM / game creator can add their own classes and alter those provided.</p>'
	...
	var init = function() {
		...
		if (_.isUndefined(state.MagicMaster.spellRules))
			{state.MagicMaster.spellRules = {specMU:true,strictNum:false};allowAll:false}
	...
	var parseClassDB = function(silent=true) {
		...
		for (const ClassName in DBindex.class_db) {
			let classDef = abilityLookup(fields.ClassDB, ClassName).obj[1].body,
				classSpecs = classDef.match(reSpecs),
				classData = classDef.match(reClassData),
				classType;
			if (classSpecs && !_.isNull(classSpecs)) {
				classSpecs = [...('['+classSpecs[0]+']').matchAll(reSpecsAll)];
...
/* ------------------------------- Magic Utility functions ----------------------------- */
    ...
    **/
	...
	var shapeSpellbook = function( charCS, spellbook ) {
		...
		if (charCS) {
			...
			case 'MU':
				...
				maxLevel = spellsPerLevel[charClass]['MU'][0][0];
				specSpells = (state.MagicMaster.spellRules.specMU ? specMU.includes(casterSpecs.cl.toUpperCase().replace(reIgnore,'')) : !ordMU.includes(casterSpecs.cl.toUpperCase().replace(reIgnore,''))) ? 1 : 0;
				for (let i=1; i<levelSpells.length; i++) {
				    noSpells = parseInt(spellsPerLevel[charClass]['MU'][(Math.min(i,maxLevel))][level]);
			...
			case 'PR':
                ...
                if (!level || !charClass) {return levelSpells;}
				maxLevel = spellsPerLevel[charClass]['PR'][0][0];
				for (let i=1; i<levelSpells.length; i++) {
				    noSpells = parseInt(spellsPerLevel[charClass]['PR'][(Math.min(i,maxLevel))][level]);
	...
	var checkValidSpell = function( args ) {
		...
		school = spellData.match(reSpecSuperType)[1];
	...
	var moveMIspells = function( fromCS, toCS, MIname ) {
		...
		var MIobj = getAbility( fields.MagicItemDB, MIname, toCS );
		
		if ((!fromCS && toCS) || (!!fromCS && !!toCS && (fromCS.id === toCS.id))) {
			if (!MIobj.obj) {
			...
			if (MUspellList.length) {
				miSpellValues = changeMIspells( fromCS, MIname, 'MU', 'REMOVE', MUspellList, attrLookup( fromCS, [fields.ItemMUspellValues[0]+MIname, fields.ItemMUspellValues[1]] ));
				if (toCS) {
					setAttr( toCS, [fields.ItemMUspellValues[0]+MIname, fields.ItemMUspellValues[1]], changeMIspells( toCS, MIname, 'MU', 'ADD', MUspellList, miSpellValues ));
				}
				if (!fromCS.get('name').startsWith('MI-DB')) MUspellObj.remove();
			}
			if (PRspellList.length) {
				miSpellValues = changeMIspells( fromCS, MIname, 'PR', 'REMOVE', PRspellList, attrLookup( fromCS, [fields.ItemPRspellValues[0]+MIname, fields.ItemPRspellValues[1]] ));
				if (toCS) {
					setAttr( toCS, [fields.ItemPRspellValues[0]+MIname, fields.ItemPRspellValues[1]], changeMIspells( toCS, MIname, 'PR', 'ADD', PRspellList, miSpellValues ));
				}
				if (!fromCS.get('name').startsWith('MI-DB')) PRspellObj.remove();
			}
			if (powerList.length) {
				miSpellValues = removeMIpowers( fromCS, MIname, powerList, attrLookup( fromCS, [fields.ItemPowerValues[0]+MIname, fields.ItemPowerValues[1]] ));
				if (toCS) {
					setAttr( toCS, [fields.ItemPowerValues[0]+MIname, fields.ItemPowerValues[1]], changeMIspells( toCS, MIname, 'POWER', 'ADD', powerList, miSpellValues ));
				}
				if (!fromCS.get('name').startsWith('MI-DB')) powerObj.remove();
			}
		}
		return;
	};
	...
	var getShownType = function( specs ) {
		var mi;
		if (specs) {
			mi = specs[0][2];
...
// ---------------------------------------------------- Make Menus ---------------------------------------------------------
	...
	var makeMIlist = function( charCS, includeEmpty=true, include0=true, showTypes=false ) {
		...
		maxSize = libRPGMaster.attrLookup( charCS, fields.ItemContainerSize ) || fields.MIRows;
		
		while (i < rows) {
			if (i<0) {
				miText = mi = libRPGMaster.attrLookup( charCS, fields.Items_name );
				qty = libRPGMaster.attrLookup( charCS, fields.Items_qty ) || 0;
			} else {
		...
		while (i < rows) {
			...
			if (mi.length > 0 && (includeEmpty || mi != '-')) {
				...
				if (isView && mi.replace(reIgnore,'').length) {
					if (Items.tableLookup( fields.Items_reveal, i ) == 'view') mi = Items.tableLookup( fields.Items_trueName, i );
	...
	var makeSpellList = function( senderId, tokenID, command, selectedButton, noDash = false, submitted = false, extension = '' ) {
...
//			spellToMemorise = spell.name;
		...
		spellbook = libRPGMaster.attrLookup( charCS, [fields.Spellbook[0]+((fields.SpellsFirstColNum || levelSpells[level].book != 1) ? levelSpells[level].book : ''), fields.Spellbook[1]] ) || '';
		...
		if (selectedSlot) {
		    slotSpell = libRPGMaster.attrLookup( charCS, fields.Spells_name, fields.Spells_table, spellRow, spellCol ) || '';
		}
		content += '}}{{desc2=...Then\n'
	...
	var makeStoreMIspell = function(args,senderId,msg = '') {
		...
		var isMU = command.includes('MU'),
			...
			content = '&{template:'+fields.defaultTemplate+'}{{name=Store Spell in '+tokenName+'\'s Magic Item}}'
					...
					+ '{{desc=1.Choose a spell to store\n'+makeSpellList( senderId, tokenID, (isMU ? BT.MU_TO_STORE : BT.PR_TO_STORE), spellButton, true, false, ('|'+MIbutton+'|'+MIrow+'|'+MIcol) )+'}}'
					+ '{{desc1=2.Choose where to store it\n'+makeSpellList( senderId, tokenID, (isMU ? BT.MU_MI_SLOT : BT.PR_MI_SLOT), MIbutton, true, false, ('|'+spellButton+'|'+spellRow+'|'+spellCol) )+'}}';
		...
		content += '{{desc2=3.Once both spell and slot selected\n'
				...
				+  ' or switch to ['+(isMU ? 'Priest' : 'Wizard')+'](!magic --mem-spell MI-'+(isMU ? 'PR' : 'MU')+'|'+tokenID+') spells'
				+  '}}';
		if (msg.length) {
			content += '{{='+msg+'}}';
		}
	...
	var makeCastSpellMenu = function( args, senderId, submitted = false ) {
		...
		content += '{{desc=' + makeSpellList( senderId, tokenID, selectCmd, spellButton, true, submitted, '|'+charged );
		...
		content += '}}{{desc1=Select '+magicWord+' above, then '
				...
				+'&#13;'+(spell.api ? '' : sendToWho(charCS,senderId,false,true))+'&#37;{' + spell.dB + '|' + spellName + '})')
				+ '}}';
	...
	var makeViewUseMI = function( args, senderId, menuType ) {
		...
		if (shortMenu || !isView) {
			if (MIrowref >= 0) {
				...
				let magicItem = getAbility( fields.MagicItemDB, trueMI, charCS );
				content += '['+actionText+' '+selectedMI+'](!magic --button '+ submitAction +'|'+ tokenID +'|'+ MIrowref
															+'&#13;'+(magicItem.api ? '' : sendToWho(charCS,senderId,false,true))+'&#37;{'+magicItem.dB+'|'+trueMI+'})';
			} else {
	...
	**/
	...
	var makeEditBagMenu = function(args,senderId,msg='',menuType) {
		...
		if (!shortMenu || !selected) {
			content += '{{desc=**1.Choose what item to store**\n'
					+  (editMartial ? '' : '[Potion](!magic --button '+chooseCmd+'|'+tokenID+'|'+MIrowref+'|?{Potion to store|'+getMagicList(fields.MagicItemDB,miTypeLists,'potion')+'}|'+charges+')')
		...
		if (!shortMenu || selected) {
			if (!remove) {
				if (shortMenu) {
					content += '{{desc=**1.Item chosen** ['+selectedMI+'](!magic --button '+redoCmd+'|'+tokenID+'|'+MIrowref+'), click to reselect\n';
				}
    			content += '\nOptionally, you can '+(selected ? '[' : '<span style='+design.grey_button+'>')+'Review '+selectedMI+(selected ? ('](!magic --button '+reviewCmd+'|'+tokenID+'|'+MIrowref+'|'+selectedMI+'|&#13;'+(magicItem.api ? '' : sendToWho(charCS,senderId,false,true))+'&#37;{'+magicItem.dB+'|'+selectedMI+'})') : '')+'</span>';
            } else {
				content += '{{desc=**1.Action chosen** ***Remove***, [click](!magic --button '+redoCmd+'|'+tokenID+'|'+MIrowref+') to change';
			}
			content += '}}';
		}
		...
		if (!shortMenu || (selected && !bagSlot)) {
			...
			if (shortMenu) {
				content += '[Select slot](!magic --button '+slotCmd+'|'+tokenID+'|?{Which slot?'+makeMIlist( charCS, true )+'}|'+selectedMI+')';
			} else {
				content += makeMIbuttons( tokenID, senderId, 'current', fields.Items_qty[1], slotCmd, '|'+selectedMI, MIrowref, false, true );
			}
		    ...
		    content += '{{desc1=**2.Selected** ['+qty+' '+mi+'](!magic --button '+slotCmd+'|'+tokenID+'|?{Which other slot?'+makeMIlist( charCS, true )+'}|'+selectedMI+'|)'
	...
	var makeGMonlyMImenu = function(args, senderId, msg) {
		...
		content += '[Potion](!magic --button GM-MItoStore|'+tokenID+'|'+MIrowref+'|?{Which Potion?|'+getMagicList(fields.MagicItemDB,miTypeLists,'potion')+'})'
				...
				+  '[DM only list](!magic --button GM-MItoStore|'+tokenID+'|'+MIrowref+'|?{Which DM only item?|'+getMagicList(fields.MagicItemDB,miTypeLists,'dmitem')+'})}}';
		...
		// build the character's visible MI Bag
		content += makeMIbuttons( tokenID, senderId, 'max', 'max', 'GM-MIslot', '|'+MItoStore, MIrowref, false, true ) + '}}';
		...
		if (chosenSlot) {
			let mi = abilityLookup( fields.MagicItemDB, slotName, charCS );
			if (mi.obj) {
				spellStoring = reCastMIspellCmd.test(mi.obj[1].body) || reCastMIpowerCmd.test(mi.obj[1].body);
			}
		}
		...
		content += '{{desc3=**3. '+selectableBoth+(chosenBoth ? ('Store '+MItoStore+'](!magic --button GM-StoreMI|'+tokenID+'|'+MIrowref+'|'+MItoStore+'|?{Quantity?|'+slotQty+'+1})') : ('Store'+(chosenSlot ? ('d '+slotActualName) : MItoStore)+'</span>'))+' **'
				+  ' or '+selectableBoth+'Hide'+(chosenBoth ? (' '+slotName+' as '+MItoStore+'](!magic --button GM-RenameMI|'+tokenID+'|'+MIrowref+'|'+MItoStore+')') : (hiddenMI ? ('n as '+slotName) : (' Item'+(chosenMI?(' as '+MItoStore):''))+'</span>'))+'<br><br>}}'
				...
				+  selectableSlot+'Change Type'+(chosenSlot ? ('](!magic --button GM-ChangeMItype|'+tokenID+'|'+MIrowref+'|'+MItoStore+'|?{Currently '+slotType+'. What type should '+slotName+' now be?|charged|uncharged|recharging|rechargeable|selfchargeable|cursed|cursed+charged|cursed+recharging|cursed+rechargeable|cursed+selfchargeable}) ') : '</span> ')+'<br>'
	...
	var makeShortPOPmenu = function( args, senderId, menuType ) {
        ...
        var putName = putCS.get('name'),
            ...
            miList = makeMIlist( pickCS, false, true, showTypes ),
		...
		if (pickRow >= 0) {
			...
			if (showTypes) {
				pickedMI = getShownType( abilityLookup( fields.MagicItemDB, pickedMI, pickCS ).specs() );
			}
		}
		i = slotsUsed = 0;
	...
	var makeSpellsMenu = function( args, senderId, msg='' ) {
		...
		var tokenID = args[1],
			...
			curSpells = '',
			pwList = [fields.ItemPowersList[0]+item,fields.ItemPowersList[1]],
			pwVals = [fields.ItemPowerValues[0]+item,fields.ItemPowerValues[1]],
			muList = [fields.ItemMUspellsList[0]+item,fields.ItemMUspellsList[1]],
			muVals = [fields.ItemMUspellValues[0]+item,fields.ItemMUspellValues[1]],
			prList = [fields.ItemPRspellsList[0]+item,fields.ItemPRspellsList[1]],
			prVals = [fields.ItemPRspellValues[0]+item,fields.ItemPRspellValues[1]],
...
// ------------------------------------------------------------ Menu Button Press Handlers --------------------------------------------
	...
	**/
	...
	var handleCastSpell = function( args, senderId ) {
		...
		var tokenID = args[1],
			...
			delScrollSpell = function ( charCS, spellName, scrollName, nameField, valueField ) {
				...
				var muSpellList = (libRPGMaster.attrLookup( charCS, [nameField[0]+scrollName, nameField[1]] ) || '').split(','),
				...
				if (nameIndex >= 0) {
					...
					muSpellList = (libRPGMaster.attrLookup( charCS, [valueField[0]+scrollName, valueField[1]] ) || '').split(',');
					muSpellList.splice( nameIndex, 1 );
		...
		var spellTables = getTable( charCS, fieldGroups.SPELLS, colIndex ),
			spellName = spellTables.tableLookup( fields.Spells_name, rowIndex ),
			spellMsg = spellTables.tableLookup( fields.Spells_msg, rowIndex ),
		...
		case BT.CAST_MISPELL:
    		miName = libRPGMaster.attrLookup( charCS, fields.ItemChosen );
			db = spellTables.tableLookup( fields.Spells_db, rowIndex );
		...
		setValue( charCS, fields.SpellCharges, spellValue-1 );
	...
	var handleReviewSpell = function( args, senderId ) {
		...
		var isMU = args[0].toUpperCase().includes('MU'),
			...
			isMI = args[0].toUpperCase().includes('MI'),
			isPower = args[0].toUpperCase().includes('POWER'),
			isSpell = args[0].toUpperCase().includes('SPELL'),
	...
	var handleViewUseMI = function( args, isSilent, senderId, charges=1 ) {
		...
		var charName = charCS.get('name'),
			...
			MItype = MItables.tableLookup( fields.Items_type, MIrowref, 'uncharged' ).toLowerCase(),
		...
		let muSpellsList = attrLookup( charCS, [fields.ItemMUspellsList[0]+MIname, fields.ItemMUspellsList[1]] ),
			...
			powersList = attrLookup( charCS, [fields.ItemPowersList[0]+MIname, fields.ItemPowersList[1]] );
		...
		case 'cursed+charged':
			...
			if (!_.isUndefined(powersList) && powersList.trim().length) break;
			if (MIqty == charges) {
				MItables.addTableRow( MIrowref );	// Blanks this row of the table
			} else {
	...
	var handleSelectMIpower = function( args, isUse, senderId ) {
		...
		log('handleSelectMIpower: called');
		...
		var	powerObj = attrLookup( MIlibrary, [fields.MIpowerPrefix[0]+itemName+'-'+powerName, null] );
		if (!powerObj) powerObj = attrLookup( MIlibrary, [fields.MIpowerPrefix[0]+powerName, null] );
		if (!powerObj) {
			sendDebug('handleSelectMIpower: not found item power index attribute for '+itemName+'-'+powerName);
		...
		log('handleSelectMIpower: found item and power');
		...
		if (isUse) {
			if (powerCount > 0) {
				Powers = Powers.tableSet( fields.Powers_castValue, powerRow, powerCount-1 );
			}
			...
			content = '&{template:'+fields.defaultTemplate+'}{{name='+itemDesc+'\'s '+powerDesc+' power}}'
					...
					+ '{{desc1=[Use '+powerDesc+'](!magic --button '+ BT.MI_POWER_USED +'|'+ tokenID +'|'+ powerName +'|'+ itemName +'|'+ castLevel
					+ '&#13;'+(powerLib.api ? '' : toWho)+'&#37;{'+powerLib.dB +'|'+ power +'})'
					+ ' or [Return to '+itemDesc+'](!&#13;'+(magicItem.api ? '' : toWho)+'&#37;{'+magicItem.dB+'|'+itemName+'})\nOr just do something else}}';
			sendResponse(charCS,content,senderId, flags.feedbackName, flags.feedbackImg, tokenID);
		}
	    return;
	}
	...
	var handleStoreMIspell = function( args, senderId ) {
		...
		var values = new Object(MIspellsTable.values);
		
		values[fields.Spells_name[0]][fields.Spells_name[1]] = spellName;
		values[fields.Spells_db[0]][fields.Spells_db[1]] = (isMU ? fields.MU_SpellsDB : fields.PR_SpellsDB);
		values[fields.Spells_speed[0]][fields.Spells_speed[1]] = attrLookup( charCS, fields.Spells_speed, fields.Spells_table, spellRow, spellCol );;
	...
	**/
	...
	var handlePickOrPut = function( args, senderId ) {
		...
		if (showType) {
			MItext = getShownType( abilityLookup( fields.MagicItemDB, MIname, fromCS ).specs() );
		}
		...
		log('handlePickOrPut: returned from moveMIspells');
		...
		log('handlePickOrPut: set MI quantity in bag. about to call checkac');

//		content = fields.attackMaster + ' --checkac ' + tokenID + '|silent||' + senderId;
// 		sendAPI( content, senderId );

		log('handlePickOrPut: about to send pickup message');
	...
	var handleRenameItem = function( args, senderId ) {
		...
		if (!MInewName || !MInewName.length) {
			...
			let MUspellObj = attrLookup( charCS, [fields.ItemMUspellsList[0]+MIoldName, null] ),
				PRspellObj = attrLookup( charCS, [fields.ItemPRspellsList[0]+MIoldName, null] ),
				powerObj = attrLookup( charCS, [fields.ItemPowersList[0]+MIoldName, null] );
			if (MUspellObj) {
				MUspellObj.set('name',fields.ItemMUspellsList[0]+MInewName);
				let spellList = MUspellObj.get('current').split(',');
				_.each(spellList,s => {
					let o = attrLookup( charCS, [fields.MIspellPrefix[0]+MIoldName+'-'+s,null] );
					if (!o) {
			...
			if (powerObj) {
				...
				_.each(spellList,s => {
					let o = attrLookup( charCS, [fields.MIpowerPrefix[0]+MIoldName+'-'+s,null] );
					if (!o) {
	...
	var handleHideMI = function( args, senderId ) {
		...
		Items = Items.tableSet( fields.Items_type, MIrowref, (newItem.obj ? newItem.obj[1].charge : 'uncharged' ));
	...
	var handleRemoveMI = function( args, GMonly, senderId ) {
		...
		getTable( charCS, fieldGroups.MI ).addTableRow( MIrowref );
		...
		var getNamesList = function( token ) {
			...
			return new Promise(resolve => {
				...
				try {
					...
					if (!(!charCS || !_.isUndefined(namesList[charCS.id]))) {
						Items = getTableField( charCS, {}, fields.Items_table, fields.Items_name );
						...
						while (!_.isUndefined(itemName = Items.tableLookup( fields.Items_name, ++i, false ))) {
							...
							itemList = itemList.concat((attrLookup( charCS, [fields.ItemMUspellsList[0]+itemName,fields.ItemMUspellsList[1]] ) || '').split(','),
													   (attrLookup( charCS, [fields.ItemPRspellsList[0]+itemName,fields.ItemPRspellsList[1]] ) || '').split(','),
													   (attrLookup( charCS, [fields.ItemPowersList[0]+itemName,fields.ItemPowersList[1]] ) || '').split(','));
//							itemList = itemList.concat(itemList.map(s=>itemName+'-'+s));
						...
						if (itemList.length) {
							...
							log('handleCStidy: found '+namesList[charCS.id].length+' named objects on character '+charCS.get('name'));
						}
					}
				} catch (e) {
					log('AttackMaster getNamesList: JavaScript '+e.name+': '+e.message+' while doing a tidy');
					sendDebug('AttackMaster getNamesList: JavaScript '+e.name+': '+e.message+' while doing a tidy');
...
//				log('handleCStidy: checking token '+obj.get('name')+' which represents character id '+obj.get('represents'));
				return (!(!obj.get('represents') || !obj.get('represents').length));
			});
		}
		tokenList = tokenList.filter( n => !!n );
//		log('handleCStidy: tokenList has '+tokenList.length+' entries');
		for (const token of tokenList) {
			errFlag = await getNamesList( token );
			if (errFlag) return;
		}
//		log('handleCStidy: found '+_.size(namesList)+' named objects in total');
...
// ------------------------------------------------------------- Command Action Functions ---------------------------------------------
	...
	var doViewUseMI = function( args, action, senderId, selected ) {
		...
		libRPGMaster.setAttr( charCS, fields.CastingLevel, casterLevel( charCS, 'MI' ));
...
//		Items = Items.tableSet( fields.Items_qty, MIrowref, Items.tableLookup( fields.Items_trueQty, MIrowref ));
	 ...
	 * SELFCHARGE value 0 sets current to max, 01,02,... (note the preceeding 0)
	...
	var doChangeCharges = function( args, selected, senderId ) {
		...
		case 'SET':
			...
			Items.tableSet( fields.Items_trueQty, r, charges );
			break;
		case 'DEDUCT':
			handleViewUseMI( ['',tokenID,r], true, senderId, charges );
			break;
		case 'SELFCHARGE':
	...
	*/
	...
	var doWriteDB = function(args) {
		...
		if (dbVersion && dbVersion.length && isNaN(parseFloat(dbVersion))) {
		...
		_.each(args, function(e) {
			setTimeout( () => {
				...
				try {
					...
					log('MagicMaster handleChatMsg: JavaScript '+e.name+': '+e.message+' while processing command '+cmd+' '+argString);
					sendDebug('MagicMaster handleChatMsg: JavaScript '+e.name+': '+e.message+' while processing command '+cmd+' '+argString);
					sendError('MagicMaster JavaScript '+e.name+': '+e.message);
				}
			}, (300*t++), e);
    	});
