...
//        sendChat( fields.feedbackName, content );
	...
	var makeMIlist = function( charCS, includeEmpty, include0=true ) {
		...
		for (let i = 0; i < fields.MIRows; i++) {
			...
			if (mi.length > 0 && (includeEmpty || mi != '-')) {
				qty = attrLookup( charCS, fields.MI_qty, fields.MI_table, i );
				if (include0 || qty > 0) {
    				miList += '|' + qty + ' ' + mi + ',' + i;
				}
			}
	...
	var makeMIbuttons = function( tokenID, miField, qtyField, cmd, extension='', MIrowref=-1, disable0=true, includeEmpty=false, pickID ) {
		...
		for (let i = 0; i < fields.MIRows; i++) {
			qty = attrLookup( charCS, [fields.MI_qty[0], qtyField], fields.MI_table, i ) || 0;
			...
			while (levelSpells[l].spells > 0) {
				...
				for (let w = 1; (w <= fields.SpellsCols) && (levelSpells[l].spells > 0); w++) {
					...
					if (!noDash || spellName != '-') {
    					...
    					content += (spellType.includes('POWER') ? (spellValue + ' ') : '') + spellName;
		...
		while (isMU || isPR || isPower || isMIPower) {
			...
			while (level < levelLimit && levelSpells[level].spells > 0) {
				...
				while (levelSpells[level].spells > 0) {
					c = levelSpells[level].base;
					for (w = 1; (w <= fields.SpellsCols) && (levelSpells[level].spells > 0); w++) {
						setAttr( charCS, fields.Spells_castValue, (!restType.includes('POWER') ? 1 : (attrLookup( charCS, fields.Spells_castMax, fields.Spells_table, r, c ) || 0)), fields.Spells_table, r, c );
						c++;
						levelSpells[level].spells--;
					}
					r++;
...
//		log('MIlibrary is '+MIlibrary.get('name')+', power is '+power+', row is '+powerRow+', col is '+powerCol);
	...
	**/
	...
	var handleStoreMI = function( args, GMonly, senderId ) {
		...
		if (!GMonly && slotTypeObj && slotTypeObj.get(fields.MI_type[1]).includes('cursed')) {
...
// ------------------------------------------------------------- Command Action Functions ---------------------------------------------
	...
	var doRest = function( args ) {
		...
		var timeSpent = attrLookup( charCS, fields.Timespent );
