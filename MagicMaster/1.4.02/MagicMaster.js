 ...
 * v1.4.02 13/11/2022  Added ability to specify "cast as level" in Class/Race/Creature powers. Weaponised
 *                     spells and powers used by creatures can be automatically equipped e.g. Spitting
 *                     Snake venom.
 */
...
//			classData = classData ? [...('['+classData[1]+']').matchAll(/\[[\s\w\-\+\,\:\/]+?\]/g)] : [];
	...
	var setSpell = function( charCS, spellTables, spellDB, spellName, r, c, cost, msg, levelOrPerDay, castAsLvl ) {
...
//			spellToMemorise = spell.name;
	...
	**/
	...
	var makeGMonlyMImenu = function(args, senderId, msg) {
		...
		// build the character's visible MI Bag
		content += makeMIbuttons( tokenID, senderId, 'max', 'current', 'GM-MIslot', '|'+MItoStore, MIrowref, false, true ) + '}}';
...
// ------------------------------------------------------------ Menu Button Press Handlers --------------------------------------------
	...
	**/
	...
	var handleMemoriseSpell = function( args, senderId ) {
		...
		spellTables = setSpell( charCS, spellTables, rootDB, spellName, row, col, undefined, '', [noToMemorise,noToMemorise], castAsLvl );
		...
		log('handleMemoriseSpell: row = '+row+', col = '+col+', weaponised = '+spellTables.tableLookup( fields.Spells_weapon, row )+', hand = '+hand);
...
*/		}
	...
	var handleViewUseMI = function( args, isSilent, senderId, charges=1 ) {
		...
		case 'cursed+charged':
			...
			if (MIqty == charges && (MItype != 'cursed+charged')) {
				MItables.addTableRow( MIrowref );	// Blanks this row of the table
				inHand = getTable( charCS, fieldGroups.INHAND );
				inHandRow = inHand.tableFind( fields.InHand_name, MIname );
				if (!_.isUndefined(inHandRow)) {
					inHand.addTableRow( inHandRow );
				}
			} else {
...
/*		case 'absorbing':
		case 'cursed+absorbing':
			MIqtyObj.set('current',(MIqty-charges));
			MIqtyObj.set('max',(MIqtyObj.get('max')-charges));
			break;
*/
		case 'rechargeable':
		case 'selfchargeable':
...
//		Items = Items.tableSet( fields.Items_qty, MIrowref, Items.tableLookup( fields.Items_trueQty, MIrowref ));
	...
	var doChangeCharges = function( args, selected, senderId ) {
		...
		absorbing = (Items.tableLookup( fields.Items_type, r ) || '').toLowerCase().includes('absorbing');
		switch (command) {
		case 'ADD':
		    i = (parseInt( Items.tableLookup(fields.Items_qty, r) ) || 0);      // attrLookup( charCS, fields.Items_qty, fields.Items_table, r ) || 0);
			charges = charges + i;
		case 'SET':
			if (absorbing && isNaN(maxCharges)) maxCharges = Items.tableLookup( fields.Items_trueQty, r );
