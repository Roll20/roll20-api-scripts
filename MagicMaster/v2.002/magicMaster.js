...
//        sendChat( fields.feedbackName, content );
	...
	var makeCastSpellMenu = function( args, submitted ) {
		...
		for (l = 1; l <= (isMU ? 9 : 7); l++) {
			...
			while (levelSpells[l].spells > 0) {
				...
				for (w = 1; (w <= fields.SpellsCols) && (levelSpells[l].spells > 0); w++) {
					firstCol = (fields.SpellsFirstColNum || c != 1);
					repSpellField = fields.Spells_table[0] + ( firstCol ? c : '' ) + '_$' + r + '_' + fields.Spells_name[0] + ( firstCol ? c : '' );
		...
		if (spellButton >= 0) {
			firstCol = (fields.SpellsFirstColNum || spellCol != 1);
			spellName = attrLookup( charCS, fields.Spells_table[0]+(firstCol ? spellCol : '')+'_$'+spellRow+'_'+fields.Spells_name[0]+(firstCol ? spellCol : ''), fields.Spells_name[1] ) || '-';
		}
    ...
    */
	...
	var makeMUSpellsMenu = function( args ) {
		...
		var content = '&{template:2Edefault} {{name='+curToken.get('name')+'\'s Magic User Spells menu}}'
					+ '{{desc=[2. Cast MU spell](!magic --cast-spell MU|'+tokenID+')'
	...
	var makePRSpellsMenu = function( args ) {
		...
		var content = '&{template:2Edefault} {{name='+curToken.get('name')+'\'s Clerical Spells menu}}'
					+ '{{desc=[2. Cast Priest spell](!magic --cast-spell PR|'+tokenID+')'
...
// ------------------------------------------------------------ Menu Button Press Handlers --------------------------------------------
	...
	**/
	...
	var handleCastSpell = function( args ) {
		...
		content = charName + ' is casting [' + spellName + '](!&#13;&#47;w gm &#37;{'+(isMU ? fields.MU_SpellsDB : fields.PR_SpellsDB) + '|' + spellName + '})'
...
// ------------------------------------------------------------- Command Action Functions ---------------------------------------------
	...
	var doCastSpell = function( args ) {
		...
		args.push(-1,-1,-1,'');
		setAttr( charCS, fields.Casting_level[0], fields.Casting_level[1], (attrLookup( charCS, (isMU ? fields.Wizard_level[0] : fields.Priest_level[0]), (isMU ? fields.Wizard_level[1] : fields.Priest_level[1]) ) || 0) );
	...
	var doSpellsMenu = function( args ) {
		...
		var tokenID = args[0],
			isMU = args[1] && args[1].includes('MU'),
			isPR = args[1] && args[1].includes('PR'),
			curToken = getObj('graphic',tokenID),
			charCS = getCharacter(tokenID);
		...
		var sheetTypes = getSheetTypes( charCS ),
			muLevel = attrLookup( charCS, fields.Wizard_level[0], fields.Wizard_level[1] ) || 0,
			prLevel = attrLookup( charCS, fields.Priest_level[0], fields.Priest_level[1] ) || 0;
