...
// -------------------------------------------- utility functions ----------------------------------------------
	...
	*/
    ...
    var attrLookup = function(character,attrDef,tableDef,r,c,caseSensitive) {
        let name, match,
			property = attrDef[1];
...
//            log('attrLookup: table:'+tableDef[0]+', r:'+r+', c:'+c+', name:'+name);
		} else {
			name = attrDef[0];
//            log('attrLookup: name:'+name);
		}
...
//            log('setAttr: name:'+name);
	...
	**/
	...
	var getMagicList = function( rootDB, listAttr ) {
		...
		filterObjs( function(objs) {
			...
			newList = attrLookup( objs, listAttr ) || ' ';
    ...
    **/
	...
	var setSpell = function( charCS, spellDB, spellName, r, c, cost, msg, levelOrPerDay ) {
		
		var newSpellObj = abilityLookup( spellDB, spellName );

		setAttr( charCS, fields.Spells_name, spellName, fields.Spells_table, r, c );
		setAttr( charCS, fields.Spells_db, newSpellObj.dB, fields.Spells_table, r, c );
		setAttr( charCS, fields.Spells_speed, (newSpellObj.ct ? newSpellObj.ct[0].get('current') : 0), fields.Spells_table, r, c );
		setAttr( charCS, fields.Spells_cost, cost, fields.Spells_table, r, c );
		setAttr( charCS, fields.Spells_msg, msg, fields.Spells_table, r, c );

		if (spellDB.toUpperCase().includes('POWER')) {
			setAttr( charCS, fields.Spells_castValue, (levelOrPerDay[1] || 0), fields.Spells_table, r, c );
			setAttr( charCS, fields.Spells_castMax, levelOrPerDay[0], fields.Spells_table, r, c );
		} else {
			setAttr( charCS, fields.Spells_miSpellSet, (levelOrPerDay[1] || 0), fields.Spells_table, r, c );
			setAttr( charCS, fields.Spells_storedLevel, levelOrPerDay[0], fields.Spells_table, r, c );
			setAttr( charCS, fields.Spells_castValue, (levelOrPerDay[0]==0 ? 0 : 1), fields.Spells_table, r, c );
			setAttr( charCS, fields.Spells_castMax, 1, fields.Spells_table, r, c );
		}
		return;
	}
	...
	var changeMIspells = function( charCS, listType, isAdd, spellList, spellValues = '' ) {

		var	l=1, r=0, w=1, s=1, c, valueItem,
 			...
 			while (s < 100) {
				...
				for (w = 1; (w <= fields.SpellsCols) && (s < 100); w++) {
					...
					if (isAdd && spellName == '-') {
						...
						setSpell( charCS, spellDB, spellName, r, c, 0, '', valueItem );
						maxSpell = s;
						if (listType == 'POWER') {
						...
						spellQty = attrLookup( charCS, fields.Spells_castValue, fields.Spells_table, r, c ) || 0;
						valueList[valueItem] = (spellQty > 0 ? attrLookup( charCS, fields.Spells_storedLevel, fields.Spells_table, r, c ) : 0)
											 + '.' + (attrLookup( charCS, fields.Spells_miSpellSet, fields.Spells_table, r, c ) || 0);
						toDoList[valueItem] = '';
						...
						setAttr( charCS, fields.Spells_storedLevel, 0, fields.Spells_table, r, c );
						setAttr( charCS, fields.Spells_miSpellSet, 0, fields.Spells_table, r, c );
					}
	...
	var moveMIspells = function( fromCS, toCS, MIname ) {
		...
		if (MUspellList && MUspellList.length>0) {
			...
			setAttr( toCS, [fields.MI_MUspellValues[0]+MIname, fields.MI_MUspellValues[1]], changeMIspells( toCS, 'MU', true, MUspellList,
			                                                                                changeMIspells( fromCS, 'MU', false, MUspellList,
		...
		if (PRspellList && PRspellList.length>0) {
			...
			setAttr( toCS, [fields.MI_PRspellValues[0]+MIname, fields.MI_PRspellValues[1]], changeMIspells( toCS, 'PR', true, PRspellList,
			                                                                                changeMIspells( fromCS, 'PR', false, PRspellList, 
		...
		if (powerList && powerList.length>0) {
			...
			setAttr( toCS, [fields.MI_PowerValues[0]+MIname, fields.MI_PowerValues[1]], changeMIspells( toCS, 'POWER', true, powerList,
...
//        sendChat( fields.feedbackName, content );
	...
	var makeMIlist = function( charCS, includeEmpty, include0=true ) {
		...
		for (let i = 0; i < fields.MIRows; i++) {
			mi = attrLookup( charCS, fields.MI_name, fields.MI_table, i );
			if (_.isUndefined(mi)) {break;}
			if (mi.length > 0 && (includeEmpty || mi != '-')) {
				qty = attrLookup( charCS, fields.MI_qty, fields.MI_table, i ) || 0;
				if (include0 || qty > 0) {
    				miList += '|' + qty + ' ' + mi + ',' + i;
				}
			}
	...
	var makeMIbuttons = function( tokenID, miField, qtyField, cmd, extension='', MIrowref=-1, disable0=true, includeEmpty=false, pickID ) {
		...
		for (let i = 0; i < fields.MIRows; i++) {
			mi = attrLookup( charCS, [fields.MI_name[0], miField], fields.MI_table, i ) || '';
			if (_.isUndefined(mi)) {break;}
			if (mi.length > 0 && (includeEmpty || mi != '-')) {
				qty = attrLookup( charCS, [fields.MI_qty[0], qtyField], fields.MI_table, i ) || '';
		...
		for (let l = 1; l < levelSpells.length; l++) {
			...
			while (levelSpells[l].spells > 0) {
				...
				for (let w = 1; (w <= fields.SpellsCols) && (levelSpells[l].spells > 0); w++) {
					let spellName = attrLookup( charCS, fields.Spells_name, fields.Spells_table, r, c );
					if (_.isUndefined(spellName)) {
						levelSpells[l].spells = 0;
						break;
					}
					let	spellValue = parseInt((attrLookup( charCS, fields.Spells_castValue, fields.Spells_table, r, c ) || 0),10),
	...
	var makeManageSpellsMenu = function( args, msg ) {
		...
		while (levelSpells[level].spells > 0) {
			...
			for (w = 1; (w <= fields.SpellsCols) && (levelSpells[level].spells > 0); w++) {
				...
				spellValue = parseInt((attrLookup( charCS, fields.Spells_castValue, fields.Spells_table, r, c ) || 0),10);
	...
	var makeShortPOPmenu = function( args, senderId, menuType ) {
		...
		if (pickRow >= 0) {
			...
			while (i < fields.MIRows && putRow < 0) {
				mi = (attrLookup( putCS, fields.MI_name, fields.MI_table, pickRow ) || '');
				miTrueName = (attrLookup( putCS, fields.MI_trueName, fields.MI_table, pickRow ) || '');
				miType = (attrLookup( putCS, fields.MI_type, fields.MI_table, pickRow ) || '');
...
//		log('makeShortPOPmenu: after namesearch putRow is '+putRow);
		while (i < fields.MIRows && putRow < 0) {
			mi = attrLookup( putCS, fields.MI_name, fields.MI_table, i );
			if (_.isUndefined(mi)) {break;}
			if (mi == '-') {
				putRow = i;
			}
	...
	var handleRest = function( args ) {
		...
		if (casterType.includes('MI') && casterType.includes('POWER')){
			for (r = 0; (r < fields.MIPowersRows); r++) {
				...
				for (w = 1; (w <= fields.PowersCols); w++) {
					setAttr( dB.miDB, fields.Spells_castValue[0], (attrLookup( dB.miDB, fields.Spells_castMax, fields.Powers_table, r, c ) || 0), fields.Powers_table, r, c );
					c++;
				}
			}
			return;
				...
				while (levelSpells[level].spells > 0) {
					...
					for (w = 1; (w <= fields.SpellsCols) && (levelSpells[level].spells > 0); w++) {
						if (restType.includes('POWER')) {
							powerQty = attrLookup( charCS, fields.Spells_castMax, fields.Spells_table, r, c );
							if (_.isUndefined(powerQty)) {
								levelSpells[level].spells = 0;
								break;
							}
							setAttr( charCS, fields.Spells_castValue, powerQty, fields.Spells_table, r, c );
						} else {
							setAttr( charCS, fields.Spells_castValue, 1, fields.Spells_table, r, c );
						}
						c++;
						levelSpells[level].spells--;
					}
		...
		if (isMI) {
			for (r = 0; (r < fields.MIRows); r++) {
				if (!attrLookup( charCS, fields.MI_name, fields.MI_table, r)) {break;}
				setAttr( charCS, [fields.MI_speed[0], 'current'], attrLookup( charCS, [fields.MI_speed[0], 'max'], fields.MI_table, r ), fields.MI_table, r );
				setAttr( charCS, [fields.MI_qty[0], 'current'], attrLookup( charCS, [fields.MI_qty[0], 'max'], fields.MI_table, r ), fields.MI_table, r );
			}
		}
		return;
	}
...
//		log('MIlibrary is '+MIlibrary.get('name')+', power is '+power+', row is '+powerRow+', col is '+powerCol);
	...
	**/
	...
	var handleConfirmedBlank = function( args ) {
		...
		for (let i=0; i<fields.MIRows; i++) {
			setAttr( charCS, [fields.MI_name[0], 'current'], '-', fields.MI_table, i );
			setAttr( charCS, [fields.MI_name[0], 'max'], '-', fields.MI_table, i );
			setAttr( charCS, [fields.MI_speed[0], 'current'], '0', fields.MI_table, i );
			setAttr( charCS, [fields.MI_speed[0], 'max'], '0', fields.MI_table, i );
			setAttr( charCS, [fields.MI_qty[0], 'current'], '', fields.MI_table, i );
			setAttr( charCS, [fields.MI_qty[0], 'max'], '0', fields.MI_table, i );
			setAttr( charCS, fields.MI_cost, 0, fields.MI_table, i );
			setAttr( charCS, fields.MI_type, '', fields.MI_table, i  );
		}
...
// ------------------------------------------------------------- Command Action Functions ---------------------------------------------
	...
	var doChangeCharges = function( args ) {
		...
		log('doChangeCharges: command='+command);
		...
		case 'ADD':
		    ...
		    log('doChangeCharges: current charges are '+i+', adding '+charges);
			charges = charges + i;
		case 'SET':
			if (!isNaN(maxCharges) && maxCharges < charges) {
			    log('doChangeCharges: charges '+charges+' more than maxCharges '+maxCharges);
				charges = maxCharges;
			}
		    log('doChangeCharges: setting charges as '+charges);
			setAttr( charCS, fields.MI_trueQty, charges, fields.MI_table, r );
			break;
		case 'DEDUCT':
		    log('doChangeCharges: deducting '+charges+' charges');
