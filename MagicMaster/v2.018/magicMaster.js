...
// -------------------------------------------- utility functions ----------------------------------------------
	...
	var getTable = function(character,tableDef,attrDef,col='',caseSensitive=false) {
	    let name;
	    if (tableDef && !tableDef[1] && col && col==1) {col = '';}
	    if (tableDef) {
        ...
        if(match){
            ...
            return {attrs:attrs,sortKeys:sortOrderKeys};
        }
		return undefined;
	};
	...
	var tableLookup = function( tableObj, index, attrDef ) {
		if (tableObj) {
			let attrs = tableObj.attrs,
				sortOrderKeys = tableObj.sortKeys;
			if (index<sortOrderKeys.length && _.has(attrs,sortOrderKeys[index])){
				if (_.isUndefined(attrDef) || _.isNull(attrDef) || !attrDef[1] || _.isUndefined(attrs[sortOrderKeys[index]])) {
					return attrs[sortOrderKeys[index]];
				} else {
					return attrs[sortOrderKeys[index]].get(attrDef[1]);
				}
			}
		}
		return undefined;
	...
	var tableSet = function( charCS, tableObj, attrDef, attrValue, tableDef, r, c, caseSensitive ) {
		if (tableObj) {
			let attrs = tableObj.attrs,
				sortOrderKeys = tableObj.sortKeys;
		...
		setAttr( charCS, attrDef, attrValue, tableDef, r, c, caseSensitive );
		return undefined;
	}
	...
	*/
    ...
    var attrLookup = function(character,attrDef,tableDef,r,c='',caseSensitive=false) {
        ...
        if(match){
			...
			return tableLookup(tableObj,index,attrDef);
		} else {
...
//		log('getMagicList magicList:'+magicList);
    ...
    **/
	...
	var blankSlot = function( storeCS, rowRef ) {
		...
		setAttr( storeCS, fields.MI_name, '-', fields.MI_table, rowRef );
		setAttr( storeCS, fields.MI_trueName, '-', fields.MI_table, rowRef );
		setAttr( storeCS, fields.MI_qty, '', fields.MI_table, rowRef );
		setAttr( storeCS, fields.MI_trueQty, '', fields.MI_table, rowRef );
		setAttr( storeCS, fields.MI_speed, 0, fields.MI_table, rowRef );
		setAttr( storeCS, fields.MI_trueSpeed, 0, fields.MI_table, rowRef );
		setAttr( storeCS, fields.MI_cost, 0, fields.MI_table, rowRef );
		setAttr( storeCS, fields.MI_type, '', fields.MI_table, rowRef );
	...
	var setSpell = function( charCS, spellTables, spellDB, spellName, r, c, cost, msg, levelOrPerDay ) {
		...
		tableSet( charCS, spellTables.nameTable, fields.Spells_name, spellName, fields.Spells_table, r, c );
		tableSet( charCS, spellTables.dbTable, fields.Spells_db, newSpellObj.dB, fields.Spells_table, r, c );
		tableSet( charCS, spellTables.speedTable, fields.Spells_speed, speed, fields.Spells_table, r, c );
		tableSet( charCS, spellTables.costTable, fields.Spells_cost, cost, fields.Spells_table, r, c );
		tableSet( charCS, spellTables.msgTable, fields.Spells_msg, msg, fields.Spells_table, r, c );

		if (spellDB.toUpperCase().includes('POWER')) {
			tableSet( charCS, spellTables.valueTable, fields.Spells_castValue, (levelOrPerDay[1] || 0), fields.Spells_table, r, c );
			tableSet( charCS, spellTables.maxTable, fields.Spells_castMax, levelOrPerDay[0], fields.Spells_table, r, c );
		} else {
			tableSet( charCS, spellTables.miSpellTable, fields.Spells_miSpellSet, (levelOrPerDay[1] || 0), fields.Spells_table, r, c );
			tableSet( charCS, spellTables.levelTable, fields.Spells_storedLevel, levelOrPerDay[0], fields.Spells_table, r, c );
			tableSet( charCS, spellTables.valueTable, fields.Spells_castValue, (levelOrPerDay[0]==0 ? 0 : 1), fields.Spells_table, r, c );
			tableSet( charCS, spellTables.maxTable, fields.Spells_castMax, 1, fields.Spells_table, r, c );
		}
		return;
	}
			...
			r = 0; maxSpell = 0; s = 1;
 			while (s < 100) {
				...
				for (w = 1; (w <= fields.SpellsCols) && (s < 100); w++) {
				    if (!spellTables[w]) {
				        spellTables[w]={};
						spellTables[w].nameTable = getTable( charCS, fields.Spells_table, fields.Spells_name, c );
						spellTables[w].valueTable = getTable( charCS, fields.Spells_table, fields.Spells_castValue, c );
						spellTables[w].maxTable = getTable( charCS, fields.Spells_table, fields.Spells_castMax, c );
						spellTables[w].levelTable = getTable( charCS, fields.Spells_table, fields.Spells_storedLevel, c );
						spellTables[w].miSpellTable = getTable( charCS, fields.Spells_table, fields.Spells_miSpellSet, c );
					    spellTables[w].dbTable = getTable( charCS, fields.Spells_table, fields.Spells_db, c );
					    spellTables[w].speedTable = getTable( charCS, fields.Spells_table, fields.Spells_speed, c );
					    spellTables[w].costTable = getTable( charCS, fields.Spells_table, fields.Spells_cost, c );
					    spellTables[w].msgTable = getTable( charCS, fields.Spells_table, fields.Spells_msg, c );
					}
					spellName = tableLookup( spellTables[w].nameTable, r, fields.Spells_name );
					...
					if (isAdd && spellName == '-') {
						...
						setSpell( charCS, spellTables[w], spellDB, spellName, r, c, 0, '', valueItem );
						maxSpell = s;
						if (listType == 'POWER') {
						...
						spellQty = tableLookup( spellTables[w].valueTable, r, fields.Spells_castValue) || 0;
						valueList[valueItem] = (spellQty <= 0 ? 0 : (tableLookup(spellTables[w].levelTable, r, fields.Spells_storedLevel) || 1))
													  + '.' + (tableLookup( spellTables[w].miSpellTable, r, fields.Spells_miSpellSet) || 0);
						
						tableSet( charCS, spellTables.nameTable, fields.Spells_name, '-', fields.Spells_table, r, c );
						tableSet( charCS, spellTables.valueTable, fields.Spells_castValue, 0, fields.Spells_table, r, c );
						tableSet( charCS, spellTables.maxTable, fields.Spells_castMax, 0, fields.Spells_table, r, c );
						tableSet( charCS, spellTables.levelTable, fields.Spells_storedLevel, 0, fields.Spells_table, r, c );
						tableSet( charCS, spellTables.miSpellTable, fields.Spells_miSpellSet, 0, fields.Spells_table, r, c );
					}
...
//        sendChat( fields.feedbackName, content );
	...
	var makeMIlist = function( charCS, includeEmpty=true, include0=true ) {
		...
		while (i < rows) {
			if (i<0) {
				mi = attrLookup( charCS, fields.MI_name );
				qty = attrLookup( charCS, fields.MI_qty ) || 0;
			} else {
			    mi = tableLookup( MInames, i, fields.MI_name );
			    qty = tableLookup( MIqtys, i, fields.MI_qty ) || 0;
			}
	...
	var makeMIbuttons = function( tokenID, miField, qtyField, cmd, extension='', MIrowref=-1, disable0=true, includeEmpty=false, pickID ) {
		...
		var MInames = getTable( charCS, fields.MI_table, fields.MI_name ),
			MIqtys  = getTable( charCS, fields.MI_table, fields.MI_qty ),
			rows = i+(MInames && MInames.sortKeys) ? MInames.sortKeys.length : 0;
		
		while (i < rows) {
			if (i<0) {
				mi = attrLookup( charCS, [fields.MI_name[0], miField] );
				qty = attrLookup( charCS, [fields.MI_qty[0], miField] ) || 0;
			} else {
			    mi = tableLookup( MInames, i, ['',miField] );
			    qty = tableLookup( MIqtys, i, ['',miField] ) || 0;
			}
			if (_.isUndefined(mi)) {break;}
			if (mi.length > 0 && (includeEmpty || mi != '-')) {
			...
			while (levelSpells[l].spells > 0) {
				...
				for (let w = 1; (w <= fields.SpellsCols) && (levelSpells[l].spells > 0); w++) {
					if (_.isUndefined(spellTables[w])) {
						spellTables[w] = {};
						spellTables[w].namesTable = getTable( charCS, fields.Spells_table, fields.Spells_name, c );
						spellTables[w].valuesTable = getTable( charCS, fields.Spells_table, fields.Spells_castValue, c );
					}
					let spellName = tableLookup( spellTables[w].namesTable, r, fields.Spells_name );
					if (_.isUndefined(spellName)) {
						levelSpells[l].spells = 0;
						break;
					}
					let	spellValue = parseInt((tableLookup( spellTables[w].valuesTable, r, fields.Spells_castValue ) || 0),10),
	...
	var makeManageSpellsMenu = function( args, msg ) {
		...
		while (levelSpells[level].spells > 0) {
			...
			for (w = 1; (w <= fields.SpellsCols) && (levelSpells[level].spells > 0); w++) {
				...
				spellName = tableLookup( spellTables[w].namesTable, r, fields.Spells_name );
				if (_.isUndefined(spellName)) {
					levelSpells[level].spells = 0;
					break;
				}
				spellValue = parseInt((tableLookup( spellTables[w].valuesTable, r, fields.Spells_castValue ) || 0),10);
	...
	**/
	...
	var makeViewUseMI = function( args, senderId, menuType ) {
		...
		if (shortMenu || !isView) {
			if (MIrowref >= 0) {
				selectedMI = attrLookup( charCS, [fields.MI_name[0], (isGM ? 'max' : 'current')], fields.MI_table, MIrowref ) || '-';
	...
	**/
	...
	var makeEditBagMenu = function(args,senderId,msg='',menuType) {
		...
		if (bagSlot) {
			qty = attrLookup( charCS, [fields.MI_qty[0], 'current'], fields.MI_table, MIrowref ) || 0;
		}
		if (!shortMenu || (selected && !bagSlot)) {
			...
			removeMI = mi = attrLookup( charCS, [fields.MI_name[0], 'current'], fields.MI_table, MIrowref );
	...
	var makeGMonlyMImenu = function(args, msg) {
		...
		var slotName = (MIrowref >= 0) ? (attrLookup( charCS, [fields.MI_name[0], 'current'], fields.MI_table, MIrowref ) || '') : '',
			slotActualName = (MIrowref >= 0) ? (attrLookup( charCS, [fields.MI_name[0], 'max'], fields.MI_table, MIrowref ) || '') : '',
			slotType = (MIrowref >= 0) ? (attrLookup( charCS, fields.MI_type, fields.MI_table, MIrowref ) || '') : '',
			slotQty = (attrLookup( charCS, [fields.MI_qty[0], 'current'], fields.MI_table, MIrowref ) || 0),
			slotActualQty = (attrLookup( charCS, [fields.MI_qty[0], 'max'], fields.MI_table, MIrowref ) || 0),
			slotCost = (attrLookup( charCS, fields.MI_cost, fields.MI_table, MIrowref ) || 0),
	...
	var makeShortPOPmenu = function( args, senderId, menuType ) {
		...
		putRow = -1;
		i = 0;
		putMInames = getTable( putCS, fields.MI_table, fields.MI_name );
		if (pickRow >= 0) {
			pickedMI = attrLookup( pickCS, fields.MI_name, fields.MI_table, pickRow ) || '';
			pickedTrueMI = attrLookup( pickCS, fields.MI_trueName, fields.MI_table, pickRow ) || '';
			pickableQty = attrLookup( pickCS, fields.MI_qty, fields.MI_table, pickRow ) || '';
			pickedType = attrLookup( pickCS, fields.MI_type, fields.MI_table, pickRow ) || '';
			putMItrueNames = getTable( putCS, fields.MI_table, fields.MI_trueName );
			putMItypes = getTable( putCS, fields.MI_table, fields.MI_type );
			while (i < putMInames.sortKeys.length && putRow < 0) {
				mi = tableLookup(putMInames,i,fields.MI_name);
				miTrueName = tableLookup(putMItrueNames,i,fields.MI_trueName) || '-';
				miType = tableLookup(putMItypes,i,fields.MI_type) || '';
				if (_.isUndefined(mi)) {break;};
...
//		}
	...
	var handleRest = function( args ) {
				...
				while (levelSpells[level].spells > 0) {
					...
					for (w = 1; (w <= fields.SpellsCols) && (levelSpells[level].spells > 0); w++) {
						...
						if (_.isUndefined(spellObjs[w].spellObjs)) {
							spellObjs[w].castValue = getTable( charCS, fields.Spells_table, fields.Spells_castValue, c );
						}
						valueObj = tableLookup( spellObjs[w].castValue, r );
						...
						if (restType.includes('POWER')) {
							if (_.isUndefined(spellObjs[w].castMax)) {
								spellObjs[w].castMax = getTable( charCS, fields.Spells_table, fields.Spells_castMax, c );
							}
							valueObj.set( fields.Spells_castValue[1], tableLookup( spellObjs[w].castMax, r, fields.Spells_castMax ));
						} else {
		...
		if (isMI) {
			let MIspeeds = getTable( charCS, fields.MI_table, fields.MI_speed ),
				MIqtys = getTable( charCS, fields.MI_table, fields.MI_qty );
			if (!fields.MI_table[1]) {
				setAttr( charCS, [fields.MI_speed[0], 'current'], attrLookup( charCS, [fields.MI_speed[0], 'max']));
				setAttr( charCS, [fields.MI_qty[0], 'current'], attrLookup( charCS, [fields.MI_qty[0], 'max']));
			}
			for (r = 0; (r < MIspeeds.sortKeys.length); r++) {
				let miSpeedObj = tableLookup( MIspeeds, r ),
					miQtyObj = tableLookup( MIqtys, r );
	...
	var handleViewUseMI = function( args, isSilent, charges=1 ) {
		...
		if (isNaN(MIrowref) || (fields.MI_table[1] && MIrowref < 0)) {
		...
		var charName = charCS.get('name'),
			MIname = attrLookup( charCS, fields.MI_name, fields.MI_table, MIrowref ),
			MIqtyObj = attrLookup( charCS, [fields.MI_qty[0]], fields.MI_table, MIrowref ),
			MIqty = MIqtyObj.get(fields.MI_qty[1]),
			MItype = (attrLookup( charCS, fields.MI_type, fields.MI_table, MIrowref ) || 'uncharged').toLowerCase(),
	...
	**/
	...
	var handlePickOrPut = function( args ) {
		...
		var	toSlotType = (attrLookup( toCS, fields.MI_type, fields.MI_table, toRowRef ) || '').toLowerCase(),
			fromSlotType = (attrLookup( fromCS, fields.MI_type, fields.MI_table, fromRowRef ) || '').toLowerCase(),
			MIname = attrLookup( fromCS, fields.MI_name, fields.MI_table, fromRowRef ),
			MItrueName = attrLookup( fromCS, fields.MI_trueName, fields.MI_table, fromRowRef ),
		    toSlotName = attrLookup( toCS, fields.MI_name, fields.MI_table, toRowRef ),
			toSlotTrueName = attrLookup( toCS, fields.MI_trueName, fields.MI_table, toRowRef ),
		...
		var	MIqty = parseInt( (attrLookup( fromCS, fields.MI_qty, fields.MI_table, fromRowRef ) || 0), 10),
			MItrueQty = attrLookup( fromCS, fields.MI_trueQty, fields.MI_table, fromRowRef ),
			MIspeed = attrLookup( fromCS, fields.MI_speed, fields.MI_table, fromRowRef ),
			MItrueSpeed = attrLookup( fromCS, fields.MI_trueSpeed, fields.MI_table, fromRowRef ),
		    MItype = attrLookup( fromCS, fields.MI_type, fields.MI_table, fromRowRef ),
		    MIcost  = parseFloat(attrLookup( fromCS, fields.MI_cost, fields.MI_table, fromRowRef ) || 0),
    		toSlotQty = parseInt((attrLookup( toCS, fields.MI_qty, fields.MI_table, toRowRef ) || 0),10),
	    	toSlotCharges = parseInt((attrLookup( toCS, fields.MI_trueQty, fields.MI_table, toRowRef ) || 0),10),
		...
		setAttr( toCS, fields.MI_name, MIname, fields.MI_table, toRowRef );
		setAttr( toCS, fields.MI_trueName, MItrueName, fields.MI_table, toRowRef);
		setAttr( toCS, fields.MI_qty, finalQty, fields.MI_table, toRowRef );
		setAttr( toCS, fields.MI_trueQty, finalCharges, fields.MI_table, toRowRef );
		setAttr( toCS, fields.MI_speed, MIspeed, fields.MI_table, toRowRef );
		setAttr( toCS, fields.MI_trueSpeed, MItrueSpeed, fields.MI_table, toRowRef );
		setAttr( toCS, fields.MI_cost, 0, fields.MI_table, toRowRef );
		setAttr( toCS, fields.MI_type, MItype, fields.MI_table, toRowRef );
		...
		if (MIqty == 0) {
			...
			setAttr( fromCS, fields.MI_trueQty, (MItrueQty - charges), fields.MI_table, fromRowRef );
			setAttr( fromCS, fields.MI_qty, (MIqty - qty), fields.MI_table, fromRowRef );
		};
	...
	var handleSelectSlot = function( args, GMonly, senderId ) {
		...
		setAttr( charCS, ['MIrowref', 'current'], MIrowref );
		setAttr( charCS, ['MIcur', 'current'], (slotItem = (attrLookup( charCS, fields.MI_name, fields.MI_table, MIrowref ) || '-')) );
		setAttr( charCS, ['MIcurqty', 'current'], (attrLookup( charCS, fields.MI_qty, fields.MI_table, MIrowref ) || 0 ) );
		setAttr( charCS, ['foundtype', 'current'], (attrLookup( charCS, fields.MI_type, fields.MI_table, MIrowref ) || 'uncharged' ) );
		setAttr( charCS, ['expenditure', 'current'], (attrLookup( charCS, fields.MI_cost, fields.MI_table, MIrowref ) || 0 ) );
		setAttr( charCS, ['MI-chosen', 'current'], 1 );
	...
	var handleStoreMI = function( args, GMonly, senderId ) {
		...
		var slotName = attrLookup( charCS, fields.MI_name, fields.MI_table, MIrowref ) || '-',
			slotType = attrLookup( charCS, fields.MI_type, fields.MI_table, MIrowref ) || 'uncharged',
		...
		setAttr( charCS, fields.MI_name, MIchosen, fields.MI_table, MIrowref );
		setAttr( charCS, fields.MI_trueName, MIchosen, fields.MI_table, MIrowref );
		setAttr( charCS, fields.MI_speed, MIspeed, fields.MI_table, MIrowref );
		setAttr( charCS, fields.MI_trueSpeed, MIspeed, fields.MI_table, MIrowref );
		setAttr( charCS, fields.MI_qty, MIqty, fields.MI_table, MIrowref );
		setAttr( charCS, fields.MI_trueQty, MIqty, fields.MI_table, MIrowref );
		setAttr( charCS, fields.MI_cost, 0, fields.MI_table, MIrowref );
		setAttr( charCS, fields.MI_type, MItype, fields.MI_table, MIrowref );
	...
	var handleRemoveMI = function( args, GMonly, senderId ) {
		...
		var slotType = attrLookup( charCS, fields.MI_type, fields.MI_table, MIrowref ) || '';
	...
	var handleChangeMItype = function( args ) {
		...
		var MIname = attrLookup( charCS, fields.MI_name, fields.MI_table, MIrowref ) || '-';

		setAttr( charCS, fields.MI_type, newType, fields.MI_table, MIrowref );
	...
	var handleChangeMIcharges = function( args, changeType ) {
		...
		if (changeType == 'Displayed' || changeType == 'Both') {
			setAttr( charCS, [fields.MI_qty[0], 'current'], MInewQty, fields.MI_table, MIrowref );
		}
		if (changeType == 'Actual' || changeType == 'Both') {
			setAttr( charCS, [fields.MI_qty[0], 'max'], MInewQty, fields.MI_table, MIrowref );
		}
	...
	var handleResetSingleMI = function( args ) {
		...
		var MIname = attrLookup( charCS, [fields.MI_name[0], 'max'], fields.MI_table, MIrowref ) || '-';

		setAttr( charCS, [fields.MI_name[0], 'current'], MIname, fields.MI_table, MIrowref );
		setAttr( charCS, [fields.MI_qty[0], 'current'], attrLookup( charCS, [fields.MI_qty[0], 'max'], fields.MI_table, MIrowref ), fields.MI_table, MIrowref );
		setAttr( charCS, [fields.MI_speed[0], 'current'], (attrLookup( charCS, [fields.MI_speed[0], 'max'], fields.MI_table, MIrowref ) || 0), fields.MI_table, MIrowref );
	...
	var handleSetMIcost = function( args ) {
		...
		setAttr( charCS, fields.MI_cost, newMIcost, fields.MI_table, MIrowref );
	...
	var handleConfirmedBlank = function( args ) {
		...
		var MInames = getTable( charCS, fields.MI_table, fields.MI_name ),
			MIspeeds = getTable( charCS, fields.MI_table, fields.MI_speed ),
			MIqtys = getTable( charCS, fields.MI_table, fields.MI_qty ),
			MIcosts = getTable( charCS, fields.MI_table, fields.MI_cost ),
			MItype = getTable( charCS, fields.MI_table, fields.MI_type ),
			miNameObj, miSpeedObj, miQtyObj, miCostObj, miTypeObj;
		for (let i=0; i<fields.MIRows; i++) {
			miNameObj = tableLookup( MInames, i );
			miSpeedObj = tableLookup( MIspeeds, i );
			miQtyObj = tableLookup( MIqtys, i );
			miCostObj = tableLookup( MIcosts, i );
			miTypeObj = tableLookup( MItypes, i );
			...
			miQtyObj.set('max','0');
			miCostObj(fields.MI_cost[1],0);
			miTypeObj(fields.MI_type[1],'');
		}
	...
	var doChangeCharges = function( args ) {
		...
		if (MIname && MIname.length>0) {
			for (i=0; i<fields.MIRows; i++) {
				r = (fields.MI_table[1] ? i : i-1);
				if (MIname == attrLookup( charCS, fields.MI_name, fields.MI_table, r )) {
					break;
				}
			}
		}
		...
		switch (command) {
		case 'ADD':
		    i = parseInt(attrLookup( charCS, fields.MI_qty, fields.MI_table, r ) || 0);
			charges = charges + i;
		case 'SET':
			if (!isNaN(maxCharges) && maxCharges < charges) {
				charges = maxCharges;
			}
			setAttr( charCS, fields.MI_trueQty, charges, fields.MI_table, r );
			break;
		case 'DEDUCT':
