...
var magicMaster = (function() {
	...
	var init = function() {
		...
		var miDB = findObjs({ _type: 'character' , name: fields.MagicItemDB });
       		 state.magicMaster.miDB = false;
		if (miDB) {
			if (miDB.length > 0) {
				state.magicMaster.miDB = miDB[0];
			}
		}
		
		// RED: v2.001 update the global ID of the Powers, MU and Priest Spell DB Libraries
		var muDB = findObjs({ _type: 'character' , name: fields.MU_SpellsDB }),
       	...
       	state.magicMaster.muDB = false;
       	state.magicMaster.prDB = false;
		state.magicMaster.pwDB = false;
		if (muDB && muDB.length > 0) {
			state.magicMaster.muDB = muDB[0];
		}
		if (prDB && prDB.length > 0) {
			state.magicMaster.prDB = prDB[0];
		}
		if (pwDB && pwDB.length > 0) {
			state.magicMaster.pwDB = pwDB[0];
		}
...
// -------------------------------------------- utility functions ----------------------------------------------
    ...
    */
    ...
    var checkNewMIBag = function(charID, tokenID) {
        ...
        if (!charID || !charCS) {
            sendParsedMsg( charCS, messages.noChar, '', tokenName );
            return false;
        }
        ...
        if (MIBagType != MIB_StateEnum.V4BAG) {
            sendParsedMsg( 'MIBag', ((MIBagType === MIB_StateEnum.NOBAG) ? messages.noMIBag : messages.oldMIBag), tokenID );
            return false;
        }
        return true;
    };
...
//        sendChat( fields.feedbackName, content );
	...
	var makeManageSpellsMenu = function( args, msg ) {
		...
		content = '&{template:2Edefault}{{name=Select Slot to Memorise in '+tokenName+'\'s '+(isMU ? 'MU' : 'Priest')+' Spellbook}}'
				+ '{{subtitle=Level '+level+' spells}}';
				
		if (msg && msg.length > 0) {
			content += '{{='+msg+'}}';
		}
					
		content += '{{desc=1. [Choose](!magic --button '+(isMU ? BT.EDIT_MUSPELLS : BT.EDIT_PRSPELLS)+'|'+tokenID+'|'+level+'|'+spellRow+'|'+spellCol+'|?{Spell to memorise|'+spellbook+'}) spell to memorise<br>'
				+ '...Optionally [Review](!magic --button '+(isMU ? BT.REVIEW_MUSPELL : BT.REVIEW_PRSPELL)+'|'+tokenID+'|'+level+'|'+spellRow+'|'+spellCol+'|'+spellToMemorise+') the spell<br>'
				+ '}}{{desc1=2. Choose slot to use\n'
				+ makeNumberOfSpells(curToken,isMU,level,levelSpells[level].spells)+'\n';
		...
		while (levelSpells[level].spells > 0) {
			...
			for (w = 1; (w <= fields.SpellsCols) && (levelSpells[level].spells > 0); w++) {
				firstCol = (fields.SpellsFirstColNum || level != 1 || c != 1);
				selectedSlot = (r == spellRow && c == spellCol);
				repSpellField = fields.Spells_table[0] + ( firstCol ? c : '' ) + '_$' + r + '_' + fields.Spells_name[0] + ( firstCol ? c : '' );
				content += (selectedSlot ? ('<span style=' + design.selected_button + '>') : '[');
				content += attrLookup( charCS, repSpellField , fields.Spells_name[1]);
				content += (selectedSlot ? '</span>' : '](!magic --button ' + (isMU ? BT.EDIT_MUSPELLS : BT.EDIT_PRSPELLS) + '|' + tokenID + '|' + level + '|' + r + '|' + c + '|' + spellToMemorise + ')');
				c++;
		...
		selectedSpell = (spellRow >= 0 && spellCol >= 0 && spellToMemorise.length > 0);
		selectedSlot = (spellRow >= 0 && spellCol >= 0);
		if (selectedSlot) {
			firstCol = (fields.SpellsFirstColNum || level != 1 || spellCol != 1);
		    slotSpell = attrLookup( charCS, fields.Spells_table[0]+(firstCol ? spellCol : '')+'_$'+spellRow+'_'+fields.Spells_name[0] + (firstCol ? spellCol : ''), fields.Spells_name[1] ) || '';
		}
		content += '}}{{desc2=...Then\n'
				+  '3. '+(selectedSpell ? '[' : '<span style='+design.grey_button+'>')
				+		'Memorise '+spellToMemorise
				+		 (!selectedSpell ? '</span>' : ('](!magic --button '+(isMU ? BT.MEM_MUSPELL : BT.MEM_PRSPELL)+'|'+tokenID+'|'+level+'|'+spellRow+'|'+spellCol+'|'+spellToMemorise+')'))+' the spell\n'
				+  '4. When ready [Go to Level '+nextLevel+'](!magic --button '+(isMU ? BT.EDIT_MUSPELLS : BT.EDIT_PRSPELLS)+'|'+tokenID+'|'+nextLevel+'|-1|-1|)\n'
				+  'Or just do something else anytime\n\n'

				+  'Or ' + (selectedSlot ? '[' : '<span style='+design.grey_button+'>')
				+  'Remove '+slotSpell
				+  (!selectedSlot ? '</span>' : ('](!magic --button '+(isMU ? BT.MEM_MUSPELL : BT.MEM_PRSPELL)+'|'+tokenID+'|'+level+'|'+spellRow+'|'+spellCol+'|-)') )+' the spell}}';
		...
		for (l = 1; l <= (isMU ? 9 : 7); l++) {
			...
			while (levelSpells[l].spells > 0) {
				...
				for (w = 1; (w <= fields.SpellsCols) && (levelSpells[l].spells > 0); w++) {
					...
					repSpellField = fields.Spells_table[0] + ( firstCol ? c : '' ) + '_$' + r + '_' + fields.Spells_name[0] + ( firstCol ? c : '' );
					content += (buttonID == spellButton ? '<span style=' + design.selected_button + '>' : (submitted ? '<span style=' + design.grey_button + '>' : '['));
					content += attrLookup( charCS, repSpellField , fields.Spells_name[1]);
					content += (((buttonID == spellButton) || submitted) ? '</span>' : '](!magic --button ' + (isMU ? BT.MU_SPELL : BT.PR_SPELL) + '|' + tokenID + '|' + buttonID + '|' + r + '|' + c + ')');
					buttonID++;
					c++;
	...
	var makeViewMemSpells = function( args ) {
		...
		content = '&{template:2Edefault}{{name=View ' + tokenName + '\'s currently memorised spells}}'
				+ '{{subtitle=' + (isMU ? 'MU' : 'Priest') + ' spells}}'
				+ '{{desc=';
		...
		for (l = 1; l <= (isMU ? 9 : 7); l++) {
			...
			while (levelSpells[l].spells > 0) {
				...
				for (w = 1; (w <= fields.SpellsCols) && (levelSpells[l].spells > 0); w++) {
					firstCol = (fields.SpellsFirstColNum || l != 1 || c != 1);
					repSpellField = fields.Spells_table[0] + ( firstCol ? c : '' ) + '_$' + r + '_' + fields.Spells_name[0] + ( firstCol ? c : '' );
					spellName= attrLookup( charCS, repSpellField , fields.Spells_name[1]);
					content += (buttonID == spellButton ? '<span style=' + design.selected_button + '>' : '[');
					content += spellName
					content += (buttonID == spellButton ? '</span>' : '](!magic --button ' + (isMU ? BT.VIEW_MUSPELL : BT.VIEW_PRSPELL) + '|' + tokenID + '|' + buttonID + '|' + r + '|' + c + '|' + spellName + ')');
					buttonID++;
					c++;
					levelSpells[l].spells--;
				}
		...
		content += '}}{{desc1=Select the spell above that you want to view the details of.  It will not be cast and will remain in your memorised spell list.}}';
		sendResponse( charCS, content );
	};
...
// ------------------------------------------------------------ Menu Button Press Handlers --------------------------------------------
	...
	/**
	* Handle the results of pressing a spell-casting button
	**/
	...
	var handleChooseSpell = function( args ) {
		...
		spellCost = attrLookup( charCS, repSpellField + fields.Spells_cost[0] + ( colNum ? colIndex : '' ), fields.Spells_cost[1] ) || 0;
		...
		sendResponse( charCS, makeCastSpellMenu( args ) );
		return;
	...
	var handleCastSpell = function( args ) {
		...
		var colNum = (fields.SpellsFirstColNum || colIndex != 1),
			spellRef = fields.Spells_table[0] + ( colNum ? colIndex : '' ) + '_$' + rowIndex + '_',
			spellName = attrLookup( charCS, spellRef+fields.Spells_name[0]+(colNum?colIndex:''), fields.Spells_name[1] ) || '-',
			spellCost = attrLookup( charCS, spellRef+fields.Expenditure[0]+(colNum?colIndex:''), fields.Expenditure[1] ) || 0,
			charName = charCS.get('name'),
			totalLeft,
			content;
			
		setAttr( charCS, spellRef+fields.Spells_name[0]+(colNum?colIndex:''),fields.Spells_name[1], '-' );
		
		totalLeft = spendMoney( charCS, spellCost );
		
		content = charName + ' is casting [' + spellName + '](!&#13;&#47;w gm &#37;{'+(isMU ? 'MU' : 'PR')+'-Spells-DB|' + spellName + '})'
				+ ' at a cost of [[' + spellCost + ']] (leaving [[' + totalLeft + ']]GP).'
				+ '  Select ' + charName + '\'s token before pressing to see effects.';
		sendFeedback( content );

		if (isMU) {
			sendAPImagic( tokenID, state.magicMaster.muDB, spellName );
		} else {
			sendAPImagic( tokenID, state.magicMaster.prDB, spellName );
		}
	...
	var handleReviewSpell = function( args ) {
		...
		if (isView) {
			followOn = (isMU ? 'VIEWMEM_MUSPELLS' : 'VIEWMEM_PRSPELLS');
		} else {
			followOn = (isMU ? 'EDIT_MUSPELLS' : 'EDIT_PRSPELLS');
		}
		
		msg = '\n/w "'+charCS.get('name')+'" [Return to menu](!magic --button '+followOn+'|'+args[1]+'|'+args[2]+'|'+args[3]+'|'+args[4]+'|'+args[5]+')';
		
		if (isMU) {
			sendAPImagic( tokenID, state.magicMaster.muDB, spellName, msg );
		} else {
			sendAPImagic( tokenID, state.magicMaster.prDB, spellName, msg );
		}
		return;
	}
	...
	var handleMemoriseSpell = function( args ) {
		...
		var dbCS = (isMU ? state.magicMaster.muDB : state.magicMaster.prDB),
			castTime = attrLookup( dbCS, 'ct-'+spellName, 'current' ) || 0,
			castCost = attrLookup( dbCS, 'ct-'+spellName, 'max' ) || 0,
			firstCol = (fields.SpellsFirstColNum || level != 1 || col != 1),
			tableRef = fields.Spells_table[0] + (firstCol ? col : '') + '_$' + row + '_';
		
		setAttr( charCS, tableRef + fields.Spells_name[0] + (firstCol ? col : ''), 'current', spellName );
		setAttr( charCS, tableRef + fields.Spells_name[0] + (firstCol ? col : ''), 'max', spellName );
		setAttr( charCS, tableRef + fields.Spells_speed[0] + (firstCol ? col : ''), fields.Spells_speed[1], castTime );
		setAttr( charCS, tableRef + fields.Spells_cost[0] + (firstCol ? col : ''), fields.Spells_cost[1], castCost );
		setAttr( charCS, tableRef + fields.Spells_castValue[0] + (firstCol ? col : ''), fields.Spells_castValue[1], 1 );
		setAttr( charCS, tableRef + fields.Spells_castMax[0] + (firstCol ? col : ''), fields.Spells_castMax[1], 1 );
	...
	**/
	...
	var handlePPfailed = function( args, senderId ) {
		...
		targetLevel = parseInt((attrLookup( targetCS, fields.Fighter_level[0], fields.Fighter_level[1] ) || 0), 10);
		targetLevel += parseInt((attrLookup( targetCS, fields.Wizard_level[0], fields.Wizard_level[1] ) || 0), 10);
		targetLevel += parseInt((attrLookup( targetCS, fields.Priest_level[0], fields.Priest_level[1] ) || 0), 10);
		targetLevel += parseInt((attrLookup( targetCS, fields.Rogue_level[0], fields.Rogue_level[1] ) || 0), 10);
		targetLevel += parseInt((attrLookup( targetCS, fields.Psion_level[0], fields.Psion_level[1] ) || 0), 10);
	...
	**/
	...
	var handleSelectMI = function( args, GMonly ) {
		...
		setAttr( charCS, 'MIct', 'current', (attrLookup( state.magicMaster.miDB, 'ct-'+MItoStore, 'current' ) || 0) );
		setAttr( charCS, 'MItype', 'current', (attrLookup( state.magicMaster.miDB, 'ct-'+MItoStore, 'max' ) || 'uncharged') );
	...
	var handleStoreMI = function( args, GMonly ) {
		...
		var slotRef = fields.MI_table[0] + '_$' + MIrowref + '_',
			...
			MIspeed = attrLookup( state.magicMaster.miDB, 'ct-'+MIchosen, 'current' ),
			MItype = attrLookup( state.magicMaster.miDB, 'ct-'+MIchosen, 'max' );
...
// ------------------------------------------------------------- Command Action Functions ---------------------------------------------
	...
	var doCastSpell = function( args, spellCasterType ) {
		...
		args = args.split('|');
		if (args.length < 1) {
			sendDebug('doCastSpell: invalid arguments, missing token_id');
	...
	var doMemoriseSpells = function( args, spellCasterType ) {
		...
		args = args.split('|');
		if (args.length < 1) {
			sendDebug('doMemoriseSpells: invalid arguments, missing token_id');
			sendError('Incorrect magicMaster syntax');
			return;
		}
		...
		var tokenID = args[0],
			curToken = getObj('graphic',tokenID),
			isMU = spellCasterType.includes('MU'),
			charCS = getCharacter(tokenID);
		...
		args.unshift( spellCasterType );
		args.push(1,-1,-1,'');
		setAttr( charCS, fields.Casting_level[0], fields.Casting_level[1], (attrLookup( charCS, (isMU ? fields.Wizard_level[0] : fields.Priest_level[0]), (isMU ? fields.Wizard_level[0] : fields.Priest_level[0]) ) || 0) );
	...
	var doViewMemorisedSpells = function( args, spellCasterType ) {
	...
	*/
    ...
    var doMIBagMenu = function( args, senderId ) {
        ...
        if (checkNewMIBag( charID, tokenID )) {
...
// -------------------------------------------------------- Event Handlers --------------------------------------------------
	...
	var handleChatMessage = function(msg) { 
		...
		_.each(args, function(e) {
			...
			// type commands
    		if (msg.type === 'api') {
    	    	if (arg.indexOf('cast-muspell ') === 0) {
    	    	    arg = arg.replace('cast-muspell ','').trim();
        			doCastSpell(arg,BT.MU_SPELL);
	    		} else if (arg.indexOf('cast-prspell ') === 0) {
	    			arg = arg.replace('cast-prspell ','').trim();
					doCastSpell(arg,BT.PR_SPELL);
    	    	} else if (arg.indexOf('mem-muspell ') === 0) {
    	    	    arg = arg.replace('mem-muspell ','').trim();
        			doMemoriseSpells(arg,BT.EDIT_MUSPELLS);
    	    	} else if (arg.indexOf('mem-prspell ') === 0) {
    	    	    arg = arg.replace('mem-prspell ','').trim();
        			doMemoriseSpells(arg,BT.EDIT_PRSPELLS);
    	    	} else if (arg.indexOf('view-muspell ') === 0) {
    	    	    arg = arg.replace('view-muspell ','').trim();
        			doViewMemorisedSpells(arg,BT.VIEWMEM_MUSPELLS);
    	    	} else if (arg.indexOf('view-prspell ') === 0) {
    	    	    arg = arg.replace('view-prspell ','').trim();
        			doViewMemorisedSpells(arg,BT.VIEWMEM_PRSPELLS);
    	    	...
    	    	} else if (arg.indexOf('gm-edit-mi ') === 0 && (playerIsGM(senderId) || state.MIBag.debug === senderId)) {
