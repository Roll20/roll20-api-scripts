...
//		log('makeManageSpellsMenu: isMI='+isMI+', isPower='+isPower+', is miDB='+(charCS.id == dB.miDB.id)+', no of spells='+levelSpells[1].spells);
	...
	**/
	...
	var makeViewUseMI = function( args, isGM ) {
		...
		var	actionText = (isView ? 'View' : 'Use'),
			selectAction = (isView ? BT.VIEW_MI : BT.CHOOSE_USE_MI),
		...
		for (let i = 0; i < fields.MIRows; i++) {
			...
			if (mi != '-') {
    			...
    			content += qty + ' ' + mi;
    			if (i != MIrowref && qty > 0) {
					magicItem = abilityLookup( fields.MagicItemDB, mi );
    				content += '](!magic --button '+selectAction+'|' + tokenID + '|' + i + (isView ? ('\n&#37;{'+magicItem.dB+'|'+mi+'})') : ')');
    			} else {
    				if (i == MIrowref) {
    					selectedMI = mi;
    				}
		...
		if (!isView) {
			...
			content	+= '{{desc2='+(MIrowref < 0 ? ('<span style='+design.grey_button+'>') : '[')
