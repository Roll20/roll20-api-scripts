...
// -------------------------------------------- utility functions ----------------------------------------------
	...
	**/
	...
	var abilityLookup = function( rootDB, ability ) {
		
		abilityObj = filterObjs( function(objs) {
			if (objs.get('type') !== 'ability' || objs.get('name') !== ability) return false;
			let magicDB = getObj( 'character', objs.get('characterid'));
			if (!magicDB || magicDB.get('name').indexOf(rootDB) !== 0) return false;
			return !(/\s*v\d*\.\d*/i.test(magicDB.get('name')));
		}
		if (!abilityObj or abilityObj.length === 0) return undefined;
		ctObj = findObjs({ type: 'attribute',
						   name: 'ct-'+ability,
						   characterid: abilityObj.get('characterid')
						});
		return {ability: abilityObj, ct: ctObj };
	}
...
//		}
	...
	var handleViewUseMI = function( args, isGM ) {
		...
		var content = '&{template:2Edefault}{{name='+charName+' is using '+MIname+'}}'
					+ '{{desc=To see the effects, select '+charName+'\'s token and press ['+MIname+'](~'+fields.MagicItemDB+'|'+MIname+')}}';
		sendFeedback( content );
		return;
	}
