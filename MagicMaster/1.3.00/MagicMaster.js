...
var MagicMaster = (function() {
	...
	MagicMaster_Help:	{name:'MagicMaster Help',
						 ...
						 bio:'<div style="font-weight: bold; text-align: center; border-bottom: 2px solid black;">'
							...
							+'<p>This command tidies up the character sheet, removing Spell and Magic Item attribute and ability objects that are no longer for items held and spells no longer in any spell book.  Attack ability objects will also all be removed.  All of these will be recreated as and when these items, spells or attacks are again picked up, added to spell books, or used for attacks. This simplifies and speeds up the system, removing redundant processing and memory usage.</p>'
...
/* ------------------------------- Magic Utility functions ----------------------------- */
    ...
    **/
	...
	var setSpell = function( charCS, spellTables, spellDB, spellName, r, c, cost, msg, levelOrPerDay ) {
		...
		var newSpellObj = abilityLookup( spellDB, spellName, charCS, true );
...
// ---------------------------------------------------- Make Menus ---------------------------------------------------------
			...
			while (levelSpells[l].spells > 0) {
				...
				for (let w = 1; (w <= fields.SpellsCols) && (levelSpells[l].spells > 0); w++) {
					if (_.isUndefined(spellTables[w])) {
						...
						spellTables[w] = getTableField( charCS, spellTables[w], fields.Spells_table, fields.Spells_castValue, c, '0' );
						spellTables[w] = getTableField( charCS, spellTables[w], fields.Spells_table, fields.Spells_db, c, '' );
						spellTables[w] = getTableField( charCS, spellTables[w], fields.Spells_table, fields.Spells_msg, c, '0' );
					}
	...
	var makeManageSpellsMenu = function( args, senderId, msg ) {
		...
		if (selectedSpell) {
			spell = getAbility( magicDB, spellToMemorise, charCS );
			content += '...Optionally [Review '+spellToMemorise+'](!magic --button '+reviewCmd+'|'+tokenID+'|'+level+'|'+spellRow+'|'+spellCol+'|'+spellToMemorise 
					+  '&#13;'+(spell.api ? '' : sendToWho(charCS,senderId,false,true))+'&#37;{'+spell.dB+'|'+spellToMemorise+'})}}';
		} else {
	...
	**/
	...
	var handleCastSpell = function( args, senderId ) {
		...
		var	spell = getAbility( db, spellName, charCS ),
			spellCost = ((!!spell.ct && (args[0] != BT.CAST_MISPELL) && (args[0] != BT.CAST_MIPOWER) && (args[0] != BT.CAST_SCROLL)) ? spell.obj[1].cost : 0),
			totalLeft,
			content,
	...
	var handleSelectMIpower = function( args, isUse, senderId ) {
		...
		var powerName = args[2] || '',
			...
			powerLib = getAbility( fields.PowersDB, power, charCS ),
		...
		if (isUse) {
			...
			makeUseMIpowerMenu( args, senderId );
		}
	    return;
	}
	...
	**/
	...
	var handleRenameItem = function( args, senderId ) {
		...
		if (!MInewName || !MInewName.length) {
			...
			if (MUspellObj) MUspellObj.set('name',MUspellObj.get('name').replace(MIoldName,MInewName));
			if (PRspellObj) PRspellObj.set('name',PRspellObj.get('name').replace(MIoldName,MInewName));
			if (powerObj) powerObj.set('name',powerObj.get('name').replace(MIoldName,MInewName));
	...
	var handleCStidy = function( tokenList, silent=false ) {
		
		var Items, i, objList, itemName, charCS, itemList,
			namesList = {},
			spellsList = '';
		...
		for (const token of tokenList) {
			...
			if (!charCS || !_.isUndefined(namesList[charCS.id])) continue;
			Items = getTableField( charCS, {}, fields.Items_table, fields.Items_name );
			...
			for (const caster in spellLevels) {
				for (const l of caster) {
					if (l.book != 0) {
	...
	*/
	...
	var doWriteDB = function(args) {
		...
		if (dbName && dbName.length) {
			...
			sendFeedback('Updating all handouts',flags.feedbackName);
