	...
	var makeEditBagMenu = function(senderId,args,menuType) {
		...
		if (!shortMenu || !selected) {
			content += '{{desc=**1.Choose what item to store**\n'
					+  '[Potion](!magic --button '+BT.CHOOSE_MI+'|'+charID+'|'+MIrowref+'|?{Potion to store|'+getMagicList(fields.MagicItemDB,fields.MI_PotionList)+'}|'+charges+')'
					+  '[Scroll](!magic --button '+BT.CHOOSE_MI+'|'+charID+'|'+MIrowref+'|?{Scroll to store|'+getMagicList(fields.MagicItemDB,fields.MI_ScrollList)+'}|'+charges+')'
					+  '[Rods, Staves, Wands](!magic --button '+BT.CHOOSE_MI+'|'+charID+'|'+MIrowref+'|?{Rod Staff Wand to store|'+getMagicList(fields.MagicItemDB,fields.MI_WandsList)+'}|'+charges+')'
					+  '[Weapon](!magic --button '+BT.CHOOSE_MI+'|'+charID+'|'+MIrowref+'|?{Weapon to store|'+getMagicList(fields.MagicItemDB,fields.MI_WeaponList)+'}|'+charges+')'
					+  '[Armour](!magic --button '+BT.CHOOSE_MI+'|'+charID+'|'+MIrowref+'|?{Armour to store|'+getMagicList(fields.MagicItemDB,fields.MI_ArmourList)+'}|'+charges+')'
					+  '[Ring](!magic --button '+BT.CHOOSE_MI+'|'+charID+'|'+MIrowref+'|?{Ring to store|'+getMagicList(fields.MagicItemDB,fields.MI_RingList)+'}|'+charges+')'
					+  '[Miscellaneous](!magic --button '+BT.CHOOSE_MI+'|'+charID+'|'+MIrowref+'|?{Misc Item to store|'+getMagicList(fields.MagicItemDB,fields.MI_MiscList)+'}|'+charges+')';
            if (shortMenu) {
			content +=  '\n**OR**\n'
