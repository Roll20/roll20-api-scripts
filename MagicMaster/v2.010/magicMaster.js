...
//        		sendDebug('sendAPImacro: macroBody is ' + macroBody );
    ...
    */
	...
	var pickupMessage = function( args, miName, miType, pickedQty, fromCharges, toCharges ) {
		...
		content += '.}}{{desc1=[Pick or put another MI](!magic --pickorput '+charID+'|'+fromID+'|'+toID+')}}';
...
// -------------------------------------------- utility functions ----------------------------------------------
	...
	**/
	...
	var abilityLookup = function( rootDB, abilityName ) {
        ...
        var dBname,
            ...
            abilityObj = findObjs({type: 'ability', name: abilityName}, {caseInsensitive:true})
                            .filter(function(obj) {
                                let magicDB = getObj('character',obj.get('characterid'));
                                if (_.isUndefined(magicDB)) return false;
                                let magicName = magicDB.get('name').toLowerCase();
	...
	var getMagicList = function( rootDB, listAttr, listField ) {
		...
		filterObjs( function(objs) {
			...
			if ((newList = (attrLookup( objs, listAttr, listField ) || '').trim())) magicList += (magicList.length !== 0 ? '|' : '') + newList;
			return true;
		});
	...
	var howMany = function( args, MIname, MItype, MIqty ) {
		...
		var charID = args[1],
			...
			content = '&{template:2Edefault}{{name=How Many Items?}}'
					+ '{{desc=How many '+MIname+' do you want to '+(charID == toID ? 'take' : 'put away')+'?}}'
...
//        sendChat( fields.feedbackName, content );
	...
	var makeSpellList = function( curToken, command, selectedButton, submitted = false, extension = '' ) {
		
		var content = '',
			buttonID = 0,
			charCS = getCharacter(curToken.id),
			spellType = (command.includes('MI') ? 'MI' : (command.includes('POWER') ? 'POWER' : (command.includes('MU') ? 'MU' : 'PR' ))),
			...
			while (levelSpells[l].spells > 0) {
				...
				for (let w = 1; (w <= fields.SpellsCols) && (levelSpells[l].spells > 0); w++) {
					let col = (fields.SpellsFirstColNum || c != 1) ? c : '',
						...
						spellValue = parseInt(attrLookup( charCS, rep + fields.Spells_castValue[0] + col, fields.Spells_castValue[1]) || 0),
...
//		log('makeManageSpellsMenu: isMI='+isMI+', isPower='+isPower+', is miDB='+(charCS.id == dB.miDB.id)+', no of spells='+levelSpells[1].spells);
		...
		content	+= '{{desc1=2. Choose slot to use\n'
				+  (isPower ? '' : (makeNumberOfSpells(curToken,magicType,level,levelSpells[level].spells)))+'\n';
	...
	var makeStoreMIspell = function(args,msg = '',submitted = false) {
		...
		var isMU = command.includes('MU'),
			...
			content = '&{template:2Edefault}{{name=Store Spell in '+tokenName+'\'s Magic Item}}'
					...
					+ '{{desc=1.Choose a spell to store\n'+( tokenID, (isMU ? BT.MU_TO_STORE : BT.PR_TO_STORE), spellButton, submitted, '|'+MIbutton+'|'+MIrow+'|'+MIcol )+'}}'
					+ '{{desc1=2.Choose where to store it\n'+makeSpellList( tokenID, (isMU ? BT.MU_MI_SLOT : BT.PR_MI_SLOT), MIbutton, submitted, '|'+spellButton+'|'+spellRow+'|'+spellCol )+'}}';
	...
	var makeCastSpellMenu = function( args, submitted = false; ) {
		...
		content = '&{template:2Edefault}{{name=What Spell is ' + tokenName + ' casting?}}'
				...
				+ '{{desc='+makeSpellList( tokenID, (isMU ? BT.MU_SPELL : BT.PR_SPELL), spellButton, submitted );

		if (spellButton >= 0) {
			col = (fields.SpellsFirstColNum || spellCol != 1) ? spellCol : '';
			spellName = attrLookup( charCS, fields.Spells_table[0]+col+'_$'+spellRow+'_'+fields.Spells_name[0]+col, fields.Spells_name[1] ) || '-';
			if (spellName.length > 0) {
				spell = abilityLookup( (isMU ? fields.MU_SpellsDB : fields.PR_SpellsDB), spellName );
			}
		} else {
			spellName = '';
		}
	...
	var makeViewMemSpells = function( args ) {
		...
		for (l = 1; l < levelSpells.length; l++) {
            ...
            if (!isPower && levelSpells[l].spells > 0) {
                if (l != 1 )
	    		    {content += '\n';}
			    content += makeNumberOfSpells(curToken,magicType,l,levelSpells[l].spells)+'\n';
            }
			while (levelSpells[l].spells > 0) {
	...
	/*
	 * Create a menu for a player to use a power
	 */
	...
	var makeUsePowerMenu = function( args, submitted ) {
		...
		if (spellButton >= 0) {
			...
			spellName = attrLookup( charCS, fields.Spells_table[0]+col+'_$'+spellRow+'_'+fields.Spells_name[0]+col, fields.Spells_name[1] ) || '-';
			if (spellName.length > 0) {
				spell = abilityLookup( fields.PowersDB, spellName );
			}
		} else {
			spellName = '';
		}
	...
	**/
	...
	var makeViewUseMI = function( args, isGM ) {
		...
		var	actionText = (action.includes('VIEW') ? 'View' : 'Use'),
			selectAction = (action.includes('VIEW') ? BT.CHOOSE_VIEW_MI : BT.CHOOSE_USE_MI),
			submitAction = (action.includes('VIEW') ? BT.VIEW_MI : BT.USE_MI),
			content = '&{template:2Edefault}{{name='+actionText+' '+charCS.get('name')+'\'s Magic Items}}'
					+ '{{desc=Select a Magic Item below to '+actionText+', and then press the **'+actionText+'** button}}'
					+ '{{desc1=',
			magicItem, qty, mi, selectedMI;
		
		// build the character's visible MI Bag
		for (let i = 0; i < fields.MIRows; i++) {
			
			mi = attrLookup( charCS, fields.MI_table[0] + '_$' + i + '_' + fields.MI_name[0], (isGM ? 'max' : 'current') );
			...
			if (mi != '-') {
    			...
    			content += qty + ' ' + mi;
    			if (i != MIrowref && qty > 0) {
    				content += '](!magic --button '+selectAction+'|' + tokenID + '|' + i + ')';
    			} else {
    				if (i == MIrowref) {
    					selectedMI = mi;
    				}
		...
		content += '}}'
				+ '{{desc2='+(MIrowref < 0 ? ('<span style='+design.grey_button+'>') : '[')
	...
	**/
	...
	var makeEditBagMenu = function(senderId,args,menuType) {
		...
		if (!shortMenu || !selected) {
			content += '{{desc=**1.Choose what item to store**\n'
					+  '[Potion](!magic --button '+BT.CHOOSE_MI+'|'+charID+'|'+MIrowref+'|?{Potion to store|'+getMagicList(fields.MagicItemDB,fields.MI_PotionList[0],fields.MI_PotionList[1])+'}|'+charges+')'
					+  '[Scroll](!magic --button '+BT.CHOOSE_MI+'|'+charID+'|'+MIrowref+'|?{Scroll to store|'+getMagicList(fields.MagicItemDB,fields.MI_ScrollList[0],fields.MI_ScrollList[1])+'}|'+charges+')'
					+  '[Rods, Staves, Wands](!magic --button '+BT.CHOOSE_MI+'|'+charID+'|'+MIrowref+'|?{Rod Staff Wand to store|'+getMagicList(fields.MagicItemDB,fields.MI_WandsList[0],fields.MI_WandsList[1])+'}|'+charges+')'
					+  '[Weapon](!magic --button '+BT.CHOOSE_MI+'|'+charID+'|'+MIrowref+'|?{Weapon to store|'+getMagicList(fields.MagicItemDB,fields.MI_WeaponList[0],fields.MI_WeaponList[1])+'}|'+charges+')'
					+  '[Armour](!magic --button '+BT.CHOOSE_MI+'|'+charID+'|'+MIrowref+'|?{Armour to store|'+getMagicList(fields.MagicItemDB,fields.MI_ArmourList[0],fields.MI_ArmourList[1])+'}|'+charges+')'
					+  '[Ring](!magic --button '+BT.CHOOSE_MI+'|'+charID+'|'+MIrowref+'|?{Ring to store|'+getMagicList(fields.MagicItemDB,fields.MI_RingList[0],fields.MI_RingList[1])+'}|'+charges+')'
					+  '[Miscellaneous](!magic --button '+BT.CHOOSE_MI+'|'+charID+'|'+MIrowref+'|?{Misc Item to store|'+getMagicList(fields.MagicItemDB,fields.MI_MiscList[0],fields.MI_MiscList[1])+'}|'+charges+')';
            if (shortMenu) {
			content +=  '\n**OR**\n'
					+  '[Choose item to Remove](!magic --button '+BT.CHOOSE_MI+'|'+charID+'|'+MIrowref+'|'+'Remove) from your MI bag'
                    +  '}}';
            }
		}
		if (!shortMenu || selected) {
            if (shortMenu) {
				content += '{{desc=1.Item chosen ['+selectedMI+'](!magic --edit-mi '+charID+'), click to reselect\n';
            }
            if (!remove) {
    			content += '\nOptionally, you can '+(selected ? '[' : '<span style='+design.grey_button+'>')+'Review '+selectedMI+(selected ? ('](!magic --button '+BT.REVIEW_MI+'|'+charID+'|'+MIrowref+'|'+selectedMI+'|\n&#37;{'+magicItem.dB+'|'+selectedMI+'})') : '')+'</span>';
            }
			content += '}}';
		}
		...
		if (!shortMenu || (selected && !bagSlot)) {
    		...
    		for (let i = 0; i < fields.MIRows; i++) {
    			...
    			content += qty + ' ' + mi;
    			if ( i != MIrowref ) {
    				content += '](!magic --button '+BT.SLOT_MI+'|' + charID + '|' + i + '|' + selectedMI + ')';
    			} else {
    				content += '</span>';
					removeMI = mi;
    			};
			...
			qty = attrLookup( charCS, fields.MI_table[0] + '_$' + MIrowref + '_' + fields.MI_qty[0], 'current' );
			removeMI = mi = attrLookup( charCS, fields.MI_table[0] + '_$' + MIrowref + '_' + fields.MI_name[0], 'current' );
		    
		    content += '{{desc1=Selected ['+qty+' '+mi+'](!magic --button '+BT.CHOOSE_MI+'|'+charID+'|-1|'+selectedMI+'|), click to change}}';
		}
		
		if (!shortMenu || (selected && bagSlot)) {
			content += '{{desc2='+((selected && bagSlot && !remove) ? '[' : ('<span style='+design.grey_button+'>'))
					+  'Store '+selectedMI
					+  ((selected && bagSlot && !remove) ? ('](!magic --button '+BT.STORE_MI+'|'+charID+'|'+MIrowref+'|'+selectedMI+'|?{Quantity?})') : '</span>')
					+  ' in your MI Bag\n\n'
					+  'or '+(bagSlot ? '[' : ('<span style='+design.grey_button+'>'))
					+  'Remove '+(!!removeMI ? removeMI : 'item')
					+  (bagSlot ? ('](!magic --button '+BT.REMOVE_MI+'|'+charID+'|'+MIrowref+'|'+selectedMI+')') : '</span>')
					+  ' from your MI Bag}}'
		}
		sendResponse( charCS, content );
		return;
	...
	var makeGMonlyMImenu = function(args, msg) {
		...
		content += '[Potion](!magic --button GM-MItoStore|'+tokenID+'|'+MIrowref+'|?{Which Potion?|'+getMagicList(fields.MagicItemDB, fields.MI_PotionList[0])+'})'
				+  '[Scroll](!magic --button GM-MItoStore|'+tokenID+'|'+MIrowref+'|?{Which Scroll?|'+getMagicList(fields.MagicItemDB, fields.MI_ScrollList[0])+'})'
				+  '[Rods, Staves, Wands](!magic --button GM-MItoStore|'+tokenID+'|'+MIrowref+'|?{Which Rod, Staff or Wand?|'+getMagicList(fields.MagicItemDB, fields.MI_WandsList[0])+'})'
				+  '[Weapon](!magic --button GM-MItoStore|'+tokenID+'|'+MIrowref+'|?{Which Weapon?|'+getMagicList(fields.MagicItemDB, fields.MI_WeaponList[0])+'})'
				+  '[Armour](!magic --button GM-MItoStore|'+tokenID+'|'+MIrowref+'|?{Which piece of Armour?|'+getMagicList(fields.MagicItemDB, fields.MI_ArmourList[0])+'})'
				+  '[Ring](!magic --button GM-MItoStore|'+tokenID+'|'+MIrowref+'|?{Which Ring?|'+getMagicList(fields.MagicItemDB, fields.MI_RingList[0])+'})'
				+  '[Miscellaneous MI](!magic --button GM-MItoStore|'+tokenID+'|'+MIrowref+'|?{Which Misc MI?|'+getMagicList(fields.MagicItemDB, fields.MI_MiscList[0])+'})'
				+  '[DM only list](!magic --button GM-MItoStore|'+tokenID+'|'+MIrowref+'|?{Which DM only item?|'+getMagicList(fields.MagicItemDB, fields.MI_DMList[0])+'})}}';
...
//		log('makeGMonlyMImenu: containerNo='+containerNo);
	...
	var makeShortPOPmenu = function( args ) {
        ...
        var putCS = getCharacter( putID ),
			...
			charCS = (tokenID == putID) ? putCS : pickCS;
        ...
        var putName = putCS.get('name'),
            ...
            content = '&{template:2Edefault}{{name=Pick up or Put Away MIs from ' + pickName + ' and ' + putName + '\'s Magic Item Bag}}';
			
		for (i = 0; i < fields.MIRows; i++) {
			mi = attrLookup( pickCS, fields.MI_table[0] + '_$' + i + '_' + fields.MI_name[0], fields.MI_name[1] );
			if (mi.length > 0 && mi != '-') {
				qty = attrLookup( pickCS, fields.MI_table[0] + '_$' + i + '_' + fields.MI_qty[0], fields.MI_qty[1] );
				miList += '|' + qty + ' ' + mi + ',' + i;
				if ( i == pickRow ) {
					pickedMI = mi;
					pickedTrueMI = attrLookup( pickCS, fields.MI_table[0] + '_$' + i + '_' + fields.MI_trueName[0], fields.MI_trueName[1] ) || '';
					pickableQty = qty;
					pickedType = attrLookup( pickCS, fields.MI_table[0] + '_$' + i + '_' + fields.MI_type[0], fields.MI_type[1] ) || '';
				}
			}
		}
...
//		log('makeShortPOPmenu: after emptyslot search putRow is '+putRow);
		...
		var pickOrPut = ((tokenID == putID) ? 'Pick up' : 'Put away'),
			content = '&{template:2Edefault}{{name=Pick up from ' + pickName + ' to put in ' + putName + '\'s Magic Item Bag}}'
			+ '{{desc=Press the **[Select]** button to select the item you want to '+pickOrPut+' from a list of items in a container, then press the **[Store]** button to automatically put it away in an empty slot}}'
			+ '{{Select=[Select Item to '+pickOrPut+'](!magic --button '+BT.POP_PICK+'|'+tokenID+'|'+pickID+'|'+putID+'|?{'+pickOrPut+' which Item?'+miList+'}|'+putRow+')}}'
			+ '{{Store='+((pickRow >= 0 && putRow >= 0) ? '[' : '<span style='+design.grey_button+'>')
			+ 'Store '+((pickRow >= 0) ? pickedMI : 'item')
			+ ((pickRow < 0 || putRow < 0) ? '</span>' : ('](!magic --button '+BT.POP_STORE+'|'+tokenID+'|'+pickID+'|'+putID+'|'+pickRow+'|'+putRow+'|-1)'))
			+ ' in free slot}}';
		
		if (tokenID == putID) {
			content += '{{desc1=Or [View Coins, Gems & Treasure](!magic --button POPtreasure|' + tokenID + '|' + pickID + '|' + putID + '|-1|-1|-1)}}';
		}
	...
	**/
	...
	var handleCastSpell = function( args ) {
		...
		var colNum = (fields.SpellsFirstColNum || colIndex != 1) ? colIndex : '',
			...
			spell = abilityLookup( db, spellName ),
			spellCost = (!!spell.ct ? spell.ct.get('max'): 0),
			totalLeft,
			content,
...
//		}
	...
	var handleMemoriseSpell = function( args ) {
		...
		var spell = abilityLookup( isPower ? fields.PowersDB : (isMU ? fields.MU_SpellsDB : fields.PR_SpellsDB), spellName ),
			castTime = (!!spell.ct ? spell.ct.get('current') : 0),
			castCost = (!!spell.ct ? spell.ct.get('max') : 0),
			col = (fields.SpellsFirstColNum || level != 1 || spellCol != 1) ? spellCol : '',
	...
	var handleChooseMI = function( args, isGM ) {
		
		makeViewUseMI( args, isGM );
		return;
	}
	...
	var handleViewUseMI = function( args, isGM ) {
		...
		case 'charged':
			if (MIqty <= 1) {
				...
				MIqtyObj.set('current',(MIqty-1));
				MIqtyObj.set('max',(MIqty-1));
			}
			break;
	...
	var handleSelectMIspell = function( args ) {
		...
		if (spellButton >= 0) {
			col = (fields.SpellsFirstColNum || spellCol != 1) ? spellCol : '';
			spellName = attrLookup( charCS, fields.Spells_table[0]+col + '_$'+spellRow + '_' + fields.Spells_name[0], fields.Spells_name[1] ) || '-';
		...
		if (MIbutton >= 0) {
			...
			spellName = attrLookup( charCS, fields.Spells_table[0]+col + '_$'+MIrow + '_' + fields.Spells_name[0], fields.Spells_name[1] ) || '-';
	...
	var handleStoreMIspell = function( args ) {
		...
		var isMU = args[0].toUpperCase().includes('MU'),
			...
			spellRow = args[6],
			spellCol = args[7],
			toCol = (fields.SpellsFirstColNum || MIcol != 1) ? MIcol : '',
			toRef = fields.Spells_table[0]+toCol + '_$'+MIrow + '_',
			fromCol = (fields.SpellsFirstColNum || spellCol != 1) ? spellCol : '',
			fromRef = fields.Spells_table[0]+fromCol + '_$'+spellsRow + '_';
			
		setAttr( charCS, toRef+fields.Spells_name[0]+toCol, fields.Spells_name[1], (spellName = attrLookup( charCS, fromRef+fields.Spells_name[0]+fromCol, fields.Spells_name[1] )));
		...
		makeStoreMIspell( args, 'Stored '+spellName+' in '+getObj('graphic',tokenID).get('name')+' spell storing magic item' );
		return;
	}	
	...
	**/
	...
	var handlePickOrPut = function( args ) {
		...
		if (fromSlotType.includes('cursed') && fromID == charID) {
		...
		if (toSlotName.toLowerCase() == MIname.toLowerCase()) {
			...
			content += '{{[Choose another slot](!magic --button pickorput|'+charID+'|'+fromID+'|'+toID+'|'+fromRowRef+'|-1)}}';
			sendParsedMsg( charID, content );
			return;
		}
	...
	var handleStoreMI = function( args, GMonly, senderId ) {
		...
		var slotRef = fields.MI_table[0] + '_$' + MIrowref + '_',
			...
			MIspeed = (!!magicItem.ct ? magicItem.ct.get('current') : undefined);
			MItype = (!!magicItem.ct ? magicItem.ct.get('max') : undefined);
...
// ------------------------------------------------------------- Command Action Functions ---------------------------------------------
	...
	var doCastAgain = function( args ) {
		...
		var castingName = attrLookup( charCS, fields.Casting_name[0], fields.Casting_name[1] ),
			...
			spellCharges = parseInt(attrLookup( charCS, rep+fields.Spells_castValue[0]+col, fields.Spells_castValue[1] ) || 0);
...
//		    log('Resetting MI powers');
	...
	var doViewUseMI = function( args, action, isGM ) {
		...
		makeViewUseMI( [action, tokenID, -1], isGM );
		return;
	...
	**/
	...
	var doSearchForMIs = function( args, senderId ) {
		...
		MIBagSecurity = parseInt((attrLookup( pickCS, 'check-for-MIBag', 'current' ) || '0'));
...
//		}
	...
	var doButton = function( args, senderId ) {
		...
		case BT.CHOOSE_USE_MI:
			...
			makeViewUseMI( args, playerIsGM(senderId) );
			break;
    ...
    var doMIBagMenu = function( args, senderId ) {
        ...
        if (checkNewMIBag( tokenID )) {
            ...
            content = '&{template:2Edefault} {{name=' + tokenName + '\'s\n'
                    ...
                    + '[5. Use '+menuType+' menus](!magic --options menutype|'+menuType+')}}';
