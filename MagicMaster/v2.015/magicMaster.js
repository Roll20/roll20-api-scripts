...
//            log('setAttr: name:'+name);
	...
	**/
	...
	var abilityLookup = function( rootDB, abilityName ) {
        ...
        log('abilityLookup: rootDB '+rootDB+', abilityName '+abilityName);
...
//		}
	...
	var handleViewUseMI = function( args, isSilent, charges=1 ) {
		...
		var charName = charCS.get('name'),
			...
			MIname = attrLookup( charCS, [ref+fields.MI_name[0], fields.MI_name[1]] ),
...
// ------------------------------------------------------------- Command Action Functions ---------------------------------------------
	...
	**/
	...
	var doSearchForMIs = function( args, senderId ) {
		...
		log('doSearchForMIs: security type '+MIBagSecurity);
		...
		case 4:
			...
			log('doSearchForMI: trapVersion '+trapVersion+', trapName '+trapName+', found trap is '+!!trapMacro);
			if (!trapMacro || trapMacro.length === 0) {
				trapName = 'Check-for-MIBag'+trapVersion;
				trapMacro = findObjs({ _type : 'ability', characterid : pickCS.id, name : trapName }, {caseInsensitive: true});
    			log('doSearchForMI: trapName '+trapName+', found trap is '+!!trapMacro);
			}
