...
var magicMaster = (function() {
	...
	var fields = Object.freeze({
		...
		PRbaseCol:          28,
		MISpellNo_table:	['spell-level','current'],
...
// -------------------------------------------- utility functions ----------------------------------------------
	...
	*/
    ...
    var attrLookup = function(character,attrDef,caseSensitive){
        let name = attrDef[0],
			property = attrDef[1],
    ...
    **/
    ...
    var characterLevel = function( charCS ) {
        var level = parseInt((attrLookup( charCS, fields.Fighter_level ) || 0),10)
	...
	var shapeSpellbook = function( charCS, spellbook ) {
		...
		var levelSpells, sheetTypes;
			...
			case 'MI':
				...
				log('number of spells field '+fields.MISpellNo_table[0] + fields.MIspellLevel + fields.MISpellNo_memable[0]+' is '+attrLookup(charCS,[fields.MISpellNo_table[0] + 15 + fields.MISpellNo_memable[0],fields.MISpellNo_memable[1]]));
	...
	var setSpell = function( charCS, spellDB, spellName, r, c, cost, msg, levelOrPerDay ) {
        ...
        log('setSpell: setting '+rep + fields.Spells_name[0] + col, fields.Spells_name[1]+', casting time '+(newSpellObj.ct ? newSpellObj.ct[0].get('current') : 0));
		setAttr( charCS, [rep + fields.Spells_name[0] + col, fields.Spells_name[1]], spellName );
		setAttr( charCS, [rep + fields.Spells_db[0] + col, fields.Spells_db[1]], newSpellObj.dB );
		setAttr( charCS, [rep + fields.Spells_speed[0] + col, fields.Spells_speed[1]], (newSpellObj.ct ? newSpellObj.ct[0].get('current') : 0) );
		setAttr( charCS, [rep + fields.Spells_cost[0] + col, fields.Spells_cost[1]], cost );
		setAttr( charCS, [rep + fields.Spells_msg[0] + col, fields.Spells_msg[1]], msg );

		if (spellDB.toUpperCase().includes('POWER')) {
			setAttr( charCS, [rep + fields.Spells_castValue[0] + col, fields.Spells_castValue[1]], levelOrPerDay );
			setAttr( charCS, [rep + fields.Spells_castMax[0] + col, fields.Spells_castMax[1]], levelOrPerDay );
		} else {
		    log('setSpell: setting cating level to '+levelOrPerDay);
			setAttr( charCS, [rep + fields.Spells_storedLevel[0] + col, fields.Spells_storedLevel[1]], levelOrPerDay );
			setAttr( charCS, [rep + fields.Spells_castValue[0] + col, fields.Spells_castValue[1]], 1 );
			setAttr( charCS, [rep + fields.Spells_castMax[0] + col, fields.Spells_castMax[1]], 1 );
		}
		return;
	}
	...
	var changeMIspells = function( charCS, listType, isAdd, spellList, spellValues ) {
		...
		case 'MU':
			...
			indexPrefix = fields.MIspellPrefix[0];
		...
		log('listType '+listType+', levelSpells length '+levelSpells.length);
		for (l = 1; l < levelSpells.length; l++) {
		    log('checking level '+l+', no of spells '+levelSpells[l].spells);
			r = 0; maxSpell = 0; s = 1;
 			while (s < 10) {
				c = levelSpells[l].base;
				log('no of spells '+levelSpells[l].spells+', c is '+c)
				for (w = 1; (w <= fields.SpellsCols) && (s < 100); w++) {
					...
					log('Searching '+rep + fields.Spells_name[0] + col+', which holds '+spellName);
					if (_.isUndefined(spellName)) {
						s=100;
						log('undefined slot, so breaking out');
						break;
					}
					if (isAdd && spellName == '-') {
						spellName = spellList.shift() || '';
						log('Found free slot, r:'+r+', c:'+c+', indexing attribute will be:'+indexPrefix+spellName);
						setSpell( charCS, spellDB, spellName, r, c, 0, '', (spellValues.shift() || '') );
						maxSpell = s;
						if (listType == 'POWER') {
							setAttr( charCS, [indexPrefix+spellName, 'current'], r );
							setAttr( charCS, [indexPrefix+spellName, 'max'], c );
						}
					} else if (!isAdd && spellList.includes(spellName)) {
						spellList.splice(spellList.indexOf(spellName),1);
						setAttr( charCS, [rep + fields.Spells_name[0] + col, fields.Spells_name[1]], '-' );
						setAttr( charCS, [rep + fields.Spells_castValue[0] + col, fields.Spells_castValue[1]], 0 );
						setAttr( charCS, [rep + fields.Spells_castMax[0] + col, fields.Spells_castMax[1]], 0 );
					}
					if (!spellList || spellList.length==0) {
					    s=100;
					    log('spell list now empty.  breaking');
					    break;
					}
					c++;
					s++;
		...
		l--;
/*		while (isAdd && spellList.length>0) {
		    log('still have spells to add, w is '+w)
			for (; (w <= fields.SpellsCols); w++) {
				let col = (fields.SpellsFirstColNum || c != 1) ? c : '',
					rep = fields.Spells_table[0] + col + '_$' + r + '_';
				spellName = attrLookup( charCS, [rep + fields.Spells_name[0] + col, fields.Spells_name[1]]);
				log('searching '+rep + fields.Spells_name[0] + col+' which contains'+spellName);
				if (_.isUndefined(spellName)) {
                    log('undefined, so breaking');
				    break;
				}
				spellName = spellList.shift() || '';
				log('Found slot, r:'+r+', c:'+c+', indexing attribute will be:'+indexPrefix+spellName);
				setSpell( charCS, spellDB, spellName, r, c, 0, '', spellValues.shift() || '' );
				if (listType == 'POWER') {
					setAttr( charCS, [indexPrefix+spellName, 'current'], r );
					setAttr( charCS, [indexPrefix+spellName, 'max'], c );
				}
				if (!spellList || spellList.length==0) {break;}
				c++;
				s++;
			}
			if (_.isUndefined(spellName)) {break;}
			c = levelSpells[l].base;
			w=1;
			r++;
		}
*/
        ...
        if (isAdd && spellList && spellList.length>0) {
	...
	var removeMIpowers = function( charCS, spellList ) {
		...
		while (spellList.length > 0) {
			spellName = spellList.shift();
			r = attrLookup( charCS, [fields.MIpowerPrefix[0]+spellName, 'current'] );
			c = attrLookup( charCS, [fields.MIpowerPrefix[0]+spellName, 'max'] );
			log('removing '+spellName+', from row '+r+', column '+c);
			if (!_.isUndefined(r) && !_.isUndefined(c)) {
				col = (fields.SpellsFirstColNum || c != 1) ? c : '';
				rep = fields.Spells_table[0] + col + '_$' + r + '_';
				log('table reference: '+rep);
				setAttr( charCS, [rep + fields.Spells_name[0] + col, fields.Spells_name[1]], '-' );
				setAttr( charCS, [rep + fields.Spells_castValue[0] + col, fields.Spells_castValue[1]], 0 );
				setAttr( charCS, [rep + fields.Spells_castMax[0] + col, fields.Spells_castMax[1]], 0 );
			}
	...
	var moveMIspells = function( fromCS, toCS, MIname ) {
		...
		var MUspellList = attrLookup( fromCS, [fields.MI_MUspellsList[0]+MIname, fields.MI_MUspellsList[1]] ),
			MUspellValues = attrLookup( fromCS, [fields.MI_MUspellValues[0]+MIname, fields.MI_MUspellValues[1]] ),
			PRspellList = attrLookup( fromCS, [fields.MI_PRspellsList[0]+MIname, fields.MI_PRspellsList[1]] ),
			PRspellValues = attrLookup( fromCS, [fields.MI_PRspellValues[0]+MIname, fields.MI_PRspellValues[1]] ),
			powerList = attrLookup( fromCS, [fields.MI_PowersList[0]+MIname, fields.MI_PowersList[1]] ),
			powerValues = attrLookup( fromCS, [fields.MI_PowerValues[0]+MIname, fields.MI_PowerValues[1]] );
		
		if (MUspellList && MUspellList.length>0) {
			setAttr( toCS, [fields.MI_MUspellsList[0]+MIname, fields.MI_MUspellsList[1]], MUspellList );
			setAttr( toCS, [fields.MI_MUspellValues[0]+MIname, fields.MI_MUspellValues[1]], MUspellValues );
			changeMIspells( fromCS, 'MU', false, MUspellList );
			changeMIspells( toCS, 'MU', true, MUspellList, MUspellValues );
		}
		if (PRspellList && PRspellList.length>0) {
			setAttr( toCS, [fields.MI_PRspellsList[0]+MIname, fields.MI_PRspellsList[1]], PRspellList );
			setAttr( toCS, [fields.MI_PRspellValues[0]+MIname, fields.MI_PRspellValues[1]], PRspellValues );
			changeMIspells( fromCS, 'PR', false, PRspellList );
			changeMIspells( toCS, 'PR', true, PRspellList, PRspellValues );
		}
		if (powerList && powerList.length>0) {
			setAttr( toCS, [fields.MI_PowersList[0]+MIname, fields.MI_PowersList[1]], powerList );
			setAttr( toCS, [fields.MI_PowerValues[0]+MIname, fields.MI_PowerValues[1]], powerValues );
			removeMIpowers( fromCS, powerList );
			changeMIspells( toCS, 'POWER', true, powerList, powerValues );
		}
		return;
	}
...
//		}
	...
	var handleRest = function( args ) {
		...
		if (casterType.includes('MI') && casterType.includes('POWER')){
			...
			log('Done mi-powers reset, returning');
			return;
		}

        log('about to do a character rest');
			...
			while (level < levelLimit && levelSpells[level].spells > 0) {
				...
				while (levelSpells[level].spells > 0) {
					...
					for (w = 1; (w <= fields.SpellsCols) && (levelSpells[level].spells > 0); w++) {
						col = (fields.SpellsFirstColNum || c != 1) ? c : '';
						rep = fields.Spells_table[0] + col + '_$' + r + '_';
						setAttr( charCS, [rep+fields.Spells_castValue[0]+col, fields.Spells_castValue[1]], (!restType.includes('POWER') ? 1 : (attrLookup( charCS, [rep+fields.Spells_castMax[0]+col, fields.Spells_castMax[1]] ) || 0)) );
						c++;
						levelSpells[level].spells--;
					}
					r++;
	...
	var handleSelectMIpower = function( args, isUse ) {
		...
		log('Looking for '+args[2]+' using index attribute '+fields.MIpowerPrefix[0]+args[2]);
		...
		var powerName = args[2] || '',
			...
			rep = fields.Spells_table[0] + col + '_$' + powerRow + '_',
			power = attrLookup( MIlibrary, [rep+fields.Spells_name[0]+col, fields.Spells_name[1]] ) || '-',
			powerLib = abilityLookup( fields.PowersDB, power ),
			powerCount = attrLookup( MIlibrary, [rep+fields.Spells_castValue[0]+col, fields.Spells_castValue[1]] ) || 0,
			content = '';
		...
		if (isUse) {
			if (powerCount > 0) {
				setAttr( MIlibrary, [rep+fields.Spells_castValue[0]+col, fields.Spells_castValue[1]], powerCount-1 );
			}
	...
	var handleStoreMIspell = function( args ) {
		...
		setAttr( charCS, [toRef+fields.Spells_storedLevel[0]+toCol, fields.Spells_storedLevel[1]], attrLookup( charCS, fields.Casting_level ));
