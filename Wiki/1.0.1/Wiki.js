...
const wiki = (() => {
    ...
    //1.0.0 Debut
    //1.0.1 2026-09-22: Added full-text Search mode (ranked results, highlighted
    //      snippets, in-place highlighting while browsing a result); redesigned
    //      control bar (icon-only Previous/Next/Back/Forward/Clear History with
    //      hover tooltips, floated groups); fixed header navigation for "All"
    //      level and GM Notes headers, cross-handout link targeting, and header
    //      text matching for handouts containing &nbsp; entities; updated the
    //      in-app Help handout to document all of the above.
    ...
    const STATE_DEFAULTS = () => (
    {
        ...
        displayHTML:            "",
        // Search mode -- small and transient, NOT the search index itself
        // (that stays a volatile module-level variable, never in state).
        // Just the current query text and each result's id/top-matching
        // header, so the results list/nav can be re-rendered while browsing.
        _searchQuery:           null,
        _searchResults:         []
    });
    ...
    const initializeState = () =>
    {
        ...
        const ensurePartition = (p) =>
        {
            ...
            if(typeof p._searchQuery === 'undefined')     p._searchQuery   = null;
            if(!Array.isArray(p._searchResults))          p._searchResults = [];
        };
...
`;
    ...
    const CSS =
    {
        ...
        controlBar:      "background:#2e1f1a; padding:6px; border-bottom:1px solid #111;",
        controlBarTable: "width:100%; border-collapse:collapse; border-style:none; margin:0; padding:0;",
        controlBarLeftCell:   "text-align:left; vertical-align:middle; white-space:nowrap; width:1%; border-style:none; margin:0; padding:0;",
        controlBarCenterCell: "text-align:center; vertical-align:middle; border-style:none; margin:0; padding:0;",
        controlBarRightCell:  "text-align:right; vertical-align:middle; white-space:nowrap; width:1%; border-style:none; margin:0; padding:0;",
        ...
        controlButtonDisabled: "display:inline-block; margin:0 3px; padding:4px 8px; background:#3a2a22; color:#77655e !important; border-radius:4px; font-size:12px;font-weight:bold;",
        ...
        searchResultsHeader:   "color:#eee; font-weight:bold; font-size:13px; margin-bottom:6px;",
        searchResultBlock:     "background:#2e2116; border:1px solid #444; border-radius:4px; padding:6px 8px; margin-bottom:8px;",
        searchResultTitle:     "margin-bottom:4px;",
        searchResultTitleLink: "color:#f0c98f !important; font-weight:bold; font-size:13px; text-decoration:none;",
        searchSnippet:         "font-size:12px; color:#ccc; margin:3px 0; line-height:1.4;",
        searchHighlight:       "background:#f0c98f; color:#222; font-weight:bold; padding:0 2px; border-radius:2px;",
        searchGmTag:           "color:#999; font-size:10px; font-weight:bold;",
        searchHeaderTag:       "color:#d9a873 !important; font-size:11px; font-weight:bold; text-decoration:underline;",
        searchQueryLabel:      "color:#ddd; font-size:11px; margin:4px 2px;",
        searchNoResults:       "color:#aaa; font-size:12px; padding:6px;"
    };
    ...
    const makeButton      = (label, command, style, title) => `<a href="${command}" style="${style}"${title ? ` title="${title}"` : ''}>${label}</a>`;
    const normalizeForChat = (html) => html.replace(/\r?\n/g, '');

    const helpButton = `<a style="float:right; color:#ddd; font-family:Nunito, Arial, sans-serif; font-weight:bold; font-size:20px; padding:0px;" href="!wiki --help" title="Help">?</a>`;
    const homeButton = `<a style="float:right; color:#ddd; font-family:Nunito, Arial, sans-serif; font-weight:bold; font-size:20px; padding:0px 6px 0px 0px;" href="!wiki --home" title="Wiki Home">&#8962;</a>`;
...
<p>
Search Mode searches across every searchable handout at once &mdash; names,
headers, and body text (including GM Notes, for the GM). Click the
<strong>SEARCH</strong> button or type <strong>!wiki --search</strong> to be
prompted for a search term.
</p>

<h3>Results</h3>

<p>
Results are grouped by handout and ranked: a handout whose <strong>name</strong>
matches is listed first, followed by handouts matched by a <strong>header</strong>,
then by <strong>body text</strong>. Within each handout, every matching section
is listed with a highlighted snippet of the surrounding text. Click a section's
header to jump straight to it; for the GM, sections matched in GM Notes are
marked <strong>[GM]</strong>.
</p>

<h3>Browsing Results</h3>

<p>
While in Search Mode, the navigation panel stays on a list of matched
handouts (with a <strong>New Search</strong> button at the top) so you can
jump between hits without re-running the search. Clicking a section's link
loads that section into the content panel with the search term highlighted
inline, using the same highlight style as the results list. Switching to
Handout Mode or Pin Mode, or starting a new search, clears the highlight.
</p>

<h3>What's Searched</h3>

<p>
Archived handouts and handouts tagged <strong>wiki-</strong> are never
searched. Players only see matches in handouts they can already access
(shared handouts or those tagged <strong>wiki+</strong>), and never see
matches from GM Notes.
</p>
...
<p>
Most of these buttons are icon-only to save space — hover over one to see
what it does. A grayed-out button means there's nothing to do in that
direction right now (nothing to go back to, no next section, etc.).
</p>

<ul>
<li><strong>&#9650; / &#9660;</strong> (left side) — Previous / Next: steps through the filtered navigation list sequentially</li>
<li><strong>&#9664; / &#10005; / &#9654;</strong> (right side) — Back / clear history / Forward: Back returns to the previously viewed handout or section, Forward moves forward again after going Back, and the &#10005; clears all Back/Forward history. GM and player histories are tracked independently.</li>
<li><strong>Edit</strong> — opens the source handout for editing (GM only)</li>
<li><strong>To: GM / Players</strong> — sends the current content to GM chat, or the player-visible version to everyone (GM only)</li>
...
<li><strong>!wiki --search</strong> — prompt for a search term and switch to Search Mode</li>
...
<li>Clear, descriptive headers make Search results easier to scan, since the header text doubles as the jump link.</li>
</ul>
`;
...
const rewriteLinksForChat = (html) =>
{
    // Strip all styling from api command links and replace with plain inline text style
    return html.replace(
        /<a\s([^>]*href="![^"]*")[^>]*>([^<]*)<\/a>/gi,
        (match, hrefAttr, label) =>
        {
            return `<a ${hrefAttr} style="color:#337ab7!important; text-decoration:underline!important;background-color:transparent!important;padding:0!important; margin:0!important;border:none!important;">${label}</a>`;
        }
    );
};
    ...
    // Highlights every occurrence of `query` in `html`, skipping anything
    // inside a tag (so it can never match inside an href/attribute or break
    // a tag). Walks the string as alternating <tag> / text tokens and only
    // replaces within the text tokens.
    const highlightQueryInHTML = (html, query) =>
    {
        ...
        return html.replace(/(<[^>]*>)|([^<]+)/g, (whole, tag, text) =>
            tag ? tag : text.replace(regex, `<span style="${getCSS().searchHighlight}">$1</span>`)
        );
    };

    // rewriteHandoutLinks, plus the active search term highlighted inline --
    // applied only while a session is actively browsing inside Search mode
    // with a live query (s.mode === "search" && s._searchQuery). Leaving
    // Search mode clears _searchQuery (see the --mode handler), so the next
    // render through here naturally drops the highlight with no extra work.
    // Used at every displayHTML assignment reachable while browsing search
    // results (opening a matched section, Prev/Next, Back/Forward); other
    // displayHTML assignments (Pin Mode, Handout Mode's own header-level UI)
    // aren't reachable while s.mode === "search" in the first place, so they
    // keep calling rewriteHandoutLinks directly.
    const renderSectionHTML = (rawContent, s) =>
    {
        const linked = rewriteHandoutLinks(rawContent);
        return (s.mode === "search" && s._searchQuery) ? highlightQueryInHTML(linked, s._searchQuery) : linked;
    };
    ...
    // Strips tags AND decodes &nbsp; (entity or literal U+00A0) to a plain space
    // before trimming. Rich-text editors (Roll20's included) commonly leave a
    // trailing &nbsp; entity in header HTML, and plain .trim() does not remove
    // it (it's 6 literal characters, not whitespace, to a naive tag-strip).
    // Every place that derives "the header's text" from raw content must use
    // this same normalization, or two representations of the same header
    // (e.g. one from a clicked link, one re-extracted from content) can end up
    // as different strings and fail to match each other.
    const cleanHeaderText = (raw) =>
        raw
            .replace(/<[^>]+>/g, '')
            .replace(/&nbsp;/gi, ' ')
            .replace(/\u00A0/g, ' ')
    ...
    const extractHeaders = (html, level) =>
    {
        ...
        while((match = regex.exec(html)) !== null)
            results.push(cleanHeaderText(match[1]));

        return results;
    };

    /* ============================================================
     * NAVIGABLE HEADER LIST (shared by rendering + Prev/Next/Back/Forward)
     * ============================================================ */

    const collectHeaderSections = (html, field, effectiveLevel, s) =>
    {
        if(!html) return [];
        ...
        while((match = headerRegex.exec(html)) !== null)
        {
            ...
            const text  = cleanHeaderText(match[2]);
            if(!text) continue;
            ...
            results.push({ text, field, level });
        }

        return results;
    };

    const getNavigableHeaders = (htmlContent, isGM, s) =>
    {
        const effectiveLevel = s.selectedHeaderLevel === 0
            ? getHighestHeaderLevel((htmlContent || "") + (isGM ? (s._cachedGmNotes || "") : ""))
            : s.selectedHeaderLevel;

        let headers = collectHeaderSections(htmlContent, 'notes', effectiveLevel, s);
        ...
        if(isGM && s.selectedHandout)
        {
            ...
            if(handout)
            {
                ...
                headers = headers.concat(collectHeaderSections(gmContent, 'gmnotes', effectiveLevel, s));
            }
        }
    ...
    const extractSection = (content, headerText) =>
    {
        ...
        {
            const level    = parseInt(match[1][1], 10);
            const stripped = cleanHeaderText(match[2]);
    ...
    // Shared handout-tags reader. Roll20 stores tags as either a JSON-array
    // string or (rarely) an actual array, hence the try/parse. Several places
    // in Wiki need "does this handout have tag X" (wiki+/wiki- checks); this
    // is the version used by the code added below. buildHandoutSelector and
    // getWikiBackgroundURL have their own long-standing inline copies of this
    // same logic -- left as-is here since touching them isn't part of this
    // change.
    const getHandoutTags = (handout) =>
    {
        let tags = [];
        try
        {
            const raw = handout.get('tags');
            if(Array.isArray(raw))                  tags = raw;
            else if(typeof raw === 'string' && raw) tags = JSON.parse(raw);
        }
        catch(e) {}
    ...
    /* ============================================================
     * SEARCH INDEX
     * Volatile, module-level only -- deliberately never stored in `state`.
     * Rebuilt in full on ready(), then kept current incrementally via the
     * change:handout/destroy:handout listeners below (see on('ready')).
     * ============================================================ */

    let searchIndex = {};

    const isHandoutSearchable = (handout) =>
        handout && !handout.get('archived') && !getHandoutTags(handout).includes('wiki-');

    // Splits raw handout HTML into {header, text} sections for search: one
    // entry per h1-h6, plus a leading entry (header: null) covering any text
    // before the first header (or the whole document, if it has no headers).
    // header/text both go through cleanHeaderText, matching how every other
    // part of Wiki normalizes header/body text -- so a search match's stored
    // header string is always comparable to one produced anywhere else.
    const splitIntoSections = (html) =>
    {
        ...
        while((match = headerRegex.exec(html)) !== null)
        {
            sections.push({ header: pendingHeader, text: cleanHeaderText(html.slice(lastIndex, match.index)) });
            pendingHeader = cleanHeaderText(match[2]);
            lastIndex     = headerRegex.lastIndex;
        }

        sections.push({ header: pendingHeader, text: cleanHeaderText(html.slice(lastIndex)) });

        return sections.filter(sec => sec.header || sec.text);
    };

    // (Re)indexes a single handout. Safe to call for a handout that has just
    // become excluded (archived, tagged wiki-) -- it removes any existing
    // entry in that case, rather than leaving a stale one behind.
    const indexHandout = (handout) =>
        new Promise(resolve =>
        {
            if(!isHandoutSearchable(handout))
            {
                delete searchIndex[handout.id];
                return resolve();
            }

            const tags             = getHandoutTags(handout);
            const visibleToPlayers = Boolean(handout.get('inplayerjournals')) || tags.includes('wiki+');

            handout.get('notes', notes =>
            {
                handout.get('gmnotes', gmnotes =>
                {
                    searchIndex[handout.id] =
                    {
                        id:             handout.id,
                        name:           handout.get('name') || "(unnamed)",
                        visibleToPlayers,
                        publicSections: splitIntoSections(notes || ""),
                        gmSections:     splitIntoSections((gmnotes && gmnotes !== 'undefined' && gmnotes !== 'null') ? gmnotes : "")
                    };
                    resolve();
                });
            });
    ...
    {
        searchIndex = {};
        const handouts = findObjs({ _type: 'handout' }).filter(isHandoutSearchable);
        await Promise.all(handouts.map(h => indexHandout(h)));
        log(`Wiki: search index built (${handouts.length} handouts).`);
    };

    // Builds a highlighted excerpt around the first match of `query` in
    // `text`, or null if `text` doesn't contain it. `text` is assumed
    // already cleaned (splitIntoSections output), so slicing it can't cut
    // across an HTML tag boundary.
    const highlightSnippet = (text, query) =>
    {
        const lc  = text.toLowerCase();
        const pos = lc.indexOf(query);
        ...
        if(start > 0)         excerpt = "&hellip;" + excerpt;
        if(end < text.length) excerpt = excerpt + "&hellip;";

        const escapedQuery = query.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&');
        const regex         = new RegExp(`(${escapedQuery})`, 'i');
        return excerpt.replace(regex, `<span style="${getCSS().searchHighlight}">$1</span>`);
    };

    // Ranked search across the volatile index. Player calls (isGM = false)
    // never see gmSections and never see a handout that wouldn't appear in
    // their own handout chooser -- same visibility rule as buildHandoutSelector,
    // just checked against the cached visibleToPlayers flag instead of a
    // fresh handout lookup.
    const runSearch = (query, isGM = true) =>
    {
        ...
        {
            if(!isGM && !entry.visibleToPlayers) return;

            const nameMatch       = entry.name.toLowerCase().includes(q);
            const matchedSections = [];

            const scanSections = (sections, isGMSection) =>
            {
                sections.forEach(sec =>
                {
                    const headerMatch = sec.header && sec.header.toLowerCase().includes(q);
                    const snippet     = highlightSnippet(sec.text, q);
                    if(!headerMatch && !snippet) return;

                    matchedSections.push({ header: sec.header, snippet, isGM: isGMSection, tier: headerMatch ? 2 : 3 });
                });
            };

            scanSections(entry.publicSections, false);
            if(isGM) scanSections(entry.gmSections, true);

            if(!nameMatch && !matchedSections.length) return;

            const rank = nameMatch ? 1 : Math.min(...matchedSections.map(sec => sec.tier));
            results.push({ id: entry.id, name: entry.name, rank, sections: matchedSections });
        });

        results.sort((a, b) => a.rank - b.rank || b.sections.length - a.sections.length);
        return results;
    };
...
const buildPinList = async (playerid, isGM = true) =>
{
    ...
    for(const p of pins)
    {
        ...
        const pingButton   = makeButton("@",   `!wiki --ping-pin ${p.id}`,    css.pinPingButton, "Ping for all players");
        const pingButtonGM = makeButton("@",   `!wiki --ping-pin-gm ${p.id}`, css.pinPingButtonGM, "Ping for GM only");
...
// Nav panel body for Search mode: a "New Search" button plus a flat list of
// the current results' handout names, each jumping straight to that
// handout's best-matching section. Reads only s._searchQuery/_searchResults
// (small, per-session) -- never touches the volatile search index directly.
const buildSearchNavList = (isGM = true) =>
{
    const css = getCSS();
    const s   = getState(isGM);

    let html = makeButton("New Search", `!wiki --search ?{Search Wiki}`, css.handoutButton, "Start a new search");

    if(s._searchQuery)
        html += `<div style="${css.searchQueryLabel}">Results for: <b>${s._searchQuery}</b></div>`;

    if(!s._searchResults || !s._searchResults.length)
    {
        html += `<div style="${css.searchNoResults}">${s._searchQuery ? "No matches." : "Enter a search term above."}</div>`;
        return html;
    }

    s._searchResults.forEach(r =>
    {
        const handout = getObj('handout', r.id);
        if(!handout) return;

        const isActive    = s.mode === "search" && s.selectedHandout === r.id;
        const activeStyle = isActive ? " outline:2px solid #ddd; outline-offset:1px;" : "";
        const url         = r.topHeader
            ? `!wiki --selectHandout ${r.id} --show ${encodeURIComponent(r.topHeader)}`
            : `!wiki --selectHandout ${r.id}`;

        html += makeButton(handout.get('name'), url, css.listButton + activeStyle);
    });
    ...
    const buildHandoutSelector = (isGM = true) =>
    {
        ...
        return makeButton(label, `!wiki --selectHandout ${query}`, css.handoutButton, "Choose a different handout");
    };
...
const buildModeRow = (isGM = true) =>
{
    ...
    return `<div style="${css.modeRow}">
        ${makeButton("HANDOUTS", "!wiki --mode handout",         css.modeRowButton + (s.mode === "handout" ? active : ""), "Browse handouts")}
        ${makeButton("PINS",     "!wiki --mode pins",            css.modeRowButton + (s.mode === "pins"    ? active : ""), "Browse pins on this page")}
        ${makeButton("SEARCH",   "!wiki --search ?{Search Wiki}", css.modeRowButton + (s.mode === "search"  ? active : ""), "Search the wiki")}
    </div>`;
};
    ...
    const buildHeaderRow = (isGM = true) =>
    {
        const css = getCSS();
        const s   = getState(isGM);
        let buttons = makeButton(`All`, `!wiki --level 0`, css.headerRowButton, "Show the entire handout");

        for(let i = 1; i <= 4; i++)
            buttons += makeButton(`h${i}`, `!wiki --level ${i}`, css.headerRowButton, `Show only level ${i} sections`);

        const belowLabel = s.showBelow ?
            `<span style="font-family:Pictos">}</span>` :
            `<span style="font-family:Pictos">{</span>`;
        const belowTitle = s.showBelow ? "Hide subordinate headers" : "Also show subordinate headers";

        buttons += makeButton(belowLabel, `!wiki --below ${s.showBelow ? "false" : "true"}`, css.headerRowButton, belowTitle);
...
const buildHeaderList = (htmlContent, isGM = true) =>
{
    ...
    const headers = getNavigableHeaders(htmlContent, isGM, s);
    s.currentList = headers;

    let buttons = "";

    headers.forEach(({ text, field, level }) =>
    {
        const isGMField   = field === 'gmnotes';
...
// Content Panel body for a set of search results: one block per matched
// handout, listing every matching section with a highlighted snippet and a
// jump link. Rendered once per search (not re-rendered on every subsequent
// click while browsing results -- the nav list, not this, stays visible then).
const buildSearchResultsHTML = (results, query, isGM = true) =>
{
    const css = getCSS();

    if(!results.length)
        return `<div style="${css.searchNoResults}">No matches for "<b>${query}</b>".</div>`;

    // Enclosed in the same dark result-block styling as each handout's hits
    // below, rather than plain text on whatever background the page has --
    // readable regardless of the underlying handout/page background color.
    let html = `<div style="${css.searchResultBlock}"><span style="${css.searchResultsHeader}">Search: "${query}" &mdash; ${results.length} match${results.length === 1 ? "" : "es"}</span></div>`;

    results.forEach(r =>
    {
        const openUrl = `!wiki --selectHandout ${r.id}`;

        html += `<div style="${css.searchResultBlock}">`;
        html += `<div style="${css.searchResultTitle}">${makeButton(r.name, openUrl, css.searchResultTitleLink)}</div>`;

        r.sections.forEach(sec =>
        {
            const showUrl    = sec.header
                ? `!wiki --selectHandout ${r.id} --show ${encodeURIComponent(sec.header)}${sec.isGM ? ' --field gmnotes' : ''}`
                : openUrl;
            const gmTag      = sec.isGM ? `<span style="${css.searchGmTag}">[GM] </span>` : '';
            // The header itself (or a fallback label when the match is only in
            // the preamble text before any header) IS the jump link now --
            // no separate "Jump to Section" text needed.
            const headerLink = makeButton(sec.header || "(top of page)", showUrl, css.searchHeaderTag);
            const snippet     = sec.snippet || '(matched in header)';

            html += `<div style="${css.searchSnippet}">${gmTag}${headerLink} &mdash; ${snippet}</div>`;
        });
    ...
    const buildControlBar = (isGM = true) =>
    {
        ...
        // Icon-only nav buttons: the label is just the glyph, and the button's
        // purpose lives in `title` (hover text) instead, so a learned button
        // doesn't keep spending width on a label once you know what it does.
        // Applied to both the live link and its disabled/inert form, so the
        // tooltip still explains the (grayed-out) button either way.
        const navButton = (enabled, label, command, title) =>
            enabled
                ? makeButton(label, command, css.controlButton, title)
                : `<span style="${css.controlButtonDisabled}" title="${title}">${label}</span>`;

        // --- left group: Previous / Next (step through the filtered list) ---
        const listLen      = Array.isArray(s.currentList) ? s.currentList.length : 0;
        const prevEnabled  = s.currentIndex > 0;
        const nextEnabled  = s.currentIndex >= 0 && s.currentIndex < listLen - 1;

        let leftButtons = "";
        leftButtons += navButton(prevEnabled, "&#9650;", "!wiki --prev", "Previous");
        leftButtons += navButton(nextEnabled, "&#9660;", "!wiki --next", "Next");

        // --- right group: Back / History / Forward (navigation history) ---
        let rightButtons = "";
        if(s.mode !== "pins")
        {
            const backEnabled    = s.navBack.length    > 0;
            const forwardEnabled = s.navForward.length > 0;
            const historyEnabled = backEnabled || forwardEnabled;

            rightButtons += navButton(backEnabled,    "&#9664;",  "!wiki --nav-back",    "Back");
            rightButtons += navButton(historyEnabled, "&#10005;", "!wiki --nav-clear",   "Clear navigation history");
            rightButtons += navButton(forwardEnabled, "&#9654;",  "!wiki --nav-forward", "Forward");
        }

        // --- center group: everything else ---
        let centerButtons = "";
        ...
        if(s.mode === "pins" && s.currentItemType === "pin" && s.currentItemId)
        {
            ...
            {
                if(isGM)
                    centerButtons += makeButton("@", `!wiki --ping-pin-gm ${s.currentItemId}`, css.pinPingButtonGM + "margin-left:10px;font-size:24px;", "Ping for GM only");
                centerButtons += makeButton("@", `!wiki --ping-pin ${s.currentItemId}`, css.pinPingButton + "margin-left:10px;font-size:24px;", "Ping for all players");
            }
        }
        ...
        if(isGM)
        {
            if(s.currentItemType === "header")
            {
                ...
                centerButtons += `<a href="${url}" style="${css.controlButton}" title="Open this handout for editing"><span style="color:#eee!important; font-size:12px;">Open</span></a>`;//span necessary to override Roll20 default text color styling.
            ...
            else if(s.currentItemType === "pin")
            {
                ...
                if(pin && pin.get("link"))
                {
                    ...
                    centerButtons += `<a href="${url}" style="${css.controlButton}" title="Open the linked handout for editing"><span style="color:#eee!important; font-size:12px;">Open</span></a>`;//span necessary to override Roll20 default text color styling.

                }
                else
                {
                    centerButtons += makeButton("Edit", "!wiki --edit", css.controlButton, "Edit this pin");
                }
            }
            ...
            {
                centerButtons += makeButton("Pintool", "!wiki --pintool", css.controlButton, "Open Pintool in Pin Mode");
            }

            centerButtons += `<span style ="margin-left:15px; color:#eee;">To: </span>`;
            centerButtons += makeButton("GM", "!wiki --send-chat", css.controlButton, "Send this content to GM chat");
            centerButtons += makeButton("Players", "!wiki --send-chat-players", css.controlButton, "Send the player-visible version to chat");
        }

        const table =
            `<table style="${css.controlBarTable}"><tr>` +
                `<td style="${css.controlBarLeftCell}">${leftButtons}</td>` +
                `<td style="${css.controlBarCenterCell}">${centerButtons}</td>` +
                `<td style="${css.controlBarRightCell}">${rightButtons}</td>` +
            `</tr></table>`;

        return `<div style="${css.controlBar}">${table}</div>`;
    };
...
const buildViewportHTML = async (htmlContent, playerid, isGM = true) =>
{
    ...
    on('ready', async () =>
    {
        ...
        // Invalidate handout selector cache on any handout change, and if the
        // changed handout is the one currently open in a session, refresh that
        // session's cached GM Notes too -- otherwise editing GM Notes on the
        // open handout leaves stale content/headers displayed until the
        // handout is reselected or mode is toggled.
        on('change:handout', (obj) =>
        {
            state.Wiki.gm._handoutSelectorCache     = null;
            state.Wiki.player._handoutSelectorCache = null;

            if(!obj) return;
            const changedId = obj.id;

            [state.Wiki.gm, state.Wiki.player].forEach(partition =>
            {
                if(partition.selectedHandout !== changedId) return;

                obj.get('gmnotes', gmContent =>
                {
                    partition._cachedGmNotes = (gmContent && gmContent !== 'undefined' && gmContent !== 'null')
                        ? gmContent : "";
                });
            });

            // Keep the search index current -- re-indexes just this one
            // handout rather than rescanning the whole campaign.
            indexHandout(obj).catch(e => log(`Wiki: indexHandout error for ${obj.id}: ${e}`));
        });

        // A deleted handout must not keep showing up in search results until
        // the next full rebuild (which otherwise only happens on script/game
        // restart).
        on('destroy:handout', (obj) =>
        {
            if(obj) delete searchIndex[obj.id];
        });

        // Fire-and-forget: builds alongside Wiki Home setup below rather than
        // blocking it. A search made before this finishes just returns
        // partial/no results, same as any cache warming up.
        buildSearchIndex().catch(e => log(`Wiki: buildSearchIndex error: ${e}`));
...
const getWikiBackgroundStyle = () =>
{
    ...
    on('chat:message', async msg =>
    {
        ...
        // Captured before any block below may synthesize/overwrite args.selectHandout
        // (plain "!wiki", --home). Used later to tell a real combined
        // "--selectHandout X --show Y" link (one user action) apart from a
        // same-handout header jump, so Back history isn't double-pushed.
        const requestedSelectHandout = args.selectHandout;
        ...
        if(args.mode)
        {
            const leavingSearch = s.mode === "search" && args.mode !== "search";
            s.mode = args.mode;
            ...
            if(s.mode === "pins")
            {
                ...
                // No remembered pin, or it no longer resolves (e.g. deleted) --
                // clear the Content Panel instead of leaving stale Handout Mode
                // content displayed underneath the now-active Pin Mode nav list.
                if(!restoredPin)
                {
                    s.currentItemType = null;
                ...
                await new Promise(resolve =>
                {
                    handout.get('notes', async content =>
                    {
                        ...
                        if(!args.show)
                        {
                            ...
                            s.currentItemType = "header";
                            s.currentItemId   = '__all__';
                            s.displayHTML     = renderSectionHTML(
                                getHandoutAvatarHTML(s.selectedHandout) +
                                (await getHandoutSectionHTML(s.selectedHandout, '__all__', 'notes', isGM) || ""),
                                s
                            );
...
if(args.level)
{
    ...
    if(s.selectedHandout)
    {
        ...
        if(args.search)
        {
            // An empty/cancelled ?{} prompt parses to boolean true (parseArgs'
            // convention for a flag with no value), not a string -- leave any
            // existing results/display untouched in that case rather than
            // clearing them.
            const query = (typeof args.search === 'string') ? args.search.trim() : "";

            if(query)
            {
                const results = runSearch(query, isGM);

                s.mode            = "search";
                s._searchQuery    = query;
                s._searchResults  = results.map(r => ({ id: r.id, topHeader: (r.sections[0] && r.sections[0].header) || null }));
                s.currentItemType = "search";
                s.currentItemId   = null;
                s.currentIndex    = -1;
                s.currentList     = [];
                s.displayHTML     = rewriteHandoutLinks(buildSearchResultsHTML(results, query, isGM));
            }
        ...
        if(args.prev)
        {
            if(s.currentItemType === "header" && s.selectedHandout)
            {
                ...
                if(handout)
                {
                    ...
                    s.currentList = getNavigableHeaders(fullContent, isGM, s);
                }
            }
        ...
        {
            // cleanHeaderText matches the normalization extractHeaders/extractSection/
            // collectHeaderSections apply to header text pulled from content, so a
            // decoded &nbsp; (entity or literal U+00A0) at the edge of the text
            // doesn't cause a mismatch here regardless of which form the link's
            // source (a rewritten cross-handout anchor, or our own nav list) used.
            const headerText = cleanHeaderText(decodeURIComponent(args.show));
            ...
            if(handout)
            {
                ...
                // A combined "--selectHandout X --show Y" link already pushed the
                // correct {previous handout, previous header} pair in the
                // --selectHandout block above, before selectedHandout was reassigned.
                // Pushing again here would use the NEW handout paired with the OLD
                // header (issue: mismatched Back target). Only push here for an
                // ordinary same-handout header jump.
                const crossHandout = Boolean(requestedSelectHandout);
...
if(args['send-chat'] && s.displayHTML)
{
    ...
    const chatHTML  = rewriteLinksForChat(s.displayHTML);
    sendChat("", `/w gm <div style="padding:10px; border:1px solid #444; ${bgStyle}">${chatHTML}</div>`);
    return;
}
...
if(args['send-chat-players'] && s.selectedHandout)
{
    ...
    const bgStyle = getWikiBackgroundStyle();
const chatHTML = rewriteLinksForChat(rewriteHandoutLinks(playerContent));
sendChat(
    scriptName,
    `<div style="padding:10px; border:1px solid #444; ${bgStyle}">${chatHTML}</div>`
);
    return;
}
            ...
            if(handout)
            {
                await new Promise(resolve =>
                {
                    handout.get('notes', async content =>
                    {
                        ...
                        s.currentList         = getNavigableHeaders(content, isGM, s);
                        s.currentIndex        = target.headerText
                            ? s.currentList.findIndex(item => item.text === target.headerText)
                            : 0;
                        s.currentItemType     = "header";
                        s.currentItemId       = target.headerText || (s.currentList[0] && s.currentList[0].text) || null;
                        s.displayHTML         = renderSectionHTML(
