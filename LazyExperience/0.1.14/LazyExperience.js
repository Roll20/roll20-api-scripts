/* 
 * Version 0.1.14
 * Made By Robin Kuiper (bug fixes by The Aaron)
...
/* global TokenMod DeathTracker StatusInfo */
var LazyExperience = LazyExperience || (function() {
    ...
    asNum = (str) => (parseFloat(`${str}`.replace(/[^\d.]/g,''))||0),

    updateExperienceOnSheet = (experience, characterid) => {
        if(characterid){
            let new_experience = asNum(getAttrByName(characterid, state[state_name].config.experience_attribute_name, 'current')) + asNum(experience);
            ...
            for(let playerid in state[state_name].players){
                state[state_name].players[playerid].characters.forEach((character) => {
                    let new_experience = asNum(getAttrByName(character.id, state[state_name].config.experience_attribute_name, 'current')) + asNum(experience);
    ...
    refreshPlayers = () => {
        let saved_players = Object.keys(state[state_name].players).map((playerid) => { return playerid; });
