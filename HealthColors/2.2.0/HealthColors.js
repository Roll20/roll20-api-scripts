...
/* global createObj TokenMod spawnFxWithDefinition spawnFx getObj state playerIsGM sendChat findObjs Campaign log on */
...
var HealthColors =
  ...
  (() => {
    ...
    const UPDATED = '2026-06-29 17:35 UTC';
    ...
    * This HP-path manages the baseline:
    ...
    function setDeathSaves(enable) {
      ...
      if (enable) {
        gmWhisper('Death Save Integration enabled. Death-save marker syncing now updates automatically from watched attributes.');
    ...
    * Use `!aura deathsaves debug`.
      ...
      selected.forEach((sel) => {
        ...
        gmWhisper(
          [
            ...
            `<strong>deathSavesOn:</strong> ${s.deathSavesOn}`,
            `<strong>success attrs:</strong> ${escapeForChat(s.dsSuccessAttr)}`,
            `<strong>failure attrs:</strong> ${escapeForChat(s.dsFailureAttr)}`,
    ...
    function watchDeathSaveAttrs(msg, watchArg) {
      ...
      selected.forEach((sel) => {
        ...
        const current = collectDeathSaveWatchValues(character.id, watched);
        ...
        watched.forEach((name) => {
          ...
          if (!prev.exists && now.exists) {
            changes.push(`${name}: newly registered as "${escapeForChat(now.value)}"`);
            return;
          }
          if (prev.exists && !now.exists) {
            changes.push(`${name}: attribute disappeared`);
            return;
          }
          if (prev.value !== now.value) {
            if (prev.value === '' && now.value !== '') {
              changes.push(`${name}: new value registered "${escapeForChat(now.value)}"`);
            } else if (prev.value !== '' && now.value === '') {
              changes.push(`${name}: value cleared (was "${escapeForChat(prev.value)}")`);
            } else {
              changes.push(`${name}: "${escapeForChat(prev.value)}" → "${escapeForChat(now.value)}"`);
            }
          }
        });
    ...
    function whisperDeathSaveWatchStatus() {
      ...
      const lines = activeIds
        .map((id) => {
          ...
          const attrs = watched
            .map((attr) => {
              const entry = snap[attr];
              const value = entry?.exists ? `"${escapeForChat(entry.value)}"` : '(missing)';
              return `${attr}=${value}`;
            })
            .join(', ');
          return `<strong>${name}</strong>: ${attrs}`;
        })
        .join('<br>');
      ...
      gmWhisper(
        [
          ...
          activeIds.length ? lines : '<i>No active death-save watchers.</i>',
        ].join('<br>'),
      );
    }
     ...
     */
    function getDeathSaveCountsFromSnapshot(characterId, s) {
      const watched = getConfiguredDeathSaveWatchAttrs(s);
      const snapshot = s.dsAttrWatch?.[characterId] || collectDeathSaveWatchValues(characterId, watched);
    ...
    function getDeathSaveAttrKind(attrName, s) {
      const lowered = String(attrName || '').trim().toLowerCase();
    ...
    function syncDeathSaveMarkersForCharacter(characterId, s, changeHint) {
      ...
      const { succ, fail } = getDeathSaveCountsFromSnapshot(characterId, s);
      let desired = '';
    ...
    function whisperMarkerList() {
      ...
      const rows = custom
        ...
        .map((m) => {
          ...
          const setDying = makeBtn('<span style="color:#fff">Dying</span>', `!aura deathsaves dyingmarker ${tag}`, setBtnStyle);
          const setStable = makeBtn('<span style="color:#fff">Stable</span>', `!aura deathsaves stablemarker ${tag}`, setBtnStyle);
    ...
    function handleDeathSaves(msg, parts) {
      ...
      switch (sub) {
        ...
        default:
          gmWhisper(
            'Death Saves: use on, off, toggle, success &lt;attr,...&gt;, failure &lt;attr,...&gt;, dyingmarker &lt;marker&gt;, stablemarker &lt;marker&gt;, markers, attrs &lt;filter&gt;, watch [status|reset|off], watchstatus, debug.',
          );
      }
      return true;
    }
    ...
    function whisperWatchedAttrDelta(watchEnabled, charName, attrName, priorExists, priorValue, nowValue) {
      ...
      gmWhisper(`<strong>${charName}:</strong> ${safeAttrName} "${escapeForChat(priorValue)}" -> "${escapeForChat(nowValue)}"`);
    }
