...
(() => {
    ...
    function hexToRgb(hex) {
        const parts = (/^#?([0-9a-fA-F]{2})([0-9a-fA-F]{2})([0-9a-fA-F]{2})$/).exec(hex || '');
        if (parts) {
    ...
    function makeSmartAttrCache(attribute, options = {}) {
        ...
        return function(character) {
            ...
            )[0] || createObj('attribute', { name: attribute, characterid: character.id, current: defaultValue });
    ...
    function spawnFX(scale, hitSize, left, top, fx, pageId) {
        ...
        spawnFxWithDefinition(left, top, {
    ...
    function checkInstall() {
        ...
        if (!fxHurt) {
            ...
            createObj('custfx', { name: '-DefaultHurt', definition: DEFAULT_HURT_FX });
        }
        if (!fxHeal) {
            gmWhisper('Creating Default Heal FX');
            createObj('custfx', { name: '-DefaultHeal', definition: DEFAULT_HEAL_FX });
        }
    }
        ...
        if (useBlood === 'DEFAULT' || useBlood === undefined) {
            ...
            if (hurtRgb.some(v => v !== 0)) {
                ...
                useBlood.split(',').forEach(fxName => {
                    const custom = findObjs({ _type: 'custfx', name: fxName }, { caseInsensitive: true })[0];
    ...
    function maybeSpawnFX(obj, oCharacter, curValue, prevValue, maxValue, update) {
        ...
        buildFXList(isHeal, useBlood)
            .forEach(fx => spawnFX(scale, hitSize, obj.get('left'), obj.get('top'), fx, obj.get('pageid')));
    }
