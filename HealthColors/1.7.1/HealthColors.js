...
Version 1.7.1 - Mar 6 2025
  - Updated FX effects: The damage/healing FX now use the simpler DeathTracker approach.
    (Damage FX uses the HurtFX type (default "splatter-blood") and Healing FX uses the HealFX type (default "glow-holy").)
  - Added a new command/button ("LISTFX") that displays a graphical menu listing custom FX objects with their IDs.
*/
...
------------------------*/
        handleToken = function (obj, prev, update) {
            ...
            if(obj.get("represents") !== "" || (obj.get("represents") === "" && state.HealthColors.OneOff === true)) {
                ...
                //**FX for Damage/Healing using DeathTracker's approach------------
                if(curValue != prevValue && prevValue != "" && update !== "YES") {
                    ...
                    if(state.HealthColors.FX === true) {
                        if(curValue < prevValue) {
                            // Damage FX – using HurtFX type (default: splatter-blood)
                            spawnFxBetweenPoints({ x: left, y: top }, { x: left, y: top }, state.HealthColors.HurtFX, obj.get('_pageid'));
                        } else if(curValue > prevValue) {
                            // Healing FX – using HealFX type (default: glow-holy)
                            spawnFxBetweenPoints({ x: left, y: top }, { x: left, y: top }, state.HealthColors.HealFX, obj.get('_pageid'));
                        }
                    }
                }
            }
    ...
    // New function: List Custom FX IDs in a formatted menu
        listCustomFX = function() {
            var allCustFX = findObjs({ _type: "custfx" });
            var output = "<div style='border: 1px solid #000; background-color: #f2f2f2; padding: 5px; border-radius: 4px; max-width: 400px;'>";
            output += "<strong>Custom FX IDs:</strong><br><ul style='list-style: none; padding: 0; margin: 0;'>";
            if(allCustFX.length > 0) {
                allCustFX.forEach(function(custfx) {
                    var fxName = custfx.get("name") || "[Unnamed]";
                    output += "<li style='padding: 2px 0;'><span style='font-weight:bold;'>" + fxName + ":</span> " + custfx.get("_id") + "</li>";
                });
            } else {
                output += "<li>No custom FX objects found.</li>";
            }
            output += "</ul></div>";
            sendChat('HealthColors', "/w gm " + output);
        },
    //SET TOKEN COLORS------------
    ...
    //HELP MENU------------
        aurahelp = function (OPTION) {
            ...
            // Base style for buttons with fixed width (for most items)
            ...
            // FX type buttons use a wider style
            var fxStyle = 'style="padding-top: 1px; text-align:center; font-size: 9pt; width: 80px; height: 14px; border: 1px solid black; margin: 1px; background-color: #6FAEC7; border-radius: 4px; box-shadow: 1px 1px 1px #707070;"';
            // For buttons that should adjust to the text length, we use auto width.
            var autoStyle = 'style="padding: 1px 3px; text-align:center; font-size: 9pt; border: 1px solid black; margin: 1px; background-color: #6FAEC7; border-radius: 4px; box-shadow: 1px 1px 1px #707070; display:inline-block; width:auto;"';
            var off = "#A84D4D";
            var disable = "#D6D6D6";
            ...
            // New button renamed "Custom FX ids" using autoStyle so it adjusts to the text length.
            var listFXButton = '<a ' + autoStyle + ' href="!aura LISTFX">Custom FX ids</a><br>';
            sendChat('HealthColors', "/w gm <b><br>" +
                '<div style="border-radius: 8px; padding: 5px; font-size: 9pt; text-shadow: ' + tshadow + '; box-shadow: 3px 3px 1px #707070; ' + img + ' color:#FFF; border:2px solid black; text-align:right; vertical-align:middle;">' +
                ...
                'HealFX Type: <a ' + fxStyle + 'background-color:#' + state.HealthColors.HealFX + ';" href="!aura HEAL ?{FX Type?|glow-holy}">' + state.HealthColors.HealFX + '</a><br>' +
                'HurtFX Type: <a ' + fxStyle + 'background-color:#' + state.HealthColors.HurtFX + ';" href="!aura HURT ?{FX Type?|splatter-blood}">' + state.HealthColors.HurtFX + '</a><br>' +
    ...
    //CHECK INSTALL & SET STATE------------
        checkInstall = function () {
            ...
            // Updated defaults for FX types (using DeathTracker FX names)
            if(_.isUndefined(state.HealthColors.HealFX)) state.HealthColors.HealFX = "glow-holy"; //set Heal FX type
