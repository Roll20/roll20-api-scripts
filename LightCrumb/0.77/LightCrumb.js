...
/* 		statusMessage += "<li>";
		...
		statusMessage += "</li>"; */
        let token=getObj('graphic', currentMapperID);
        if(token){
            statusMessage += "<li>";
            currentMapperName = token.get("name");
            if (currentMapperName !== "" ) {
                statusMessage += currentMapperName;
            } else {
                statusMessage += currentMapperID;
...
*/	
    ...
    var handleRemove = function tokenRemoveHandler(obj){
        if(_.contains(activeLightCrumbMappers,obj.id)){
           deregisterMapper( obj.id, 'api');
        }
    };	
...
*/	
    ...
    var checkInstall = function () {
        ...
        // clean up mappers that have been deleted
        state.LightCrumb.activeMappers = _.without(
            state.LightCrumb.activeMappers,
            _.chain(state.LightCrumb.activeMappers)
                .map((id)=>{
                    return {
                        id: id,
                ...
                .filter((o)=>_.isUndefined(o.token))
                .pluck('id')
                .value()
        );		
...
/* ---------------------------------------------------------------------------- */
	...
	var registerEventHandlers = function() {
        ...
        on('remove:graphic', handleRemove );		
	};
