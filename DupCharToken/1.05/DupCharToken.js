...
//     This is OK except that the character copy is not done until the ending message comes.
...
// Update   1.04  Sep 2020  Chris D.  Fixed the bug where it would timeout, killing api sandbox, if copying too many attributes. Implemented burndown method.
// Update   1.05  Apr 2022  Chris D.  Fixed bug where thread that was doing setDefaultTokenForCharacter() was doing it before other threads were done and
//                                    new tokens dragged out are not correctly linked.  setDefaultTokenForCharacter() should now be guarantied to be last. 
//
//
// Commands:
            ...
            if ( _.isUndefined( charObj ) )
              ...
              for( let i = startnum; i < (startnum + num); ++i) {     // We are going to want to duplicate both the character and token this many times.
                ...
                tokenArray.push( newTObj );
                    ...
                    // Note, burndown is a method to make sure that the thread returns control to the operating system frequently enough that the op sys does
                    // not think it is stuck in an infinite loop. we call setTimeout after processing just one attribute or ability. 
                    // it first does all of attBurndown, and then AbBurndown, then charBurndown.
              ...
              const attBurndown = () => {       // create the function that will process the next element of the queue
                if( attQueue.length ) {
                  ...
                  let sa = JSON.parse( JSON.stringify( a ));    // Deep copy of the attribute. 
                  let lnk = links.indexOf( sa._id );    // Bar x is linked to the old attributes ID. Link it to the new one. 
                  delete sa._id;
                  delete sa._type;
                  for( let i = 0; i < charArray.length; ++i ) {
                    ...
                    if( lnk > -1 ) {
                      ...
                      tokenArray[ i ].set( "bar" + (lnk + 1).toString() + "_link", newA.id);   // Link the new token to the new attribute.
                    } else
                      createObj('attribute', sa);
                  };
                  setTimeout( attBurndown, 0 );
                } else  // Have finished the last attribute, now do the abilities. 
              ...
              const abBurndown = () => {        // create the function that will process the next element of the queue
                if( abQueue.length ) {
                  ...
                  setTimeout( abBurndown, 0);
                } else  // Have finished the last ability, now do the attributes. 
                  setTimeout( charBurndown, 0 );
              };    // end abBurndown;

              const charBurndown = () => {      // For each character.  This is done last as it burns down the character and token arrays. 
                if( charArray.length ) {
                  let c = charArray.shift(),
                      t = tokenArray.shift();
                  ...
                  toFront( t );
                  setTimeout( charBurndown, 0);
                } else  // Have finished everything. This is the last thread executing. 
                  sChat( "Done duplicating: " + charObj.get( "name" ) + " " + num + " times.   It took " + Math.round(((new Date().getTime()) - start) / 1000) + " seconds.");
              };    // end charBurndown

                    // The above routines have not executed yet. Get some info and execute them. 
              charObj.get("bio", function( b ) {
                bio = b;
                charObj.get("gmnotes", function( g ) {
                  gmnotes = g;
                  setTimeout( attBurndown, 0);       // start the execution of all the burnnowns by doing the first element
                }); });

              sChat( "Working on duplicating: " + charObj.get( "name" ) + " " + num + " times." );    // This main thread terminates way before the burndown threads. 
            }
          }   // End tokenObj is OK.
