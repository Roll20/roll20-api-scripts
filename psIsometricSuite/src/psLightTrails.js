...
var psLightTrails = psLightTrails || (function plexsoupLightTrailMaker() {
	...
	var debug = true; // set to true if you want to see logs
...
*/
	...
	var initializeUserCommands = function() {
		...
		ILScmd.parameters = ["current_page_id"];
	...
	var getLightTrailsStatus = function (pageID) {
		if (debug) log("entering getLightTrailsStatus with " + pageID);
		...
		statusMessage += "<h4>"+getTorchTokensFromLayer("objects", pageID).length + " Active LightTrailss</h4>";
...
*/
	...
	var reset = function resetter(pageID, parentTokenID, resetMode) {
		...
		if (debug) { log("entering reset with " + pageID); }
		...
		if (!getObj("page", pageID)  ) { // this pageID is not valid.
			...
			if (debug ) { log("    in reset: pageID is valid: " + pageID); }
		}
...
*/
	...
	var setAuraRadius = function(radius, pageID) {
		config.AURA_RADIUS = radius;
		simpleConvertCrumbs(undefined, {aura1_radius: radius} );
	};

	var toggleShowAura = function(pageID) {
	...
	var setLightRadius = function(radius, pageID) {
		config.LIGHT_RADIUS = radius;
		simpleConvertCrumbs(pageID, {light_radius: radius} );
	};
	
	var setDimRadius = function(radius, pageID) {
		config.DIM_RADIUS = radius;
		simpleConvertCrumbs(pageID, {dim_radius: radius});
	};
	
	var setCrumbSeparation = function(distance, pageID) {
		config.DROP_DISTANCE = distance;
		// no point converting the existing crumbs.
	};
	
	var chooseIcon = function(iconName, pageID) {
	...
	var simpleConvertCrumbs = function crumbConverter(pageID, requestedOptions) {
		if (debug) log("entering simpleConvertCrumbs with " + pageID + ", " + requestedOptions);
		if (pageID === undefined) {
			pageID = psUtils.GetPlayerPage("gm");
		}
		...
		_.each(crumbsToConvert, function loopToChangeCrumbs(torchToken) {
			_.each(requestedOptions, function(propertyValue, propertyName) {
				//if (debug) psGUI.psLog(info.name + ": setting " + propertyValue + " to " + propertyName, "orange");
	...
	var convertCrumbsToNewSettings = function crumbsResetter(pageID, requestedOptions, resetRequired) { // requestedOptions are an object with typical roll20 graphic object properties
		...
		if (debug) {log("entering convertCrumbsToNewSettings with " + JSON.stringify(requestedOptions) ); }
		...
		_.each(ICONS, function(imageUrl) { // note: this won't find anything where the user passed their own url to !LightTrails-image
			if (debug) {log("checking for: " + imageUrl);}
			_.each(findObjs({
					_pageid: pageID,
					_type: 'graphic',
					_subtype: 'token',
					...
					if (debug ) { log("converting lightcrumb token: " + currentCrumb.get("_id")); }
					
					// figure out who owns it.
					
					var currentCrumbID = currentCrumb.get("_id");
					if (debug) { log("currentCrumbID = " + currentCrumbID); }
					var parentTokenID = getParentTokenID(currentCrumbID);
					if (debug) { log("in convertCrumbsToNewSettings: parentTokenID = " + parentTokenID); }
					...
					if (debug) { log("in convertCrumbToNewSettings: controlledByString = " + controlledByString); }
		...
		if (debug) {log("leaving convertCrumbToNewSettings. Nothing to return."); }
	};
	...
	var getParentTokenID = function parentIDGetter(lightCrumbID) {  
		if (debug ) { log("entering getParentTokenID with " + lightCrumbID); }
		...
		if (!lightCrumbObj) {
			...
			if (debug) { log("exiting getParentTokenID: returning " + parentTokenID); }
	...
	var deregisterMapper = function mapperDeregistrar(tokenID, playerName) {
		...
		if (debug) { log("entering deregisterMapper with " + tokenID + "," + playerName );}
	...
	var registerMapper = function mapperRegistrar(tokenID, playerName) {
		if (debug) { log("entered registerMapper with: " + tokenID + ", " + playerName); }
	...
	var getOffsetLocation = function offsetLocationGetter(currentLocation, lastLocation) { // expect two points [x,y], [x1,y1]
		...
		if (debug ) { log("offsetVector = " + offsetUnitVector); }
		if (debug ) { log("offsetLocation = " + offsetLocation); }
...
*/
	...
	var dropSingleTorch = function lightCrumbDropper(customLightTrailsSettings, parentToken) { // expects an object with properties matching roll20 graphic objects
		...
		if (debug) { log("entered dropSingleTorch with " + JSON.stringify(customLightTrailsSettings)); }
		...
		if(currentTorchToken) {
			...
			if (debug) { log("Found existing lightCrumb on GM Layer" + currentTorchToken); }
			...
			if (debug) { log("No existing lightcrumb. About to make one."); }
			currentTorchToken = createObj('graphic',customLightTrailsSettings);
			
			if (debug) {log( "dropped brand new currentTorchToken: " + JSON.stringify(currentTorchToken) );}
		}
		...
		if (debug) {
			...
			var allLightTrailssOnPlayerLayer = getTorchTokensFromLayer("objects", currentPageID);
			log("There are " + allLightTrailssOnPlayerLayer.length + " known torch tokens on object layer: ");
		}

		if ( debug ) { log("leaving dropSingleTorch. Returning currentTorchToken." + JSON.stringify(currentTorchToken)); }
	...
	var findShortestDistance = function shortestDistancetoTokenFinder(listOfGraphicObjectsToCompare, referenceLocation, tokenMoved ) {
		...
		if (debug) { log("entering findShortestDistance with: " + listOfGraphicObjectsToCompare + ", " + referenceLocation + ", " + tokenMoved); }
		...
		if (debug) { log("leaving findShortestDistance: returning shortestDistance = " + shortestDistance); }
...
*/
	...
	var handleTokenMove = function tokenMoveHandler(tokenMoved) {
		...
		if (activeLightTrailsMappers.indexOf(tokenID) !== -1) { // you're authorized to leave a trail of light.. continue
			if(debug) {log("in handleTokenMove. About to get tokenLocation");}
			...
			if (debug ) { log("crumbsToConsider before SHARED_VISION test: " + _.size(crumbsToConsider) ); }
			...
			if (debug ) { log("crumbsToConsider after SHARED_VISION test: " + _.size(crumbsToConsider + ", " + JSON.stringify(crumbsToConsider)) ); }
						
			shortestDistanceToExistingCrumb = findShortestDistance(crumbsToConsider, referenceLocation, tokenMoved);            

			if (debug) { log("config.DROP_DISTANCE: " + config.DROP_DISTANCE + ", Current Distance: " + shortestDistanceToExistingCrumb); }
			...
			if ( shortestDistanceToExistingCrumb == -1 || shortestDistanceToExistingCrumb > minSeperation ) {
				if (debug) { log("LightTrails.handleTokenMove: registered token, "+ tokenID +", moved. dropping light token"); }
				...
				if (debug) { log("    in handleTokenMove: controllingPlayerID = " + controllingPlayerID ); }
				if ( config.SHARED_VISION === true ) {
					controllingPlayerID += ",all";
				}
				...
				if (debug) { log("    in handleTokenMove: dropped LightTrails [" + lastLocation + "], "+ tokenPageID); }

			}

		}
		if (debug) { log("leaving handleTokenMove. Nothing to return."); }
		return;
	};
...
*/  
	...
	var checkInstall = function () {
		...
		if( ! _.has(state,'psLightTrails') ) { // first time running the LightTrails script for this Roll20 campaign. It needs a state.
			...
			if (debug) { log("state.psLightTrails.config = " + JSON.stringify(state.psLightTrails.config) ); }
			if (debug) { log("config = " + JSON.stringify(config) ); }
