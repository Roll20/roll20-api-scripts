...
const ShowButtons = (() => { // eslint-disable-line no-unused-vars
    ...
    const vd = new Date(1711998033639);
    const versionInfo = () => {
    ...
    const handleInput = (msg) => {
        ...
        let reportList = {};
        const finders = {
            macro: ({ name }) => findObjs({ type: 'macro', name })[0],
            ability: ({ name, characterid }) => findObjs({ type: 'ability', name, characterid })[0]
        };

        Object.keys(argObj).forEach(k => {
            argObj[k].forEach(a => {
                let character = getChar(a.char, msg.playerid) || { id: undefined };
                let o = (finders[a.type] || (() => { }))({ name: a.query, characterid: character.id });
                if (o) {
                    reportList[k] = [...(reportList[k] || []), { ...a, action: switchObj[k](o.get('istokenaction')) }];
        ...
        if (report) {
            ...
            let message = html.table(
                ...
                Object.keys(reportList, {})
                    .filter(k => k !== 'notfound')
                    .map(k => {
                        return reportList[k].map(o => {
                            return html.tr(html.td('L', localCSS.actionindicator, { 'color': o.action ? '#0bcd0b' : '#DD2222' }) + html.td(o.type === 'ability' ? '%' : '#', {'text-align': 'right'}) + html.td(o.query) + html.td(o.char))
                        }).join('')
                    }).join('')
            );
    ...
    const handleConfig = msg => {
        ...
        let cfgrx = /^(\+|-)(report|playerscanids)$/i;
        let res;
        let cfgTrack = {};
        let message;
