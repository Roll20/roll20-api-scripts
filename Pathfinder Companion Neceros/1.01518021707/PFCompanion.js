...
*/
    ...
    var version = '1.01518021707',
        sheetVersion = [1.6,1.7],
        lastUpdate = 1518021707,
        schemaVersion = 1.01518021707,
        defaults = {
            css: {
                button: {
    ...
    handleSpellCommand = function(spell,character,spellClass,changeCurrent,silent){
        ...
        var attributes = findObjs({type:'attribute',characterid:character.id}),
            ...
            log('handlespellcommand');
            log(`spell:${spell} | character: ${character} | spellClass: ${spellClass} | ${changeCurrent} | silent: ${silent}`);
            ...
            if(spellMax){
                ...
                if(!silent){
                    insufficient.then((i)=>{
                        ...
                        +'{{qty_max=-}}')
                        +'{{spelltracking1=[**_**](!pfc --resource,spell='+spell+',current=-1|'+character.id+')[**&**](!pfc --resource,spell='+spell+',current=+1|'+character.id+')[**?**](!pfc --resource,spell='+spell+',current=?'+HE('{')+spell+' Adjustment}|'+character.id+'})[**1**](!pfc --resource,spell='+spell+',current=0|'+character.id+')}}'
