...
<p>These handouts are automatically created and updated. Do not manually edit their GM Notes section, as this may corrupt your calendar data.</p>
  ...
  const InterfaceRenderer = {
    ...
    renderCalendarCell: (date, calendar, otherMonth, data) => {
      ...
      html += `<a style="display: block; text-decoration: none; color: inherit; height: 100%;" href="!chr --viewdate ${date.year}|${date.month}|${date.day}">`;
      ...
      if (hasContent) {
        if (verboseMode) {
          ...
          html += '<div style="font-size: 11px; margin-top: 3px; clear: both;">';
          if (events.length > 0) {
          ...
          // Indicator only
          html += '<div style="font-size: 9px; margin-top: 2px; font-weight: bold; clear: both;">';
  ...
  const Commands = {
    ...
    sendCalendarToChat: (msg) => {
      DataLoader.loadAll((data) => {
                  ...
                  if (todayWeather) {
                    output += `<div style="margin-top:5px;font-size:11px;"><strong>Weather:</strong> ${todayWeather.description} (${todayWeather.temperature.value}°${todayWeather.temperature.unit})</div>`;
                  }
                  ...
                  if (todayEvents.length > 0) {
                    output += '<div style="margin-top:5px;font-size:11px;"><strong>Events:</strong></div><ul style="margin:2px 0;padding-left:15px;font-size:10px;">';
                    todayEvents.forEach(e => output += `<li>${e.content}</li>`);
                    output += '</ul>';
                  }
                  ...
                  if (todayNotes.length > 0) {
                    output += '<div style="margin-top:5px;font-size:11px;"><strong>Notes:</strong></div><ul style="margin:2px 0;padding-left:15px;font-size:10px;">';
                    todayNotes.forEach(n => output += `<li>${n.content}</li>`);
                    output += '</ul>';
                  }
                  ...
                  for (let i = 0; i < Math.min(daysInWeek, 7); i++) {
                    ...
                    if (day < 1 || day > daysInMonth) {
                      ...
                      if (w) output += `<div style="font-size:8px;">${WeatherGenerator.getWeatherEmoji(w.description)}</div>`;
