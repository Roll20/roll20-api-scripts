  ...
  const Commands = {
    ...
    root: (msg, parsed) => {
      ...
      if (args.chat) {
        ...
        } else if (args.chat === 'timeline') {
          return Commands.sendTimelineToChat(msg);
    ...
    sendTimelineToChat: (msg) => {
      DataLoader.loadAll((data) => {
        ...
        if (timelineState.showEvents) {
          events.forEach(e => {
            if (!(e.tags && e.tags.includes('gm'))) {
              allItems.push({ ...e, type: 'event', date: e.dateRef });
            }
          });
        }
        ...
        if (timelineState.showNotes) {
          notes.forEach(n => {
            if (!(n.tags && n.tags.includes('gm'))) {
              allItems.push({ ...n, type: 'note', date: n.dateRef });
            }
          });
        }

        // Filter by tags
        if (timelineState.selectedTags.length > 0) {
          allItems = allItems.filter(item => {
            ...
            if (timelineState.tagMode === 'OR') {
              ...
              return timelineState.selectedTags.every(tag => itemTags.includes(tag));
            }
          });
        } else if (timelineState.showUntagged) {
          allItems = allItems.filter(item => !item.tags || item.tags.length === 0);
        } else {
          allItems = [];
        }
        ...
        // Build output
        let output = `<div style="${CSS_CURRENT.chatOutput}">`;
        output += `<div style="font-weight: bold; margin-bottom: 10px; font-size: 14px;">Timeline</div>`;
        ...
        allItems.forEach(item => {
          ...
          // Calculate weekday
          let weekdayName = '';
          if (!d.isInterannual && d.month && d.day) {
            const absDay = DateUtils.toAbsoluteDay(d, calendar);
            const weekdayIndex = (absDay - 1) % calendar.weeks.daysInWeek;
            weekdayName = calendar.weeks.weekdayNames[weekdayIndex] || 'Day';
          }

          // Calculate elapsed time using the exact format from timeline view
          const referenceDate = State.config().currentDate || { year: 1, month: 1, day: 1 };
          const elapsed = DateUtils.getElapsedTime(referenceDate, d, calendar);
          let elapsedText = '';
          if (elapsed.isFirstOfYear) {
            elapsedText = (elapsed.isNegative ? '-' : '') + elapsed.years + 'y';
          } else {
          ...
          if (d.isInterannual) {
            ...
            if (d.month !== lastMonth) {
              dateCell += `<strong>${monthName}</strong><br>`;
              lastMonth = d.month;
            }
            dateCell += `<span>${weekdayName} ${d.day}</span>`;
          }
        ...
        // Active tags section
        if (timelineState.selectedTags.length > 0) {
          // Get all unique tags from events and notes
          const allTags = new Set();
          events.forEach(e => {
            if (e.tags) e.tags.forEach(t => allTags.add(t));
          });
          ...
          if (timelineState.selectedTags.length === allTags.size) {
            ...
            const displayTags = timelineState.selectedTags.slice(0, 10);
            output += displayTags.join(', ');
            if (timelineState.selectedTags.length > 10) {
              output += ' ...';
            }
          }
