...
const Walls = (() => { // eslint-disable-line no-unused-vars
  ...
  const handleCommandArray = (args,selected,who) => {
    let mapGraphic,left,top,width,height;
    switch(args[0]) {

      case 'begin':
        resetWork();
        if(selected && 1 === selected.length && 'graphic'=== selected[0]._type) {
          mapGraphic=getObj('graphic',selected[0]._id);
          if(mapGraphic) {
  ...
  const ImportFromGMNotes = (token,who) => {
    let rawNotes = token.get('gmnotes');
    let notes = unescape(rawNotes).replace(/(?:<[^>]*>|\\t|&nbsp;)/g,'').split(/!walls\s*/).filter(s=>s.length);

    const burndown = () => {
      let c = notes.shift();
      if(c){
        let args = c.split(/\s+/);
        handleCommandArray(args,[{_id:token.id, _type: "graphic"}],who);
  ...
  const handleInput = (msg) => {
    if('api' === msg.type && /^!walls\b/i.test(msg.content) && playerIsGM(msg.playerid)){
      ...
      handleCommandArray(args,msg.selected,who);
    }

    if('api' === msg.type && /^!walls-gmnotes\b/i.test(msg.content) && playerIsGM(msg.playerid)){
      let who = (getObj('player',msg.playerid)||{get:()=>'API'}).get('_displayname');
      ...
      if(graphics.length) {
          graphics
            .map(g=>ImportFromGMNotes(g,who));
      } else {
        sendChat('',`/w "${who}" Select a graphic with walls commands in the GMNotes.`);
      }
    }
  };
