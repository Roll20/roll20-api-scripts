...
//		log('tableLookup returning undefined');
	 ...
	 */
/*	 
	var createTableRow = function( tableObj, name, attrValue ) {
	
		var	col = tableObj.column,
			rowID = generateRowID(),
			rowNo = tableObj.sortKeys.length,
			rowObj,
			newTableObj = tableObj,
			value = {},
			finalName, rowObj;
			
//        _.each(newTableObj,(elem,key) => {
//            if (!elem.property) {return;}
			finalName = tableObj.table[0]+col+'_'+rowID+'_'+tableObj[name].property[0]+col,
			rowObj = createObj( "attribute", {characterid: tableObj.character.id, name: finalName} );
			value[tableObj[name].property[1]] = String(attrValue);
			rowObj.setWithWorker(value);
			
//			rowObj.setWithWorker(tableObj[key].property[1],String(tableObj[key].defaultVal));
            tableObj = getTable( tableObj.character, tableObj, name, tableObj.table, tableObj[name].property, tableObj.column, tableObj[name].defaultVal );
//            log('createTableRow got new table '+tableObj.table[0]+', key '+key+', length '+tableObj.sortKeys.length);
//	    });
		return tableObj;
	};
	...
	var addTableRow = function( tableInfo, tableType, index, values ) {
	    
		let rows = {}, rowObj, tableObj = tableInfo[tableType];
		    
	    // log('addTableRow called, tableType='+tableType+', index='+index);
		if ((index < 0) || !_.isUndefined(tableLookup( tableObj, (tableObj.fieldGroup+'name'), index, false ))) {
			rowObj = tableObj;
			_.each( rowObj, (elem,key) => {

				if (_.isUndefined(elem.property)) {/*log('addTableRow '+tableType+'.'+key+'.property is undefined');*/return;}
				if (_.isUndefined(values[elem.property[0]])) {/*log('addTableRow values['+elem.property[0]+'] is undefined')*/;return;}
				// log('addTableRow setting '+tableType+'.'+key+'.'+elem.property[1]+' as '+values[elem.property[0]][elem.property[1]] );
				tableObj = tableSet( tableObj, key, index, values[elem.property[0]][elem.property[1]] );
			});
		} else {
			let newVal, namePt1 = tableObj.table[0]+'_'+generateRowID()+'_';
			_.each( fields, (elem,key) => {
				// log('addTableRow found elem '+key+'.'+elem[0]+'.'+elem[1]);
				if (key.toLowerCase().startsWith(tableObj.fieldGroup) && String(elem[1]).toLowerCase() == 'current') {
					rows[key] = elem;
					// log('addTableRow creating elem[0] '+elem[0]+', values[elem[0]].current '+values[elem[0]].current+' max '+values[elem[0]].max);
					rowObj = createObj( "attribute", {characterid: tableObj.character.id, name: (namePt1+elem[0])} );
					if (_.isUndefined(values[elem[0]])) {
						newVal = {current: (_.isUndefined(elem[2] ? '' : elem[2])), max: ''};
					} else {
						newVal = values[elem[0]];
					}
		...
		return weaponTables( tableObj.character, tableType, tableInfo );
	}
	...
	var tableFind = function( tableObj, name, val ) {
		...
		if (!tableObj.table[1] && val == attrLookup( tableObj.character, tableObj[name].property )) {
			return -1;
		}
		let tableIndex = tableObj.sortKeys.indexOf(
								_.findKey(tableObj[name].attrs, function( elem, objID ) {return val == elem.get(property);})
								);
...
//		log('classProficiency wt='+wt+', wst='+wst+', validWeapon='+validWeapon);
	...
	var initValues = function( fieldGroup ) {
        ...
        fieldGroup = fieldGroup.toLowerCase();

		var values = [new Set()],
    		rows = _.filter( fields, (elem,f) => {return f.toLowerCase().startsWith(fieldGroup)})
						.map(elem => {
							// log('initValues values['+elem[0]+'] initialised');
	...
	var insertAmmo = function( charCS, ammoName, ammoSpecs, rangeSpecs, tableInfo, ammoType, sb, miIndex ) {
		...
		for (let w=0; w<ammoSpecs.length; w++) {
			...
			tableInfo = addTableRow( tableInfo, 'AMMO', tableInfo.ammoInfo.ammoIndex++, values );
		}
		return tableInfo;
	}
		...
		for (let i=0; i<Math.min(weaponSpecs.length,toHitSpecs.length); i++) {
			...
			if ((noOfHands == 0) || (noOfHands ==  (attk2H+1))) {
    			...
    			if (weapon[2].toLowerCase() == 'melee') {
					...
					tableInfo = addTableRow( tableInfo, 'MELEE', indicies.MW, values );
    				...
    				if (dmgSpecs && i<dmgSpecs.length && !_.isUndefined(dmg=dmgSpecs[i][0])) {
						...
						tableInfo = addTableRow( tableInfo, 'DMG', indicies.MW, values );
    				} else {
					...
					tableInfo = addTableRow( tableInfo, 'RANGED', indicies.RW, values );
...
//							    log('addAmmo and found range.  Inserting ammo');
	...
	var weaponTables = function( charCS, tableTypes, tableInfo ) {
		...
		_.each( tableTypes, setupType => {
			...
			_.each( fields, (elem,key) => {
				if (key.toLowerCase().startsWith(tableInfo[setupType].fieldGroup) && ['current','max'].includes(String(elem[1]).toLowerCase())) {
...
// --------------------------------------------------------------- Button press Handlers ----------------------------------------------
	...
	var handleAmmoChange = function( args ) {
		...
		var tokenID = args[1],
			...
			Ammo = getTable(charCS,Ammo,'Ammo_miName',fields.Ammo_table,fields.Ammo_miName),
			ammoIndex = tableFind( Ammo, 'Ammo_name', ammoName ),
			ammoMIname = tableLookup( Ammo, 'Ammo_miName', ammoIndex),
			MagicItems = getTable(charCS,{},'MI_name',fields.MI_table,fields.MI_name),
		...
		var	Ammo = getTable(charCS,Ammo,'Ammo_qty',fields.Ammo_table,fields.Ammo_qty,'',0),
			AmmoMax = getTable(charCS,Ammo,'Ammo_maxQty',fields.Ammo_table,fields.Ammo_maxQty,'',0),
			MagicItems = getTable(charCS,MagicItems,'MI_qty',fields.MI_table,fields.MI_qty,'',0),
		...
		if (!isNaN(ammoIndex) && ammoIndex >= -1) {
			Ammo = tableSet( Ammo, 'Ammo_qty', ammoIndex, qty );
			Ammo = tableSet( Ammo, 'Ammo_maxQty', ammoIndex, maxQty );
		}
		if (!silent) {
			makeAmmoMenu( args );
