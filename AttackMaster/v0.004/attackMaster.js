...
// -------------------------------------------- utility functions ----------------------------------------------
	...
	var getTable = function(character,tableDef,attrDef,col='',caseSensitive=false) {
	    let name;
        ...
        if(match){
            ...
            return {character:character,attrs:attrs,sortKeys:sortOrderKeys,property:attrDef};
        }
		return undefined;
	};
	...
	var tableLookup = function( tableObj, index, getObj=false ) {
//        log('tableLookup tableObj:'+!!tableObj+', index:'+index+', getObj:'+getObj);
		if (tableObj && !_.isNull(tableObj)) {
			...
			if (index>=0) {
				...
				if (index<sortOrderKeys.length && _.has(attrs,sortOrderKeys[index])){
					if (_.isUndefined(property) || _.isNull(property) || !property || !property[1] || _.isUndefined(attrs[sortOrderKeys[index]])) {
						...
						return attrs[sortOrderKeys[index]].get(property[1]);
					}
				}
				...
				return attrLookup( tableObj.character, (index<0 ? [property[0],null] : property) );
			}
		}
//		log('tableLookup returning undefined');
	...
	var tableSet = function( tableObj, attrDef, attrValue, tableDef, r, c, caseSensitive ) {
		if (tableObj) {
			let attrs = tableObj.attrs,
				sortOrderKeys = tableObj.sortKeys;
			...
			setAttr( tableObj.character, attrDef, attrValue, tableDef, r, c, caseSensitive );
		}
		return undefined;
	}
...
//			if (_.isNull(tableDef)) {log('attrLookup tableLookup for '+tableObj.property);}
			return tableLookup(tableObj,index);
		} else {
	...
	**/
	...
	var weaponQuery = function( charCS, handed ) {
		...
		do {
			item = tableLookup( MInames, i );
			if (_.isUndefined(item)) {log('weaponQuery reached end of MI list, i:'+i);break;}
...
//			log('weaponQuery i:'+i+', item:'+item+', itemDef.dB:'+itemDef.dB+', itemDef.obj[0]:'+itemDef.obj[0]);
			if (itemDef.obj[0]) {
				let re = new RegExp( '{{\\s*?weapon\\s*?=.*?[\\[,]\\s*?'+handed+'H\\s*?[\\],].*?}}', 'i' ),
				    action = itemDef.obj[0].get('action');
				if (re.test(itemDef.obj[0].get('action'))) {
...
//		log('final i:'+i+', fields.MIRows:'+fields.MIRows);
	...
	var proficient = function( charCS, weaponType, weaponSuperType ) {
		...
		var i = fields.WP_table[1] ? 0 : -1,
			...
			spec;
		do {
			let wpName = tableLookup( WPnames, i ),
				wpType = tableLookup( WPtypes, i );
			if (_.isUndefined(wpName)) {break;}
			if (weaponType.includes(wpName)) {
				prof = 0;
			...
			} else if (wpType && weaponSuperType.includes(wpType)) {
				prof = -1;
			}
			i++;
		} while (prof < 0);
	...
	var addWeapon = function( charCS, hand, lineNo, tableInfo ) {
	    log('addWeapon hand:'+hand+', lineNo:'+lineNo);
		if (isNaN(lineNo) || lineNo < -1) {return tableInfo;}
		var indicies = tableInfo.indicies,
			...
			weaponSpecs = [...('['+specs.match(/{{\s*weapon\s*=(.*?)}}/im)[0]+']').matchAll(/\[\s*?(\w[\s\;\w\-]*?)\s*?,\s*?(\w[\s\w]*?\w)\s*?,\s*?(\w[\s\w]*?\w)\s*?,\s*?(\w[\s\w\-]*?\w)\s*?\]/g)],
			toHitSpecs = [...('['+specs.match(/{{\s*to-hit\s*=(.*?)}}/im)[0]+']').matchAll(/\[[\s\w\-\+\,\:\/]+?\]/g)],
			dmgSpecs = [...('['+specs.match(/{{\s*damage\s*=(.*?)}}/im)[0]+']').matchAll(/\[[\s\w\-\+\,\:\/]+?\]/g)],
			dmg,
			ammoInfo = {};
		...
		for (let i=0; i<Math.min(weaponSpecs.length,toHitSpecs.length); i++) {
			let weapon = weaponSpecs[i],
				...
				proficiency = proficient( charCS, weapon[1], weapon[4] );
			...
			let attkName = (toHit.match(/w:([\s\w\-\+\,\:]+?)[,\]]/i) || [''])[1],
				attk2H   = weapon[3]=='2H' ? 1 : 0,
				attkStrBonus = (toHit.match(/sb:([01])/i) || [0])[1],
				attkDexBonus = (toHit.match(/db:([01])/i) || [1])[1],
				attkAdj = (toHit.match(/\+:([+-]?\d+?)[,\]]/i) || [0])[1],
				attkNum = (toHit.match(/n:([\d\/]+)[,\]]/i) || [1])[1],
				attkCritHit = (toHit.match(/c:(\d+?)[,\]]/i) || [20])[1],
				attkCritMiss = (toHit.match(/m:(\d+?)[,\]]/i) || [1])[1],
				attkSize = (toHit.match(/sz:([tsmlh])/i) || ['M'])[1],
				attkRange = (toHit.match(/r:([\d\/]+)[,\]]/i) || [''])[1],
				attkSpeed = (toHit.match(/sp:(\d+?)[,\]]/i) || [5])[1];
			if (weapon[2].toLowerCase() == 'melee') {
			    log('setting up melee weapon '+indicies.MW+' as '+attkName);
				setAttr( charCS, fields.MW_name,     attkName,     fields.MW_table, indicies.MW );
				setAttr( charCS, fields.MW_type,     weapon[1],    fields.MW_table, indicies.MW );
				setAttr( charCS, fields.MW_twoHanded,attk2H,       fields.MW_table, indicies.MW );
				setAttr( charCS, fields.MW_strBonus, attkStrBonus, fields.MW_table, indicies.MW );
				setAttr( charCS, fields.MW_attkAdj,  attkAdj,      fields.MW_table, indicies.MW );
				setAttr( charCS, fields.MW_noAttks,  attkNum,      fields.MW_table, indicies.MW );
				setAttr( charCS, fields.MW_critHit,  attkCritHit,  fields.MW_table, indicies.MW );
				setAttr( charCS, fields.MW_critMiss, attkCritMiss, fields.MW_table, indicies.MW );
				setAttr( charCS, fields.MW_size,     attkSize,     fields.MW_table, indicies.MW );
				setAttr( charCS, fields.MW_range,    attkRange,    fields.MW_table, indicies.MW );
				setAttr( charCS, fields.MW_speed,    attkSpeed,    fields.MW_table, indicies.MW );
				setAttr( charCS, fields.MW_profLevel,Math.min(proficiency,1),fields.MW_table, indicies.MW );
				setAttr( charCS, fields.MW_superType,weapon[4],    fields.MW_table, indicies.MW );
				if (_.isUndefined(dmg=dmgSpecs[i][0])) {break;}
				log('addWeapon setting up dmg');
				setAttr( charCS, fields.MW_dmgName,      (dmg.match(/w:([\s\w\-\+\,\:]+?)[,\]]/i) || [''])[1], fields.MW_dmgTable, indicies.MW );
				setAttr( charCS, fields.MW_dmgStrBonus,  (dmg.match(/sb:([01])/i) || [0])[1],                  fields.MW_dmgTable, indicies.MW );
				setAttr( charCS, fields.MW_dmgAdj,       (dmg.match(/\+:([+-]?\d+?)[,\]]/i) || [0])[1],             fields.MW_dmgTable, indicies.MW );
				setAttr( charCS, fields.MW_dmgSM,        (dmg.match(/sm:(.*?\d+?d\d+?.*?)[,\]]/i) || [0])[1],  fields.MW_dmgTable, indicies.MW );
				setAttr( charCS, fields.MW_dmgL,         (dmg.match(/l:(.*?\d+?d\d+?.*?)[,\]]/i) || [0])[1],   fields.MW_dmgTable, indicies.MW );
				setAttr( charCS, fields.MW_dmgSpecialist,(proficiency>=1)?1:0,                                 fields.MW_dmgTable, indicies.MW );
				indicies.MW++;
			} else if (weapon[2].toLowerCase() == 'ranged') {
			    log('addWeapon setting up ranged weapon '+indicies.RW+' as '+attkName);
				setAttr( charCS, fields.RW_name,     attkName,     fields.RW_table, indicies.RW );
				setAttr( charCS, fields.RW_type,     weapon[1],    fields.RW_table, indicies.RW );
				setAttr( charCS, fields.RW_twoHanded,attk2H,       fields.RW_table, indicies.RW );
				setAttr( charCS, fields.RW_strBonus, attkStrBonus, fields.RW_table, indicies.RW );
				setAttr( charCS, fields.RW_dexBonus, attkDexBonus, fields.RW_table, indicies.RW );
				setAttr( charCS, fields.RW_attkAdj,  attkAdj,      fields.RW_table, indicies.RW );
				setAttr( charCS, fields.RW_noAttks,  attkNum,      fields.RW_table, indicies.RW );
				setAttr( charCS, fields.RW_critHit,  attkCritHit,  fields.RW_table, indicies.RW );
				setAttr( charCS, fields.RW_critMiss, attkCritMiss, fields.RW_table, indicies.RW );
				setAttr( charCS, fields.RW_size,     attkSize,     fields.RW_table, indicies.RW );
				setAttr( charCS, fields.RW_speed,    attkSpeed,    fields.RW_table, indicies.RW );
				setAttr( charCS, fields.RW_profLevel,Math.min(proficiency,0),fields.RW_table, indicies.RW );
				setAttr( charCS, fields.RW_superType,weapon[4],    fields.RW_table, indicies.RW );
//				ammoInfo = addAmmo( charCS, weapon[1], weapon[4], proficiency, tableInfo.ammoInfo );
//				setAttr( charCS, fields.RW_range,    attkRange,    fields.RW_table, indicies.RW );
				indicies.RW++;
			}
		}
		return {indicies:indicies, ammoInfo:ammoInfo};
	};
	...
	var addAmmo = function( charCS, weaponType, weaponSuperType, proficiency, sb, ammoInfo ) {
		
		var i = fields.MI_table[1] ? 0 : -1,
			ammoIndex = ammoInfo.ammoIndex,
			ammoTypes = ammoInfo.ammoTypes,
			MInames = getTable( charCS, fields.MI_table, fields.MI_name ),
			attrs, sortKeys, ammoName,
			ammoSpecs, rangeSpecs, t;

		ammoName = tableLookup(MInames,i);
			
		while (!_.isUndefined(ammoName)) {
			let ammo = abilityLookup( fields.MagicItemDB, ammoName );
			if (ammo.obj) {
				ammoSpecs = [...('['+ammo.obj.get('action')
											.match(/{{\s*?ammo\s*?=.*?}}/im)+']')
											  .matchAll(/\[.*?\]/g)]
											    .filter(elem => elem.test(new RegExp('t:\\s*?'+weaponType+'\\s*?[,\\]]', 'i')));
				t = weaponType;
				if (!ammoSpecs.length) {
					ammoSpecs = [...('['+ammo.obj.get('action')
											.match(/{{\s*?ammo\s*?=.*?}}/im)+']')
											  .matchAll(/\[.*?\]/g)]
											    .filter(elem => elem.test(new RegExp('st:\\s*?'+weaponSuperType+'\\s*?[,\\]]', 'i')));
					t = weaponSuperType;
				}
				if (!!ammoSpecs.length) {
					rangeSpecs = [...('['+ammo.obj.get('action')
												.match(/{{\s*?range\s*?=.*?}}/im)+']')
												  .matchAll(/\[.*?\]/g)]
												    .filter(elem => elem.test(new RegExp( 't:\\s*?'+weaponType+'\\s*?[,\\]]','i' )));
					if (!rangeSpecs.length) {
						rangeSpecs = [...('['+ammo.obj.get('action')
												.match(/{{\s*?range\s*?=.*?}}/im)+']')
												  .matchAll(/\[.*?\]/g)]
												    .filter(elem => elem.test(new RegExp( 't:\\s*?'+weaponSuperType+'\\s*?[,\\]]','i' )));
					}
				}
				
				if (ammoSpecs.length && rangeSpecs.length) {
					setAttr( charCS, fields.Ammo_type,       t,                                                            fields.Ammo_table, ammoIndex );
					setattr( charCS, fields.Ammo_specialist, Math.max(proficiency,0),                                      fields.Ammo_table, ammoIndex );
					setAttr( charCS, fields.Ammo_name,       ammoName,                                                     fields.Ammo_table, ammoIndex );
					setAttr( charCS, fields.Ammo_strBonus,   (ammoSpecs[w].match(/sb:\s*?([01])\s*?[,\]]/i)[1] || 0),      fields.Ammo_table, ammoIndex );
					setAttr( charCS, fields.Ammo_dmgAdj,     (ammoSpecs[w].match(/\+:\s*?([+-]?\d+?)\s*?[,\]]/i)[1]||0),   fields.Ammo_table, ammoIndex );
					setAttr( charCS, fields.Ammo_attkAdj,    (rangeSpecs[0].match(/\+:\s*?([+-]?\d+?)\s*?[,\]]/i)[1]||''), fields.Ammo_table, ammoIndex );
					setAttr( charCS, fields.Ammo_dmgSM,      (ammoSpecs[w].match(/sm:(.*?\d+?d\d+?.*?)[,\]]/i)[1] || 0),   fields.Ammo_table, ammoIndex );
					setAttr( charCS, fields.Ammo_dmgL,       (ammoSpecs[w].match(/l:(.*?\d+?d\d+?.*?)[,\]]/i)[1] || 0),    fields.Ammo_table, ammoIndex );
					setAttr( charCS, fields.Ammo_range,      (rangeSpecs[0].match(/r:\s*?([\d\/]+?/i)[1]||''),             fields.Ammo_table, ammoIndex );
					setAttr( charCS, fields.Ammo_qty,        (attrLookup( charCS, fields.MI_qty ) || 1),                   fields.Ammo_table, ammoIndex );
					setAttr( charCS, fields.Ammo_maxQty,     (attrLookup( charCS, fields.MI_qty ) || 1),                   fields.Ammo_table, ammoIndex );
					ammoTypes.push(t+'-'+a);
					ammoIndex++;
				}
			}
	...
	var findWeapon = function( charCS, ...weapons ) {
		...
		while (!_.isUndefined(itemName = tableLookup( MInames, i ))) {
	...
	**/
	...
	var makeRangeButtons = function( args, charCS, disabled ) {
		...
		if (!_.isNull(weapon) && !(parseInt(weapon,10) % 2)) {
    		ranges = (attrLookup( charCS, fields.RW_range, fields.RW_table, rowIndex ) || '');
    		specialist = parseInt((attrLookup( charCS, fields.WP_specialist, fields.WP_table, (2*(rowIndex+1)) ) || 1), 10);
    		...
    		log( 'weapon:'+weapon+', specialist:'+specialist+', rowIndex:'+rowIndex+', specialist lookup:'+attrLookup( charCS, fields.WP_specialist, fields.WP_table, (2*(rowIndex+1)) )+', wpTableRef:'+wpTableRef);
		...
		if (specialist) {
			...
			content += (disabled || range) ? '</span>' : ('](!attk --button ' + BT.RANGEMOD + '|' + tokenID +  '|' + attkType + '|' + weapon + '|' + rowIndex + '|0|PB)');
		}
		...
		content += (disabled || range) ? '</span>' : ('](!attk --button ' + BT.RANGEMOD + '|' + tokenID +  '|' + attkType + '|' + weapon + '|' + rowIndex + '|0|S)');
		...
		if (ranges.length != 1) {
			...
			content += (disabled || range) ? '</span>' : ('](!attk --button ' + BT.RANGEMOD + '|' + tokenID +  '|' + attkType + '|' + weapon + '|' + rowIndex + '|0|M)');
		}
		if (!ranges.length || ranges.length > 2) {
			...
			content += (disabled || range) ? '</span>' : ('](!attk --button ' + BT.RANGEMOD + '|' + tokenID +  '|' + attkType + '|' + weapon + '|' + rowIndex + '|0|L)');
		}
		...
		content += (disabled || range) ? '</span>' : ('](!attk --button ' + BT.RANGEMOD + '|' + tokenID +  '|' + attkType + '|' + weapon + '|' + rowIndex + '|0|F)');
    ...
    */
	...
	var makeAttackMenu = function( args, submitted, monsterAttk1, monsterAttk2, monsterAttk3 ) {
		...
		if ( monsterAttk1 || monsterAttk2 || monsterAttk3 ) {
			...
			if (monsterAttk1) {
				content += '[' + monsterAttk1 + '](!attk --button ' + BT.MON_INNATE + '|' + tokenID + '|' + attkType + '|1)';
			}
			if (monsterAttk2) {
				content += '[' + monsterAttk2 + '](!attk --button ' + BT.MON_INNATE + '|' + tokenID + '|' + attkType + '|2)';
			}
			if (monsterAttk3) {
				content += '[' + monsterAttk3 + '](!attk --button ' + BT.MON_INNATE + '|' + tokenID + '|' + attkType + '|3)';
			}
			content += '}}{{desc1=';
		}
		...
		if (fields.MWfirstRowNamed) {
			content += (1 == weaponButton) ? '<span style=' + design.selected_button + '>' : (submitted ? '<span style=' + design.grey_button + '>' : '[');
			content += attrLookup( charCS, fields.MW_name );
			content += ((1 == weaponButton) || submitted) ? '</span>' : '](!attk --button ' + BT.MELEE + '|' + tokenID + '|' + attkType + '|1|-1|' + twoWeapons + ')';
		}
		...
		for (i = 0; i < (sheetTypes.sheetType-1); i++) {
			...
			content += attrLookup( charCS, fields.MW_name, fields.MW_table, i );
		...
		if (fields.RWfirstRowNamed) {
			...
			content += attrLookup( charCS, fields.RW_name);
			if (ammoQty) {
				content += ' x ' + ammoQty;
			}
			content += ((2 == weaponButton) || submitted || !ammoQty) ? '</span>' : '](!attk --button ' + BT.RANGED + '|' + tokenID + '|' + attkType + '|2|-1|0)';
		}

		for (i = 0; i < (sheetTypes.sheetType-1); i++) {
			w = 4 + (i * 2);
			ammoPointer = attrLookup( charCS, [fields.Ammo_indirect[0] + (i+2) + '-', fields.Ammo_indirect[1]]);
			ammoQty = Math.max(attrLookup( charCS, [ammoPointer + fields.Ammo_qty[0], fields.Ammo_qty[1]]),0);
			content += (w == weaponButton ? '<span style=' + design.selected_button + '>' : (submitted ? '<span style=' + design.grey_button + '>' : '['));
			content += attrLookup( charCS, fields.RW_name, fields.RW_table, i );
			if (ammoQty) {
				content += ' x ' + ammoQty;
			}
	...
	var makeChangeWeaponMenu = function( args ) {
		...
		var tokenID = args[1],
			...
			charCS = getCharacter(tokenID),
			left = isNaN(leftIndex) ? '' : (attrLookup( charCS, fields.MI_name, fields.MI_table, leftIndex ) || ''),
			right = isNaN(rightIndex) ? '' : (attrLookup( charCS, fields.MI_name, fields.MI_table, rightIndex ) || ''),
			both = isNaN(bothIndex) ? '' : (attrLookup( charCS, fields.MI_name, fields.MI_table, bothIndex ) || ''),
			auto1 = isNaN(auto1Index) ? '' : (attrLookup( charCS, fields.MI_name, fields.MI_table, auto1Index ) || ''),
			auto2 = isNaN(auto2Index) ? '' : (attrLookup( charCS, fields.MI_name, fields.MI_table, auto2Index ) || ''),
...
// --------------------------------------------------------------- Button press Handlers ----------------------------------------------
    ...
    **/
	...
	var handleMeleeAttack = function( args ) {
		...
		var	attackAdj = (attrLookup( charCS, fields.MW_attkAdj, fields.MW_table, rowIndex ) || '0'),
			strBonus = parseInt((attrLookup( charCS, fields.MW_strBonus, fields.MW_table, rowIndex ) || '0'), 10),
			profLevel = (attrLookup( charCS, fields.MW_profLevel, fields.MW_table, rowIndex ) || '0'),
			proficiency = parseInt(profLevel,10),
			weapon = (attrLookup( charCS, fields.MW_name, fields.MW_table, rowIndex ) || ''),
			critThreshold = parseInt((attrLookup( charCS, fields.MW_crit, fields.MW_table, rowIndex ) || '20'), 10),
			fighterClass = (attrLookup( charCS, fields.Fighter_class ) || ''),
			dexReact = parseInt((attrLookup( charCS, fields.Dex_react ) || '0'), 10),
			twoWeapons = state.attackMaster.twoWeapons[tokenID],
			twoWeapPenalty = 0;
			
		log('Raw profLevel:'+profLevel+', profLevel lookup:'+attrLookup( charCS, fields.MW_profLevel )+', proficiency:'+proficiency);
		...
		var	mastery = parseInt((attrLookup( charCS, fields.WP_mastery, fields.WP_table, (1+(2*rowIndex)) ) || '0'), 10),
			strengthHit = parseInt((attrLookup( charCS, fields.Strength_hit ) || '0'), 10),
    ...
    **/
	...
	var handleRangedAttackCalc = function( args ) {
		...
		var	weapon = (attrLookup( charCS, fields.RW_name, RW_table, rowIndex ) || ''),
            attackAdj = (attrLookup( charCS, fields.RW_attkAdj, RW_table, rowIndex ) || '0'),
			profLevel = (attrLookup( charCS, fields.RW_profLevel) || '0'),
            proficiency = parseInt(profLevel,10),
			strBonus = parseInt((attrLookup( charCS, fields.RW_strBonus) || 0), 10),
			dexBonus = parseInt((attrLookup( charCS, fields.RW_dexBonus) || 0), 10),
			critThreshold = parseInt((attrLookup( charCS,fields.RW_crit) || 0), 10);

		log('Raw profLevel:'+profLevel+', profLevel lookup:'+attrLookup( charCS, fields.RW_profLevel, RW_table, rowIndex )+', proficiency:'+proficiency);
		...
		var	specialist = parseInt((attrLookup( charCS, fields.WP_specialist, fields.WP_table, (2*(rowIndex+1)) ) || 0), 10),
			mastery = parseInt((attrLookup( charCS, fields.WP_mastery, fields.WP_table, (2*(rowIndex+1)) ) || 0), 10),
			strengthHit = parseInt((attrLookup( charCS, fields.Strength_hit ) || 0), 10),
			dexMissile = parseInt((attrLookup( charCS, fields.Dex_missile ) || 0), 10),
	...
	**/
	...
	var handleMeleeDamage = function( args ) {
		...
		var dmgType = args[0],
            ...
            tokenName = getObj('graphic',tokenID).get('name'),
			charCS = getCharacter( tokenID ),
            weapon = (attrLookup( charCS, fields.MW_name, fields.MW.dmgTable, rowIndex ) || ''),
			dmgAdj = (attrLookup( charCS, fields.MW_dmgAdj, fields.MW.dmgTable, rowIndex ) || '0'),
			specialist = parseInt((attrLookup( charCS, fields.MW_dmgSpecialist, fields.MW.dmgTable, rowIndex ) || '0'), 10),
			strBonus = parseInt((attrLookup( charCS, fields.MW_dmgStrBonus, fields.MW.dmgTable, rowIndex ) || '0'), 10),
			strDmg = parseInt((attrLookup( charCS, fields.Strength_dmg ) || '0'), 10),
			magicDmgAdj = parseInt((attrLookup( charCS, fields.Dmg_magicAdj ) || '0'), 10),
			backstabMult = parseInt((attrLookup( charCS, fields.Backstab_mult ) || '0'), 10),
			content,
			dmgDice;
		...
		if (dmgType == BT.MW_DMGL) {
			dmgDice = (attrLookup( charCS, fields.MW_dmgL, fields.MW.dmgTable, rowIndex ) || '0');
		} else {
			dmgDice = (attrLookup( charCS, fields.MW_dmgSM, fields.MW.dmgTable, rowIndex ) || '0');
		}
		
		var mastery = parseInt((attrLookup( charCS, fields.WP_mastery, fields.WP_table, (1+(2*rowIndex)) ) || '0'), 10),
			backstab = parseInt((attrLookup( charCS, fields.WP_backstab, fields.WP_table, (1+(2*rowIndex))  ) || '0'), 10),
	...
	**/
	...
	var handleRangedDamage = function( args ) {
		...
		if (dmgType = BT.RW_DMGL) {
			dmgDice = (attrLookup( charCS, fields.Ammo_dmgL, fields.Ammo_table, rowIndex ) || '0');
		} else {
			dmgDice = (attrLookup( charCS, fields.Ammo_dmgSM, fields.Ammo_table, rowIndex ) || '0');
		}
		
		weapon = (attrLookup( charCS, fields.RW_name, fields.RW_table, rowIndex ) || 'ranged weapon');
		specialist = parseInt((attrLookup( charCS, fields.WP_specialist, fields.WP_table, (1+(2*rowIndex)) ) || '0'), 10);
	...
	var blankTable = function( charCS, attr, table ) {
		let i = table[1] ? 0 : -1,
			TableObj = getTable( charCS, table, attr );
		while (!_.isUndefined(tableLookup(TableObj,i,attr))) {
		    log('blankTable blanking table '+table[0]+' row '+i+' with -');
			tableSet( TableObj, attr, '-', table, i );
			i++;
		}
		return;
	};
	...
	var handleChangeWeapon = function( args ) {
		...
		indicies.MW = fields.MW_table[1] ? 0 : -1;
		indicies.RW = fields.RW_table[1] ? 0 : -1;
		indicies.dmg = fields.MW_dmgTable[1] ? 0 : -1;
		
		ammoInfo.ammoIndex = fields.Ammo_table[1] ? 0 : -1;
		ammoInfo.ammoTypes = [];
		
		tableInfo.indicies = indicies;
		tableInfo.ammoInfo = ammoInfo;
		tableInfo.MWnames = getTable( charCS, fields.MW_table, fields.MW_name );
		tableInfo.MWtypes = getTable( charCS, fields.MW_table, fields.MW_type );
		tableInfo.MW2H = getTable( charCS, fields.MW_table, fields.MW_twoHanded );
		tableInfo.MWstrBonus = getTable( charCS, fields.MW_table, fields.MW_strBonus );
		tableInfo.MWattkAdjs = getTable( charCS, fields.MW_table, fields.MW_attkAdj );
		tableInfo.MWnoAttks = getTable( charCS, fields.MW_table, fields.MW_noAttks );
		tableInfo.MWcritHits = getTable( charCS, fields.MW_table, fields.MW_critHit );
		tableInfo.MWcritMisses = getTable( charCS, fields.MW_table, fields.MW_critMiss );
		tableInfo.MWsizes = getTable( charCS, fields.MW_table, fields.MW_size );
		tableInfo.MWranges = getTable( charCS, fields.MW_table, fields.MW_range );
		tableInfo.MWspeeds = getTable( charCS, fields.MW_table, fields.MW_speed );
		tableInfo.MWprofs = getTable( charCS, fields.MW_table, fields.MW_profLevel );
		tableInfo.MWsuperTypes = getTable( charCS, fields.MW_table, fields.MW_superType );
		tableInfo.MWdmgNames = getTable( charCS, fields.MW_dmgTable, fields.MW_dmgName );
		tableInfo.MWdmg = getTable( charCS, fields.MW_dmgTable, fields.MW_dmgName );
				setAttr( charCS, fields.MW_dmgStrBonus,  (dmg.match(/sb:([01])/i) || [0])[1],                  fields.MW_dmgTable, indicies.MW );
				setAttr( charCS, fields.MW_dmgAdj,       (dmg.match(/\+:([+-]?\d+?)[,\]]/i) || [0])[1],             fields.MW_dmgTable, indicies.MW );
				setAttr( charCS, fields.MW_dmgSM,        (dmg.match(/sm:(.*?\d+?d\d+?.*?)[,\]]/i) || [0])[1],  fields.MW_dmgTable, indicies.MW );
				setAttr( charCS, fields.MW_dmgL,         (dmg.match(/l:(.*?\d+?d\d+?.*?)[,\]]/i) || [0])[1],   fields.MW_dmgTable, indicies.MW );
...
//				setAttr( charCS, fields.RW_range,    attkRange,    fields.RW_table, indicies.RW );
		...
		blankTable( charCS, fields.MW_name, fields.MW_table );
		blankTable( charCS, fields.RW_name, fields.RW_table );
		blankTable( charCS, fields.MW_dmgName, fields.MW_dmgTable );
		blankTable( charCS, fields.Ammo_name, fields.Ammo_table );
		

		tableInfo = addWeapon( charCS, fields.Equip_leftHand,  leftIndex,  tableInfo );
		tableInfo = addWeapon( charCS, fields.Equip_rightHand, rightIndex, tableInfo );
		tableInfo = addWeapon( charCS, fields.Equip_bothHands, bothIndex,  tableInfo );
		tableInfo = addWeapon( charCS, fields.Equip_auto1Slot, auto1Index, tableInfo );
		tableInfo = addWeapon( charCS, fields.Equip_auto2Slot, auto2Index, tableInfo );
...
// ------------------------------------------------------------- Command Action Functions ---------------------------------------------
	...
	*/
	...
	var doAttkMenu = function(args,attkType) {
		...
		makeAttackMenu( ['', tokenID, attkType, null, null, multiWeapNo, ''], MenuState.ENABLED, args[2], args[3], args[4] );
		return;
    };
