	...
	const version = '1.1.3',
		...
		showConfig = function (who) {
			let output = `/w "${who}" <div style="border: 1px solid black; background-color: #FFFFFF;` +
				...
				'<p>There is currently one configuration option:</p><div style="padding-left: 10px;' +
				'padding-right:20px"><ul><li style="border-top: 1px solid #ccc;border-bottom: 1px solid' +
				' #ccc;"><div style="float:right;width:40px;border:1px solid black; background-color:#ffc;' +
				' text-align:center;">' + (state.ChatSetAttr.playersCanModify ?
				...
				`</div><b><span style="font-family: serif;">players-can-modify</span></b>${htmlReplace('-')}` +
				' Determines if players can use <i>--name</i> and <i>--charid</i> to change attributes' +
				' of characters they do not control. Specifying this option toggles the current setting.' +
				'</li></ul> </div>' + getConfigOption_playersCanModify() + '</div></div>';
			sendChat('ChatSetAttr', output)
		},
		getConfigOption_playersCanModify = function () {
			...
			return '<div><b>playersCanModify</b> is currently ' + text +
