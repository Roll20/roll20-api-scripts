...
import type { MockAttribute } from "../../vitest.globals";

describe("Modern ChatSetAttr Integration Tests", () => {
  ...
  describe("Feedback Options", () => {
    it("should send public feedback with --fb-public option", async () => {
      ...
      await vi.waitFor(() => {
        ...
        expect(sendChat).toHaveBeenCalledWith(
          "ChatSetAttr",
          expect.stringMatching(/Created attribute <strong>Attribute<\/strong>/),
          undefined,
          expect.anything()
        );
      });
    ...
    it("should use custom sender with --fb-from option", async () => {
      ...
      await vi.waitFor(() => {
        ...
        expect(sendChat).toHaveBeenCalledWith(
          "Wizard",
          expect.stringMatching(/Created attribute <strong>Spell<\/strong>/),
          undefined,
          expect.anything()
        );
      });
    ...
    it("should use custom header with --fb-header option", async () => {
      ...
      await vi.waitFor(() => {
        ...
        expect(sendChat).toHaveBeenCalledWith(
          "ChatSetAttr",
          expect.stringMatching(/Magic Item Acquired/),
          undefined,
          expect.anything()
        );
      });
    ...
    it("should use custom content with --fb-content option", async () => {
      ...
      await vi.waitFor(() => {
        ...
        expect(sendChat).toHaveBeenCalledWith(
          "ChatSetAttr",
          expect.stringMatching(/Level Up.*Character 1 is now level 5!/),
          undefined,
          expect.anything()
        );
      });
    ...
    it("should combine all feedback options together", async () => {
      ...
      await vi.waitFor(() => {
        ...
        expect(sendChat).toHaveBeenCalledWith(
          "Dungeon_Master",
          expect.stringMatching(/Combat Stats Updated.*Character 1's health increased to 25!/g),
          undefined,
          expect.anything()
        );
      });
