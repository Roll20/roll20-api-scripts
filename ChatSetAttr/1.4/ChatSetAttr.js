...
var chatSetAttr = chatSetAttr || (function () {
	...
	const version = '1.4',
		...
		addRepeatingAttributes = function (list, attrNames, allAttrs, errors, createMissing, deleteMode) {
			let allRepAttrs = {},
				repRowIds = {},
				repRowIdsLo = {},
				rowsToCreate = [],
			...
			let attrNamesRead = _.reduce(attrNames, function (prev, attrName) {
				...
				else if (match && match[1][0] === '$' && deleteMode) {
					prev[attrName] = {
						rowNum: parseInt(match[1].slice(1)),
						rowMatch: true
					};
				}
				else if (match && deleteMode) {
					prev[attrName] = {
						rowIdLo: match[1].toLowerCase(),
						rowMatch: true
					};
				...
				if (prev[attrName].rowIdLo === '-create' && !deleteMode) {
					rowsToCreate.push(prev[attrName]['sName'][0]);
				}
				return prev;
			}, {});
			repSections = _.uniq(repSections);
			rowsToCreate = _.uniq(rowsToCreate);
			...
			_.each(list, function (charid) {
				...
				_.each(rowsToCreate, function (prefix) {
				...
				_.each(attrNamesRead, function (p, attrName) {
					...
					else if (p.rowIdLo === '-create' && !deleteMode) {
					...
					if (finalId && p.rowMatch) {
						let repRowUpper = (p.sName[0] + '_' + finalId).toUpperCase();
						_.each(allRepAttrs[charid][p.sName[0]], (attr, attrName) => {
							if (attrName.toUpperCase().indexOf(repRowUpper) === 0) {
								allAttrs[charid][attrName] = attr;
							}
						});
					}
					else if (finalId) {
						let finalName = p.sName[0] + '_' + finalId + '_' + p.sName[1],
							attrNameCased = getCIKey(allRepAttrs[charid][p.sName[0]], finalName);
						if (attrNameCased) {
		...
		addStandardAttributes = function (list, attrNames, allAttrs, errors, createMissing, deleteMode) {
		...
		getAllAttributes = function (list, attrNames, errors, createMissing, deleteMode) {
			let allAttrs = {},
			...
			addStandardAttributes(list, _.difference(attrNames, attrNamesRepeating), allAttrs,
				errors, createMissing, deleteMode);
			addRepeatingAttributes(list, attrNamesRepeating, allAttrs, errors, createMissing, deleteMode);
			return allAttrs;
		},
