...
// 1.0.4 Correcting for bad version number.
...
const helpHTML = `
  ...
  <div style="${css.tracklist}; padding:8px; line-height:1.6;">
    ...
    <div style="${css.headerButtonContainer}float:none;">
      ...
      <a style="${css.headerSubButton}">Sequential</a>
      <a style="${css.headerSubButton}">↻</a>
      <a style="${css.headerSubButton}">Shuffle</a>
      <a style="${css.headerSubButton}">↻</a>
    ...
    <div style="${css.descHelp}">
      ...
      <div style="margin-left:15px">
        ...
        <b>Sequential</b> — Plays all visible tracks one after the other.<br>
        <b>↻</b> — Plays all visible tracks one after the other, then starts over.<br>
        <b>Shuffle</b> — Plays all visible tracks in a random order, once through.<br>
        <b>↻</b> — Plays all visible tracks in a random order, reshuffling and looping after each full playthrough.<br>
...
<div style="${css.headerButtonContainer}">Play All Modes&nbsp;
  <a href="!jb playall" 
     title="Play all visible tracks at once (up to 5 simultaneously)" 
     style="${data.settings.playAllMode === 'together' ? css.headerSubButtonActive : css.headerSubButton}; margin-right:5px;">
    Together
  </a>
  ...
  <a href="!jb playall-seq-loop" 
     ...
     style="${data.settings.playAllMode === 'loop' ? css.headerSubButtonActive : css.headerSubButton}; margin-right:5px;">
    <img src="${icons.loop}" alt="Loop" style="${css.controlButtonImg};max-width:10px; height:12px; margin: 0px; padding:0px;">
  </a>

  <a href="!jb playall-rand" 
     title="Play all visible tracks once, in a random order (no repeats)" 
     style="${data.settings.playAllMode === 'random' ? css.headerSubButtonActive : css.headerSubButton}">
    Shuffle
  </a>

  <a href="!jb playall-rand-loop" 
     title="Play all visible tracks in a random order, reshuffling after each full playthrough" 
     style="${data.settings.playAllMode === 'random-loop' ? css.headerSubButtonActive : css.headerSubButton}; margin-right:5px;">
    <img src="${icons.loop}" alt="Loop" style="${css.controlButtonImg};max-width:10px; height:12px; margin: 0px; padding:0px;">
  </a>
