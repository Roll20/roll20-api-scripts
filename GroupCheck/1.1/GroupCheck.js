...
//			'Will Save': { 'name' : 'Will Saving Throw', 'formula' : '[[d20 + %will_saving_throw_mod%]]' },
	...
	htmlReplace = function (str, rollsOnly) {
		let entities = {
			'<' : 'lt',
			'>' : 'gt',
			"'" : '#39',
	...
	printHelp = function(who) {
		let helpString = `/w "${who}" <div style="border:1px solid black;` +
			...
			'roll20-api-scripts/tree/master/GroupCheck/1.1/README.md">documentation' +
		...
		return '<div style="border:1px solid black;background-color:#FFFFFF;padding:' +
			...
			_.map(state.groupCheck.checkList, function(value, key) {
				return `<tr><td>${key}</td><td>${value.name}</td><td>` +
					`${htmlReplace(value.formula, false)}</td></tr>`;
			}).join('') +
			'</table></div>';
	},
		...
		if (opts.import) {
			...
			if (_.isObject(data)) {
				...
				output = 'Checks added. The imported JSON was: <br>'
					+ htmlReplace(JSON.stringify(data), false);
			} else {
