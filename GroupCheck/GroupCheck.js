  ...
  const stateVersion = 8,
    ...
    themes = {
      ...
      dark: {
        ...
        rowSeparator: "#555",
        boxBorder: "#FFF",
        errorBackground: "#882020",
      },
    },
    getTheme = () => {
      return (state?.groupCheck?.options?.darkmode ?? false) ? themes.dark : themes.light
    },
    // Roll appearance
    outputStyle = (() => {
      const makeBox = (header, subheader, freetext, content) => {
          const theme = getTheme();
          return `<div style="border: 1px solid ${theme.boxBorder};background:${theme.background};border-radius:15px;padding:3px 3px 1px;margin-left:-42px">` +
        ...
        makeCommandButton = (name, command, useBorder) => {
          const theme = getTheme();
          const style = `style="font-weight:bold;color:${theme.foreground};background:${theme.background};border:${useBorder?`1px solid ${theme.foreground};padding:2px;margin:1px 0`:"none;padding:0"}"`;
    ...
    getAttribute = async (characterId, property) => {
        ...
        if (sheetShortname === "dnd2024byroll20") {
          // This handles "multiple sheets, 2014 main" and can be removed once we support multi-sheets in API
    ...
    updateState = () => {
      ...
      case 7:
        state.groupCheck.options.darkmode = false;
      }
    ...
    sendChatBox = (playerid, content, background) => {
      ...
      const output = `<div style="border:1px solid ${theme.foreground};background:${background};` +
        `padding:3px;margin:0 10px 0 -32px">${content}</div>`;
      sendChatNoarchive(playerid, output);
    },
    handleError = (playerid, errorMsg) => sendChatBox(playerid, `<h4>Error</h4><p>${errorMsg}</p>`, getTheme().errorBackground),
    ...
    sendWelcomeMessage = () => {
      const theme = getTheme();
      const output = `/w GM <div style="border:1px solid ${theme.foreground};background:${theme.background};padding:3px;margin:0 10px 0 -32px">` +
    ...
    processFormula = async (formula, special, charID) => {
      ...
      // Incompatibility is just for "multi-sheet, 2014 main" and it can all be removed after the API support multi-sheets
