...
on('ready', () => {
    ...
    const ConfigureState = () => {
        ...
        if (!state.DifficultyRating.Version || state.DifficultyRating.Version < 1.04) {
            log('Upgrading Difficulty Rating to ' + version);
            state.DifficultyRating.Sheet = 'STARTUP';
            state.DifficultyRating.Version = version;
        }
        if (!state.DifficultyRating.Version || state.DifficultyRating.Version < 1.05) {
            log('Upgrading Difficulty Rating to ' + version);
            state.DifficultyRating.Version = version;
        }
    }; ConfigureState();
    ...
    const getCharByAny = (nameOrId) => {
        ...
        // Try loading through character name
        const list = findObjs({
            _type: 'character',
            name: nameOrId
        });
    ...
    const getAttrs = (character, attrName) => {
        ...
        if (!attr || attr.length === 0) {
            log('No Attr: ' + character + ': ' + attrName);
            return null;
        }
        return attr;
    };

    const getAttr = (character, attrName) => {
        let attr = getAttrs(character, attrName);
        if (!attr) {
            return null;
        }
        return attr[0];
...
/*
    const getAttrsFromSub = (character, substringName) => {
        const attr = filterObjs((obj) => {
            if (obj.get('type') === 'attribute'
                && obj.get('characterid') === character.id
        ...
        if (!attr || attr.length === 0) {
            log('No Substr Attr: ' + character + ': ' + substringName);
            return null;
        }
        return attr;
    };
...
/*
    const getAttrFromSub = (character, substringName) => {
        return getAttrsFromSub(character, substringName)[0];
    };
*/
    ...
    const Decaret = (quotedString) => {
        ...
        if (startQuote >= endQuote) {
            /*if (!quietMode) */ {
                sendChat(drname, `**ERROR:** You must have a string within carets in the phrase ${quotedString}`);
            }
            return null;
        }
        ...
        // Calculate difficulty rating of selected monsters
            ...
            msg.selected.forEach((selection) => {
                ...
                if (existingEntry) {
                    ...
                    let character = getCharByAny(type);
                    if(null !== character){
                        const npcAttr = getAttr(character, state.DifficultyRating.IsNPCField);
                        // Make sure it's actually an NPC
                        if (npcAttr && npcAttr.get('current')) {
                            let rawCR = getAttr(character, state.DifficultyRating.NPCChallengeField).get('current');
                            let cr = parseInt(rawCR) || 0;
                            if (rawCR.startsWith('1/')) {
                                const denominator = parseInt(rawCR.substring(2));
                                cr = 1 / denominator;
                            }
                          ...
                          log(`Warning! Token doesn't represent a character: ` + name);
                      }
                }
            });
