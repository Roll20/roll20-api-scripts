var SkillBook = SkillBook || (function() {
    ...
    checkInstall = function() { log('-=> Skillbook v'+version+' <=-  ['+(new Date(lastUpdate*1000))+']'); },

	getCharacterObj = function (obj) {
		"use strict";
	...
	HandleInput = function(msg) {
		...
		if (msg.type === "api" && msg.content.indexOf("!skillbook") !== -1 )
		{
			...
			if (msg.content.indexOf("--createability") !== -1) createBook = true;
			_.each(selected, function(obj) {
				...
				macroText = "&{template:pf_generic} {{character_name=@{"+characterName+"|character_name}}} {{character_id=@{"+characterName+"|character_id}}} {{name=Skills}} {{";	// Base macro text
				if (getAttrByName(characterID,"header_image-pf_generic"))
					macroText = macroText +  "header_image=@{"+characterName+"|header_image-pf_generic}}} {{";
				for (i=0; i < skillAttrs.length; i++)
				{
					...
					if (_.indexOf(miscSkills, skillAttrs[i]) !== -1)
					{
						...
						skillName = skills[skillAttrs[i]] + " (@{"+characterName+"|"+attrName+"})";
					}
					else if (skillAttrs[i].indexOf("misc-skill") !== -1)
					{
						...
						skillName = "@{"+characterName+"|"+skillAttrs[i]+"-name}";
					}
					else
				...
				if (createBook)
				{
					...
					if (abil.length === 0)  // If you don't find one...
					{
						...
						createObj("ability", {
							name: "Skills",
							characterid: characterObj.get("_id"),
							action: "/w \""+characterName+"\" "+macroText,
							istokenaction: true
						});
					}
				...
				{
					playerObjs = findObjs({ type: "player", _id: msg.playerid });
					sendChat(characterName,"/w \""+playerObjs[0].get("_displayname")+"\" "+macroText);
				}
			});
		}
	},
