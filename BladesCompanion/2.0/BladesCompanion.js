// Blades in the Dark Companion Script v2.0
...
//
var bladesCompanion = bladesCompanion || (function () {
	'use strict';
	...
	const version = '2.0',
		...
		checkInstall = function () {
			if ('BladesHelper' in state && !('BladesCompanion' in state)) {
				state.BladesCompanion = Object.assign(state.BladesHelper, {
					harmData: []
				});
				delete state.BladesHelper;
			}
		...
		sendChatMessage = function (playerid, content) {
			const divWrapper = text => {
					return '<div style="border:1px solid #888;border-radius:5px;' +
						'background-color:#eee;padding:1px 3px;margin-left:-42px;">' + text + '</div>';
				},
				getWhisperPrefix = id => {
		...
		sendWelcomeMessage = function () {
			const output = '<div style="border:1px solid #888;border-radius:5px;' +
				'background-color:#eee;padding:1px 3px;margin-left:-42px;">' +
				'<h3>Blades in the Dark Companion Script</h3>' +
				'<p>Welcome to the Blades in the Dark companion script.' +
				' To get started, enter *!blades* in chat.</p>' +
				'</div>';
			sendChat('BladesCompanion', output);
		},
		createChatLink = function (text, ref) {
			return '<a style="color:#333;font-weight:bold;margin:2px;padding:3px;background:' +
				`transparent;border-color:black;" href="!blades ${ref.replace(/"/g, '&' + 'quot' + ';')}">${text}</a>`;
		},
		...
		getOrCreateAttribute = function (charid, name, current) {
			return findObjs({
				characterid: charid,
				name: name,
				type: 'attribute'
		...
		checkPermission = function (playerid, character, silent) {
		...
		getClockTokenData = function (size, label, charID, playerid, current) {
		...
		getBarTokenData = function (type, charID, initial, playerid) {
		...
		// Output functions
		constructQueries = function (playerid) {
			const validChars = findObjs({
				...
				'}',
				clockTargetQuery = '?{Linked to which character?|' + ['None,none', ...validChars.map(ch => `${ch.get('name')},char ${ch.id}`)].join('|') +
				'}',
		...
		showConfigMenu = function (playerid) {
		...
		showState = function (playerid) {
			const output = '<h3>Synchronisation data:</h3><table style="margin:3px;">' +
				'<tr><td><b>Character</b></td><td><b>Attribute name</b></tr>' +
				myState.data.filter(v => checkPermission(playerid, getObj('character', v.character), true))
				.map(v => `<tr><td>${getObj('character', v.character).get('name')}</td><td>${v.attribute}</td></tr>`).join('') +
				'</table><h3>Harm trackers</h3><table style="margin:3px;">' +
				myState.harmData.filter(v => checkPermission(playerid, getObj('character', v.character), true))
				.map(v => `<tr><td style="padding-right: 10px">${getObj('character', v.character).get('name')}</td></tr>`).join('') +
				'</table><h3>Configuration:</h3><table style="margin:3px;">' +
				Object.entries(myState.config)
		...
		showHelp = function (playerid) {
			const output = `<h3>BladesCompanion v${version}</h3>` +
				...
				'<h4>CONFIGURATION</h4>' +
				'<ol><li>You can change the URLs in the clockData and barData variables to use different graphics. Due to Roll20 restrictions, they need to be images uploaded to your Roll20 library, and it needs to be the "thumb" size variant.</li>' +
				'<li>The GM configure the script via the following commands (or enter **!blades config-menu** for a graphical menu):<br>' +
				'a) **!blades config autoclock [on|off]** will turn automatic creation of clocks on the tabletop on or off, respectively (if one is created in a character sheet)<br>' +
				'b) **!blades config clocksize [size]** will set the size for new clocks to [size] pixels (square).<br>' +
				'c) **!blades config position [x] [y]** will set the position of newly created tokens to the specified [x] and [y] coordinates. Defaults to 200 and 300.</li></ol>' +
				'<h4>USAGE</h4>' +
				...
				'<ul><li>**!blades add-clock &lt;size&gt; none &lt;name&gt;**<br>' +
				...
				'!blades add-clock 8 none Drive off the Red Sashes gang</li>' +
				...
				'!blades add-bar trauma &#64;{Bloodletters|character_id} wanted</li>' +
				...
				'!blades add-by-id &#64;{Silver|character_id} trauma</li>' +
				'<li>**!blades add-harm-tracker &lt;charid&gt;**<br>' +
				'Creates a harm tracker on the tabletop for character with the specified id.</li>' +
				'<li>**!blades clock-macro**<br>' +
				'Creates (or updates) a macro for you which allows for easy creation of clocks on the tabletop via queries. The GM can also use **!blades clock-macro all** to create such a macro for every player.</li>' +
				'<li>**!blades show**<br>' +
				'Shows all currently active links between tokens and character atributes.</li>' +
				'<li>**!blades config-menu**<br>' +
				'Shows a menu to access configuration options.</li>' +
				'<li>**!blades remove**<br>' +
				'Removes any link for the currently selected tokens.</li>' +
				'<li>**!blades reset**<br>' +
				'Clears all links between tokens and attributes.</li>' +
				'<li>**!blades help**<br>' +
				'Shows this help screen.</li>' +
				'<li>**!blades menu**, or just **!blades**<br>' +
				'Shows a handy menu for the most common options.</li>' +
				'</ul>';
			sendChatMessage(playerid, output);
		},
		// Workhorse functions
		resetState = function (playerid) {
		...
		updateClockMacro = function (playerid, arg) {
		...
		createClockFromAttribute = function (attribute, toggle, passedSize) {
			const match = attribute.get('name').match(/^(repeating_(?:clock|crewclock|factionclock)_(?:[A-Za-z0-9-]+?))_name/),
				sizeMatch = attribute.get('name').match(/^(repeating_(?:clock|crewclock|factionclock)_(?:[A-Za-z0-9-]+?))_size/),
			...
			if (toggle && match) {
				...
				if (clockData[size]) {
					...
					sendChatMessage(null, `New ${size}-clock added for character ${charName}.`);
				}
			}
			else if (toggle && sizeMatch) {
				const nameAttr = findObjs({
					characterid: charID,
					name: sizeMatch[1] + '_name',
					type: 'attribute'
				})[0];
				if (nameAttr) createClockFromAttribute(nameAttr, true, attribute.get('current'));
			}
		},
		addBar = function (playerid, errors, type, charid, attrName) {
		...
		addClock = function (playerid, errors, args) {
			const size = args[0],
				target = args[1],
		...
		addSheetClocks = function (playerid, errors, charid) {
			if (charid) {
				const character = getObj('character', charid);
				if (checkPermission(playerid, character)) {
					findObjs({
		...
		addHarmTracker = function (playerid, errors, charid) {
			if (charid) {
				const character = getObj('character', charid);
				if (checkPermission(playerid, character)) {
		...
		updateHarmText = function (data, attr) {
		...
		handleConfig = function (playerid, errors, args) {
		...
		addToSync = function (playerid, errors, token, character, attrName) {
		...
		removeSelectedFromSync = function (playerid, selected) {
		...
		handleInput = function (msg) {
			if (msg.type === 'api' && /^!blades/.test(msg.content)) {
				...
				case 'add-harm-tracker':
					addHarmTracker(msg.playerid, errors, args[0]);
					break;
				case 'remove':
		...
		handleLinkedSideChange = function (token) {
			myState.data.filter(v => (v.token === token.id)).forEach(function (data) {
				const attr = getOrCreateAttribute(data.character, data.attribute, 0);
				attr.set('current', token.get('currentSide'));
			});
		},
		handleLinkedAttributeChange = function (attr) {
		...
		handleTextChange = function (text, remove) {
			const data = myState.harmData.find(o => Object.values(o).includes(text.id));
			if (data) {
				const attrName = Object.entries(data).find(([name, id]) => text.id === id)[0],
		...
		updateHarmFromToken = function (token) {
			myState.harmData.filter(o => o.token === token.id).forEach(updateHarmText);
		},
		updateHarmFromAttr = function (attr) {
