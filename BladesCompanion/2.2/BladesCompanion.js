// Blades in the Dark Companion Script v2.2
	...
	const version = '2.2',
		...
		showHelp = function (playerid) {
			const output = `<h3>BladesCompanion v${version}</h3>` +
				...
				'b) **!blades config publicclocks [on|off]** will toggle if clocks linked to no character will be controllable by everyone or only their creator and the GM<br>' +
				'c) **!blades config clocksize [size]** will set the size for new clocks to [size] pixels (square).<br>' +
				'd) **!blades config position [x] [y]** will set the position of newly created tokens to the specified [x] and [y] coordinates. Defaults to 200 and 300.</li></ol>' +
				'<h4>USAGE</h4>' +
				...
				'!blades add-by-table &#64;{Silver|character_id} stress stress</li>' +
		...
		addFromTable = function (playerid, errors, charid, tableName, attrName) {
			const character = getObj('character', charid);
