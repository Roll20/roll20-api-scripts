...
on('ready', () => {
    const version = '1.1.11'; //version number set here
    //Changelog

    //1.1.11
    //Revised Help System - Changed to Handout
    //1.1.10
    //Fixed isTokenNotesRow scope issue
    //Added "c|attribute,max" notation to pull max value
...
function help() {
    const HANDOUT_NAME = 'Help: Reporter';
    const AVATAR_URL = 'https://files.d20.io/images/470559564/QxDbBYEhr6jLMSpm0x42lg/original.png?1767857147';
...
<p>
Reporter is a script that reads the tokens on the board that are associated with character sheets and builds a report of them in the chat, returning selected values from either the token settings or the character sheets they are associated with. You can either select a set of tokens to work with, or if you select <em>no</em> tokens, it will assume all tokens on the Object/Token layer. This behavior can be altered using <em>keywords</em>, described below. The basic syntax is:
</p>

<pre><code>!report --[queries] ---[buttonline] ----[keywords]</code></pre>

<h2>Queries</h2>

<p>Queries are constructed using</p>
<ul>
    <li><code>t|attribute</code> to poll a token attribute</li>
    <li><code>c|attribute</code> to poll a character sheet attribute. To get the Max value of an attribute, use the format: <code>c|attribute,max</code></li>
</ul>

<p><em>Examples</em></p>
<ul>
    <li><code>--t|name</code> would return a report of all selected token names</li>
    <li><code>--c|strength</code> would return a report of all strength values on the character sheets of the selected tokens</li>
</ul>
...
<p>
You may not always want each attribute reported on its own line. You can add a code after the attribute name (not the alias) to use something other than a line return between attributes.
</p>

<ul>
    <li><strong>comma (,)</strong> Adds three non-breaking spaces between this attribute and the next, keeping them on the same line when possible.</li>
    <li><strong>period (.)</strong> Adds a vertical pipe between this attribute and the next, keeping them on the same line when possible.</li>
    <li><strong>dash (-)</strong> Adds a thin gray horizontal rule between this attribute and the next.</li>
    <li><strong>Hashtag (#)</strong> Adds a bit of horizontal space between this attribute and the next.</li>
</ul>

<p><em>Examples:</em></p>

<pre><code>!report --t|emits_bright_light,|Bright-Light t|bright_light_distance|Distance t|emits_low_light,|Low-Light t|low_light_distance|Distance ---light</code></pre>

<p>
will return this Bright Light and Distance on the first line of each record and Low-Light and its Distance on the second, instead of each value taking up its own line
</p>
...
<p>For example, if a token has 60 feet of Night Vision:</p>

<pre><code>t|night_vision_distance</code></pre>

<p>might produce:</p>

<pre><code>${tokenChar}: night_vision_distance = 60</code></pre>

<p>but</p>

<pre><code>t|night_vision_distance?NV</code></pre>

<p>would yield:</p>

<pre><code>${tokenChar}: NV= 60</code></pre>
...
<p>Examples:</p>
<ul>
    <li><code>@{token|name}</code> is written as <code>A{token|name}</code></li>
    <li><code>?{question|default_answer}</code> is written as <code>Q{question|default_answer}</code></li>
</ul>
...
<p>There are four types of operator.</p>
<ul>
    <li><code>+</code> only includes the token/character pairs that matches the query</li>
    <li><code>-</code> excludes any token/character pair that exactly matches the query</li>
    <li><code>~</code> only includes any character that is a partial match for the query</li>
    <li><code>^</code> excludes any character that is a partial match for the query</li>
</ul>

<p>thus:</p>

<pre><code>!report||-|c|name|Goblin</code></pre>
<pre><code>!report||~|c|name|Goblin</code></pre>
<pre><code>!report||-|c|npc|1||+|t|has_night_vision|true</code></pre>
...
<p>There is as yet, no way to test for an empty, or undefined value.</p>
...
<ul>
    <li><code>--vision</code> as the Query will replace any declared query line with one designed to report most vision situations. It will give values for whether the token has sight, night vision and what the distance of any night vision is.</li>
    <li><code>---vision</code> as the Buttonline will replace any declared button line with a buttonline designed to handle most cases of vision and darkvision.</li>
    <li><code>--light</code> as the Query will replace any declared query line with one designed to report most lighting situations. It will give values for the amount of light, distance and what type.</li>
    <li><code>---light</code> as the Buttonline will replace any declared button line with a buttonline designed to handle most cases of lighting.</li>
    <li><code>---actions</code> as the Buttonline will replace any declared button line with a buttonline made up of the token action buttons associate with the character. This is designed for synergy with the Token Action Maker script, but is not essential. Not that the token actions created by this command cannot contain roll templates and will not convert the %{selected|commandname} structure. this requires very careful parsing and is best avoided. It should work flawlessly with Token Action Maker commands, with the exception of the "Check" and "Save" buttons, for the reasons just mentioned.</li>
</ul>
...
<p><code>layer|[gmlayer|objects|map|walls|tracker|all]</code> will constrain the report to a particular layer or all layers at once, so long as no tokens are selected. If any tokens are selected, Reporter will default to the layer the selected tokens are on. This makes it easier for instance to check the vision settings of tokens on the token layer and the gmlayer simultaneously, or to ping pull to note tokens on the gm layer without switching manually to that layer.</p>

<p>If the layer keyword all is used the report will be on all token/character pairs on all layers. In this case, a layer character will appear on each subhead line of the report to let you know which layer the token is on.</p>

<p>If the layer keyword tracker is used the report will be on all token/character pairs on the Turn Tracker as if it were a layer. In this case, a layer character will appear on each subhead line of the report to let you know which layer the token is on. If you click on the layer token, it will switch the token from the GM/Notes layer to the Token/Objects layer and back.</p>

<p><code>compact|[true|false]</code> (default=false): The compact mode shows the token image at half size, and eliminates the second line of the report subhead, since it is not always desired.</p>

<p><code>showheader|[true|false]</code> (default=true): This will control whether the header will display at the top of the report.</p>
<p><code>showfooter|[true|false]</code> (default=true): This will control whether the footer will display at the bottom of the report.</p>
<p><code>printbutton|[true|false]</code> (default=true): This will control whether the print button will display on each line of the report.</p>

<p><code>notesbutton|[true|false]</code> (default=false): This will control whether a notes button will display on each line of the report.</p>

<p><code>visibility|[gm|whisper|all]</code> (default=gm): This will determine how the report is presented.</p>

<p><code>source|[true|false]</code> (default=true): if source is set to false, the C and T characters that show whether an attribute comes from the token or the sheet will not be displayed.</p>

<p><code>charactersheetlink|[true|false]</code> (default=true): if this keyword is set to false, the link to open the token's corresponding character sheet will not display.</p>

<p><code>subtitle|[true|false]</code> (default=true): if this keyword is set to false, the line directly below the character name will not display.</p>

<p><code>ignoreselected|[true|false]</code> (default=false): if this keyword is set to true, the search will not be preset to whichever tokens are selected.</p>

<p><code>npcsubstitutions[true|false]</code> (default=true): if this keyword is set to false, the script will not automatically substitute npc attributes for their PC counterparts.</p>

<p><code>sort|attribute</code> (default is the raw order): This keyword will sort the final list.</p>
...
<p><code>title|Title|</code> If this is present in the keywords, the string in between pipes will be placed at the top of the report.</p>

<h3>Supernotes Buttons</h3>

<p>
These are small buttons that will appear on each line of the report that call up Supernotes commands.
</p>

<p><code>tokennotesbutton|[true|false]</code> (default=false)</p>
<p><code>charnotesbutton|[true|false]</code> (default=false)</p>
<p><code>biobutton|[true|false]</code> (default=false)</p>
<p><code>avatarbutton|[true|false]</code> (default=false)</p>
<p><code>tooltipbutton|[true|false]</code> (default=false)</p>
<p><code>imagebutton|[true|false]</code> (default=false): If this keyword is set to true, the report will place a small shortcut button to return images from the Bio field of the character assigned to the reported token.</p>
`;
    ...
    sendChat(
        'Reporter',
        `/w gm Click to open help document: [Help: Reporter](handout:${handout.id})`,
        null,
        { noarchive: true }
    );
}
    ...
    const TCSort = {
        ...
        cr: (a, b) => ((getAttrByName(a.character.get('_id'), "npc_challenge")) < (getAttrByName(b.character.get('_id'), "npc_challenge")) ? -1 : 0),
        crI: (a, b) => ((getAttrByName(a.character.get('_id'), "npc_challenge")) > (getAttrByName(b.character.get('_id'), "npc_challenge")) ? -1 : 0)
    };
...
customTitle = keywords.match(/minimal\|.*?$/).toString().split("|")[1];
            ...
            */
            ...
            //qq
            //log("Number of tokens = " + TCData.length); 
                                    ...
                                    if (attribute === "bar1" || attribute === "bar2" || attribute === "bar3") {
                                        ...
                                        if (target === 't') {
                                            ...
                                            if (undefined === tc.character.get(attribute)) {
                                                if (attribute.endsWith(",max")) {
                                                value = getAttrByName(cId, attribute.slice(0, -4), "max");
                                                } else {
