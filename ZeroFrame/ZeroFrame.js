...
const ZeroFrame = (() => {
    ...
    API_Meta[apiproject].version = '1.0.10';
    const schemaVersion = 0.2;
    const vd = new Date(1636725847879);
    ...
    const releaseMsg = (preserved, preservedstate, apitrigger, msg) => {
        ...
        if (preserved.content.match(escaperx)) {
            ...
            escapearray.forEach(e => {
                preserved.content = preserved.content.replace(new RegExp(escapeRegExp(e), 'g'), (m, padding, escchar) => {
                    return ``;
                });
            })
        }
