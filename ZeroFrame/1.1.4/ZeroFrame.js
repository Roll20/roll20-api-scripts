...
const ZeroFrame = (() => { //eslint-disable-line no-unused-vars
    ...
    API_Meta[apiproject].version = '1.1.4';
    const schemaVersion = 0.2;
    const vd = new Date(1681758282272);
                    ...
                    restoreMsg = Object.keys(batchMsgLibrary).reduce((m, k) => {
                        if (!Object.keys(m).length) {
                            if (batchMsgLibrary[k].handles.includes(bres[1])) {
                                m = batchMsgLibrary[k];
                                m.handles = m.handles.filter(h => h !== bres[1]);
                            }
                        }
                        return m;
                    }, {});
    ...
    const handleBatchInput = (msg) => {
        ...
        const storeOutbound = (cmd) => {
            if (!msg.messageID) {
                ...
                batchMsgLibrary[msg.messageID] = { message: _.clone(msg), handles: [], time: Date.now(), commands: [] };
            }
