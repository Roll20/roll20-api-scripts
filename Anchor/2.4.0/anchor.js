...
//
// Dependencies: MatrixMath, ScriptKit
//
// Commands:
...
var Anchor = Anchor || (() => {
    ...
    const handleInput = (msg) => {
        ...
        try {
            ...
            // ScriptKit handles help, man, whatsnew, gen-help, gen-dev-docs, examples
    ...
    const checkInstall = () => {
        ...
        (() => {
            ...
            html += `<p>Select a parent token and one or more children, then run <code>!anchor</code>. The first selected token becomes the parent — the rest follow it.</p>`;
            html += `<p>To create an invisible anchor instead (children move together with no visible parent): <code>!anchor --new</code></p>`;
            html += `<h2>Commands</h2>`;
            html += `<ul>`;
            html += `<li><code>!anchor [anchor_id] [flags] [--new] [--persist]</code> — Anchor selected tokens. First selected = parent, rest = children. <code>--new</code> forces auto-create. <code>--persist</code> keeps auto-created anchors when childless.</li>`;
            html += `<li><code>!anchor remove [--up] [--down]</code> — Remove relationships. Parent selected = unanchor children. Child selected = detach from parent.</li>`;
            html += `<li><code>!anchor lock [flags]</code> — Lock components (manual moves are undone)</li>`;
            html += `<li><code>!anchor unlock [flags]</code> — Unlock components</li>`;
            html += `<li><code>!anchor track [flags] [--down]</code> — Add component tracking. <code>--down</code> applies to children.</li>`;
            html += `<li><code>!anchor untrack [flags] [--down]</code> — Remove component tracking. <code>--down</code> applies to children.</li>`;
            html += `<li><code>!anchor retrack [flags]</code> — Replace tracked set entirely</li>`;
            html += `<li><code>!anchor center [--up] [--down]</code> — Snap to center. <code>--up</code> centers on parent, <code>--down</code> centers children on self.</li>`;
            html += `<li><code>!anchor update</code> — Force immediate sync</li>`;
            html += `<li><code>!anchor info</code> — Show anchor state</li>`;
            html += `<li><code>!anchor chain [flags]</code> — Mutually anchor tokens in a ring (A→B→C→A)</li>`;
            ...
            html += `<li><code>!anchor --help</code> — Command reference in chat</li>`;
            html += `</ul>`;
            html += `<h2>Modifiers</h2>`;
            html += `<ul>`;
            html += `<li><code>--new</code> — Force auto-create invisible anchor (all selected become children)</li>`;
            html += `<li><code>--persist</code> — Keep auto-created anchor even when childless</li>`;
            html += `<li><code>--up</code> — Operate on own parent relationship</li>`;
            html += `<li><code>--down</code> — Operate on children's relationships</li>`;
            html += `<li><code>--ignore-selected</code> — Skip current selection (use explicit IDs only)</li>`;
            html += `</ul>`;
            html += `<p>When a token is both parent and child, <code>--up</code>/<code>--down</code> is required to disambiguate.</p>`;
            html += `<h2>Component Flags</h2>`;
            html += `<p><code>-all</code> (everything incl. layer/z), <code>-pos</code> (x+y), <code>-x</code>, <code>-y</code>, <code>-rot</code>, <code>-scale</code>, <code>-w</code>, <code>-h</code>, <code>-layer</code>, <code>-flip</code>, <code>-flipv</code>, <code>-fliph</code>, <code>-z</code></p>`;
            html += `<p>Default (no flags): position + rotation + scale + flip. Use <code>-all</code> to include layer and z-order.</p>`;
            hh.set('notes', html);
        })();
    ...
    const registerWithScriptKit = () => {
        ...
        ScriptKit.register(SCRIPT_NAME, {
            ...
            newSince: '2.4.0',
            motd: [
                'Use `!anchor chain` to link tokens in a ring — move any one and all follow.',
                'Anchor supports per-component control: `-pos`, `-rot`, `-scale`, `-flip`, `-z`, `-layer`.',
                'Lock components with `!anchor lock -rot` to prevent rotation changes while allowing position sync.',
                '`!anchor track` adds components to an existing relationship without recreating it.',
                'Auto-created anchors are invisible and auto-destroy when childless. Add `--persist` to keep them.',
                'Other scripts can use `Anchor.getPosition(obj)` / `Anchor.setPosition(obj, x, y)` for anchor-local coordinates.',
                'Use `!anchor man` to browse topics — component flags, config options, and the scripting API.',
            ],
            motdHeader: '⚓ **Anchor** v' + SCRIPT_VERSION,
            motdStyle: { borderLeft: '3px solid #2196f3' },
            help: {
                description: 'Attach child tokens to a parent anchor so they automatically mirror transforms (position, rotation, scale, flip). Supports arbitrary chains and per-component control.',
                quickStart: [
                    'Select two tokens. First selected = parent, rest = children. Type `!anchor`.',
                    'Move the parent — children follow, maintaining their relative offset.',
                    'Use `!anchor remove` on children to detach.',
                    'Use `!anchor chain` to link tokens in a ring (any one moves all).',
                ],
                changelog: [
                    { version: '2.4.0', date: '2026-08-16', changes: [
                        'ScriptKit integration: help, man, whatsnew, gen-help, gen-dev-docs, motd',
                    ]},
                    { version: '2.3.0', date: '2026-08-02', changes: [
                        'Chain linking: mutual anchor rings via !anchor chain / unchain',
                        'Component flags overhaul: short aliases (-pos, -rot, -scale, etc.)',
                        'Track/untrack/retrack commands for surgical component management',
                        'Lock/unlock per-component enforcement',
                        'Auto-created invisible anchors with --persist option',
                        'Z-order tracking component',
                    ]},
                    { version: '2.2.2', date: '2026-07-04', changes: [
                    ...
                    { version: '2.1.0', date: '2026-06-26', changes: [
                        ...
                        'Integration with Choreograph (token.anchor.* variables)',
                    ]},
                ],
                commands: [
                    { group: 'Core', commands: [
                        { syntax: '[anchor_id] [flags] [--ignore-selected] [--new] [--persist] [child_id...]', description: 'Anchor selected tokens to parent', version: '1.0.0',
                          details: 'First selected = parent, rest = children. If anchor_id is omitted, auto-creates an invisible anchor.',
                          items: [
                              { name: '--new', description: 'Force auto-create invisible anchor (all selected become children)', version: '2.3.0' },
                              { name: '--persist', description: 'Keep auto-created anchor even when childless', version: '2.3.0' },
                          ]},
                        { syntax: 'remove [--up] [--down] [--ignore-selected] [child_id...]', description: 'Remove anchor relationships', version: '1.0.0',
                          items: [
                              { name: '--up', description: 'Remove only parent link', version: '2.3.0' },
                              { name: '--down', description: 'Remove only children', version: '2.3.0' },
                          ]},
                        { syntax: 'chain [component flags] [--ignore-selected] [child_id...]', description: 'Link tokens in a mutual ring (move any, all follow)', version: '2.3.0' },
                        { syntax: 'unchain [--ignore-selected] [child_id...]', description: 'Dissolve a chain ring', version: '2.3.0' },
                        { syntax: 'update [--ignore-selected] [child_id...]', description: 'Force immediate transform sync', version: '1.0.0' },
                        { syntax: 'center [--ignore-selected] [child_id...]', description: 'Snap children to anchor center (offset 0)', version: '2.1.0' },
                        { syntax: 'info [--ignore-selected] [child_id...]', description: 'Show anchor state for selected tokens', version: '1.0.0' },
                    ]},
                    { group: 'Component Control', commands: [
                        { syntax: 'lock [component flags] [--ignore-selected] [child_id...]', description: 'Lock components (re-enforced every tick)', version: '2.3.0' },
                        { syntax: 'unlock [component flags] [--ignore-selected] [child_id...]', description: 'Unlock components', version: '2.3.0' },
                        { syntax: 'track [component flags] [--down] [--ignore-selected] [child_id...]', description: 'Add tracking to existing relationship', version: '2.3.0' },
                        { syntax: 'untrack [component flags] [--down] [--ignore-selected] [child_id...]', description: 'Remove tracking for components', version: '2.3.0' },
                        { syntax: 'retrack [component flags] [--ignore-selected] [child_id...]', description: 'Replace tracked component set entirely', version: '2.3.0' },
                    ]},
                    { group: 'Configuration', commands: [
                        { syntax: 'config [key value] [reset]', description: 'View or change configuration', version: '2.2.1',
                          items: [
                              { name: 'poll-interval <ms>', description: 'Polling interval (default: 1000, min: 100)', version: '2.2.1' },
                              { name: 'default-anchor-layer', description: 'gmlayer | objects | map (default: gmlayer)', version: '2.2.1' },
                              { name: 'default-anchor-size <px>', description: 'Auto-anchor token size (default: 35)', version: '2.2.1' },
                              { name: 'default-anchor-name', description: 'Auto-anchor token name (default: Anchor)', version: '2.2.1' },
                              { name: 'default-anchor-aura-color', description: 'Aura color for anchors (default: #00ff00)', version: '2.2.1' },
                              { name: 'default-anchor-aura-visible', description: 'Show aura on auto-anchors (default: true)', version: '2.2.1' },
                              { name: 'allow-player-use', description: 'Let players use info/lock/unlock/update/center (default: false)', version: '2.2.1' },
                              { name: 'reset', description: 'Clear all runtime config overrides', version: '2.2.1' },
                          ]},
                    ]},
                ],
                topics: {
                    components: {
                        ...
                        description: 'Per-component tracking and control',
                        version: '2.3.0',
                        body: 'Component flags control which transform properties are tracked. Default (no flags): position + rotation + scale + flip. Use `-all` to include layer + z-order.\n\n'
                            + '**Long form:** `anchor-position`, `anchor-x`, `anchor-y`, `anchor-rotation`, `anchor-scale`, `anchor-width`, `anchor-height`, `anchor-layer`, `anchor-flip`, `anchor-flipv`, `anchor-fliph`, `anchor-z`\n\n'
                            + '**Short aliases:** `-pos`, `-x`, `-y`, `-rot`, `-scale`, `-w`, `-h`, `-layer`, `-flip`, `-flipv`, `-fliph`, `-z`\n\n'
                            + '`-all` = every component including z-order and layer.',
                    },
                    scriptingApi: {
                        title: 'Scripting API',
                        description: 'Public functions for other scripts',
                        version: '2.1.0',
                        handouts: 'dev',
                        body: 'Access via `Anchor.*` after `on(\'ready\')` fires.\n\n'
                            + '**Querying:**\n'
                            ...
                            + '`Anchor.unchainAnchorObjs(startId)` — dissolve ring → [ids] or null\n\n'
                            + '**Position / Rotation / Scale (anchor-local):**\n'
                            ...
                            + '**Flip:**\n'
                            + '`Anchor.getFlipV(obj)` / `setFlipV(obj, bool)` — relative to parent\n'
                            + '`Anchor.getFlipH(obj)` / `setFlipH(obj, bool)`\n\n'
                            + '**Lock / Unlock:**\n'
                            + '`Anchor.getLocked(obj)` → [components...]\n'
                            + '`Anchor.lock(obj, [components])` — null = lock all\n'
                            + '`Anchor.unlock(obj, [components])` — null = unlock all\n\n'
                            + '**Updates:**\n'
                    ...
                    integrations: {
                        title: 'Integrations',
                        description: 'How Anchor works with other scripts',
                        version: '2.1.0',
                        handouts: 'dev',
                        body: '**Choreograph:** Registers token variables as `token.anchor.*` — parent, left, top, rotation, scaleW, scaleH, flipV, flipH, zOffset, locked, unlocked, siblings, children.\n\n'
                            + '**Sequence:** Registers virtual attributes `anchor.left`, `anchor.top`, `anchor.rotation` for animating in anchor-local space.',
                    },
                },
            },
        });
    ...
    return {
        ...
        chainAnchorObjs,
        unchainAnchorObjs,
        trackComponents: addTrackedComponents,
        untrackComponents: removeTrackedComponents,
    };
})();
