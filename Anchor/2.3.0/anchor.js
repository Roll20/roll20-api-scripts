    ...
    const handleInput = (msg) => {
        ...
        try {
            ...
            if (flags.has('gen-dev-docs')) {
                ...
                html += `<h2>Component Tracking</h2>`;
                html += `<pre>Anchor.trackComponents(childId, components)    // add tracking: { left: true, top: true }\nAnchor.untrackComponents(childId, components)  // remove tracking for specific components</pre>`;
                html += `<p>Use these to surgically add or remove per-component sync without destroying and recreating the relationship.</p>`;
