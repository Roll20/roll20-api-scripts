    ...
    on('chat:message', (msg) => {
        if ('api' === msg.type && /!r-(short|long|charges|ammo|help|list)\b/i.test(msg.content)) {
            ...
            function sendMessage(messageText) {
                sendChat("Rest and Recovery", `/w "${who}"` + header + "<b>" + (getObj('character', characterID)).get('name') + "</b><BR>" + messageText + footer);
            }
            if (msg.selected && msg.selected.length===1) {
                ...
                myCharacter = getObj('character', characterID);
                ...
                sendChat("Rest and Recovery", `/w "${who}"` + header + "Rest and Recovery works by recovering the resources of the sheet associated with a single selected token. You must have exactly one token selected." + footer);
            }
        }
    });
