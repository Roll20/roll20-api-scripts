...
const SelectManager = (() => { //eslint-disable-line no-unused-vars
    ...
    const vd = new Date(1782227947459);
    const versionInfo = () => {
    ...
    const asGM = (msg) => playerIsGM(msg.playerid) || msg.hasOwnProperty('reactionProps');
    const getTokens = (query, msg, owner = true) => {
        if (msg.playerid === 'API') msg.playerid = preservedMsgObj[maintrigger].playerid;
        let pageid = getPageForPlayer(msg.playerid);
        let qrx = RX(query);
        let alltokens = [...findObjs({ type: 'graphic', id: query }), ...findObjs({ type: 'graphic', pageid: pageid }), ...findObjs({ type: 'text', pageid: pageid }), ...findObjs({ type: 'path', pageid: pageid })]
            .filter(t => t.get('layer') === 'objects' || asGM(msg));
        if (owner) {
            alltokens = alltokens.filter(t => asGM(msg) || playersCanUseIDs() || playerCanControl(t, msg.playerid));
        }
    ...
    const typeitemrx = /^(?<type>bar|max|aura|color|layer|tip|gmnotes|type|pc|npc|pt|side|party)(?<ident>1|2|3|4)?(?<=(?:bar|max)\d|(?:aura|color)[1,2]|(?:layer|tip|gmnotes|type|pc|npc|pt|side|party))$/i;
