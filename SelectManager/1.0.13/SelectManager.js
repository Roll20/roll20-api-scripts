...
const SelectManager = (() => { //eslint-disable-line no-unused-vars
    ...
    const vd = new Date(1683407016242);
    const versionInfo = () => {
    ...
    const getToken = (query, pid) => {
        ...
        let tokens = [(tokensIControl.filter(t => t.id === query)[0] ||
            tokensIControl.filter(t => t.get('name') === query)[0])]
            .filter(t => t);

        if (!tokens.length) {
            tokens = tokensIControl.filter(t => {
                qrx.lastIndex = 0;
                return qrx.test(t.get('name'))
            });
        }
        return tokens;
    };
