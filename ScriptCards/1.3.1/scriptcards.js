...
/* Inline Update Notes - So I can keep track of what to put in the release notes :)
	...
	- Rewrote all content substitution code as a single function that works from the innermost set
	of references outward, so something like [&wow[$count[&suffix]]] will process [&suffix] first,
	then [$countX] where X is the result of [&suffix] and finally [&wow1] where 1 is the result of
	[$countX]. This makes the language more logical and variable resolution more deterministic. It
	is also a huge performance improvement, since the way variables were replaced previously was
	highly non-optimal.

	- When generating tooltips, the "~" character will be removed. This makes it easier to hide what
	look like rolls from the chat server when referencing nested values (ie, [~[&shortSaveNameUpper]~])
	when you just want the value of the variable in square brackets as an explanation, since using
	[[&shortSaveNameUpper]] would look like an inline roll to the chat server.

	- Added support for new token properties added to Roll20 on 2021-06-03
...
*/
			...
			if (msg.type === "api") {		
				...
				if (processThisAPI) {
					...
					while (lineCounter < cardLines.length) {
						...
						thisContent = replaceVariableContent(thisContent, cardParameters, (thisTag.charAt(0) == "+" || thisTag.charAt(0) == "*"));
	...
	function replaceVariableContent(content, cardParameters, rollHilighting) {
		...
		var charId = "";
		while(content.match(/\[(?:[\$|\&|\@|\%|\*])[\w|\%|\(|\:|\.|\_|\>|\^|\-|\)]*?(?!\w+[\[])(\])/g) !== null) {
			var thisMatch = content.match(/\[(?:[\$|\&|\@|\%|\*])[\w|\%|\(|\:|\.|\_|\>|\^|\-|\)]*?(?!\w+[\[])(\])/g)[0];			
			var replacement = "";
			matchCount++;
	...
	function replaceSubattributeReferences(content, characterid) {
		...
		while (content.toString().match(/\@\{.*?\}/g) !== null) {
			...
			if (theCharacter !== undefined) {
				...
				//log(`AttrName: ${attrName}, char: ${theCharacter}, Attr: ${theCharacter.get(attrName)}`)
				replacement = getAttrByName(characterid, attrName);
			}
			//log(`Subattribute: Replacing ${thisMatch} with ${replacement} for char ${theCharacter} attr: ${attrName}`)
