	...
	*/
									...
									case "array":
										if (params.length > 2) {
											...
											if (params[1].toLowerCase() == "indexof") {
												if (arrayVariables[params[2]] && arrayVariables[params[2]].length > 0) { 
													var wasFound = false;
													for (var x=0; x<arrayvariables[params[2]].length; x++) {
														if (params[3] == arrayVariables[params[2]]) {
															stringVariables[variableName] = x.toString();
														}
													}
													if (!wasFound) {
														stringVariables[variableName] = "ArrayError";
			...
			while (parts.length >= 3) {
				...
				if (parts.length > 0) {
					if ((parts[0].toLowerCase() == "-or") || (parts[0].toLowerCase() == "-and")) {
						...
						log(`ScriptCards conditional error: Condition contains an invalid clause joiner. Only -and and -or are supported. Assume results are incorrect. ${conditional}`);
					}
					parts.shift();
				}
			}
		...
		function replaceStyleInformation(outputLine, cardParmeters) {
			var styleList = [
				...
				"dicefontcolor", "dicefontsize", "lineheight", "backgroundimage", "buttonfontsize", "buttonfontface",
			];
