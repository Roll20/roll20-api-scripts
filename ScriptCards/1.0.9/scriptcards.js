	...
	*/
		...
		const defaultParameters = {
			...
			backgroundimage: "background-image: url(https://media.giphy.com/media/19Z7rpzllZY8o/giphy.gif); ",
		...
		var tokenAttributes = "token_name:token_id:statusmarkers:bar1_value:bar1_max:bar2_value:bar2_max:bar3_value:bar3_max:top:left:width:height:rotation:layer:aura1_radius:aura1_color:aura2_radius:aura2_color:aura1_square:aura2_square:tint_color:ligt_radius:light_dimradius:light_angle:light_losangle:light_multiplier:light_otherplayers:light_hassight:flipv:fliph:controlledby:_cardid:_pageid:imgsrc:bar1_link:bar2_link:bar3_link:represents:layer:isdrawing:name:gmnotes:showname:showplayers_name:showplayers_bar1:showplayers_bar2:showplayers_bar3:showplayers_aura1:showplayers_aura2:playersedit_name:playersedit_bar1:playersedit_bar2:playersedit_bar3:playersedit_aura1:playersedit_aura2:lastmove:adv_fow_view_distance";
				...
				if (msg.type === "api") {		
					...
					if (processThisAPI) {
						...
						if (isResume) {
							...
							if (scriptCardsStashedScripts[resumeArgs[0]]) { delete scriptCardsStashedScripts[resumeArgs[0]]; }
						} else {
						...
						for (var x=0; x<cardLines.length; x++) {
							...
							if (thisTag.charAt(0)==":") {
								if (lineLabels[thisTag.substring(1)]) { log(`Warning: redefined label ${thisTag.substring(1)}`); }
						...
						while (lineCounter < cardLines.length) {
							...
							if (thisTag.charAt(0).toLowerCase() == "i") {
								...
								stashAScript(myGuid, cardLines, cardParameters, stringVariables, rollVariables, returnStack, parameterStack, lineCounter + 1, outputLines, varList, "X");
								lineCounter = cardLines.length + 100;
								cardParameters.hidecard = "1";
								sendChat(msg.who, `/w ${msg.who} ${flavorText}` + makeButton(buttonLabel, `!sc-resume ${myGuid}-|-${buildLine}`));
							}
		...
		function replaceStyleInformation(outputLine, cardParmeters) {
			var styleList = [
				...
				"dicefontcolor", "dicefontsize", "lineheight", "backgroundimage"
			];
		...
		function replaceCharacterAttributes(outputLine, cardParameters)
		{
			...
			outputLine = replaceStringVariables(outputLine, cardParameters);
			...
			if (cardParameters.sourcetoken !== undefined || cardParameters.sourcecharacter !== undefined) {
				...
				if (matches) {
					matches.forEach(function(item) {
						...
						var attribute = "";
						if (attrName.toLowerCase().startsWith("t-") && tokenAttributes.indexOf(attrName.substring(2)) >= 0) {
							var t = getObj("graphic", cardParameters.sourcetoken);
							if (t) {
								attribute = t.get(attrName.substring(2));
							}
						} else {
							attribute = getAttrByName(cardParameters.sourcecharacter.id, attrName, opType);
						}
			...
			if (cardParameters.targettoken !== undefined || cardParameters.targetcharacter !== undefined) {
				...
				if (matches) {
					matches.forEach(function(item) {
						...
						var attribute = "";
						if (attrName.toLowerCase().startsWith("t-") && tokenAttributes.indexOf(attrName.substring(2)) >= 0) {
							var t = getObj("graphic", cardParameters.targettoken);
							if (t) {
								attribute = t.get(attrName.substring(2));
							}
						} else {
							attribute = getAttrByName(cardParameters.targetcharacter.id, attrName, opType);
						}
						while(outputLine.indexOf(item) >= 0) { outputLine = outputLine.replace(item,attribute) };
					});
				}
				...
				matches.forEach(function(item) {
					...
					for (var i in repeatingSection) {
						if (repeatingSection[i].startsWith(searchText)) {
							...
							attribute = repeatingSection[i].split("|").slice(1,999).join("|");
						}
					}
			...
			if (matches) {
				matches.forEach(function(item) {
					...
					if (token && attrName.toLowerCase().startsWith("t-") && tokenAttributes.indexOf(attrName.substring(2)) >= 0) {
		...
		function stashAScript(stashIndex, scriptContent, cardParameters, stringVariables, rollVariables, returnStack, parameterStack, programCounter, outputLines, resultStringName, stashType) {
