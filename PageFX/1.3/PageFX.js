...
var PageFX = (function() {
    ...
    function activateEffect(effectToken) {
        ...
        var time = getRandomTime(effectToken);
        ...
        var timeoutFunc = () => {
          if(activeEffects[id]) {
            spawnFunc();
            var nextTime = getRandomTime(effectToken);
            var timeout = setTimeout(timeoutFunc, nextTime);
            activeEffects[id].interval = timeout;
          }
        };
        var timeout = setTimeout(timeoutFunc, time);
        activeEffects[id] = {
            interval: timeout,
            token: effectToken
        };
    ...
    function deactivateEffect(effectToken) {
        ...
        if(activeEffect) {
            clearTimeout(activeEffect.interval);
    ...
    /**
     * Creates a random number of milliseconds ranging between the
     * effect token's bar 1 current and max values.
     */
    function getRandomTime(effectToken) {
      var timeLow = effectToken.get('bar1_value') || 100;
      var timeHigh = effectToken.get('bar1_max') || timeLow;
      var alpha = Math.random();
      return (1-alpha)*timeLow + alpha*timeHigh;
    }
