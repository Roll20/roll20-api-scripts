...
Earthdawn.Version = "3.33";       // This is the version number of this API file.
...
Earthdawn.constantButton = function( what, nestingLevel ) {   
  ...
  try {
    ...
    if( nestingLevel ) {      // level 1 (default) is &#124; level 2 is &amp;#124;  Level 3 is &amp;amp;#124;   Etc. One more amp; for each level.
    ...
    return r;
          ...
          // routine to standardize the use of symboles.
          // return an icon that we use to differenciate between talents, skills, powers, knacks, spells, etc. This gathers all the icons into one place.
          ...
          // we put a bunch of zero width non breaking spaces in front of the icons to force them into sorting how we want them to. Those will sort with the most zwnbsp to the front and the least to the end of the list.
...
/*    case "nac":
      ...
      ı-Killing Glare  "test"
no icon   karma   target  power   talent  knack,   skill,  wpn    spell   
*/
      case "t":
      ...
      case "zwnbsp":                c = 0xFEFF; sort = 0;   break;    // Zero Width No Break SPace.   Used to sort thing to the start of the ability list. The one with the most of these will go to the front of the list. 
      case "none":                              sort = 0;   break;
      case undefined:   return "";          // undefined is a legal value that returns an empty string.
      default:
    ...
    if( sortOffset !== false ) {
      ...
      for( let i = 0; i < sort; ++i )
        r += String.fromCharCode( 0xFEFF );
    }
    if( c0 )
      return r + String.fromCharCode( c0, c );
    else if ( c )
            ...
            if( act.startsWith( "!Earthdawn" ) || act.startsWith( "!edToken" ) || act.startsWith( "!edsdr" ) || act.startsWith( "!edInit" ) )
              if( what === -1 )   // If fixing everything, just get rid of all existing token actions and build from scratch.
                ...
                oldab.push({ id: abObj.get( "_id" ), name: abObj.get( "name" ), action: act });
                  ...
                  // second, look at all talents, knacks, etc. that ought to have a token action.  Put them into canidate or one of the specialty lists.
              ...
              _.each( attributes, function (att) {      // First we are listing all the spellcasting Talents.
                if (att.get( "name" ).endsWith( "_T_Special" ) && att.get( "current" ).startsWith( "SPL-" )){
                  ...
                  s += Earthdawn.makeButton( getAttrByName( id, pre + "Name" ), "!edToken~ " + Earthdawn.constantButton( "percent" )
                        ...
                        , att.get("current").endsWith("Willforce") ? "effect" : "action" );
                }
                      ...
                      switch( typeof v ) {
                        ...
                        case "object":
                          if( Array.isArray( v ) ) {    // This is best practices for building Roll20 nested chat menu queries. 
                                                        // The innermost query can be of the form ?{}. Inner queires must call Earthdawn.constantButton for pipe, comma, and braseClose. 
                ...
                if (att.get("name").endsWith( "_T_Name" ) || att.get("name").endsWith( "_NAC_Name"))
                  if( !non || (Earthdawn.getAttrBN( id, Earthdawn.buildPre( att.get("name") ) + "CombatSlot", "") != "1")) {
                    let special = Earthdawn.getAttrBN( id, Earthdawn.buildPre( att.get("name") ) + "Special", ""),
                      ...
                      bg;
                    if (special.startsWith( "Recovery" ))     bg = "health";
                    else if ( special === "Initiative" )      bg = "effect";
...
//log( nm);
          ...
          case "test":    // This is just temporary test code. Try stuff out here and see if it works.  Make sure nothing dangerous is here when released.
            try {
              log("Debug Test");
/*
              Earthdawn.abilityAdd( this.charID, "bbb", "!edToken" );
              Earthdawn.abilityAdd( this.charID, Earthdawn.constantIcon( "zwnbsp" ) + "ccc", "!edToken" );
              Earthdawn.abilityAdd( this.charID, Earthdawn.constantIcon( "zwnbsp" ) +Earthdawn.constantIcon( "zwnbsp" ) +"ddd", "!edToken" );
              Earthdawn.abilityAdd( this.charID, "mmm", "!edToken" );
              Earthdawn.abilityAdd( this.charID, Earthdawn.constantIcon( "zwnbsp" ) + "nnn", "!edToken" );
              Earthdawn.abilityAdd( this.charID, Earthdawn.constantIcon( "zwnbsp" ) +Earthdawn.constantIcon( "zwnbsp" ) +"ppp", "!edToken" );
              let a = Earthdawn.constantIcon( "spell" ) + "aaa ", b = Earthdawn.constantIcon( "zwnbsp" ) +Earthdawn.constantIcon( "zwnbsp" ) + Earthdawn.constantIcon( "spell" ) +"aaa ";
              log(a + a.length ), log(b + b.length ); log (a == b); log("break"); log(a.trim() == b.trim());
// ddd ppp ccc nnn bbb mmm
// **ddd **ppp *ccc *nnn bbb mmm
              let a2 = a.replace( /\W/g, ""), b2 = b.replace( /\W/g, "");
              log( a2 + a2.length );  log("mid"); log( b2 + b2.length); log( a2 === b2);
              "﻿﻿⏻﻿🔧⚠"
                    case "t":
      case "talent":                c = 0x2600; sort = 1;   break;    // Black sun with rays: &# 9728;
      case "karma":                 c = 0x262F; sort = 0;   break;    // yin yang:  &# 9775;
      case "wpn":
      case "weapon":                c = 0x2694; sort = 2;   break;    // Crossed swords:  &# 9876;
      case "warning":               c = 0x26A0;             break;    // Red Exclaimation point in a red triangle.  &# 9888;    This one is never an ability prefix just an icon we sometimes use.
      case "power":                 c = 0x26A1; sort = 1;   break;    // Lightning Bolt or High Voltage: &# 9889;
      case "sp":    case "spm":
      case "target":                c = 0x27b4; sort = 2;   break;    // Black-feathered South East Arrow
      case "nac":
      case "knack":   c0 = 0xD83D;  c = 0xDCAB; sort = 1;   break;    // Dizzy Symbol: &# 128171;
      case "sk":    case "ska":     case "skk":     case "skl":
      case "skill":   c0 = 0xD83D;  c = 0xDD27; sort = 1;   break;    // Wrench       &# 128295;
      case "spell":   c0 = 0xD83E;  c = 0xDE84; sort = 2;   break;    // Magic Wand.  &# 129668;
      case "zwnbsp":                c = 0xFEFF; sort = 0;   break;    // Zero Width No Break SPace.   Used to sort thing to the start of the ability list. The one with the most of these will go to the front of the list. 
      case "none":                              sort = 0;   break;

*/
              log( Earthdawn.constantIcon( "talent" ) +  Earthdawn.constantIcon( "karma" ) +  Earthdawn.constantIcon( "wpn" ) + Earthdawn.constantIcon( "warning" ));
              log( Earthdawn.constantIcon( "power" ) +  Earthdawn.constantIcon( "sp" ) +  Earthdawn.constantIcon( "sk" ));
              log( Earthdawn.constantIcon( "target" ) +  Earthdawn.constantIcon( "nac" ) +  Earthdawn.constantIcon( "sk" ) + Earthdawn.constantIcon( "spm" ));
              log( Earthdawn.constantIcon( "zwnbsp" ) +  Earthdawn.constantIcon( "none" ) );
...
//            Earthdawn.errorLog( "ParseObj.Karma() error, SSA is not a string in version 2.0", this );
          ...
          if( kdice > 0 ) {   // Are we spending more than zero karma?
            ...
            if( kdice > currKarma ) {
              let bad = true;
              if( currKarma == 0 && Earthdawn.getAttrBN( this.charID, "Karma", 0) != 0) {     // before saying anything about not having enough karma, check that the token is linked properly.
            ...
            try {
              if( !ssa.includes( "firstRun" ) && !(this.edClass && this.edClass.msg && this.edClass.msg.playerid && playerIsGM( this.edClass.msg.playerid )))
                ...
                if( ssa.includes( "Create" ) || ssa.includes( "Refresh" )) {    // create the Earthdawn macros.
                  ...
                  macroDetail( "ResetChars", "!Earthdawn~ Misc: ResetChars: ?{Mods Only or Full Reset|Mods Only|Full}", false );
...
/*
222
{"SeqStep":0,"RowId":"-O1IHof48sGUUMDu-2yV","bGrim":false,"bGrimAtt":false,"Type":"M","ExtraThreads":[]
,"inMatrix":"-O1IHof48sGUUMDu-2yV","MatrixType":"-10"

,"spelldata":{
  "Name":"Ice Mace and Chain","RowId":"-O1IHzkVk-bzg0Z81yZF","Strain":1,"sThreads":"0","Casting":"MDh","Duration":"2 rd","Range":"20  y","WilSelect":"Wil","Discipline":"Elementalism","WilEffect":"2","Effect":"WIL + 2/Physical","Numbers":"7 / 12 / 13 / 18","ExtraThreads":"Inc Efct Step (2), Inc Rng (10 yards), Add Tgt (+1)","SuccessLevels":"Inc Dur (+2)","AoE":"","EffectArmor":"PA","FailText":"","SuccessText":"","DisplayText":"","sayTotalSuccess":"0","FX":""
  ,"Notes":"Water–Cold.
     This spell creates an icy weapon that is thrown at an opponent. The magician hurls two pebbles, coins, or other objects at the target, then makes a Spellcasting test against the target’s Mystic Defense. If successful, the objects transform into frozen spiked mace-heads joined by an icy chain, which wrap around the target. The magician’s Effect test determines how much damage is inflicted. If he rolls any Bonus Dice on the Damage test, the mace and chain shatter. If the mace and chain survive the blow, the target is Harried until he frees himself at the end of the following round.
2024-7-9 : Entry update triggered by Compendium Drop - data comes from compendium
Range : 20 yards"
,"Knacks":["Double Ice Mace and Chain","Reinforced Ice Mace and Chain","Double Reinforced Ice Mace and Chain"]
,"KnacksId":["-O1II1OWJSOv2S0KjANS","-O1II2m-G9ztJctSe1MV","-O1II3nnBVLX6HazqtP0"]
,"LinksProvideValue":["Double Ice Mace and Chain","Reinforced Ice Mace and Chain","Double Reinforced Ice Mace and Chain"]
,"LinksProvideValueList":["SP;-O1II1OWJSOv2S0KjANS","SP;-O1II2m-G9ztJctSe1MV","SP;-O1II3nnBVLX6HazqtP0"],"Strain_max":1
}
,"TotThreads":0,"CurThreads":0,"RemThreads":0
,"Knacks":["Double Ice Mace and Chain"]
,"KnacksId":["-O1II1OWJSOv2S0KjANS"]}
3333
*/
//log("button knack"); log(spellseq.spelldata.KnacksId); log(spellseq.spelldata.KnackNames);
              let ttn = "";
// put the knacks available in the tooltip.
// problem here is that knacksid is defined, but knacksnames is not, which should not be possible.
...
//log( "BoD " + what + "   " + t);
        ...
        if( txtwv == "")          txtwv = "Weaving Talent missing!";
          ...
          if( pre ) {
            ...
            switch( as ) {
              ...
              case "x Casting Difficulty":
                if(( "targetNum" in this ) && this.targetNum != undefined )
                  strainStandard( this.targetNum, 0x02 );
                else
