...
Earthdawn.Version = "3.19";       // This is the version number of this API file.
...
//          if( state.Earthdawn.logMsg ) log( "New character: SWflag ping sent." );
          ...
          if( Earthdawn.getAttrBN( cID, "edition", "4" ) != state.Earthdawn.edition ) {
            edc.chat( "Error, settings mismatch. New character had edition set to Edition " + Earthdawn.getAttrBN( cID, "edition", "99" ) 
                + ". While API state variable has edition set to " + state.Earthdawn.edition
                + ". Go to Campaign settings Default Sheet Settings and set it to correct edition to ensure that all future sheets will be set correctly, then go to any character sheets " 
                + "Special Functions - GM Special Commands and set Edition to the correct value as well to update the API and all existing sheets.", Earthdawn.whoFrom.apiWarning );
          }
              ...
              if( Woodskin == 1 )           // We know this is woodskin and we have negative damage.
                ...
                this.chat( "Is this a "
                      ...
                      + "~ " + ssa.join( ":" ).replace( /recovery/gi, "WoodSkin"), 
                          "This recovery test healed all current normal damage. Wood Skin tests are recorded by setting your current damage to a negative number. Pressing this button will allow this recovery test to leave you with a negative amount of damage representing the additional damage Wood Skin will allow you to take.") 
                      + " or " 
...
//log( "Talent Found " + pre + " " + name + " "  + code +" " + rid + " " + cname);
        ...
        if( !bRecovery && gmMsg.length > 0)
          ...
          // Cancel, Inspect, RepSecFix, RepSecBatch, RepSecBatchFixAll, sheetworkertest, showeach, test.
...
//          step = Earthdawn.parseInt2(this.misc[ "step" ] || 0) + this.ssaMods( ssa );   // Add any mods to roll.
        ...
        this.doNow();
        if( step == 0 && "reason" in this.misc && this.misc[ "reason" ].toLowerCase().indexOf( "karma only" ) !== -1 )    // One karma only is allowed to make step zero rolls.
          ;
        else if ( step < 1 ) {
          if( init )
...
//log(JSON.stringify(rolls));
        function walk( item ) {      // Walk through the roll structure, extracting what we need.
          ...
          case "M":     // This is an expression, such as "+" between two items, or "-1" as a modifier to a roll.
            ...
            break;
          default:
            Earthdawn.errorLog( "Error in ED.BuildRoll(). Unknown type '" + item[ "type" ] + "' in rolls " + item + ". Complete roll is ...", this );
            log( JSON.stringify( rolls ));
          }
        } // end walk()
