...
Earthdawn.Version = "2.041";       // This is the version number of this API file.  state.Earthdawn.sheetVersion is the version number of the html file.
...
Earthdawn.abilityAdd = function ( cID, Ability, ActionStr )  {
  ...
  try {
    ...
    // This is done on every token drop. 
Earthdawn.abilityRebuild = function ( cID ) {
  'use strict';
  try {
          //First delete them all... I know we discussed to do something to allow custom macros... Do you have example of such macros to try to do it a bit less brutal
          //Another option would be to have a setting that enables the autocleaning of macros at each token drop
    let abs = findObjs({ _type: "ability", _characterid: cID});
    _.each( abs, function (abObj) {
      ...
      abObj.remove();
        ...
        //then rebuild
    abs = findObjs({ _type: "attribute", _characterid: cID });
    _.each( abs, function (att) {
      let sa = att.get( "name" ) || "";
      if( sa.startsWith( "repeating_" )) {      // This loop only deals with repitems
        if( sa.endsWith( "T_Special ") && att.get("current") == "Corrupt Karma" )
          Earthdawn.abilityAdd( cID, Earthdawn.constant( "Target" ) + "Activate-Corrupt-Karma", "!edToken~ SetToken: @{target|to have Karma Corrupted|token_id}~ Misc: CorruptKarma: ?{How many karma to corrupt|1}");
              //Dealing with T, NAC, SK, SKK, SKA, SKK, SPN and SP/SPM
        if( sa.endsWith( "_Name" )) {
          let code=Earthdawn.repeatSection( 3, sa ),
            rowID = Earthdawn.repeatSection( 2, sa ),
            pre = Earthdawn.buildPre( code, rowID ),
            cbs = Earthdawn.getAttrBN( cID, pre + "CombatSlot", (code == "WPN" || code == "SP") ? "1" : "0") == "1";    //WPN and SPM default to 1, not the others
                //All except SP/SPM
          if (cbs && (code === "T" || code === "NAC" || code === "SK" || code === "SKA" || code === "SKK" || code === "WPN"))
            Earthdawn.abilityAdd( cID, Earthdawn.constant( code )+"-" + att.get("current"), "!edToken~ %{selected|" + Earthdawn.buildPre( code, rowID ) + "Roll}" );

          if( code === "SP" ) {     //This is a spell in Matrix
            if( cbs && (Earthdawn.getAttrBN( cID, pre+"spmRowID", "0").length > 1)) {     //This is a spell in Matrix
              let preTo = Earthdawn.buildPre("SPM", Earthdawn.getAttrBN( cID, pre + "spmRowID", "0"));
// log( " SPM " + Earthdawn.getAttrBN( cID, pre + "spmRowID", "0") +  " " + cbs + "preTo " + preTo);
              Earthdawn.abilityAdd( cID, Earthdawn.constant( "Spell" ) + Earthdawn.getAttrBN( cID,  preTo + "Contains"," "), "!edToken~ %{selected|" + preTo + "Roll}" );
    ...
    if( Earthdawn.getAttrBN( cID, "Creature-DiveCharge", "0" ) !== "0" )
      Earthdawn.abilityAdd( cID, "Charge",  "!edToken~ ForEach~ marker: divingcharging: t")
    if( Earthdawn.getAttrBN( cID, "Creature-Ambush", "0" ) !== "0" )
      Earthdawn.abilityAdd( cID, "Ambush",  "!edToken~ ForEach~ marker: ambushing: t");
    if( Earthdawn.getAttrBN( cID, "Hide-Spells-Tab", "1" ) !== "1" || Earthdawn.getAttrBN( cID, "Hide-Spells-tab", "1" ) !== "1" ) {
  ...
  } catch(err) { log( "Earthdawn:abilityRebuild() error caught: " + err ); }
} // End abilityRebuild
  ...
  this.newCharacter = function( cID )  {
    ...
    try {
      ...
      setTimeout(function() {       // When a character is imported from the character vault, the recalc caused by edition has been observed to cause a race condition. Delay this processig long enough for the import to have been done.
        try {
          if( state && state.Earthdawn && state.Earthdawn.edition ) {
            let aobj = findObjs({ _type: 'attribute', _characterid: cID, name: "edition" }) [ 0 ];	
            if( aobj === undefined )	
              aobj = createObj("attribute", { name: "edition", characterid: cID });	
            if( !NaN( aobj.get( "edition" )))	
              aobj.set( "current", state.Earthdawn.edition );	
          }
