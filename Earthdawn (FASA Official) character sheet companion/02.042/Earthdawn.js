...
Earthdawn.Version = "2.042";       // This is the version number of this API file.  state.Earthdawn.sheetVersion is the version number of the html file.
...
Earthdawn.tokenLinkNPC = {      // Bit flags that control how the sheet links NPC tokens. These mostly default to true when linking PC tokens.
  ...
  pcMask:               0x0340, // This is all bits in position.
...
//log( stem + slink);
      if( bCount === undefined && state.Earthdawn.sheetVersion <= parseFloat( Earthdawn.getAttrBN( cID, "edition_max", 0 ) )) {
        ...
        if( count > maxcount && state.Earthdawn.sheetVersion <= parseFloat( Earthdawn.getAttrBN( cID, "edition_max", 0 ))) {
          ...
          // This is functional start of main part of attribute change handling routine. Only APIflag and API got processed above recordWrap().
...
Earthdawn.tildiFix      = function ( txt ) { 'use strict'; return txt.replace( /~/g, Earthdawn.constant( "tildi" )).replace( /:/g, Earthdawn.constant( "colonalt" )); };
Earthdawn.tildiRestore  = function ( txt ) { 'use strict'; return txt.replace( new RegExp( Earthdawn.constant( "tildi" ), "g" ), "~" ).replace( new RegExp( Earthdawn.constant( "colonalt" ), "g" ), ":" ); };
...
Earthdawn.constant = function( what ) {
  ...
  try {
    ...
    switch (what.toLowerCase() ) {    // For this upper half, we want to return html codes.
      ...
      case "comma":       c = 0x00F1;   break;    // Comma replacement character.  Character name LATIN SMALL LETTER N WITH TILDE
      case "colon":       c = 0x00F2;   break;    // Colon replacement character.  Buttons don't like colons, so anytime we want one in a button, replace it for a while with this.     Latin Small Letter O with Grave
      case "tildi":       c = 0x00F3;   break;    // Tildi replacement character.  To get the parser to ignore tildis for a while, replace it for a while with this.     Latin Small Letter O with Acute
      case "colonalt":    c = 0x00F4;   break;    // Colon replacement character alternate. Used in conjunction with tildiFix.     LATIN SMALL LETTER O WITH CIRCUMFLEX
      case "t":
  ...
  try {
    ...
    if( !cID ) {
      ...
      if( !nm ) {
        ...
        if( nm === "charName" )     // due to charName being a special case that is not a true attribute, we need special handling.
        ...
        else if( dflt === undefined && !nm.startsWith( "repeating_" ))    // If we are not passed a dflt and it is a repeating section anyway, go ahead and try the getAttrByName just to see if it works better. 
          ...
          if( attribBN && attribBN.length > 1 ) {
            log( "Error Earthdawn:getAttrBN( " + cID + ", " + nm + ( dflt === undefined ? "": ", " + dflt ) + ( toInt === undefined ? "": ", " + toInt ) 
                  + " )  returned more than one attribute! Attributes are: " );
            log( attribBN );
            for( let i = 0; i < attribBN.length; ++i )
              if( attribBN[ i ].get( mx ? "max" : "current") != dflt )
                best = i;     // In the weird case of having duplicate entries, we want to use one that isnot the default. 
          }
  ...
  this.updateVersion2p001 = function( cID, ed, charCount ) {
    ...
    try {
      ...
      return count;
        ...
        // update routine for next version (not numbered yet). 
  this.updateVersionxxxxx = function( cID, ed, charCount ) {
    'use strict';
    try {
      let count = 0,
          tabBad, tabGood;
      ...
      let attributes = findObjs({ _type: "attribute", _characterid: cID });
      _.each( attributes, function (att) {
        if ( att.get( "name" ) === "Hide-Spells-tab" )
          tabBad = att;
        if ( att.get( "name" ) === "Hide-Spells-Tab" )
          tabGood = att;
      }); // End for each attribute.
  ...
  this.newCharacter = function( cID )  {
    ...
    try {
      ...
      setTimeout(function() {       // When a character is imported from the character vault, the recalc caused by edition has been observed to cause a race condition. Delay this processig long enough for the import to have been done.
        ...
        } catch(err) { log( "ED.newCharacter setTimeout() error caught: " + err ); }    // Note, can't call errorLog on catch inside a timeout.
...
*/
        ...
        // ssa[0] addGraphic: from on add graphic. Test to see if Token Actions are up to date. 
        // else: User has pressed a button saying that the actual rebuild should happen. 
      ...
      try {
        let po = this,
            ...
            txt = "";
                ...
                // what == 0 then just basic test, 1 then test and get details.   what == -1 then do a fix all on everything.
        function buildTA( what) {   // go through and get information needed to figure out what the token actions should be.
          ...
          let oldab = [],       // List of token actions that DO exist.
              ...
              cbs = [],         // List of RowIDs where combat slot is true
              spmrowid = new Map(),
              contains = new Map(),
          ...
          _.each( findObjs({ _type: "ability", _characterid: cID}), function( abObj ) {
            ...
            if( act.startsWith( "!Earthdawn" ) || act.startsWith( "!edToken" ) || act.startsWith( "!edsdr" ) || act.startsWith( "!edInit" ) )
              if( what === -1 )
                ...
                oldab.push({ id: abObj.get( "_id" ), name: abObj.get( "name" ), action: abObj.get( "action" ) });
          });
          ...
          _.each( findObjs({ _type: "attribute", _characterid: cID }), function( att ) {
            ...
            if( sa.startsWith( "repeating_" )) {      // This loop only deals with repitems
              ...
              } else if( sa.endsWith( "_CombatSlot" )) {
                if( att.get("current") == "1" )
                  cbs.push( Earthdawn.repeatSection( 2, sa ));
              ...
              // Due to a weird and unknown bug where getAttrBN nor getAttrByName work in this routine for brand new attributes for some reason. 
              // In the previous loop we just saved some candidate values and the combat slot value (which we would normally get from getAttrBN). 
              // Now we actually process the candidates. 
              // Note, bug was that there was TWO attributes for getAttrBN to find. This was fixed so doing it this way is no longer necessary. 
          _.each( candidate, function( att ) {
            let sa = att.get( "name" ),
                code = Earthdawn.repeatSection( 3, sa);
// Problem, if combat slots are not correctly defaulted, This routine has no clue. 
//            if( _.contains( cbs, Earthdawn.repeatSection( 2, sa ))) {
//            if( Earthdawn.getAttrBN( cID, Earthdawn.buildPre( sa ) + "CombatSlot", code === "SPM" ? 1 : 0) == "1" ) {
// Try the roll20 function, that seems to be working these days. 
              ...
              if( code === "SP" ) {
                ...
                if( spmID && spmID.length > 1 ) {
                  ...
                  if( cont && cont.length > 1 )
                    ...
                    po.edClass.errorLog( "abilityRebuild:BuildTA: " + sa + " could not find out what: " + spmID + " contains." );
                }
...
// CDD ToDo.    Just before next release, this can be changed to Tab only, not tab as well. 
//          if( Earthdawn.getAttrBN( cID, "Hide-Spells-Tab", "1" ) != "1" ) {
          if( Earthdawn.getAttrBN( cID, "Hide-Spells-Tab", "1" ) == "0" || Earthdawn.getAttrBN( cID, "Hide-Spells-tab", "1" ) == "0" ) {
...
//log( oldab); log(newab);
        ...
        if( ssa[ 0 ] === "addGraphic" || (( ssa.length > 1 ) && ( ssa[ 1 ] !== "forceTest"))) {    // Test to see if Token Actions are correct and up to date.
          ...
          buildTA( 0 );
          if( problems > 0 )
            this.chat( "Some token actions were not what we expected.   " 
                ...
                , Earthdawn.whoTo.player | Earthdawn.whoFrom.noArchive, "API" );
          ...
          this.chat( "It will no longer test the token actions when the token is dropped.", Earthdawn.whoTo.player | Earthdawn.whoFrom.noArchive, "API" );            
          ...
          if( problems > 0 )
            this.chat( Earthdawn.makeButton( "Fix All", "!Earthdawn~ charID: " + this.charID + "~ abilityRebuild: fixAll",
              ...
              + txt.trim(), Earthdawn.whoTo.player | Earthdawn.whoFrom.noArchive, "API" );
        } else if( ssa[ 1 ] == "fixAll" ) {     // Rebuild all Token Actions. 
          buildTA( -1 );
          this.chat( problems + " issues fixed.", Earthdawn.whoTo.player | Earthdawn.whoFrom.noArchive, "API" );
        } else if( ssa[ 1 ] == "removeOne" ) {  // remove one token action by name
          let abObj = getObj( "ability", ssa[ 2 ]);
          if( abObj ) {
            this.chat( abObj.get( "name" ) + " removed.", Earthdawn.whoTo.player | Earthdawn.whoFrom.noArchive, "API" );            
            abObj.remove();
          } else
            this.edClass.errorLog( "ED:abilityRebuild() warning bad remove: "  + JSON.stringify( ssa ));
        } else if( ssa[ 1 ] == "buildOne" ) {    // Rebuild a Token Action. 
          if( ssa.length > 2 ) {
            Earthdawn.abilityAdd( cID, ssa[ 2 ], Earthdawn.tildiRestore( ssa[ 3 ]));
            this.chat( ssa[ 2 ] + " added.", Earthdawn.whoTo.player | Earthdawn.whoFrom.noArchive, "API" );            
          } else
            this.edClass.errorLog( "ED:abilityRebuild() warning bad build: "  + JSON.stringify( ssa ));
        } else
          this.edClass.errorLog( "ED:abilityRebuild() Warning, unknown command " + JSON.stringify( ssa ));
      } catch(err) { this.edClass.errorLog( "ED:abilityRebuild() error caught: " + err ); }
    } // End abilityRebuild
...
log( ssa);
          ...
          case "repsecfix": {     // RepSecFix: Repeating Section Verify and Fix.    Note: This is now labeled Character Sheet Verify/Fix.
                        ...
                        // This will also repeat the AbilityRebuild test (in case user has set it to Never do it automatically).
            try {
              let orig = [],
                lcase = [],
                dup = [],
                ...
                po = this;
              let attributes = findObjs({ _type: "attribute", _characterid: this.charID });
//              let sorted = _.sortBy( attributes, 'name');   For unknown reason this does not work, though the book says it should.
              let sorted = _.sortBy( attributes, function( att ){ return att.get("name"); });
              log( "Debug RepSecFix: " + attributes.length + " attributes found.   Sorting took " + (Date.now() - timer) + " ms.");

//              function findSorted( nm ) {   // if the name is in the sorted array of attribures, return it, otherwise return blank. 
...
log( nm);
            ...
            } catch(err) { this.edClass.errorLog( "Debug RepSecFix error caught: " + err ); }
          } break;
          ...
          if( Item.indexOf( "SP" ) != -1 ) {
            if( state.Earthdawn.gED ) {     // Earthdawn always just uses SP, upgrade would be to make change in GP if needed.
              ...
              if( iAmountSP != 0) {
                ...
                Earthdawn.setWithWorker( aobj, "current", newTotal );
                post += ssa[ 6 ] + " " + Math.abs( iAmountSP ) + " " + "SP" + " (new total " + newTotal + ") ";
              }
          ...
          if ( bgmline || buttonLine )
            setTimeout(function() {
              try {
                ...
                log( "ED.rollFormat setTimeout() error caught: " + err );     // Note, can't use errorLog inside a timeout. 
              }
            }, 500);
...
// CDDFiund
//JBF@CD : temporary ? I got some errors on this sect.replace... But sect is not a string
// this.chat( sect.replace( /(\r|\n)/g , " "), recipients | Earthdawn.whoFrom.player | Earthdawn.whoFrom.character );
          this.chat( sect.toString().replace( /(\r|\n)/g , " ") , recipients | Earthdawn.whoFrom.player | Earthdawn.whoFrom.character );
          if( playerCard ) {
            ...
            for( let i = blockIndexArray.length -2; i > -1; --i) {
              ...
              switch( tf.toLowerCase() ) {
                ...
                case "devotion required": {     // Questor Devotions      Devotions Required: Yes
                  ...
                  bobj = Earthdawn.findOrMakeObj({ _type: 'attribute', _characterid: po.charID, name: pre + "DP-Limit" }, "0");
                  y = bobj.get( "max" );
                  if( !y || y == "0" )    // If not already set to be able to set DP set.
                    bobj.setWithWorker( "max", "1");
...
// This needs update. CDD ToDo
                ...
                case "requirements": {    // Knacks.              Requirements: Rank 3
                  ...
                  if( res !== null ) {
                    ...
                    po.setWW( pre + "Requirements", rnk, 0);
                  }
...
// CDDFind
//JBF@CD Not sure 100%, but I think there is a mismatch with the default value of PA-Nat, that messes up with the import... Tried it and did solve it I had an offset of 2 between the imported value and expected one. Could be linked to a change of default value in the HTML
                      Adjust( 0x01, tf, undefined, "PA-Nat", 0, "Armor-Phys-Nat-Adjust" );    break;
...
//log( cmdSegment);
    ...
    //                this.misc[ "result" ] = (this.misc[ "result" ] || 0) + Earthdawn.parseInt2( subsegmentArray[ 1 ] );    // Mod : x - Adds X to any result obtained.
          ...
          case "rolltype":
            ...
            break;
          case "set":   // Don't do anything fancy, just set attribute to argument.   except weirdness (? can't be in a button).
