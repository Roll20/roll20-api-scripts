...
Earthdawn.Version = "3.001";       // This is the version number of this API file.  state.Earthdawn.sheetVersion is the version number of the html file.
...
//log( prev);   // use prev["name"] and prev["current"]           // {"name":"Wounds","current":0,"max":8,"_id":"-MlqexKD2f4f744TgzlK","_type":"attribute","_characterid":"-MlqeuXlNO51-RYxmJv8"}
    ...
    function recordWrap( wrapper ) {
      'use strict';
              // If edition is not up to date, wait for the end of update.
      if( parseFloat( Earthdawn.getAttrBN( cID, "edition_max", 0 )) < state.Earthdawn.sheetVersion )
        return;
      let rankTo = current;
      ...
      if( !tdate ) {
        ...
        if( party )     // Look for throalic date on the "Party" sheet.
...
//          Earthdawn.SetDefaultAttribute( cID, Earthdawn.buildPre( code, rowID ) + "CombatSlot", code === "SPM" ? 1 : 0 );
      ...
      if ( t ) {                // No matter what, Remove the token action associated with the old name.
        ...
        if( attr.get( "max" ) === "ACK" ) {    // We have got an ACK from the sheetworker. Delete the row.
          ...
          Earthdawn.fromSheetworkerToAPI( current, cID );
          let pre = Earthdawn.buildPre( code, rowID );
          if( state.Earthdawn.logMsg ) log("API sending ACK: " + sa + " :   " + current.trim());
        ...
        if( Earthdawn.getAttrBN( cID, "NPC", "0" ) == Earthdawn.charType.pc ) {
          ...
          if( rnk ) {
            ...
            let stem = "&{template:chatrecord} {{header=" + getAttrByName( cID, "charName" ) + ": " + name + "}}"
...
Earthdawn.buildPre = function ( code2, rowID, lowercase ) {
  ...
  try {
    ...
    switch ( code ) {
      ...
      case "MSG": ret = "repeating_message_"    + rowID + "_" + ( lowercase ? code.toLowerCase() : code ) + "_"; break;
      ...
      case "PER": ret = "repeating_personality" + rowID + "_" + ( lowercase ? code.toLowerCase() : code ) + "_";  break;
      ...
      default: log( Earthdawn.timeStamp() + "Earthdawn:buildPre() error. Unknown code: " + code + "   RowID: " + rowID );
    }
    return ret;
  } catch(err) { log( Earthdawn.timeStamp() + "Earthdawn:buildPre() error caught: " + err ); }
};  // end buildPre
...
Earthdawn.codeToName = function ( code ) {
  ...
  try {
    switch( code ) {
      ...
      default: log( Earthdawn.timeStamp() + "Earthdawn:codeToName() error. Unknown code: " + code );
    }
  ...
  try {
    ...
    switch ( Earthdawn.safeString( what.toLowerCase() )) {    // For this upper half, we want to return html codes.
      ...
      case "power":         c = 0x23FB;   break;    // Power symbol
              // Here we have alternates for some strings we can't have interprited too soon. So change them so roll20 does not recognize it wants to interprite them.
...
//log(attrs);
    ...
    if( objs ) {
      if( objs.length > 1 ) {
        log( Earthdawn.timeStamp() + "Warning Earthdawn:findOrMakeObj() found multiple objects: " );
        log( objs );
      }
      if( objs.length > 0 )
        obj = objs[ 0 ];
...
Earthdawn.fromSheetworkerToAPI = function ( msg, cID ) {
  'use strict';
  try {
    msg = Earthdawn.safeString( msg ).trim();
    if( state.Earthdawn.logMsg ) log("toAPI: " + msg);
    let ED = new Earthdawn.EDclass(),
        cmdArray = msg.split( "\n" );
    ...
    for( let ind = 0; ind < cmdArray.length; ++ind ) {
      ...
      switch ( cmdLine.slice( 0, comma).trim() ) {
        ...
        case "ChatRecord": {    // ChatRecord, SP: 0: (sp price): Spend: Buy a broadsword
          ...
          if( !tdate && state.Earthdawn.gED )
            tdate = "1517-1-1";
          let stem = "&{template:chatrecord} {{header=" + getAttrByName( cID, "charName" ) + ": " + Earthdawn.getParam( dataLine, 5, ":" ) + "}}"
      ...
      if( ret === undefined ) {     // We want to do this if any of the above returned a ret of undefined.
        ...
        if( attribBN && attribBN.length > 1 ) {
          log( Earthdawn.timeStamp() + "Warning Earthdawn:getAttrBN( " + cID + ", " + nm + ( dflt === undefined ? "": ", " + dflt )
                + ( toInt === undefined ? "": ", " + toInt ) + " )  returned more than one attribute! Attributes are: " );
...
*/
  try {
    ...
    return 0;
      ...
      // So fake one.
Earthdawn.pseudoMsg = function( pclass, msg ) {
  'use strict';
  try {
...
Earthdawn.safeArray = function( arr )  {
  ...
  try {
    ...
    log( Earthdawn.timeStamp() + "Earthdawn:safeString() error caught: " + err );
        ...
        // If it is not already a string, return an empty string.
...
//    if( state.Earthdawn.karmaRitual     === undefined )   state.Earthdawn.karmaRitual     = "-1";
    ...
    state.Earthdawn.newChars = [];      // set the collection of new characters to empty.
...
//          log( "Warning! Earthdawn.js  -> WelcomePackage integration failed!  Error: " + err );
      ...
      try {
// CDD Totest t
Earthdawn.errorLog("cleanup started", this );
              ...
              for( let i = 0; i < rowIndex.length; ++i ) {
                ...
                for( let j = 0; j < rowattribs[ i ].length; ++j ) {    // we don't know what order we are going to get these, so now put each row into variables.
                  ...
                  if( n.endsWith( "_MSG_TimeStamp" ) )
                    ...
                    Earthdawn.errorLog( "Earthdawn messageCleanup error. invalid attribute: " + n, this);
                ...
                if( toAPI && !toSheetAck ) {    // Message to API was never ACK. So process it now, but delete it instead of sending ACK.
                  Earthdawn.fromSheetworkerToAPI( toAPI, id );
                  rDel = true;
                  ++ApiUnproc;
                }
//                if( toSheet && !toApiAck ) {
...
Earthdawn.errorLog("cleanup done", this  );
            }
          } catch(err) {
...
//log("this.chat " + iFlags + " customFrom " + customFrom);
    ...
    if( customFrom )   // if we specified a customFrom as argument, it overrides everything
      wf = customFrom;
            ...
            && edc && edc.msg && edc.msg.who ) // From a player and we have the name of the token
      ...
      if( fnd && fnd.length > 0 )
        wf = fnd;
    } else if (!edc.msg || (iFlags & Earthdawn.whoFrom.api) || (edc.msg.type === "api") || (edc.msg.playerid === "API"))
      wf = "API";
    ...
    if ( !(iFlags & Earthdawn.whoTo.playerList )) {     // Don't do this section if specified to do the playerList section.
      ...
      if( edc && edc.msg && edc.msg.who && ( iFlags & Earthdawn.whoTo.player ) && ( !(iFlags & Earthdawn.whoTo.gm) || ((iFlags & Earthdawn.whoTo.gm)
            && edc.msg.playerid && !playerIsGM( edc.msg.playerid ))))
        sc( wf + whoTo, '/w "' + edc.msg.who.replace(" (GM)","") +'" ' +  newMsg );
      else if( iFlags & Earthdawn.whoTo.gm )
    ...
    if( cnt === 0 ) {   // If we did not send any messages above, send to everybody on the token control list and every gm.
      ...
      for( let i = 0; i < lplr.length; i++ ) {  //go through the list of online players and send to the right ones
        let snd = false,
            pid = lplr[ i ].get( "_id" ),
            pn = lplr[ i ].get( "_displayname" )
        //log("testing for " + pn + " with id " + pid);
        snd |= (iFlags & Earthdawn.whoTo.player) && edc.msg && edc.msg.playerid && ( edc.msg.playerid == pid ); // Always send to the originator of the message
        snd |= (iFlags & Earthdawn.whoTo.gm) && playerIsGM( pid );      // Send to GM if flagged for it
        snd |= (iFlags & Earthdawn.whoTo.player) && (!playerIsGM( pid )) && (ctrall || lplrctrl.includes( pid )); // Player controlling the same token.character. GM is excluding because vague results are sent in 2 separate messages
        if( snd )
          sc( wf + whoTo, '/w "' + pn +'" ' + newMsg );
    } }  // end playerList
...
//log(this.SWflag);
    ...
    this.toSheet = function( cmd, line ) {
      try {
        if( this.charID ) {
          if( state.Earthdawn.logMsg ) log( "toSheet: " + cmd + "," + line);
          let rowid = Earthdawn.generateRowID(),
              d = new Date();
          let pre = Earthdawn.buildPre( "MSG", rowid );
          createObj( "attribute", { _characterid: this.charID, name: pre + "TimeStamp", current: d.toString() });
          createObj( "attribute", { _characterid: this.charID, name: pre + "toSheetworker", current: cmd + "," + line });
        } else
          Earthdawn.errorLog( "ED.toSheet error, no charID for this operation " + cmd + ", " + line );
      } catch(err) { Earthdawn.errorLog( "ED.toSheet( " + cmd + ", " + line + " ) error caught: " + err, this ); }
    } // end toSheet()
      ...
      try {
        ...
        switch ( ssa[ 1 ] ) {
          ...
          case "nac": {
            ...
            this.doLater += "~Strain:" + this.strainCalc( pre )    // Strain from this action, aggressive attacks, called shots and split movement.
            ...
            switch( special ) {
              case "Recovery":        this.misc[ "headcolor" ] = "recovery";      break;
...
*/
        ...
        let actType = Earthdawn.getAttrBN( this.charID, pre + "Action", "Standard");
        if( actType !== "NA" )
        ...
        // ssa[1]
        //	forceTest: FixRepSec forces this routine to run by user commnd even when user does not want it automatically run on token drop.
        //	Menu
        //  fixAll
        //  addOne ssa[2]
        //  removeOne ssa[2]
      ...
      try {
        let cID = this.charID,
            ...
            newChar = (state.Earthdawn.newChars.indexOf( cID ) != -1);      // This character was created within the last few seconds. 
                ...
                // what == 0 then just basic test, 1 then test and get details for the menu.   what == -1 then do a fix all on everything.
        function buildTA( what) {   // go through and get information needed to figure out what the token actions should be.
          'use strict';             // This routine also does the actual destruction and rebuilding of the TA.
          ...
          function addit( nm, ab ) {
            'use strict';
            if( what === -1 )     // If fixing all, then just recreate all canidates.
...
//log("x" ); log(oldmap); log(newmap ); log(extra);
            txt += "Unknown Token Action" + ((extra.length > 1) ? "s" : "") + " found. Do you want to remove: ";
...
//log("z" ); log(oldmap); log(newmap ); log(build); log(oldab); log(newab);
            txt += "Missing Token Action" + ((build.length > 1) ? "s" : "") + ". Do you want to create: ";
        ...
        if( newChar )
          buildTA( -1 );
        else if( ssa[ 0 ] === "addGraphic" || (( ssa.length > 1 ) && ( ssa[ 1 ] === "forceTest"))) {    // Test to see if Token Actions are correct and up to date.
          if ( ssa[ 0 ] === "addGraphic" )      // "forceTest" overrides "Never"
            if ( Earthdawn.getAttrBN( cID, "abilityRebuildControl", "" ) === "Never" )
              return;     // If this character is set to never do this action, exit this routine.
          buildTA( 0 );
          if( problems > 0 )      // This block just writes a chat window message and a button warning that there is a problem.
            this.chat( problems + " token actions were not what we expected.   "
                ...
                , Earthdawn.whoFrom.character | Earthdawn.whoTo.player | Earthdawn.whoFrom.noArchive );
        } else if ( ssa.length < 2 )
          Earthdawn.errorLog( "ED:abilityRebuild() Warning, bad SSA " + JSON.stringify( ssa ));
        else if( ssa[ 1 ] == "Never") {    // skip this routine in the future.
        ...
        } else if( ssa[ 1 ] == "Menu" ) {    // Present a menu asking what to do about it.
          buildTA( 1 );
          if( problems > 0 )
        ...
        } else if( ssa[ 1 ] == "fixAll" ) {     // Rebuild all Token Actions.
          buildTA( -1 );
          this.chat( "Attempting to fix all issues. Retesting...", Earthdawn.whoFrom.character | Earthdawn.whoTo.player | Earthdawn.whoFrom.noArchive );
          this.abilityRebuild( [ "abilityRebuild", "Menu" ] );
        } else if( ssa[ 1 ] == "removeOne" ) {  // remove one token action by name
        ...
        } else if( ssa[ 1 ] == "buildOne" ) {    // Rebuild a Token Action.
          if( ssa.length > 3 ) {
            let abObj = getObj( "ability", ssa[ 2 ]);
      ...
      try {
        ...
        switch( Earthdawn.safeString( ssa[ 1 ] ).toLowerCase()) {
          ...
          case "damage": {
            ...
            s += "<br>Selected Tokens Roll Dmg - ";
            s += Earthdawn.makeButton( "1 Strain", "!Earthdawn~ foreach: st~ Damage: Strain: 1",
                  "The Selected Token(s) take 1 strain.", Earthdawn.Colors.health, Earthdawn.Colors.healthfg );
            s += Earthdawn.makeButton( "Fire", "!Earthdawn~ foreach: st~ Damage: PA: "
                  + "?{Size of fire|Torch,4|Small Campfire,6|Large Campfire,8|House fire,10|Forest fire,12}: ?{Mods|0}",
                  "The Selected Token(s) take Fire damage. Physical Armor applies.", Earthdawn.Colors.health, Earthdawn.Colors.healthfg );
            s += Earthdawn.makeButton( "Falling", "!Earthdawn~ foreach: st~ Damage: NA: " + "?{Distance Fallen|2-3 Yrd,5|4-6 Yrd,10|7-10 Yrd,15|"
                  + "11-20 Yrd,40|21-30 Yrd,50|31-50,75|51-100 Yrd,90|101-150 Yrd,120|151-200 Yrd,140|201 Yrd or more,175)}: ?{Mods|0}",
            ...
            s += Earthdawn.makeButton( "Fire", "!Earthdawn~ foreach: st~ Damage: PA: "
                  ...
                  "The Selected Token(s) take Fire damage. Physical Armor applies.", Earthdawn.Colors.health, Earthdawn.Colors.healthfg );
//CDD ToDo Jan 23
//Make a roll, send button to all selected tokens to apply.
// instead of 50, try 20*2
...
//                  if( att.get("current") != "1" )     // commenting this out shows all skills, combat slot or not.
        ...
        if( ssa[ 1 ].startsWith( "Custom" )) {
          ...
          if( Earthdawn.getAttrBN( cID, pre + "Type", "1") ) {    // type 1 is Creature
            ...
            if( d ) {
              ...
              body.append( (( ++linenum % 2) ? ".odd" : ".even"), d.replace( /\n/g, "   "));
        ...
        while ( "CursedLuck" in this.misc && curse-- > 0 ) {
          ...
          for( let i = 0; i < working.length; ++i) {
            ...
            case "M":
              break;
            default:
              Earthdawn.errorLog( "Error in ED.CursedLuck(). Unknown type '" + r[ "type" ] + "' in rolls " + i + ". Complete roll is ...", this );
              log( rolls );
            }
          }
    ...
    this.Damage = function( ssa ) {
      ...
      try {
        ...
        if( !bStun ) {    // normal damage. This is identical for characters and mooks.
          ...
          if( bRecovery ) {
            this.chat( this.tokenInfo.name + " recovered " + ((currDmg < 0) ? ((dmg - currDmg) * -1) : (dmg * -1) )
                  + " damage. New value " + ((currDmg < 0) ? 0 : currDmg ) + "." , Earthdawn.whoTo.player | Earthdawn.whoTo.gm | Earthdawn.whoFrom.character);
          ...
          if( stunDmg != 0 || dmg != 0 ) {
            ...
            if( bRecovery ) {
              this.chat( this.tokenInfo.name + " recovered " + ((stunDmg < 0) ? ((dmg - stunDmg) * -1) : (dmg * -1) )
                    + " stun. New value " + ((stunDmg < 0) ? 0 : stunDmg ) + "." , Earthdawn.whoTo.player | Earthdawn.whoTo.gm | Earthdawn.whoFrom.character);
              if( stunDmg < 0)
                stunDmg = 0;
            }
        ...
        } else if( dmg >= (WoundThreshold + 5)) {     // Character is wounded and need to make a Knockdown test
            ...
            try {
              ...
              for( let i = 1; i < sorted.length; ++i ) {    // Go through each and every attribute in sorted order.
                ...
                if( att.get( "name" ).startsWith( "repeating_")) {    // Now we are specifically fixing repeating sections that have no rowID, or that are all lower or upper case.
                  ...
                  if( !rowID || !code ) {
                    po.chat( "Error found, badly formated repeating section name: " + nm, Earthdawn.whoFrom.apiWarning )
                    continue;
                  }
                  if( code === "TI" || code === "MSK" || code === "PER" || code === "MSG" )     // These don't have a RowID.
                    continue;
...
//log( nm);
                    ...
                    po.chat( "Errors found with " + needFix[ i ].length + " attributes of row " + needFix[ i ][0].get( "name") + " but could not be fixed.", Earthdawn.whoFrom.apiWarning )
                    log( needFix[ i ] );
                    ++issues;
                  }
              ...
              this.abilityRebuild ([ "AbilityRebuild", "forceTest" ] );
...
// CDD Todo, finish this
//log( "point a"); log( pg);
        if ( pg )
{
//log( "point b");
            pg = pg.get( Earthdawn.safeString( playerID ));
//log(pg);
}
        if( !pg )
{
        ...
        if( Item !== "Other" ) {
          ...
          if( Item.indexOf( "SP" ) != -1 ) {
            if( state.Earthdawn.gED ) {     // Earthdawn always just uses SP, we are going to figure it in copper.  upgrade would be to make change in GP if needed.
              let rawSP = ssa[ 5 ], borrow = 0, amount;
              if( rawSP.trim().indexOf( "-" ) > 0 )
                rawSP = rawSP.slice( 0, rawSP.indexOf( "-" ));     // The price is a range, such as 100 - 175.   Just lop off the 2nd half.
              if( rawSP.toLowerCase().indexOf( "cp" ) > 0 )
                amount = Earthdawn.parseInt2( rawSP, true );
              else
                amount = Math.round( parseFloat( rawSP ) * 10);
              ...
              if( amount != 0) {      // we have a non-zero amount
                ...
                post += ssa[ 6 ] + " " + rawSP + " " + "SP" + " (new total " + ( newGold ? ( newGold + " gold, ") : "")
...
// log( post);
        ...
        if( post )
          this.chat( Earthdawn.getAttrBN(this.charID, "charName") + " : " + post, Earthdawn.whoTo.player | Earthdawn.whoTo.gm | Earthdawn.whoFrom.noArchive, null, this.charID);
        if( clear ) {
...
//                  this.setWW( "condition-KnockedDown", "1" );
        ...
        for( let i = lstart; i < lend; ++i ) {      // do this for each target
          if ( this.misc[ "Special" ] === "CorruptKarma" ) {
            ...
            if( !(this.bFlags & Earthdawn.flags.NoOppMnvr )) {    // We don't record these extra successes as something that affects damage rolls.
              ...
              let attributes = findObjs({ _type: "attribute", _characterid: this.charID }),
                  cc = (( "ModType" in po.misc ) && (po.misc[ "ModType" ].endsWith( "CC" ) != -1) ? 1 : -1);
              _.each( attributes, function (att) {
                if (att.get("name").endsWith( "_WPN_Name" ))
    ...
    this.rollFormat = function( recipients, rolls )  {
      ...
      try {
        ...
        if( "headcolor" in this.misc )
          cls += " sheet-rolltemplate-icon-" + Earthdawn.safeString( this.misc[ "headcolor" ] ).toLowerCase();
        ...
        if( bsub )
          sect.append("", this.misc[ "subheader" ], { class: "sheet-rolltemplate-subheadertext"});
        let linenum = 0, modBit = 0, x, x2,
            body = Earthdawn.newBody( sect ),
            txt = "";
        if( "warnMsg" in this.misc )
          body.append( "", this.misc[ "warnMsg" ], { style: { "background-color": "Orange", "text-align":   "center" }});

        if( "SP-Step" in this.misc )
          this.misc[ "ModType" ] = "@{Adjust-All-Tests-Total}";   // The spellcasting talents are talents too, make them look a bit more like the other talents.
        if( "ModType" in this.misc ) {    // Conditions and options that affected the step.
          ...
          else if( modBit === 0x28 && ((x = this.getValue( "Initiative-Mods" ) + this.getValue( "Initiative-Mods-Auto" ) - this.getValue( "Adjust-Effect-Tests-Total" )) != 0))
          ...
          if((( modBit & 0x10 ) || ( modBit & 0x20 && Earthdawn.getAttrBN( this.charID, "effectIsAction", "0" )))
                && (x = this.getValue( "Adjust-All-Tests-Misc" )) != 0)
            txt += Earthdawn.texttip( "  Action Misc: " + x, "Misc modifiers that apply to Action Tests." );
          if(( modBit & 0x20 ) && !Earthdawn.getAttrBN( this.charID, "effectIsAction", "0" ) && (x = this.getValue( "Adjust-Effect-Tests-Misc" )) != 0)
...
//          } // from Action() or spell.
          ...
          if(( modBit & 0x02 || modBit & 0x04 || modBit == 28 )&& this.getValue( "Creature-Ambushing" ) != "0" )
            txt += Earthdawn.texttip( "  Ambush: " + this.getValue( "Creature-Ambushing_max" ), "Ambush: Added to Initiative, Attack, and Damage." );
          if(( modBit & 0x02 || modBit & 0x04 )&& this.getValue( "Creature-DivingCharging" ) != "0" )
            txt += Earthdawn.texttip( "  Diving or Charging: " + this.getValue( "Creature-DivingCharging_max" ), "Diving or Charging: Added to Attack, and Damage" );
          ...
          if( txt.length > 0 || modvalue )
            body.append( (( ++linenum % 2) ? ".odd" : ".even"), Earthdawn.texttip( "<b>Mods:</b> " + ((modvalue) ? modvalue.toString() + " " : " " ),
...
//                  class: "showtip tipsy",
        ...
        if( "actType" in this.misc )
          body.append( (( ++linenum % 2) ? ".odd" : ".even"), "<b>Action:</b> " + this.misc[ "actType" ] );
        ...
        function procMsg( msg, otherTests ) {   // Anything before the first Tildi gets displayed to the user.
          if( otherTests && (msg in po.misc )) {
            let m = po.misc[ msg ];
            if( m.length > 1) {
              body.append( (( ++linenum % 2) ? ".odd" : ".even"), Earthdawn.getParam( m, 1, "~" ));
              playerCardNix.push( body._children.length );
              let i = m.indexOf( "~" );
              if( i !== -1 )
                po.doLater += m.slice( i );      // Anything after the first Tildi gets treated as a command an run though the API parser.
        } } }
        procMsg( "displayMsg", true );
...
//                class: "showtip tipsy",   // removing this gives us black on white long tool tips rather than the white on black tooltips that have a display bug.
        ...
        if( whichMsgs === Earthdawn.whoTo.public ) {                        // public:    Main message to everybody. Maybe supplementary messages to player and/or gm.
          ...
          if( ("gmTN" in this.misc) || gmResult.length > 0 || (( "secondaryResult" in this.misc ) && !TNall)) {
            ...
            // if ( pIsGM && buttonLine )
            //   bodygm.append( (( ++linenum % 2) ? ".odd" : ".even"), buttonLine); //buttonLine is sent to all controlling players
            ...
            if ( bgmline) //Send the gdmLine, i.e. the part only visible to GM
              setTimeout(function() {
                try {
                          // After public message, supplementary message to GM with TN
                    po.chat( sectgm.toString().replace( /(\r|\n)/g , " "), Earthdawn.whoTo.gm | Earthdawn.whoFrom.player | Earthdawn.whoFrom.character );
                ...
                setTimeout(function() {
                  try {
                      ...
                      bodyplayer.append( (( ++linenum % 2) ? ".odd" : ".even"), buttonLine);
                            // After public message, supplementary message to GM and player with buttons.
                      po.chat( sectplayer.toString().replace( /(\r|\n)/g , " "), Earthdawn.whoTo.gm | Earthdawn.whoTo.player | Earthdawn.whoFrom.player | Earthdawn.whoFrom.character | Earthdawn.whoFrom.noArchive );
                  } catch(err) {Earthdawn.errorLog( "ED.rollFormat setTimeout() error caught: " + err, po );}
                }, 700);
          ...
          if(( "secondaryResult" in this.misc ) && !TNall) {
            ...
            playerCardNix.push( body._children.length );
                  ...
                  // Message to GM only, with all gm information appended to the main body that would have been the public message.
          this.chat( sect.toString().replace( /(\r|\n)/g , " "), Earthdawn.whoTo.gm | Earthdawn.whoFrom.player | Earthdawn.whoFrom.character );
          if( playerCard ) {
            while( playerCardNix.length > 0 )
              body._children.splice( playerCardNix.pop() -1, 1);
                  // After a message to the GM only, send a player card to player saying the roll was made.
                  // Strip out the bit that tells it to roll some dice.
            this.chat(( "roll" in this.misc ) ? sect.toString().replace( /(\r|\n)/g , " ").replace( " = " + this.misc[ "roll" ], "") : sect.toString(),
...
//          let popto = body._children.length;
          if( buttonLine )
            body.append( (( ++linenum % 2) ? ".odd" : ".even"), buttonLine);
                  // Roll results to both player and GM. This is the message to the player.
          this.chat( sect.toString().replace( /(\r|\n)/g , " "), Earthdawn.whoTo.player | Earthdawn.whoFrom.player | Earthdawn.whoFrom.character );
// JBF reports this might be crashing the API sometimes?     Plus wants the GM to see the buttons.
...
//            body._children.pop();
          ...
          if(( "secondaryResult" in this.misc ) && !TNall)
            body.append( (( ++linenum % 2) ? ".odd" : ".even"), "<b>Cntr-Atk: TN</b> " + this.misc[ "targetNum2" ] );
                  // Roll results to both player and GM. This is the message to the GM. Has what the player saw, plus Target number information.
...
//                    f = '<strong style=&quot;color: #f08080;&quot;>' + v + "</strong>";   // Value is 1. Reddish.
            ...
            case "M":
              ...
              break;
            default:
              Earthdawn.errorLog( "Error in ED.BuildRoll(). Unknown type '" + r[ "type" ] + "' in rolls " + i + ". Complete roll is ...", this );
              log( rolls );
...
// log("this.Spell ssa is " + JSON.stringify(ssa));
        ...
        if( noseqactive ) {     // The sequence has been called from a clean start
          ...
          if( seqended || other ) { // We are actually restarting a new sequence. We could potentially send warning if other is true, but still, we would treat as a new sequence
            ...
            bGrimAtt = Earthdawn.safeString( aseq[ 1 ] ).toLowerCase().endsWith( "a" );   //If a Grimoire is attuned we stored GA
            seqcontinue = true;
...
// log("seq reinitialized with " + seq);
        ...
        let tmp = Earthdawn.getAttrBN( this.charID, presp + "Discipline", "0"),// Name of discipline, not short
          disp = Earthdawn.dispToName( tmp, "weaving" ),
        ...
        let target = Earthdawn.getAttrBN( this.charID, presp + "Casting", "MD1: @{target|Target|token_id}"),
          wilselect = Earthdawn.getAttrBN( this.charID, presp + "WilSelect", "None");
        if ( target && (typeof target == "string" ) && target.startsWith( "MD1" ) && seq.includes( "Target" ))
...
//            let strain = Earthdawn.getAttrBN( po.charID, pre2 + "Strain", "", true),
        ...
        switch( ssa[ 2 ] ) {
          case "Sequence": {    //This is the master sequence... Where the magic happens
            ...
            if( isng && bGrim ) { //isng because creatures cast raw, like if it was matrix
            ...
            if(noseqactive || seqended) {     //This is a clean start either after a reset, or after ending correctly a sequence
              ...
              let bGrim2 = Earthdawn.safeString( aseq[ 1 ] ).toLowerCase().startsWith( "g" );
...
// CDD toCheck
// 	        let castdiff = Earthdawn.getAttrBN( this.charID, presp + "Casting", "MD1: @{target|Target|token_id}");
            ...
            if( !(noseqactive || seqended || (step==0 && !other))) {
              if ( other ) {
                ...
                line( "<b>Warning!</b> Trying to add threads for:<b> " + Earthdawn.getAttrBN( this.charID, pre + (bGrim ? "Name" : "Contains"), "" )
                    + "</b>When sequence was already started. Choosing an Extra Thread will restart the sequence","Extra Threads Can only be selected when starting a sequence. You can still select it at any time, but you will jump back at the start of the sequence.");
              }
            } else //Normal Case
  ...
  //JBF@JBF@CD Attempt at Targetting
            ...
            if( fx && ( fx.startsWith( "Attempt" ) || fx.startsWith( "Success")))
              this.misc[ "FX" ] = fx;
            this.misc[ "displayMsg" ] = Earthdawn.getAttrBN( this.charID, presp + "DisplayText", "");
            this.misc[ "successMsg" ] = Earthdawn.getAttrBN( this.charID, presp + "SuccessText", "");
            this.misc[ "failMsg" ] = Earthdawn.getAttrBN( this.charID, presp + "FailText", "");
          ...
          case "Effect": {    // Spell Will Effect test.
              ...
              if(wilselect=="Circle") this.misc[ "reason" ] += isng ? (disp + " Circle") : "Circle/SR";
              else this.misc[ "reason" ] +=(Earthdawn.getAttrBN( this.charID, pre2 + "Name", "0"))
                    + ((wilselect=="Rank") ? " Rank" : "");
              let fx = Earthdawn.getAttrBN( this.charID, presp + "FX", "");
              ...
              let t2 = Earthdawn.parseInt2( Earthdawn.getAttrBN( this.charID, (!isng && wilselect=="Circle") ? "SrRating" : (pre2 + n), 5));   //This is the step
              this.Lookup( 1, [ "value", presp + "WilEffect", t2.toString(),  aseq[ 5 ]  ] );
              if( !skipseq ) {
                aseq[ 3 ] = "5";
                SaveSeq();
          ...
          case "WipeMatrix": {
            ...
            SaveSeq();
            this.chat( "All Matrix have been wiped !" , Earthdawn.whoTo.player | Earthdawn.whoTo.gm | Earthdawn.whoFrom.character);
          ...
          case "Reset": {
            ...
            /*
              ...
              T_Strain_max is used in a ~Strain: @{T_STrain_max}. It is either equal to T_Strain or to a query with T_Strain as default
              T_Strain_Advanced is a dropdown that is either "" (fixed strain) or "Ask" 
              T_StrainAdvanced_max is the formula to be displayed to the user
            */
    this.strainCalc = function( pre )  {
      'use strict';
      try {
        return ("strain" in this.misc) ? Earthdawn.parseInt2( this.misc[ "strain" ] ) : this.getValue( pre + "Strain");
    ...
    'use strict';
      ...
      try {
        ...
        if( ret & Earthdawn.flagsTarget.Mask ) {        // Do this only if found a valid string above.
          if( tmp.slice( 2, 3) === "h" || tmp.endsWith( "each"))  //JBF@CD-v3.1 temporary to deal with the XX-each case as a Highest until able to deal with it
            ...
            if( ssa === undefined || ssa.length < 3 || ssa[2] === "") {
              ...
              _.each( attributes, function ( indexAttributes ) {
                if( indexAttributes.get( "name" ).endsWith( "_SPM_RowID" )) {
                  ...
                  if ( Earthdawn.getAttrBN( po.charID, Earthdawn.buildPre( "SPM", row ) + "Rank", 0, true) >= circle && !matrices.includes( row ))   // Only list the matrices of a rank ot hold this spell and are not already listed.
                    matrices.push( row );
                }
              }); // End for each attribute.
...
//            Earthdawn.setWithWorker( aobj, "current", matrixTo );
          ...
          case "inmatrix": {    // This alternate entry point is for when it detects a matrix has been destroyed.
            ...
            _.each( spells, function (rID ) {     // Set InMatrix depending upon if spellsInMatrix includes the spell.
              ...
              Earthdawn.setWW( Earthdawn.buildPre( "SP", rID ) + "spmRowID", "current", (ind === -1) ? "0" : matrixes[ ind ] );
            });
          } break;
...
// convert to message
      Earthdawn.setWW( "SWflag", "SheetUpdate,"+ Math.random(), rep );
...
//log("destroying");  log( attr);
  ...
  on("change:character:name", function (attr, prev) {
    let nm = attr.get("name")
    if( nm )
      Earthdawn.setWW( "charName", nm, attr.get( "_id" ));
