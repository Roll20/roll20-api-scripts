...
Earthdawn.Version = "3.000";       // This is the version number of this API file.  state.Earthdawn.sheetVersion is the version number of the html file.
...
//          Earthdawn.SetDefaultAttribute( cID, Earthdawn.buildPre( code, rowID ) + "CombatSlot", code === "SPM" ? 1 : 0 );
      ...
      if ( t ) {                // No matter what, Remove the token action associated with the old name.
        ...
        if( Earthdawn.getAttrBN( cID, "NPC", "0" ) == Earthdawn.charType.pc ) {
          let tdate = Earthdawn.getAttrBN( cID, "record-date-throalic", "" ),     // First look on the current character sheet
            today = new Date(),
            rnk = Earthdawn.parseInt2( attr.get( "current" )),
    ...
    switch( sa ) {
      ...
      case "APIflag":     // sheetworker has sent us something it wants us to do.
        if( attr.get( "current" )) {   // We tend to get two, one with "current" blank when it is created, and a 2nd one with the real value. Only pay attention to the 2nd one.
        //JBF@CD various trials for the watchdog... To be cleaned up if I give up on that
        // Earthdawn.setWW( "API", 1, cID );
        //
        // let att= Earthdawn.findOrMakeObj({_type: 'attribute', _characterid: cID, name: "API"});
        // att.set( "current", "1");
        //
        // setAttrs(cID,{"API" : "1"});
//log("API FLAG " + attr.get( "current" ));
          let ED = new Earthdawn.EDclass(),
    ...
    //              state.Earthdawn.defRolltype = 0x03;
        ...
        } break;  // end APIflag (sent from sheetworker).
...
//log("at status change"); log(attr); log(prev);
      ...
      case "Durability-Rank":
        ...
        break;
      case "Hide-Spells-tab":       // CDD Todo remove just before next release
...
//log("this.chat " + iFlags + " customFrom " + customFrom);
    ...
    if( customFrom )   // if we specified a customFrom as argument, it overrides everything
      wf = customFrom;
    else if ((iFlags & Earthdawn.whoFrom.player) && (iFlags & Earthdawn.whoFrom.character) && po && po.tokenInfo && ("name" in po.tokenInfo )) // From a player and we have the name of the token
      ...
      if( fnd && fnd.length > 0 )
        wf = fnd;
    } else if (!this.msg || (iFlags & Earthdawn.whoFrom.api) || (this.msg.type === "api") || (this.msg.playerid === "API"))
      wf = "API";
    ...
    if ( !(iFlags & Earthdawn.whoTo.playerList )) {     // Don't do this section if specified to do the playerList section.
      ...
      if(( iFlags & Earthdawn.whoTo.player ) && ( !(iFlags & Earthdawn.whoTo.gm) || ((iFlags & Earthdawn.whoTo.gm) 
            && edc && edc.msg && edc.msg.playerid && !playerIsGM( edc.msg.playerid ))))
        ...
        switch ( ssa[ 1 ] ) {
          ...
          case "nac": {
            ...
            if( state.Earthdawn.sheetVersion < 1.8 ) {
              ...
              if( Earthdawn.getAttrBN( this.charID, pre + "NotVisionBased", "0" ) != "1" ) {    // NotVisionBased depreciated 8/22
...
*/
        ...
        if( modtype.search( /Armor/ ) != -1 )         // "@{Adjust-All-Tests-Total}+(-1*@{Armor-IP})"
          ...
          modtypevalue = this.getValue( "Initiative-Mods") + this.getValue( "Initiative-Mods-Auto");
    ...
    this.abilityRebuild = function ( ssa ) {
      ...
      try {
        ...
        function buildTA( what) {   // go through and get information needed to figure out what the token actions should be.
          'use strict';             // This routine also does the actual distruction and rebuilding of the TA.
...
//log("z" ); log(oldmap); log(newmap ); log(build); log(oldab); log(newab);
        ...
        if( ssa[ 0 ] === "addGraphic" || (( ssa.length > 1 ) && ( ssa[ 1 ] === "forceTest"))) {    // Test to see if Token Actions are correct and up to date.
          ...
          let aobj = Earthdawn.findOrMakeObj({ _type: 'attribute', _characterid: cID, name: "abilityRebuildControl" });
          Earthdawn.set( aobj, "current", "Never" );
...
log( ssa);
          ...
          case "repsecfix": {     // RepSecFix: Repeating Section Verify and Fix.    Note: This is now labeled Character Sheet Verify/Fix.
...
//              function findSorted( nm ) {   // if the name is in the sorted array of attributes, return it, otherwise return blank.
//                                            // Used as findObjs substitute, but never returns an array.
//                let ind = _.sortedIndex(sorted, {name: nm}, 'name');
// For some reason the above gives the wrong result and the below gives runtime error.
//                let ind2 = _.sortedIndex(sorted, {name: nm.slice( 0, -5) + "_RowID"}, function( a ) { log(typeof a); return a.get( "name" ); });
//                if( ind < 0 ) return;
//                if( sorted[ ind ].get( "name" ) !== nm ) return;
//                else return sorted[ ind ];
//              }
              let save = sorted[ 0 ];
...
//                    let attrib = findSorted( nm.slice( 0, -5) + "_RowID" );
                  ...
                  if( orig.indexOf( rowID ) === -1 )  {   // This is a list of all unique rowID's we found.
                    orig.push( rowID );
                    lcase.push( Earthdawn.safeString( rowID ).toLowerCase() );
                  }
                  let r = Earthdawn.safeString( rowID );
                  if( rowID === r.toLowerCase() || rowID === r.toUpperCase() ) {
...
//log( nm);
              ...
              if( !silent ) log( "Debug RepSecFix: Finished after " + (Date.now() - timer) + " ms.");
          ...
          if ( this.misc[ "Special" ] === "CorruptKarma" ) {
            ...
            if( !(this.bFlags & Earthdawn.flags.NoOppMnvr )) {    // We don't record these extra successes as something that affects damage rolls.
              ...
              _.each( attributes, function (att) {
                ...
                if( att.get("name").endsWith( "_Mod-Type" ) && att.get("current").startsWith("Damage") && (((att.get("current").endsWith("CC") ? 1 : -1 ) < 0) == (cc < 0))) {      // Again just looking to make sure they have the same sign.
                  let pre = Earthdawn.buildPre( att.get( "name")),
                    name = Earthdawn.getAttrBN( po.charID, pre + "Name", "" )
        ...
        _.each( attributes, function (att) {
          if (att.get( "name" ).endsWith( "_T_Special" ) && att.get( "current" ).startsWith( "SPL-" )) {      // If any of the SPL- Talents.
            ...
            if(att.get( "current" ).endsWith( disp )) {       // Thread Weaving.
              ...
              if(Earthdawn.getAttrBN( po.charID, pre2 + "Strain", "0") == "0")
