...
Earthdawn.addIcon = function ( icon, size ) {
  'use strict';
  try {
    return "";
/*
    ...
    let x = '<span class="sheet-rolltemplate-' + s + ' sheet-rolltemplate-icons-' + icon + '"></span>';
log(x); return x;
*/
...
Earthdawn.testNoRowID = function ( code, strict ) {
  'use strict';
   return ( code === "PER" || code === "MSG" || ( !strict && (code === "TI" || code === "MSK" )))     // These don't have a RowID.
};  // end testNoRowID. 
    ...
    this.Damage = function( ssa ) {
      ...
      try {
        ...
        if( !bStun ) {    // normal damage. This is identical for characters and mooks.
          currDmg += dmg;
          if( bRecovery ) {
            this.chat( this.tokenInfo.name + " recovered " + ((currDmg < 0) ? ((dmg - currDmg) * -1) : (dmg * -1) ) + Earthdawn.addIcon( "damage", "l" )
          ...
          if( stunDmg != 0 || dmg != 0 ) {
            ...
            if( bRecovery ) {
              this.chat( this.tokenInfo.name + " recovered " + ((stunDmg < 0) ? ((dmg - stunDmg) * -1) : (dmg * -1) )
                     + Earthdawn.addIcon( "stun", "l" ) + " stun. New value " + ((stunDmg < 0) ? 0 : stunDmg ) + "." , Earthdawn.whoTo.player | Earthdawn.whoTo.gm | Earthdawn.whoFrom.character);
              if( stunDmg < 0)
                stunDmg = 0;
            }
...
//          step = Earthdawn.parseInt2(this.misc[ "step" ] || 0) + this.ssaMods( ssa );   // Add any mods to roll.
        ...
        if( "targettype" in this.misc ) {
          ...
          sh += " " + (e ? "each " : "") + this.misc[ "targettype" ].replace( "p1p", " +1p").replace( "-each", "" ).replace(
