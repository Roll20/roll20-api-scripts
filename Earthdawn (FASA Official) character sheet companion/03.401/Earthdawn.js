...
Earthdawn.Version = "3.401";       // This is the version number of this API file.
...
Earthdawn.constantAlt = function( what ) {   
  ...
  try {
    ...
    switch ( Earthdawn.safeString( what.trim().toLowerCase() )) {
      ...
      default:
        log( Earthdawn.timeStamp() + "Earthdawn.constantAlt: Illegal argument '" + what + "'." );
        return;
    }
    return String.fromCharCode( a );
  } catch(err) { log( Earthdawn.timeStamp() + "Earthdawn:constantAlt( " + what + " ) error caught: " + err ); }
} // end constantAlt()
...
Earthdawn.constantButton = function( what, nestingLevel ) {   
  ...
  try {
    ...
    switch ( Earthdawn.safeString( what.trim().toLowerCase() )) {    // For this upper half, we want to return html codes.
      ...
      case undefined:   return "";          // undefined is a legal value that returns an empty string.
      default:
        log( Earthdawn.timeStamp() + "Earthdawn.constantButton: Illegal argument '" + what + "'." );
        return;
    }
    ...
    return r;
  } catch(err) { log( Earthdawn.timeStamp() + "Earthdawn:constantButton( " + what + " ) error caught: " + err ); }
} // end constantButton()
...
Earthdawn.constantIcon = function( what, sortOffset ) {   
  ...
  try {
    ...
    switch ( Earthdawn.matchString( what )) {    // For this upper half, we have symboles that are usually used in Token Actions. They have to be sorted.
      ...
      default:
        log( Earthdawn.timeStamp() + "Earthdawn.constantIcon: Illegal argument '" + what + "'." );
        return;
    }
    if( sortOffset !== false ) {
  ...
  } catch(err) { log( Earthdawn.timeStamp() + "Earthdawn:constantIcon( " + what + " ) error caught: " + err ); }
} // end constantIcon()
...
//log( "keywordCheck '" + kl  + "'   ret " + ret);
    }
    return ret;
  } catch(err) { log( Earthdawn.timeStamp() + "Earthdawn.keywordCheck() error caught: " + err ); }
} // end keywordCheck()
      ...
      try {
        ...
        switch ( ssa[ 1 ] ) {
          ...
          case "nac": {
            ...
            let kw = Earthdawn.keywordCheck( special, "Recovery", "Recovery-Woodskin", "Initiative", "Knockdown" );
            if( kw & 0x02 )
              ...
              if( code === "SP" ) {
                ...
                if( spmID && spmID.length > 1 ) {
                  ...
                  if( cont && cont.length > 1 )
                    addit( Earthdawn.constantIcon( "Spell" ) + cont, "!edToken~ %{selected|" + Earthdawn.buildPre("SPM", spmID) + "Roll}" );
                  else
                    Earthdawn.errorLog( "abilityRebuild:BuildTA: " + sa + " could not find out what: " + spmID + " contains.", po );
                }
              } else            // All except SP/SPM
                addit( Earthdawn.constantIcon( code ) + att.get("current"), "!edToken~ %{selected|" + Earthdawn.buildPre( code, Earthdawn.repeatSection( 2, sa ) ) + "Roll}" );
            }
          });
              ...
              _.each( attributes, function (att) {
                if (att.get("name").endsWith( "_DSP_Name" ))
                  s += Earthdawn.makeButton( att.get("current") + " " + edParse.getValue( Earthdawn.buildPre( "DSP", att.get("name") ) + "Circle", id),
                        "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{" + id + "|" + Earthdawn.buildPre( "DSP", att.get("name") ) + "halfMagic}",
              ...
              _.each( attributes, function (att) {
                if (att.get("name").endsWith( "_SK_Name" )) {
                    s1.push( { a: Earthdawn.getAttrBN( id, Earthdawn.buildPre( "SK", att.get("name"))  + "Name", ""),
                        b: "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{" + id + "|" + Earthdawn.buildPre( att.get("name")) + "Roll}",
              ...
              _.each( attributes, function (att) {
                if (att.get("name").endsWith( "_T_Name" ) || att.get("name").endsWith( "_NAC_Name"))
                  if( !non || (Earthdawn.getAttrBN( id, Earthdawn.buildPre( att.get("name") ) + "CombatSlot", "") != "1")) {
                    let kw = Earthdawn.keywordCheck( Earthdawn.getAttrBN( id, Earthdawn.buildPre( att.get("name") ) + "Special", ""), true, "Recovery", "Initiative"),
                    ...
                    s += Earthdawn.makeButton( Earthdawn.getAttrBN( id, Earthdawn.buildPre( att.get("name") ) + "Name", ""),
                        "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{" + id + "|" + Earthdawn.buildPre( att.get("name")) + "Roll}",
...
//                        this.chat( this.tokenInfo.name + " spent " + kdice + " karma." );
        ...
        //
        // Note, this routine is not used yet, but cuold be. 
    this.keyword = function( pre ) {
      'use strict';
      try {
        let a = arguments;
log( typeof a); log ("this.keyword"); log( typeof arguments);
        return Earthdawn.keywordCheck( Earthdawn.getAttrBN( this.charID, pre + "Special", "None"), a.slice( 1 ));    // We cut off the pre, and instead send the keyword list and all the canidates.
//        return Earthdawn.keywordCheck( Earthdawn.getAttrBN( this.charID, pre + "Special", "None"), arguments.slice( 1 ));    // We cut off the pre, and instead send the keyword list and all the canidates.
...
/*
        ...
        if( arguments.length > 1 ) {
          ...
          if( special )
            for( let i = 1; i < arguments.length; ++i )
              if( special === arguments[ i ] )      // in future versions this can check if in the lst, not just equal.
                ret += 1 << (i - 1);
        }
        return ret;
*/
...
//log(lpvm);
          for( let i = 0; i < lpvm.length; i++)
            if( lpvm[ i ] && lpvm[ i ].includes( ";" ) && [ "SpellKnack", "BindingSecret" ].includes( Earthdawn.getAttrBN( po.charID, 
                  Earthdawn.buildPre( Earthdawn.getParam( lpvm[ i ], 1, ";"), Earthdawn.getParam( lpvm[ i ], 2, ";")) + "Type", ""))) {
...
//log( "BoD " + what + "   " + t);
        ...
        _.each( attributes, function (att) {
          ...
          if( wilselect == "Circle" && isng &&  att.get( "name" ).endsWith( "_DSP_Name" ) && ( disp == Earthdawn.dispToName( att.get( "current" ), "weaving" ))) {   // Effect based on Circle Namegiver
            txtfx = addit( txtfx, "CircleNG", Earthdawn.buildPre( att.get( "name" )), disp);
          }
        ...
        switch( ssa[ 2 ] ) {
          ...
          case "TuneGrimoire": {
            ...
            if( ssa[ 3 ].startsWith( "repeating_" ))    // we have a pre
              ...
              pre2 = Earthdawn.buildPre( "T", ssa[ 3 ]);
            this.strainAdd( pre2 );
        ...
        //    initSpell(this); //Action was taken initialize sequence
            ...
            if( spellseq.KnacksChosenID )
              for( let i = 0; i < spellseq.KnacksChosenID.length; i++)
                this.strainAdd( Earthdawn.buildPre( "SP", spellseq.KnacksChosenID[ i ] ));     // This is the strain for the Knacks.
            this.Karma( pre2 + "Karma", 0 );
            ...
            if( fail )
              ...
              if( spellseq.KnacksChosenName )
                for(let i=0 ; i < spellseq.KnacksChosenName.length ; i++ )
                  lines.push( "<b> Spell Knack :</b> " + Earthdawn.texttip(spellseq.KnacksChosenName[ i ], Earthdawn.getAttrBN( this.charID, Earthdawn.buildPre("SP",spellseq.KnacksChosenID[ i ]) + "Notes", "")));
      ...
      try {
        ...
        for( let i = 0; i < m.length; ++i ) {
          ...
          if( Earthdawn.safeString( n[ 0 ] ).trim().startsWith( "repeating_" ))    // we have a pre
            ...
            pre = Earthdawn.buildPre( n[ 0 ], n[ 1 ]);
          else
            pre = undefined;
            ...
            if( ssa === undefined || ssa.length < 3 || ssa[2] === "") {
              ...
              if( matrices.length > 0) {      // For each matrix found, make a chat button.
                ...
                let s = Earthdawn.makeButton( "Pseudo matrix", "!Earthdawn~ charID: " + po.charID + "~ TuneMatrix: Share: " 
                      + "?{What type of Pseudo Matrix|Standard,Std|Enhanced,Enh|Armored,Arm|Shared,Sha}: " + ssa[ 2 ], "Create a Pseudo Matrix & Attune", "param" );
