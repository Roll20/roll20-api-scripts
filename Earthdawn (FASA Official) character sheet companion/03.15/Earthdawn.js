...
// Last updated: 2023 Sep 5th.
//
// Earthdawn (FASA Official) Character sheet and associated API Copyright 2015-2023 by Christopher D. Dickey.
//
...
Earthdawn.Version = "3.15";       // This is the version number of this API file.  state.Earthdawn.sheetVersion is the version number of the html file. 
...
Earthdawn.abilityRemove = function ( cID, Ability ) {
  ...
  try {
    ...
    // size: "s" then class sheet-rolltemplate-iconSmall. "l" (lowercase l) or "b" then -iconLarge. Otherwise sheet0rolltemplate-icon
...
<b>Result:</b> 
<span style="background-color: #fef68e ; border: 2px solid #fef68e ; padding: 0 3px 0 3px ; font-weight: bold ; cursor: help ; font-size: 1.1em" 
title="Rolling 2d10! = (5+9)">14</span>
<span style="color: green">Success</span>!<span style="background: lightgoldenrodyellow ; color: black" 
title="2 total successes!"> (1 EXTRA Success)</span>
</div>
...
Earthdawn.testNoRowID = function ( code, strict ) {
  'use strict';
   return ( code === "PER" || code === "MSG" || ( !strict && code === "MSK" ))     // These don't have a RowID.
};  // end testNoRowID. 
        ...
        if( !bStun ) {    // normal damage. This is identical for characters and mooks.
          ...
          if( bRecovery ) {
            if( currDmg < 0 ) {     // If all damage has been healed, we have to know whether this is a wood skin test or not, since those are handled differently. 
              if( Woodskin == 1 )           // We know this is woodskin and we have negative damage.
                recMsg += " Wood Skin added " + (dmg * -1) + Earthdawn.addIcon( "damage", "l" ) + " Health. New damage value " + currDmg + ".";
              else if (Woodskin === 2 ) {   // We know this is NOT woodskin and we have negative damage, have the leftover heal stun damage.
                recMsg += " recovered " + ((dmg - currDmg) * -1) + Earthdawn.addIcon( "damage", "l" ) + " damage. New value 0.";
                bStun = true;
                dmg = currDmg;
                currDmg = 0;
              } else {                        // Ask if this is woodskin or not. 
                this.chat( "Is this a "
                      + Earthdawn.makeButton( "Wood Skin", "!Earthdawn~ charID: " + this.charID + "~ TokenList: " + this.tokenIDs.join( ":" ) 
                      + "~ " + ssa.join( ":" ).replace( /recovery/gi, "WoodSkin"), 
                          "This recovery test healed all current normal damage. Wood Skin tests are recorded by setting your current damage to a negative number. Pressing this button will allow this recovery test to leave you with a negative amount of damage.") 
                      + " or " 
                      + Earthdawn.makeButton( "Not a WS recovery test", "!Earthdawn~ charID: " + this.charID + "~ TokenList: " + this.tokenIDs.join( ":" ) 
                      + "~ " + ssa.join( ":" ).replace( /recovery/gi, "NotWoodSkin"),
                          "This button will heal all of your normal damage down to zero, then start healing stun damage down to zero. Excess healing will be discarded." )
                      , Earthdawn.whoTo.player | Earthdawn.whoTo.gm | Earthdawn.whoFrom.character);
                return;   // After sending the two buttons, exit without doing anything else.
              }
        ...
        switch ( Earthdawn.safeString( ssa[ 1 ] ).toLowerCase() ) {
          ...
          case "macrocreate":
            try {
              ...
              if( cmd != "delete" && ( !this.edClass || !this.edClass.msg || !this.edClass.playerid || !playerIsGM( this.edClass.msg.playerid ))) {
...
//                rawSP = rawSP.slice( 0, rawSP.indexOf( "-" ));     // The price is a range, such as 100 - 175.   Just lop off the 2nd half and assume they got the cheap price.
              ...
              if( rawSP.toLowerCase().indexOf( "cp" ) === -1 )      // convert to copper. 
                amount *= 10;
              if( rawSP.toLowerCase().indexOf( "gp" ) === -1 ) 
                amount *= 100;
...
//        if( sh ) {
//          sh = sh.replace( Earthdawn.constant( "commaalt" ), "");
// This code substitutes in icons for PD, MD, SD. But the icons don't really work well in the subheader. 
//          let ind = sh.indexOf( Earthdawn.constant( "commaalt" ));    // commaalt get substituted in for defense type so we know the actual length of the subheader.
//          if( ind !== -1 ) {
//            let ty = sh.slice( ind + 1, ind + 3);
//            if( ty === "PD" || ty === "MD" || ty === "SD" ) {
//              sh = sh.replace( Earthdawn.constant( "commaalt" ) + ty, Earthdawn.addIcon( ty.toLowerCase(), "s", ty ));
//        } }
//          ind = sh.indexOf( Earthdawn.constant( "colonalt" ));      // colonalt gets substituted in for strain. 
//          if( ind !== -1 ) {
//            let strain = Earthdawn.parseInt2( sh.slice( ind + 1)),
//                ind2 = sh.indexOf( "train", ind );
//            if( strain ) {
//              sh = sh.slice( 0, ind ) + Earthdawn.addIcon( "strain" + ((strain < 4) ? strain.toString() : ""), "s", 
//                    strain.toString() + " strain", (strain < 4) ? undefined : strain.toString()) + sh.slice( ind2 + 5);
//        } } }

        sectMain.append( "", (("headcolor" in this.misc) ? Earthdawn.addIcon( Earthdawn.safeString( this.misc[ "headcolor" ] ).toLowerCase(), "l", this.misc[ "headcolor" ]) : "") + this.misc[ "reason" ]      // Main Header
              + ((bsub || !sh ) ? "" : "  --  " + new HtmlBuilder( "span", sh, { class: "sheet-rolltemplate-subheadertext" })), 
...
//          bodyMain.append( "", this.misc[ "warnMsg" ], { style: { "background-color": "Orange", "text-align":   "center" }});
        ...
        if( "ModType" in this.misc ) {    // Conditions and options that affected the step.
          ...
          if(( modBit & 0x20 ) && !Earthdawn.getAttrBN( this.charID, "effectIsAction", "0", true ) && (x = this.getValue( "Adjust-Effect-Tests-Misc" )) != 0)
            txt += Earthdawn.addIcon( "effect", "Effect Misc: Misc modifiers that apply to Effect Tests.", undefined, x );
          if(( modBit == 0x12 || modBit == 0x14 ) && (x = this.getValue( "Adjust-Attacks-Misc" )) != 0)
            txt += Earthdawn.addIcon( "attack", "Attack Misc: Misc modifiers that apply to Attack Tests.", undefined, x );
          if(( modBit == 0x22 || modBit == 0x24 ) && (x = this.getValue( "Adjust-Damage-Misc" )) != 0)
            txt += Earthdawn.addIcon( "damage", "Damage Misc: Misc modifiers that apply to Damage Tests.", undefined, x );
...
//          } // from Action() or spell.
          ...
          if(( modBit & 0x02 || modBit & 0x04 )&& this.getValue( "Creature-DivingCharging" ) != "0" )
            txt += Earthdawn.addIcon( "charging", "Diving or Charging: Added to Attack, and Damage", undefined, this.getValue( "Creature-DivingCharging_max" ) );
        ...
        if( "FunnyStuffDone" in this.misc ) {
          ...
          if( txt.length > 0 ) {
            bodyMain.append( "", txt + ". Roll was " + this.BuildRoll( "showResult" in this.misc && this.misc[ "showResult" ], this.misc[ "DiceOrigional" ].total, this.misc[ "DiceOrigional" ] ),
                { style: { "background-color": "Orange", "color": "Black" }});
            if( playerCard ) playerCardNix.push( body._children.length );
          }
...
//log(sectMain.toString().replace( /(\r|\n)/g , " "));
...
// log("seq reinitialized with " + seq);
        ...
        sectSpell.append( "", Earthdawn.getAttrBN( this.charID, presp +  "Name", ""), { style: {
...
// 	        let castdiff = Earthdawn.getAttrBN( this.charID, presp + "Casting", "MD1: @{target|Target|token_id}");
          ...
          case "Cast": {
                  ...
                  //At this stage, Action was taken, so we actually reset the sequence
            if( other && !seqended ) { //Another spell was in middle of an unfinished sequence
              this.misc[ "warnMsg" ]= "Canceling Sequence for "
  ...
  //JBF@JBF@CD Attempt at Targeting
            ...
            let tt = Earthdawn.getAttrBN( this.charID, presp + "sayTotalSuccess", 0, true );
          ...
          case "Cast2": {       // RollFormat calls back to here. We return additional formatted lines. 
            ...
            if( fail )
              ...
              let nmbrs = Earthdawn.getAttrBN( po.charID, presp + "Numbers", "" );
              if( Earthdawn.getAttrBN( po.charID, presp + "Discipline", "0") === "Illusionism" )
