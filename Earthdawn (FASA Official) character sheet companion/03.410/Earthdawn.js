...
Earthdawn.Version = "3.41";       // This is the version number of this API file.
...
Earthdawn.buildPre = function ( cid, code2, rid, special ) {
  'use strict';
  try {
    let badCid;
    ...
    {
      badCid = true;
      Earthdawn.errorTrace( "API Earthdawn:buildPre() error. Recived code as a cID: " + cid + "   code: " + code2 + "   RowID: " + rid );
      cid = code2; code2 = rid; rid = special;
    }
    let ret,
    ...
    if( code === rid ) 
      ...
      switch( code ) {
        ...
        case "SKA": ret = "repeating_skilla_"         // Obsolete
                                        code = "SK";    break;    // Set the 4 above to just be SK skills.
...
Earthdawn.constantAlt = function( what ) {   
  ...
  try {
    ...
    switch ( Earthdawn.safeString( what.trim().toLowerCase() )) {
      ...
      case "colonalt2":             a = 0x00F4;   break;    // Colon replacement character alternate. Used in conjunction with tildiFix.     LATIN SMALL LETTER O WITH CIRCUMFLEX
      ...
      try {
        ...
        switch ( ssa[ 1 ] ) {
          ...
          case "nac": {
            ...
            kw = Earthdawn.keywordCheck( special, "Recovery", "Recovery-Woodskin", "Initiative", "Knockdown" );
            if( kw & 0x02 )
            ...
            for( let k in lst ) {
              ...
              _.each( attributes, function (att) {
                if (att.get("name").endsWith( "_SK_Name" )) {
                    s1.push( { a: Earthdawn.getAttrBN( id, edParse.buildPre( "SK", att.get("name"))  + "Name", ""),
                  ...
                  s2.push( { a: Earthdawn.getAttrBN( id, edParse.buildPre( "SKK", att.get("name") ) + "Name", ""),
                        b: "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{" + id + "|" + edParse.buildPre( att.get("name")) + "Roll}"});
                } else if (att.get("name").endsWith( "_SKA_Name" ))
                  s3.push( { a: Earthdawn.getAttrBN( id, edParse.buildPre( "SKA", att.get("name") ) + "Name", ""),
                        b: "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{" + id + "|" + edParse.buildPre( att.get("name")) + "Roll}"});
              }); // End for each attribute.
...
//                        this.chat( this.tokenInfo.name + " spent " + kdice + " karma." );
    ...
    this.keyword = function( pre ) {
      'use strict';
      try {
        arguments[ 0 ] = Earthdawn.getAttrBN( this.charID, pre + "Special", "None");      // change the first argument from pre to the value of special. 
...
*/
          case "macrocreate":
            try {
              if( !ssa.includes( "firstRun" ) && !(this.edClass && this.edClass.msg && this.edClass.msg.playerid && playerIsGM( this.edClass.msg.playerid )))
                ...
                if( ssa.includes( "Create" ) || ssa.includes( "Refresh" )) {    // create the Earthdawn macros.
...
// If this talent targets the targets defenses, check all of targets talents, knacks, and skills to see if they have an active defnese., if so make a cButton.
                ...
                if((( "Special" in po.misc ) && Earthdawn.keywordCheck( po.misc[ "Special" ], "CursedLuck", "CorruptKarma", "SPL-Spellcasting" )
                      && ( "targetName" in po.misc ))   // We need rollESbutton to memorize hits on spellcasting tests
    ...
    this.rollESbuttons = function()  {
      ...
      try {
        ...
        for( let i = lstart; i < lend; ++i ) {      // do this for each target
          if(( "Special" in this.misc) && Earthdawn.keywordCheck( this.misc[ "Special" ], "CorruptKarma" )) {
            if( playerIsGM( this.edClass.msg.playerid ) ) {
          ...
          } else if(( "Special" in this.misc) && Earthdawn.keywordCheck( this.misc[ "Special" ], "CursedLuck" )) {
            if( playerIsGM( this.edClass.msg.playerid ) ) {
              let targetChar = Earthdawn.tokToChar( this.targetIDs[ i ] );
              if( targetChar )
