...
// Programming Note: HTML Builder call ( @param {string} [tag='div'], @param {(HtmlBuilder|string)} [content=''],  @param {object} [attrs={}] )
// Examples:   return new HtmlBuilder( "", "", { "sheet-rolltemplate-sectSmall" });
//
// Release History: V3.41 Feb 2025.
//
/*
    ...
    Note that this one is a link (a), with a display text, and all of an href, a class, a style, and a title in the third parameter. 
    return new HtmlBuilder( "span", txt, {
        style: { "border": "solid 1px yellow" },
        title: Earthdawn.encode( Earthdawn.encode( tip )) })
    And not this one has a span, some text, and an object that contains a style and a title. 
    return new HtmlBuilder( "span", txt, { class: "sheet-rolltemplate-texttip" });
*/
...
Earthdawn.Version = "3.43";       // This is the version number of this API file.
...
dlog = function( txt ) {      // debug log. This is a function I use ONLY for debug logging. Search for it to find debug logs that need to be taken out.
  log( txt );
}
...
Earthdawn.abilityAdd = function ( cID, ability, actionStr )  {
  ...
  } catch(err) { Earthdawn.errorTrace( "Earthdawn:abilityAdd() error caught: " + err ); }
} // End abilityAdd
...
Earthdawn.abilityRemove = function ( cID, ability ) {
  'use strict';
  try {
    let aname = Earthdawn.matchString( ability );      // get the ability name we are looking for with all the non-latin alphabetic characters stripped out.
    _.each( findObjs({ _type: "ability", _characterid: cID }), function( abObj ) {
      if( aname == Earthdawn.matchString( abObj.get( "name" )))
        abObj.remove();
    });
 } catch(err) { Earthdawn.errorTrace( "Earthdawn:abilityRemove() error caught: " + err ); }
} // End abiltiyRemove
...
Earthdawn.addIcon = function ( icon, size, tip, pre, post ) {
  ...
  try {
    ...
    if( outer )         t += '</span>';
    return t;
 } catch(err) { Earthdawn.errorTrace( "Earthdawn:addIcon() error caught: " + err ); }
} // end addIcon
...
//log( attr);   // use attr.get("name") and attr.get("current").  // {"name":"Wounds","current":"1","max":8,"_id":"-MlqexKD2f4f744TgzlK","_type":"attribute","_characterid":"-MlqeuXlNO51-RYxmJv8"}
      let code = Earthdawn.repeatSection( 3, sa ),
          rid = Earthdawn.repeatSection( 2, sa ),
          name = Earthdawn.repeatSection( 4, sa ),
          pre = Earthdawn.buildPre( cID, code, rid, (prev === undefined) ? "noSafe" : ((name === "RowID" || name === "Name" || name === "Rank" ) ? "search" : undefined ));    // if prev is undefined, then we are in on add attribute, and we don't want to check for a rowID quite yet. If prev exists, then it should be safe to check for one. 
/*
          pre = Earthdawn.buildPre( cID, code, rid, "noSafe" );     // Lets not do safeRowID() in on change attribute. 
          pre = Earthdawn.buildPre( cID, code, rid, (name === "RowID" || name === "Name" || name === "Rank" ) ? "search" : undefined );
      if( rid !== Earthdawn.repeatSection( 2, sa ))     // rid was changed in buildPre() by safeRowID()
        rid = Earthdawn.repeatSection( 2, sa );
*/

/* Obsolete Jan 25
      else if( !(rid in state.Earthdawn.rowIDobj) && !Earthdawn.testNoRowID( code, true ) 
                  && rid !== Earthdawn.getAttrBN( cID, pre + "RowID", "" )) {    // If a repeating_section is changed and no RowID is stored and it is not in the list of ones already being checked.
          state.Earthdawn.rowIDobj[ rid ] = true;
          setTimeout( function() {
            'use strict';
            try{
              if( Earthdawn.getAttrBN( cID, Earthdawn.buildPre( cID, code, rid ) + "RowID", "") !== rid )
                Earthdawn.setWW( Earthdawn.buildPre( cID, code, rid ) + "RowID", rid, cID )
              delete state.Earthdawn.rowIDobj[ rid ]
            } catch(err) { Earthdawn.errorTrace( "Earthdawn:setRowID() error caught: " + err ); }
          }, 900);
        }
*/
      if( name === "Name" || name === "Rank") {
            ...
            if ( att.get("name").startsWith( "repeating_maneuvers_" ))
              if( att.get("current") ) {
                ...
                if ( a ) {
                  ...
                  if( r && t.indexOf( r ) == -1)    // Only if this rid is not already in t.
                    t += ";" + r;
                }
              }
      ...
      let t = (name === "CombatSlot") ? 0x01 : 0x00;
      if ((( name === "Name") && (code === "T" || code === "NAC" || code === "SK" || code === "WPN"))
              || (( name === "Contains") && (code === "SPM" )))
        t = 0x02;
      if ( t ) {                // No matter what, Remove the token action associated with the old name.
        let nmo, nmn,
          symbol = Earthdawn.constantIcon( code );
        ...
        if( cbs ) {
//log(2222); log( cID); log( code); log( rid); log(Earthdawn.buildPre( cID, code, rid )); log(33333);
          Earthdawn.abilityAdd( cID, symbol + nmn, "!edToken~ %{selected|" + pre + "Roll}" );
          if( code === "SKA" )
            Earthdawn.abilityAdd( cID, symbol + nmn + "-Cha", "!edToken~ %{selected|" + pre + "Rollc}" );
        }
        ...
        if( attr.get( "max" ) === "ACK" ) {    // We have got an ACK from the sheetworker. Delete the row.
          ...
          Earthdawn.setWW( pre + "toSheetworker", undefined, cID, undefined, "ACK" );     // Set max but not current
        }
      } else if( sa.endsWith( "_T_Special" )) {
        if( Earthdawn.keywordCheck( current, "CorruptKarma" ))
          Earthdawn.abilityAdd( cID, Earthdawn.constantIcon( "Target" ) + "Activate-Corrupt-Karma", "!edToken~ SetToken: @{target|to have Karma Corrupted|token_id}~ Misc: CorruptKarma: ?{How many karma to corrupt|1}");
        else
          Earthdawn.abilityRemove( cID, Earthdawn.constantIcon( "Target" ) + "Activate-Corrupt-Karma");
      }
      ...
      case "Creature-DivingCharging":
        if( prev && ( prev[ "max" ] != attr.get( "max" ))) {
          ...
          if( Earthdawn.parseInt2( attr.get( "max" ) ))
            Earthdawn.abilityAdd( cID, Earthdawn.constantIcon( "power" ) + w,  "!edToken~ ForEach~ marker: " + ( b ? "ambushing" : "divingcharging") + " :t");
...
//og("at status change"); log(attr); log(prev);
        let ED = new Earthdawn.EDclass( "Attribute", sa );
        ...
        if( mia === undefined || mia.length === 0) {
          Earthdawn.errorTrace( "Earthdawn: On Attribute. Warning. '" + sa + "' not be found in StatusMarkerCollection." );
          break;
      ...
      case "Attack-Rank": {
        if( Earthdawn.getAttrBN( cID, "NPC", "1", true) > 0 && Earthdawn.getAttrBN( cID, "Attack-Rank", 0) != 0)     // NPC or Mook and have a generic attack value.
          Earthdawn.abilityAdd( cID, Earthdawn.constantIcon( "power" ) + "Attack", "!edToken~ %{selected|Attack}");
        else    // PC
          Earthdawn.abilityRemove( cID, Earthdawn.constantIcon( "power" ) + "Attack" );
      } break;
      case "Hide-Spells-Tab": {     // If we are hiding the spell pages, also remove the spell token actions.
        if( current == "1") {       // Check-box is being turned on
          Earthdawn.abilityRemove( cID, Earthdawn.constantIcon( "Spell" ) + "-Grimoire" );
          if (state.Earthdawn.gED)
              Earthdawn.abilityRemove( cID, Earthdawn.constantIcon( "Spell" ) + "-Spells" );
        } else {      // Checkbox is being turned off
          Earthdawn.abilityAdd( cID, Earthdawn.constantIcon( "Spell" ) + "-Grimoire",  "!edToken~ ChatMenu: Grimoire");
          if (state.Earthdawn.gED)
            Earthdawn.abilityAdd( cID, Earthdawn.constantIcon( "Spell" ) + "-Spells",  "!edToken~ ChatMenu: Spells");
        }
      } break;
/*
      case "Karma":
...
*/
      case "Questor": {
        if( current === "None" ) {
          ...
          Earthdawn.abilityAdd( cID, Earthdawn.constantIcon( "karma" ) + "DP-Roll", "!edToken~ %{selected|DevotionOnly}" );
          Earthdawn.abilityAdd( cID, Earthdawn.constantIcon( "karma" ) + "DP-T", "!edToken~ !Earthdawn~ ForEach ~ marker: devpnt :t" );
        }
      } break;
      case "SP-WillforceShow": {
        ...
        let edParse = new ED.ParseObj( ED );
        edParse.charID = cID;
        edParse.TokenActionToggle( Earthdawn.constantIcon( "karma" ) + "Willforce", current === "1" );
      } break;
    } // End switch sa
  } catch(err) { Earthdawn.errorTrace( "Earthdawn.attribute() error caught: " + err ); }
}   // End Attribute()



    // Code = SP, SPM, WPN, etc.
    // rid may EATHER be a rowID, or it may be a whole repeating section attribute name, in which case this routine will extract just the rowID needed.
    // due to a roll20 bug, there are some things that require the code to be lowercase instead of the standard upper. when special == l (el lowercase L) instead of undefined, that happens.
    // Special: noSafe, search, safe
    // when special == "noSafe" it will not call safeRowID (used to prevent then calling each other more than once).
    // Note: keep this in sync with codeToName. 
Earthdawn.buildPre = function ( cid, code2, rid2, special ) {
  'use strict';
  try {
    let badCid;
    if( !cid || cid.startsWith( "repeating_" ) || Earthdawn.codeToName( cid, true ) )     // it used to be that this routine took code as its first argument, I put cid at the top of the list, and this should trigger if it is ever called with a code first.
    {
      badCid = true;
      Earthdawn.errorTrace( "API Earthdawn:buildPre() error. received code as a cID: " + cid + "   code: " + code2 + "   RowID: " + rid );
      cid = code2; code2 = rid2; rid2 = special;
    }
    let ret,
        code = Earthdawn.safeString( code2 ).trim(),
        rid = Earthdawn.safeString( rid2 ).trim();
    if( code.startsWith( "repeating_" )) {          // If code is not a code, but is instead a repeating varable name, get code and rowID from that.
      rid = Earthdawn.repeatSection( 2, code );
      code = Earthdawn.repeatSection( 3, code );
    } else if( rid.startsWith( "repeating_" )) {    // handle case where code was OK, but rid still starts with repeating
      Earthdawn.errorTrace( "API Earthdawn:buildPre() warning.  OK code: '" + code2 + "' but full RowID: " + rid );
      code = Earthdawn.repeatSection( 3, rid );
      rid = Earthdawn.repeatSection( 2, rid );
    }

    if((( special != "noSafe" ) && !badCid ) && ((rid === rid.toLowerCase()) || (special === "search") || ( special === "safe"))) {
      let r = Earthdawn.safeRowID( cid, code, rid, special );
      if( !r ) return undefined;
      if( r != rid )
        rid = r;
    }
    if( rid.lastIndexOf( "_" ) > 15 )   // normal rid are 16 characters, but if a name contains an underscore then the code gets mashed in with the rid, and the code is the first part of the name.
      Earthdawn.errorTrace( "API Earthdawn:buildPre() warning. RowID looks suspicious. It might just contain an underscore, or it might be because the Name contains an underscore.   code: " + code2 + "   RowID: " + rid );

    if( code === rid ) 
      Earthdawn.errorTrace( "API Earthdawn:buildPre() error. Code and RowID are the same, probably rowID was passed blank: " + code2 + "   RowID: " + rid );
    else {
      code = Earthdawn.safeString( code ).toUpperCase();
      switch( code ) {
        case "ARM": ret = "repeating_armor_";           break;
        case "BL":  ret = "repeating_blood_";           break;
        case "DSP": ret = "repeating_discipline_";      break;
        case "ENC": ret = "repeating_enchanting_";      break;
        case "I":         // Obsolete
        case "INV": ret = "repeating_inventory_";       break;
        case "MAN": ret = "repeating_maneuvers_";       break;
        case "MNT": ret = "repeating_mount_";           break;
        case "MSG": ret = "repeating_message_";         break;  // Does not have RowID
        case "MSK": ret = "repeating_masks_";           break;
        case "NAC": ret = "repeating_knacks_";          break;
        case "NSM": ret = "repeatingknacksummary";      break;
        case "PER": ret = "repeating_personality";      break;  // Does not have RowID
        case "SKC": // ret = "repeating_skills_";        // Obsolete. v2.0 and greater skill artistic charisma code uses all attributes of skill artistic.
        case "SKK": // ret = "repeating_skillk_";        // Obsolete
        case "SKAC":                                  // Obsolete. V lower than 2.0: skill artistic charisma code uses all attributes of skill artistic.
        case "SKA": // ret = "repeating_skilla_"         // Obsolete
                    code = "SK";                      // Set the 4 above to just be SK skills.
        case "SK":  ret = "repeating_skills_";          break;
        case "SKL": ret = "repeating_skilll_";          break;    // Language skills are still a seperate thing.
        case "SPM": ret = "repeating_matrix_";          break;
        case "SP":  ret = "repeating_spell_";           break;
        case "SPP": ret = "repeating_spellpreset_";     break;
        case "T":   ret = "repeating_talents_";         break;
        case "TI":  ret = "repeating_threads_";         break;
        case "TR":  ret = "repeating_transaction_";     break;
        case "WPN": ret = "repeating_weapons_";         break;
        default: Earthdawn.errorTrace( "API Earthdawn:buildPre() error. Unknown code: " + code2 + "   RowID: " + rid );
    } }
    return (ret === undefined ) ? undefined: (ret + rid + "_" + ((special == "l") ? code.toLowerCase() : code) + "_");
  } catch(err) { Earthdawn.errorTrace( "API Earthdawn:buildPre() error caught: " + err ); }
};  // end buildPre



// Central test for codes that have no RowID.
// Note that PER, MSG, and TR actually have no RowID. 
//     ENC, TI and MSK actually do, but there is no reason for them to, so unless strict is true we return true for them as well.
Earthdawn.testNoRowID = function ( code, strict ) {
  'use strict';
  code = code.trim();
  return ( code === "MSG" || code === "NSM" || code === "PER" ||  code === "TR" || ( !strict && ( code === "ENC" || code === "MSK" || code === "TI" )))     // These don't have a RowID.
};  // end testNoRowID. 
...
Earthdawn.codeToName = function ( code, silent ) {
  ...
  try {
   switch( code.trim() ) {
      ...
      default: if( !silent ) Earthdawn.errorTrace( "Earthdawn:codeToName() error. Unknown code: " + code );
    }
  } catch(err) { Earthdawn.errorTrace( "Earthdawn:codeToName() error caught: " + err ); }
};  // end codeToName
...
Earthdawn.colonFix = function ( txt ) {
  'use strict';
  return Earthdawn.safeString( txt ).replace( /:/g, Earthdawn.constantAlt( "colonAlt" ));
}; // end colonFix()
...
Earthdawn.tildiFix      = function ( txt ) { 'use strict'; return Earthdawn.safeString( txt ).replace( /~/g, Earthdawn.constantAlt( "tildiAlt" )).replace( /:/g, Earthdawn.constantAlt( "colonAlt2" )).replace( /\|/g, Earthdawn.constantAlt( "pipeAlt" )).replace( /\{/g, Earthdawn.constantAlt( "braceOpenAlt" )).replace( /\}/g, Earthdawn.constantAlt( "braceCloseAlt" )); };
Earthdawn.tildiRestore  = function ( txt ) { 'use strict'; return Earthdawn.safeString( txt ).replace( new RegExp( Earthdawn.constantAlt( "tildiAlt" ), "g" ), "~" ).replace( new RegExp( Earthdawn.constantAlt( "colonAlt2" ), "g" ), ":" ).replace( new RegExp( Earthdawn.constantAlt( "pipeAlt" ), "g" ), "\|" ).replace( new RegExp( Earthdawn.constantAlt( "braceOpenAlt" ), "g" ), "\{" ).replace( new RegExp( Earthdawn.constantAlt( "braceCloseAlt" ), "g" ), "\}" ); };



          // routine to standardize the use of symboles.
          // return alternates for characers we can't use in certain places. So when building a button with a comma, we will use commaAlt and then swap it for a comma later.
Earthdawn.constantAlt = function( what ) {   
  'use strict';
  try {
    let a;
    switch ( Earthdawn.safeString( what.trim().toLowerCase() )) {
      case "commaalt":              a = 0x00F1;   break;    // Comma replacement character.  Character name LATIN SMALL LETTER N WITH TILDE
      case "colonalt":              a = 0x00F2;   break;    // Colon replacement character.  Buttons don't like colons, so anytime we want one in a button, replace it for a while with this.     Latin Small Letter O with Grave
      case "tildialt":              a = 0x00F3;   break;    // Tildi replacement character.  To get the parser to ignore tildis for a while, replace it for a while with this.     Latin Small Letter O with Acute
      case "colonalt2":             a = 0x00F4;   break;    // Colon replacement character alternate. Used in conjunction with tildiFix. Also with the keyword keyword.     LATIN SMALL LETTER O WITH CIRCUMFLEX
      case "pipealt":               a = 0x00F5;   break;    // | Latin Small Letter O with Tilde
      case "braceopenalt":          a = 0x014D;   break;    // { Latin Small Letter O with Macron
      case "braceclosealt":         a = 0x014F;   break;    // } Latin Small Letter O with Breve
      ...
      default:
        Earthdawn.errorTrace( "Earthdawn.constantAlt: Illegal argument '" + what + "'." );
        return;
    }
    return String.fromCharCode( a );
  } catch(err) { Earthdawn.errorTrace( "Earthdawn:constantAlt( " + what + " ) error caught: " + err ); }
} // end constantAlt()



          // routine to standardize the use of symboles.
          // Due to roll20 processing, queries within queries within buttons within chat messages need increasing levels of nesting levels to be specified. 
          // Return the html code for certain symboles that can not appear too early. The number of nestinglevel determines the number of &amp; prepended to the string. Each &amp lets it go through one roll20 interpriation without actually being interprited as the base code yet.
Earthdawn.constantButton = function( what, nestingLevel ) {   
  ...
  try {
    ...
    switch ( Earthdawn.safeString( what.trim().toLowerCase() )) {    // For this upper half, we want to return html codes.
      ...
      case undefined:   return "";          // undefined is a legal value that returns an empty string.
      default:
        Earthdawn.errorTrace( "Earthdawn.constantButton: Illegal argument '" + what + "'." );
        return;
    }
    if( nestingLevel ) {      // level 1 (default) is of the form &#124; level 2 is of the form &amp;#124;  Level 3 is of the form &amp;amp;#124;   Etc. One more amp; for each level.
      for( let i = nestingLevel; i > 1; --i )
        r += "amp;";
      r += "#" + s + ";";     // of the form &#123;
    } else    // level 0 is base symbol ie: |,( etc.
      r = String.fromCharCode( s );
    return r;
  } catch(err) { Earthdawn.errorTrace( "Earthdawn:constantButton( " + what + " ) error caught: " + err ); }
} // end constantButton()


          // routine to standardize the use of symboles.
          // return an icon.
          // We often use icons to differenciate between talents, skills, powers, knacks, spells, etc. Sometimes we just use an icon in a chat message.
          // This gathers all the icons into one place.
          // For this routine, we want to return an actual character, which is part of the extended UTF-16 set,
          // and which roll20 one-click install library seems to corrupt if we use the code, so we can't store it as a literal.
          // For Token Actions we put a bunch of zero width non breaking spaces in front of the icons to force them into sorting how we want them to. 
          // Those will sort numerically with the most zwnbsp to the end of the list. So for example karma and target are both sort 0, so they have no zwnbsp in front of them, but karma appears first since it is lower. After those, Power has sort 1, then all the sort 2 in assending order.

          // if sortOffset false, don't put any zwnbsp in front of the icon.
          // if sortoffSet true of undefined, put the default amount. 
          // if sortOffset numeric, offset the number of zwnbsp by that amount.
Earthdawn.constantIcon = function( what, sortOffset ) {   
  'use strict';
  try {
    let c, c0, sort = 0, r = "";
    switch ( Earthdawn.matchString( what )) {    // For this upper half, we have symboles that are usually used in Token Actions. They have to be sorted.
      case "t":
      case "talent":                c = 0x2600; sort = 2;   break;    // Black sun with rays: &# 9728;
      case "karma":                 c = 0x262F; sort = 0;   break;    // yin yang:  &# 9775;
      case "wpn":
      case "weapon":                c = 0x2694; sort = 3;   break;    // Crossed swords:  &# 9876;
      case "power":                 c = 0x26A1; sort = 1;   break;    // Lightning Bolt or High Voltage: &# 9889;
      case "target":                c = 0x27b4; sort = 0;   break;    // Black-feathered South East Arrow
      case "nac":
      case "knack":   c0 = 0xD83D;  c = 0xDCAB; sort = 2;   break;    // Dizzy Symbol: &# 128171;
      case "sk":    case "ska":     case "skk":     case "skl":
      case "skill":   c0 = 0xD83D;  c = 0xDD27; sort = 2;   break;    // Wrench       &# 128295;
      case "sp":    case "spm":
      case "spell":   c0 = 0xD83E;  c = 0xDE84; sort = 3;   break;    // Magic Wand.  &# 129668;
              // This part of the list are icons we return in various chat window messages.
      case "carriagereturn": case "cr":
      case "return":                c = 0x000D;             break;    // Carraige Return;
      case "pointleft":             c = 0x261A;             break;    // Black Left Pointing Index. &# 9754;
      case "warning":               c = 0x26A0;             break;    // Red Exclaimation point in a red triangle.  &# 9888;    This one is never an ability prefix just an icon we sometimes use.
      case "zwnbsp":                c = 0xFEFF;             break;    // Zero Width No Break SPace.   Used to sort thing to the start of the ability list. The one with the most of these will go to the front of the list. 
      case undefined: case "none":   return "";          // undefined is a legal value that returns an empty string.
      default:
        Earthdawn.errorTrace( "Earthdawn.constantIcon: Illegal argument '" + what + "'." );
        return;
    }
    if( sortOffset !== false ) {
      if( typeof( sortOffset ) == "number" )
        sort += sortOffset;
      for( let i = 0; i < sort; ++i )
        r += String.fromCharCode( 0xFEFF );       // zwnbsp - Zero Width Non Breaking SPace.
    }
    if( c0 )
      return r + String.fromCharCode( c0, c );    // This is a UFT-32 character, so needs an extra character to specify the block.
    else if ( c )
      return r + String.fromCharCode( c );        // This is a UFT-16 character.
    else return r;
  } catch(err) { Earthdawn.errorTrace( "Earthdawn:constantIcon( " + what + " ) error caught: " + err ); }
} // end constantIcon()
...
Earthdawn.dispToName = function ( disp, mode ) {
  'use strict';
  try {
    if( !["short","name","weaving"].includes(mode)) { Earthdawn.errorTrace( "Earthdawn:dispToName() unknown mode: " + mode ); return; }
    let t;
    switch( disp.trim() ) {
      case "Elementalism":
      case "Elementalist":
    ...
    return t[ mode ];
  } catch(err) { Earthdawn.errorTrace( "Earthdawn:dispToName() error caught: " + err ); }
};  // end dispToName
...
//      ' ' : '&'+'nbsp'+';',
  ...
  } catch(err) { Earthdawn.errorTrace( "Earthdawn:encode() error caught: " + err ); }
  }()); // end encode()
  ...
  try {
    ...
    if( !context )
      ...
      if( "edClass" in context && "msg" in context.edClass ) {       // This is a parseObj
        ...
        s = context.edClass.traceStack;
        i = context.edClass.traceInfo;
      } else if ( "msg" in context ) {     // This is an edClass object.
        m = context.msg;
        s = context.traceStack;
        i = context.traceInfo;
      } else {
        log( Earthdawn.timeStamp + "Earthdawn.errorLog invalid context." );
        p = "Possible ";
        s = Earthdawn.traceStack;
        i = Earthdawn.traceInfo;
      }

      if( m && !state.Earthdawn.logCommandline )    // If have not already logged the command line, do so.
        log( m );
    }
    log( Earthdawn.timeStamp() + msg);
    if( s || i )
      log( p + (s ? "TraceStack: " + s : "") + (i ? "   TraceInfo: " + i: ""));
      ...
      // The above routine (ErrorLog) can only be called from within EDclass and parseObj classes, they can not be run from Earthdawn context routines.
      // So this one simply prints the last trace information it has. 
      // THIS IS NOT NESSISARILY THE TRACE INFORMATION FOR ANY PARTICULAR OBJECT, IT IS JUST THE LAST TRACE INFORMATION RECIVED. ANOTHER ROUTINE MIGHT HAVE OVERWRITTEN THIS TRACE INFORMATION SINCE THE ROUTINE THAT TRIGGREED THE LOGGING WAS LAUNCHED. THE TRACE INFORMATION MIGHT BE HELPFULL, BUT MIGHT BE WRONG.
...
Earthdawn.fibonacci = function(num, memo) {
  ...
  } catch(err) { Earthdawn.errorTrace( "Earthdawn:fibonacci() error caught: " + err ); }
};  // end fibonacci
...
//log(attrs);
    ...
    if( objs ) {
      if( objs.length > 1 ) {
        Earthdawn.errorTrace( "Error Earthdawn:findOrMakeObj() found multiple objects: " );
        log( objs );
...
Earthdawn.fromSheetworkerToAPI = function ( msg, cID, wherefrom ) {
  ...
  try {
    ...
    let ED = new Earthdawn.EDclass( "fromSheetworkerToAPI", msg ),
        cmdArray = msg.split( "\n" );
    ...
    for( let ind = 0; ind < cmdArray.length; ++ind ) {
      ...
      switch ( cmdLine.slice( 0, comma).trim() ) {
        case "abilityAdd": {    // abilityAdd, (name)\r (text)\r optional symbol code.
          Earthdawn.abilityAdd( cID, Earthdawn.constantIcon( Earthdawn.getParam( dataLine, 3, "\r" ))
        ...
        case "abilityRemove": {   // abilityRemove, name of ability, optional symbol code.    Remove this ability.
          Earthdawn.abilityRemove( cID, Earthdawn.constantIcon( Earthdawn.getParam( dataLine, 2, "," )) + Earthdawn.getParam( dataLine, 1, "," ));
        } break;
...
//          let ED = new Earthdawn.EDclass();
        ...
        case "LinkAdd1": {      //APIflag LinkAdd1,code:RowId:code:name
          ...
          if( dataArray.length !== 4 ) {
            Earthdawn.errorTrace( "Earthdawn - APIFlag call incorrectly formatted for LinkAdd1 : " + dataLine);
            return;
          }
  // See if this works.
  ...
  // See if this works.
        ...
        case "RemoveRow": {   // RemoveRow, (code), (rowID)     // remove everything with this rowID.
          let pre = Earthdawn.buildPre( cID, Earthdawn.getParam( dataLine, 1, ","), Earthdawn.getParam( dataLine, 2, ",")).toLowerCase(),
        ...
        case "SheetUpdate": {
          ...
          if( shouldUpdate( 3.3300 ))
            ED.updateVersion3p330( cID, ED );
            ...
            function tryLink( count ) {
              ...
              if( edParse.TokenFind() && ("tokenInfo" in edParse) && ("tokenObj" in edParse.tokenInfo)) {
                ...
                if( tID ) {
                  ...
                  edParse.LinkToken( [ "forceLink", tID ] );
                }
...
//          edParse.Spell( [ "Spell", dataLine, "WipeMatrix", "M"] );
        ...
        default:
          Earthdawn.errorTrace( "Unknown command in APIflag: " + cmdLine);
      } // end switch cmdLine
    } // end for cmdArray
  } catch(err) { Earthdawn.errorTrace( "Earthdawn:fromSheetworkerToAPI() error caught: " + err ); }
...
Earthdawn.generateRowID = function () {
  ...
  return EarthdawnGenerateUUID().replace(/_/g, "Z");    // I don't want underscores in my RowID!
    ...
    // if toInt is true or 1, convert to int. if toInt is 2, force through safestring.
Earthdawn.getAttrBN = function ( cID, nm, dflt, toInt ) {
  ...
  try {
    let ret,
        best = 0;
    if( !cID ) {
      Earthdawn.errorTrace( "Invalid character_id '" + cID + "' for getAttrBN()   name: " + nm + "   default: " + dflt)
      ret = dflt;
    } else if( !nm ) {
      Earthdawn.errorTrace( "Invalid attribute '" + nm + "' for getAttrBN().   dflt: '" + dflt + "'    cID: " + cID )
      ret = dflt;
    } else {
      ...
      if( ret === undefined ) {     // We want to do this if any of the above returned a ret of undefined.
        ...
        if( attribBN && attribBN.length > 1 ) {
          Earthdawn.errorTrace( "Warning Earthdawn:getAttrBN( " + cID + ", " + nm + ( dflt === undefined ? "": ", " + dflt )
    ...
    if( ret === undefined && dflt != undefined ) ret = dflt;
    return ((toInt === true) || toInt == 1) ? Earthdawn.parseInt2( ret ) : (( toInt == 2) ? Earthdawn.safeString( ret ) : ret);
  } catch(err) {
    Earthdawn.errorTrace( "Earthdawn:getAttrBN() error caught: " + err );
...
Earthdawn.getIcon = function ( mi ) {
  return mi[ "customTag" ] ? mi[ "customTag" ] : mi[ "icon" ]
        ...
        // Find the nth parameter in str. ie: getParam( "11, 22, 33", 2 ) is "22".
        // num is 1 or more. delim defaults to comma. 
Earthdawn.getParam = function ( str, num, delim ) {
  'use strict';
  try {
    if( typeof str !== 'string' ) {
      Earthdawn.errorTrace( "Error getParam argument not string." );
    ...
    return found;
  } catch(err) { Earthdawn.errorTrace( "Earthdawn:getParam() error caught: " + err ); }
}; // end getParam()
...
// 700-Grey, 700-Skyblue, 700-Yellowish, end of list.
    ...
    return txt;
  } catch(err) { Earthdawn.errorTrace( "Earthdawn.getStatusMarkerCollection() error caught: " + err ); }
} // End Earthdawn.getStatusMarkerCollection()
...
Earthdawn.isString = function ( str ) {  return (str != null && typeof str.valueOf() === "string") ? true : false }
        ...
        // keywordCheck( klist, canidate1, canidate2, etc )
        // Look for all the other arguments within the keyword list. 
        // return a bitmap of each keyword found.        if second and third canidates found but first is not, then would return 0x110;   (third, second, first)
        // if any argument is a boolean true, then the next argument should be tested as a "startsWith"
        // Called standalone and via ED.keyword
Earthdawn.keywordCheck = function( klist ) {
  'use strict';
  try {
    ...
    return ret;
  } catch(err) { Earthdawn.errorTrace( "Earthdawn.keywordCheck() error caught: " + err ); }
} // end keywordCheck()
...
Earthdawn.matchString = function( str, allowHyphen, allowComma ) {   // return this string with everything that is not a /W or a hyphen stripped out, and lower cased. Used to test for matches.
  return Earthdawn.safeString( str ).replace(( allowHyphen ? ( allowComma ? /[^\w\,\-]/g : /[^\w\-]/g) : /\W/g ), "").toLowerCase();
    ...
    // for chat menu messages made with htmlBuilder, we need a section (this.newSect) and a body (this routine passed the sect).
Earthdawn.newBody = function ( sectnew ) {
  return sectnew.append( ".body", "", { class:"sheet-rolltemplate-body" });
};
...
Earthdawn.makeButton = function( buttonDisplayTxt, linkText, tipText, colr, noColonFix )  {
  'use strict';
  try {
    return new HtmlBuilder( "a", buttonDisplayTxt, Object.assign( {}, {
      href: noColonFix ? linkText : Earthdawn.encode( Earthdawn.colonFix( linkText )),
      class: "sheet-rolltemplate-chatbutton sheet-rolltemplate-ireallymeanit" + ((colr) ? " sheet-rolltemplate-color-" + colr : "") },
      tipText ? {     // Optional tipText section
        title: Earthdawn.encode( Earthdawn.encode( tipText )) } : {}
      )) + " ";
  } catch(err) { Earthdawn.errorTrace( "Earthdawn.makeButton error caught: " + err ); }
} // end makeButton()
...
/*
    ...
    if( isNaN( x )) {
      if( !silent )
        Earthdawn.errorTrace( "Earthdawn, parseInt2 was passed not a number " + i );
...
*/
  try {
    ...
    Earthdawn.errorTrace( "Earthdawn.parseInt2() error caught: " + err );
    return 0;
  }
} //end parseInt2
...
Earthdawn.pseudoMsg = function( pclass, msg, sel ) {    // msg is the text of the message. sel is a tokenID that should be selected. 
  ...
  } catch(err) { Earthdawn.errorTrace( "Earthdawn.pseudoMsg error caught: " + err ); }
...
Earthdawn.repeatSection = function ( section, str ) {
  ...
  try {
    if( !section ) {
      Earthdawn.errorTrace( "Earthdawn:repeatSection() error, invalid section: " + section + " str : " + str );
      return;
    }
    if( !str ) {
      Earthdawn.errorTrace( "Earthdawn:repeatSection() error, invalid str: " + str + " section : " + section );
      return;
    }
    let x = str.split( "_" );
    ...
    Earthdawn.errorTrace( "Earthdawn:repeatSection() error caught: " + err );
  }
};  // end repeatSection
...
Earthdawn.safeArray = function( arr )  {
  ...
  try {
    ...
    Earthdawn.errorTrace( "Earthdawn:safeString() error caught: " + err );
  }
};  // end safeArray



        // Make sure the returned value is safely a valid RowID, and is present in the database with the associated code. 
        // Pass a Character ID, and the same thing thing as buildPre. Name is usually only used or needed if code does not support _RowID.
Earthdawn.safeRowID = function( cid, code2, rid, special )  {
  'use strict';
  try {

    let r, result = 0, search = (special === "search"),
        code = Earthdawn.safeString( code2 ).trim();
    if( code.startsWith( "repeating_" )) {      // if instead of two seperate items, we were passed a full name, parse it. 
      rid = Earthdawn.repeatSection( 2, code );
      code = Earthdawn.repeatSection( 3, code );
    } else
      rid = Earthdawn.safeString( rid ).trim();

    if( code === rid ) {
      Earthdawn.errorTrace( "API Earthdawn:safeRowID() error. Code and RowID are the same. " + code2 + "   RowID: " + rid );
      return;
    }
    if( !code || !rid ) {
      Earthdawn.errorTrace( "API Earthdawn:safeRowID() error. Missing or invalid parameter: " + code2 + "   RowID: " + rid );
      return;
    }
    if( rid === rid.toLowerCase() ) {      // passed one was all lowercase, use stored.
      search = true;
      Earthdawn.errorTrace( "API Earthdawn:safeRowID() Warning. Rid is lowercase.  Code: " + code2 + "   rid: " + rid );
      result = 0x02;
    }

    let pre = Earthdawn.buildPre( cid, code, rid, "noSafe" );   // noSafe prevents buildPre from calling back to here.
    if( !Earthdawn.testNoRowID( code, true )) {    // see if we can find a rowID in the database.
      r = Earthdawn.getAttrBN( cid, pre + "RowID", "");
      if( r ) {
        if( r != rid ) {
          search = true;    // In this case, no matter what, try search, but if search is unsuccessful, pick a result.
          if ( r === r.toLowerCase() ) {     // stored one was all lowercase, store the passed one and use it.
            Earthdawn.errorTrace( "API Earthdawn:safeRowID() Warning. Stored " + code +"_RowID: '" + r + "' does not match passed value of '" + rid + "' Code: " + code2 + ".  Using passed rid and storing it." );
            result = 0x11;
          } else 
            Earthdawn.errorTrace( "API Earthdawn:safeRowID() Warning. Stored " + code +"_RowID: '" + r + "' does not match passed value of '" + rid + "' Code: " + code2 + "." );
        } else
          result = 0x01     // Everything is fine, and nothing more needs to be done.
      } else {    // There is no stored RowID and there should be. 
        search = true;
        Earthdawn.errorTrace( "API Earthdawn:safeRowID() Warning. no " + code + "_RowID found for rid '" + rid +"'. Creating one.   Code: " + code2 + ".  Using passed RowID and storing it." );
        result = 0x11;
      }
    } else      // This code does not have a stored RowID.
      search = true;

    if( search ) {    // search should be true only if "search" was passed in special, or if this is a code without a RowID, a RowID was not found, or if the RowID does not match what was passed.
                      // search will not be done if everything looks OK up to this point.
                      // go through all attributes for this character and see if we can find any that match except for the case.
//log("search " + code + " " + rid);
      let attributes = findObjs({ _type: "attribute", _characterid: cid }),
          start = "repeating_" + Earthdawn.repeatSection( 1, pre ),
          low = pre.toLowerCase(), 
          found = [], count = [];

      _.each( attributes, function (att) {
        if( att.get( "name" ).startsWith( start ))      // I think it is probably quicker to just look for repeating_xxx before lowercasing everything. Then lowercase for the rowID test.
          if( att.get( "name" ).toLowerCase().startsWith( low )) {
            let f = Earthdawn.repeatSection( 2, att.get( "name" )),
                ind = found.indexOf( f );
            if( ind === -1 ) {
              found.push( f );
              count.push( 1 );
      ...
      if( found.length === 0 ) {
        result = 0x11;
        Earthdawn.errorTrace( "API Earthdawn:safeRowID() Error. Search found no attributes    Code: " + code2 + ".   Rid: '" + rid + "' Using passed RowID and storing it." );
      } else if( found.length === 1 ) {    // We found exactly one RowID actually in use, use it. 
        r = found[ 0 ];
        result = 0x12;
      } else {      // we found more than one. We might be unable to find know which is correct, so find the one with the most fields that is not lowercaed.
        Earthdawn.errorTrace( "API Earthdawn:safeRowID() Error. Search found more than one rowID that matches this.   Code: " + code2 + ".  Using and storing most common one." );
        let mx = 0, ind = -1, t = "Passed was '" + rid + "'   ";
        if( r ) t += code + "_RowID was '" + r + "'.    Found:   ";
        for( let i = 0; i < found.length; ++i ) {
          t += "'" + found[ i ] + "' (" + count[ i ] + ")   ";
          if( count[ i ] > mx && found[ i ] != found[ i ].toLowerCase() ) {     // Find the index with the most instances that is not all lower case.
            mx = count[ i ]
            ind = i;
        } }
        if( ind != -1 ) {   // There is a small possibility that only one is found, and it is lowercased, in which case search finds nothing. Don't know what else could be done.
          r = found[ ind ];
          result = 0x12;
          t += "We will use '" + r + "'";

          _.each( attributes, function (att) {    // Now for each one that does NOT use the prefured rowID, ether rename or delete it.
            if( att.get( "name" ).startsWith( start ))
              if( att.get( "name" ).toLowerCase().startsWith( low )) {
                let f = Earthdawn.repeatSection( 2, att.get( "name" ));
                if( f != r ) {
                  let fix = Earthdawn.buildPre( cID, code, r, "noSafe" ) + Earthdawn.repeatSection( 4, att.get( "name" ));
                  let chk = Earthdawn.getAttrBN( cid, fix, "bad");
                  if( chk === "bad" )
                    Earthdawn.set( att, "name", fix );    // read found nothing, there is not already something with this exact name, so just correct the name
                  else
                    att.remove();   // There is already an attribute of this exact name, so remove the bad one.
              } }
          }); // End for each attribute.
        } 
        log( t );
    ...
    if( result & 0x02 )    // used stored value instead of the passed value. 
      rid = r;
    if( result & 0x10 && !Earthdawn.testNoRowID( code, true ))   // Store the passed or found value into _RowID
      Earthdawn.setWW( Earthdawn.buildPre( cid, code, rid, "noSafe" ) + "RowID", rid, cid );

    return (result ? rid : undefined);
  } catch(err) {
    Earthdawn.errorTrace( "Earthdawn:safeRowID() error caught: " + err );
  }
};  // end safeRowID
...
// log( "set   " + obj.get("name") + "    val " + val);
    if(( val === undefined && dflt != undefined ) || (val !== val)) {   // val !== val is the only way to test for it equaling NaN. Can't use isNan() because many values are not supposed to be numbers. But we do want to test for val having been set to NaN.
      Earthdawn.errorTrace( "Warning!!! Earthdawn:set()   Attempting to set '" + att + "' to " + val + " setting to '" + dflt + "' instead. Object is ...");
      log( obj );
      obj.set( type, (dflt === undefined) ? "" : dflt );
    } else
      obj.set( type, (val === undefined) ? "" : val );
  } catch(err) { Earthdawn.errorTrace( "Earthdawn:set() error caught: " + err ); }
} // end of set()
...
Earthdawn.setWithWorker = function( obj, type, val, dflt )  {   // type is "current" or "max"
  ...
  try {
    if( !obj ) {
      Earthdawn.errorTrace( "Earthdawn:setWithWorker() Error: obj is undefined.   Type: " + type + "   val: " + val + "   dflt: " + dflt );
      return;
    }
// log( "setww " + obj.get("name") + "    val " + val);
    if(( val === undefined && dflt != undefined ) || (val !== val)) {   // val !== val is the only way to test for it equaling NaN. Can't use isNan() because many values are not supposed to be numbers. But we do want to test for val having been set to NaN.
      Earthdawn.errorTrace( "Warning!!! Earthdawn:setWithWorker()   Attempting to set '" + att + "' to " + val + " setting to '" + dflt + "' instead. Object is ...");
      log( obj );
      obj.setWithWorker( type, (dflt === undefined) ? "" : dflt );
    } else
      obj.setWithWorker( type, (val === undefined) ? "" : val );
  } catch(err) { Earthdawn.errorTrace( "Earthdawn:setWithWorker() error caught: " + err ); }
...
Earthdawn.setWW = function( attName, val, cID, dflt, maxVal, maxDflt ) {
  ...
  try {
    if( !cID ) {
      ...
      if( maxVal !== undefined )
        Earthdawn.setWithWorker( aobj, "max", maxVal, maxDflt );
    }
  } catch(err) { Earthdawn.errorTrace( "Earthdawn.SetWW() error caught: " + err ); }
} // End Earthdawn.SetWW()
...
Earthdawn.SetDefaultAttribute = function( cID, attr, dflt, maxdflt )  {
  ...
  } catch(err) { Earthdawn.errorTrace( "Earthdawn:SetDefaultAttribute() error caught: " + err ); }
...
Earthdawn.texttip = function ( txt, tip ) {
      'use strict';
      try {
    return new HtmlBuilder( "span", txt, { class: "sheet-rolltemplate-texttip", title: Earthdawn.encode( Earthdawn.encode( tip )) });
  } catch(err) { Earthdawn.errorTrace( "Earthdawn:texttip() error caught: " + err ); }
}; // end texttip()
...
//log( "refreshing token " + obj.get("name"));
      ...
      if( rep && rep != "" ) {
        ...
        if( ch ) {
          let ED = new Earthdawn.EDclass( "tokenRefresh", ch.get( "_id" ) + "   " + ch.get( "name" ));
          let edParse = new ED.ParseObj( ED );
          edParse.charID = rep;
  ...
  } catch(err) { Earthdawn.errorTrace( "Earthdawn:tokenRefresh() error caught: " + err ); }
} // End tokenRefresh
...
Earthdawn.tokToChar = function ( tokenID ) {
  ...
  } catch(err) { Earthdawn.errorTrace( "Earthdawn:tokToChar() error caught: " + err ); }
}; // end tokToChar()
...
Earthdawn.waitToRemove = function ( cID, nm, timelimit ) {
  ...
  } catch(err) { Earthdawn.errorTrace( "Earthdawn:waitToRemove() error caught: " + err ); }
}; // end waitToRemove()
...
Earthdawn.EDclass = function( traceS, traceI, origMsg ) {
  'use strict';

          // define any class variables
  this.traceStack = traceS;
  this.traceInfo  = traceI;
  this.msg = origMsg;
  ...
  Earthdawn.traceStack = traceS;  // Store a copy of the trace stacks in Earthdawn, which while not attached to any specific EDclass will at least tell us the last one called. 
  Earthdawn.traceInfo  = traceI;
  ...
  this.Ready = function () {
    ...
    if( state.Earthdawn.game            === undefined ) {
      state.Earthdawn.game     = "ED";
      state.Earthdawn.gED      = true;
      state.Earthdawn.edition  = 4;
      state.Earthdawn.g1879    = undefined;
    }
...
//    if( state.Earthdawn.karmaRitual     === undefined )   state.Earthdawn.karmaRitual     = "-1";
    ...
    if( state.Earthdawn.Rolltype        === undefined ) {
      ...
      state.Earthdawn.Rolltype.PC.Exceptions = {"horrormark": {name: "Horror Mark", display: "GM Only"}};   // set with key of Earthdawn.matchString( name ) = { name: name, display: ssa[ 5 ] };
      state.Earthdawn.Rolltype.NPC = {};
      state.Earthdawn.Rolltype.NPC.Default = "GM Only";
      state.Earthdawn.Rolltype.NPC.Exceptions = {"horrormark": {name: "Horror Mark", display: "GM Only"}};
    }
    if( state.Earthdawn.linking         === undefined ) {
      state.Earthdawn.linking = {};
      state.Earthdawn.linking.PC = {};
      state.Earthdawn.linking.NPC = {};
    }
...
// obsolete    state.Earthdawn.rowIDobj = {};      // erase collection of rowIDs waiting to be checked. 
    ...
    if( state.Earthdawn.version != Earthdawn.Version ) {        // This code will be run ONCE when a new API version is loaded. However the update routines below will be run once for each character.
      ...
      if( state.Earthdawn.version < 3.330) {
        let edParse = new this.ParseObj( this );
        edParse.funcMisc( [ "funcMisc", "macroCreate", "refresh" ] );     // This version needs new macros, so refreash them.
        vUpdate( this, this.updateVersion3p330, 3.330 );
      }
    ...
    if( smcText ) {
      ...
      smcText += " For instructions open this link " +
					Earthdawn.makeButton( "Wiki Link", "https://wiki.roll20.net/Earthdawn_-_FASA_Official_V2#Import_the_Custom_Marker_Set",
								"This button will open this character sheets Wiki Documentation, which should answer most of your questions about how to use this sheet.", "dflt", true )
                + " in another tab and go to section 4.1.2.";
    ...
    _.each( chars, function ( charObj ) {
      ...
      if( aObj.get( "max" ) != "1" )
        aObj.setWithWorker( "max", "1" );
          ...
          //
          // Put anything that you want to happen 5 seconds after startup here.
          //
    let edc = this;
    setTimeout(function() {
      try {
...
//          log( "Warning! Earthdawn.js  -> WelcomePackage integration failed!  Error: " + err );
      ...
      try {
        let sectPlayer = new HtmlBuilder( "", "", {class: "sheet-rolltemplate-background-white" });
      } catch(err) {
        log( Earthdawn.timeStamp() + "Error! Earthdawn.js  -> HtmlBuilder integration failed!  Error: " + err );
        log( "   ***** The Earthdawn character sheet compainion API will ***NOT*** work, unless htmlBuilder is also installed! *****" );
              ...
              if( charQueue.length > 0 ) {
                ...
                _.each( attributes, function (att) {
                  if (att.get("name").startsWith( "repeating_message_" )) {
                    let rid = Earthdawn.safeString( Earthdawn.repeatSection( 2, att.get( "name" ))),
                        t;
                    if( rid ) {
                      t = rowIndex.indexOf( rid );    // One-dimensional array of message rowIDs.
                      if( t == -1 ) {
                        t = rowIndex.push( rid ) - 1;
                        rowattribs.push( [] );
                      }
  ...
  //                if( SheetUnproc ) log( SheetUnproc + " messages to the Sheetworker that were unprocessed. ");
      ...
      if( firstRun ) {      // This is the first time the API has been run. 
        let edParse = new edc.ParseObj( edc ),
            charQueue = findObjs({ _type: "character" }),      // create the queue we'll be processing.
            count = 0;
        Earthdawn.pseudoMsg( edc, "dummy" );    // fake a msg with no token selected.
        if( charQueue && charQueue.length > 0) {
          const charBurndown2 = () => {				// function that will process the next element of the queue
            try {
              if( charQueue.length > 0 ) {
                ...
                edParse.Debug( [ "Debug", "repSecFix", "silent", "firstRun", count ? "notFirst" : "first" ] );
                ++count;
                setTimeout( charBurndown2, 1);   // Do the next character
              } else {    // Have finished the last character.
                edc.chat( "We recommend relinking all existing characters using the process described in the WiKi.", Earthdawn.whoFrom.apiWarning | Earthdawn.whoTo.public );
                edc.chat( "Suggest opening "  + Earthdawn.makeButton( "Wiki Link", "https://wiki.roll20.net/Earthdawn_-_FASA_Official_V2"
                        ...
                        , "dflt", true ) + " in another tab and reading it."
                        , Earthdawn.whoFrom.apiWarning | Earthdawn.whoTo.public );
              }
            } catch(err) {
              log( "firstRun error caught: " + err );
          } };
          edc.chat( "This is the first time the API has run in this campaign space.", Earthdawn.whoFrom.apiWarning | Earthdawn.whoTo.public );
          edParse.funcMisc( [ "funcMisc", "macroCreate", "Refresh", "firstRun" ] );
          edc.chat( "Earthdawn Macros created.", Earthdawn.whoTo.public );
          edc.chat( 'Suggest to your players that they put "Roll-Public", "Roll-Player-GM", and "Roll-GM-Only" in their macro bar.', Earthdawn.whoTo.public );
          edc.chat( 'As GM, you should also have "NpcReInit", "ResetChars", and "Dur-Track" in your macro bar.', Earthdawn.whoTo.public );
          edc.chat( "We are now Verifying all preexisting characters.", Earthdawn.whoFrom.apiWarning | Earthdawn.whoTo.public );
          edc.chat( charQueue.length + " characters detected.", Earthdawn.whoFrom.apiWarning | Earthdawn.whoTo.public );
          charBurndown2();   // start the execution by doing the first element. Each element will call the next.
      } }   // end firstRun
      else {
        let edParse = new edc.ParseObj( edc );
          edParse.funcMisc( [ "funcMisc", "macroCreate", "AutoCreate" ] );
      }
      ...
      if ( a )
        ...
        if( e !== -1 ) {
          let act = Earthdawn.safeString( a[i].get("action") ).trim(),
            symbol;
          if(      act.endsWith( "willforce:t" ))   setNameSafe( a[i], "ability", name, Earthdawn.constantIcon( "karma" ) + "WillFrc-T" );
          else if( act.endsWith( "_T_Roll}" ))      symbol = Earthdawn.constantIcon( "talent" );
          else if( act.endsWith( "_NAC_Roll}" ))    symbol = Earthdawn.constantIcon( "knack" );
          else if( act.endsWith( "_SK_Roll}" ))     symbol = Earthdawn.constantIcon( "skill" );
          else if( act.endsWith( "_WPN_Roll}" ))    symbol = Earthdawn.constantIcon( "weapon" );
          else if( act.endsWith( "Target: Set" ))   symbol = Earthdawn.constantIcon( "target" );
          else if( act.endsWith( "TargetsClear" ))  symbol = Earthdawn.constantIcon( "target" );
          else if( act.endsWith( "Grimoire" ))      symbol = Earthdawn.constantIcon( "spell" );
          else if( act.endsWith( "Spells" ))        symbol = Earthdawn.constantIcon( "spell" );
          else if (name.match( /[A-Z][a-z]+-\d '/)) symbol = Earthdawn.constantIcon( "spell" );
          if( symbol )
    ...
    try {
      if( charCount === 0 ) {     // If this is true then this is being called because a new API version has been detected (as opposed an old character sheet being imported) and this is the very first character. So do this once when new API is detected.
        ...
        _.each( macs, function (macObj) {
          ...
          if( n.startsWith( Earthdawn.constantIcon( "Target" )) && n.endsWith( "r-Targets" ))
            macObj.remove();
        });
      ...
      _.each( attributes, function (att) {
        if ( att.get( "name" ).startsWith( "repeating_")) {
          ...
          if ( nm.endsWith( "_CombatSlot") || (nm.endsWith( "_Contains") && (Earthdawn.repeatSection( 3, nm) === "SPM" ))) {
            let nmn,
              rid = Earthdawn.repeatSection( 2, nm),
            ...
            if ( code !== "SP" ) {    // skip if it is SP, we don't do those token actions.
              nmn = Earthdawn.getAttrBN( cID, Earthdawn.buildPre( cID, code, rid ) + lu, "" );
              Earthdawn.abilityRemove( cID, symbol + nmn );
              if( cbs == "1" )
                Earthdawn.abilityAdd( cID, symbol + nmn, "!edToken~ %{selected|" + Earthdawn.buildPre( cID, code, rid ) + "Roll}" );
            }
          } // End Token Action maint.
        }
      }); // End for each attribute.
  ...
  this.updateVersion3p000 = function( cID, ed, charCount ) {
    ...
    try {
      ...
      return count;
        ...
        // Note, before vUpdate is called, we will have already run macroCreate - Refreash
  this.updateVersion3p330 = function( cID, ed, charCount ) {
    'use strict';
    let edc = this, // Debug: RepSecFix: forceNameRefresh
        count = 0;
    try {
        if( ed.msg === undefined )    // fake up a playerID.
          Earthdawn.pseudoMsg( ed );
    		let edp = new ed.ParseObj( ed );
        edp.charID = cID;
      count += edp.abilityRebuild( [ "abilityRebuild", "batchFix", "forceNameRefresh" ]);
      return count;
    } catch(err) { Earthdawn.errorLog( "ED.updateVersion3p330() cID=" + cID + "   error caught: " + err, edc ); }
  }; // end updateVersion3p330()
...
//
  ...
  var callbackWelcomePackage = function( character ) {
    'use strict';
    let ED = new Earthdawn.EDclass( "callbackWelcomePackage", character );
...
//    this.SWflag = "";               // This holds commands that will later be sent to the sheetworker. Newline delimited list of command lines that start with a command and a comma.
    ...
    this.rollQueue = [];            // Array to hold information on each roll the system wants to make when multiple rolls are launched at the same time. Right now it only holds this.misc in the state wanted.
    ...
    this.misc = {};                 // An object to store miscellaneous values that don't really need a dedicated space on this top level. It starts out empty, but many routines store various stuff here.
            ...
            // targetNum:     If this action involves a target number, it is stored here.
...
*/
    ...
    this.toSheet = function( cmd, line ) {
      try {
        if( this.charID ) {
          let t = cmd + "," + line,
              pre = this.buildPre( "MSG", Earthdawn.generateRowID() );
        ...
        switch ( ssa[ 1 ] ) {
          ...
          case "nac": {
            ...
            if(Earthdawn.getAttrBN( this.charID, pre + "Notes", "")!=="")
              this.misc[ "endNote" ] = "<b>Description</b>  "+ Earthdawn.texttip("(Hover)",Earthdawn.getAttrBN( this.charID, pre + "Notes", "").replace( /\n/g, Earthdawn.constantIcon( "cr" )));
            if( modtype != "(0)" && modtype != "NoRoll" )
              this.doLater += "~Karma: " + pre + "Karma";
            this.strainAdd( pre );     // This is the strain for the base Talent.
            if( modtype === "Attack" || modtype === "Attack CC" ) {
              let aa = this.getValue( "combatOption-AggressiveAttack");
              if( aa != 0 )
                this.strainAdd( "Aggressive Attack", aa * Earthdawn.getAttrBN( this.charID, "Misc-AggStance-Strain", "1", true));
              let cs = this.getValue( "combatOption-CalledShot");
              if( cs != 0 )
                this.strainAdd( "Called Shot", cs);
            }
            if( Earthdawn.getAttrBN( this.charID, pre + "Action", "Standard") === "Standard" ) {
              let sm = this.getValue( "combatOption-SplitMovement");
              if( sm != 0 )
                this.strainAdd( "Split Movement", sm );
            }
            let special = Earthdawn.getAttrBN( this.charID, pre + "Special", "None");
            if( Earthdawn.matchString( special ) !== "none" && Earthdawn.matchString( special ) !== "" )
              this.misc[ "Special" ] = special;
            kw = Earthdawn.keywordCheck( special + (("keyword" in this.misc) ? "," + this.misc[ "keyword" ] + "," : ""), "Recovery", "Recovery-Woodskin", "Initiative", "Knockdown", "Recovery-Stun" );
            if( kw & 0x02 )
              this.misc[ "Recovery-WoodSkin" ] = true;
            if( (kw & 0x01) || (kw & 0x02) || (kw & 0x10)) {
            ...
            if(( kw & 0x01) | ( kw & 0x02))           this.misc[ "headcolor" ] = "recovery";    // Recovery or Recovery-Woodskin
            else if( kw & 0x04 )                      this.misc[ "headcolor" ] = "initrep";     // Initiative
            else if( kw & 0x08 )                      this.misc[ "headcolor" ] = "knockdown";   // Knockdown
            else if( modtype === "Action" )           this.misc[ "headcolor" ] = "action";
            else if( modtype === "Effect" )           this.misc[ "headcolor" ] = "effect";
            else if( modtype === "Attack" )           this.misc[ "headcolor" ] = "attack";
            else if( modtype === "Attack CC" )        this.misc[ "headcolor" ] = "attackcc";
            else if( modtype === "Damage" )           this.misc[ "headcolor" ] = "damage";
            else if( modtype === "Damage Poison" )    this.misc[ "headcolor" ] = "damagepoison";
            else if( modtype === "Damage CC" )        this.misc[ "headcolor" ] = "damagecc";
            else if( modtype === "JumpUp" )           this.misc[ "headcolor" ] = "jumpup";
            else                                      this.misc[ "headcolor" ] = "none";
            ...
            let kwc = this.keyword( pre, "Movebased", "Notvisionbased" );
...
/*
            if(( kwc & 0x02 ) || ( Earthdawn.getAttrBN( this.charID, pre + "VisionBased", "0" ) == "1" )) {    // VisionBased depreciated 8/22
...
*/
        ...
        this.misc[ "effRank" ] = this.getValue( pre + stepAttrib);
        this.misc[ "step" ] = (this.misc[ "step" ] || 0) + step + modtypevalue + this.misc[ "effRank" ] + this.getValue( pre + "Mods" ) - this.mookWounds();
//log("Step Calculation " + " thistep " +(this.misc[ "step" ] || 0) + " step " +step + " modtype " +modtypevalue + " rank " +this.getValue( pre + stepAttrib) + " mods " +this.getValue( pre + "Mods" ));
        ...
        //  Standard: This can be passed, but we don't ever use it, it is just the opposite of forceNameRefresh.   On standard we only refresh abilities is the ability text is changed, a different name does not matter.
        //  forceNameRefresh: We will refresh an ability if ether the name or text is different. 
    this.abilityRebuild = function ( ssa ) {
      ...
      try {
        let cID = this.charID,
            ...
            refreshNames = ssa.includes( "refreshNames" ),    // force a refresh of the ability names (otherwise it does not matter if the names are different).
            silent = ssa.includes( "silent" ),
//            standard = ssa.includes( "Standard" ),    // Present if forceNameRefresh is not true, but no need to use it.   In Standard it only updates if the ability text changes
            forceNameRefresh = ssa.includes( "forceNameRefresh" ),          // Also updates if the ability name has changed.
        ...
        function buildTA( what) {   // go through and get information needed to figure out what the token actions should be.
          ...
          function addit( nm, ab ) {    // Make this token action.
            ...
            if( what === -1 )     // If fixing all, then just recreate all candidates.
              ...
              newab.push( { name: nm ,action: ab });
                  ...
                  // first look at the existing Token Actions.  Remove them or put them into oldab.
          _.each( findObjs({ _type: "ability", _characterid: cID}), function( abObj ) {
            let act = abObj.get( "action" ),
                nm  = abObj.get( "name" );
            if( act.startsWith( "!Earthdawn" ) || act.startsWith( "!edToken" ) || act.startsWith( "!edsdr" ) || act.startsWith( "!edInit" ) )
              if( what === -1 || nm === "Untitled" )   // If fixing everything, just get rid of all existing token actions and build from scratch.
                abObj.remove();
              else
                oldab.push({ id: abObj.get( "_id" ), name: nm, action: act });
          });

                  // second, look at all talents, knacks, etc. that ought to have a token action.  Put them into candidate or one of the specialty lists.
          _.each( findObjs({ _type: "attribute", _characterid: cID }), function( att ) {
            ...
            if( sa.startsWith( "repeating_" )) {      // This loop only deals with repitems
              if( sa.endsWith( "T_Special" ) && Earthdawn.keywordCheck( att.get( "current" ), "CorruptKarma" )) {
                addit( Earthdawn.constantIcon( "Target" ) + "Activate-Corrupt-Karma",
              ...
              // Now we actually process the candidates, using the list of SPM rowid's and the spell contains list we got in the previous loop.  Among other things this builds newab
          _.each( candidate, function( att ) {
            let sa = att.get( "name" ),
                code = Earthdawn.repeatSection( 3, sa);
            if( getAttrByName( cID, po.buildPre( sa ) + "CombatSlot" ) == "1" ) {
              if( code === "SP" ) {
                ...
                if( spmID && spmID.length > 1 ) {
                  ...
                  if( cont && cont.length > 1 )
                    addit( Earthdawn.constantIcon( "Spell" ) + cont, "!edToken~ %{selected|" + po.buildPre("SPM", spmID) + "Roll}" );
                  else
                    Earthdawn.errorLog( "abilityRebuild:BuildTA: " + sa + " could not find out what: " + spmID + " contains.", po );
                }
              } else            // All except SP/SPM
                addit( Earthdawn.constantIcon( code ) + att.get("current"), "!edToken~ %{selected|" + po.buildPre( code, Earthdawn.repeatSection( 2, sa ) ) + "Roll}" );
            }
          });

                //Attack, Charge, Ambush, Grimoire, Spell, and Quester DP.
          if( Earthdawn.getAttrBN( cID, "NPC", "1", true) > 0 && Earthdawn.getAttrBN( cID, "Attack-Rank", 0) != 0)     // NPC or Mook and have a generic attack value.
            addit( Earthdawn.constantIcon( "power" ) + "Attack", "!edToken~ %{selected|Attack}");
          if( Earthdawn.getAttrBN( cID, "Creature-DivingCharging_max", "0" ) != "0" )
            addit( Earthdawn.constantIcon( "power" ) + "Charge",  "!edToken~ ForEach~ marker: divingcharging: t")
          if( Earthdawn.getAttrBN( cID, "Creature-Ambushing_max", "0" ) != "0" )
            addit( Earthdawn.constantIcon( "power" ) + "Ambush",  "!edToken~ ForEach~ marker: ambushing: t");
          if( Earthdawn.getAttrBN( cID, "Hide-Spells-Tab", "1" ) != "1" ) {
            addit( Earthdawn.constantIcon( "Spell" ) + "-Grimoire",  "!edToken~ ChatMenu: Grimoire");
            addit( Earthdawn.constantIcon( "Spell" ) + "-Spells",  "!edToken~ ChatMenu: Spells");
          }
          if( Earthdawn.getAttrBN( cID, "Questor", "None" ) != "None" ) {
            addit( Earthdawn.constantIcon( "karma" ) + "DP-Roll", "!edToken~ %{selected|DevotionOnly}" );
            addit( Earthdawn.constantIcon( "karma" ) + "DP-T", "!edToken~ !Earthdawn~ ForEach~ marker: devpnt: t" );
          }

                // We now have a list of old abilities and new abilities. compare them.
                // For each thing that is not up to date, give button to fix just that.
          let oldmap, newmap;
          if( forceNameRefresh ) {     // oldmap and newmap are the names and the actions joined. If ether are different it will trigger a rebuild.
            oldmap = _.map( oldab, function( o ) { return o.name + Earthdawn.constantAlt( "pipealt" ) + o.action; });
            newmap = _.map( newab, function( o ) { return o.name + Earthdawn.constantAlt( "pipealt" ) + o.action; });
          } else {                    // oldmap and newmap are JUST the actions, not the id or name. We want to test only the actions and don't care if the names are different.
            oldmap = _.map( oldab, function( o ) { return o.action; });
            newmap = _.map( newab, function( o ) { return o.action; });
          }

          let extra = _.difference( oldmap, newmap );       // For each entry in extra (which is a list of things to be removed), find the record in oldab (which has the ID, which extra does not) and remove the ability.
          if( extra.length > 0 ) {
            txt += "Unknown Token Action" + ((extra.length > 1) ? "s" : "") + " found. " + ((what == 1) ? "Do you want to remove: " : "Removing: " );
            _.each( extra, function( a ) {
              ++problems;
              let old = _.filter( oldab, function ( ab ) { return (forceNameRefresh ? ab.name + Earthdawn.constantAlt( "pipealt" ) + ab.action : ab.action) === a});
              _.each( old, function( b ) {
                if( what == 1 )   // menu entry asking if want to remove it. 
                  txt += " " + Earthdawn.makeButton( b.name, "!Earthdawn~ charID: " + po.charID + "~ abilityRebuild: removeOne: " + b.id
                        , "This will remove the named Token Action.", "param" );
                else if( what == -1 ) {   // automatically remove it. 
                  txt += " " +b.name;
                  abiltiyRebuild( "removeOne", b.id );
                }
              });
            });
          ...
          let build = _.difference( newmap, oldmap );     // We want to know newab that are not in oldab (probably need to be created)
          if( build.length > 0 ) {
            txt += "Missing Token Action" + ((build.length > 1) ? "s" : "") + ". " + ((what == 1) ? "Do you want to create: " : "Creating: " );
            _.each( build, function( a ) {
              ++problems;
              let nw = _.filter( newab, function ( ab ) { return (forceNameRefresh ? ab.name + Earthdawn.constantAlt( "pipealt" ) + ab.action : ab.action) === a});
              _.each( nw, function( b ) {
                if( what == 1 )   // menu entry asking if want to create it. 
                  txt += " " + Earthdawn.makeButton( b.name, "!Earthdawn~ charID: " + po.charID + "~ abilityRebuild: buildOne: "
                        + b.name + ": " + Earthdawn.tildiFix( b.action )
                        , "This will create a Token Action for the named ability.", "param" );
                else if( what == -1 ) {   // automatically build it. 
                  txt += " " + b.name;
                  abiltiyRebuild( "buildOne", b.id, Earthdawn.tildiFix( b.action ) );
                }
              });
            });
            if( txt.length > oldTlength ) {
          ...
          _.each( group, function( act ) {
            if( act.length > 1 ) {    // duplicates
              ...
              let old = _.filter( oldab, function ( ab ) { return (forceNameRefresh ? ab.name + Earthdawn.constantAlt( "pipealt" ) + ab.action : ab.action) === act[ 0 ]});
        ...
        if( newChar )       // This is a brand new character, just created from roll20 or a compendium drop. 
          ...
          if( problems > 0 )      // This block just writes a chat window message and a button warning that there is a problem.
            this.chat( problems + " token actions were not as expected.   "
                + Earthdawn.makeButton( "Menu", "!Earthdawn~ charID: " + this.charID + "~ abilityRebuild: Menu : " + (forceNameRefresh ? "forceNameRefresh" : "Standard")
                , "This will present a menu of options on how to deal with this.", "param" ) + " to fix."
            ...
            case "menu":    // Present a menu asking what to do about it.
              ...
              if( problems > 0 )
                this.chat( Earthdawn.makeButton( "Fix All", "!Earthdawn~ charID: " + this.charID + "~ abilityRebuild: fixAll",
                        "This will delete all Token Actions for this character that use this API module and recreate them correctly.", "param" ) + "<br>"
      ...
      try {
        ...
        if( kstep > 0 ) {       // do we really have a bonus dice?
          ...
          this.misc[ "bonusDice" ] = ( ("bonusDice" in this.misc) ? this.misc[ "bonusDice" ] : "" ) + "+" + t;
            ...
            // wrapper for Earthdawn.buildPre
    this.buildPre = function( code, rid, special ) {
      'use strict';
      try {
        return Earthdawn.buildPre( this.charID, code, rid, special )
      } catch(err) { Earthdawn.errorLog( "ED.buildPre() error caught: " + err, this ); }
    } // End ParseObj.buildPre )




          // ParseObj.CalculateStep()
          // This subroutine has a purpose similar to Lookup, but the processing needed is too complex to be passed on the command line.
    ...
    this.chat = function ( newMsg, iFlags, customFrom ) {
      ...
      this.edClass.chat( newMsg, iFlags, customFrom, this );
          ...
          // oppmnvr, RolltypeEdit, RolltypeMulti, skills, spells, stateEdit, status, strainChange, talents.
      ...
      try {
        ...
        function addSheetButtonCall( label, item, tip ) {
          s += Earthdawn.makeButton( label,
              ...
              + Earthdawn.constantButton( "pipe" ) + Earthdawn.constantButton( "at" ) + Earthdawn.constantButton( "braceOpen" ) + "selected"
              + Earthdawn.constantButton( "pipe" ) + item + Earthdawn.constantButton( "braceClose" ) + "}", tip, "dflt" );
        }
        function addButtonWithCharID( label, item, tip, color) {
          s += Earthdawn.makeButton( label, "!Earthdawn~ charID: " + edParse.charID + "~ " + item, tip, color);
        }
        ...
        switch( Earthdawn.safeString( ssa[ 1 ] ).toLowerCase()) {
          case "attrib": {
            ...
            s += Earthdawn.makeButton( buildLabel( "Dex" ),
                  "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{selected|Dex-Check}", "Dex Action test", "action" );
            s += Earthdawn.makeButton( buildLabel( "FX", "Dex-Effect" ),
                  "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{selected|Dex-Effect-Check}", "Dex Effect test", "effect" );
            s += Earthdawn.makeButton( buildLabel( "Str" ),
                  "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{selected|Str-Check}", "Str Action test", "action" );
            s += Earthdawn.makeButton( buildLabel( "FX", "Str-Effect" ),
                  "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{selected|Str-Effect-Check}", "Str Effect test", "effect" );
            s += Earthdawn.makeButton( buildLabel( "Tou" ),
                  "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{selected|Tou-Check}", "Toughness Action test", "action" );
            s += Earthdawn.makeButton( buildLabel( "FX", "Tou-Effect" ),
                  "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{selected|Tou-Effect-Check}", "Toughness Effect test", "effect" );
            s += Earthdawn.makeButton( buildLabel( "Per" ),
                  "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{selected|Per-Check}", "Per Action test", "action" );
            s += Earthdawn.makeButton( buildLabel( "FX", "Per-Effect" ),
                  "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{selected|Per-Effect-Check}", "Per Effect test", "effect" );
            s += Earthdawn.makeButton( buildLabel( "Wil" ),
                  "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{selected|Wil-Check}", "Will Action test", "action" );
            s += Earthdawn.makeButton( buildLabel( "FX", "Wil-Effect" ),
                  "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{selected|Wil-Effect-Check}", "Will Effect test", "effect" );
            s += Earthdawn.makeButton( buildLabel( "Cha" ),
                  "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{selected|Cha-Check}", "Charisma Action test", "action" );
            s += Earthdawn.makeButton( buildLabel( "FX", "Cha-Effect" ),
                  "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{selected|Cha-Effect-Check}", "Charisma Effect test", "effect" );
            s += nl;
            s += Earthdawn.makeButton( buildLabel( "Init", "Initiative"), "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{selected|Dex-Initiative-Check}",
                  "Roll Initiative for this character.", "effect" );
            s += Earthdawn.makeButton( "JumpUp", "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{selected|JumpUp}",
                  "Make a jump-up test. 2 strain, Target #6", "action" );
            let x;
            if( id )
              x = this.getValue("Knockdown", id) + this.getValue("Adjust-Effect-Tests-Total", id);

            s += Earthdawn.makeButton( "Knockdown" + (x ? " " + x.toString() : ""),
                  "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{selected|Knockdown}", "Make a standard Knockdown test.", "action" );

            s += Earthdawn.makeButton( buildLabel( "Recovery", "Recovery-Step" ),
                  "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{selected|Recovery}", "Make a Recovery test for this character.", "damage" );
            if( id )
              x = this.getValue("Recovery-Step", id) + this.getValue("Wil", id);
            s += Earthdawn.makeButton( "Recov Stun" + (x ? " " + x.toString() : ""),
                  "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{selected|RecoveryStun}", "Make a Stun Damage only Recovery test for this character.", "damage" );
            s += Earthdawn.makeButton( "New Day", "!Earthdawn~ charID: " + Earthdawn.constantButton( "at" ) + "{selected|character_id}~ ForEach: c~ Misc: NewDay",
                  "Set characters recovery tests used to zero and refill karma to max", "damage" );
            if( state.Earthdawn.g1879 ) {
              s += Earthdawn.makeButton( buildLabel( "Language Speak", "Speak-Step" ),
                    "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{selected|Speak-Roll}", null, "action" );
              s += Earthdawn.makeButton( buildLabel( "Language Read/Write", "ReadWrite-Step" ),
                    "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{selected|ReadWrite-Roll}", null, "action" );
            }
            if( id && state.Earthdawn.gED ) {     // go through all attributes for this character and look for ones we are interested in
              ...
              _.each( attributes, function (att) {
                if (att.get("name").endsWith( "_DSP_Name" ))
                  s += Earthdawn.makeButton( att.get("current") + " " + edParse.getValue( edParse.buildPre( "DSP", att.get("name") ) + "Circle", id),
                        "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{" + id + "|" + edParse.buildPre( "DSP", att.get("name") ) + "halfMagic}",
                        "Half Magic test.", "action" );
              }); // End for each attribute.
            }
            this.chat( s.trim(), Earthdawn.whoTo.player | Earthdawn.whoFrom.api | Earthdawn.whoFrom.noArchive, "Attributes" );
            if( tid ) edParse.charID = tid;   // If charID was set before we got here, put it back.
          } break;  // end attrib
          case "damage": {
            ...
            s += Earthdawn.makeButton( "Other", "!Earthdawn~ foreach: st~ Damage: ?{Damage)|1}: ?{Type of damage|Damage|Stun|Strain} : ?{Armor| None,NA| Physical,PA| Mystic,MA| Nat PA,PA-Nat| Nat MA,MA-Nat}",
                  "The Selected Token(s) take damage, which might be of type Stun, Strain, or Normal. Armors may be applied.", "damage" );
            s += Earthdawn.makeButton( "1 Strain", "!Earthdawn~ foreach: st ~ Damage: Strain: 1: Verbose",
                  "The Selected Token(s) take 1 strain.", "damage" );
            ...
            s += Earthdawn.makeButton( "Other", "!Earthdawn~ setToken: @{target|token_id}~ Damage: ?{Damage)|1}: ?{Type of damage|Damage|Stun|Strain} : ?{Armor| None,NA| Physical,PA| Mystic,MA| Nat PA,PA-Nat| Nat MA,MA-Nat }",
                  "Give damage to a Target token, which might be of type Stun, Strain, or Normal. Armors may be applied.", "damage" );
            s += "<br>Roll Dmg - ";
            s += Earthdawn.makeButton( "Fire", "!Earthdawn~ Quick: Fire: ?{Size of fire|Torch,Torch: 4|Small Campfire,Small Campfire: 6"
                  ...
                  "The Selected Token(s) take Fire damage. Physical Armor applies.", "damage" );
            s += Earthdawn.makeButton( "Fall", "!Earthdawn~ Quick: Fall: "
                  ...
                  "The Selected Token(s) take Falling damage. No Armor applies.", "damage" );
            let tid = edParse.charID;
            lst = this.getUniqueChars( 1 );
            if ( Earthdawn.safeArray( lst).length === 1 )
              for( let k in lst )
...
//                   if( Earthdawn.getAttrBN(id, att.get( "name" ).slice( 0, -5) + "_CombatSlot", "0") != "1")
                    t += Earthdawn.makeButton( att.get( "current" ),
                        "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{" + id + "|" + Earthdawn.safeString( att.get( "name" )).slice(0, -4) + "Roll}",
                        "Roll a Weapon Damage.", "damage" );
                  if( att.get( "name" ).endsWith( "_Mod-Type" ) && att.get( "current" ).startsWith( "Damage" )){
                    let pre = edParse.buildPre( att.get( "name" )),
                    name = Earthdawn.getAttrBN( id, pre + "Name", "" )
                  t1 += Earthdawn.makeButton( name, "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{" + id + "|" + pre + "Roll}", "Roll an Ability Damage.", "damage" );}
              }); // End for each attribute.
              if( t.length > 1 )
          ...
          case "editspell2": {
            let nm = edParse.buildPre( "SP", ssa[ 2 ]);
            switch ( ssa[ 3 ] ) {
              case "Dur": {
                let dur = "";
                if( ssa[ 4 ] != "Nothing" )
              ...
              case "MenuAddExtraThread": {
                addButtonWithCharID( "ET Add Target", "chatMenu: editSpell2: " + ssa[ 2 ] + ": AddExtraThread: Add Tgt (+?{How many Additional Targets or Additional Effects|Rank})", null, "param" );
                addButtonWithCharID( "ET Inc Area", "chatMenu: editSpell2: " + ssa[ 2 ] + ": AddExtraThread: Inc Area (?{How much increased area|2 Yards})", null, "param" );
                addButtonWithCharID( "ET Inc Dur (minutes)", "chatMenu: editSpell2: " + ssa[ 2 ] + ": AddExtraThread: Inc Dur (min)", null, "param" );
                addButtonWithCharID( "ET Inc Dur", "chatMenu: editSpell2: " + ssa[ 2 ] + ": AddExtraThread: Inc Dur (?{How many units})", null, "param" );
                addButtonWithCharID( "ET Inc Effect Step", "chatMenu: editSpell2: " + ssa[ 2 ] + ": AddExtraThread: Inc Efct Step (?{How many units|2})", null, "param" );
                addButtonWithCharID( "ET Inc Effect (other)", "chatMenu: editSpell2: " + ssa[ 2 ] + ": AddExtraThread: Inc Efct (Other) (?{How many units|2})", null, "param" );
                addButtonWithCharID( "ET Inc Range", "chatMenu: editSpell2: " + ssa[ 2 ] + ": AddExtraThread: Inc Rng (?{How many units|10 yards})", null, "param" );
                addButtonWithCharID( "ET Special", "chatMenu: editSpell2: " + ssa[ 2 ] + ": AddExtraThread: Special (?{What is the text of the Extra Thread Effect})", null, "param" );
                addButtonWithCharID( "ET Remove Targets", "chatMenu: editSpell2: " + ssa[ 2 ] + ": AddExtraThread: Rmv Tgt (?{How many targets to remove|-Rank})", null, "param" );
                addButtonWithCharID( "ET Not Applicable", "chatMenu: editSpell2: " + ssa[ 2 ] + ": AddExtraThread: NA", null, "param" );
              ...
              case "MenuRemoveExtraThread": {
                ...
                for( let i = 0; i < aet.length; ++i )
                  if( aet[ i ].trim().length > 0 )
                    addButtonWithCharID( "Remove " + aet[ i ].trim(), "chatMenu: editSpell2: " + ssa[ 2 ] + ": RemoveExtraThread: " + aet[ i ].trim(), null, "param2" );
            ...
            if( s.length > 1 )
              this.chat( s.trim(), Earthdawn.whoTo.player | Earthdawn.whoFrom.api | Earthdawn.whoFrom.noArchive, "Edit Spell: "
                  + Earthdawn.getAttrBN( this.charID, edParse.buildPre( "SP", ssa[ 2 ] ) + "Name", "" ));
          ...
          case "fxset": {   // chatMenu: fxSet: (subMenu): (code): (rowID): [additional parameters already specified in previous menus]
            ...
            let pre = edParse.buildPre( ssa[ 3 ], ssa[ 4 ]),
              submenu = ssa[ 2 ],
            ...
            switch (submenu) {
              case "0":   // set or Clear
                addButtonWithCharID( "Set FX",   already + ": Set", "  This sets a new Special Effect, replacing any old FX.", "param" );
                addButtonWithCharID( "Clear FX", "fxSet: " + ssa[ 3 ] + ": " + ssa[ 4 ] + ": Clear", "  This removes any Special Effects for this ability.", "param2" );
                break;
              case "1":     // When does this FX display.
                s += "  When should this Effect be displayed?  Every time the ability is ";
                addButtonWithCharID( "Attempted", already + ": Attempt", "  Effect will be displayed every time the ability is attempted, whether it succeeds or not.", "param" );
                s += " or only when it ";
                addButtonWithCharID( "Succeeds",  already + ": Success", "  Effect will be displayed only when the ability succeeds.", "param" );
                break;
              case "2":
                s += "  What type of effect? ";
                addButtonWithCharID( "Beam",     already + ": Beam",     "  A beam effect (and other buttons of this color) travels from the caster to one or more targets.", "param2" );
                addButtonWithCharID( "Bomb",     already + ": Bomb",     "  A bomb effect (and other buttons of this color) takes place ether at the caster location, or the target locations.", "param" );
                addButtonWithCharID( "Breath",   already + ": Breath",   "  A breath effect (and other buttons of this color) travels from the caster to one or more targets.", "param2" );
                addButtonWithCharID( "Bubbling", already + ": Bubbling", "  A bubbling effect (and other buttons of this color) takes place ether at the caster location, or the target locations.", "param" );
                addButtonWithCharID( "Burn",     already + ": Burn",     "  A burn effect (and other buttons of this color) takes place ether at the caster location, or the target locations.", "param" );
                addButtonWithCharID( "Burst",    already + ": Burst",    "  A burst effect (and other buttons of this color) takes place ether at the caster location, or the target locations.", "param" );
                addButtonWithCharID( "Explode",  already + ": Explode",  "  An explode effect (and other buttons of this color) takes place ether at the caster location, or the target locations.", "param" );
                addButtonWithCharID( "Glow",     already + ": Glow",     "  A glow effect (and other buttons of this color) takes place ether at the caster location, or the target locations.", "param" );
                addButtonWithCharID( "Missile",  already + ": Missile",  "  An missile effect (and other buttons of this color) takes place ether at the caster location, or the target locations.", "param" );
                addButtonWithCharID( "Nova",     already + ": Nova",     "  A nova effect (and other buttons of this color) takes place ether at the caster location, or the target locations.", "param" );
                addButtonWithCharID( "Splatter", already + ": Splatter", "  A splatter effect (and other buttons of this color) travels from the caster to one or more targets.", "param2" );
                addButtonWithCharID( "Other/Custom", already + ": Custom ?{Name of Custom FX}",
                    ...
                    + "'[New Custom FX]' option under the '--Custom FX--.  Read the help wiki (above) for additional instructions.' header.", "param2" );
                s += "   For more information "
                    + Earthdawn.makeButton( "FX Help Wiki", "https://help.roll20.net/hc/en-us/articles/360037258714-F-X-Tool#F/XTool-CustomFXTool",
                    "This button will open Roll20's help page for custom Special Effects.   Important! Open this in a new tab, not this tab.", "dflt", true );
              break;
              case "3":   // Color.   Also, if last one was a Custom Effect, verify that it is Good.
                if( Earthdawn.safeString( ssa[ 7 ]).startsWith( "Custom" ) ) {
                  ...
                  addButtonWithCharID( "Acid",  already + ": Acid",  "  Acid effects are a dark green.", "param" );
                  addButtonWithCharID( "Blood", already + ": Blood", "  Blood effects are crimson.", "param" );
                  addButtonWithCharID( "Charm", already + ": Charm", "  Charm effects are a light purple.", "param" );
                  addButtonWithCharID( "Death", already + ": Death", "  Death effects are black.", "param" );
                  addButtonWithCharID( "Fire",  already + ": Fire",  "  Fire effects are a mix of red and orange.", "param" );
                  addButtonWithCharID( "Frost", already + ": Frost", "  Frost effects are a mix of white and blue.", "param" );
                  addButtonWithCharID( "Holy",  already + ": Holy",  "  Holy effects are a light yellow.", "param" );
                  addButtonWithCharID( "Magic", already + ": Magic", "  Magic effects are a mix of many differnt colors with black and blue predominating.", "param" );
                  addButtonWithCharID( "Slime", already + ": Slime", "  Slime effects are a sickly green.", "param" );
                  addButtonWithCharID( "Smoke", already + ": Smoke", "  Smoke effects are white.", "param" );
                  addButtonWithCharID( "Water", already + ": Water", "  Water effects are sea blue.", "param" );
                  break;
              ...
              case "4":     // Where effect appears.
                ...
                if( typ !== "beam" && typ !== "breath" && typ !== "splatter" ) {
                  addButtonWithCharID( "Caster Only",           already + ": CO",    "  The effect appears on the token taking the action.", "param" );
                  addButtonWithCharID( "First Target Only",     already + ": FTO",   "  The effect appears only on the first target.", "param" );
                  addButtonWithCharID( "All Targets",           already + ": AT",    "  The effect appears multiple times, once on each targets.", "param" );
                }
                if( typ === "beam" || typ === "breath" || typ === "splatter" || typ.startsWith( "custom ") ) {
                  addButtonWithCharID( "Caster to 1st Target",  already + ": CtFTO", "  The effect appears on the caster, and travels to the first target.", "param" );
                  addButtonWithCharID( "Caster to all Targets", already + ": CtAT",  "  The effect appears multiple times, starting on the caster and traveling to each target.", "param" );
                }
                break;
              default:
          ...
          case "gmstate": {
            ...
            if( this.charID )
              cid = this.charID;
            if( !cid && this.edClass && this.edClass.msg && this.edClass.msg.rolledByCharacterId )
              cid = this.edClass.msg.rolledByCharacterId;
...
//              this.chat( "Only GM can do this!", Earthdawn.whoTo.player | Earthdawn.whoFrom.api | Earthdawn.whoFrom.noArchive, "gmState" );
                  macroDetail( "ResetChars", "!Earthdawn~ ", false );

              s = "Most GM Special commands are not allowed for players. Players can "
                  + Earthdawn.makeButton("ResetChars", "!Earthdawn" + (cid ? "~ charID: " + cid : "") + "~ Misc: ResetChars: ?{(de)buffs Only or Full (including damage and karma) Reset|Mods Only|Full}",
                  "Remove all buffs and debuffs, reset all status markers, and optionaly set karma and health to full.", "param" )
                  + Earthdawn.makeButton("Clean Tokens", "!Earthdawn" + (cid ? "~ charID: " + cid : "") + "~ Debug: CleanTokens: Character",
                  "Clean this characters tokens from all pages.", "param" );
              ...
              s += JSON.stringify( t ) + "   ";
              s += Earthdawn.makeButton("ResetChars", "!Earthdawn" + (cid ? "~ charID: " + cid : "") 
                  + "~ Misc: ResetChars: ?{(de)buffs Only or Full (including damage and karma) Reset|Mods Only|Full}",
                  "Remove all buffs and debuffs, reset all status markers, and optionaly set karma and health to full.", "param" );
              s += Earthdawn.makeButton("Clean Tokens", "!Earthdawn" + (cid ? "~ charID: " + cid : "") + "~ Debug: CleanTokens: ?{Which Tokens|This Character,Character|All Tokens Ask for each page,Ask|All Tokens All Pages,All}",
                  "Clean Tokens from the campaign. This character cleans this characters tokens from all pages. Ask for each page presents a list of pages with tokens and asks which ones to clean all tokens from. All tokens cleans all tokens from all pages (except pages with 'bullpen' in their name)." , "param" );
              s += Earthdawn.makeButton("Verify/Fix All", "!Earthdawn~ Debug: ?{Do you want to run Verify Fix on ALL characters (use Special Function to run on just one sheet). "
                  + "Have console log open to see results.|Cancel|Yes but ask about each fix,RepSecBatch|Yes and automatically fix them,RepSecBatchFixAll}"
                  + ": ?{Refresh Abilities|Only if Ability text changes,Standard|If Ability name or text changes,forceNameRefresh}"
                  , "Verify/Fix All sheets will run a routine for each character that will check all attributes looking for problems and fixing what it can." , "param" );
              s += Earthdawn.makeButton("Rolltype Editor", "!Earthdawn~ ChatMenu: RolltypeEdit: state.Earthdawn.Rolltype: : Display",
                  "Change Rolltype. Change defaults, add or remove exceptions, Set an override." , "param" );
              s += Earthdawn.makeButton("Linking options", "!Earthdawn~ Misc: State: Linking",
                  "This allows various linking options to be changed (whether or not nameplates are automatically shown on PCs and NPCs for example)" , "param" );
              s += Earthdawn.makeButton("Result Style", "!Earthdawn~ Misc: State: Style: ?{What roll results style do you want? "
                  + "Value when this button was generated was '" + ((state.Earthdawn.style == 2) ? "Vauge Result" :((state.Earthdawn.style == 1) ? "Vauge Success" : "Full"))
                  + "'|Vague Roll Result,2|Vague Success (not recommended),1|Full,0}"
                  , "Switch API to provide different details on roll results. Vague Roll result is suggested. It does not say what the exact result is, but says how much it "
                  + "was made by. Vague Success says exactly what the roll was, but does not say the TN or how close you were.", "param")
              s += Earthdawn.makeButton("Show Dice", "!Earthdawn~ Misc: State: showDice: ?{Show roll results dice on seperate line. "
                  + "Value when this button was generated was '" + state.Earthdawn.showDice + "'|False,0|True,1}",
                  "If True dice roll details are displayed as a seperate line. Otherwise they are in the tooltip of the yellow Results box.", "param");
              s += Earthdawn.makeButton("No Pile on - Dice", "!Earthdawn~ Misc: State: NoPileonDice: ?{Enter max number of times each die may explode. -1 to disable"
                  + "|" + ((state.Earthdawn.noPileonDice == undefined) ? "-1" : state.Earthdawn.noPileonDice) + "}"
                  , "If a GM desires a less lethal game, or to reduce risk in specific encounters, they can use this option to keep NPC dice from exploding too many times. "
                  + "This controls the maximum number of times a single dice can 'explode' (be rolled again after rolling a max result). "
                  + "-1 is the standard default of unlimited. 0 means no dice will ever explode. 2 (for example) means a dice can explode twice, but not three times. "
                  + "This can done quietly, without announcing it to the players (see silent)." , "param" );
              s += Earthdawn.makeButton("No Pile On - Step", "!Earthdawn~ Misc: State: NoPileonStep: ?{Enter multiple of step number to limit result to. -1 to disable"
                  + "|" + ((state.Earthdawn.noPileonStep == undefined) ? "-1" : state.Earthdawn.noPileonStep) + "}"
                  , "If a GM desires a less lethal game, or to reduce risk in specific encounters, they can use this option to keep NPC roll results from exceeding a specific "
                  + "multiple the step number. If zero or -1, this is disabled and roll results are unlimited. if 1, "
                  +"then the dice result will never be very much greater than the step result. If (for example) the value is 3.5, then the result will never get very much "
                  + "greater than 3.5 times the step being rolled. This can done quietly, without announcing it to the players (see silent)." , "param" );
              s += Earthdawn.makeButton("Curse Luck / Pile On Silent", "!Earthdawn~ Misc: State: CursedLuckSilent: ?{Does the Cursed Luck Horror power work silently "
                  + "Value when this button was generated was '" + ((state.Earthdawn.CursedLuckSilent & 0x04) ? "Yes" : "No") + "'.|No,0|Yes,1}"
                  + "?{Does No-pile-on-dice work silently. Value when this button was generated was '" + ((state.Earthdawn.CursedLuckSilent & 0x02) ? "Yes" : "No") + "'.|No,0|Yes,1}"
                  + "?{Does No-pile-on-step work silently. Value when this button was generated was '" + ((state.Earthdawn.CursedLuckSilent & 0x01) ? "Yes" : "No") + "'.|No,0|Yes,1}"
                  , "Normally the system announces when a dice roll has been affected by Cursed Luck or Pile on Dice or Step. However the possibility exists that a "
                  + "GM might want it's effects to be unobtrusive. When these are set to 'yes', then the program just silently changes the dice rolls without announcing it." , "param" );
              s += Earthdawn.makeButton("API Logging", "!Earthdawn~ Misc: State: ?{What API logging event should be changed|LogStartup|LogCommandline|LogMsg}: ?{Should the API log this event|Yes,1|No,0}",
                  "What API events should the API log to the console?" , "param" );
//              s += Earthdawn.makeButton("Version", "!Earthdawn~ Misc: State: ?{API version or HTML version|API|HTML}: ?{Version number to store}",
//                  "You can change the html or javascript version number stored in the state. This can force character sheet update routines to be run next startup." , "param" );
              s += Earthdawn.makeButton("Change Edition", "!Earthdawn~ Misc: State: edition: ?{What rules Edition "
                  + "Value when this button was generated was '" + state.Earthdawn.edition + "'|Earthdawn Forth Ed,4 ED|1879 1st Ed,-1 1879|Earthdawn Third Ed,3 ED|Earthdawn First Ed,1 ED}",
                  "Switch API and Character sheet to Earthdawn 1st/3rd/4th Edition or 1879 1st Edition. Note this NEEDs to also be done in campaign default sheet settings." , "param" );
              s += Earthdawn.makeButton("Set all attribute", "!Earthdawn~ Debug: SetAttribAll: ?{Attribute name} : ?{Attribute Value}: ?{Confirm really do this|No|Yes}",
                  "Go through every character in the whole campaign and set a variable to the same value." , "param" );
              s += Earthdawn.makeButton("State Editor", "!Earthdawn~ ChatMenu: StateEdit: state.Earthdawn: : Display",
                  "A general state editor. Can change anything in the state variable." , "param" );
//              s += Earthdawn.makeButton("RollType", "!Earthdawn~ ChatMenu: RollType: Menu",
//                  "Determine the visibility of different roll results by type. For example some rolls might be public, others might be displayed to GM only." , "param" );
...
//                  "By default, PCs default to public rolls, but NPCs and Mooks default to rolls visible only by the GM. This can be changed." , "param" );
...
//                  "At charcter creation each charcters Karma Ritual behavior is set to this value. By default, The 'New Day' button refills a characters Karma pool up to Full." , "param" );
//              s += Earthdawn.makeButton("ToAPI", "!Earthdawn~ Misc: toAPI: ?{Remove API|Never Mind|Set Characters noAPI,Set|Fully remove API,noAPI}",
//                  "FromAPI will set all sheets to use noAPI buttons and remove all Earthdawn state variables (can be run just before deactivating API or uninstalling sheet to clean things up)" , "param" );
//              s += Earthdawn.makeButton("ToAPI", "!Earthdawn~ Misc: toAPI: ?{Set all characters to use API or noAPI|API|noAPI}",
//                  "ToAPI will set all sheets to use API. FromAPI will set all sheets to use noAPI buttons and remove all Earthdawn state variables (can be run just before deactivating API or uninstalling sheet to clean things up)" , "param" );
              this.chat( s.trim(), Earthdawn.whoTo.player | Earthdawn.whoFrom.api | Earthdawn.whoFrom.noArchive, "gmState" );
          } } break;  // end gmstate and gmspecial
          case "grimoire": {    // list the spells available to this character
            let tid = edParse.charID;
            lst = this.getUniqueChars( 1 );
            for( let k in lst ) {
              id = k;
              ...
              _.each( attflt, function ( att ) {
                  edParse.charID = att.get( "_characterid" );
                  let tmpPre = edParse.buildPre( att.get( "name" ));
                  s += Earthdawn.makeButton( att.get("current"),
                      "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{" + id + "|" + tmpPre + "Load}"
                      , "Load Preset "
              ...
              _.each( attflt, function ( att ) {
                 ...
                 s += Earthdawn.makeButton( att.get("current"),
                      "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{" + id + "|" + tmpPre + "Roll}"
                      , (bnot ? "" :"Spell in Matrix, use the Spells Menu instead! - ")
                      + Earthdawn.getParam( Earthdawn.getAttrBN( id, tmpPre + "DisplayText", ""), 1, "~") + " "
                      + Earthdawn.getParam( Earthdawn.getAttrBN( id, tmpPre + "SuccessText", ""), 1, "~"), bnot ? "param" : "param2" );
                }
              }); // End for each attribute.
          ...
          case "help": {
            s = Earthdawn.makeButton( "Wiki Link", ( state.Earthdawn.sheetVersion < 1.8 ) ? "https://wiki.roll20.net/Earthdawn_-_FASA_Official"
                  ...
                  "This button will open this character sheets Wiki Documentation, which should answer most of your questions about how to use this sheet.", "dflt", true ) + "   Important! Open this in a new tab, not this tab.";
          ...
          case "inspect": {
            s = "Inspect: "
                  + Earthdawn.makeButton( "Lookup Value", "!Earthdawn~ foreach: st: tuc~ Debug: Inspect : GetValue: ?{Enter full name}",
                      "Enter an attribute name (simple or whole repeating section name), and get the value.", "param" )
                  + Earthdawn.makeButton( "IDs from Name", "!Earthdawn~ foreach: st: tuc~ Debug: Inspect : GetIDs: ?{Enter Name fragment}",
                      "Enter a text fragment, and get IDs of every repeating section name that contains that fragment.", "param" )
                  + Earthdawn.makeButton( "TokenObj", "!Earthdawn~ foreach: st~ Debug: Inspect : TokenObj",
                      "Test code to cause selected tokens to display TokenInfo to chat.", "param" )
                  + Earthdawn.makeButton( "Repeating section", "!Earthdawn~ charID: ?{Char ID|@{selected|character_id}}~ Debug: Inspect: RepeatSection: ?{RowID}: ?{Detail|Short|Full}",
                      "Test code to show information on a row.", "param" )
                  + Earthdawn.makeButton( "Object ID", "!Earthdawn~ Debug: Inspect: ObjectID: ?{Object ID}",
                      "Test code to show information on what an ID is. Character ID, Token ID, etc.", "param" )
                  + Earthdawn.makeButton( "Statusmarkers", "!Earthdawn~ foreach: st~ Debug: Inspect : statusmarkers",
                      "Test code to cause selected tokens to display statusmarkers to Chat.", "param" );
            this.chat( s.trim(), Earthdawn.whoTo.player | Earthdawn.whoFrom.api | Earthdawn.whoFrom.noArchive, "API" );
          } break;  // end inspect
          case "languages": {
            let tid = edParse.charID;
            ...
            for( let k in lst ) {
              ...
              _.each( attributes, function ( att ) {
                if( att.get( "name" ).endsWith( "_SKL_Name" )) {
                  ...
                  s += "<div><strong>";
                  s += att.get( "current" );
                  let pre = edParse.buildPre( "SKL", att.get("name") );
                  s += "</strong>";
              ...
              s += "</div>";
              if( cnt )
                edParse.chat( s.trim(), Earthdawn.whoTo.player | Earthdawn.whoFrom.api | Earthdawn.whoFrom.noArchive,
          ...
          case "link": {      // chatmenu: Link: (code - T, NAC, SK, SPM, WPN): rowID
            ...
            let pre = edParse.buildPre( ssa[ 2 ], ssa[ 3 ]);
            ...
            addButtonWithCharID( "Add a Link", "ChatMenu: Linkadd1: " + ssa[ 2 ] + ": " + ssa[ 3 ] +
                  ...
                  "Link this to an Talent, Discipline Circle, Skill, Weapon, Thread Item or Attribute, such that this item will use that items Rank." , "param");
            ...
            if( !lst || !lst2 || lst.length == 0 )
              ...
              s += "Remove ";
              if( att )
                addButtonWithCharID( lst[ i ].trim(), "ChatMenu: LinkRemove: " + ssa[ 2 ] + ": " + ssa[ 3 ] + ": Attribute: "  + lst2[ i ], "Make this item no longer linked to this Attribute.", "param2");
              else {
                let tmp = "item.";
                ...
                addButtonWithCharID( lst[ i ].trim(),
                    ...
                    "Make this item no longer linked to this " + tmp, "param2");
            } }
                      this.chat( s.trim(), Earthdawn.whoTo.player | Earthdawn.whoFrom.api | Earthdawn.whoFrom.noArchive, "API" );
          } break;  // end link
  ...
  //            }
          ...
          case "linkadd2": {      // User has pressed a button telling us which exact one to link to OR we fell through from above due to only finding only one candidate.
            ...
            function walkLinks( nextCode, nextRow, badLink, nameList ) {    // Check to make sure row being linked does not reference row adding link.
              let links = Earthdawn.getAttrBN( edParse.charID, edParse.buildPre( nextCode, nextRow) + "LinksGetValue_max" );
              if( links ) {
                let alinks = links.split();
                for( let i = 0; i < alinks.length; ++i ) {
                  if( alinks[ i ].indexOf( badLink ) !== -1 ) {
                    edParse.chat( "Error! attempted circular reference:  " + nameList + " to "
                        + Earthdawn.getAttrBN( edParse.charID, edparse.buildPre( nextCode, nextRow) + "Name", Earthdawn.whoFrom.apiWarning ));
                    return false;
                    ...
                    if ( !walkLinks( Earthdawn.repeatSection( 3, alinks[ i ]), Earthdawn.repeatSection( 2, alinks[ i ]), badLink,
                          nameList + " to " + Earthdawn.getAttrBN( edParse.charID, edParse.buildPre( nextCode, nextRow) + "Name", "" )))
                      return false;
              } }
              return true;
            };  // end walkLinks()

            if( !walkLinks( ssa[ 4 ], ssa[ 5 ], ssa[ 3 ], Earthdawn.getAttrBN( this.charID, edParse.buildPre( ssa[ 2 ], ssa[ 3 ]) + "Name", "" )))
              return;
...
//log(333); log(linkName);}
            ...
            let t =  att ? ssa[ 5 ] : Earthdawn.getAttrBN( this.charID, edParse.buildPre( ssa[ 4 ], ssa[ 5 ] ) + "Name", "" ),
              l;
            function lnk() {
              l = "";
              for( let i = 0; i < arguments.length; ++i ) {
                let tmp = (att ? arguments[ i ] : edParse.buildPre( ssa[ 4 ], ssa[ 5 ] )) + arguments[ i ];
                l += ((i === 0) ? "" : "+" ) + tmp;
              }
            };
                  // repeating_talents_xxx_T_name+repeating_talents_xxx_T_name2,another
            ...
            linkAdd( edParse.buildPre( ssa[ 2 ], ssa[ 3 ]) + "LinksGetValue",
            ...
            if( !att )      // LinksProviceValue are comma delimited lists of form (code);(rowID).
              linkAdd( edParse.buildPre( ssa[ 4 ], ssa[ 5 ]) + "LinksProvideValue",
                    Earthdawn.getAttrBN( this.charID, edParse.buildPre( ssa[ 2 ], ssa[ 3 ] ) + "Name", "" ).replace( /[\,|\+]/g, ""),
                    ssa[ 2 ] + ";" + ssa[ 3 ]);
...
//            this.sendSWflag();
          ...
          case "linkremovehalf": {    // ChatMenu: linkRemoveHalf: attribute name, rowID to remove.
            ...
            function linkRemove( linkName, rid, countIt ) {
              let obj = Earthdawn.findOrMakeObj({ _type: "attribute", _characterid: edParse.charID, name: linkName }, "");
              let dlst = obj.get( "current" ),
                llst = obj.get( "max" );
...
//                edParse.addSWflag( "Trigger2", linkName );
            ...
            if( Earthdawn.safeString( ssa[ 1 ] ).toLowerCase() === "linkremovehalf") {
              ...
              if( ssa.length < 5 || ssa[ 5 ].length < 2 )   // Make sure got a valid command line.
                return;
              let att = (ssa[ 4 ] === "Attribute");
              linkRemove( edParse.buildPre( ssa[ 2 ], ssa[ 3 ] ) + "LinksGetValue", ssa[ 5 ], true);
              if( !att )
                linkRemove( edParse.buildPre( ssa[ 4 ], ssa[ 5 ]) + "LinksProvideValue", ssa[ 3 ], false);
...
//            this.sendSWflag();
          ...
          case "oppmnvr": { // Opponent Maneuverer.   Here we are just displaying a list of buttons that are possible.
            ...
            function makeButtonLocal( txt, lnk, es, tip ) {
              ...
              t += Earthdawn.makeButton( txt, tssa, tip, "damage" );
            };

            let sectPlayer = new HtmlBuilder( "", "", { class: "sheet-rolltemplate-chatbox" });
            let sectSpell = edParse.newSect();
            ...
            if( lst !== "bad" && lst.length > 1 ) {
              ...
              for( let i = 0; i < arr.length; ++i ) {
                let pre = edParse.buildPre( "MAN", arr[ i ] );
          ...
          case "skills": {      // List out all skills.      Called from a macro Token action (visible when any character is selected).
            let tid = edParse.charID;
            lst = this.getUniqueChars( 1 );
            for( let k in lst ) {
              id = k;
              edParse.charID = k;
// RowID looks suspicious
              let s1 = [], s2 = [], s3 = [];
              s = "";
                                          // go through all attributes for this character and look for ones we are interested in
              let attributes = findObjs({ _type: "attribute", _characterid: id });
              _.each( attributes, function (att) {
                if (att.get("name").endsWith( "_SK_Name" )) {
                  s1.push( { a: Earthdawn.getAttrBN( id, edParse.buildPre( att.get( "name" ))  + "Name", ""),
                        b: "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{" + id + "|" + edParse.buildPre( att.get( "name" )) + "Roll}",
                        c: Earthdawn.getAttrBN( id, edParse.buildPre( att.get("name") ) + "Notes", "" ).replace( /\n/g, Earthdawn.constantIcon( "cr" )) });
                } else if (att.get("name").endsWith( "_SKK_Name" )) {
                  s2.push( { a: Earthdawn.getAttrBN( id, edParse.buildPre( att.get("name" )) + "Name", ""),
                        b: "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{" + id + "|" + edParse.buildPre( att.get( "name" )) + "Roll}"});
                } else if (att.get("name").endsWith( "_SKA_Name" ))
                  s3.push( { a: Earthdawn.getAttrBN( id, edParse.buildPre( att.get( "name" )) + "Name", ""),
                        b: "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{" + id + "|" + edParse.buildPre( att.get( "name" )) + "Roll}"});
              }); // End for each attribute.
              for( let j = 0; j < s1.length; ++j )
                s += Earthdawn.makeButton( s1[j].a, s1[j].b, s1[j].c, "action");
              for( let j = 0; j < s2.length; ++j )
                s += Earthdawn.makeButton( s2[j].a, s2[j].b, null, "action" );
              for( let j = 0; j < s3.length; ++j )
                s += Earthdawn.makeButton( s3[j].a, s3[j].b, null, "action" );
              s += Earthdawn.makeButton( "Languages", "!Earthdawn~ chatmenu: languages", null, "dflt" );
          ...
          case "spells": {      // List out all the spellcasting skills, spells in matrix, and spells in grimour.
            ...
            for( let k in lst ) {
              ...
              _.each( attributes, function (att) {      // First we are listing all the spellcasting Talents.
                if( att.get( "name" ).endsWith( "_Special" ) && Earthdawn.keywordCheck( att.get( "current" ), true, "SPL-" )) {
                  let pre = edParse.buildPre( att.get( "name" ));
                  s += Earthdawn.makeButton( getAttrByName( id, pre + "Name" ), "!edToken~ " + Earthdawn.constantButton( "percent" )
                        + "{" + id + "|" + pre + "Roll}"
                        , Earthdawn.getParam( Earthdawn.getAttrBN( id, pre + "DisplayText", "" ), 1, "~") + " "
                        + Earthdawn.getParam( Earthdawn.getAttrBN( id, pre + "SuccessText", "" ), 1, "~")
                        , att.get( "current" ).includes("-Willforce") ? "effect" : "action" );
                }
                if( att.get("name").endsWith( "_SPM_Contains" ))    // Then all the spells in the grimoire
                  s2 += Earthdawn.makeButton( att.get("current"), "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{" + id + "|"
                      + edParse.buildPre( "SPM", att.get("name")) + "Roll}"
                      , Earthdawn.getParam( Earthdawn.getAttrBN( id, edParse.buildPre( att.get("name")) + "DisplayText", "" ), 1, "~") + " "
                      + Earthdawn.getParam( Earthdawn.getAttrBN( id, edParse.buildPre( att.get("name")) + "SuccessText", "" ), 1, "~")
                      , "param" );
              }); // End for each attribute.
            }
                  ...
                  if( entryPoint === 2 ) {    // RolltypeEdit
                    if( scope.endsWith( "PC" )) {    // We are in the PC or NPC section.
                      ...
                      s += Earthdawn.makeButton( "Back to main level."    // + walk.slice( 0, -1).join( ".")
                            ...
                            ,"Move back to the Rolltype root." , "param" );
                      ...
                      Object.entries( currentScope[ "Exceptions" ] ).forEach(([key, val]) => {
                        s += Earthdawn.makeButton( val[ "name" ]
                              ...
                              + ": ?{change roll display to" +  Earthdawn.constantButton( "pipe", 2 ) + "Public" +  Earthdawn.constantButton( "pipe", 2 ) + "GM Only" 
                              + Earthdawn.constantButton( "pipe", 2 ) + "Player and GM" +  Earthdawn.constantButton( "pipe", 2 ) + "Controlling Only" 
                              + Earthdawn.constantButton( "braceClose", 2 ) + "|Delete, exceptionDelete}"
                              ,"Edit or Delete exception." , "param" );
                        s += " " + val[ "display" ] + " ";
                      ...
                      s += Earthdawn.makeButton( "Refresh",
                            ...
                            "Display this again, so that you can see what has changed." , "param" );
                      ...
                      s += Earthdawn.makeButton( currentScope[ "Override" ] === false ? "None" : currentScope[ "Override" ]
                            , "", tipText( currentScope[ "Override" ] ), "dflt" );    // This button does not do anything, it just providees a place to hang a tooltip.
                      [ "PC", "NPC" ].forEach( function( what ) {
                        'use strict';
                        s += "<br>";
                        s += Earthdawn.makeButton( what, "!Earthdawn~ ChatMenu: rolltypeEdit: " + scope + ": " + what + ": In", "Edit " + what + " default and exceptions", "param" );
...
//                        s += "<b>Exceptions :</b> " + currentScope[ what ][ "Exceptions" ].length;
                      ...
                      s += Earthdawn.makeButton( "Help",
                            ...
                            "Helpful hints as to how to set exceptions." , "param" );
                    }
                  } else {    // stateEdit
                    ...
                    if( walk.length > 1 )
                      s += Earthdawn.makeButton( "Up to level " + walk.slice( 0, -1).join( "."),
                            ...
                            "Move one step closer to the root." , "param" );
                    s += Earthdawn.makeButton( "Refresh",
                    ...
                    s += Earthdawn.makeButton( "Add new element or collection ",
                          ...
                          + "|Number" + Earthdawn.constantButton( "comma" ) + "Number: ?{Value" + Earthdawn.constantButton( "pipe", 2 ) + "0" + Earthdawn.constantButton( "braceClose", 2 ) 
                          + "|String" + Earthdawn.constantButton( "comma" ) + "String: ?{Value" + Earthdawn.constantButton( "braceClose", 2 ) + "|Boolean true|Boolean false|Array|Object}",
                          "Add an Element, Array or Collection." , "param" );
                    _.each( currentScope, function( v, k ) {
                      ...
                      switch( typeof v ) {
                        ...
                        case "null":
                        case "undefined":
                        case "string":    str = "?{Edit or Delete string " + k + " Old value ( " + v + " ) |Edit, Edit: ?{New Value" + Earthdawn.constantButton( "braceClose", 2 ) + "|Delete}";
                                          tip = "Edit or delete this string.";
                          break;
                        case "number":    str = "?{Edit or Delete number " + k + " Old value ( " + v + " ) |Edit, Edit: ?{New Value" + Earthdawn.constantButton( "pipe", 2 ) + "0" 
                                              + Earthdawn.constantButton( "braceClose", 2 ) + "|Delete}";
                                          tip = "Edit or delete this number.";
                          break;
                        case "boolean":   str = "?{New Value " + k + " Old value ( " + v + " ) |True, Edit: true|False, Edit: false|Delete}";
                        ...
                        case "object":
                          if( Array.isArray( v ) ) {    // This is best practices for building Roll20 nested chat menu queries. 
                                                        // The outermost query can be of the form ?{}. Inner queires must call Earthdawn.constantButton for pipe, comma, and braseClose. 
                            str = "?{What do you want to do with array \'" + k + "\'|" 
                                + "Delete|Add Elements, arrayAdd: " 
                                  + "?{Add Where (F for first. L for last. or after a zero based index number)" + Earthdawn.constantButton( "braceClose", 2 ) + ": "
                                  + "?{Add What type of element"
                                    + Earthdawn.constantButton( "pipe", 2 ) + "Number" + Earthdawn.constantButton( "comma", 2 ) 
                                    + "Number: ?{Value" + Earthdawn.constantButton( "pipe", 3 ) + "0" + Earthdawn.constantButton( "braceClose", 3 ) 
                                    + Earthdawn.constantButton( "pipe", 2 ) + "String" + Earthdawn.constantButton( "comma", 2 ) + "String: ?{Value" + Earthdawn.constantButton( "braceClose", 3 ) 
                                    + Earthdawn.constantButton( "pipe", 2 ) + "Boolean true" + Earthdawn.constantButton( "pipe", 2 ) + "Boolean false" 
                                    + Earthdawn.constantButton( "pipe", 2 ) + "Array" + Earthdawn.constantButton( "pipe", 2 ) + "Object" + Earthdawn.constantButton( "braceClose", 2 )
                                + "|Remove Elements, arrayRemove: " 
                                  + "?{Remove starting at what index number (zero based)" + Earthdawn.constantButton( "pipe", 2 ) + "0" + Earthdawn.constantButton( "braceClose", 2 )
                          ...
                          break;
                        default: 
                          log("stateEdit error. Unknown typeof " + typeof v + " for " + k + " Old value ( " + v + " )");
                          bad = true;
                      }
                      if( !bad ) s += Earthdawn.makeButton( k, "!Earthdawn~ ChatMenu: StateEdit: " + scope + ": " + k + ":" + str, tip , "param" );
                    });
                  }   // end entrypoint 1
              ...
              switch( ssa[ i ].toLowerCase() ) {
                ...
                case "exceptionis":
                  ...
                  if( wname in except ) {
                    ...
                    s += "Edit display exception for " 
                      ...
                      + "| Change to Player and GM, exceptionEdit: Player and GM|Delete exception, exceptionDelete}", tip, "action" ) + ".";
                  } else
                ...
                case "exceptionwouldbe":
                  ...
                  s += "Create a display exception for " 
                    + Earthdawn.makeButton( wname, "!Earthdawn~ ChatMenu: RolltypeEdit: " + ssa[ 2 ] + ": " + wname + ": ?{Create a display exception for '" + wname
                    + "'|Public, exceptionAdd: Public|GM Only, exceptionAdd: GM Only|Player and GM, exceptionAdd: Player and GM}", tip, "action" ) + ".";
                break;
                default:
          ...
          case "status": {      // Called from a macro Token action. Visible when any character is selected.
            ...
            _.each( Earthdawn.StatusMarkerCollection, function( menuItem ) {
              ...
              s += Earthdawn.makeButton( buildLabelStatus( menuItem, markers ),
                    ...
                    null, basic ? "statusoff" : "statuson" );
            });
            this.chat( s.trim(), Earthdawn.whoTo.player | Earthdawn.whoFrom.noArchive, "Status" );
          } break;  // end Status
          case "strainchange": {      // strainChange, old strain, new strain.
            let att = state.Earthdawn.g1879 ? "Damage-Stun" : "Damage"
            let dmg = Earthdawn.getAttrBN( this.charID, att );
            this.Damage( [ "STRAINSILENT", "NA", "AllowNeg", ( Earthdawn.parseInt2( ssa[ 2 ] ) * -1) + Earthdawn.parseInt2( ssa[ 3 ] ) ]);      // adjust with difference between old and new values.
            this.chat( "Strain changed from " + ssa[ 2 ] + " to " + ssa[ 3 ] + ".  Old '" + att + "' was " + dmg
                  + ". New value " + Earthdawn.getAttrBN( this.charID, att ), Earthdawn.whoTo.player | Earthdawn.whoFrom.noArchive, "Strain Change" );
          } break;
            ...
            for( let k in lst ) {
              ...
              _.each( attributes, function (att) {
                if (att.get("name").endsWith( "_T_Name" ) || att.get("name").endsWith( "_NAC_Name"))
                  if( !non || (Earthdawn.getAttrBN( id, edParse.buildPre( att.get( "name" )) + "CombatSlot", "") != "1")) {
                    let kw = Earthdawn.keywordCheck( Earthdawn.getAttrBN( id, edParse.buildPre( att.get("name") ) + "Special", ""), true, "Recovery", "Initiative"),
                        modtype = Earthdawn.getAttrBN( id, edParse.buildPre( att.get("name") ) + "Mod-Type", ""),
                        bg;
                    if( kw & 0x02 )                           bg = "health";    // Recovery or Recovery-Woodskin
                    else if( kw & 0x04 )                      bg = "effect";    // Initiative
                    else if( modtype === "Action" )           bg = "action";
                    else if( modtype === "Effect" )           bg = "effect";
                    else if( modtype.startsWith( "Attack" ))  bg = "attack";
                    else if( modtype.startsWith( "Damage" ))  bg = "damage";
                    else                                      bg = "action";

                    s += Earthdawn.makeButton( Earthdawn.getAttrBN( id, edParse.buildPre( att.get("name") ) + "Name", ""),
                        "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{" + id + "|" + edParse.buildPre( att.get("name")) + "Roll}",
                        Earthdawn.getAttrBN( id, edParse.buildPre( att.get("name") ) + "Notes", "" ).replace( /\n/g, Earthdawn.constantIcon( "cr" )), bg );
                  }
              }); // End for each attribute.
    ...
    this.CreaturePower = function( ssa )  {
      ...
      try {
        if( ssa[ 1 ].startsWith( "Custom" )) {
          ...
          //     {"type":"V","rolls":[{"type":"R","dice":1,"sides":20,"mods":{"exploding":""},"results":[{"v":20},{"v":16}]},{"type":"M","expr":"+"},{"type":"R","dice":1,"sides":8,"mods":{"exploding":""},"results":[{"v":6}]},{"type":"M","expr":"+"},{"type":"R","dice":1,"sides":6,"mods":{"exploding":""},"results":[{"v":1}]}],"resultType":"sum","total":43}
    ...
    this.CursedLuck = function( curse, roll )  {
      ...
      try {
        ...
        if( Earthdawn.getAttrBN( this.charID, "NPC", "1", true ) > 0 ) {      // Pile-on prevention is only done on NPC dice rolls. PCs can pile on all they want.
          ...
          let done = false,
              lowLimit = Math.max( Math.max( ("targetNum" in this.misc) ? this.misc[ "targetNum" ] : 0, this.misc[ "effectiveStep" ] || 0) * (state.Earthdawn.noPileonStep || 1), 14);     // No matter how low the TN, step and multiplier, let it be at least 14 (number picked at arbitrarily)
        ...
        return roll;
          ...
          //      0 - (optional) Damage (default), Strain, Stun, Recovery, Woodskin, notWoodskin;   // Note that these can also show up in this.misc[ "keyword" ] and have the same effect.
    ...
    this.Damage = function( ssa ) {
      ...
      try {
        ...
        if( "keyword" in this.misc ) {
          let kwc = Earthdawn.keywordCheck( this.misc[ "keyword" ] , true, "Recovery", "Recovery-Stun", "Stun" );
          if( kwc & 0x06 ) bRecovery = true;    // Recovery or Recovery-Stun
          if( kwc & 0x0C ) bStun = true;        // Recovrery-Stun or Stun
        }
        ...
        } } // end switch and end loop getting parameters.

        if( armor < 1 )
          armorType = "";
        if( !bAllowNeg && (bRecovery ? (dmg >= 0) : (dmg <= 0 )))        // If the passed damage evaluates to zero, just exit.
          return;
        ...
        if( bStrain && !bVerbose ) {      // Strain normally (when part of some other action) does not have a separate strain message, since the strain is reported as part of the roll results.
          ...
          if( dmg < armor )               newMsg += "most";
          else if ( dmg <= (armor * 2))   newMsg += "some";
          else                            newMsg += "little";
          newMsg += " of the damage";
        }

        let currDmg, attributeDmg;
        if( bToken && ( bMook || !(this.tokenInfo.tokenObj.get( "bar3_link" )) ))     // If it is a mook, get it from the token. 
          currDmg = Earthdawn.parseInt2( this.tokenInfo.tokenObj.get( "bar3_value" ));
        if( !bMook ) {     // if not a mook, get damage from the character sheet. 
          attributeDmg = Earthdawn.findOrMakeObj({ _type: 'attribute', _characterid: this.charID, name: "Damage" }, 0);
        ...
        if( !bStun ) {    // normal damage. This is identical for characters and mooks.
          ...
          if( !bMook )     // if not a mook, set it on the character sheet. 
              Earthdawn.setWithWorker( attributeDmg, "current", currDmg, 0 );
          if( bToken && ( bMook || !(this.tokenInfo.tokenObj.get( "bar3_link" )) ))     // If it is a mook, set it on the token. 
        ...
        let unc = Earthdawn.getAttrBN( this.charID, "Damage_max", 20 ),     // for pc's and npc's unc is current unc rating (with stun damage already subtracted). For mooks it is the core characters unc rating.
            WoundThreshold = Earthdawn.getAttrBN( this.charID, "Wound-Threshold", 7 ) || 0;
        if( bStun ) {     // Stun uses a different procedure than normal damage because of the calculated fields on the character sheet.
          ...
          if( dmg >= WoundThreshold ) {
            this.MarkerSet( ["d", "harried", "++"] );      // Stun damage does not cause wounds, set harried until end of round.
            newMsg += " and is harried until end of round";
        } } // end stun damage section.

        if( !bStrain && !bStun && dmg >= WoundThreshold ) {     // wound.
          let currWound;
          if( !bMook ) {     // if not a mook, get from the character sheet. 
          ...
          if( bToken && ( bMook || !(this.tokenInfo.tokenObj.get( "bar2_link" )) )) {    // If it is a mook, do token. 
            currWound = Earthdawn.parseInt2( this.tokenInfo.tokenObj.get( "bar2_value" ));
            if( isNaN( currWound ) )
              currWound = 1;
            else
              currWound += 1;
...
//          if( bMook )
        ...
        } else {    // not dead or unconscious, so OK.
//          if( bMook )
            this.MarkerSet( ["d", "healthunconscious", "u"] );
          if( dmg >= (WoundThreshold + 5)      // Character is wounded and need to make a Knockdown test
                && ( Earthdawn.getAttrBN( this.charID, "Knockdown-Adjust", "0", true ) < 990 )          // but is not immune to knockdown
                && ( Earthdawn.getAttrBN( this.charID, "condition-KnockedDown", "0" ) != "1" )) {       // and is not already knocked down.
            ...
            gmMsg   += " TN " + ( dmg - WoundThreshold ) + "<br>" +
                    ...
                    ,"Make a standard Knockdown test.", "action" );
            let attributes = findObjs({ _type: "attribute", _characterid: this.charID }),
              po = this;
            _.each( attributes, function (att) {
              if( att.get( "name" ).endsWith( "_Special" ) && Earthdawn.keywordCheck( att.get("current"), "Knockdown" )) {
                let pre = po.buildPre( att.get( "name")),
...
//log( "Talent Found " + pre + " " + name + " "  + code +" " + rid + " " + cname);
        ...
        if( !bRecovery && gmMsg.length > 0)
          ...
          // ParseObj.Debug()
          // This is a collection of test, repair, or diagnostic commands.
          //
          // Most are probably invoked from the "Special Function" drop-down and button on the Adjustments page.
          // Cancel, CleanTokens, Inspect, RepSecFix, RepSecBatch, RepSecBatchFixAll, SetAttribAll, sheetworkertest, showeach, test.
      ...
      try {
        switch ( Earthdawn.safeString( ssa[ 1 ] ).toLowerCase() ) {
          case "cancel":                  // This does nothing. It is just a target for a repsecBatch - cancel that is meant to be ignored.
            break;
          case "cleantokens":                  // Clean tokens from the campaign. All tokens of current character (available to players or gm), all tokens in campaign, or all tokens on requested pages.
            if( ssa[ 2 ] != "Character" && !playerIsGM( this.edClass.msg.playerid ) )
              this.chat( "Only GM can do this!", Earthdawn.whoTo.player | Earthdawn.whoFrom.api | Earthdawn.whoFrom.noArchive, "gmState" );
            else {
              let tkns;
              switch ( Earthdawn.safeString( ssa[ 2 ] ).toLowerCase() ) {
                case "ask":           // Give a list of pages with tokens. If they press one it calls with Page.
                  tkns = findObjs({ _type: "graphic",  _subtype: "token" });
                  let s = "Which map pages do you wish to clean all tokens from: ",
                      pages = {};
                  _.each( tkns, function( TokObj ) {
                    if( TokObj.get( "_pageid" ) in pages )
                      ++pages[ TokObj.get( "_pageid" )];
                    else
                      pages[ TokObj.get( "_pageid" )] = 1;
                  }); // End ForEach Token
                  _.each( pages, function( v, k ) {
                    let pg = getObj("page", k)
                    if( pg )
                      s += Earthdawn.makeButton( pg.get( "name" ) + ": " + v, "!Earthdawn~ charID: " + po.charID + "~ Debug: CleanTokens: Page: " + k,
                          "Clean all " + v + " tokens from this page.", "param" );
                  });
                  this.chat( s.trim(), Earthdawn.whoTo.player | Earthdawn.whoFrom.api | Earthdawn.whoFrom.noArchive, "gmState" );
                  break;
                case "all":           // Clean all tokens from all pages (except bullpen).
                  tkns = findObjs({ _type: "graphic",  _subtype: "token" });
                  break;
                case "character":     // clean tokens for the current character whereever they may be (except bullpen).
                  tkns = findObjs({ _type: "graphic",  _subtype: "token", represents: po.charID });
                  break;
                case "page":          // Clean all tokens from this page. Called from "Ask".
                  tkns = findObjs({ _type: "graphic",  _subtype: "token", _pageid: ssa[ 3 ] });
                  break;
                default:
                  Earthdawn.errorLog( "ED.Debug:CleanToken() invalid argument: " + ssa.toString(), po );
              }
              if( ssa[ 2 ] != "Ask" ) {
                let bpage = (ssa[ 2 ] == "Page"),
                    count = 0,
                    bullpens = [];
                if( !bpage) {
                  let pgs = findObjs({ _type: "page" });
                  if( pgs )
                    _.each( pgs, function( pg ) {
                      if( Earthdawn.safeString( pg.get( "name" )).toLowerCase().includes( "bullpen" ))   // Find all pageid's that include Bullpen in the name. Unless cleaning named pages.
                        bullpens.push( pg[ "_id" ] );
                    });
                }
                _.each( tkns, function( tokobj ) {
                  if( !bullpens.includes( tokobj[ "_pageid" ] )) {
                    ++count
                    tokobj.remove();
                  }
                });
                this.chat( count + " Tokens removed.", Earthdawn.whoTo.player | Earthdawn.whoFrom.api | Earthdawn.whoFrom.noArchive, "gmState" );
            } }
            break;    // end CleanToken
                ...
                if( ssa[ 4 ] === "Full" ) {
                  ...
                  _.each( attributes, function (att) {
                    ...
                    if ( name.startsWith( "repeating_" ) && name.indexOf( ssa[3] ) > -1) {
                      log( att );
                          // Note, this chat can throw a console log error message if contents contains chat code. For example "name":"repeating_spell_-OHWMT1dam9KDRxhMgB0_SP_Casting","current":"MD1: @{target|Target|token_id} the @{target}, chat will try and fail to interprite that.  Don't worry about it.
                  ...
                  if( state.Earthdawn.gED )
                    checkRepeat( po.buildPre( "DSP", ssa[3] ), "Code", "Name", "Circle" );
                  else
                    checkRepeat( po.buildPre( "DSP", ssa[3] ), "Name", "Circle", "Tier" );
                  checkRepeat( po.buildPre( "T", ssa[3] ), "Name", "Rank" );
                  checkRepeat( po.buildPre( "NAC", ssa[3] ), "Name", "Rank" );
                  checkRepeat( po.buildPre( "SK", ssa[3] ), "Name", "Rank" );
                  checkRepeat( po.buildPre( "SKK", ssa[3] ), "Name" );
                  checkRepeat( po.buildPre( "SKA", ssa[3] ), "Name" );
                  checkRepeat( po.buildPre( "SKL", ssa[3] ), "SKL_Name" );
                  checkRepeat( po.buildPre( "SPM", ssa[3] ), "Type", "Origin", "Contains" );
                  checkRepeat( po.buildPre( "SP", ssa[3] ), "Name", "Circle", "Discipline" );
                  checkRepeat( po.buildPre( "WPN", ssa[3] ), "Name" );
                  checkRepeat( po.buildPre( "MNT", ssa[3] ), "Name" );
                  checkRepeat( po.buildPre( "TI", ssa[3] ), "Name" );
                }
              } break;
          ...
          case "repsecfix": {     // RepSecFix: Repeating Section Verify and Fix.   Note: This is now labeled Character Sheet Verify/Fix and fixes some things not in repeating sections.
            ...
            try {
              let orig = [],
                ...
                firstRun = ssa.includes( "firstRun" ),  // The API has never been run in this campaign before. Not used in current implmentation, but could be.
                first = ssa.includes( "first" ),        // This is the first character in a firstRun.
                issues = 0,
                rep = 0,
                nonrep = 0,
                timer = Date.now();
              ...
              for( let i = 1; i < sorted.length; ++i ) {    // Go through each and every attribute in sorted order.
                ...
                if( att.get( "name" ).startsWith( "repeating_")) {    // Now we are specifically fixing repeating sections that have no rowID, or that are all lower or upper case.
                  ++rep;
                  let nm = Earthdawn.safeString( att.get("name") ),
                    rid = Earthdawn.safeString( Earthdawn.repeatSection( 2, nm)),
                  ...
                  if( rid && Earthdawn.codeToName( rid, true )) {    // We have seen cases where somehow a Code gets placed where a rowID should be. If we find that the rowID matches a valid code, then just delete the attribute.
                  ...
                  if( code && Earthdawn.testNoRowID( code, true ))    // Certain types of repeating sections do not need to test if there is a rowID. 
                    continue;
                  if( nm.endsWith( "_Name" )) {     // When we find a repeating section name, Make sure we have a RowID that matches the rowID of the name.
                    ...
                    if( !attrib || attrib.length == 0 ) {   // RowID does not exist at all for this item.
                      if( rid !== rid.toLowerCase()) {    // Don't do this if this row is likely corrupt anyway.
                        attrib = createObj( "attribute", { _characterid: po.charID,  name: nm.slice( 0, -5) + "_RowID","current": rid });
                        po.chat( "Error found, " + att.get( "current" ) + " at " + nm + " did not have a rowID.", Earthdawn.whoFrom.apiWarning )
                        ++issues;
                      }
                    } else if( !attrib[ 0 ].get( "current" ) || attrib[ 0 ].get( "current" ) !== rid) {
                      po.chat( "Error found, " + att.get( "current" ) + " at " + nm + " had incorrect rowID of - " + attrib[ 0 ].get( "current" ) + ". Fixed", Earthdawn.whoFrom.apiWarning )
                      attrib[ 0 ].set( "current", rid);
                      ++issues;
                    }
                  } // end _name
                  if( orig.indexOf( rid ) === -1 )  {   // This is a list of all unique rowID's we found.
                    orig.push( rid );
                    lcase.push( rid.toLowerCase() );
                  }
                  if( rid === rid.toLowerCase() || rid === rid.toUpperCase() ) {    // This rowID is all the same case which is almost certainly got stored wrong.
                    let t = needFixi.indexOf( rid );
                    if( t == -1 ) {
                      t = needFixi.push( rid ) -1;
                      needFix.push( [] );
                    }
                    needFix[ t ].push( att );
                  }
                } else {   // end repeating. start not repeating.
                  if( att.get( "name" ) === "edition" ) {   // Also don't do the tests below of edition and/or sheetVersion is zero.
                    if( first ) {      // This is called automatically by firstRun the very first time the API is detected and furthermore this is the first character sheet processed. 
                                       // state sheetVersion is almost certainly not correct and state edition might not be correct ether.
                                       // So if there is an edition that is not zero, set the state values using what we find in this first record.
                      if(( !state.Earthdawn.edition || state.Earthdawn.edition === 4) && att.get( "current" ) 
                            && (att.get( "current" ) != "0") && (att.get( "current" ) != Earthdawn.safeString( state.Earthdawn.edition ))) {
                        po.chat( "Note: state.Earthdawn.edition was: '" + state.Earthdawn.edition + "' and was different from the first characters (" 
                            + Earthdawn.getAttrBN( po.charID, "character_name", "" ) + ") edition, so state.Earthdawn.edition was set to: " + att.get( "current" )  
                            + ". If this is not desired, change edition using Parameters / Sheet / Special Functions / Do SF / GM Special Commands / Change Edition.", Earthdawn.whoFrom.apiWarning )
                        state.Earthdawn.edition = att.get( "current" );
                      }
                      if(( state.Earthdawn.sheetVersion == 0.0) && att.get( "max" ) && ( parseFloat( att.get( "max" )) != 0.0 )) {
                        po.chat( "Note: state.Earthdawn.sheetVersion was: '" + state.Earthdawn.sheetVersion + "' and was different from the first characters (" 
                            + Earthdawn.getAttrBN( po.charID, "character_name", "" ) + ") sheetVersion, so state.Earthdawn.sheetVersion was set to: " + att.get( "max" )  
                            + ". If this is incorrect it should eventually fix itself.", Earthdawn.whoFrom.apiWarning )
                        state.Earthdawn.sheetVersion = parseFloat( att.get( "max" ));
                      }
                    } else {      // Not first. We want to set everything to use the systems edition and sheetVersion.
                      if( state.Earthdawn.edition && (att.get( "current" ) != Earthdawn.safeString( state.Earthdawn.edition ))) {
                        log( "was game edition " + att.get( "current" ) + "  new " + Earthdawn.safeString( state.Earthdawn.edition ));
                        Earthdawn.setWithWorker( att, "current", Earthdawn.safeString( state.Earthdawn.edition ));
                      }
                      if(( state.Earthdawn.sheetVersion ) && (att.get( "max" ) != Earthdawn.safeString( state.Earthdawn.sheetVersion ))) {
                        log( "Warning, sheet edition is still " + att.get( "max" ) + "  should be " + Earthdawn.safeString( state.Earthdawn.sheetVersion ));
...
//log( "needFix[ " + i + " ] length is " + needFix[ i ].length);   log( "new RowID " + f);
                    for( let j = 0; j < needFix[ i ].length; ++j ) {      // For each attribute that has the rowID we are correcting.
                      ...
                      pre  = po.buildPre( Earthdawn.repeatSection( 3, nm), f);     // This is the "good" rowID.
//log( nm);
              ...
              let problems = this.abilityRebuild( [ "abilityRebuild", ssa.includes( "batchFix" ) ? "batchFix" : ( ssa.includes( "batch" ) ? "batch" : "forceTest" ), 
                    ssa.includes( "forceNameRefresh" ) ? "forceNameRefresh" : "Standard" ]);
              ...
              if( !silent ) {
                ...
                if( !issues && !problems ) this.chat( Earthdawn.getAttrBN( this.charID, "character_name", "" ) + ": Verify / Fix: No issues found." );
              }
            } catch(err) { Earthdawn.errorLog( "Debug RepSecFix error caught: " + err, po ); }
          } break;    // end repsecfix
          case "repsecbatchfixall": 
            batchFix = true;     // don't ask before fixing. 
          case "repsecbatch":                  // Run the previous routine for all characters (instead of just one), Ask before fixing anything.    These are called for SF func GM commands. 
              ...
              const fixBurndown = () => {     // function that will process the next element of the queue
                try {
                  if( charQueue.length > 0 ) {
                    ...
                    log( "Verifying " + c.get( "name" ));
                    po.charID = c.get( "_id" );
                    po.Debug( [ "Debug", "repSecFix", "silent", batchFix ? "batchFix" : "batch", ssa.includes( "forceNameRefresh") ? "forceNameRefresh" : "Standard" ] );
...
/*
            ...
            function btest( obj ) {
              let ret = 0;
              if( obj != undefined ) {
                let c = obj.get( "current" ), m = obj.get( "max" );
                if( c != undefined ) ret += (typeof c === "string") ? c.length : c.toString().length;
                if( m != undefined ) ret += (typeof m === "string") ? m.length : m.toString().length;
              }
              return ret;
            }
            let attributes = findObjs({ _type: "attribute", _characterid: this.charID });
            log( attributes.length + " attributes found" );
            let sorted = _.sortBy( attributes, function( att ){ return btest( att ) });
            function atest( ind ) {
              let obj = sorted[ ind ];
              return btest( obj );
            }
            log( sorted.length + " after sorting" );
//            log( "shortest length " + sorted[ 0 ][ "current " ].length + sorted[ 0 ][ "max" ].length );
            let step = Math.floor( sorted.length / 10);
            for( let i  = 0; i < sorted.length; i += step )
              log( i + " length " + atest( i ) + "     " + JSON.stringify( sorted[ i ]));
            log( "longest length " + atest( sorted.length -1 ));
            
            let cleanind = 0, cleanchar = 0, done = false;
            while( !done ) {
              let obj = sorted.pop();
              ++cleanind;
              let totlen = btest( obj );
              cleanchar += totlen;
              log( "removed " + obj.get( "name" ) + "  current length " + obj.get( "current" ).length + " max length " + obj.get( "max" ).length); 
              obj.remove();
              if( totlen < 32 ) done = true;
            }
            log( "removed " + cleanind + " objects with " + cleanchar + " total characters. " + sorted.length + " attributes remaining"  );
*/
...
/*
              log( i + " length " +  (sorted[ i                ].get( "current " ).length ) 
                                   + (sorted[ i                ].get( "max" ).length )); 
            log( "longest length " + (sorted[ sorted.length -1 ].get( "current" ).length ) 
                                   + (sorted[ sorted.length -1 ].get( "max" ).length ));

            for( let i  = 0; i < sorted.length; i += step )
              log( i + " length " +  ((("current" in (sorted[ i                ])) ? sorted[ i                ][ "current " ].length : 0) 
                                   + (("max"      in (sorted[ i                ])) ? sorted[ i                ][ "max" ].length : 0))); 
            log( "longest length " + ((("current" in (sorted[ sorted.length -1 ])) ? sorted[ sorted.length -1 ][ "current" ].length : 0) 
                                   + (("max"      in (sorted[ sorted.length -1 ])) ? sorted[ sorted.length -1 ][ "max" ].length : 0 )));
*/
//              log( po.buildPre( "ENC", "abcdefghIJKLMN" ));
//              log( po.buildPre( "abc", "abcdefghIJKLMN" ));
...
// cdd see if we can get this to not happen on skills!
        ...
        if( (objarr.length || 0) > 0) {
          if( flag === 1 )    // list
            ...
            _.each( objarr, function ( obj ) {
              ...
              if( edParse.TokenGet( stat ).length > 0 )
                ret = true;
            }); // End for each Token
          ...
          if( this.bFlags & Earthdawn.flagsTarget.Highest ) {    // If target is looking for highest one in the list, find it and then continue with the rest of the commands (we don't do anything for each target, we just find the one target).
            ...
            for( let i = 0; i < this.targetIDs.length; i++ ) {
              ...
              if( x !== undefined ) {
                ...
                if(( "spiritRating" in x) && x[ "spiritRating"] && x[ "spiritRating" ] > highsr )
                  highsr = x[ "spiritRating" ];
            } }
            if( highindex !== undefined ) {
              ...
              if( highnum )
                this.misc[ "targetNum" ] = Math.max( highnum, ( "targetNum" in this.misc ) ? this.misc[ "targetNum" ] : 0);
            }
            if( highsr > 0 )
              this.misc[ "highSR" ] = highsr;
            ...
            this.misc[ "queueRolls" ] = true;
            let saveMisc = JSON.stringify( this.misc ),
                rq = [];
            for( this.indexTarget = 0; this.indexTarget < this.targetIDs.length; this.indexTarget++ ) {
              ...
              if( this.tokenIDs.length < 2 || ( this.indexTarget % this.tokenIDs.length) == (this.indexToken || 0 )) {
                ...
                newParse.misc = JSON.parse( saveMisc );    // Make certain we get rid of stuff from the last loop.
                let x =  newParse.TargetCalc( newParse.targetIDs[ newParse.indexTarget ], newParse.bFlags );
                if( x !== undefined ) {
                  newParse.misc[ "targetNum" ] = (( "targetNum" in newParse.misc) ? newParse.misc[ "targetNum" ] : 0) + x[ "val" ];
                  newParse.misc[ "targetName" ] = x["name"];
                  if(( "spiritRating" in x) && x[ "spiritRating"] )
                    newParse.misc[ "highSR" ] = x[ "spiritRating" ];
                  else
                    newParse.misc[ "highSR" ] = undefined;
                  newParse.ParseLoop();
                  this.bFlags = newParse.bFlags;      // bFlags and "misc" are the only two things we want to save from that instance, the rest can be discarded. 
                  rq.push( JSON.stringify( newParse[ "misc" ] ));     // save the state for a roll we will be making later. 
            } } }
            ...
            this.rollQueue = rq;
            if( this.rollQueue.length > 0 ) {    // For each roll we want to make, we have collected the information we need in this.misc.   Now we launch the first roll (subsequent rolls will be launched by rollFormat()
              this.indexTarget = 0;
              this.misc = JSON.parse( this.rollQueue.shift() );
              this.rollPost();
        } } }
...
//log( "willeffect dmg roll");
// cdd note, we also want a keyword for this, not only willeffect.   Or possibly a keyword for the else below. 
            this.indexTarget = 0;
            this.rollPre( [ "Roll"] );
            ...
            for( this.indexTarget = 0; this.indexTarget < this.targetIDs.length; this.indexTarget++ ) {
              if( this.indexTarget > 0 )
                this.misc = JSON.parse( saveMisc );   // restore this.misc to what it was before the first loop through.
              this.rollPre( [ "Roll"] );
              this.rollQueue.push( JSON.stringify( this[ "misc" ] ));     // save the state for a roll we will be making later. 
            }
            if( this.rollQueue.length > 0 ) {    // For each roll we want to make, we have collected the information we need in this.misc.   Now we launch the first roll (subsequent rolls will be launched by rollFormat()
              this.indexTarget = 0;
              this.misc = JSON.parse( this.rollQueue.shift() );
        ...
        if( typeof ssa === "string" ) {     // Being called from one of two places in Roll().
          ...
          if (ssa[ 3 ] === "Set" ) {    // If not Set, it is Clear, so just leave the empty string.
            ...
            this.chat( "Special Effects for " + Earthdawn.codeToName( Earthdawn.safeString( ssa[ 1 ] )) + " "
                + Earthdawn.safeString( Earthdawn.getAttrBN( this.charID, this.buildPre( ssa[ 1 ], Earthdawn.safeString( ssa[ 2 ])) + "Name")) + " Set." );
          } else
            this.chat( "Special Effects for " + Earthdawn.codeToName( ssa[ 1 ] ) + " "
                  + Earthdawn.getAttrBN( this.charID, this.buildPre( ssa[ 1 ], ssa[ 2 ]) + "Name") + " Cleared." );
          this.setWW( this.buildPre( ssa[ 1 ], ssa[ 2 ] ) + "FX", to );
        } else
...
//log( "ca " + ca + "   ret " + ret);
            return ret;
              ...
              // Return fallout
// cdd
// This is not called yet. wait until METracker is done for this to integrate with it.
...
*/
// Note don't get final value, get raw value and then look for both character sheet and token mods. If ether appears, apply the moe. but don't do both.
/*
...
*/
      ...
      return ret;
          ...
          //    {"Cid1":[{"token":"Tid1","character":"Cid1"},"token":"Tid2","character":"cid1"],"-O1IHlYDchqYzWPQuJNM":[{"token":"-O1IHoF82AvcOIdVmnUh","character":"-O1IHlYDchqYzWPQuJNM"}]}
              ...
              while ( !err && begin != -1 ) {
                ...
                if( end < begin )
                  ...
                  if( attrib.startsWith( "repeating_" ) && tst.startsWith( Earthdawn.repeatSection( 3, attrib) + "_" ))   // if the original attrib has a listed repeating section and rowID, and the derived numbers are from the same section, add the prefix to all the derived numbers.
                    tst = this.buildPre( attrib ) + tst.slice( tst.indexOf( "_" ) +1);
...
//            Earthdawn.errorLog( "ParseObj.Karma() error, SSA is not a string in version 2.0", this );
          ...
          if( kdice > 0 ) {   // Are we spending more than zero karma?
            let kstep = Earthdawn.parseInt2( Earthdawn.getAttrBN( this.charID, "KarmaStep", "4" )),
                currKarma,
                bToken = ( this.tokenInfo.type === "token" ),           // We have both a token and a character object.
                npc = Earthdawn.getAttrBN( this.charID, "NPC", "1" ),
                bMook = !((npc == Earthdawn.charType.pc) || (npc == Earthdawn.charType.npc));
            if( bToken && ( bMook || !(this.tokenInfo.tokenObj.get( "bar1_link" )) )) {    // If it is a mook, get it from the token. 
            ...
            if( kdice > currKarma ) {
              let bad = true;
              if( !bMook && currKarma < 1 && Earthdawn.getAttrBN( this.charID, "Karma", 0) != 0) {     // before saying anything about not having enough karma, check that the token is linked properly.
                log( Earthdawn.timeStamp() + "Note: Karma() thinks the token is not linked correctly, since the character sheet has karma, but the token does not. Attempting to relink." )
                this.LinkToken( [ "forceLink", this.tokenInfo.tokenObj.get( "_id" ) ] );
                currKarma = Earthdawn.parseInt2( this.tokenInfo.tokenObj.get( "bar1_value" ));
                if( kdice <= currKarma ) {    // and see if that helped.
                  bad = false;      // fixed, and do have karma to spend.
                  log( "Linking seems to have fixed it");
                } else
                  log( "Linking does not seem to have fixed it");
              }
              if( bad ) {
            ...
            if( kdice > 0 ) {
              ...
              if( bToken && ( bMook || !(this.tokenInfo.tokenObj.get( "bar1_link" )) ))     // If it is a mook, set it on the token. 
...
//                        this.chat( this.tokenInfo.name + " spent " + kdice + " karma." );
        ...
        // keyword( pre, canidate1, canidate2, ...)
        // look in (pre + "Special") for a list of keywords this item has.
        // Then look for all the other arguments within the keyword list. 
        // return a bitmap of each keyword found.
        // if first and third canidates found, then would return 0x0101;
    this.keyword = function( pre ) {
      'use strict';
      try {
        arguments[ 0 ] = Earthdawn.getAttrBN( this.charID, pre + "Special", "None") + (("keyword" in this.misc) ? "," + this.misc[ "keyword" ] + "," : "");      // change the first argument from pre to the value of special. 
        return Earthdawn.keywordCheck( ...arguments);                                     // pass the changed arguments, expanded from an object to be seperate parameters.
      } catch(err) { log( Earthdawn.timeStamp() + "ED.keyword error caught: " + err ); }
    } // end keyword
          ...
          // if ssa includes "SetTokenOnly", then set the token only, don't link it.
          // ssa including "autoLink" works, but is now depretiated. It was called when the user changed 'represents' in token settings, but the problem was that 
          // the event fired while the user was still on the token settings screen this routines linking was overwritten if they pressed Save settings on that screen.
          // Everything was OK if they pressed cancel instead and then entered the token screen again and saw the new values, but users might miss the message telling them that.
    this.LinkToken = function( ssa ) {
      'use strict';
      let edParse = this;
      try {
        let setTokenOnly = ssa.includes( "SetTokenOnly" ),    // standard modifiers are: Standard|SetTokenOnly|LinkOnly   of which only setTokenOnly really does anything different.
            linkOnly     = ssa.includes( "LinkOnly" ),
            forceLink    = ssa.includes( "forceLink" ),
            autoLink     = ssa.includes( "autoLink" ),
            buttonLink   = ssa.includes( "buttonLink" );      // on change represents gives a chat window button.
        if( this.charID === undefined ) {
            this.chat( "Error! Trying to Link Token when don't have a CharID.", Earthdawn.whoFrom.apiError );
            return;
        }
        if( !forceLink && !buttonLink && this.tokenAction ) {
            this.chat( "Error! Linktoken must be a character sheet action, never a token action.", Earthdawn.whoFrom.apiError);
            return;
        }
        if( !forceLink && !autoLink && !buttonLink && (this.edClass.msg.selected === undefined )) {
        ...
        let s = pc ? state.Earthdawn.linking.PC : state.Earthdawn.linking.NPC,
            count = 0;
        if( !forceLink && !autoLink && !buttonLink && (pc == Earthdawn.charType.mook) && (this.edClass.msg.selected.length > 1 ))
        {
            this.chat( "Error! You can't link more than one token to a non-mook character!", Earthdawn.whoFrom.apiWarning );
            return;
        }
        let sName = "",
            CharObj = getObj( "character", edParse.charID ),
            sels = (forceLink || autoLink || buttonLink) ? [ {_id: ssa[ ssa.length -1 ] } ] : edParse.edClass.msg.selected;

        _.each( sels, function( sel ) {   // This is ether the token passed in ssa[ 1 ] or it is messages selected.
          'use strict';
          let TokenObj = getObj("graphic", sel._id);
          if (typeof TokenObj === 'undefined' )
              return;

          if( !setTokenOnly && !autoLink )    // if we did not specifically say not to link it, and if we are not called because we detected somebody linking it, link it.
          ...
          if( !linkOnly && s ) {
            ...
            if( s.compact_bar != undefined )      Earthdawn.set( TokenObj, "compact_bar", s.compact_bar, null );
          }
          ...
          if( typeof CharObj != 'undefined' ) {
            ++count;
            if( autoLink )    // We are called with autoLink, so only set the character to use this token by default if there is no other token set.
              CharObj.get("_defaulttoken", function( dt ) {
                'use strict';
                if( !dt ) {   // only if the current default token is empty.
                  setDefaultTokenForCharacter( CharObj, TokenObj);
                  if( !CharObj.get( "avatar" ))
                    Earthdawn.set( CharObj, "avatar", TokenObj.get( "imgsrc" ) );
                }
              });
            else if( !setTokenOnly ) {    // if linking without autoLinking, then set the character to use this token.
              setDefaultTokenForCharacter( CharObj, TokenObj);
              if( !CharObj.get( "avatar" ))
                Earthdawn.set( CharObj, "avatar", TokenObj.get( "imgsrc" ) );
          } }
        ...
        if( count == 0) {
          ...
          if( autoLink )
            this.chat( "***Importaint The API has linked the token. Press 'Cancel' in 'Token Settings' instead of 'Save Settings' in order to avoid overwriting the changes. ***", Earthdawn.whoTo.player | Earthdawn.whoFrom.noArchive );
        }
...
//log("this.Lookup " + JSON.stringify(ssa));
        ...
        function luResult( what, po )  {
          ...
          if( what !== undefined )
            ...
            case 3: if( what ) 
                        po.misc[ "targetNum" ]  = ( po.misc[ "targetNum" ] || 0)  + what;  break;
            case 4:
...
//log(" parsing the defensive " + Earthdawn.getAttrBN( this.charID, "Misc-DefStance-Penalty", "0" ));
// Do I need to look at T_Defensive in html for the values?
...
/*                        // note: visionbased is depreciated. 
              case "visionbased": { // This action is Vision Based. If Vision Penalties are in effect, subtract them from this result.
                ...
                if( tstep > 0 ) {
                  lu -= tstep;
                  this.misc[ "VisionBased" ] = (tstep == 2) ? "Partial" : "Full:;    // VisionBased depreciated 8/22
                }
              } break;
*/
              case "notvisionbased":
...
//log("after ssa[i] " + ssa[i] + " lu is "+lu);
    ...
    this.MarkerSet = function( ssa )  {
      ...
      let po = this;
      try {
//og("this.MarketSet ssa is : " + JSON.stringify(ssa));
        if( this.tokenInfo === undefined ) {
...
//og(mi); log( JSON.stringify( valid));
...
//og( "index " + index + "   adjust " + adjust);
          ...
          if( ssa[ 0 ] !== "sheetDirect" && !dupChar && ( attrib != undefined ) && !(mook && attrib === "condition-Health" )) {
            ...
            if( setObj && setObj.attrib && (typeof setObj.attrib === "number" ))
              setObj.attrib = setObj.attrib.toString();       // Make the attrib a string, which is what sheetworkers set to.
          ...
          if(( attrib === "condition-Health") && ( ssa[ 0 ] === "d")) {   // Health has changed from the damage routine.
...
// Oct 23  obsolete. moved to sheetworker.
            ...
            break;
*/
          case "macrocreate":   // Create, and Delete do all macros, Refresh is a global delete followed by a global create. AutoCreate is create only if a macro of that name does not already exist (and is run on API startup).
            try {
              let dowhat =  (ssa.includes( "AutoCreate" ) ? 0x10 : 0);
              if( !ssa.includes( "firstRun" ) && !dowhat && !(this.edClass && this.edClass.msg && this.edClass.msg.playerid && playerIsGM( this.edClass.msg.playerid )))
                this.chat( "Error! Only GM is allowed to run MacroCreate.", Earthdawn.whoFrom.apiError);
              else {
                ...
                macroDetail = function( macName, macText, macTokenAction )  {
                  'use strict';
                  try {
                    let macObj = findObjs({ _type: "macro", name: macName });
                    if( macObj && !dowhat)    // If we are not in autoCreate, delete any macro found so they can be created again.
                      for( let i = 0; i < macObj.length; ++i )
                        macObj[ i ].remove();
                    if( !(dowhat && macObj ))   // unless we are in autoCreate and a macObj was found, create a new macro. 
                      macObj = createObj("macro", {
                        _playerid:      po.edClass.msg.playerid,
                        name:           macName,
                        action:         macText,
                        visibleto:      "all",
                ...
                if( ssa.includes( "Delete" ) || ssa.includes( "Refresh" )) {    // get rid of all macros that reference Earthdawn.
                ...
                if( ssa.includes( "Create" ) || ssa.includes( "Refresh" ) || dowhat) {    // create the Earthdawn macros.
                  macroDetail( "Roll-Public", "!edsdr~ ?{Step|0}~ ?{Bonus or Karma Step|0}~ for ?{reason| no reason}", false );
                  macroDetail( "Roll-Player-GM", "!edsdrGM~ ?{Step|0}~ ?{Bonus or Karma Step|0}~ for ?{reason| no reason}", false );
                  macroDetail( "Roll-GM-Only", "!edsdrHidden~ ?{Step|0}~ ?{Bonus or Karma Step|0}~ for ?{reason| no reason}", false );
                  macroDetail( "NpcReInit", "!Earthdawn~ rerollnpcinit", false );
                  macroDetail( "ResetChars", "!Earthdawn~ Misc: ResetChars: ?{(de)buffs Only or Full (including damage and karma) Reset|Mods Only|Full}", false );
                  macroDetail( "Token", "!edToken~", false );
                  macroDetail( "Dur-Track", "!Earthdawn~ ChatMenu: DurationTracker : ?{Name of the Effect|Effect}: ?{How many rounds ?|1}", false ); //new in v2 - Records duration of an effect in the turn tracker
//                macroDetail( "Test", "!edToken~ %{selected|Test}", true );     // Leave Test in here. It can be uncommented when it is actually needed.

                  macroDetail( "Attrib", "!Earthdawn~ ChatMenu: Attrib", true );
                  macroDetail( "Init", "!edToken~ %{selected|Dex-Initiative-Check}", true );
                  macroDetail( "KnockD", "!edToken~ %{selected|Knockdown}", true );
                  macroDetail( "Status", "!Earthdawn~ ChatMenu: Status", true );
                  macroDetail( "Strain", "!edToken~ !Earthdawn~ setToken: @{target|token_id}~ Damage: ?{Damage)|1}: Strain: NA", true);
                  macroDetail( Earthdawn.constantIcon( "karma" ) + "Karma-R",  "!edToken~ !Earthdawn~ Reason: 1 Karma Only~ ForEach~ Karma: 1~ Roll: 0", true );
                  macroDetail( Earthdawn.constantIcon( "karma" ) + "Karma-T",  "!edToken~ !Earthdawn~ ForEach~ marker: KarmaGlobalMode :t", true );
                  macroDetail( Earthdawn.constantIcon( "T" )     + "-Talents", "!Earthdawn~ ChatMenu: Talents", true );
                  macroDetail( Earthdawn.constantIcon( "SK" )    + "-Skills",  "!Earthdawn~ ChatMenu: Skills", true );
                  macroDetail( Earthdawn.constantIcon( "WPN" )   + "-Damage",  "!Earthdawn~ ChatMenu: Damage", true );
//                    macroDetail( "•Status", "!edToken~ !Earthdawn~ ForEach~ Marker: ?{@{selected|bar2|max}}", true );
//                    macroDetail( "⚡Cast", "!edToken~ %{selected|SP-Spellcasting}", true );   /* high voltage: 9889;
//              macroDetail( Earthdawn.constantIcon( "Target" ) + "Clear-Targets", "!Earthdawn~ charID: @{selected|character_id}~ ForEach~ TargetsClear", true);
//              macroDetail( Earthdawn.constantIcon( "Target" ) +   "Set-Targets", "!Earthdawn~ charID: @{selected|character_id}~ Target: Set", true);
                if( !dowhat )
                  this.chat( "Macros created (look in collections tab).", Earthdawn.whoTo.gm | Earthdawn.whoFrom.api );
              } }
...
//log(rt);
                ...
                case -4: {    // There is a Karma Ritual Talent. Find it and use it's rank.
                  ...
                  _.each( attributes, function (att) {
                    if( att.get( "name" ).endsWith( "_Special" ))
                      if( Earthdawn.keywordCheck( att.get( "current" ), "Karma Ritual" )) {
                        if( add === undefined ) {
                          let nm = att.get( "name" );
                          add = Earthdawn.getAttrBN( po.charID, po.buildPre( Earthdawn.repeatSection( 3, nm), Earthdawn.repeatSection( 2, nm)) + "Effective-Rank", "0" );
                        } else
          ...
          case "resetchars":      // reset all selected tokens to full health, karma, Recovery Tests, and no modifications or status markers.
            try {       // Note, when this routine enters, we will not have this.charID or TokenInfo set. So be careful what other functions get called.
              let lst = this.getUniqueChars( 1 ),     // All selected Token IDs and Character IDs, grouped by character. 
                  full = ssa.includes( "Full" ),
                  nlist = "";
              if( this.charID && _.isEmpty( lst )) {      // No tokens selected, but this is done from the Special Functions menu, so do all tokens for this character.
                let tkns = findObjs({ _type: "graphic",  _subtype: "token", represents: this.charID }),
                    cid = this.charID,
                    arr = [];
                _.each( tkns, function( TokObj ) {
                  arr.push( { token: TokObj.get( "_id" ), character: cid});
                }) // End ForEach Token
                lst = _.groupBy( arr, "character" );
              } // end no tokens selected
              for( let c in lst ) {     // For each unique character, do the following.
                this.charID = c;
                ...
                let karma = kObj.get( "max" ),
                    ...
                    markers = "";
                if( full ) {    // karma and recovery tests reset to max, no wounds or damage.
                ...
                this.setWW( "Dex-Mods", 0);   // reset all modifications (buffs and debuffs) to zero.
                ...
                for( let t in lst[ c ] ) {    // For each unique token found, do the following.
                  ...
                  if( TokenObj ) {
                    ...
                    if( full ) {
                      let npc = Earthdawn.getAttrBN( this.charID, "NPC", "1" ),
                          bMook = !((npc == Earthdawn.charType.pc) || (npc == Earthdawn.charType.npc )); // We only need to process tokens if they are mooks or not linked correctly. Otherwise everything is linked to the character.
                      if( bMook || !(this.tokenInfo.tokenObj.get( "bar1_link" )))    Earthdawn.set( TokenObj, "bar1_value", karma);    // karma
                      if( bMook || !(this.tokenInfo.tokenObj.get( "bar2_link" )))    Earthdawn.set( TokenObj, "bar2_value", 0);        // wounds
                      if( bMook || !(this.tokenInfo.tokenObj.get( "bar3_link" ))) {
                        Earthdawn.set( TokenObj, "bar3_value", 0 );         // damage
                        Earthdawn.set( TokenObj, "bar3_max",   unc );       // Unconscious rating
                  } } }  // end tokenObj
              } }
              this.chat( "Reset characters: " + ( nlist ? nlist.slice( 0, -2 ) : "None (no selected tokens)" ) + ".", Earthdawn.whoTo.gm | Earthdawn.whoFrom.noArchive );
            } catch(err) { Earthdawn.errorLog( "ED.funcMisc.ResetChars error caught: " + err, po ); }
            break;    // end resetchars
          case "setadjust":
              ...
              if( !playerIsGM( this.edClass.msg.playerid ) )
                ...
                switch( Earthdawn.safeString( ssa[ 2 ] ).toLowerCase() ) {
                  case "cursedlucksilent": {
                    let t = parseInt( ssa[ 3 ], 2 );    // This is a binary value, so use the supplied parseInt instead of Earthdawn.parseInt2()
                  ...
                  case "edition": {
                    ...
                    if( ssa[ 3 ].slice( -4 ) === "1879") {
                      state.Earthdawn.game = "1879";
                      state.Earthdawn.g1879 = true;
                      state.Earthdawn.gED = undefined;
                    } else if ( ssa[ 3 ].slice( -2 ) === "ED" ) {
                      state.Earthdawn.game = "ED";
                      state.Earthdawn.g1879 = undefined;
                      state.Earthdawn.gED = true;
                    } else
                      this.chat( "Error! state parameters not correctly formed: " + ssa.toString(), Earthdawn.whoFrom.apiError );
...
// Dead code as of changes to JSON.
                  ...
                  case "logstartup":
                    ...
                    break;
                  case "logcommandline":
                    state.Earthdawn.logCommandline = Earthdawn.parseInt2( ssa[ 3 ] ) ? true: undefined;
                    logging = true;
                    break;
                  case "logmsg":
                    state.Earthdawn.logMsg = Earthdawn.parseInt2( ssa[ 3 ] ) ? true: undefined;
                    logging = true;
                    break;
                  case "nopileondice": {
...
//                    break;
              ...
              _.each( attributes, function (att) {
                ...
                if( nm === "API" )
                  ...
                  if( nm.endsWith( "_Name" ))     // No matter what, make sure we get the RowID set for this section.
                    ...
                    if ( code !== "SP" ) {    // skip if it is SP, we don't do those token actions.
                      nmn = Earthdawn.getAttrBN( cID, po.buildPre( code, rid ) + lu, "" );
                      if( !noApi && cbs == "1" )   // If we are going toAPI, and combatslot is one, make a token action.
                        Earthdawn.abilityAdd( cID, symbol + nmn, "!edToken~ %{selected|" + po.buildPre( code, rid ) + "Roll}" );
                    }
                  } // End Token Action maint.
                }
              }); // End for each attribute.
    ...
    this.mookWounds = function() {
      ...
      try {
        ...
        // For chat messages created with htmlBuilder. You need a section and a body. Use this.newSect and Earthdawn.newBody to get them.
    this.newSect = function ( size ) {
      'use strict';
      try {
        let is1879 = false;
        if( "charID" in this ) {
          if( Earthdawn.getAttrBN( this.charID, "edition", "4" ) === "-1" )
            is1879 = true;
        } else {
          log( Earthdawn.timeStamp() + "Earthdawn.newSect warning. newSect without charID: " + size );
          if( !state.Earthdawn.logCommandline ) log( this.msg );
          log( this );
        }

        return new HtmlBuilder( "", "", {
                class: (( size && size.toLowerCase().startsWith( "s" )) ? "sheet-rolltemplate-sectSmall" : "sheet-rolltemplate-sect") 
                + (is1879 ? " sheet-rolltemplate-1879" : " sheet-rolltemplate-Earthdawn")})
      } catch(err) { log( Earthdawn.timeStamp() + "Earthdawn.newSect error caught: " + err ); }
    };
      ...
      try {
        ...
        if( chatMsg.length > 0 )
          this.chat( "!Earthdawn" + chatMsg);
            ...
            // ParseObj.rollPre ( ssa )
            // prepair a Roll for selected token.
            //      ssa[] - Step modifiers.
    this.rollPre = function( ssa )  {
      'use strict';
      try {
        ...
        this.misc[ "recipients" ] = this.WhoSendTo();
        let dice,
          step = this.ssaMods( ssa ),   // Add any mods to roll.
          dark = this.getValue( "condition-Darkness" );     // See if a darkness penalty has already been applied to Adjust-All-Tests-Auto
        if( dark > 0 && ("NotVisionBased" in this.misc ))      // If action is Not Vision Based, then back out the penalty. 
          step += dark;
        ...
        if( step == 0 && "reason" in this.misc && this.misc[ "reason" ].toLowerCase().indexOf( " only" ) !== -1 ) {    // One karma only and one devpoint only are allowed to make step zero rolls.
            // empty statement to allow an else if.
        } else if ( step < 1 ) {
          if( init )
        ...
        this.misc[ "finalStep" ] = (("moreSteps" in this.misc) && ("step" in this.misc)) ? this.misc[ "step" ]  : step;     // "step" and "finalStep" both might be of the form "30 (3)".
        this.misc[ "effectiveStep" ] = (("effectiveStep" in this.misc) ? this.misc[ "effectiveStep" ] : 0 ) + step;         // if finalStep is of form 30 (3), then variable step and effectiveStep will be 30.
        ...
        this.misc[ "natural" ] = ((this.misc[ "targetNum" ] || 0) == 0)     // The roll does not have a target number, and does not have several other conditions (such as being a recovery test) that need special processing.
              && !(this.misc[ "recipients" ] & Earthdawn.whoTo.player )
        ...
        if( !( "queueRolls" in this.misc ))   // We might be queueing several rolls up, or we might want to just roll it right now.
          this.rollPost();
      } catch(err) { Earthdawn.errorLog( "ED.rollPre() error caught: " + err, this ); }
    } // End ParseObj.rollPre(ssa)



            // ParseObj.rollPost ()
            // Perform a Roll for selected token and interprite the results.
    this.rollPost = function( ssa )  {
      'use strict';
      try {
//        if( !( "dice" in this.misc ) || !( "recipents" in this.misc)) {
//        }
        if ( this.misc[ "natural" ] ) {   // A natural roll is one without a callback. It just goes out, and the raw results are returned.  Done for rolls without target numbers, and rolls that are sent to both player and gm, but not public.
          this.misc[ "roll" ] = "[[" + this.misc[ "dice" ] + "]]";   // The actual roll is done as an inline roll when this is evaluated inside of rollformat. 
          this.rollFormat( this.misc[ "recipients" ] );
          ...
          sendChat( "player|" + po.edClass.msg.playerid, "/r " + po.misc[ "dice" ], function( ops ) {
            'use strict';
                  // NOTE THAT THIS IS THE START OF A CALLBACK FUNCTION
            if( ops.length !== 1 )
              Earthdawn.errorLog( "Earthdawn.js program warning! ops returned length of " + ops.length, po);
            ...
            po.misc[ "showResult" ] = (( "StyleOverride" in po.misc ? po.misc[ "StyleOverride" ] : state.Earthdawn.style) != Earthdawn.style.VagueRoll)
                  ...
                  || ((po.misc[ "targetNum" ] || 0) == 0) || po.misc[ "init" ]
                  || ( po.misc[ "recipients" ] == Earthdawn.whoTo.gm);    // going to gm only.

                      // We have a target number. Count successes.
            if( "targetNum" in po.misc && po.misc[ "targetNum" ] !== undefined ) {
              po.misc[ "targetNum" ] = Math.max( po.misc[ "targetNum" ], 2 );     // target number may not be less than 2.
              let res = po.misc[ "result" ] - po.misc[ "targetNum" ];
              if( res < 0 ) {   // Failed
                ...
                if(( "Special" in po.misc) && Earthdawn.keywordCheck( po.misc[ "Special" ], "Knockdown" )) {
...
// cdd todo.  Suggestion, when a hit, if they have Active Defense for that, link it.
// If this talent targets the targets defenses, check all of targets talents, knacks, and skills to see if they have an active defnese., if so make a cButton.

                let par = ((( "Special" in po.misc) && ( "targetName" in po.misc )) ? Earthdawn.keywordCheck( po.misc[ "Special" ], 
                              "CursedLuck", "CorruptKarma", "SPL-Spellcasting" ) : 0)   // We need rollESbutton to memorize hits on spellcasting tests
                        + ((( po.bFlags & Earthdawn.flagsTarget.Mask ) && (( "ModType" in po.misc ) && ( po.misc[ "ModType" ].search( /Attack/ ) != -1))  //Attack rolls
                          && (po.targetIDs[ po.indexTarget] !== undefined )) ? 0x10 : 0);     // The last bit of this keeps riposte tests from triggering this.
                  po.rollESbuttons( par );
              }   // End have successes
            }   // End we have a target number
            ...
            if( dcode !== undefined ) {   // This is a damage roll.
              ...
              for( let i = 0; i < dcode.length; ++i )
                po.misc[ "cButtons" ].push({
                    link: "!Earthdawn~ setToken: " + Earthdawn.constantButton( "at" ) + "{target|Apply damage to which token|token_id}~ Damage: " +dcode[ i ]
            ...
            if( "AfterRoll" in po.misc ) {    // After the Roll, we might do something else with the result. 
              ...
              if( sub[ 1 ].trim() === "buttonDamageBoth" || sub[ 1 ].trim() === "buttonDamageTargeted" )    // AfterRoll: buttonDamageBoth: (type of damage PA/NA)
                po.misc[ "cButtons" ].push({
                    link: "!Earthdawn~ setToken: " + Earthdawn.constantButton( "at" ) + "{target|Apply damage to which token|token_id}~ "
            ...
            if( po.misc[ "init" ] ) {      // This is an initiative roll.
              ...
              turnorder.push({ id: tID, _pageid: TokenObj.get( "pageid" ), pr: po.misc[ "result" ], secondary: ("000" + po.misc[ "result" ]).slice(-3)
                  ...
                  + ("000" + Earthdawn.getAttrBN( po.charID, "Attrib-Dex-Curr", "5" )).slice(-3) + ("000" + po.misc[ "effectiveStep"]).slice( -3)});
            ...
            po.rollFormat( po.misc[ "recipients" ], con );

            if(( --po.edClass.rollCount === 0) && (po.edClass.countFail + po.edClass.countSuccess) > 1)
              po.chat( "Group " + po.misc[ "reason" ] + " results. " + po.edClass.countSuccess.toString() + " succeeded and " + po.edClass.countFail.toString() + " failed."
                    , po.misc[ "recipients" ]);
      ...
      } catch(err) { Earthdawn.errorLog( "ED.rollPost() error caught: " + err, this ); }
    } // End ParseObj.rollPost()
          ...
          // Par tells what part of the condition for this to run was triggered.  0x01: CursedLuck, 0x02: CorruptKarma, 0x04: SPL-Spellcasting, 0x10: Attack tests.
    this.rollESbuttons = function( par )  {
      'use strict';
      let po = this;
      try {
        let suc = this.misc[ "extraSucc" ],
          tName, pGM = playerIsGM( this.edClass.msg.playerid ),
          lstart, lend;
        ...
        for( let i = lstart; i < lend; ++i ) {      // do this for each target
          if( par & 0x03 ) {      // CorruptKarma or Cursed Luck
            if( pGM ) {
              let targetChar = Earthdawn.tokToChar( this.targetIDs[ i ] );
              if( targetChar ) {
                if( par & 0x01 ) {    // Corrupt Karma
              ...
              this.chat( "Error! Only GM is allowed to do run " + (( par & 0x01) ? "CorruptKarma" : "Cursed Luck" ) + " powers!", Earthdawn.whoFrom.apiError );
            ...
            if( numTargets > 1 )
              tName = this.getTokenName( this.targetIDs[ i ] );
            if( !( par & 0x04 ) && !(this.bFlags & Earthdawn.flags.NoOppMnvr )) {    // We don't record these extra successes as something that affects damage rolls.
              makeButtonLocal( tName, "Bonus Dmg", charStr1 + "~ StoreInToken: Hit: " + this.targetIDs[ i ]
              ...
              makeButtonLocal( tName, "Opp Mnvr", charStr1 + "~ ChatMenu: OppMnvr: " + this.targetIDs[ i ], "Choose Opponent Maneuvers that might or might not be applicable to this Target." );
              ...
              _.each( attributes, function (att) {
                if (att.get("name").endsWith( "_WPN_Name" ))
                  if(( Earthdawn.getAttrBN(po.charID, att.get( "name" ).slice( 0, -5) + "_CloseCombat", "1", true) < 0) == ( cc < 0 ))    // We want to see if they have the same sign. So test (x<0 == y<0). If both are true or both is false, then true.
                    makeButtonLocal( tName, att.get( "current" ),
                        "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{" + po.charID + "|" + att.get( "name" ).slice(0, -4) + "Roll}",
                ...
                if( att.get("name").endsWith( "_Mod-Type" ) && ( att.get( "current" ) == "Damage Poison" 
                  ...
                  let pre = po.buildPre( att.get( "name")),
                    name = Earthdawn.getAttrBN( po.charID, pre + "Name", "" )
                  makeButtonLocal( tName, name, "!edToken~ " + Earthdawn.constantButton( "percent" ) + "{" + po.charID + "|" + pre + "Roll}", "Roll an Ability Damage.");
                }
              }); // End for each attribute.
              ...
              if( cflags & Earthdawn.flagsCreature.CreatureMask ) {
                if( cflags & Earthdawn.flagsCreature.GrabAndBite )
                  makeButtonLocal( tName, "Grab & Bite", charStr1 + "~ CreaturePower: " + "GrabAndBite: " + this.targetIDs[ i ],
                ...
                if( cflags & Earthdawn.flagsCreature.Hamstring )
                  makeButtonLocal( tName, "Hamstring", charStr1 + "~ CreaturePower: " + "Hamstring: " + this.targetIDs[ i ],
                ...
                if( cflags & Earthdawn.flagsCreature.Overrun )
                  makeButtonLocal( tName, "Overrun", charStr1 + "~ CreaturePower: " + "Overrun: " + this.targetIDs[ i ],
                ...
                if( cflags & Earthdawn.flagsCreature.Pounce )
                  makeButtonLocal( tName, "Pounce", charStr1 + "~ CreaturePower: " + "Pounce: " + this.targetIDs[ i ],
                ...
                if( cflags & Earthdawn.flagsCreature.SqueezeTheLife )
                  makeButtonLocal( tName, "Squeeze the Life", charStr1 + "~ CreaturePower: " + "SqueezeTheLife: " + this.targetIDs[ i ],
              ...
              if( lst !== "bad" && lst.length > 1 ) {
                ...
                for( let i = 0; i < arr.length; ++i ) {
                  ...
                  if( Earthdawn.getAttrBN( po.charID, pre + "Type", "1") == "1" ) {   // type 1 is Creature
                    ...
                    if( n.length > 0 || d.length > 0 )
                      makeButtonLocal( tName, Earthdawn.getAttrBN( po.charID, pre + "Name", "" ),
                ...
                //
                // sectMain: The main card can go to public, GM only, or player and GM. 
                // GM might get a message with target numbers and buttons.
                // playerCard: Player might get message with buttons that public did not get. 
    this.rollFormat = function( recipients, rolls )  {
      'use strict';
      let po = this;
      try {
        let pIsGM = playerIsGM( this.edClass.msg.playerid ),
...
// newBody and 4 bAppends.
// {"_tag":".body","_attrs":{"class":"sheet-rolltemplate-body"},"_css":{},"_children":
// [""
// ,{"_tag":".odd","_attrs":{},"_css":{},"_children":["<span class=\"sheet-rolltemplate-emphasis\" title=\"2d6.   Base step 8\" style=\"\"><b>Step:</b> 8</span>."]}
// ,{"_tag":".even","_attrs":{"class":"sheet-rolltemplate-ResultLine"},"_css":{},"_children":["<b>Result:</b> <span class=\"sheet-rolltemplate-chatbox2\" title=\"Rolling 2d6! = (3+6+4)\" style=\"\">13</span>"]}
// ,{"_tag":".odd","_attrs":{},"_css":{},"_children":["<b>Dice:</b> <span><img src=\"/images/quantumrollwhite.png\"> <span class='sheet-rolltemplate-diceroll sheet-rolltemplate-dice-d6'>3</span><span class='sheet-rolltemplate-diceroll sheet-rolltemplate-dice-d6 sheet-rolltemplate-dice-max'>6</span><span class='sheet-rolltemplate-diceroll sheet-rolltemplate-dice-d6 sheet-rolltemplate-dice-bonus'>4</span></span>"]}
// ,{"_tag":".even","_attrs":{"class":"sheet-rolltemplate-line-i"},"_css":{},"_children":["<b>Action:</b> <span class=\"sheet-rolltemplate-iconSmall sheet-rolltemplate-icons-action-standard\" title=\"Standard\"></span>"]}
// ]}
          ...
          try {
            ...
            if( content.indexOf( "sheet-rolltemplate-icons-" ) == -1 )
              ...
              if( !attrs )      // No existing attras, so just supply one. 
                ...
                // Message destination, overrides and exceptions section.
                // In the header we have an button to allow control of overrides. This floats at the right top of the header, so needs to be the first html.
        let whoString;
        ...
        if( bis )
          sectMain.append( "span", Earthdawn.makeButton(
              ...
              "This message is being sent to '" + whoString + (( "whoReason" in this.misc) ? "' due to " + this.misc[ "whoReason" ] : ""), "icon" ),
          ...
          sectMain.append( "span", Earthdawn.makeButton( " ! ", "!Earthdawn~ ChatMenu: RolltypeEdit: state.Earthdawn.Rolltype: : Display",
            ...
            { class:  "sheet-rolltemplate-floatRight sheet-rolltemplate-overrideOn" });
              ...
              // Roll Header & subheader.
              // In general, information provided is header information, subheader information, and strain information. 
              // Try to fit them into a few lines as possible. Subheader info might be included with header, and strain might be included with subheader, or each be their own seperate line.
              // So the first thing we do is build the subheader, so we know how long it is.
        let subhead = "vs.";         // vs. target name, TN#, and Strain x. 
        if( "targetName" in this.misc )
          subhead += " " + this.misc[ "targetName" ] + (( "targettype" in this.misc) ? "'s  " : "");
        if( "targettype" in this.misc ) {
          let e = this.misc[ "targettype" ].endsWith( "-each" );
          subhead += " " + (e ? "each " : "") + this.misc[ "targettype" ].replace( "p1p", " +1p").replace( "-each", "" );
        }
        if(( "targetNum" in this.misc) && this.misc[ "targetNum" ] != undefined )
          if ((this.bFlags & (Earthdawn.flagsTarget.Ask | Earthdawn.flagsTarget.Riposte | Earthdawn.flags.VerboseRoll))
            ...
            subhead += " Target # " + this.misc[ "targetNum" ];
          else
            this.misc[ "gmTN" ] = this.misc[ "targetNum" ];
        if( subhead.length > 4 )
          subhead += ".   ";
        else
          subhead = "";    // we had neither targetName nor targettype above. Forget the whole section.   Except that it might be used for strain below.

        let sc = this.strainCalc(), 
            header = ("reason" in this.misc) ? this.misc[ "reason" ] : "",
            shLen = subhead.length, strainButton, strainLine, strainLen = 20 + sc[ "special" ].join().length,
            bsub = ( subhead && header && ((header.length + shLen) > 28));
        if( sc && ((sc[ "strain" ] > 0 ) || sc[ "special" ] )) {
          if( sc[ "strain" ] > 0 )
            this.Damage( [ "strainsilent", "NA", sc[ "strain" ]]);    // strain, No Armor
          strainButton = Earthdawn.makeButton( sc[ "strain" ] + Earthdawn.addIcon( "strain", "s") + (sc[ "special" ] ? " " + sc[ "special" ].join() : ""),
                "!Earthdawn~ " + (( this.tokenInfo !== undefined && this.tokenInfo.tokenObj !== undefined)
                ? "setToken: " + this.tokenInfo.tokenObj.get( "id" ) : "charID: " + this.charID ) +
                "~ ChatMenu: strainChange: " + sc[ "strain" ] + ": ?{How much strain should this action have cost|" + sc[ "strain" ] + "}",
                sc[ "tooltip" ], (( "special" in sc) && (sc[ "special" ].length > 0)) ? "damage" : "dflt" );      // This button allows people to change the amount of strain that was charged.

          if(( shLen + strainLen ) < 65) {     // If strain fits on the subheader line, add it there.
            subhead += strainButton;
            shLen += strainLen;    // new length of the visible portion of the subheader (displayed text vs html code). 
          } else 
            strainLine = "<b>Strain: </b> " + strainButton;
        }
        ...
        sectMain.append( "", (("headcolor" in this.misc) ? Earthdawn.addIcon( Earthdawn.safeString( this.misc[ "headcolor" ] ).toLowerCase(), "l", this.misc[ "headcolor" ]) : "") 
              + header + ((bsub || !subhead ) ? "" : "  --  " + new HtmlBuilder( "span", subhead, { class: "sheet-rolltemplate-subheadertext" })),       // Main Header
        ...
        if( bsub )      // This is a subheader. If is long enough it needs its own line as opposed to be appended to the main header. 
          sectMain.append("", subhead, { class: "sheet-rolltemplate-subheadertext"});

                // end of header and subheader. Start of body of message.
        let modBit = 0, x, x2,
            bodyMain = Earthdawn.newBody( sectMain ),
            txt = "", txt2 = "";
        if( strainLine )    // there was no room for strain in the subheader, so we are putting it on a seperate line.
          bodyMain.append( (( ++linenum % 2) ? ".odd" : ".even"), strainLine);
        ...
        if( "ModType" in this.misc ) {    // Conditions and options that affected the step.
          let modtype = this.misc[ "ModType" ],
            extraMod = 0;
                      ...
                      // 0x10 range are all action tests.   0x11 is general action test. 0x12 is Attack test, 0x14 Attack CC. 0x18 is jumpup. 
                      // 0x20 range are all effect tests.   0x21 is general effect test. 0x22 is damage test. 0x24 Damage CC. 0x28 Init.  0x40 No modifier and Damage Poison.
          if ( modtype == "Action" )
            modBit = 0x11;
...
/*          if( "VisionBased" in this.misc ) {
            extraMod -= this.getValue( "condition-Darkness" );    // VisionBased depreciated 8/22
...
*/
          let dark = this.getValue( "condition-Darkness" );       // See if a darkness penalty has already been applied to Adjust-All-Tests-Auto
          if( dark > 0 && !("NotVisionBased" in this.misc ))      //  document the penalty. 
              txt += Earthdawn.addIcon( dark == 4 ? "vision4" : "vision", "s", 
                    "Impaired Vision: -2/-4 Penalty to all sight based tests due to Darkness, Blindness or Dazzling.", undefined, "-" + dark );
//          } // from Action() or spell.
          ...
          if(( modBit == 0x11 || modBit == 0x12 || modBit == 0x14  || ("SP-Step" in this.misc)) && this.getValue( "condition-Blindsiding" ) == "1")
            txt2 += Earthdawn.addIcon( "blindsiding", "s", "Blindsiding: The acting character is blindsiding the targeted character, who takes -2 penalty to PD and MD. Can't use shield. No active defenses.", undefined, -2 );
        ...
        if( this.indexTarget !== undefined && modBit & 0x10 ) {
          ...
          if ( targetChar ) {   // These are modifiers from the TARGET character.
            ...
            if( Earthdawn.getAttrBN( targetChar, "condition-Blindsided", "0" ) == "1" ) { 
              ...
              txt2 = "";    // If blindsided, then don't mention blindsiding.
            }
            ...
            if((x = Earthdawn.getAttrBN( targetChar, "condition-Cover", "0" )) != "0" )
              txt += Earthdawn.addIcon( "cover", "s",  "Cover Target has cover and gains bonus to PD and MD."
                  , undefined, ((x == "2") ? "Partial: -2" : "Full:<span class='sheet-rolltemplate-fail'> Can't hit.</span>"));
        ...
        if( txt.length > 0 || txt2.length > 0 )
          bAppend(bodyMain, (( ++linenum % 2) ? ".odd" : ".even"), "<b>Target:</b> <nbsp>" + txt2 + txt.trim());
...
// CDD todo
// when displaying step, give a number of extra success bonus appliled (or maybe an info icon), and tooltip of base and and modifiers.
          bAppend( bodyMain, ( ++linenum % 2) ? ".odd" : ".even",
              new HtmlBuilder( "span", "<b>Step:</b> " + (( "finalStep" in this.misc ) ? this.misc[ "finalStep" ] : (( "step" in this.misc ) ? this.misc[ "step" ] : "")) ,{
                    class: "sheet-rolltemplate-emphasis",
        ...
        if( "CorruptedKarma" in this.misc )
          bodyMain.append( "", new HtmlBuilder( "span", this.misc[ "CorruptedKarma" ] + " karma corrupted.", {
                class: "sheet-rolltemplate-warning",
        ...
        if( "FunnyStuffDone" in this.misc ) {
          ...
          if( txt.length > 0 ) {
            bodyMain.append( "", txt + ". Roll was " + this.BuildRoll( "showResult" in this.misc && this.misc[ "showResult" ], 
                this.misc[ "DiceOrigional" ].total, this.misc[ "DiceOrigional" ] ), { class: "sheet-rolltemplate-warning" });
        ...
        if( "failBy" in this.misc ) {
          if ( "SpellBuffSuccess" in this.misc )     // Did not actually succeed, but spell is a buff, so pretend we did.
            txt += ((txt.length > 0) ? "   " : "" ) + "<span class='sheet-rolltemplate-autosuccess'>Buff Spell: Auto succeed.</span> "
                + this.BuildRoll( true, this.misc[ "result" ], rolls ) + ".";
          else
            txt += ((txt.length > 0) ? "   " : "" ) + "<span class='sheet-rolltemplate-fail'>Failure</span> "
        ...
        if( "succBy" in this.misc ) {
          ...
          txt += ((txt.length > 0) ? "   " : "" ) + "<span class='sheet-rolltemplate-success'>Success</span> "
                + ((( "StyleOverride" in this.misc ? this.misc[ "StyleOverride" ] : state.Earthdawn.style) != Earthdawn.style.VagueSuccess )
                ? " by " + this.BuildRoll( this.misc[ "showResult" ], this.misc[ "succBy" ], rolls ) : "!" )
                + (("sayTotalSuccess" in this.misc && this.misc[ "sayTotalSuccess" ] )
                ? new HtmlBuilder( "span", " (" + (es +1) + " Total)", { class: "sheet-rolltemplate-good",
                ...
                : (( es > 0) ? new HtmlBuilder( "span", " (" + es + " Extra)", { class: "sheet-rolltemplate-emphasis",
        ...
        if( "RuleOfOnes" in this.misc ) {
          bodyMain.append( "", "Rule of Ones", { class: "sheet-rolltemplate-bad" });
          if( playerCard ) playerCardNix.push( bodyMain._children.length );
        ...
        if( "secondaryResult" in this.misc ) {
          ...
          txt = "<b>Cntr-Atk:</b>"
              ...
              + ((this.misc[ "secondaryResult" ] < 0)
                  ? " <span class='sheet-rolltemplate-fail'>Failed</span>  by "
                  : " <span class='sheet-rolltemplate-success'>Succeeded</span> by ")
...
//log(splines[i]);
        ...
        if( "eachTargets" in this )     // We compare the roll result to each target number in the list.
          for( let i = 0; i < this.eachTargets.length; ++i ) {
            ...
            bAppend( bodyMain, (( ++linenum % 2) ? ".odd" : ".even"), Earthdawn.constantIcon( "Target" ) + "<b>" + this.eachTargets[ i ][ "tName" ] + "</b>  "
        ...
        procMsg( "successMsg", ( "succBy" in this.misc ) || !( "targetNum" in this.misc));
        ...
        if( ("endNoteSucc" in this.misc) && (this.misc[ "endNoteSucc" ].length > 1) && (( "succBy" in this.misc ) || !( "targetNum" in this.misc))) {
          bodyMain.append( (( ++linenum % 2) ? ".odd" : ".even"), this.misc[ "endNoteSucc" ]);
            if( playerCard ) playerCardNix.push( bodyMain._children.length );
        }
        if( ("endNoteFail" in this.misc) && (this.misc[ "endNoteFail" ].length > 1) && (( "failBy" in this.misc ) || !( "targetNum" in this.misc))) {
        ...
        if(("cButtons" in this.misc) && this.misc[ "cButtons" ].length > 0) {
          ...
          for( let i = 0; i < this.misc[ "cButtons" ].length; ++i ) {
            ...
            buttonLine += Earthdawn.makeButton( this.misc[ "cButtons" ][ i ].text, this.misc[ "cButtons" ][ i ].link,
								("tip" in (this.misc[ "cButtons" ][ i ])) ? this.misc[ "cButtons" ][ i ].tip.replace( /(\r|\n)/g , " "): "", "effect");
        } }
 ...
 */
        ...
        if( whichMsgs === Earthdawn.whoTo.public ) {                        // public:    Main message to everybody. Maybe supplementary messages to player and/or gm.
          ...
          if (buttonLine && !pIsGM )
            setTimeout(function() {                        // After public message, if player is not GM, supplementary message to player with buttons.
              ...
              } catch(err) {Earthdawn.errorLog( "ED.rollFormat setTimeout2() error caught: " + err, po );}
            }, 70);    // end public message and GM and player info cards. 
        ...
        if( this.rollQueue.length > 0 ) {
          setTimeout(function() {
            try {
              po.indexTarget++;
              po.misc = JSON.parse( po.rollQueue.shift());     // make the next roll in the queue active.
              po.rollPost();
            } catch(err) {Earthdawn.errorLog( "ED.rollFormat setTimeout3() error caught: " + err, po );}
          }, 100);
        }
    ...
    this.BuildRoll = function( howMuchDetail, main, rolls ) {
      ...
      try {
        let po = this,
            tip1 = 'Rolling ',
            tip2 = " = ",
            f, level = 0, levelmax = 0, dice = 0, ones = 0;
//log(JSON.stringify(rolls));
        function walk( item ) {      // Walk through the roll structure, extracting what we need.
          ...
          case "G":     // This is a group delimited by brackets. {{1d4!-1}+d1}kh1 is two nested groups. "1d4!-1" and that and d1, with a keep highest 1.
            ...
            if( ++level > levelmax) levelmax = level;    // When level was zero, we were  not in a group. Now we are.
            for( let k2 = 0; k2 < item[ "rolls" ].length; ++k2)
              for( let k3 = 0; k3 < item[ "rolls" ][ k2 ].length; ++k3 )
...
//              tip1 += "k" + item[ "mods" ][ "keep" ][ "end" ] + item[ "mods" ][ "keep" ][ "count" ];
            ...
            break;
          case "R":     // This is a sub-roll result. {"type":"R","dice":1,"sides":4,"mods":{"exploding":""},"results":[{"v":4},{"v":3}]}
            if( level === levelmax )      // only do tip1 this if it is the first R of this level. This will strip out the +d1 kh1.
            ...
            break;
          case "M":     // This is an expression, such as "+" between two items, or "-1" as a modifier to a roll.
            if( level === 0 || item[ "expr" ].startsWith( "-" ))
              tip1 += item[ "expr" ]
            if( level === 0 )
              tip2 += item[ "expr" ]
            break;
        ...
        return new HtmlBuilder( "span", main.toString(), { class: "sheet-rolltemplate-chatbox2", title: Earthdawn.encode( Earthdawn.encode( tip1 + tip2 )) });
...
//        return "<span class='sheet-rolltemplate-dicewrap'>" + txt + "</span>";
    ...
    this.SetToken = function( ssa )  {
      ...
      try {
        if( ssa.length > 1 ) {
          ...
          // All that is required is name and val. To set max only, use name, undefine, undefined, maxVal. 
    ...
    this.ssaMods = function( ssa, start, fSpecial, cID ) {
      ...
      try {
        ...
        for( let i = start; i < ssa.length; i++ ) {
          ...
          if( typeof ssa[ i ] === "string" ) {
            let fnd = ssa[ i ].match( /\d+\s*\(\s*\d+\s*\)/g );      // Falling damage table uses a format nothing else does. step (reps). Example 20 (3). Look for it.
            if( fnd ) {
              let item = fnd[ 0 ];
              ...
              if( fnd2 ) {
                ...
                if( rep > 0 && stp > 0 ) {
                  ret += stp;
                  this.misc[ "moreSteps" ] = { step: stp, reps: rep };    // this is repeating steps such as falling damage, step 20 three times. 
                  nomatch = false;
                } else Earthdawn.errorLog( "ParseObj.ssaMods failed to parse " + ssa[ i ] + " got " + rep + " and  " + stp ); 
              } else Earthdawn.errorLog( "ParseObj.ssaMods failed to parse " + ssa[ i ] );
          } }
          if( nomatch ) {
      ...
      return ret;
        ...
        //
        // Earthdawn.abilityAdd( this.charID, Earthdawn.constantIcon( "Spell" ) + t,
        //  "!edToken~ %{selected|" + preTo + "Roll}" );
        //    ssa: (0) Spell (1) SPM RowId (2) what {options} (n-1) G/M
        ...
        //   3.19 - Rewrote the sequence. Instead of reading the data from the SP or SPM Attributes, the Sequence now looks for the data in a seqdata JSON that stores sequence and spell data.
...
//log("this.Spell ssa is " + JSON.stringify(ssa));
        ...
        pre = po.buildPre( bGrim ? "SP" : "SPM", ssa[ 1 ] ),               // The prefix for the spell, SPM or SP depending where the call was made from
        presp = (bGrim ? pre : po.buildPre( "SP", Earthdawn.getAttrBN( this.charID, pre + "spRowID" ))),     //The prefix for the SP, even if call was done from SPM
...
//log("this.Spell spellseqsaved is " + JSON.stringify(spellseqsaved));
        ...
        let spellseq = {};
        let fieldarray = {"sThreads":"0","Casting":"MD1: @{target|Target|token_id}","Duration":"","Range":"","WilSelect":"None","Discipline":"","WilEffect":"0","Effect":"","Numbers":"","ExtraThreads":"None","SuccessLevels":"None","AoE":"","EffectArmor":"N/A","FailText":"","SuccessText":"","DisplayText":"","sayTotalSuccess":"0","FX":"","Circle":"1"};
...
// log('this.Spell : initspell');
          ...
          spellseq.spelldata.StrainSave = po.misc[ "strainSave" ];
          ...
          if(!bGrim && Earthdawn.safeString(Earthdawn.getAttrBN( po.charID, pre + "EnhThread", "")).length > 2) {
          ...
          let lpv = spellseq.spelldata.LinksProvideValue = Earthdawn.getAttrBN( po.charID, presp + "LinksProvideValue", "").split( "," ),
              lpvm = spellseq.spelldata.LinksProvideValueList = Earthdawn.getAttrBN( po.charID, presp + "LinksProvideValueList", "").split( "," );    //LPVM is SP;RowID
//log(lpv);
//log(lpvm);
          for( let i = 0; i < lpvm.length; i++)
            if( lpvm[ i ] && lpvm[ i ].includes( ";" ) && [ "SpellKnack", "BindingSecret" ].includes( Earthdawn.getAttrBN( po.charID, 
                  po.buildPre( Earthdawn.getParam( lpvm[ i ], 1, ";"), Earthdawn.getParam( lpvm[ i ], 2, ";")) + "Type", ""))) {
              spellseq.spelldata.KnackNames.push( lpv[ i ] );       // KnackNames and KnacksId are knacks available, not knacks chosen.
              spellseq.spelldata.KnacksId.push( Earthdawn.getParam( lpvm[ i ], 2, ";"));
            }
        }; //end InitSpell
        ...
        'use strict';
          ...
          po.setWW( "SS-Knacks", spellseq.KnacksChosenName ? spellseq.KnacksChosenName.toString() : "x");
          po.setWW( "SS-StrainAdvanced", po.misc[ "strainSave" ] || "");      // As far as I can tell, this is saved, but never used.
...
//log("seq saved with spellseq " + JSON.stringify( spellseq ));
            ...
            // We need some buttons made up. We should be able to inheirit ssa from the main routine.
        function buttonOnDemand( what, sp ) {
        'use strict';
          let txt, t = "", sp2 = ((sp === undefined || sp) ? "" : " NOTE : This is not the recommended next step of the Sequence");

          function getKask( pre3 ) {     // KarmaGlobalMode 0: Off, x: Auto, ?:Ask  when the ? is in front of the { the chat window ask triggers.
            let charname = getAttrByName( po.charID, "character_name" );
            return "modValue : ?{Modification|0}~ K-ask: @{" + charname + "|KarmaGlobalMode}@{" + charname + "|" 
                  +  pre3 + "Karma-Ask}: @{" + charname + "|DPGlobalMode}@{" + charname + "|" + pre3 + "DP-Ask}~";
          }
          ...
          switch( what ) {
            case "extra":
              let opt = _.without( Earthdawn.safeString(spellseq.spelldata.ExtraThreads).split( "," ), ""),
                  et = "";
              for( let i = 0; i < opt.length; ++i )     // Add extra threads to the tooltip.
                et += opt[ i ] + (i < (opt.length -1) ? ",   " : "");
              t = Earthdawn.makeButton( "Extra Threads", "!Earthdawn~ charID: " + po.charID + "~ ForEach: inmt~ Spell: " + ssa[ 1 ] + ": New: " + ssa[ ssa.length -1 ],
                  "Start a new casting and chose extra threads: " + et, "param" );
              break;
            case "cast":
              if( !txt ) txt = txtcst;
            case "effect":   // effect
              if( !txt ) txt = txtfx;
            case "onfly":     // thread weaving to tune on fly.
              if( !txt ) txt = txtonfly;
            case "pattern":   // patterncraft
              if( !txt ) txt = txtpatt;
            case "weave":
              if( !txt ) txt = txtwv;
//log("BoDa   " + txt);
                        // All the above falls into here. 
              let bi3 = txt.split( "$" );
              for( let i = 0; i < bi3.length; ++i ) {
                if( i > 0 ) t += " ";
                if( bi3[ i ].includes( "missing!" ))
                  t = bi3[ i ];
                else {
                  ...
                  switch( Earthdawn.safeString( ci[ 0 ] ).trim() ) {
                    case "Cast":
                      t += Earthdawn.makeButton( Earthdawn.getAttrBN( po.charID, pre + "Name", "")
                          ...
                          + " Spell: " + ssa[ 1 ] + ": Cast: " + sendon + ": " + ssa[ ssa.length -1 ]
                          , "Cast using this Talent. " + sp2, "param");
                      break;
                    case "CircleNG":    // Circle Namegiver:   CircleNG, repeating_discipline_xxx_DSP_, Elementalism.
                      t += Earthdawn.makeButton( ci[ ci.length -1 ] + " Circle"
                          , "!Earthdawn~ charID: " + po.charID + "~ ForEach: inmt~ Spell: " + ssa[ 1 ] + ": Effect: " + sendon + ": " + ssa[ ssa.length -1 ]
                          , "Effect Test based on Creature Circle/SR." + sp2, "param");
                      break;
                    case "CircleSR":    // Circle Creature or spirit:   CircleSR, noRid, the spirit rating value.
                      t += Earthdawn.makeButton( ci[ 2 ] + " Circle"
                          , "!Earthdawn~ charID: " + po.charID + "~ ForEach: inmt~ Spell: " + ssa[ 1 ] + ": Effect: " + ci[ 1 ] + ": " + ssa[ ssa.length -1 ]
                          , "Effect Test based on this Discipline Circle." + sp2, "param");
                      break;
                    case "CircleMissing":
                      t += "No Corresponding Discipline!";
                      break;
                    case "onfly":
                      t += Earthdawn.makeButton( "Attune on the Fly", "!Earthdawn~ charID: " + po.charID + "~ ForEach: inmt~ "
                          + getKask( pre ) + " Spell: " + ssa[ 1 ] + " : TuneOnFly: " + sendon + ": " + ssa[ ssa.length -1 ]
                          , "Attune Matrix on the Fly using " + ci[ 3 ] , "param2") + " ";
                      break;
                    case "Pattern":
                      t = Earthdawn.makeButton( "Attune Grimoire", "!Earthdawn~ charID: " + po.charID + "~ ForEach: inmt~ "
                          + getKask( pre ) + "Spell: " + ssa[ 1 ] + " : TuneGrimoire : " + sendon + ": " + ssa[ ssa.length -1 ]
                          , "Attune Grimoire using the Patterncraft Talent." , "param");
                    break;
                    case "Rank":      // rid of talent from which rank can be gotten.
                      t += Earthdawn.makeButton( Earthdawn.getAttrBN( po.charID, pre + "Name", "") + " Rk"
                          , "!Earthdawn~ charID: " + po.charID + "~ ForEach: inmt~ Spell: " + ssa[ 1 ] + ": Effect: " + sendon + ": " + ssa[ ssa.length -1 ]
                          , "Effect Test based on this Talent Rk." + sp2, "param");
                      break;
                    case "RankMissing":
                    ...
                    case "Weave":
                      ...
                      break;
                    case "Wil":
                      t += Earthdawn.makeButton( "WIL"
                          , "!Earthdawn~ charID: " + po.charID + "~ ForEach: inmt~ Spell: " + ssa[ 1 ] + ": Effect : Wil" +":" + ssa[ ssa.length -1 ]
                          , "Effect Test using Willpower Attribute." + sp2, "param");
                      break;
                    case "Willtalent":
                      t += Earthdawn.makeButton( Earthdawn.getAttrBN( po.charID, pre + "Name", ""), "!Earthdawn~ charID: " + po.charID + "~ ForEach: inmt~ " + getKask( pre ) 
            ...
            case "info":
              t = Earthdawn.makeButton( "Info", "!Earthdawn~ charID: " + po.charID + "~ ForEach: inmt~ Spell: " + ssa[ 1 ] + ": Info: " + ssa[ ssa.length -1 ],
                  "Press this button to get the details of the spell." , "param2");
              break;
            case "knack": {
              let ttn = "";
              for( let i = 0; i < spellseq.spelldata.KnackNames.length; ++i )
                ttn += spellseq.spelldata.KnackNames[ i ] + ", ";
              t = Earthdawn.makeButton( "Spell Knacks " + spellseq.spelldata.KnacksId.length, "!Earthdawn~ charID: " + po.charID +  "~ Spell: " + ssa[ 1 ] + " : Knack "
                  , "Choose Knacks to add to Casting: " + ttn.slice(0, -2).trim(), "param");
            } break;
            case "mark":
              t = Earthdawn.makeButton( "Horror Mark", "!Earthdawn~ charID: " + po.charID + "~  ForEach: inmt~ Spell: " + ssa[ 1 ] + ": HorrorMark: ?{ What is the region type? |Safe|Open|Tainted|Corrupt}:" +ssa[ ssa.length -1 ],
                  "When Casting Raw Magic, The unmasked magic might attract unwanted attention from astral entities. Test Against MD to see if character was Horror Marked." , "param2");
              break;
            case "reset":
              t = Earthdawn.makeButton( "Reset", "!Earthdawn~ charID: " + po.charID + "~ ForEach: inmt~ Spell: " + ssa[ 1 ] + ": Reset: " + ssa[ ssa.length -1 ],
                  "Press this button to Reset the Spell Sequence.", "param2");
              break;
            case "tunematrix":
              t = Earthdawn.makeButton( "Attune Matrix", "!Earthdawn~ %{" + getAttrByName( po.charID, "character_name" ) + "|" + pre + "AttuneButton}",
                  "Press this button to ReAttune this spell in a Matrix (using the 10 minute ritual)." , "param");
              break;
            case "warp":
              t = Earthdawn.makeButton( "Warp Test", "!Earthdawn~ charID: " + po.charID + "~ ForEach: inmt~ Spell: " + ssa[ 1 ] + ": WarpTest: ?{ What is the region type? |Safe|Open|Tainted|Corrupt} :" + ssa[ ssa.length -1 ],
                  "When Casting Raw Magic, the Astral Space can damage you. Make this test according to the Region Type. Test Against MD to know if character suffers Damage" , "param2");
              break;
            case "wipe":
              t = Earthdawn.makeButton( "Wipe All Matrix", "!Earthdawn~ charID: " + po.charID + "~ ForEach: inmt~ TuneMatrix: WipeMatrix",
                  "When re-attuning on the fly, Spellcaster must remain concentrated (Wil/Willforce chack against any damage), and continue until success, or all his matrix are wiped." , "param2");
              break;
            default:
              t = "";
              log( Earthdawn.timeStamp() + "Earthdawn.Spell.buttonOnDemand invalid value of 'what': " + what );
          }   // end switch( what )
//log( "BoD " + what + "   " + t);
          return " " + t.trim() + " ";
        } // end buttonOnDemand
        ...
        function line( txt, tip, addtxt) {
          bodySpell.append( (( ++linenum % 2) ? ".odd" : ".even"), (( !tip || tip == "" ) ? txt: Earthdawn.texttip( txt, tip )) + ( addtxt ? addtxt : ""));
        }
        ...
        let nowil = (spellseq.spelldata[ "WilSelect" ] !== "Wil" ),     //Spell has no Will Effect.
            bGrimAtt = Earthdawn.safeString( spellseq.Type ).toLowerCase().endsWith( "a" ),   //If a Grimoire is attuned we stored GA
            ...
            linenum = 0, rid, txtwv = "", txtcst = "", txtfx = "", txtpatt = "", txtonfly = "",    //This are the buttons for each of the main stuff
            ...
            disp = !isng ? "Spellcasting" : Earthdawn.dispToName( spellseq.spelldata.Discipline, "weaving" ),
            wilselect = spellseq.spelldata.WilSelect,
            sectSpell = po.newSect();                // HTML functions for the menu.   Initializes the sect for the future display if needed
        sectSpell.append( "", spellseq.spelldata.Name, { class: "sheet-rolltemplate-spelldetail" });
        let bodySpell = Earthdawn.newBody( sectSpell );

        if( Earthdawn.safeString( spellseq.spelldata.Casting ).startsWith( "MD1" ) && spellseq.ExtraThreads 
          ...
          spellseq.spelldata.Casting = "MDh";          // If we had an Extra Thread that adds Targets, change MD1 to MDh
        }

            // If there was already something in it, give origional, plus a tildi. Then each additional argument followed by a comma.
            // sample return: cast, rid1 $ cast, label, value
        function addit() {
          let t = "";
        ...
        let attributes = findObjs({ _type: "attribute", _characterid: this.charID });
        _.each( attributes, function (att) {
          if (att.get( "name" ).endsWith( "_Special" ) && Earthdawn.keywordCheck(  att.get( "current" ), true, "SPL-" )) {      // If any of the SPL- Talents.
            let pre2 = po.buildPre( att.get("name"));
            rid  = Earthdawn.repeatSection( 2, pre2 );        // if this rid is not a "T", then send pre2 instead, and the downstream code should accept that instead.

            if( att.get( "current" ).includes( "-" + disp )) {      // Thread Weaving.
              txtwv = addit( txtwv, "Weave", rid );
              if( Earthdawn.parseInt2( getAttrByName( po.charID, pre2 + "Strain" )) == 0)     // We are using Strain == 0 as a signal that this is the base thread weaving talent for this discipline. 
                txtonfly = addit( txtonfly, "onfly", rid, Earthdawn.getAttrBN( po.charID, pre2 + "Name","") );
            }
            if( att.get( "current" ).includes( "-Spellcasting" )) {     // Spellcasting
              txtcst = addit( txtcst, "Cast", rid );
              if( wilselect == "Rank" )     //Button for Effect for Rank based spell
                txtfx = addit( txtfx, "Rank", rid );
            } else if(( wilselect == "Wil" ) && att.get( "current" ).includes( "-Willforce" ))       // Wil and Willforce
              txtfx = addit( txtfx, "Willtalent", rid );
            else if( att.get( "current" ).includes( "-Patterncraft" ))        // Patterncraft
              txtpatt = addit( txtpatt, "Pattern", rid );
          } // end T_Special SPL-
          if( wilselect == "Circle" && isng &&  att.get( "name" ).endsWith( "_DSP_Name" ) && ( disp == Earthdawn.dispToName( att.get( "current" ), "weaving" ))) {   // Effect based on Circle Namegiver
            txtfx = addit( txtfx, "CircleNG", po.buildPre( att.get( "name" )), disp);
          }

          if( wilselect == "Circle" && !isng &&  att.get( "name" ).endsWith( "SrRating" ))   // Effect based on Circle SR
              txtfx = addit( txtfx, "CircleSR", "noRid",  att.get( "current" ));
        }); // End for each attribute.

        if( txtwv == "")          txtwv = "Weaving Talent missing! Possible no Discipline defined in spell.";
        if( txtcst == "")         txtcst = "Spellcasting Talent missing!";
        ...
        switch( ssa[ 2 ] ) {
          case "Sequence": {    //This is the master sequence... Where the magic happens
            if( other ) {     //Another Sequence is in progress, but for now we didn't take any action to abort it, this may still be a mistake... But any further action will re-initiate the sequence
              line( "Other Sequence Active:", 
                  "A different spellcasting sequence was in progress, continue with the old action or start a new spellcasting sequence with this new action?", 
                  Earthdawn.makeButton( "Resume-" + spellseqsaved.spelldata.Name,
                      "!Earthdawn~ charID: " + this.charID + "~ ForEach: inmt~  Spell: " + spellseqsaved.RowId + " : Sequence", "Resume Sequence." , "param"));
            ...
            if( isng && spellseq.bGrim ) { //isng because creatures cast raw, like if it was matrix
              ...
              line("Grimoire Casting, Grimoire Attuned" ,"You successfully Attuned to a Grimoire and are casting from the Grimoire. This will grant you one extra spellcasting success");
                ...
                line("", "", buttonOnDemand( "pattern" ) + buttonOnDemand( "tunematrix" ) + buttonOnDemand( "onfly" ));
            ...
            if( !other )
              ...
              line( buttonOnDemand( "info" ) + buttonOnDemand( "reset" ));
              //Body Main Action buttons
              if(( spellseq.spelldata.KnacksId && spellseq.spelldata.KnacksId.length > 0 && spellseq.SeqStep == 0) || spellseq.KnacksChosenName)
                line("<b> Spell Knacks: </b> " + (spellseq.KnacksChosenName ? spellseq.KnacksChosenName.join() : "") 
                    + ((spellseq.SeqStep == 0 && ( !spellseq.KnacksChosenName || spellseq.KnacksChosenName.length !== spellseq.spelldata.KnackNames.length))
                    ? buttonOnDemand( "knack" ): ""), "Choose Spell Knacks" );
              line("<b> Extra Threads: </b>" + (spellseq.ExtraThreads ? spellseq.ExtraThreads.toString() : 
                    (spellseq.SeqStep==0 ? "" : "None")) + "  " + (spellseq.SeqStep == 0 ? buttonOnDemand( "extra" ) : ""), "");
              line("<b> Weave (" + spellseq.CurThreads + " / " + spellseq.TotThreads +"): </b>",
                  "Choose Talent to weave Threads. " + spellseq.CurThreads + " woven (including held in Matrix) out of " 
                  + spellseq.TotThreads + (spellseq.numExtraThreads > 0 ? "including " + spellseq.numExtraThreads + " Extra" : "")
                  , spellseq.SeqStep >= 2 ? "Done" : buttonOnDemand( "weave" ));
              line("<b> Cast : </b>", "", spellseq.SeqStep == 3 
                  ? "Spellcasting Failed!" 
                  : spellseq.SeqStep == 4 
                    ? ((spellseq.spelldata.SuccessLevels && spellseq.numExtraSuccesses ) 
                      ? "Done - " + spellseq.spelldata.SuccessLevels + " x " + spellseq.numExtraSuccesses + " ES"
                      : "Spellcasting Successful" + ((spellseq.numExtraSuccesses && spellseq.numExtraSuccesses > 0)
                      ? " (" + spellseq.numExtraSuccesses + "Extra Success)"
                      : "" ))
                    : buttonOnDemand( "cast" ));
              line("<b> Effect : </b>", "", buttonOnDemand( "effect" ));
          ...
          case "Info": {  //v3.19 No change because we want to stay with the unchanged data, i.e. read it directly from the sheet
            ...
            if( Earthdawn.safeString( Earthdawn.getAttrBN( this.charID, presp + "AoE", "")).trim().length > 1 )
            ...
            { let x1 = "<b>Effect:</b>";
              ...
              if( Earthdawn.safeString( Earthdawn.getAttrBN( this.charID, presp + "Effect", "")).trim().length > 1)
            ...
            if( Earthdawn.safeString( Earthdawn.getAttrBN( this.charID, presp + "DisplayText", "")).trim().length > 1 )
            ...
            if( Earthdawn.safeString( Earthdawn.getAttrBN( this.charID, presp + "SuccessText", "")).trim().length > 1 )
            ...
            if( Earthdawn.safeString( Earthdawn.getAttrBN( this.charID, presp + "FailText", "")).trim().length > 1 )
            ...
            if( !infoPub )
              line( Earthdawn.makeButton( "Post to all", "!Earthdawn~ charID: " + this.charID + "~ ForEach: inmt~ Spell: " + ssa[ 1 ] + ": InfoPub: " +ssa[ ssa.length -1 ],
                  "Press this button to send the details of the spell to all players." , "param2" ));
          ...
          case "TuneGrimoire": {
            ...
            if( ssa[ 3 ].startsWith( "repeating_" ))    // we have a pre
              pre2 = ssa[ 3 ];
            else      // we have a rid
              pre2 = po.buildPre( "T", ssa[ 3 ]);
            this.strainAdd( pre2 );
            ...
            this.misc[ "targetNum" ] = Earthdawn.getParam( spellseq.spelldata.Numbers, 2, "/" );
            this.misc[ "sayTotalSuccess" ] = true;
            this.misc[ "endNoteSucc" ] = "Grimoire Successfully Attuned. (10 min)" + buttonOnDemand( "extra" ) + buttonOnDemand( "weave" ) + buttonOnDemand( "cast" );
          ...
          case "TuneOnFly": {     // " Spell: " + SPM rowID + " : TuneOnFly: " + (rid or pre of tuning talent) + ": " + ssa[ ssa.length -1 ]
            ...
            if( ssa[ 3 ].startsWith( "repeating_" ))    // we have a pre
              ...
              pre2 = po.buildPre( "T", ssa[ 3 ]);
            this.strainAdd( pre2 );
            this.strainAdd( "Attune on fly", 1 );
            this.Karma( pre2 + "Karma", 0 );
            ...
            this.misc[ "targetNum" ] = Earthdawn.getParam( spellseq.spelldata.Numbers, 2, "/" );
            this.misc[ "sayTotalSuccess" ] = true;
            this.misc[ "endNoteSucc" ] = "Choose Matrix : " + buttonOnDemand( "tunematrix" );
            this.misc[ "endNoteFail" ] = "Attuning on the fly failure. Retry until success or " + buttonOnDemand( "wipe" )  ;
            if( other ) {
              this.misc[ "warnMsg" ]= "Cancelling Sequence for "+ spellseqsaved.spelldata.Name + ". Sequence Restarted .";
              initSpell();
            }
          ...
          case "Knack": {     // user has said want to pick a spell knack. Give them a list of knacks that may be chosen.
            ...
            for( let i = 0; i < spellseq.spelldata.KnacksId.length; ++i ) {
              if(!spellseq.KnacksChosenName || !spellseq.KnacksChosenName.includes(spellseq.spelldata.KnackNames[ i ]))     // don't do button for any knack already chosen
                t += Earthdawn.makeButton( spellseq.spelldata.KnackNames[ i ], "!Earthdawn~ charID: " + this.charID + "~ Spell: " + ssa[ 1 ] + ": " + "Knack2: " + spellseq.spelldata.KnacksId[ i ],
                      "Add this optional spell knack to this casting." , "action");
              else
                t += "<b>" + spellseq.KnacksChosenName[ i ] + "</b>";
            }
            line( "<b>Spell Knacks:</b> " + t );
            line( buttonOnDemand( "extra" ) + buttonOnDemand( "weave" ) + buttonOnDemand( "cast" ));
            this.chat( sectSpell.toString(), Earthdawn.whoTo.player | Earthdawn.whoFrom.noArchive | Earthdawn.whoFrom.character );
          } break;  // End Knack
          case "Knack2": {      // Spell: (SPM ID): Knack2: (knackID)      They have chosen a knack to add to the spell
                    //Sequence Control. Normal Case is (noseqactive || seqended || (step==0&!other)), if not go through different cases to issue warnings.
                    //At this stage, Action was taken, so we need to update the base spell information with the information from the new knack.
            if( !(noseqactive || seqended || (spellseq.SeqStep == 0 && !other))) {
            ...
            let prenac = po.buildPre( "SP", ssa[ 3 ] ),
                nacnm = Earthdawn.getAttrBN( this.charID, prenac + "Name", "");
            line( "Knack/BSecret added: <b>" + nacnm + "</b>" );
            if(spellseq.KnacksChosenName)
              spellseq.KnacksChosenName.push( nacnm );
            else
              spellseq.KnacksChosenName = [ nacnm ];

            if(spellseq.KnacksChosenID)
              spellseq.KnacksChosenID.push( ssa[ 3 ] );
            else
              spellseq.KnacksChosenID = [ ssa[ 3 ]];

                  // Update the spelldata. This will override anything in the origional spell with matching entreis from the knack.
                  // Note that this does not update strain. 
            let castSave = spellseq.spelldata.Casting;
            for( let key in fieldarray ) {
              let x = Earthdawn.getAttrBN( po.charID, prenac + key, "");
              if( x !== "" )
                spellseq.spelldata[ key ] = x;
            }
            ...
            SaveSeq();
            if( castSave == spellseq.spelldata.Casting )
              line( buttonOnDemand( "extra" ) + buttonOnDemand( "weave" ) + buttonOnDemand( "cast" ));
            else {
              line( buttonOnDemand( "extra" ) + buttonOnDemand( "weave" ));
              line( "Spell targeting has changed. Do not use a previous Spellcasting button. " + buttonOnDemand( "cast" ));
            }
          ...
          case "New": {
            ...
            for( let i = 0; i < opt.length; ++i )
              line( Earthdawn.makeButton( opt[ i ], "!Earthdawn~ charID: " + this.charID + "~ Spell: " + ssa[ 1 ] + ": " + "Extra: " + opt[ i ],
                  "Press this button to add an optional extra thread to this casting." , "action"));
            line( buttonOnDemand( "weave" ) );
            this.chat( sectSpell.toString(), Earthdawn.whoTo.player | Earthdawn.whoFrom.noArchive | Earthdawn.whoFrom.character );
          } break;
          case "Extra": {
                // Sequence Control. Normal Case is (noseqactive || seqended || (step==0&!other)), if not go through different cases to issue warnings.
          ...
          case "Weave": {     //   " Spell: " + ssa[ 1 ] + ": Weave: " + sendon + ": "+ssa[ ssa.length -1 ]
                //Sequence Control.
            ...
            this.bFlags |= Earthdawn.flags.VerboseRoll;
            this.Lookup( 1, [ "value", pre2 + "Step" ]);
            this.misc[ "targetNum" ] = Earthdawn.getParam( spellseq.spelldata.Numbers, 1, "/" );
            this.misc[ "sayTotalSuccess" ] = true;
          ...
          case "Weave2": {      // Roll calls back to here.
            ...
            if ( spellseq.RemThreads <= 0 ) lines.push( buttonOnDemand( "cast" ) );    //If Weaving finished, offer to cast
            SaveSeq();
            return lines;
          }
          case "Cast": {            // Spellcasting.   (0) Spell, (1) SPM rowID, (2) Cast, (3) Spellcasting (or other) rowID. (4)
                //Sequence Control.
                //At this stage, Action was taken, so we actually reset the sequence
            if( other && !seqended ) { //Another spell was in middle of an unfinished sequence
      ...
      //      if(noseqactive || seqended)
        //    initSpell(this); //Action was taken initialize sequence
            ...
            if( ssa[ 3 ].startsWith( "repeating_" ))    // we have a pre
              ...
              pre2 = po.buildPre( "T", ssa[ 3 ]);
            this.strainAdd( pre2 );       // This is the strain for the spellcasting talent
            this.strainAdd( presp );      // This is the strain for the base Spell
            if( spellseq.KnacksChosenID )
              for( let i = 0; i < spellseq.KnacksChosenID.length; i++)
                this.strainAdd( po.buildPre( "SP", spellseq.KnacksChosenID[ i ] ));     // This is the strain for the Knacks.
            this.Karma( pre2 + "Karma", 0 );
            ...
            this.Lookup( 1, [ "value", pre2 + "Step" ]);
            let tType = spellseq.spelldata.Casting;
            if( tType && tType !== "None" )
              if( tType.startsWith( "Ask" ))
                this.misc[ "targettype" ] = tType.substring( 0, tType.lastIndexOf( ":" ));
              else if( tType.slice( 1, 3) === "D1")     // PD1, MD1, and SD1, go to just the first two characters.
          ...
          case "Cast2": {       // RollFormat calls back to here. We return additional formatted lines.
            let lines = [], pre2;
            if( ssa[ 3 ].startsWith( "repeating_" ))    // we have a pre
              pre2 = ssa[ 3 ];
            else      // we have a rid
            ...
            if( fail )
              lines.push("End Sequence " + buttonOnDemand( "reset" )); //If failed, we don't display all the details of the results
            else {    // is successful
              ...
              if( x4 && x4 !== "None") {
                ...
                lines.push( "<b>Effect :</b>" + buttonOnDemand( "effect", true ));
              }
              ...
              if( spellseq.KnacksChosenName )
                for(let i=0 ; i < spellseq.KnacksChosenName.length ; i++ )
                  lines.push( "<b> Spell Knack :</b> " + Earthdawn.texttip(spellseq.KnacksChosenName[ i ], Earthdawn.getAttrBN( this.charID, po.buildPre("SP",spellseq.KnacksChosenID[ i ]) + "Notes", "")));
              lines.push( "<b>Description:</b>  "+ Earthdawn.texttip("(Hover)",Earthdawn.safeString(spellseq.spelldata.Notes) .replace( /\n/g, Earthdawn.constantIcon( "cr" ))));
              if( !x4 || x4 == "None")
                lines.push("<b>No Will Effect</b> - End Sequence :" + buttonOnDemand( "reset" ));    //If no Will Effect, Sequence is finished
            ...
            if(spellseq.bGrim && !spellseq.bGrimAtt) {      //Raw Casting only
              lines.push("<b> Raw Magic </b>" + buttonOnDemand( "warp" ) + buttonOnDemand( "mark" ));
              spellseq.Type="GR";
            }
            SaveSeq();
          ...
          case "Effect": {    // Spell Effect test.           // (0) Spell, (1) SPM RowID, (2) Effect, (3) T (or other) RowID, (4), SP RowID
            let skipseq  = false;
            if( other ) {
            ...
            if( ssa[ 3 ].startsWith( "repeating_" ))    // we have a pre
              ...
              pre2 = po.buildPre( "T", ssa[ 3 ]);
// see if it works without the line below this. 
//            if( Earthdawn.repeatSection( pre, 3 ) === "T" ) {
            this.strainAdd( pre2 );
            this.Karma( pre2 + "Karma", 0 );
            ...
            let aname;
            if( !isng && wilselect == "Circle" )    aname = "SrRating";
            else if( wilattr )                      aname = "Wil";
            else if( wilselect == "Circle" )        aname = pre2 + "Circle";
            else if( wilselect == "Rank" )          aname = pre2 + "Effective-Rank";
            else                                    aname = pre2 + "Step";

            let t2 = Earthdawn.parseInt2( Earthdawn.getAttrBN( this.charID, aname, 5));
            this.Lookup( 1, [ "value", spellseq.spelldata.WilEffect, t2.toString(),  spellseq.ESEffect||0  ] );
            if( !skipseq ) {
              spellseq.SeqStep = 5;
              SaveSeq();
              this.misc[ "endNote" ] = "Apply Dmg and then " + buttonOnDemand( "reset" );
            }
            this.ForEachHit( [ "Roll" ] );  
  } break;    // end effect
          case "WarpTest": {
            let circle = Earthdawn.parseInt2( spellseq.spelldata.Circle );
            let region = ssa[ 3 ];
            let step, dmg;
            switch( region ){
            ...
            this.misc[ "targetNum" ] = Earthdawn.getAttrBN( this.charID, "MD-Nat", "0");
            this.misc[ "sayTotalSuccess" ] = true;
            ...
            "Warping Damage done to the character" , "damage");
            ssa[ 2 ] = "WarpTest2";
            this.misc[ "Spell" ] = ssa;
            this.rollPre( [ "Roll"] );
          } break; //End WarpTest
          ...
          case "HorrorMark": {
            ...
            this.misc[ "reason" ] = "Horror Mark Test " + region + " Astral Space";
            this.misc[ "headcolor" ] = "action";
            this.bFlags |= Earthdawn.flags.VerboseRoll;
            this.misc[ "step" ] = step;
            this.misc[ "targetChar" ] = this.charID;
...
//                this.TokenSet( "clear", "spellseq", "{}" );
            ...
            // ParseObj.strainCalc()
            // Note that this is meant to be used in conjunction with strainAdd and calculates the strain commands that have been saved.
            // strainSave is a $ delimited list that contains one of the following: 
            //    Code, rowID     Lookup the strain variables (below) and calculate the strain.
            //    repeating_(section)_(rowID)_(code)_   Use the code and rowID to do as above.
            //    A label, a strain value.    Add the strain to the total.
            /*
              ...
              T_StrainAdvanced_max is any numeric part from the drop down list. 

              Example, 4 + 2 per success will have "Strain": 4, "StrainAdvanced": "per Success", "StrainAdvanced_max": 2. 
               ...
               *Strain: 6
                Strain: 1 + 1 per additional target. 
                ...
                Number of threads (this would be ask). 
              Note: Deeper secets uses format "Strain: 1+ (see text)" with in the text: Each additional target costs an additional 1 Strain.
              Note: This routine will get called once per target, but that is OK as only the last one counts.
            */
    this.strainCalc = function()  {
      'use strict';
      try {
        let po = this, str = 0, tt = "", retSpecial = [], pre, save = ("strainSave" in this.misc) ? this.misc[ "strainSave" ] : "";
        let m = save.split( "$" );      // Major part is a $ delimited list. Split into seperate entries.

        for( let i = 0; i < m.length; ++i ) {
          let n = m[ i ].split( "," );
          if( Earthdawn.safeString( n[ 0 ] ).trim().startsWith( "repeating_" ))    // we have a pre
            pre = Earthdawn.safeString( n[ 0 ] ).trim();
          else if( Earthdawn.codeToName( n[ 0 ], true ) && n.length > 1 )   // we have a code, and a rid
            pre = this.buildPre( n[ 0 ], n[ 1 ]);
          else
            pre = undefined;

          if( pre ) {
            let s    = Earthdawn.getAttrBN( this.charID, pre + "Strain", 0, true ),
                as   = Earthdawn.getAttrBN( this.charID, pre + "StrainAdvanced", "", 2 ).trim(),
                asm  = Earthdawn.getAttrBN( this.charID, pre + "StrainAdvanced_max", 1, true ),
                name = Earthdawn.getAttrBN( this.charID, pre + "Name", "" );
            if( s != 0 ) {
              str += s;
              tt  += s;
            }

                // generic error message formating
            function err( tag, tip, logmsg ) {
              'use strict';
              if( tag )
                retSpecial.push( tag );
              if( tip )
                tt += Earthdawn.constantIcon( "warning" ) + "(" + tip + "). ";
              if( logmsg !== null )   // if we explicitly pass a null, then don't do an errorlog. Otherwise, use tip plus anthing that might be in logmsg (if undefined then just log tip).
                Earthdawn.errorLog( "ED.strainCalc() " + ( tip ? tip : "") + (logmsg ? " " + logmsg + " " : ""), po );
            }

                // most of these do the same basic thing with different things found in this.misc. 
                // if special 0x01 then DO NOT log an error message if the parameter is not found. 
                // 0x02 means value is passed, not a this.misc.
            function strainStandard( att, special ) {
              'use strict';
                let tmp = (special & 0x02) ? att : ((("misc" in po) & (att in po.misc)) ? po.misc[ att ] : 0);       // Usually value is in this.misc, but sometimes it is just passed. 
                if ( tmp !== undefined ) {
                  let x = tmp * asm;
                  if( x != 0 ) {
                    str += x;
                    tt += "+" + asm + as + "(" + tmp + ")=" + x + (name ? " (" + name + ")" : "");
                  }
                } else if ( !(special & 0x01 ))
                  err( "Error", "Could not find " + as, " : " + att + " : " + save );
            } // end strainStandard processing
            switch( as ) {
              case "Fixed": case "":
                if( s != 0 )
                  tt += " Fixed" + (name ? " (" + name + ")" : "");
                break;
              case "per additional Target":            // used where any book says "per target, per additional target, or target count".
                strainStandard( this.targetIDs.length -1, 0x02 );
                break;
              case "per Target":            // used where any book says "per target, per additional target, or target count".
                strainStandard( this.targetIDs.length, 0x02 );
                break;
              case "per Success":           // per success on the test being rolled. 
                strainStandard( "extraSucc", 0x01);
                if( "misc"  in this && ( !("succBy" in this.misc) && !("failBy" in this.misc)))
                  err( "Error", "Number of success' undetermined", null );
                break;
              case "x Rank":                  // Rank of talent used.
                strainStandard( "effRank" );
                break;
              case "x Step":                  // Effective step of the talent being used. 
                strainStandard( "finalStep" );
                break;
              case "x TMD":                   // Targets Mystic Defense is the same as the casting difficulty. 
                if(( "targettype" in po.misc ) && ( po.misc[ "targettype" ].indexOf( "MD") != -1 ))
                {} else
                  err( "Error", "targettype is not MD", ("targettype" in po.misc) ? ". targettype is " + po.misc[ "targettype" ] : "" );     // note, falls through to x Casting Difficulty on purpose.
              case "x Casting Difficulty":
                if(( "targetNum" in this.misc ) && this.misc[ "targetNum" ] != undefined )
                  strainStandard( this.misc[ "targetNum" ], 0x02 );
                else
                  err( "Error", "targetNum is undefined", null );
                break;
              case "x Force Rating":          // Force rating of the spirit being dealt with.
                strainStandard( "highSR" );
                if( !("highSR" in this.misc))
                  err( "Error", "Spirt or Challenge Rating undefined", null );
                break;
              case "x Number of Threads":     // Number of threads woven to the spell. 
// CDD do we want to test if this is really part of a spellcasting sequence?
                let x = Earthdawn.getAttrBN( this.charID, "SS-CurThreads", "0/0" );
                strainStandard( Math.max( Earthdawn.getParam( x, 1, "/" ), Earthdawn.getParam( x, 2, "/" )), 0x02);
                break;
              case "Ask":   // unused.
              case "Variable":
                err( as, "uncalculated value " + (name ? " (" + name + ")" : "") + " Click this to change strain to the correct value.", null );
                break;
              default:
                Earthdawn.errorLog( "ED.strainCalc() error unknown StrainAdvanced entry: " + as, this );
                err( "Error", "Unknown strain code '" + as + "'", this.misc[ "strainSave" ] );
            }
          } else {        // otherwise it is a text description and strain. 
            let x = Earthdawn.parseInt2( n[ 1 ] );
            if( x != 0 ) {
              str += x;
              tt += Earthdawn.safeString( n[ 0 ] ).trim() + ": " + Earthdawn.safeString( n[ 1 ] ).trim();
          } }
          let x2 = Earthdawn.getAttrBN( this.charID, pre + "Strain_max", "" );   // strain_max can contain a special strain explanation, usually used with Variable.
          if( x2 && isNaN( x2 ))    // Due to histtorical reasons, there might be a simple number in Strain_max, in which case ignore it.
            tt += x2;
          tt += ",   ";
        } // end outer loop. 
// cdd check for battle rites and modify. if so, modify retSpecial
        return { strain: str, tooltip: tt.replace( /^[\s|\,]+/g, "").replace( /\,\s*\,+/g, ",").replace( /[\s|\,]+$/g, ''), special: retSpecial };     // trim off trailing spaces and the trailing comma. 
      } catch(err) { Earthdawn.errorLog( "ED.strainCalc() error caught: " + err, this ); }
    } // End ParseObj.strainCalc



        // strainAdd:  save a source of strain to be calculated later.
        // arguments will ether be a valid prefix, or it will be a text, label. 
        // In ether case just save them to be evaluated later.
        // Format of this.misc[ "strainSave" ] is  what1, rid1 $ what2, rid2  etc. IE: major separator $, minor comma. 
    this.strainAdd = function()  {
      'use strict';
      let t = "";
      if( arguments.length > 0 )  t = arguments[ 0 ];
      for( let i = 1; i < arguments.length; ++i )
        t += ", " + arguments[ i ];
      this.misc[ "strainSave" ] = (( "strainSave" in this.misc) ? this.misc[ "strainSave" ] + "$ " : "") + t;
    } // End ParseObj.strainAdd
...
// cdd Aug 2024, do not automatically lose defensive stance. 
...
//                val -= Earthdawn.getAttrBN( cID, "Misc-DefStance-Bonus", 3);    // remove the Defensive stance bonus target had when blindsided.
          ...
          if( cID ) {
            let tsr = Earthdawn.getAttrBN( cID, "SrRating", "-9" );     // strainCalc wants to know about any spirit ratings the target might have.
            if( tsr > 0 )
              sr = tsr;
            ...
            if( this.charID && Earthdawn.getAttrBN( this.charID, "condition-TargetPartialCover", "0") == "1" && Earthdawn.getAttrBN( cID, "condition-Cover", "0") == "0")
            ...
            val += Earthdawn.getAttrBN( this.charID, "Adjust-TN-Total", "0", true );    // If this character has a condition which causes it's target numbers to be adjusted.
            ret = { val: val, name: TokObj.get( "name"), spiritRating: sr};
          }
...
// ["!Earthdawn","TargetType: Set","TokenList: -KMgp4hXFfF86s6ZcJJe","TargetSet : -KLOmKTy5HOb6qHm-SXK"]
    ...
    this.TargetT = function( ssa )  {
      'use strict';
      try {
        let flg, oneTarg, tType;
       if( ssa.length > 1) {
          if( !isNaN( ssa[ 1 ]) && ( ssa[ 1 ] !== ""))
            this.misc[ "targetNum" ] = this.ssaMods( ssa );
          else {
            tType = Earthdawn.safeString( ssa[ 1 ] );
            switch( tType.slice(0,2).toLowerCase() ) {
              case "":    // blank
              case "no":  // none
                break;
              case "as":  // ask
                this.misc[ "targetNum" ] = this.ssaMods( ssa, 2, 1);
                this.bFlags |= Earthdawn.flagsTarget.Ask;
                break;
              case "ri":  // Riposte
                this.bFlags |= this.TargetTypeToFlags( ssa[ 3 ] ) | Earthdawn.flagsTarget.Riposte;
                this.misc[ "targetNum" ] = this.ssaMods( ssa, 3, 1);
        ...
        if( oneTarg === true ) {      // we are not going to process this command as is.  Instead we are going to make new commands and insert them into the command line.
          ...
          for( i = this.indexMsg + 2; i < this.edClass.msgArray.length; ++i )
          ...
          if( s === undefined)
            ...
            if( s && flg === 1 )            // There are already targets assigned to at least one of these tokens.
              ...
              for( let j = 1; j < 11; ++j ) {
                ...
                for( let k = 1; k <= j; ++k )
                  s += Earthdawn.constantAlt( "ColonAlt" ) + Earthdawn.constantButton( "at" ) + Earthdawn.constantButton( "braceOpen" ) + "target"
                        + Earthdawn.constantButton( "pipe" ) + a[k] + " Target" + Earthdawn.constantButton( "pipe" ) + "token_id" + Earthdawn.constantButton( "braceClose" );
//                  s += Earthdawn.colonFix( ": " + Earthdawn.constantButton( "at" ) + "{target|" + a[k] + " Target|token_id}");
                s += t + ")";
              }
              s += "}}";
              this.chat( s, Earthdawn.whoTo.player | Earthdawn.whoFrom.noArchive );
            }
            this.misc[ "targetNum" ] += this.ssaMods( ssa, 2) || 0;
        } }
    ...
    this.TokenActionToggle = function( lu, show )  {
      ...
      try {
        ...
        if (lu === "willforce")
          name = Earthdawn.constantIcon( "karma" ) + "WilFrc-T"
        if( name != undefined ) {
          if( !show )
...
//log( this.tokenInfo);
      try {
        ...
        if( secondary !== undefined )
          secondary = secondary.replace( /,/g, Earthdawn.constantAlt( "CommaAlt" )).replace( /:/g, Earthdawn.constantAlt( "ColonAlt" ));
        if( tertiary !== undefined )
          tertiary = tertiary.replace( /,/g, Earthdawn.constantAlt( "CommaAlt" ) ).replace( /:/g, Earthdawn.constantAlt( "ColonAlt" ));
    ...
    this.TokenGet = function( key, retString )  {
      ...
      try {
        ...
        while ( i > -1 ) {
            ...
            ret.push( markers.slice( i + key.length + 1, e).replace(new RegExp( Earthdawn.constantAlt( "ColonAlt" ), "g"), ":").replace(new RegExp( Earthdawn.constantAlt( "CommaAlt" ), "g"), ",") );
    ...
    this.TokenGetWithID = function( key, tokenID )  {
     'use strict';
      ...
      return ret;
          ...
          // ssa = TuneMatrix, Spell, spell row ID.
          // ssa = TuneMatrix, Matrix, spell row ID, Matrix row ID.
          // ssa = TuneMatrix, share or pseudo, spell row ID, (Std, Enh, Arm, Sha)
          // ssa = TuneMatrix, destroy"
        ...
        function WipeMatrix() {
          ...
          _.each( attflt, function (att) {
            if( att.get( "name" ).endsWith( "_SPM_Contains" )) {
              Earthdawn.abilityRemove( po.charID, Earthdawn.constantIcon( "Spell" ) + att.get( "current" ));
              let pre3 = po.buildPre( att.get("name"));
          ...
          attflt = attributes.filter( function (att) { return att.get( "name" ).endsWith( "_SP_Name" )});
          _.each( attflt, function (att) { po.setWW( po.buildPre( att.get( "name" )) + "spmRowID", "0" ); }); //End each Attribute
        ...
        switch ( Earthdawn.safeString( ssa[ 1 ] ).toLowerCase() ) {
          case "spell": {     // The user has chosen "Attune" from the button in the spell list. Send a message to the chat window asking what matrix to put this spell into.
            if( ssa === undefined || ssa.length < 3 || ssa[2] === "") {
              ...
              let circle = Earthdawn.getAttrBN( po.charID, po.buildPre( "SP", ssa[ 2 ] ) + "Circle", "0", true ),
                attributes = findObjs({ _type: "attribute", _characterid: this.charID }),
                matrices = [];
              _.each( attributes, function ( indexAttributes ) {
                if( indexAttributes.get( "name" ).endsWith( "_SPM_RowID" )) {
                  let row = indexAttributes.get( "current" );
                  if ( ( Earthdawn.getAttrBN( po.charID, po.buildPre( "SPM", row ) + "Rank", 0, true) >= circle || Earthdawn.getAttrBN( po.charID, po.buildPre( "SPM", row ) + "Origin", "") == "Pseudo" ) && !matrices.includes( row ))   // Only list the matrices of a rank ot hold this spell and are not already listed. Pseudo are now Rank 0, but listed anyway
                    matrices.push( row );
                }
              }); // End for each attribute.
              if( matrices.length > 0) {      // For each matrix found, make a chat button.
                let t2;
                let s = Earthdawn.makeButton( "Pseudo matrix", "!Earthdawn~ charID: " + po.charID + "~ TuneMatrix: Share: " 
                      + "?{What type of Pseudo Matrix|Standard,Std|Enhanced,Enh|Armored,Arm|Shared,Sha}: " + ssa[ 2 ], "Create a Pseudo Matrix & Attune. The spell in pseudo matrix should be a companion of one in a real matrix.", "param" );

                _.each( matrices, function (indexMatrix) {
                  let prespm = po.buildPre( "SPM", indexMatrix );
                  ...
                  s += Earthdawn.makeButton( pseu + t2 + (pseu ? "" : "-" + Earthdawn.getAttrBN( po.charID, prespm + "Rank", "0")) + " "
                          ...
                          undefined , "param" );
                  if( pseu )
                    s += " " + Earthdawn.makeButton( Earthdawn.constantIcon( "pointLeft" ) + "X", "!Earthdawn~ charID: " + po.charID + "~ TuneMatrix: Destroy: " + indexMatrix
                          ...
                          + "Use this button to delete the Pseudo matrix.", "param2" );
                }); /// End each spell matrix
                this.chat( "Load " + Earthdawn.getAttrBN( po.charID, po.buildPre( "SP", ssa[2] ) + "Name", "")
...
//                this.chat( "&{template:default} {{name=Load " + Earthdawn.getAttrBN( po.charID, po.buildPre( "SP", ssa[2] ) + "Name", "")
//                      + " into which Matrix? " +  Earthdawn.colonFix( s ) + "}}", Earthdawn.whoTo.player | Earthdawn.whoFrom.noArchive);      // Chat buttons don't like colons.   Change them to something else. They will be changed back later.
          ...
          case "pseudo":    // TuneMatrix: share: (Std, Enh, Arm, Sha): (spellFrom ID)
          ...
          case "matrix": {    // The user has told us to swap a spell into a certain matrix.
                  // TuneMatrix: matrix: (spell Row ID): (Matrix Row ID)
            let spellFrom = ssa[ ssa.length -2 ],     // rowID of repeating_spell
                matrixTo = ssa[ ssa.length -1 ],      // rowID of repeating_matrix
                preFrom = po.buildPre( "SP", spellFrom ),
                preTo = po.buildPre( "SPM", matrixTo ),
                lnksold = [],
                ...
                t = Earthdawn.getAttrBN( this.charID, preTo + "spRowID", "");
                if( t ) {       // If there was a spell previously in the matrix, make a note of it. 
                  let preFromOld = po.buildPre( "SP", t );     //Prefix of the Spell that was in the Matrix before
                  if( preFromOld )
            ...
            if ( pseudoType ) {
              ...
              _.each(lnksold, function(sp) { //Destroy all the pseudo Matrices
                let x=sp.replace("SP;",""),
                  spm= Earthdawn.getAttrBN(po.charID, po.buildPre("SP",x)+"spmRowID",""),
                  spmt=spm.length>0 ? Earthdawn.getAttrBN(po.charID, po.buildPre("SPM",spm)+"Origin",""): "";
              ...
              _.each(lnks, function(sp) {
                ...
                if(x && x.length>0 && Earthdawn.getAttrBN(po.charID,po.buildPre("SP",x)+"Type","Spell")=="Spell")
            ...
            Earthdawn.abilityRemove( this.charID, Earthdawn.constantIcon( "Spell" ) + aobj.get( "current" ) );
            Earthdawn.setWithWorker( aobj, "current", t );
            if( Earthdawn.getAttrBN( po.charID, preFrom + "CombatSlot", 1, true))
              Earthdawn.abilityAdd( this.charID, Earthdawn.constantIcon( "Spell" ) + t, "!edToken~ %{selected|" + preTo + "Roll}" );
...
//            Earthdawn.setWithWorker( aobj, "current", matrixTo );
            ...
            if( mThreads > 0 && sThreads < 1 ) {
              ...
              for( let i = 0; i < opt.length; ++i )
                txt += " " + Earthdawn.makeButton( opt[ i ], "!Earthdawn~ charID: " + this.charID + "~ TuneMatrix: thread: " + matrixTo + ":" + opt[ i ],
                      "Press this button to tie this extra thread option into the matrix.", "param" );
            }
          ...
          case "inmatrix": {    // This alternate entry point is for when it detects a matrix has been destroyed.
            ...
            _.each( spells, function (rID ) {     // Set InMatrix depending upon if spellsInMatrix includes the spell.
              ...
              Earthdawn.setWW( po.buildPre( "SP", rID ) + "spmRowID", ((ind === -1) ? "0" : matrixes[ ind ]), po.charID );
            });
          } break;
          case "thread": {      // Set this extra thread option to be the enhanced thread.
            this.setWW( po.buildPre( "SPM", ssa[ 2 ] ) + "EnhThread", ssa[ 3 ] );
          ...
          case "save": { //Saves a Preset of Spell Matrices
            ...
            if( !prid ) prid = ssa[ 2 ];
            if( !pnm )  pnm  = Earthdawn.getAttrBN( po.charID, po.buildPre( "SPP", ssa[ 2 ] ) + "Name", "");
            let prespp = po.buildPre( "SPP" , prid );
            ...
            _.each( attflt, function (att) {
              ...
              this.setWW( prespp + "Preset", pst.join( "\n" ), undefined, JSON.stringify( pstm ));    // current, default undefined, max
          ...
          case "load":{ //Loads a Preset of Spell Matrices
            let prespp = po.buildPre("SPP", ssa[ 2 ] ),
             ...
             _.each( js, function (att) { //Each item saved in the Preset_max
              ...
              } else  if( !att.Contains || !(att.Contains.includes(Earthdawn.getAttrBN( po.charID, po.buildPre( "SP" , att.spRowID ) + "Name","")))) {
                ok=false;
                log("Earthdawn.TuneMatrix() Failed to load preset, spell doesn't exist  "+ att.Contains)
              } else {
                po.setWW(po.buildPre( "SP", att.spRowID ) + "spmRowID" , rid)
                for(let key in att)
                  po.setWW( prespm + key  , att[ key ] );
              }
              if( Earthdawn.getAttrBN( po.charID, po.buildPre( "SP", att.spRowID ) + "CombatSlot", 1, true))
                Earthdawn.abilityAdd( po.charID, Earthdawn.constantIcon( "Spell" ) + att.Contains, "!edToken~ %{selected|" + prespm + "Roll}" );
    ...
    this.Parse = function( cmdSegment )  {
      ...
      try {
        this.edClass.stackInfo = cmdSegment;
        Earthdawn.stackInfo = cmdSegment;
...
//log( cmdSegment);
        switch ( Earthdawn.safeString( subsegmentArray[ 0 ] ).toLowerCase() ) {
          ...
          case "increment":
            ...
            break;
          case "init":
            falloutParse = this.rollPre( subsegmentArray );
            break;
          case "karma":
          case "kc":
          case "dev pnt":
          ...
          case "dp-ask":
            ...
            break;
          case "keyword":      // comma seperated list of everything after this key. stored for use in keywordCheck()
            this.misc[ "keyword" ] = (( "keyword" in this.misc ) ? this.misc[ "keyword" ] : "") + "," + subsegmentArray.slice( 1 ).join( "," ) + ",";
            break;
          case "buttonlink":
...
// record and record2 obsolete Oct 23. Remove.
          ...
          case "statustotoken":
            ...
            break;
          case "step":            // Note also that THIS routine (unlike other routines such as ssaMods()) allows use of asynchronous process to interpret a calculated value.
          ...
          case "value":
            ...
            case "str-step":                this.Karma( "Str-Karma" );    this.misc[ "reason" ] = "Knockdown Test";         this.misc[ "headcolor" ] = "knockdown";       this.misc[ "Special" ] = "Knockdown";   this.misc[ "ModType" ] = "Effect";    break;
            case "knockdown":               this.Karma( "Str-Karma" );    this.misc[ "reason" ] = "Knockdown Test";         this.misc[ "headcolor" ] = "knockdown";       this.misc[ "Special" ] = "Knockdown";   this.misc[ "ModType" ] = "Effect";    break;
            ...
            case "casting": {
log("I think this is dead code (casting). Please report to ChrisD what you were doing when this appeared.");
log(cmdSegment);
              let base = this.buildPre( "SPM", subsegmentArray[2] );
              this.misc[ "headcolor" ] = "action";
  ...
  // redo karma control.   Dead code message July 24.
...
log("I think this is dead code (will-effect). Please report to ChrisD what you were doing when this appeared.");
log(cmdSegment);
              this.misc[ "headcolor" ] = "effect";
          ...
          case "modvalue":
            ...
            break;
          case "strainsave":
            this.misc[ "strainSave" ] = (( "strainSave" in this.misc) ? this.misc[ "strainSave" ] + "$ " : "") + cmdSegment.replace(/^\s*strainSave\s*[\~\$\:]\s*/i, "" );    // get rid the key to get us here, but save the rest.
// cdd.   Note, this might be unnessisary for spells with changes to spellinit.
//log( "strainsave " + cmdSegment);
//log( this.misc[ "strainSave" ] );
          case "storeintoken":
          ...
          case "targetnum":
            ...
            break;
          case "targetspell": {
            let tt = Earthdawn.getAttrBN( this.charID, this.buildPre( "SPM", subsegmentArray[1] ) + "Casting", "MDh");
          ...
          case "modtarget":
          case "targetvalue":
            falloutParse = this.Lookup( 3, subsegmentArray );       // 1 = this.misc[ "targetNum" ]
            break;
...
//
  ...
  if( origMsg !== undefined ) {
    origMsg.content = origMsg.content.replace( new RegExp( Earthdawn.constantAlt( "ColonAlt" ), "g"), ":");      // Buttons don't like colons, so any colon has been changed to this weird character. Change them back.
...
// }); // end  on("change:attribute"
  ...
  on("add:character", function( obj ) {     // Brand new character. Make sure that certain important attributes fully exist.
    'use strict';
    let ED = new Earthdawn.EDclass( "add character", obj.get( "_id" ));
...
//log("destroying");  log( attr);
        setTimeout( function() {
          try {
            ...
            if( arr.length > 0 ) {
              let ED = new Earthdawn.EDclass( "destroy attribute", nm );
              let edParse = new ED.ParseObj( ED );
                  ...
                  for( let j = 0; j < a2.length; ++j ) {
                    if( a2[ j ].startsWith( "repeating_") && cnt < 1 ) {      // if it is not linking to a repeating section, then it is not bidirectional, and we don't have anything to do for this link.
                      edParse.ChatMenu( [ "ChatMenu", "LinkRemoveHalf",
                          Earthdawn.buildPre( edParse.charID, Earthdawn.repeatSection( 3, a2[ j ] ), Earthdawn.repeatSection( 2, a2[ j ] ))
                          + "LinksProvideValue", Earthdawn.repeatSection( 2, nm )] );
                      ++cnt;
                  } }
                } else      // LinksProviceValue are comma delimited lists of form (code);(rowID).
                  edParse.ChatMenu( [ "ChatMenu", "LinkRemoveHalf",
                      Earthdawn.buildPre( edParse.charID, Earthdawn.getParam( arr[ i ], 1, ";" ), Earthdawn.getParam( arr[ i ], 2, ";" ))
        ...
        }, 2000 );    // end delay 2 seconds.
      }
      else if( nm.endsWith( "_SPM_spRowID" )) {      // Spell matrix has been deleted.
        setTimeout( function() {
          try {
            if( !testDeletion() ) return;
            let ED = new Earthdawn.EDclass( "destroy attribute SPM", nm);
            let edParse = new ED.ParseObj( ED );
            edParse.charID = attr.get( "_characterid" );
        ...
        // Note: Feb 2025, changed to Mooks only. 
  function onChangeDamage(what, attr, prev) {
    ...
    try {
      ...
      if( !rep )  return;
      if( Earthdawn.getAttrBN( rep, "NPC", "1" ) != "2")  return;     // Only do this for Mooks. Sheetworkers will do this for non-mooks.
...
//      log( what + "  prev '" + prev [ "bar3_value" ] + "'   '" + prev [ "bar3_max" ] + "'   attr '" + attr.get( "bar3_value" ) + "'  '" + attr.get( "bar3_max" ) 
//          + "'   Death '" + death + "'   unc '" + unc + "'   dam  '" + dam + "'" );

      function health() {
        if( dam < unc)          return "u";   // Set Healthy.
        else if( dam < death)   return "s";   // Set unconscious
        else                    return "a";   // Set to special value dead.
      }

      let whatcurr = health();
      let whatprev = whatcurr;
      if( prev ) {
        if( what === "value" && prev[ "bar3_value" ] != "")
          dam = prev[ "bar3_value"];
        else if ( what === "max" && prev[ "bar3_max" ] != "")
          unc = prev[ "bar3_max" ];
        whatprev = health();
      }
      if( !prev || whatcurr !== whatprev) {   // Health status has changed.
        let ED = new Earthdawn.EDclass( "onChangeDamage", rep);
        let edParse = new ED.ParseObj( ED );
        edParse.charID = rep;
...
//log( "change graphic represents"); log( prev); log(attr);
      let ED = new Earthdawn.EDclass( "onChangeGraphicRepresents", attr.get( "represents" ));
      let edParse = new ED.ParseObj( ED );
      edParse.charID = attr.get( "represents" );
      edParse.chat( "***You are attempting to link a token to character '" + Earthdawn.getAttrBN( edParse.charID, "character_name", "" )
          + "' using the roll20 interface.*** The Earthdawn API has its own link token button (in Parameters / Sheet / Special Functions) "
          + "that will automatically link the token and set the token bars to Damage, Wounds, and Karma. Do you want to "
          + Earthdawn.makeButton("Link token", "!Earthdawn~ charID: " + edParse.charID + "~ buttonLink: " + attr.get( "_id" )
          , "Link the token and the character and set the token bars.", "param" )
          + "? ***Note:*** please press the link button only after having saved the token parameters on the roll20 popup.", Earthdawn.whoTo.player | Earthdawn.whoFrom.noArchive )
//      edParse.LinkToken( [ "autoLink", attr.get( "_id" ) ] );   // pass tokenID
    } catch(err) { log( Earthdawn.timeStamp() + "on change represents() error caught: " + err ); }
  }); // end on change represents
...
//log("at change marker");
        ...
        if( removed.length || added.length ) {
          let ED = new Earthdawn.EDclass( "onChangeGraphicStatusmarkers", "added " + added.toString() + "   removed " + removed.toString());
          let edParse = new ED.ParseObj( ED );
...
//log("change:graphic:statusmarkers added :" + added[i]);
    ...
    let ED = new Earthdawn.EDclass( "Main Ready" );
    ED.Ready();
}); // End on("ready")
...
// cdd
// get spellcasting working without a token selected!
