          ...
          for( let ind = rankMin; ind < rankMax; ++ind) {     // We want this loop to go once for each rank being done.
            switch( wrapper ) {     // This switch is run once per rank gained.
              case "DSP_Circle":
                ...
                break;
              case "Increases":
                let stepValue = Math.floor(( 5 + ind + Earthdawn.parseInt2( Earthdawn.getAttrBN( cID, sa.slice( 0, -9 ) + "Race", "0" ))
                        + Earthdawn.parseInt2( Earthdawn.getAttrBN( cID, sa.slice( 0, -9 ) + "Orig", "0" ))) / 3);
  ...
  this.Ready = function () {
    ...
    if( state.Earthdawn.logStartup ) {
      log( "---Earthdawn.js Version: " + Earthdawn.Version
            ...
            + state.Earthdawn.game + " Edition: " + state.Earthdawn.edition + " ---");
...
//                      + "   NPC damage: "   + ((state.Earthdawn.tokenLinkNPC & Earthdawn.tokenLinkNPC.showplayers_damage) ? "true" : "false") + " ---" );
    ...
    if( smcText ) {
      ...
      smcText += " For instructions open this link " +
					edParse.makeButton( "Wiki Link", "https://wiki.roll20.net/Earthdawn_-_FASA_Official_V2#Import_the_Custom_Marker_Set",
  ...
  this.newCharacter = function( cID )  {
    ...
    try {
      setTimeout(function() {       // When a character is imported from the character vault, the recalc caused by edition has been observed to cause a race condition. Delay this processig long enough for the import to have been done.
        ...
        } catch(err) { this.edClass.errorLog( "ED.newCharacter setTimeout() error caught: " + err ); }
      }, 10000);    // end delay 1o seconds.
            ...
            if( Earthdawn.getAttrBN( this.charID, pre + "ActnEfct", "1" ) === "1") {    // 1 is action, -1 is effect, 0 is no roll.
              if( Earthdawn.getAttrBN( this.charID, "combatOption-DefensiveStance", "0" ) === "1" )
                if ( Earthdawn.getAttrBN( this.charID, pre + "Defensive", "0" ) === "1" ) {
                  step += Earthdawn.parseInt2( Earthdawn.getAttrBN( this.charID, "Misc-DefStance-Penalty", "3" ));      // Since this talent is defensive, we need to add this value that has already been subtracted out back in.
          ...
          case "wpn":
            ...
            if (state.Earthdawn.sheetVersion < 1.8 )
              ...
              this.doLater += "~Karma: " + pre + "Karma" + (strain ? "~Strain:" + strain : "");
                      def_attr = "Str";
            if (state.Earthdawn.gED)
              modtype = ((Earthdawn.getAttrBN( this.charID, pre + "CloseCombat", "0" ) == "1") ? "Damage CC": "Damage" );
            ...
            this.misc[ "headcolor" ] = ((Earthdawn.getAttrBN( this.charID, pre + "CloseCombat", "0" ) == "1") ? "damagecc": "damage" );;
            armortype = "PA";
            break;
          default:
...
//          modtypevalue = this.getValue( modtype.replace( /[@\(\{\}\)]/g, ""));    // Old, no longer valid.
        ...
        if ( special != undefined && special == "Initiative" ) {
          if( Earthdawn.getAttrBN( this.charID, "Creature-Ambushing", "0" ) == "1" )
            this.misc[ "step" ] += Earthdawn.parseInt2(Earthdawn.getAttrBN( this.charID, "Creature-Ambush", "0" ));
          newssa[ 0 ] = "Init";
          this.Roll( newssa );
...
*/
    ...
    this.calculateStep = function( ssa )  {
      ...
      try {
        ...
        case "jumpup":
            lu = Earthdawn.parseInt2( Earthdawn.getAttrBN( this.charID, "Dex", 5 ));      // This includes All-Tests-Total.
            lu += Earthdawn.parseInt2( Earthdawn.getAttrBN( this.charID, "condition-KnockedDown", "0" )) * 3;
            lu -= Earthdawn.parseInt2( Earthdawn.getAttrBN( this.charID, "Armor-IP", 0 )) + this.mookWounds();    // Note that this is only Armor-IP, not shield or misc mods.
        break;
        } // End switch
    ...
    this.ChatMenu = function( ssa )  {
      ...
      try {
        ...
        function addButtonWithCharID( label, item, tip, color ,colorfg) {
          s += edParse.makeButton( label, "!Earthdawn~ charID: " + edParse.charID + "~ " + item, tip, color, colorfg);
        }
        ...
        switch (ssa[ 1 ].toLowerCase()) {
          case "attrib": {
            ...
            s += this.makeButton( "JumpUp", "!edToken~ " + Earthdawn.constant( "percent" ) + "{selected|JumpUp}",
            ...
            s += this.makeButton( "New Day", "!Earthdawn~ charID: " + Earthdawn.constant( "at" ) + "{selected|character_id}~ ForEach: c~ Misc: NewDay",
          ...
          case "damage": {
            ...
            s += this.makeButton( "PA", "!Earthdawn~ setToken: @{selected|token_id}~ Damage: ?{Damage)|1}: ?{Type of damage|Damage|Stun} : PA",
                  "The Selected Token(s) take damage. Physical Armor applies.", Earthdawn.Colors.health, Earthdawn.Colors.healthfg );
            s += this.makeButton( "MA", "!Earthdawn~ setToken: @{selected|token_id}~ Damage: ?{Damage)|1}: ?{Type of damage|Damage|Stun} : MA",
                  "The Selected Token(s) take damage. Mystic Armor applies.", Earthdawn.Colors.health, Earthdawn.Colors.healthfg );
            s += this.makeButton( "NA", "!Earthdawn~ setToken: @{selected|token_id}~ Damage: ?{Damage)|1}: ?{Type of damage|Damage|Stun|Strain} : NA",
                  "The Selected Token(s) take damage. No Armor applies.", Earthdawn.Colors.health, Earthdawn.Colors.healthfg );
            s += this.makeButton( "Other", "!Earthdawn~ setToken: @{selected|token_id}~ Damage: ?{Damage)|1}: ?{Type of damage|Damage|Stun|Strain} : ?{Armor| None,NA| Physical,PA| Mystic,MA| Nat PA,PA-Nat| Nat MA,MA-Nat}",
            ...
            s += this.makeButton( "PA", "!Earthdawn~ setToken: @{target|token_id}~ Damage: ?{Damage)|1}: ?{Type of damage|Damage|Stun} : PA",
                  "Give damage to a Target token. Physical Armor applies.", Earthdawn.Colors.damage, Earthdawn.Colors.damagefg );
            s += this.makeButton( "MA", "!Earthdawn~ setToken: @{target|token_id}~ Damage: ?{Damage)|1}: ?{Type of damage|Damage|Stun} : MA",
                  "Give damage to a Target token. Mystic Armor applies.", Earthdawn.Colors.damage, Earthdawn.Colors.damagefg );
            s += this.makeButton( "NA", "!Earthdawn~ setToken: @{target|token_id}~ Damage: ?{Damage)|1}: ?{Type of damage|Damage|Stun|Strain} : NA",
                  "Give damage to a Target token. No Armor applies.", Earthdawn.Colors.damage, Earthdawn.Colors.damagefg );
            s += this.makeButton( "Other", "!Earthdawn~ setToken: @{target|token_id}~ Damage: ?{Damage)|1}: ?{Type of damage|Damage|Stun|Strain} : ?{Armor| None,NA| Physical,PA| Mystic,MA| Nat PA,PA-Nat| Nat MA,MA-Nat }",
            ...
            switch (submenu) {
              ...
              case "2":
                ...
                s += "   For more information "
                    + this.makeButton( "FX Help Wiki", "https://help.roll20.net/hc/en-us/articles/360037258714-F-X-Tool#F/XTool-CustomFXTool",
...
//            s += this.makeButton("Token Link Options", "!Earthdawn~ Misc: State: tokenLinkNPC: ?{Change what|Show Players NPC nameplate,0x01|Show Players NPC karma,0x02|Show Players PC karma,0x04|Show Players NPC Wounds,0x08|Show Players NPC Damage,0x10}: ?{Set or Unset|Set,1|Unset,0}",
              s += this.makeButton("Token Link Options", "!Earthdawn~ ChatMenu: gmStateLink",
...
*/
              ...
              s += this.makeButton("Version", "!Earthdawn~ Misc: State: ?{API version or HTML version|API|HTML}: ?{Version number to store}",
...
/*
              ...
              + this.makeButton("NPC karma", "!Earthdawn~ Misc: State: tokenLink: " + Earthdawn.tokenLinkNPC.showplayers_karma + ": ?{Set or Unset|Set|Unset}: NPC karma",
              ...
              + this.makeButton("PC karma", "!Earthdawn~ Misc: State: tokenLink: " + Earthdawn.tokenLinkNPC.showplayers_pcKarma + ": ?{Set or Unset|Set|Unset}: PC karma",
              ...
              + this.makeButton("NPC wounds", "!Earthdawn~ Misc: State: tokenLink: " + Earthdawn.tokenLinkNPC.showplayers_wounds + ": ?{Set or Unset|Set|Unset}: NPC wounds",
              ...
              + this.makeButton("NPC damage", "!Earthdawn~ Misc: State: tokenLink: " + Earthdawn.tokenLinkNPC.showplayers_damage + ": ?{Set or Unset|Set|Unset}: NPC damage",
              ...
              s += "  set to: "
              + this.makeButton("Above", "!Earthdawn~ Misc: State: tokenLink: " + Earthdawn.tokenLinkNPC.npcAbove + ": " + Earthdawn.tokenLinkNPC.npcMask + ": NPC bars above",
                "Set NPC Tokens to show token bars Above the token. Defaults is Above" ,Earthdawn.Colors.param,Earthdawn.Colors.paramfg)
              + this.makeButton("Overlap Top", "!Earthdawn~ Misc: State: tokenLink: " + Earthdawn.tokenLinkNPC.npcOverTop + ": " + Earthdawn.tokenLinkNPC.npcMask + ": NPC bars overlap top",
              ...
              + this.makeButton("Overlap Bottom", "!Earthdawn~ Misc: State: tokenLink: " + Earthdawn.tokenLinkNPC.npcOverBottom + ": " + Earthdawn.tokenLinkNPC.npcMask + ": NPC bars overlap bottom",
              ...
              + this.makeButton("Below", "!Earthdawn~ Misc: State: tokenLink: " + Earthdawn.tokenLinkNPC.npcBelow + ": " + Earthdawn.tokenLinkNPC.npcMask + ": NPC bars below",
              ...
              + this.makeButton("Standard", "!Earthdawn~ Misc: State: tokenLink: " + Earthdawn.tokenLinkNPC.npcCompact + ": Unset: NPC bars standard",
                "Set NPC token bars to Standard. Defaults to Standard" ,Earthdawn.Colors.param,Earthdawn.Colors.paramfg)
              + this.makeButton("Compact", "!Earthdawn~ Misc: State: tokenLink: " + Earthdawn.tokenLinkNPC.npcCompact + ": Set: NPC bars compact",
              ...
              s += "  set to: "
              + this.makeButton("Above", "!Earthdawn~ Misc: State: tokenLink: " + Earthdawn.tokenLinkNPC.pcAbove + ": " + Earthdawn.tokenLinkNPC.pcMask + ": PC bars above",
                "Set PC Tokens to show token bars Above the token. Defaults is Above" ,Earthdawn.Colors.param,Earthdawn.Colors.paramfg)
              + this.makeButton("Overlap Top", "!Earthdawn~ Misc: State: tokenLink: " + Earthdawn.tokenLinkNPC.pcOverTop + ": " + Earthdawn.tokenLinkNPC.pcMask + ": PC bars overlap top",
              ...
              + this.makeButton("Overlap Bottom", "!Earthdawn~ Misc: State: tokenLink: " + Earthdawn.tokenLinkNPC.pcOverBottom + ": " + Earthdawn.tokenLinkNPC.pcMask + ": PC bars overlap bottom",
              ...
              + this.makeButton("Below", "!Earthdawn~ Misc: State: tokenLink: " + Earthdawn.tokenLinkNPC.pcBelow + ": " + Earthdawn.tokenLinkNPC.pcMask + ": PC bars below",
              ...
              + this.makeButton("Standard", "!Earthdawn~ Misc: State: tokenLink: " + Earthdawn.tokenLinkNPC.pcCompact + ": Unset: PC bars standard",
...
*/
          ...
          case "help": {
            s = this.makeButton( "Wiki Link", ( state.Earthdawn.sheetVersion < 1.8 ) ? "https://wiki.roll20.net/Earthdawn_-_FASA_Official"
...
//                  if( att.get("current") != "1" )     // commenting this out shows all skills, combat slot or not. 
              ...
              for( let j = 0; j < s1.length; ++j )
                s += edParse.makeButton( s1[j].a, s1[j].b, s1[j].c, Earthdawn.Colors.action, Earthdawn.Colors.actionfg);
              for( let j = 0; j < s2.length; ++j )
                s += edParse.makeButton( s2[j].a, s2[j].b, null, Earthdawn.Colors.action, Earthdawn.Colors.actionfg );
              for( let j = 0; j < s3.length; ++j )
                s += edParse.makeButton( s3[j].a, s3[j].b, null, Earthdawn.Colors.action, Earthdawn.Colors.actionfg );
              s += edParse.makeButton( "Languages", "!Earthdawn~ chatmenu: languages", null, Earthdawn.Colors.dflt, Earthdawn.Colors.dfltfg );
            ...
            for( let k in lst ) {
              ...
              _.each( attributes, function (att) {      // First we are listing all the spellcasting Talents.
                if (att.get("name").endsWith( "_T_Special") && att.get("current").startsWith("SPL-")){
                  ...
                  s += edParse.makeButton( getAttrByName( id, pre + "Name" ), "!edToken~ " + Earthdawn.constant( "percent" ) 
                        + "{" + id + "|" + pre + "Roll}",
                  ...
                  s2 += edParse.makeButton( att.get("current"), "!edToken~ " + Earthdawn.constant( "percent" ) + "{" + id + "|"
    ...
    this.CursedLuck = function( curse, roll )  {
      ...
      try {
        ...
        if( Earthdawn.parseInt2(Earthdawn.getAttrBN( this.charID, "NPC", "0")) > 0 ) {      // Pile-on prevention is only done on NPC dice rolls. PCs can pile on all they want.
    ...
    this.Damage = function( ssa ) {
      ...
      try {
        ...
        if( currDmg >= Earthdawn.getAttrBN( this.charID, "Damage-Death-Rating", 25 )) {
          ...
          newMsg += ".  Need to make a " + this.makeButton( "Knockdown",
...
//log( "Talent Found " + pre + " " + name + " "  + code +" " + rid + " " + cname);
              newMsg += po.makeButton( name + " test",
            ...
            for( let i = 1; i < ssa.length; i++) {
              ...
              if( !kAsk && !skip )
                if (kc > 0)
                  ...
                  kdice += Earthdawn.parseInt2( Earthdawn.getAttrBN( this.charID, "Karma-Roll", "0" ));
              if ( !dAsk && !skip )
                if (dp > 0)
                  ddice += dp;
                else if ( dp == 0 && state.Earthdawn.gED)
                  ddice += Earthdawn.parseInt2( Earthdawn.getAttrBN( this.charID, "Devotion-Roll", "0" ));
            } // End for each ssa.
...
//                        this.chat( this.tokenInfo.name + " spent " + kdice + " karma." );
          ...
          if( ddice > 0 ) {     // Are we spending more than zero Devotion Points?    Note: Devotion is a bit simplier than karma, since it in on the sheet only, not in the token bar.
            let kstep = Earthdawn.parseInt2( Earthdawn.getAttrBN( this.charID, "DevotionStep", "3" ));
            ...
            try {
              ...
              if( state.Earthdawn.gED && state.Earthdawn.edition == "4" ) {
                let karma = Earthdawn.findOrMakeObj({ _type: 'attribute', _characterid: this.charID, name: "Karma" }, 0),
                  kparam= Earthdawn.parseInt2(Earthdawn.getAttrBN( this.charID, "Misc-KarmaRitual", "-1" )),
                  add;
                switch (kparam) {
                ...
                let newKarma = Earthdawn.parseInt2(Earthdawn.getAttrBN( this.charID, "Karma_max", "0" )) - Earthdawn.parseInt2(Earthdawn.getAttrBN( this.charID, "Karma", "0" )),
                  today = new Date();
              ...
              if( !playerIsGM( this.edClass.msg.playerid ) )
                ...
                switch( ssa[ 2 ].toLowerCase() ) {
                  ...
                  case "edition": {
                    ...
                    state.Earthdawn.game = state.Earthdawn.gED ? "ED" : "1879";
                    state.Earthdawn.edition = Earthdawn.parseInt2( ssa[ 3 ] );
                    this.chat( "Campaign now set to use " + state.Earthdawn.game + " Edition " + Math.abs(state.Earthdawn.edition) + " rules."  );
                    let count = 0,
    ...
    this.RerollNpcInit = function()  {
      ...
      try {
        ...
        _.each( turnorder, function( sel ) {
          ...
          if ((Earthdawn.parseInt2( TokObj.get( "bar3_value" )) < (Earthdawn.getAttrBN( cID, "Damage_max", "20" ))) &&
                  ( Earthdawn.parseInt2(Earthdawn.getAttrBN( cID, "NPC", "0")) > 0 ))   // Not Items, not PCs, and nobody unconscious.
        ...
        for( let i = lstart; i < lend; ++i ) {
          if ( this.misc[ "Special" ] === "CorruptKarma" ) {
            ...
            if( !(this.bFlags & Earthdawn.flags.NoOppMnvr )) {    // We don't record these extra successes as something that affects damage rolls.
              ...
              _.each( attributes, function (att) {
                if (att.get("name").endsWith( "_WPN_Name" ))
                  if( Earthdawn.getAttrBN(po.charID, att.get( "name" ).slice( 0, -5) + "_CloseCombat", "0") == cc)
...
//log("this.Spell " + JSON.stringify(ssa));
        ...
        let sthrd = Earthdawn.parseInt2( Earthdawn.getAttrBN( this.charID, presp + "sThreads", "0")),                   //Spell Base Thread Number
          thrd = Earthdawn.parseInt2( Earthdawn.getAttrBN( this.charID, pre + (bGrim ? "sThreads" : "Threads"),"")),  //Starting Threads, ignores any extra thread
...
// log('this.Spell : initspell');
          let mthrd = 0;
          if( !bGrim ) {
            let type = Earthdawn.parseInt2(Earthdawn.getAttrBN( po.charID, pre + "Type", "0"));
...
// log("seq reinitialized with " + seq);
        ...
        _.each( attributes, function (att) {
          if (att.get( "name" ).endsWith( "_T_Special" ) && att.get( "current" ).startsWith( "SPL-" )) {      // If any of the SPL- Talents. 
            ...
            if(att.get( "current" ).endsWith( disp )) {       // Thread Weaving. 
              txtwv += po.makeButton( Earthdawn.getAttrBN( po.charID, pre2 + "Name", ""),
              ...
              txtwv2 += po.makeButton( Earthdawn.getAttrBN( po.charID, pre2 + "Name", ""),
                    "!Earthdawn~ charID: " + po.charID + "~ ForEach: inmt~ " + straintxt + kask + " Spell: " + ssa[ 1 ] + " : Weave : "+ rid + " : "+ssa[ ssa.length -1 ] ,
              ...
              if(Earthdawn.getAttrBN( po.charID, pre2 + "Strain", "") == "0")
                txtonfly += po.makeButton( "Attune on the Fly",
              ...
              txtcst += po.makeButton( Earthdawn.getAttrBN( po.charID, pre2 + "Name", ""),
              ...
              txtcst2 += po.makeButton( Earthdawn.getAttrBN( po.charID, pre2 + "Name", ""),
                    "!Earthdawn~ charID: " + po.charID +"~Target:" +target+ "~ ForEach: inmt~ " +straintxt+ kask + "Spell: " + ssa[ 1 ] + ": Cast : " + rid + " : " + ssa[ ssa.length -1 ] ,
              ...
              if( wilselect == "Rank" ) {     //Button for Effect for Rank based spells
                txtfx += po.makeButton( Earthdawn.getAttrBN( po.charID, pre2 + "Name", "") + " Rk",
                ...
                txtfx2 += po.makeButton( Earthdawn.getAttrBN( po.charID, pre2 + "Name", "") + " Rk",
                      "!Earthdawn~ charID: " + po.charID + "~ ForEach: inmt~ Spell: " + ssa[ 1 ] + ": Effect : "+ rid + ": " + ssa[ ssa.length -1 ] ,
              ...
              txtfx += po.makeButton( Earthdawn.getAttrBN( po.charID, pre2 + "Name", ""),
              ...
              txtfx2 += po.makeButton( Earthdawn.getAttrBN( po.charID, pre2 + "Name", ""),
                    "!Earthdawn~ charID: " + po.charID + "~ ForEach: inmt~ " +straintxt+ kask + "Spell: " + ssa[ 1 ] + ": Effect : "+ rid + ": " + ssa[ ssa.length -1 ] ,
              ...
              txtpatt += po.makeButton( "Attune Grimoire",
          ...
          if( wilselect == "Circle" && att.get( "name" ).endsWith( "_DSP_Code" ) && ( disp == Earthdawn.dispToName( att.get( "current" ), false ))) {   //Effect based on Circle
            rid = Earthdawn.getAttrBN( po.charID, Earthdawn.buildPre( att.get("name") ) + "RowID", "");
            txtfx += po.makeButton( Earthdawn.dispToName( att.get( "current" ), false ) + " Circle",
                    "!Earthdawn~ charID: " + po.charID + "~ ForEach: inmt~ Spell: " + ssa[ 1 ] + ": Effect : " + rid + ": " + ssa[ ssa.length -1 ] ,
                    "Effect Test based on this Discipline Circle." ,Earthdawn.Colors.param,Earthdawn.Colors.paramfg);
            txtfx2 += po.makeButton( Earthdawn.dispToName( att.get( "current" ), true ) + " Circle",
                    "!Earthdawn~ charID: " + po.charID + "~ ForEach: inmt~ Spell: " + ssa[ 1 ] + ": Effect : "+ rid + ": " + ssa[ ssa.length -1 ] ,
        ...
        let txtextra = this.makeButton( "Extra Threads", "!Earthdawn~ charID: " + this.charID + "~ ForEach: inmt~ Spell: " + ssa[ 1 ] + " : New : " + ssa[ ssa.length -1 ] ,
        ...
        let txtreset = this.makeButton( "Reset", "!Earthdawn~ charID: " + this.charID + "~ ForEach: inmt~ Spell: " + ssa[ 1 ] + ": Reset: " + ssa[ ssa.length -1 ],
              "Press this button to Reset the Spell Sequence." ,Earthdawn.Colors.param2,Earthdawn.Colors.param2fg);
        let txtinfo = this.makeButton( "Info", "!Earthdawn~ charID: " + this.charID + "~ ForEach: inmt~ Spell: " + ssa[ 1 ] + ": Info: " +ssa[ ssa.length-1 ],
              "Press this button to get the details of the spell." ,Earthdawn.Colors.param2,Earthdawn.Colors.param2fg);
        let txttunematrix = this.makeButton( "Attune Matrix", "!Earthdawn~ %{"+getAttrByName( this.charID, "charName" ) +"|" + pre +"AttuneButton}" ,
        ...
        let txtwipe = this.makeButton( "Wipe All Matrix", "!Earthdawn~ charID: " + this.charID + "~ ForEach: inmt~ Spell: " + ssa[ 1 ] + ": WipeMatrix: " + ssa[ ssa.length- 1 ],
        ...
        let txtwarp = this.makeButton( "Warp Test", "!Earthdawn~ charID: " + this.charID + "~ ForEach: inmt~ Spell: " + ssa[ 1 ] + ": WarpTest: ?{ What is the region type? |Safe|Open|Tainted|Corrupt} :" + ssa[ ssa.length -1 ],
        ...
        let txtmark = this.makeButton( "Horror Mark", "!Earthdawn~ charID: " + this.charID + "~  ForEach: inmt~ Spell: " + ssa[ 1 ] + ": HorrorMark: ?{ What is the region type? |Safe|Open|Tainted|Corrupt}:" +ssa[ ssa.length -1 ],
        ...
        switch( ssa[ 2 ] ) {
          ...
          case "Info": {
            ...
            if( !infoPub )
              line( this.makeButton( "Post to all", "!Earthdawn~ charID: " + this.charID + "~ ForEach: inmt~ Spell: " + ssa[ 1 ] + ": InfoPub: " +ssa[ ssa.length-1 ],
          ...
          case "WarpTest": {
            ...
            this.misc[ "endNoteSucc" ] = this.makeButton( "Warp Damage",
  ...
  //JBF@JBF@CD need to rewrite the damage button
          ...
          case "New": {
            ...
            line( this.makeButton( opt[ i ], "!Earthdawn~ charID: " + this.charID + "~ Spell: " + ssa[ 1 ] + ": " + "Extra: " + opt[ i ] + " : " + ssa[ ssa.length-1] ,
          ...
          case "Cast": {
            ...
            if( other && !seqended ) { //Another spell was in middle of an unfinished sequence
              ...
              aseq [ 3 ]="2";
              aseq[ 4 ] = "99";   //Skipping Weaving altogether
            } else if(( Earthdawn.parseInt2(Earthdawn.getAttrBN( this.charID, presp + "sThreads", "0" )) + aseq.length - 6) > (bGrim? 0 : Earthdawn.parseInt2( Earthdawn.getAttrBN( this.charID, pre + "mThreads", "0" )))) // We can skip this test is threads are not needed.
              ...
              this.misc[ "warnMsg" ] = "Note: Have you pulled all the threads you need? Only " + aseq [ 4 ] + " of "
                    + (Earthdawn.parseInt2(Earthdawn.getAttrBN( this.charID, pre + "sThreads", "0" )) + aseq.length - 6) + " done.";
              //Normal Case
            if(noseqactive || seqended) 
            ...
            pre2 = Earthdawn.buildPre( "T", rid);
            this.Karma( pre2+"Karma", 0 );
            if( Earthdawn.parseInt2( Earthdawn.getAttrBN( this.charID, pre2 + "Strain","0" ) !== 0))
              this.misc[ "strain" ] = Earthdawn.parseInt2(Earthdawn.getAttrBN( this.charID, pre2 + "Strain","0" )) ;
      ...
      // redo karma control
            ...
            if ( aseq[ 0 ] !== "Cast" ) {
              ...
              this.chat( "Error! Trying to cast: " + Earthdawn.getAttrBN( this.charID, pre + (!bGrim ? "Contains" : "Name"), "" )
                  ...
                  + (aseq[1] == "G" ? "Name" : "Contains"), "") + ".", Earthdawn.whoFrom.apiWarning );
              return;
            } else if( (Earthdawn.parseInt2(Earthdawn.getAttrBN( this.charID, pre + "sThreads", "0" )) + aseq.length - 6) > (bGrim
                      ? 0 : Earthdawn.parseInt2( Earthdawn.getAttrBN( this.charID, pre + "mThreads", "0" )) ))  // We can skip this test is threads are not needed.
              if( aseq [ 3 ] != "2" )
                this.misc[ "warnMsg" ] = "Note: Have you pulled all the threads you need? Only " + aseq [ 4 ] + " of "
                      + (Earthdawn.parseInt2(Earthdawn.getAttrBN( this.charID, pre + "sThreads", "0" )) + aseq.length - 6) + " done.";
            let fx = Earthdawn.getAttrBN( this.charID, pre + "FX", "");
      ...
      // redi karma
            ...
            switch( Earthdawn.getAttrBN( this.charID, pre + "WilSelect", "None") ) {
              case "Circle":    let tmp;
                ...
                _.each( attributes, function (att) {
                  if ( att.get("name").endsWith( "_DSP_Code" )) {
                    let t = att.get( "current" ),
                      c = Earthdawn.parseInt2( Earthdawn.getAttrBN( po.charID, att.get( "name").slice( 0, -5) + "_Circle", "0"));
                    if( t === tmp && c > exact)
                      exact = c;
                ...
                break;
              case "Rank":  t2 += Earthdawn.parseInt2( Earthdawn.getAttrBN( this.charID, "SP-Spellcasting-Effective-Rank", "0"));   // falls through on purpose. This is +rank, minus wil
...
// log("Modify "); log( attrib); log( newVal);
          ...
          if( [ "Dex-Step", "Str-Step", "Tou-Step", "Per-Step", "Wil-Step", "Cha-Step" ].includes( attrib )) {
            let base = attrib.slice(0, 3),
              orig = ((newVal - Earthdawn.parseInt2(Earthdawn.getAttrBN( po.charID, base + "-Adjust", 0)) -2) * 3) +2
                  - ( Earthdawn.parseInt2(Earthdawn.getAttrBN( po.charID, "Attrib-" + base + "-Race", 10))
                  + Earthdawn.parseInt2(Earthdawn.getAttrBN( po.charID, "Attrib-" + base + "-Increases", 0))
                  + Earthdawn.parseInt2(Earthdawn.getAttrBN( po.charID, base + "-Adjust-attr", 0)));
            ...
            function Adjust( what, tf, val, base, baseDef, adj ) {
              ...
              try {
                ...
                if( !mult ) {           // This is a set, not an adjust.
                  v = Earthdawn.parseInt2( val ) - Earthdawn.parseInt2(Earthdawn.getAttrBN( po.charID, base, baseDef))      // desired value minus current value gives new adjust.
...
//log( "adjust what: " + what + "   to find: " + tf + "   old " + save[ tf + "Old" ] + "   value: " + val + "   base: " + base + "   baseDef: " + baseDef + "   adjustment: " + adj + "   new: " + v );
            ...
            for( let i = blockIndexArray.length -2; i > -1; --i) {      // loop 1
              ...
              switch( tf ) {
                ...
                case "phyarm":  case "physical armor":    getStuff( i, 0x00, tf, "PA-Nat", 0 );         break;
                    ...
                    case "uncon": case "unconsciousness":
                      if( isNaN( save[ tf ] )) {    // Usually "NA" but sometimes "-"
                        ...
                        if( tv.startsWith( "+" ) || tv.startsWith( "-" ))
                          save[ tf ] = Earthdawn.parseInt2( tv ) + (Earthdawn.parseInt2( Earthdawn.getAttrBN( po.charID, "Damage-Death-Rating"))
        ...
        switch ( ssa[ 1 ].toLowerCase() ) {
          case "spell": {     // The user has chosen "Attune" from the button in the spell list. Send a message to the chat window asking what matrix to put this spell into.
            if( ssa === undefined || ssa.length < 3 || ssa[2] === "") {
              ...
              let circle = Earthdawn.parseInt2( Earthdawn.getAttrBN( po.charID, Earthdawn.buildPre( "SP", ssa[ 2 ] ) + "Circle", "0")),
              ...
              _.each( attributes, function ( indexAttributes ) {
                if( indexAttributes.get( "name" ).endsWith( "_SPM_RowID" )) {
                  ...
                  if ( Earthdawn.parseInt2( Earthdawn.getAttrBN( po.charID, Earthdawn.buildPre( "SPM", row ) + "Rank", 0)) >= circle )   // Only list the matrices of a rank ot hold this spell.
                    matrices.push( row );
                }
              }); // End for each attribute.
...
//                      + " into which Matrix? " +  Earthdawn.colonFix( s ) + "}}", Earthdawn.whoTo.player | Earthdawn.whoFrom.noArchive);      // Chat buttons don't like colons.   Change them to something else. They will be changed back later.
          ...
          case "matrix": {    // The user has told us to swap a spell into a certain matrix.
            ...
            else if( Earthdawn.parseInt2( Earthdawn.getAttrBN( po.charID, preFrom + "CombatSlot", "1")))
              Earthdawn.abilityAdd( this.charID, Earthdawn.constant( "Spell" ) + t,
                    "!edToken~ %{selected|" + preTo + "Roll}" );
...
//log("destroying");  log( attr);
      ...
      if( nm.endsWith( "_SPM_spRowID" )) {      // Spell matrix has been deleted.
        let ED = new Earthdawn.EDclass();
        let edParse = new ED.ParseObj( ED );
        edParse.charID = attr.get( "_characterid" );
        edParse.TuneMatrix( [ "Tune", "inMatrix" ]);
      }
    } catch(err) { log( "on destroy attribute() error caught: " + err ); }
