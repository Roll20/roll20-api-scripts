...
Earthdawn.Version = "3.020";      // This is the version number of this API file.  state.Earthdawn.sheetVersion is the version number of the html file. 
                                  // Each individual sheets edition_max is the sheetVersion it has been updated to.
  ...
  try {
    ...
    switch ( Earthdawn.safeString( what.toLowerCase() )) {    // For this upper half, we want to return html codes.
      ...
      case "power":         c = 0x23FB;   break;    // Power symbol
              // Here we have alternates for some strings we can't have interpreted too soon. So change them so roll20 does not recognize it wants to interprite them.
...
//log("this.chat " + iFlags + " customFrom " + customFrom);
    ...
    if ( !(iFlags & Earthdawn.whoTo.playerList )) {     // Don't do this section if specified to do the playerList section.
      ...
      if( edc && edc.msg && edc.msg.who && ( iFlags & Earthdawn.whoTo.player )
...
// CDD todo, need to roll these before applying!
...
// log('this.Spell : initspell');
          let mthrd = 0;
          if( !bGrim ) {
// CDD review, Mar 23 was getting value "free" in here on NPC wizard 2. 
//            let type = Earthdawn.getAttrBN( po.charID, pre + "Type", "0", true );
            let type = Earthdawn.getAttrBN( po.charID, pre + "Type", "0" );
...
// 	        let castdiff = Earthdawn.getAttrBN( this.charID, presp + "Casting", "MD1: @{target|Target|token_id}");
  ...
  //JBF@JBF@CD Attempt at Targetting
    ...
    this.strainCalc = function( pre )  {
      'use strict';
      try {
        return Earthdawn.parseInt2(("strain" in this.misc) ?  this.misc[ "strain" ] : this.getValue( pre + "Strain"));
...
// convert to message
      edParse.setWW( "SWflag", "SheetUpdate,"+ Math.random() );
