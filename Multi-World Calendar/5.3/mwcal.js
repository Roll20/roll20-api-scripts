	...
	(function () {
		...
		var version = '5.3',
			...
			calmenu = function () {
				...
				var arrowstyle =
					'style="border: none; border-top: 3px solid transparent; border-bottom: 3px solid transparent; border-left: 195px solid rgb(126, 45, 64); margin-bottom: 2px; margin-top: 2px;"';
				var headstyle =
					'style="color: rgb(126, 45, 64); font-size: 18px; text-align: left; font-variant: small-caps; font-family: Times, serif;"';
				...
				if (state.MWCalendar.now.wtype == 'ON' && state.MWCalendar.now.mtype == 'ON') {
					sendChat(
						...
						'/w gm <div ' +
							...
							'><a ' +
							astyle1 +
							'" href="!mwcal --world ?{World?|Faerûn|Greyhawk|Modern|Eberron|Tal\'Dorei|Random}">' +
							world +
							'</a></td></tr>' + //--
							'<tr><td ' +
							tdstyle +
							...
							'><a ' +
							astyle1 +
							'" href="!mwcal --set Hour --?{Hour?|' +
							state.MWCalendar.now.hour +
							'}">' +
							hour +
							'</a></td></tr>' + //--
							'<tr><td ' +
							...
							'><a ' +
							astyle1 +
							'" href="!mwcal --set Minute --?{Minute?|' +
							state.MWCalendar.now.minute +
							'}">' +
							min +
							'</a></td></tr>' + //--
							'<tr><td ' +
							...
							'><a ' +
							astyle1 +
							'" href="!mwcal --moon ?{Moon?|Full Moon|Waning Gibbous|Last Quarter|Waning Crescent|New Moon|Waxing Crescent|First Quarter|Waxing Gibbous|Random|Normal}">' +
							moon +
							'</a></td></tr>' + //--
							'<tr><td ' +
							tdstyle +
							...
							astyle2 +
							'" href="!mwcal --adv ?{Type?|Short Rest|Long Rest|Hour|Minute|Day|Week|Month|Year} --?{Amount?|1}">Advance the Date</a></div>' + //--
							'<div style="text-align:center;"><a ' +
							astyle2 +
							'" href="!mwcal --enc">Roll Encounter</a></div>' + //--
							'<div style="text-align:center;"><a ' +
							astyle2 +
							'" href="!mwcal --toggle weather">Toggle Weather Display</a></div>' + //--
							'<div style="text-align:center;"><a ' +
							astyle2 +
							'" href="!mwcal --toggle moon">Toggle Moon Display</a></div>' + //--
							'<div style="text-align:center;"><a ' +
							astyle2 +
							'" href="!mwcal --weather ?{Weather?|Put Weather here, type Random to randomise}">Set Weather</a></div>' + //--
							'<div style="text-align:center;"><a ' +
							astyle2 +
							'" href="!alarm --new --title ?{Title?|Insert Title} --date ?{Date?|DD.MM.YYYY} --time ?{Time?|HH:MM}">Create Alarm</a></div>' + //--
							'<div style="text-align:center;"><a ' +
							astyle2 +
							'" href="!showcal">Show to Players</a></div>' + //--
							'<br><br>' + //--
							'<div style="text-align:center;"><a ' +
							astyle2 +
							'" href="!mwcal --reset">Reset Calendar</a></div>' + //--
							'</div>'
					);
				...
				if (alarm == undefined) {
					...
					sendChat(
						...
						'/w gm <div ' +
							...
							'<div style="text-align:center;"><a ' +
							astyle2 +
							'" href="!alarm --new --title ?{Title?|Insert Title} --date ?{Date?|DD.MM.YYYY} --time ?{HH:MM}">Create Alarm</a></div>' + //--
							'</div>'
					);
				}
			},
			...
			showcal = function (msg) {
				...
				switch (Number(state.MWCalendar.now.world)) {
					case 1:
						if (state.MWCalendar.now.mtype == 'ON') {
							rmoon =
								'<table style = "border: none;"><tr><td style="border: none; padding: 2px; padding-left: 5px;">Moon:</td><td style="border: none; padding: 2px; padding-left: 5px;">' +
								state.MWCalendar.now.moonImg +
								'</table>';
						} else {
							rmoon = undefined;
						}
					...
					if (state.MWCalendar.now.mtype == 'ON') {
						sendChat(
							...
							whisper +
								...
								arrowstyle +
								'></div>' + //--
								'<div style="text-align:center;">World: ' +
								world +
								'</div>' + //--
								day +
								suffix +
								...
								month +
								', ' +
								state.MWCalendar.now.year + //--
								'<br>Time: ' +
								hour +
								':' +
								minute + //--
								"<br><br>Today's Weather:<br>" +
								state.MWCalendar.now.weather + //--
								rmoon + //--
								alarmmsg + //--
								'</div>'
						);
				...
				if (state.MWCalendar.now.prevType == 'custom') {
					...
					if (state.MWCalendar.now.world == 1) {
						...
						var moon =
							'<img src="' +
							Luna +
							'" style="width:40px;height:40px;"><img src="' +
							Celene +
			...
			getMoMenu = function () {
				...
				switch (world) {
					...
					case 2:
						moMenu =
							"?{Month|Fire Seek|Readying|Coldeven|Planting|Flocktime|Wealsun|Reaping|Goodmonth|Harvester|Patchwall|Ready'reat|Sunsebb}";
						break;
					case 3:
			...
			reset = function () {
				...
				sendChat(
					...
					'/w gm <div ' +
						...
						'<div style="text-align:center;"><a ' +
						astyle2 +
						'" href="!mwcal --reset --Yes">Confirm</a></div>' + //--
						'<div style="text-align:center;"><a ' +
						astyle2 +
						'" href="!mwcal --reset --No">Cancel</a></div>' + //--
						'<div>'
				);
			},
