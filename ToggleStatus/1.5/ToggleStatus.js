var ToggleStatus = ToggleStatus || (function() {
    'use strict';
    var version = '1.5',
    lastUpdate = 1458197297,
	getAttributeObjects = function (characterObj,attributeArray) {
		"use strict";
		...
		if (msg.type === "api" && msg.content.indexOf(cmd) !== -1 )
		{
			...
			_.each(selected, function(obj) {
				...
				if (!_.isUndefined(TokenSync))
					TokenSync.syncProperty(tok,"statusmarkers");
			});
		}
	},

    checkInstall = function() {    
        log('-=> ToggleStatus v'+version+' <=-  ['+(new Date(lastUpdate*1000))+']');
	},
...
on("ready",function(){
	...
	ToggleStatus.CheckInstall();
	ToggleStatus.RegisterEventHandlers();
});