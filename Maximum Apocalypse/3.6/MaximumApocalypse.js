var MaxApoc = (function() {
	...
	*/
        ...
        if(continueCombat)
        {
            ...
            _.each(previousTurnOrder, function(turn) { 
                ...
                if(charInit)
                {
                    ...
                    turn.pr = parseInt(charInit) + parseInt(stolenInit);
                    turnorder.push(turn);
                }
                else if(turn.custom !== END_OF_PHASE)
                {
	...
	{  
        ...
        maLog('numWithActions - ' + numWithActions, DEBUG_FINE);
        
        maLog('state.MaxApoc.initPhase - ' + state.MaxApoc.initPhase, DEBUG_FINE);
	...
	var updateTurnOrderArray = function(increment, min, characterName)
	{
        ...
        _.each(turnOrder, function(turn) 
        {
            ...
            if(turn.custom != END_OF_PHASE && (!characterName || turn.custom === characterName))
            {
                ...
                if(turn.pr > min)
                {
                    turn.pr = Math.max(parseInt(turn.pr)+increment, min);
                }
	...
	var getPlayerCharacterAttributes = function(selectedTokens, attrNames) 
	{
        ...
        {   
            ...
            _.each(attrNames, function(attrName)
            {
                ...
                // handle if attribute not found
                if(!charObj[attrName])
                {
                    charObj[attrName] = createObj('attribute', {
                                        name: attrName,
                                        current: 0,
	...
	var adjustHunger = function(gmPlayerName, selectedTokens, increment, incrementIsFood)
	{
        ...
        _.each(characterAttributes, function(charObj) 
        {
            try
            {
                ...
                let charIncrement = increment;
                
                let msg = {title: 'Hunger', body:[]};
                
                if(incrementIsFood)
                {
                    // characters can only consume their requirement +4 at maximum
                    let consumedFood = Math.min(charIncrement, foodRequirement+4);
                    msg.body.push('You had ' + charIncrement + ' food units yesterday');
                    if(consumedFood < increment)
                    {
                        msg.body.push('You consumed ' + consumedFood + ' food units but were too full for the remaining ' + (charIncrement - consumedFood));
                    }
                    charIncrement = foodRequirement - consumedFood;
                }
                else
                {
                    maLog('Incrementing hunger (' + hunger + ') for ' + characterName +  ' by ' + charIncrement);
                }
                
                if(increment != 0)
                {
                    // report hunger change
                    msg.body.push('You have ' + (charIncrement < 0 ? 'recovered ' : 'taken ') + Math.abs(charIncrement) + ' hunger damage');
                }
                
                // update hunger damage (to min. zero)
                hunger = Math.max(hunger+charIncrement, 0);
