var MaxApoc = (function() {
	...
	*/
	var showConfig = function(playerName)
	{
    	...
    	_.each(CONFIG, function(configObj, configCode) {
	...
	var init = function(continueCombat) 
    {
        ...
        // update players
        sendMessage({title: 'Initiative', body: ['New Combat Round', 'Combat Phase: '+state.MaxApoc.initPhase+' (of 4)']});
	};
	...
	{  
        ...
        if(state.MaxApoc.initPhase < 4 && numWithActions > 0)
        {
            ...
            // notify players
            sendMessage({title: 'Initiative', body: 'Combat Phase: '+state.MaxApoc.initPhase+' (of 4)'});
        }
        else
        { 
	...
	var luckReRoll = function(playerid)
	{
	    maLog('Performing luck re-roll for playerid: ' + playerid)
	    
	    var reroll = state.MaxApoc.characterRolls[playerid];
	    
        if(reroll)
        {
    	    // perform luck re-roll
    	    sendChat('player|' + playerid, '&{template:maxapoc} ' + reroll);
    	    state.MaxApoc.characterRolls[playerid] = false;
        }
        else
        {
            sendMessage({title: 'Luck Re-Roll', body: 'No check to re-roll'}); 
        }
	};
	...
	*/
	...
	{
	    var speakAs = 'player|' + (config.playerId ? config.playerId : state.MaxApoc.gmPlayerId);
...
on("chat:message", function(msg) 
{
  ...
  {
      var cmdSuffix = msg.content.substring(9, msg.content.length);
      MaxApoc.rollCheck(cmdSuffix, msg.inlinerolls, msg.rolltemplate, msg.playerid);
      return;
  }
  ...
  {
      // do all re-rolls
      MaxApoc.luckReRoll(msg.playerid);
      return;
  }
  ...
  */
  ...
  if(msg.type == "api" && playerIsGM(msg.playerid)) 
  {
      ...
      if(msg.content.indexOf("!ma sea") !== -1) 
      {
      ...
      if(msg.content.indexOf("!ma rea") !== -1) 
      {
