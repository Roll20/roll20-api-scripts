...
var ModifyTokenImage = ModifyTokenImage || (function() {
    ...
    var showHelp = function(msg) {
        ...
        whisperTalker(msg, '<div style="border: solid 1px #000; background-color: #FFF; padding: 3px; width: 100%">'
			...
			+ '<p style="margin-left: 4px;"><strong>!ModifyTokenImage --previous</strong><br />Sets the previous Token Image for the selected token.</p>'
			+ '<p style="margin-left: 4px;"><strong>!ModifyTokenImage --set HandoutName</strong><br />Sets the Token Image for the selected token to the Handout image specified in HandoutName.</p>'
            + '</div>');
    };
    ...
    var handleInput = function(msg) {
        ...
        _.chain(msg.selected)
         ...
         .each(function(token) {
            ...
            var current = token.get("currentToken") || 0;
            ...
            switch (command) {
                case 'next':
                    ...
                    while (!nextHandout) {
                        current++;
                        if (tokenFolder.i.length >= current) {
                            current = 0;
                            loops++;
                        }
                        if (loops > 2) {
                        ...
                        handoutItemId = tokenFolder.i[current];
                ...
                case 'previous':
                    ...
                    while (!previousHandout) {
                        current--;
                        if (current < 0) { 
                            current = tokenFolder.i.length - 1; 
                            loops++;
                        }
                        if (loops > 2) {
                            whisperTalker(msg, name + " does not have any valid icons in its folder!");
            ...
            token.set({ imgsrc: getCleanImagesrc(handout.attributes.avatar), currentToken: current });
         });
    };
