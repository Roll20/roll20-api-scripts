...
var ModifyTokenImage = ModifyTokenImage || (function() {
    ...
    const getHandoutGmNotes = (handout) => {
            return new Promise((resolve) => {
                handout.get("gmnotes", (value)=> {
                    value = decodeURIComponent(value).trim().toString();
                    value = value.replace(/(<([^>]+)>)/ig, ';');
                    const hashTable = {};
                    var valueList = value.split(';');
                    for (let i = 0; i < valueList.length; i++) {
                        var keyval = valueList[i].split(":");
                        if (keyval.length != 2) {
                            continue;
                        }
    ...
    var resizeToken = async function(token, handout) {
        let gmnotesObj = await getHandoutGmNotes(handout);
        let unit_pixels = 70;
        var height = 1;
        var width  = 1;
        ...
        if (gmnotesObj) {
            if ("size" in gmnotesObj) {
                ...
                if ("height" in gmnotesObj) {
                    height = Number(gmnotesObj["height"]);
                }
            }
