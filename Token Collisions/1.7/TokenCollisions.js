/* globals PathMath VecMath MatrixMath */
var API_Meta = API_Meta||{}; //eslint-disable-line no-var
API_Meta.TokenCollisions={offset:Number.MAX_SAFE_INTEGER,lineCount:-1};
{try{throw new Error('');}catch(e){API_Meta.TokenCollisions.offset=(parseInt(e.stack.split(/\n/)[1].replace(/^.*:(\d+):.*$/,'$1'),10)-4);}}
API_Meta.TokenCollisions.version = '1.7';

/**
 * A small library for testing collisions between moving tokens in Roll20.
 */
const TokenCollisions = (() => { //eslint-disable-line no-unused-vars
  ...
  function _testCollision(token, other, waypoint) {
    ...
    let otherIsPath = /^path/.test(other.get('_type'));
...
{try{throw new Error('');}catch(e){API_Meta.TokenCollisions.lineCount=(parseInt(e.stack.split(/\n/)[1].replace(/^.*:(\d+):.*$/,'$1'),10)-API_Meta.TokenCollisions.offset);}}
