		...
		if (msg.type == "api" && ( msg.content.indexOf("!IsoMap") !== -1 || msg.content.indexOf("!psIsoMap") !== -1 ) )  {
			...
			switch(argsFromUser[0]) {
				...
				case '!IsoMap-topDownToIso':
					...
					whisper(playerName, "Creating isometric map on new page.");
				break;
	...
	var convertPaths = function pathConverter(pageID, output) {	// output == "isometric" or "cartesian"
		...
		_.each( paths, function(path) {
			...
			_.each( points, function(point) {
				if (point[0] == "M" || point[0] == "L") { // probably not an oval.. we'll deal with those later
					var isoPoint;
					if (output == "isometric" ) {
			...
			var customPageSettings = { showgrid: false };
			newPath.set("_pageid", makeNewPage(pageID, customPageSettings ) );
