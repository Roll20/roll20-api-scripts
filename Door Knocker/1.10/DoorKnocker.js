var DoorKnocker = DoorKnocker || (function() {let scriptStart = new Error;//Generates an error to localize the start of the script
    ...
    let helpCharacter,
        helpLink,
        markdown = true,//enables/disables logging of markdown version of the help text. Used for easy updating of the script.json file
...
//Script Constants
            ...
            helpText ={
                home:`
                    ...
                    <ul>
                        ...
                        <li><b>open/close/toggle/reveal/hide/alltoggle:</b> This option tells the script to open, close, or toggle (open closed doors and close open doors) the 
                        closest door (or all doors if also using the all option described below). Reveal, hide, and alltoggle cause the command to work on hidden doors as well and 
                        only work with the <b>key</b> kewyword</li>
                        ...
                        in the campaign. The <b>page</b> and <b>campaign</b> keywords are GM only.</li>
                    </ul>
    ...
    keyTurn = function(playerid,selection,cmd,all){
        ...
        if(/open/i.test(cmd)){
            ...
            rawDoors = findObjs({type:'path',pageid:page.id,stroke:`${state.DoorKnocker.hiddenColor||'#000000'}`,layer:'walls'});
        }else if(/hide/i.test(cmd)){
            rawDoors = findObjs({type:'path',pageid:page.id,stroke:`${state.DoorKnocker.hiddenColor||'#000000'}00`,layer:'map'});
        }else if(/alltoggle/i.test(cmd)){}else{
            return;
        }
        const doors = _.reduce(rawDoors,(memo,path)=>{
                memo[path.id]=PathMath.toSegments(path);
