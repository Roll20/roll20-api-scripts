var DoorKnocker = DoorKnocker || (function () {
  ...
  */
  ...
  const version = 1.3,
    ...
    updateScript = function () {
      ...
      if (state.DoorKnocker.version < 1.30) {
        updateTo1x30();
      }
    },
    ...
    updateTo1x30 = () => {
      ...
      paths.forEach(path => {
        ...
        if(/,\["M"/i.test(path.get('_path'))){
          ...
          template.barrierType = 'transparent';
          template.stroke = state.DoorKnocker.wallColor;
    ...
    updateTo1x17 = function () {
      ...
      delete state.DoorKnocker.pairs;
    ...
    findDoorsInDynamicRange = function (token, segmentedPaths, range) {
      let tokenPoint = [token.get('left'), token.get('top'), 1],
        ...
        increment = (page ? page.get('snapping_increment') : 1) || 1;
      let argObj = {
        token: token,
      ...
      log(`range: ${range} | scale: ${scale} | increment: ${increment} | pageIncrement: ${page.get('snapping_increment')}`);
      if (!segmentedPaths) {
      ...
      log(`segmentedPaths`);
      log(segmentedPaths);
      const tokPos = [token.get('left'), token.get('top'), 1];
      log(`|token pos:${tokPos}`);
      return _.reduce(_.keys(segmentedPaths), (memo, key) => {
        let closestSeg = segmentedPaths[key].reduce((details, seg) => {
          let distance = VecMath.ptSegDist(tokPos, ...seg);
          log(`|seg pos:${seg.join(' / ')}`);
          log(`|distance:${distance} vs ${range}`);
          if (distance <= range) {
    ...
    getSegmentedPaths = function (token, findHidden) {
      ...
      let objArray = [];
      ['door', 'unlocked'].forEach((type) => {
        objArray = [...objArray,
        ...findObjs({ type: 'path', pageid: page.id, stroke: state.DoorKnocker[`${type}Color`], layer: 'walls' }),
        ...findObjs({ type: 'path', pageid: page.id, stroke: hexToRgbString(state.DoorKnocker[`${type}Color`]), layer: 'walls' }),
        ...findObjs({ type: 'path', pageid: page.id, stroke: `${state.DoorKnocker[`${type}Color`]}00`, layer: 'map' })];
      });
      objArray = [
        ...objArray,
        ...findObjs({type:'path',pageid:page.id,stroke: state.DoorKnocker.wallColor, barrierType:'transparent',layer: 'walls' })
      ]
      if (findHidden) {
        objArray = [...objArray,
        ...findObjs({ type: 'path', pageid: page.id, stroke: `${state.DoorKnocker.hiddenColor || '#000000'}`, layer: 'walls' }),
        ...findObjs({ type: 'path', pageid: page.id, stroke: hexToRgbString(state.DoorKnocker.hiddenColor), layer: 'walls' }),
      ...
      const cmdSwitch = {},
        ...
        helpText = {
          home: `
                    ...
                    <p>
                        <b>!knock --preset|wall/door/unlocked/hidden|hex color</b>
                    </p>
                    <ul>
                        <li><b>preset:</b> This is the keyword that tells the script to change it's settings</li>
                        <li><b>wall/door/unlocked:</b> This option tells the script to update the color setting for walls, doors, and unlocked doors</li>
                        <li><b>hex color:</b> <i>Optional argument</i> Passing a hex color here will set the script to consider all polygons on the Dynamic Lighting layer with a stroke of this color (or the rgb equivalent) to be doors/walls. If this option is not passed the script will look for a selected polygon to pull a stroke color from.</li>
                    </ul>
                    ...
                    <p>
                        When you search for doors using Door Knocker, it will copy door lines to a visible layer. If a gm initiates the search with a non-pc token selected, hidden doors are copied and all door lines are shown on the gm layer.
                    ...
                    <h4>1.3</h4>
                    <ul>
                      <li>Old window setups will be transitioned to native Roll20 window setup. Window color also changed to wall color by default.</li>
                      <li>Windows can now be any of the defined door type colors and will respect access for that type of door.</li>
                      <li>Fixed issue with new path color format in Roll20</li>
                      <li>Fixed issue with searching/manipulating doors when a grid is not enabled</li>
                    </ul>`,
          settings: `<div>
                        <h3>Script Configurations</h3>
                            ...
                            <b>Unlocked Doors:</b>${makeButton(`!knock --preset|unlocked|?{What Color are your unlocked Doors|${state.DoorKnocker.unlockedColor}}`, state.DoorKnocker.unlockedColor, state.DoorKnocker.unlockedColor, setContrast(hexToRgb(state.DoorKnocker.unlockedColor)), 'Enter the hex color of your unlocked doors')}
                            </div>`
        },
        coloring = {
          'false': ['white', 'black'],
      ...
      if (/^open$/i.test(cmd)) {
        doorArray.forEach((type) => {
          rawDoors = [
            ...
            // Search by rgb
            ...findObjs({ type: 'path', stroke: hexToRgbString(state.DoorKnocker[`${type}Color`]), layer: 'walls' })
          ]
        });
      } else if (/^close$/i.test(cmd)) {
        doorArray.forEach((type) => {
        ...
        log(`toggling`);
        doorArray.forEach((type) => {
          log(`rgbstring: ${hexToRgbString(state.DoorKnocker[`${type}Color`])}`);
          rawDoors = [
            ...rawDoors,
            ...findObjs({ type: 'path', stroke: state.DoorKnocker[`${type}Color`], layer: 'walls' }),
            ...findObjs({ type: 'path', stroke: `${state.DoorKnocker[`${type}Color`]}00`, layer: 'map' }),
        ...
        rawDoors = rawDoors = [
          ...findObjs({ type: 'path', stroke: `${state.DoorKnocker.windowColor}`, layer: 'walls' }),
          // Search by rgb
          ...findObjs({ type: 'path', stroke: hexToRgbString(state.DoorKnocker.hiddenColor), layer: 'walls' })
        ];
        ...
        rawDoors = [
          ...findObjs({ type: 'path', stroke: `${state.DoorKnocker.windowColor}`, layer: 'walls' }),
          // Search by rgb
          ...findObjs({ type: 'path', stroke: hexToRgbString(state.DoorKnocker.windowColor), layer: 'walls' }),
        ...
        rawDoors = [
          // Search by rgb
          ...findObjs({ type: 'path', stroke: hexToRgbString(state.DoorKnocker.hiddenColor), layer: 'walls' }),
          ...findObjs({ type: 'path', stroke: `${state.DoorKnocker.hiddenColor}`, layer: 'walls' })
        ];
    ...
    manipulateDoors = function (token, doors, cmd, all, range) {
      ...
      _.each(validDoors, (id) => {
        ...
        if (/open/i.test(cmd)) {
          setObj.layer = 'map';
          setObj.stroke = `${rgbToHex(path.get('stroke'))}00`;
    ...
    activeSearch = function (playerid, graphics, radius) {
      ...
      _.each(interArr, (pathID) => {
        ...
        template.stroke = rgbToHex(template.stroke).replace(/(.{7}).*/, '$1');
        template.fill = rgbToHex(template.fill).replace(/(transparent|.{7}).*/i, "$1");
