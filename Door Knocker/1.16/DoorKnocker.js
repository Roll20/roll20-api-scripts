...
//Script Constants
    ...
    findDoorsInRange = function(token,segmentedPaths){
        ...
        if(!segmentedPaths){
            ...
            if(!master && token.get('represents')!==state.DoorKnocker.help){
                return;
            }
    ...
    updateSearchRadius = function(obj){
        ...
        if(!_.isEmpty(doorsInRange)){
            doorsInRange = [..._.map(doorsInRange,(range)=>{return range}),obj.get('width')];
            let doorRanges = [_.max([0,_.min(doorsInRange)]),_.min([_.max(doorsInRange)])],
    ...
    updateDoorColors = function(playerid,selection,type,color){
    ...
    keyTurn = function(playerid,selection,cmd,all){
        ...
        if(/open/i.test(cmd)){
            rawDoors = [...findObjs(_.defaults(rawObj,{type:'path',stroke:state.DoorKnocker.doorColor,layer:'walls'})),
                        ...findObjs(Object.assign(rawObj,{type:'path',stroke:state.DoorKnocker.unlockedColor,layer:'walls'}))];
        }else if(/close/i.test(cmd)){
            rawDoors = [...findObjs(_.defaults(rawObj,{type:'path',stroke:`${state.DoorKnocker.doorColor}00`,layer:'map'})),
                        ...findObjs(Object.assign(rawObj,{type:'path',stroke:`${state.DoorKnocker.unlockedColor}00`,layer:'map'}))];
        }else if(/toggle$/i.test(cmd)){
            rawDoors = [...findObjs(Object.assign(rawObj,{type:'path',stroke:state.DoorKnocker.doorColor,layer:'walls'})),
                        ...findObjs(Object.assign(rawObj,{type:'path',stroke:state.DoorKnocker.unlockedColor,layer:'walls'})),
                        ...findObjs(Object.assign(rawObj,{type:'path',stroke:`${state.DoorKnocker.doorColor}00`,layer:'map'})),
                        ...findObjs(Object.assign(rawObj,{type:'path',stroke:`${state.DoorKnocker.unlockedColor}00`,layer:'map'}))];
        }else if(/reveal/i.test(cmd)){
            rawDoors = findObjs(Object.assign(rawObj,{type:'path',stroke:`${state.DoorKnocker.hiddenColor||'#000000'}`,layer:'walls'}));
        }else if(/^hide/i.test(cmd)){
            rawDoors = findObjs(Object.assign(rawObj,{type:'path',stroke:`${state.DoorKnocker.hiddenColor||'#000000'}00`,layer:'map'}));
        }else if(/togglehide/i.test(cmd)){
            rawDoors = [...findObjs(Object.assign(rawObj,{type:'path',stroke:`${state.DoorKnocker.hiddenColor||'#000000'}`,layer:'walls'})),
                        ...findObjs(Object.assign(rawObj,{type:'path',stroke:`${state.DoorKnocker.hiddenColor||'#000000'}00`,layer:'map'}))];
        }else{
            return;
        }
        log(`pre any length:${rawDoors.length}`);
        if(/^any/i.test(cmd)){
            log('any detected');
            if(/open|toggle/i.test(cmd)){
                log('open/toggle detected');
                rawDoors = [...rawDoors,
                            ...findObjs(Object.assign(rawObj,{type:'path',stroke:`${state.DoorKnocker.hiddenColor||'#000000'}`,layer:'walls'}))]
            }
            if(/close|toggle/i.test(cmd)){
                log('close/toggle detected');
                rawDoors = [...rawDoors,
                            ...findObjs(Object.assign(rawObj,{type:'path',stroke:`${state.DoorKnocker.hiddenColor||'#000000'}00`,layer:'map'}))]
            }
        }
        log(`post any length:${rawDoors.length}`);
        const doors = _.reduce(rawDoors,(memo,path)=>{
                memo[path.id]=PathMath.toSegments(path);
                return memo;
            },{});
    ...
    handleGraphic = function(obj){
        try{
            ...
            if(!slave){
                let idKey = _.find(_.keys(state.DoorKnocker.pairs),(masterID)=>{
                    log(`${masterID}:${state.DoorKnocker.pairs[masterID]}`);
                    return obj.id === masterID || obj.id === state.DoorKnocker.pairs[masterID];
                });
                delete state.DoorKnocker.pairs[idKey];
                return;
            }
