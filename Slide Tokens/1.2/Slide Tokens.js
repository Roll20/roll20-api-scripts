...
bshields.translateToken = (function() {
    ...
    var version = 1.2,
        ...
        commands = {
            movetok: function(args, msg) {
                ...
                if (args.length === 0) {
                    sendChat('System', '/w "' + caller + '" No directions supplied.');
                    return;
                }
    ...
    function checkGlobalConfig() {
        var gc = globalconfig && globalconfig.slidetokens;
        
        if (gc && gc.lastsaved && gc.lastsaved > state.bshields.translateToken.gcUpdated) {
            state.bshields.translateToken.gcUpdated = gc.lastsaved;
            state.bshields.translateToken.mode = consts.MODE_STRINGS.indexOf(gc.Mode);
            if (state.bshields.translateToken.mode < 0) {
                log('Invalid mode value copied from globalconfig. Defaulting to SQUARES.');
                state.bshields.translateToken.mode = consts.SQUARES;
            }
        }
    }
