...
//RECORDING OR APPLYING XP: applyXP, recordXP, characterXP, levelUP
	...
	applyXP = function(){
			...
			sessionXP = +thresholds[0]['Session XP'].get('current') || 0;
			if(sessionXP!==0){
					numPCs = +activePCs.length;
					errors = 0;
					...
					_.each(activePCs, function(obj){
							...
							if(!xp){
								xp = createObj('attribute',{
									name:state.EASYEXPERIENCE.xpattr,
									characterid:obj.id,
									current:0
								});
							}
							if(+xp.get('max') === 0 || !xp.get('max')){
									levelUP(level);
							}
							if(!xp.get('current')){
								log('overwriting xp');
									xp.set('current',(sessionXP/numPCs));
							}else{
								log('adding xp');
									xp.set('current', Math.floor(+xp.get('current') + (sessionXP/numPCs)));
							}
							if(parseInt(xp.get('current'))>=parseInt(xp.get('max'))){
	...
	recordXP = function(xpAward){
			var thresholds=getThresholds(),
					currXP;
			currXP= +thresholds[0]['Session XP'].get('current');
...
//Config, Help, and Input handling:
	...
	HandleInput = function(msg_orig) {
		...
		args,
					...
					text='',
					totamount;
			if (msg.type !== 'api' || !playerIsGM(msg.playerid) || !msg.content.startsWith('!xp')){
							return;
			};
			...
			switch(args[1]) {
				...
				case 'selected':
					...
					msg.selected.forEach(function(e) {
						...
						sendChat('EASYEXPERIENCE Script','/w gm recording token ' + name)
						recordXP(xp);
						total += xp;
						tok.set("status_pink")
					})
