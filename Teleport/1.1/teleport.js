    var Teleport = Teleport || (function(){
        ...
        */
        ...
        menuDisplay = function(){
            ...
            output +='<p>' + standardButtonBuilder({param:'Global Pad List',apicall:'globalpdlist',icon:'global'}) + '</p>';
            output +='<p>' + standardButtonBuilder({param:'Limbo Menu',apicall:'globaltknlist',icon:'pad'}) + '</p>';
            output +='</div>';
            outputToChat(output); 
        },
        ...
        padDisplay = function(){
            ...
            _.each(padlist, function(pad){
                ...
                output += '<td>' + emojiButtonBuilder( {param:'Link Pad',apicall:'linkpad|' + pad.get('_id') + '|list',icon:'linked'} ) + '</td>';
                output += '</tr>';
        ...
        padGlobalDisplay = function(){
            ...
            output = ' <div style="' + boundingBoxStyles + '">'
            +'<div style="' + headingAreaStyles + '">';
            output +='<table style="padding: none;"><tr><td width="90%">Global Pad List</td><td>' + emojiButtonBuilder( {param:'Main Menu',apicall:'menu',icon:'help'} ) + '</td></tr></table>';
            output +='</div>';
            _.each(completeList, function(pageTokenArray,key){
                if(Array.isArray(pageTokenArray)){
                    ...
                    _.each(pageTokenArray, function(token){
                        ...
                        output+='<td>' + emojiButtonBuilder( {param:'Teleport Token',apicall:'teleporttoken|' + token.get('_id'),icon:'teleport'} ) + '</td>'
                        output+='<td>' + emojiButtonBuilder( {param:'Link Pad',apicall:'linkpad|' + token.get('_id') + '|global',icon:'linked'} ) + '</td>'
                        output+='<td>' + emojiButtonBuilder( {param:'Add Link',apicall:'addlink|' + token.get('_id') + '|global',icon:'addlink'} ) + '</td></tr>'
        ...
        limboDisplay = function(){
            ...
            output = ' <div style="' + boundingBoxStyles + '">'
            +'<div style="' + headingAreaStyles + '">';
            output +='<table style="padding: none;"><tr><td width="90%">Limbo Resident List</td><td>' + emojiButtonBuilder( {param:'Main Menu',apicall:'menu',icon:'help'} ) + '</td></tr></table>';
            output +='</div>';
            _.each(completeList, function(tokenArray,key){
                if(Array.isArray(tokenArray)){
                    ...
                    _.each(tokenArray, function(token){
                        ...
                        output+='<tr><td style="width:90%">' + token.get('name') + '</td>'
                        //output+='<td>' + emojiButtonBuilder( {param:'Teleport Token',apicall:'teleporttoken|' + token.get('_id'),icon:'teleport'} ) + '</td>'
                        // output+='<td>' + emojiButtonBuilder( {param:'Link Pad',apicall:'linkpad|' + token.get('_id'),icon:'linked'} ) + '</td></tr>'
                        // output += '<tr><td style="text-align:left;border-bottom:1px solid black;" colspan="3"> linked to: ';
                        //    output += targettext;
                        output += '</td></tr>';
                    });
                    output+='</table>';
        ...
        editPadDisplay = function(padid){
            ...
            output += '<tr><td>Link Pad</td><td>' + emojiButtonBuilder( {param:'Link Pad',apicall:'linkpad|' + pad.get('_id') + '|edit',icon:'linked'} ) + '</td></tr>';
        ...
        teleportSelectList = function(params){
            ...
            if(typeof player === 'null'){player=DEFAULTPLAYER;}
        ...
        globalLimboList = function(){
            ...
            _.each(rawList, function(token){
              let foundPlayer = findTokenPlayer({'obj':token,'pad':null});
              if(foundPlayer){
        ...
        limboSwap = function(obj, pageid){
            let destPageLimbo = indexLimbo(pageid), foundPlayerObj = null;
            if(typeof indexLimbo === 'undefined'){
                return false;
            }
            ...
            log('Object was not found in limbo - now we search the whole page just in case.'); 
            ...
            // gm page viewing: player._lastpage
            log('Object was not found on the destination page - now we try and create the object.'); 
            return false;
        },
        reconcileTargetPageId = function(player,targetpageid){
            let returnPageId = Campaign().get('playerpageid'),
            playerPagesObject = Campaign().get('playerspecificpages');
            if(!playerPagesObject){playerPagesObject = {}};
            // Deleted does not appear to be working... 
            // delete playerPagesObject[player.get('id')];
            playerPagesObject = _.omit(playerPagesObject, player.get('_id'));
            log("Post-transformation:")
            log( _.omit(playerPagesObject, player.get('_id')))
            // Problem: player can have only one player specific entry. Indexed by playerid... 
            if(targetpageid === Campaign().get('playerpageid')){
                Campaign().set('playerspecificpages',false);
                Campaign().set('playerspecificpages',playerPagesObject);
            }else{
                playerPagesObject[player.get('id')] = targetpageid;
                Campaign().set('playerspecificpages',false);
                Campaign().set('playerspecificpages',playerPagesObject);
            }
            log('Resulting Player Pages Object:');
            log(playerPagesObject);
            return returnPageId;
        },
        teleportToken = function(params){
            ...
            if(obj.get('_pageid') !== pad.get('_pageid')){
                ...
                if(obj === false){
                    ...
                    return false;// go ahead on
                }
                outputToChat('Teleporting to another page...still buggy!');
                // return false;
            }
            obj.set("layer","gmlayer");
            setTimeout(function(){
        ...
        addPortalPadLink = function(params){
            let pad=params.pad,linktargetids=params.linktargetids,type=params.type,completelinklist=[];
            if(type==='add'){
        ...
        msgHandler = function(msg){
            ...
            if(msg.type === 'api' && msg.content.indexOf('!teleport') === 0 ){
                ...
                if(msg.content.indexOf('--linkpad') !== -1){ 
                    ...
                    if(typeof msg.selected !== 'undefined'){
                        addPortalPadLink({pad:pad,linktargetids:msg.selected,type:'replace'});
                    }else{
                ...
                if(msg.content.indexOf('--addlink') !== -1){ 
                    let pad = getObj('graphic',msg.content.split('|')[1]), returnmenu = msg.content.split('|')[2];
                    if(typeof msg.selected !== 'undefined'){
                        addPortalPadLink({pad:pad,linktargetids:msg.selected,type:'add'});
                    }else{
