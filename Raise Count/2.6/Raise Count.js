        ...
        commands = {
            rc: function(args, msg) {
                ...
                sendChat('', '[[' + roll + ']]', function(ops) {
                    ...
                    rollOut += '" class="inlinerollresult showtip tipsy-n';
                    rollOut += (crit && fail ? ' importantroll' : (crit ? ' fullcrit' : (fail ? ' fullfail' : ''))) + '"';
                    rollOut += ' style="background-color:#fef68e;padding:0 3px 0 3px;font-weight:bold;cursor:help;font-size:1.1em;';
                    rollOut += 'border:2px solid #' + (crit && fail ? '4a57ed' : (crit ? '3fb315' : (fail ? 'b31515' : 'fef68e'))) + '">';
                    rollOut += total + '</span>';
