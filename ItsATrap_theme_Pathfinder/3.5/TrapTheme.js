(() => {
  ...
  const SHEET_ATTRS = {
    ...
    'Pathfinder Official': {
      ...
      perceptionModifier: 'perception',
      fnHasTrapSpotter: character => {
        return TrapTheme.getSheetRepeatingRow(character, 'abilities', rowAttrs => {
          if(!rowAttrs.name)
            return false;
    ...
    class TrapThemePFGeneric extends D20TrapTheme {
      ...
      getThemeProperties(trapToken) {
        ...
        result.splice(1, 0, {
          ...
          value: trapEffect.attackVs || 'none',
          options: ['none', 'AC', 'CMD', 'touch AC']
        });
      ...
      modifyTrapProperty(trapToken, argv) {
        ...
        if(prop === 'attack') {
          ...
          if(!trapEffect.attackVs || trapEffect.attackVs === 'none')
            trapEffect.attackVs = 'AC';
        ...
        if(prop === 'save') {
          trapEffect.save = params[0];
          if(trapEffect.save === 'none')
            trapEffect.saveDC = undefined;
          trapEffect.attack = undefined;
