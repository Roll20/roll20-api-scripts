...
var CombatMaster = CombatMaster || (function() {
    ...
    let round = 1,
        ...
        intervalHandle,
        debug = true,
        activeTokenMarker = null,
        animationIntervalID = null,
        animationIntervalDuration = 500,
        animationBaseTimestamp = Date.now(),
        paused = false,
        who = 'gm',
        playerID = null,
        markers = [],
...
//*************************************************************************************************************
	...
	sendTurnorderMenu = function() {
        ...
        if (turnorder.animateMarker) {
    	    listItems.push(makeTextButton('Animation Rotation Duration',turnorder.animateMarkerDuration, '!cmaster --config,turnorder,key=animateMarkerDuration,value=?{Number of Seconds to make one rotation (15 default)} --show,turnorder'))      
    	    listItems.push(makeTextButton('Animation Update Wait',turnorder.animateMarkerWait, '!cmaster --config,turnorder,key=animateMarkerWait,value=?{Wait in milliseconds|250,250|500,500|1000,1000|1500,1500|2000,2000} --show,turnorder'))         
    	 }
...
//*************************************************************************************************************	    
    ...
    getOrCreateMarker = function (next=false) {
        ...
        if(!next) startActiveTokenMarkerAnimation(marker);
    ...
    removeMarkers = function () {
        stopActiveTokenMarkerAnimation();
   ...
   changeMarker = function (token, next=false)  {
    ...
    startActiveTokenMarkerAnimation = function(marker) {
        if(animationIntervalID!==null && state[combatState].config.turnorder.animateMarkerWait != animationIntervalDuration) {
            clearInterval(animationIntervalID);
            animationIntervalID = null;
        }
        if(animationIntervalID==null) {
            animationIntervalDuration = state[combatState].config.turnorder.animateMarkerWait;
            animationIntervalID = setInterval(animateActiveTokenMarker,state[combatState].config.turnorder.animateMarkerWait);
        }        
        activeTokenMarker = marker;
    },
    stopActiveTokenMarkerAnimation = function() {
        if(animationIntervalID!==null) {
            clearInterval(animationIntervalID);
            animationIntervalID = null;
        }
    },    
    animateActiveTokenMarker = function() {
        if ( activeTokenMarker==null || !state[combatState].config.turnorder.animateMarker || state[combatState].config.hold.held || !inFight() ) {
             return;
	    }
        const millis = Date.now()- animationBaseTimestamp;
        let rotation_duration  = 1000 * parseInt(state[combatState].config.turnorder.animateMarkerDuration);
        if (rotation_duration < 1000) { rotation_duration = 1000; }
        const new_rotation = Math.floor(360*(millis%rotation_duration)/rotation_duration);
        activeTokenMarker.set('rotation',new_rotation);
    },
    ...
    setDefaults = function (reset) {
        ...
        if(!state[combatState].config || typeof state[combatState].config == 'undefined' || reset) {
            ...
            if(!state[combatState].config.hasOwnProperty('turnorder')){
				...
				if(!state[combatState].config.turnorder.hasOwnProperty('animateMarkerDuration')){
					state[combatState].config.turnorder.animateMarkerDuration = combatDefaults.config.turnorder.animateMarkerDuration;
				}
        ...
        let notes = `<div class="content note-editor notes">
                        ...
                        <ul>
                            <ul>
                                ...
                                <li><b>Marker Size </b>— Determines the size of the markers as a fraction of the token it highlights. Default is 1.35, ie slightly bigger.</li>
                                <li><b>Animate Marker </b>— Will animate the marker by rotating it (the next marker will <b>not</b> be animated). This animation is meant to provide a visual cue, it will never be a nice smooth animation.</li>
                                <li><b>Animation Rotation Duration </b>— Number of seconds in which the marker will make one full revolution.</li>
                                <li><b>Animation Update Wait </b>— Wait time in milliseconds before the marker is updated.</li>
                            </ul>
                        </ul>`
