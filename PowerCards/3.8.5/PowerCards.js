		...
		for (var x=n.length - 1 ; x >= 0; x--) {
			...
			if (n[x].substring(0,12) === "replaceattrs") {
				try {
					...
					//log(getAttrByName(replacement_charid, "spellcasting_ability"));
					//log(getAttrByName(replacement_charid, getAttrByName(replacement_charid, "spellcasting_ability").replace(/\@\{/g,"").replace(/\}/g,"").replace(/\+/g,"")));
			...
			if (n[x].startsWith("replacespell")) {
				...
				//replacementItems.push.apply(replacementItems, getSpellInfo(params[0],params[1]));
