var PowerCards = PowerCards || (function() {
	...
	var PowerCards_Version = "3.7.2";
	var PowerCards_LastUpdated = "2018-04-14";
	...
	PowerCard.Process = function(msg, player_obj) {
		...
		_.each(n, function(a) {
			...
			if (Tag.includes("npc_action_list") && getObj("character", Content) !== undefined) {
				...
				Tag = "!npc_action_list";
				Content = "";
				for (var action_count = 0; action_count < getRepeatingCount(Character_ID, "repeating_npcaction_", "_name"); action_count++) {
					var npc_action = getSheetAttr(Character_ID, "repeating_npcaction_$" + action_count + "_name");
			...
			if (Tag.includes("npc_reaction_list") && getObj("character", Content) !== undefined) {
				...
				Content = "";
				for (var action_count = 0; action_count < getRepeatingCount(Character_ID, "repeating_npcreaction_", "_name"); action_count++) {
					var npc_action = getSheetAttr(Character_ID, "repeating_npcreaction_$" + action_count + "_name");
					Content += "**" + npc_action + "** " + getSheetAttr(Character_ID, "repeating_npcreaction_$" + action_count + "_desc").replace(/\r?\n|\r/g, '') + "~~~";
				}
			...
			if (Tag.includes("npc_legendaryaction_list") && getObj("character", Content) !== undefined) {
				...
				Content = "";
				for (var action_count = 0; action_count < getRepeatingCount(Character_ID, "repeating_npcaction-l_", "_name"); action_count++) {
					var npc_action = getSheetAttr(Character_ID, "repeating_npcaction-l_$" + action_count + "_name");
			...
			if (Tag.includes("npc_trait_list") && getObj("character", Content) !== undefined) {
				...
				Tag = "!npc_trait_list";
				Content = "";
				for (var action_count = 0; action_count < getRepeatingCount(Character_ID, "repeating_npctrait_", "_name"); action_count++) {
					var npc_action = getSheetAttr(Character_ID, "repeating_npctrait_$" + action_count + "_name");
					Content += "**" + npc_action + "** " + getSheetAttr(Character_ID, "repeating_npctrait_$" + action_count + "_desc").replace(/\r?\n|\r/g, '') + "~~~";
				}
				if (Content.length > 3) { Content = Content.substr(0,Content.length-3) };
			}
