var PowerCards = PowerCards || (function() {
	...
	var PowerCards_Version = "3.8.8";
	var PowerCards_LastUpdated = "2018-10-21";
	...
	on("chat:message", function(msg) {
		...
		switch (msg.content.split(" ", 1)[0].toLowerCase()) {
			...
			case "!power_version":
				...
				break;
			case "!pc_showem":
			    var attrs = filterObjs(function(z) { return (z.get("characterid") == (msg.content.split(" ")[1]) && z.get("name").startsWith(msg.content.split(" ")[2])); });
				log(attrs);
				break;
		}
	});
	...
	PowerCard.Process = function(msg, player_obj) {
		...
		for (var x=n.length - 1 ; x >= 0; x--) {
			...
			if (n[x].startsWith("replacespell")) {
				...
				getSpellInfo(params[0],params[1]).forEach(function (a) { replacementItems.push(a) });
				spliceIt = true;
			}
			if (n[x].startsWith("replacenpcaction")) {
				var params = n[x].substring(n[x].indexOf("|") + 1).trim().split("|");
				getNPCAction(params[0],params[1]).forEach(function (a) { replacementItems.push(a) });
				spliceIt = true;
			}

			if (n[x].startsWith("replacepcattack")) {
				var params = n[x].substring(n[x].indexOf("|") + 1).trim().split("|");
				getPCAttack(params[0],params[1]).forEach(function (a) { replacementItems.push(a) });
				spliceIt = true;
			}

			if (n[x].startsWith("replacesection")) {
				var params = n[x].substring(n[x].indexOf("|") + 1).trim().split("|");
				getSectionAttrs(params[0],params[1],params[2],params[3],params[4]).forEach(function (a) { replacementItems.push(a) });
				spliceIt = true;
			}
			
			
			if (spliceIt) { n.splice(x,1); }
		}
		
		//log(replacementItems);
		...
		_.each(n, function(a) {
			...
			if (Tag.includes("spell_list") && getObj("character", Content) !== undefined) {
				...
				if (cantrips.length > 0) {
					Content += "~~~ **Cantrips:** ~~~";
					cantrips.forEach(function(s) { Content += "[" + s.get("current") + "]" + getSpellMacro(Character_ID, s.get("current"), s.get("name")); });
				}
				if (L1Spells.length > 0) {
					Content += "~~~ **1st Level Spells:** ~~~";
					L1Spells.forEach(function(s) { Content += (getAttrByName(Character_ID, s.get("name").replace("name", "prepared")) == "1") ? "[" + s.get("current") + "]" + getSpellMacro(Character_ID, s.get("current"), s.get("name")) : "";});
				}
				if (L2Spells.length > 0) {
					Content += "~~~ **2nd Level Spells:** ~~~";
					L2Spells.forEach(function(s) { Content += (getAttrByName(Character_ID, s.get("name").replace("name", "prepared")) == "1") ? "[" + s.get("current") + "]" + getSpellMacro(Character_ID, s.get("current"), s.get("name")) : "";});
				}
				if (L3Spells.length > 0) {
					Content += "~~~ **3rd Level Spells:** ~~~";
					L3Spells.forEach(function(s) { Content += (getAttrByName(Character_ID, s.get("name").replace("name", "prepared")) == "1") ? "[" + s.get("current") + "]" + getSpellMacro(Character_ID, s.get("current"), s.get("name")) : "";});
				}
				if (L4Spells.length > 0) {
					Content += "~~~ **4th Level Spells:** ~~~";
					L4Spells.forEach(function(s) { Content += (getAttrByName(Character_ID, s.get("name").replace("name", "prepared")) == "1") ? "[" + s.get("current") + "]" + getSpellMacro(Character_ID, s.get("current"), s.get("name")) : "";});
				}
				if (L5Spells.length > 0) {
					Content += "~~~ **5th Level Spells:** ~~~";
					L5Spells.forEach(function(s) { Content += (getAttrByName(Character_ID, s.get("name").replace("name", "prepared")) == "1") ? "[" + s.get("current") + "]" + getSpellMacro(Character_ID, s.get("current"), s.get("name")) : "";});
				}
				if (L6Spells.length > 0) {
					Content += "~~~ **6th Level Spells:** ~~~";
					L6Spells.forEach(function(s) { Content += (getAttrByName(Character_ID, s.get("name").replace("name", "prepared")) == "1") ? "[" + s.get("current") + "]" + getSpellMacro(Character_ID, s.get("current"), s.get("name")) : "";});
				}
				if (L7Spells.length > 0) {
					Content += "~~~ **7th Level Spells:** ~~~";
					L7Spells.forEach(function(s) { Content += (getAttrByName(Character_ID, s.get("name").replace("name", "prepared")) == "1") ? "[" + s.get("current") + "]" + getSpellMacro(Character_ID, s.get("current"), s.get("name")) : "";});
				}
				if (L8Spells.length > 0) {
					Content += "~~~ **8th Level Spells:** ~~~";
					L8Spells.forEach(function(s) { Content += (getAttrByName(Character_ID, s.get("name").replace("name", "prepared")) == "1") ? "[" + s.get("current") + "]" + getSpellMacro(Character_ID, s.get("current"), s.get("name")) : "";});
				}
				if (L9Spells.length > 0) {
					Content += "~~~ **9th Level Spells:** ~~~";
					L9Spells.forEach(function(s) { Content += (getAttrByName(Character_ID, s.get("name").replace("name", "prepared")) == "1") ? "[" + s.get("current") + "]" + getSpellMacro(Character_ID, s.get("current"), s.get("name")) : "";});
				}
				if (Content == "") {
					Tag = "$spell_list";
				}
		...
		if (roll.type === "C") {
			...
			_.each(roll.results, function(result) {
				if (result.tableItem !== undefined) {
					...
					if (expandedRoll) result.v = "<span style='" + (critRoll ? 'color: #040; font-style: italics; font-size: 1.25em;' : (failRoll ? 'color: #600; font-weight: oblique; font-size: 1.25em;' : '')) + "'>" + result.v + "</span>";
	...
	function doInlineFormatting(content, ALLOW_URLS, ALLOW_HIDDEN_URLS, Rolls, NIF) {
		...
		var urls = [],
			...
			formatter = function(s) {
				if (content.includes("[NIF]")) { return s; } else {
					if (NIF !== undefined) { return s; } else {
					return s
		...
		str = formatter(str);
		if (str.includes("[NIF]")) { str = str.replace(/\[NIF]/g,""); }
		return _.reduce(
			urls,
			function(m, s, i) {
	...
	function getPowerCardHandouts() {
		...
		HandoutBase.forEach(function(handout) {
			...
			// handout.get("notes", function(notes) {
				// log(notes.replace(/\<p\>/g,"").replace(/\<\/p\>/g,"<br>"));
			// });
