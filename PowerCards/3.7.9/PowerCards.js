var PowerCards = PowerCards || (function() {
	...
	var PowerCards_Version = "3.7.9";
	var PowerCards_LastUpdated = "2018-07-07";
	...
	var getPowerCardTemplates = getPowerCardTemplates || {};
	var getPowerCardReplacements = getPowerCardReplacements || {};
	...
	on("chat:message", function(msg) {
		...
		if (msg.content.split(" ", 1)[0] === "!power") {
			...
			// Updated to support having {{ and }} inside cards  by only matching the first and last occurance when stripping them out.
	...
	on("ready", function() {
		getPowerCardFormats();
		getPowerCardTemplates();
		getPowerCardStatusList();
		getPowerCardReplacements();
	...
	PowerCard.Process = function(msg, player_obj) {
		...
		// 2018-06-27 added the ability to use one replacement line with multiple identifiers separated by semicolons.
		...
		// Process the templates in reverse order so we can insert them without
		// messing up the order for items that will be inserted above them.
		for (var x=templateTags.length - 1; x >= 0; x--) {
			...
			//If the Content string contains a | character, there are parameters to consider. They are
			//separated by a semicolon (;), so split the remainder of the content on that.
			if (TemplateName !== TemplateContent) {
				TemplateParameters = TemplateContent.split("|")[1].trim().split(";");
			}

			// Retrieve the appropriate Template from the "PowerCard Templates" handout and split into tags
			...
			PowerCard_Templates.forEach(function(a) {
				...
				if (TemplateParameters !== undefined) {
					for (var i=0; i<TemplateParameters.length; i++) {		
						...
						// Process replacements on Tags and Content from the template
				...
				// Optional Template Tags : Putting [ and ] at the front of a tag can optionally hide the tag. If the brackets
				// are empty, the tag won't be added to the card. If the brackets contain something that can be evaluated as an
				// expression, the tag will be added if the expression evaluates to true. If the brackets contains something that
				// can't be evalualted as a tag, but aren't empty, the tag will be added to the card.
				if (Tag.startsWith("[]")) { Tag = "" };

				if (Tag.startsWith("[")) {
					var testexpr = Tag.match(/\[(.*)\]/);
				
					if (testexpr == undefined || testexpr.length < 2) {
						Tag = "";
					} else {
						testexpr = testexpr[1];
					}
					try {
						if (eval(testexpr) === false) {
							Tag = "";
						}
					} catch (e) { }
				}
			...
			// Remove the Template Entry from the tag array
	...
	function getPowerCardFormats() {
		...
		//log("PowerCards : Formats Updated");
	};

	// Retrieves the contents of a handout named "PowerCard Templates" and parses the notes field
	// to create a table of avaiable templates to use with the --template tag.
	function getPowerCardTemplates() {
		var PowerCard_TemplatesHandout = findObjs({
			_type: "handout",
			name: "PowerCard Templates"
		})[0];
		if (PowerCard_TemplatesHandout !== undefined) {
			var PowerCard_Templates = {};
			var TemplateName = "";
			var TemplateContent = "";
			PowerCard_TemplatesHandout.get("notes", function(notes) {
				notes = notes.split("<br>");
				notes.forEach(function(notes) {
					TemplateName = notes.substring(0, notes.indexOf(":")).trim();
					TemplateContent = notes.substring(notes.indexOf(":") + 1).trim();
					TemplateContent = TemplateContent.replace(/&gt;/g,">").replace(/&lt;/g,"<").replace(/&amp;/g,"&");
		...
		//log("PowerCards : Templates Updated");
	};

	function getPowerCardReplacements() {
		var ReplacementBase = findObjs({
			_type: "handout"
		});
		var PowerCard_Replacements = {};
        ReplacementBase.forEach(function(replacers) {
			if (replacers.get("name").startsWith("PowerCard Replacements")) {
				var ReplacementName = "";
				var ReplacementContent = "";
				replacers.get("notes", function(notes) {
					notes = notes.split("<br>");
					notes.forEach(function(notes) {
						ReplacementName = notes.substring(0, notes.indexOf(":")).trim();
						ReplacementContent = notes.substring(notes.indexOf(":") + 1).trim();
						ReplacementContent = ReplacementContent.replace(/&gt;/g,">").replace(/&lt;/g,"<").replace(/&amp;/g,"&");
