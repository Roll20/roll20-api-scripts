	...
	PowerCard.Process = function(msg, player_obj) {
		...
		_.each(n, function(a) {
			...
			if (Tag.includes("attribute_summary") && (getObj("character", Content) !== undefined)) {
				...
				Content = "<table width='95%' align='center'><tr><td>**Str** " + getAttrByName(Character_ID, "strength") + "(" + getAttrByName(Character_ID, "strength_mod") + ")</td>";
				Content += "<td>**Dex** "+ getAttrByName(Character_ID, "dexterity") + "(" + getAttrByName(Character_ID, "dexterity_mod") + ")</td>";
				Content += "<td>**Con** "+ getAttrByName(Character_ID, "constitution") + "(" + getAttrByName(Character_ID, "constitution_mod") + ")</td></tr>";
				Content += "<tr><td>**Int** " + getAttrByName(Character_ID, "intelligence") + "(" + getAttrByName(Character_ID, "intelligence_mod") + ")</td>";
				Content += "<td>**Wis** "+ getAttrByName(Character_ID, "wisdom") + "(" + getAttrByName(Character_ID, "wisdom_mod") + ")</td>";
				Content += "<td>**Cha** "+ getAttrByName(Character_ID, "charisma") + "(" + getAttrByName(Character_ID, "charisma_mod") + ")<br /></td></tr>";
				Content += "<tr><td>**AC** " + getAttrByName(Character_ID, "npc_ac") + "</td>";
				Content += "<td colspan=2>**Challenge Rating** "+ getAttrByName(Character_ID, "npc_challenge") + "<br /></td></table>";			
			}
			...
			if (Tag.includes("npc_qualities_summary") && (getObj("character", Content) !== undefined)) {
				...
				Content = "";
				if (getAttrByName(Character_ID, "npc_vulnerabilities").length > 0 ) {
					Content += "**Vulnerable:** " + getAttrByName(Character_ID, "npc_vulnerabilities") + "<br />";
				}
				if (getAttrByName(Character_ID, "npc_resistances").length > 0 ) {
					Content += "**Resistant:** " + getAttrByName(Character_ID, "npc_resistances") + "<br />";
				}
				if (getAttrByName(Character_ID, "npc_immunities").length > 0 ) {
					Content += "**Dmg Immune:** " + getAttrByName(Character_ID, "npc_immunities") + "<br />";
				}
				if (getAttrByName(Character_ID, "npc_condition_immunities").length > 0 ) {
					Content += "**Cond Immune:** " + getAttrByName(Character_ID, "npc_condition_immunities") + "<br />";
				}			
				if (getAttrByName(Character_ID, "npc_senses").length > 0 ) {
					Content += "**Senses:** " + getAttrByName(Character_ID, "npc_senses") + "<br />";
				}
			}	
			...
			if (Tag.includes("npc_skills_summary") && (getObj("character", Content) !== undefined)) {
				...
				attrList.forEach(function(a) {
					if (getAttrByName(Character_ID, "npc_" + a + "_base") !== "" ) {
						skillList += (a.charAt(0).toUpperCase() + a.slice(1)).replace("_"," ") + ": " + getAttrByName(Character_ID, "npc_" + a + "_base") + ", ";
					 }
				});
				if (skillList !== "**Skills:** ") { Content = skillList.slice(0,skillList.length-2); } 
			}				
			...
			if (Tag.includes("npc_reaction_list") && getObj("character", Content) !== undefined) {
				...
				Content = "";
				var npc_action = getAttrByName(Character_ID, "repeating_npcreaction_$0_name");
				var action_count = 0;
				while (npc_action !== "") {
					Content += "**" + npc_action + "** ";
					Content += getAttrByName(Character_ID, "repeating_npcreaction_$" + action_count + "_desc").replace(/\r?\n|\r/g, '') + "~~~";
					action_count++;
					npc_action = getAttrByName(Character_ID, "repeating_npcreaction_$" + action_count + "_name");
				}
				if (Content.length > 3) { Content = Content.substr(0,Content.length-3) };
			}
			
			// CREATE 5e OGL SHEET NPC LEGENDARY ACTION LIST
			if (Tag.includes("npc_legendaryaction_list") && getObj("character", Content) !== undefined) {
				Character_ID = Content;
				Tag = "!npc_legendaryaction_list";
				Content = "";
				var npc_action = getAttrByName(Character_ID, "repeating_npcaction-l_$0_name");
				var action_count = 0;
				while (npc_action !== "") {
					Content += "[" + npc_action + "](~selected|repeating_npcaction-l_$" + action_count + "_npc_action) ^^";
					action_count++;
					npc_action = getAttrByName(Character_ID, "repeating_npcaction-l_$" + action_count + "_name");
				}
			}
			...
			if (Tag.includes("npc_trait_list") && getObj("character", Content) !== undefined) {
				...
				Tag = "!npc_trait_list";
				Content = "";
				var npc_action = getAttrByName(Character_ID, "repeating_npctrait_$0_name");
				var action_count = 0;
				while (npc_action !== "") {
					Content += "**" + npc_action + "** ";
					Content += getAttrByName(Character_ID, "repeating_npctrait_$" + action_count + "_desc").replace(/\r?\n|\r/g, '') + "~~~";
					action_count++;
					npc_action = getAttrByName(Character_ID, "repeating_npctrait_$" + action_count + "_name");
				}
				if (Content.length > 3) { Content = Content.substr(0,Content.length-3) };
			}
			...
			if (Tag.includes("spell_slots") && getObj("character", Content) !== undefined) {
				...
				for (var i=1; i<=9; i++)
				{
					var slot_total = getAttrByName(Character_ID, "lvl" + i + "_slots_total");
					if (slot_total !== undefined && parseInt(slot_total) !== 0) {
						var slot_unused = getAttrByName(Character_ID, "lvl" + i + "_slots_expended");
