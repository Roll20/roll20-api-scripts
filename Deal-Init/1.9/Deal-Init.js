...
// 1.9 fixed showComps bug where complications for token on GM layer was being announced to general chat instead of just to the GM
...
x !deal-init --help
x !deal-init --reset
x   !deal-init  --show  (check 2 jokers)
x   !deal-init (no args)
x !deal-init --4jreset
x   !deal-init  --show  (check 4 jokers)
x   !deal-init (no args)
x !deal-init --reset --quiet
x   !deal-init (make sure it quiet)
x !deal-init --4jreset --quiet
x   !deal-init (make sure it quiet)
x !deal-init --onlyto --Nath
x !deal-init --deal2chat
x !deal-init --showComps
x !deal-init --clearTurnOrder
...
x  Joker detection
x  Sorting
x  Token associated with a character
x  Token not associated with a character
x  Token on GM layer ony send info to GM
...
var DealInit = DealInit || (function() {
  ...
  var version = '1.9',
    lastUpdate = '[Last Update: Apr 27, 2022, 5pm Pacific]',
    jokerLastRound = 0,
    jokerInChat = 0,
    fourJokers = 0,
    onlyToString = '',
    ...
    //      "shortname":"Short Token name" //The short name displayed in the chat
    //      "layer":"LayerName" //The layer name of the token object - one of: "gmlayer", "objects", "map", or "walls".
    //     },
    //     {   // custom item
    //      "id":"-1", //For custom items, the ID MUST be set to "-1" (note that this is a STRING not a NUMBER.
    ...
    //      "shortname":"Short Token name" //The short name displayed in the chat
    //      "layer":"none" //The layer name of the object
    //     }
    ...
    getInitiativeEdges = function(id) {
      ...
      var token_obj = {};
      var char_obj = {};
      var toid, controler, tokenOnHold, onlayer, tokentype;
      ...
      for (i = 0; i < turnorder.length; i += 1) {
        ...
        // if the turn order item is a "token that doesn't represent a character", set InitEdges to 0
          ...
          // capture the layer that the token is on.
          // Used to ensure only the gm sees messages about tokens on gm layer
          onlayer = token_obj.get("layer");
          if (! onlayer || onlayer === '' ) {
            onlayer = 'none';
          } 
        ...
        if (verboseMode) {
          var whoVM = getObj('player', id).get('_displayname').split(' ')[0];
          sendChat('', '/w ' + whoVM + " VERBOSE: " + divStart + 'DealInit: Character <p>Name: ' + initEdges[i].name + '<p>Edges: ' + initEdges[i].edges + '<p>ID: ' + initEdges[i].id + '<p>Token Type: ' + initEdges[i].toktype + '<p>Short name: ' + initEdges[i].shortname + '<p>Token On Layer: ' + initEdges[i].layer + divEnd);
        }
        ...
        if ((!onlyToActive) || (onlyToActive && initEdges[i].name.indexOf(onlyToString) !== -1) || dealToChat) {
          ...
          // deal to chat  xxx
            ...
            if (initEdges[i].edges.indexOf('TT') !== -1) {
              ...
              nextcard = deck.deal();
              // send the card drawn to chat
              sendChat('', sendto + divStart + '<div style="font-weight: bold; border-bottom: 3px solid green;font-size: 100%;">' + '<u>' + initEdges[i].shortname + '...</u> Tactician Card!' + '</div>' + nextcard.shortName + divEnd);
            ...
            if (initEdges[i].edges.indexOf('MTT') !== -1) {
              ...
              nextcard = deck.deal();
              // send the card drawn to chat
              sendChat('', sendto + divStart + '<div style="font-weight: bold; border-bottom: 3px solid green;font-size: 100%;">' + '<u>' + initEdges[i].shortname + '...</u> Master Tactician Card!' + '</div>' + nextcard.shortName + divEnd);
          ...
          var sendCompTo = '/em ';
          if ( initEdges[i].layer === "gmlayer" ) {
             sendCompTo = "/w gm ";
          }
          if (turnorder[i].rank !== "-1" && turnorder[i].rank !== "60" && turnorder[i].pr.indexOf("clubs") !== -1 && showComps ) {
            sendChat('', sendCompTo + 'Complication for ['+initEdges[i].shortname+'] if this is a Dramatic Task or Chase!');
          }
