...
API_Meta.PathSplitter.version = '1.1.3';
/* globals PathMath */
/**
...
//    $d({p1Segments,p1Len,p2Segments,p2Len});
  ...
  function splitPathAtIntersections(mainPath, splitPath) {
    ...
    if(isJumpgate()){
      let barrierType = mainPath.get('barrierType');
      let oneWayReversed = mainPath.get('oneWayReversed');
      if(undefined !== barrierType) {
        pathExtra.barrierType = barrierType;
      }
      if(undefined !== oneWayReversed) {
        pathExtra.oneWayReversed = oneWayReversed;
      }
    }
    ...
    if(!segmentPaths.length) {
      let msg = 'Path split produced no path pieces. Original paths were left unchanged.';
      sendChat('Pathsplitter', msg);
      log('!pathSplit ERROR: ' + msg);
      return [];
    }
    ...
    // Only remove the originals if every intended piece was created.
    if(results.length === segmentPaths.length && results.every(p => p)) {
      mainPath.remove();
      splitPath.remove();
    } else {
      results.filter(p => p).forEach(p => p.remove());
      let msg = 'Failed to create one or more split path pieces. Original paths were left unchanged.';
      sendChat('Pathsplitter', msg);
      log('!pathSplit ERROR: ' + msg);
      return [];
    }
