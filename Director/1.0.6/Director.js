...
on('ready', () =>
    {
        ...
        const version = '1.0.6'; //version number set here
...
const getJukeboxPlusHandoutLink = () => {
 const css = getCSS();
  if (typeof API_Meta !== 'undefined' &&
    ...
    if (handout) {
      const url = `
        http: //journal.roll20.net/handout/${handout.id}`;
    ...
    for(const part of parts)
    {
        ...
        switch(cmd.trim())
        {
                       ...
                       case 'captions-on':
            {
                ...
                if(!st.captions.template)
                {
                    sendStyledMessage("Captions", "Please define a caption template first with !director --captions|define");
                    break;
                }
