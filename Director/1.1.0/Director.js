...
// 1.0.2 Expanded Grid Mode up to 9x9 and tightened spacing, added Star system
...
// 1.0.8 Added search-again button on search results, improved backup handout selection, added move up/down/rename/redefine item buttons in edit mode, improved table rolling, Supernotes integration
// 1.1.0 Backdrop changes now fade smoothly instead of snapping instantly, are restricted to proper Director stage pages, and only affect the scene actually live on the tabletop — browsing or pre-selecting a backdrop for a different scene no longer disturbs the live display. Fixed several star-highlight bugs: wrong color, highlighting the wrong token when two tokens shared a name, and lagging one step behind after switching backdrops. Backup and restore are more reliable, fixing a bug that could corrupt or fail to restore certain backups. Fixed Capture Image, which was documented but didn't work, along with a crash it could cause. Fixed the Repair function unexpectedly stopping any music that was playing. Fixed act/scene names or image titles containing a "|" character breaking rename/delete buttons or turning an editable rename prompt into a confusing dropdown. Added a new lightweight item type, Actor, that displays live from a single reference token instead of storing its own copy, with a one-click tool to convert existing Character or Variant items to Actor (including automatically finding and converting matching duplicates), a safeguard against placing its reference token somewhere it would get wiped, and fixes for duplicate-token creation and a non-working name-click during conversion. Fixed the Supernotes button sometimes pointing at a token that no longer existed. Rewrote and reorganized the in-program Help documentation to match how the script works today, including a new interface diagram, working jump-links to each section, and a collapsible section for advanced details so new users aren't overwhelmed. Added two new macro-friendly commands, --set and --reveal, that jump straight to any act, scene, and backdrop by name in a single click — --set fully cuts to the new scene, --reveal just crossfades the backdrop without disturbing anything else already set up — both matching names in any capitalization and explaining clearly in chat if a name can't be found. Added a Margins setting so the interface displays correctly whether the handout is docked in the VTT or popped out into its own window.
...
on('ready', () =>
    {
        ...
        // NPC/Actor features require Roll20's v1.5+ API sandbox.
        const npcFeatureAvailable = Campaign().sandboxVersion !== '1.0';
        ...
        const cssDark = {
            ...
            helpContainer: 'background:transparent;padding:16px; font-size:13px; line-height:1.5;',
        ...
        // Roll20's ?{Prompt|Default} syntax can't escape "|", so any value used as a
        // pre-filled default needs its pipes swapped out first, or it turns into a dropdown.
        const sanitizeQueryDefault = (str) => String(str ?? '').replace(/\|/g, '¦');
        ...
        // Case-insensitive name lookups for --set/--reveal (macro-typed names). Returns the
        // canonical stored key/title on a match, or null.
        const findActKeyCI = (st, name) =>
        {
            const target = (name || '').toLowerCase();
            return Object.keys(st.acts || {}).find(k => k.toLowerCase() === target) || null;
        };

        const findSceneKeyCI = (act, name) =>
        {
            const target = (name || '').toLowerCase();
            return Object.keys(act?.scenes || {}).find(k => k.toLowerCase() === target) || null;
        };

        // First image (in display order) in the scene whose title matches, case-insensitively.
        // Multiple images can share a title, so this only ever reaches the first one.
        const findImageByTitleCI = (scene, name) =>
        {
            const target = (name || '').toLowerCase();
            return (scene?.images || []).find(img => (img.title || '').toLowerCase() === target) || null;
        };
        ...
        // Backup and restore
        //
        // Handout notes/gmnotes must be populated via .set() after createObj, not passed into
        // createObj's properties directly — Roll20's API has a known bug where that fails
        // quietly (see https://wiki.roll20.net/API:Objects).
        //
        // The backup payload itself is base64url-encoded before being stored: state can contain
        // percent-encoded text (e.g. from token/image URLs), and Roll20's handout notes storage
        // percent-decodes stored text on save, which corrupts plain JSON backups containing
        // those sequences. base64url's alphabet has no '%' or '+', so it's immune to that.
        //
        // No Buffer global exists in Roll20's sandbox, so base64url is implemented here on top
        // of encodeURIComponent/decodeURIComponent instead.
        const BASE64URL_ALPHABET = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_';
        const BASE64_REV = (() =>
        {
            const map = {};
            'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'.split('').forEach((c, idx) =>
            {
                map[c] = idx;
            });
            map['-'] = 62;
        ...
        // JS strings are UTF-16; base64 needs raw bytes. Walking encodeURIComponent's %XX
        // output recovers the UTF-8 byte sequence without needing Buffer or TextEncoder.
        ...
        // Inverse of toUtf8Bytes: percent-encode every byte, then let decodeURIComponent
        // reassemble the original (possibly multi-byte) characters.
        const fromUtf8Bytes = (bytes) =>
        {
            ...
            for(let i = 0; i < bytes.length; i++)
            {
                ...
                percentEncoded += '%' + (hex.length < 2 ? '0' : '') + hex;
            }
            return decodeURIComponent(percentEncoded);
        };

        const encodeBackupText = (str) =>
        {
            ...
            for(; i + 3 <= bytes.length; i += 3)
            {
                ...
                result += BASE64URL_ALPHABET[((b0 & 3) << 4) | (b1 >> 4)];
                result += BASE64URL_ALPHABET[((b1 & 15) << 2) | (b2 >> 6)];
                result += BASE64URL_ALPHABET[b2 & 63];
            }
            ...
            if(rem === 1)
            {
                const b0 = bytes[i];
                result += BASE64URL_ALPHABET[b0 >> 2];
                result += BASE64URL_ALPHABET[(b0 & 3) << 4];
            }
            else if(rem === 2)
            {
                ...
                result += BASE64URL_ALPHABET[b0 >> 2];
                result += BASE64URL_ALPHABET[((b0 & 3) << 4) | (b1 >> 4)];
                result += BASE64URL_ALPHABET[(b1 & 15) << 2];
            }
            return result;
        };

        // Reads current base64url backups plus older plain-JSON/base64 formats, so backups
        // already saved in a campaign still load.
        const decodeBackupText = (str) =>
        {
            const trimmed = String(str || '').trim();
            if(trimmed.startsWith('{')) return trimmed; // plain JSON
            ...
            {
                const c = trimmed[i];
                if(!(c in BASE64_REV)) continue; // skip '=' padding, whitespace, etc.
                buffer = (buffer << 6) | BASE64_REV[c];
                bits += 6;
                if(bits >= 8)
                {
                    bits -= 8;
        ...
        const makeBackup = () =>
        {
            ...
            const handout = createObj('handout', { name });
            handout.set('notes', encodeBackupText(JSON.stringify(st)));
        ...
        const restoreBackup = (name) =>
        {
            ...
            handout.get('notes', (notes) =>
            {
                try
                {
                    const restored = JSON.parse(decodeBackupText(notes));
        ...
        const handleCaptureImage = (msg) =>
        {
            ...
            if(sceneName)
            {
                // Find the act that actually owns the active scene by searching, the same
                // way handleSetScene/handleSetGrid/set-backdrop do — st.activeAct is a
                // separately-tracked field and isn't guaranteed to match the real owner.
            ...
            if(!scene)
            {
                ...
                // Ensure there's an active act to hold the default scene — falling through
                // with no act at all would create a bogus act literally named "null".
                let actName = getActiveAct();
                if(!actName || !st.acts[actName])
                {
                    actName = actName || 'Act 1';
                    if(!st.acts[actName])
                    {
                ...
                if(!st.acts[actName].scenes[sceneName])
                {
                    ...
                    if(!Array.isArray(st.acts[actName].scenesOrder)) st.acts[actName].scenesOrder = [];
                    st.acts[actName].scenesOrder.push(sceneName);
                    sendStyledMessage('Director', `No active scene found. Created default scene "${sceneName}".`);
                }
                scene = st.acts[actName].scenes[sceneName];
        ...
        const getBadgeColor = (type) =>
        {
            const colors = {
                handout: '#2a80b9',
                character: '#16a085', // Teal
                npc: '#27ae60', // Green — Actor is now the preferred scene-token type
                variant: '#526f69', // Same teal-green hue, desaturated and darkened — signals "legacy, prefer Actor"
                track: '#e67e22',
                macro: '#e4048c',
                table: '#7f6c4f',
                all: '#888'
        ...
        const getBadge = (type, css) =>
        {
            ...
            else if(type === 'npc') badgeLetter = 'A'; // "Actor" — internal type string stays npc, display label doesn't
        ...
        // Display label for a stored item type. Internal type strings don't always match the
        // user-facing name (npc → "Actor", table → "Rollable Table"); falls back to capitalized.
        const typeDisplayName = (type) =>
        {
            const names = {
                npc: 'Actor',
                table: 'Rollable Table'
            };
        ...
        // Recognizes a "Director page" (stage/scene/theater/theatre). Wipe Scene runs on these
        // automatically on every scene change, so a token there is a bad pick for an NPC
        // reference — it will vanish and orphan the reference.
        const isDirectorPageName = (name) => /stage|scene|theater|theatre/.test((name || '').toLowerCase());
        ...
        // Backdrop crossfade — eases between images instead of an instant swap. The new image
        // is created on top at baseOpacity 0 and stepped up to 1, covering the old graphic,
        // which is then removed.
        const BACKDROP_FADE_STEPS = 16;
        const BACKDROP_FADE_STEP_MS = 50; // ~800ms total

        // Keyed by pageId: tracks a fade animating on that page, so a second set-backdrop click
        // settles it instantly instead of letting two fades run at once.
        const backdropFadeState = {};

        // Immediately completes whatever fade is animating on pageId, if any, and returns the
        // now-settled graphic, or null if nothing was in progress.
        const finishBackdropFade = (pageId) =>
        {
            const fade = backdropFadeState[pageId];
            if(!fade) return null;

            clearTimeout(fade.timeoutId);
            delete backdropFadeState[pageId];

            try
            {
                fade.newGraphic.set('baseOpacity', 1);
            }
            catch(e) {}
            try
            {
                if(fade.oldGraphic) fade.oldGraphic.remove();
            }
            catch(e) {}

            return fade.newGraphic;
        };

        const stepBackdropFade = (pageId, step) =>
        {
            const fade = backdropFadeState[pageId];
            if(!fade) return; // cancelled/finished elsewhere (e.g. a rapid re-click)

            try
            {
                fade.newGraphic.set('baseOpacity', step / BACKDROP_FADE_STEPS);
            }
            catch(e)
            {
                // Graphic is gone (e.g. page/scene was wiped mid-fade) — give up quietly.
                delete backdropFadeState[pageId];
                return;
            }

            if(step >= BACKDROP_FADE_STEPS)
            {
                try
                {
                    if(fade.oldGraphic) fade.oldGraphic.remove();
            ...
            fade.timeoutId = setTimeout(() => stepBackdropFade(pageId, step + 1), BACKDROP_FADE_STEP_MS);
        };

        // Starts a crossfade to newGraphic on pageId; oldGraphic is removed once the fade
        // completes. If a fade is already animating on this page, it's snapped to completion
        // first so rapid clicks don't let fades pile up.
        const startBackdropFade = (pageId, oldGraphic, newGraphic) =>
        {
            const settledFromPriorFade = finishBackdropFade(pageId);
            const effectiveOld = settledFromPriorFade || oldGraphic;

            backdropFadeState[pageId] =
            {
                oldGraphic: effectiveOld,
                newGraphic,
                timeoutId: null
            };
            stepBackdropFade(pageId, 1);
        };
...
const createOrUpdateCaption = (actName, sceneName, imageTitle, pageId) =>
{
    ...
    // Find the act that owns the active scene
...
const placeNextToken = () => {
    ...
    // NPC: duplicate the live reference token via createCopy() rather than a stored snapshot,
    // so sizing always reflects the reference's current state. If the reference token is
    // missing, skip and log — NPC buttons are never auto-pruned for a temporarily-gone reference.
    if (btn.type === 'npc') {
        try {
            const refGraphic = getObj('graphic', btn.refId);
            if (!refGraphic) {
                log(`[Director] NPC reference token not found for "${btn.name}" (refId ${btn.refId}) — skipping placement.`);
                return setTimeout(placeNextToken, 0);
            }

            const tokenWidth = refGraphic.get('width') || 70;
            const tokenHeight = refGraphic.get('height') || 70;
            ...
            const newToken = refGraphic.createCopy({
                _pageid: pageId,
                layer: 'objects',
                left: tokenLeft + tokenWidth / 2,
                top: tokenTop + tokenHeight / 2,
            });
            tagGraphicAsDirector(newToken);
            btn.tokenId = newToken.id;

            tokenTop += tokenHeight + 20;
        } catch (e) {
            log(`[Director] Error placing NPC "${btn.name}": ${e.message}`);
        }
        return setTimeout(placeNextToken, 0);
    }
    ...
    char.get('_defaulttoken', (blob) => {
        try {
            ...
            // Default tokens (unlike Variant snapshots) can use a Marketplace image directly —
            // no fallback substitution needed here.
        ...
        const wipeScene = (sceneName, playerid) =>
        {
            ...
            // Remove all gold stroke paths (star highlights) on GM layer for this page.
            // Matches both the hex value and the legacy named-color string 'gold'.
            const goldPaths = findObjs(
                ...
                .filter(p => p.get('stroke') === '#ffd700' || p.get('stroke') === 'gold');
    ...
    removeCaption(pageId);
            ...
            // Always include 2x2 layout if exactly 4 images, for a perfect fit
            ...
            const placeNextToken = () =>
            {
                ...
                // NPC: duplicate the live reference token via createCopy() — see the matching
                // branch in handleSetScene's placeNextToken for the full rationale.
                if(btn.type === 'npc')
                {
                    try
                    {
                ...
                char.get('_defaulttoken', (blob) =>
                {
                    try
                    {
                        ...
                        // Default tokens (unlike Variant snapshots) can use a Marketplace image
                        // directly — no fallback substitution needed here.
                        props.imgsrc = cleanImg(props.imgsrc);
                        props._pageid = pageId;
                        props.layer = 'objects';
        ...
        // Normalizes an imgsrc for "is this the same image?" comparisons (used by the bulk
        // Convert-to-NPC matcher). Unlike cleanImg, never appends a cache-busting query string
        // and never collapses a non-CDN URL to '', so repeated calls give a stable, comparable key.
        const imgKeyForMatch = (src) =>
        {
            if(!src) return '';
            const parts = src.match(/(.*\/images\/.*)(thumb|med|original|max)([^?]*)(\?[^?]+)?$/);
            if(parts) return parts[1] + 'thumb' + parts[3];
            return src.split('?')[0];
        };


        // handoutId is the handout this render is being written into (live "Director" handout
        // or the standalone "Director Help" handout) — header-jump links need it to build the
        // right URL for whichever document they'll live in.
        const renderHelpHtml = (css, handoutId) => {
            const helpMoreExpanded = !!getState().helpMoreExpanded;
            const helpAnchor = (headerText) => handoutId
                ? `http://journal.roll20.net/handout/${handoutId}/#${headerText.replace(/ /g, '%20')}`
                : '#';
            return `
<div style="${css.helpContainer}">
  ...
  <img src="https://files.d20.io/images/501300273/C2EKEI0PH17ZYP4zAJP_9g/original.webp?1789660087" alt="Diagram labeling the interface's four main sections" style="float:right; width:280px; height:auto; margin:0 0 10px 14px; border:1px solid #666; border-radius:4px;">
  <p>The interface appears in a Roll20 handout. It consists of five main sections:</p>
  <ul>
    <li><a href="${helpAnchor('Header Buttons')}" style="color:inherit;"><b>Header</b></a> — search, edit mode, help toggle, and scene controls (Set as Grid/Scene, Wipe the Scene, Stop Audio)</li>
    <li><a href="${helpAnchor('Acts & Scenes')}" style="color:inherit;"><b>Acts & Scenes</b></a> — scene navigation and management</li>
    <li><a href="${helpAnchor('Images')}" style="color:inherit;"><b>Images</b></a> — a scene's list of images: click any one to make it the backdrop; the rest are highlights</li>
    <li><a href="${helpAnchor('Items (Actor, Character, Variant, Handout, Tracks, Macros, Rollable Tables)')}" style="color:inherit;"><b>Items</b></a> — Actors, Characters, Variants, Handouts, Tracks, Macros, and Rollable Tables</li>
    <li><a href="${helpAnchor('Settings')}" style="color:inherit;"><b>Settings</b></a> — light/dark mode, captions, margins, backup/restore, report generation, and repair</li>
  </ul>
...
<p>Each scene holds a single list of images. Click any image in that list to make it the <b>Backdrop</b> — the main background image for the scene, displayed on the map layer to set the overall environment. Every <i>other</i> image in the list is automatically a <b>Highlight</b>.</p>
<p>When a scene is set, the backdrop is placed on the map layer, while all highlight images appear on the object layer, aligned left beyond the page boundary. From there, the GM can lightbox-preview a highlight (select it and use the shift-Z keyboard command) or drag it onto the map as a prop or visual aid whenever it's needed.</p>
<p>Clicking an image to make it the backdrop — on the scene that's currently live on the tabletop — crossfades smoothly into the new image instead of swapping instantly (the full <b>Set Scene</b>/<b>Set Grid</b> commands below still place the backdrop instantly, with no crossfade). Clicking an image while browsing a <i>different</i> scene than the one actually live doesn't touch the tabletop at all — it just changes which image that scene will use the next time it's set, so you can line up backdrops ahead of time without disturbing what's currently on screen.
  <h4>Adding Images</h4>
  <p>To add an image:
    <ol>
  ...
  <ul>
    ...
    <li><b>Click the thumbnail</b>: sets this image as the scene's backdrop</li>
    <li><b>Bottom-right icon</b>:
      <ul>
        <li>${Pictos('m')} Assign track (uses currently playing audio). When an image that has a track is made into the Backdrop image, the track assigned to it immediately starts playing.</li>
      </ul>
    </li>
...
<p>Click to toggle. When this button is red, the audio track auto-play behavior of backdrops is suppressed.</p>
  ...
  <h3>Items (Actor, Character, Variant, Handout, Tracks, Macros, Rollable Tables)</h3>
...
<div style="margin-bottom:10px;">
  ${npcFeatureAvailable ? `<div style="${css.itemBadge} background:${getBadgeColor('npc')}; float:none; position:relative;">A</div>` : ''}
...
<ul style="list-style: none; padding-left: 0; margin-left: 10px;">
  ${npcFeatureAvailable ? `<li style="position: relative; padding-left: 30px; margin-bottom: 6px;">
    <div style="${css.itemBadge} background:${getBadgeColor('npc')}; position: absolute; left: 0; top: 0;">A</div>
    <b>Actors</b>: Appears on scene set, copied live from its reference token. Opens the reference token's character sheet if it's linked to one; otherwise clicking does nothing. This is the preferred item type
  </li>` : ''}
  ...
  <li style="position: relative; padding-left: 30px; margin-bottom: 6px;">
    ...
    <b>Variants</b>: Does not open — appears on scene set${npcFeatureAvailable ? ' (legacy — prefer Actor where possible)' : ''}
  </li>
  <li style="position: relative; padding-left: 30px; margin-bottom: 6px;">
  ...
  <div style="border:1px solid #000; background:transparent; border-radius:4px; padding:6px 10px; margin:10px 0;">
    <h4 style="margin:0;">
      <a href="!director --toggle-help-more" style="text-decoration:none; color:inherit; cursor:pointer;">${helpMoreExpanded ? '▾' : '▸'} More about Actors, Characters and Variants</a>
    </h4>
    ${helpMoreExpanded ? `<div style="margin-top:8px;">
      ${npcFeatureAvailable ? `<p><i>Actors reference a token directly instead of storing a snapshot — select one or more tokens and click the A badge. Each placement makes a live copy of the reference token, so edits to it (image, bars, auras, etc.) are picked up automatically next time it's placed, taking far less storage space than Variants. If a reference token is deleted or moved, that Actor is skipped at placement (not removed) until it's restored. This is the preferred type going forward.</i></p>
      <p><i>The reference token must live somewhere permanent — a Token Page you keep for this purpose. (All Roll20-produced modules ship with a Token Page, which is a convenient place to keep your Actor reference tokens.) Director refuses to use a token sitting on a Director stage/scene/theater/theatre page as a reference, since those get wiped every time a scene is set, which would orphan any Actor pointing at it.</i></p>` : ''}

      <p><i>Variants are token snapshots that share a sheet.${npcFeatureAvailable ? ' Kept for existing installations — prefer Actor for new items where possible.' : ' Use these when default tokens cannot be reliably spawned, or to represent unique versions of a shared character sheet.'}</i></p>

      ${npcFeatureAvailable ? `<p><i>Converting a Character or Variant to an Actor (⇄, in Edit Mode): select the token to use as the reference first, then click the control. Director also scans every act and scene for other Character/Variant items that reference the same character sheet and share the same name (and, when comparing two Variants, the same image) and converts all of them to point at this same new reference token in one step, so scattered duplicates of the same Actor across a campaign don't have to be converted one at a time. An item with no linked character sheet has no reliable "same Actor" signal to match on, so it always converts alone.</i></p>` : ''}

      ${npcFeatureAvailable
        ? `<p><i>Actor and Character tokens can use Marketplace-asset images directly, since they're always created from a default token or an already-existing token. Variants create a brand-new token from a stored image, and the API can't place a Marketplace asset that way — if a Variant's assigned image is a Marketplace asset, Director substitutes a fallback image and warns you when this happens. This is another reason to prefer Actor over Variant.</i></p>`
        : `<p><i>Character tokens can use Marketplace-asset images directly, since they're created from your character's default token. Variants create a brand-new token from a stored image, and the API can't place a Marketplace asset that way — if a Variant's assigned image is a Marketplace asset, Director substitutes a fallback image and warns you when this happens.</i></p>`}
    </div>` : ''}
  </div>
  ...
  <ul>
    ...
    <li><b>${Pictos('p')}</b> — Rename (changes the display name only, not what the item points to)</li>
    <li><b>${Pictos('R')}</b> — Redefine (reassign what this item points to — a different character, handout, macro, table, track, or reference token)</li>
    ${npcFeatureAvailable ? `<li><b>⇄</b> — Convert to Actor (Character and Variant items only). Select the token to use as the reference first, then click this — see below</li>` : ''}
    <li><b>${Pictos('#')}</b> — Delete</li>
  </ul>

  <p>
  <b>Star system:</b> Use stars to link specific Actors, Characters, or Variants to a backdrop image. For instance, if a scene has several shops, you can star each proprietor for their shop image. When that image is the backdrop, the starred items' tokens are highlighted in gold in the token list. This feature is disabled in Grid mode.<br>
  ...
  <p>
  <b>Supernotes Integration:</b> If you have installed the Supernotes Mod script, a small info button appears on the same line as each Actor, Character, or Variant in the Items list. Clicking it whispers the item's token notes to chat (using Supernotes' card layout).<br>
  This button only shows when there's a live token for it to reference, so Supernotes is never called with an ID that no longer exists. For Actors, that means whenever the reference token still exists — regardless of which scene you're browsing. For Characters and Variants, it means only while their scene is the one actually live on the tabletop; browsing a different scene hides the button, since that scene's tokens aren't the ones currently on screen.
  </p>
  <br>
...
<h4>Margins</h4>
<p>Use Margins Off if you use the Director interface docked in the VTT, and turn them On if you use it popped out into its own window. Off is the default.</p>
<h4>Backup</h4>
<ul>
...
<br>
  ...
  <p><span style="${css.headerSubButton}; float:none; position:relative;">Scene</span> populates the tabletop with:
    <ul>
      ...
      <li>Actor, Character, and Variant tokens (Object Layer, right-aligned off page edge)</li>
  ...
  <ul>
    ...
    <li>Items are grouped by type and display the full set of controls described under Edit Controls above (rename, redefine, convert to Actor, delete, and reorder)</li>
  </ul>
    <h4>JB+</h4>
...
!director --capture-image
  ...
  <p>An entire scene setup can also be triggered from a single macro, which is handy for GMs who want a preset list of scenes keyed to macro buttons. Both commands take the Act, Scene, and Backdrop/Image name directly (spelled however you like — matching is case-insensitive), so these macros are easy to write and read by hand:</p>
  <pre style="background:#eee; padding:6px; font-size:12px;">
!director --set|Act I|Opening Scene|Tavern Interior
  </pre>
  <ul>
    <li><b>--set|Act|Scene|Backdrop</b> — cuts straight to a brand-new scene: wipes whatever's currently up, then sets and pushes live the given act/scene/backdrop</li>
    <li><b>--reveal|Act|Scene|Backdrop</b> — just navigates and sets the backdrop, no wipe, nothing pushed live. Matches the <b>Reveal</b> button on a search/Find result: it crossfades to a new backdrop within a scene that's already live, or pre-stages which scene/backdrop will show next if that scene isn't live yet</li>
  </ul>
  <p>If the Act, Scene, or Backdrop name can't be found, neither command changes anything — chat reports which name(s) couldn't be matched so you can check spelling.</p>
</div>
`; };

// Keeps the exported "Director Help" handout (if one exists) in sync with state that affects
// renderHelpHtml's output. No-op if the handout was never created.
const refreshHelpHandoutIfExists = () =>
{
    const handout = findObjs(
    {
        type: 'handout',
    ...
    const css = getCSS();
    const helpHtml = renderHelpHtml(css, handout.id);
    handout.set(
    {
        notes: helpHtml
    });
...
const highlightStarredTokens = (sceneName, pageId) =>
{
    ...
    const padding = 12;
    // Roll20's pathv2 stroke/fill only support "transparent" or a hex color, not the named
    // color 'gold' (renders black) — use gold's hex value instead.
    ...
    starredList.forEach(btnId =>
    {
        ...
        // Match the exact token this item placed (btn.tokenId), not by shared name — matching
        // by name would highlight every token sharing it, not just the one starred.
        if(!btn.tokenId) return;
        const g = getObj('graphic', btn.tokenId);
...
const renderFilterBarInline = (css) =>
{
    ...
    const buttons = [
        // Actor is the preferred way to bring a token onto the stage — listed first.
        // Requires Roll20's v1.5+ sandbox (createCopy) — hide the button entirely on
        // older campaigns rather than let the GM create buttons that can't be placed.
        ...(npcFeatureAvailable ? [`<a href="!director --add-item|npc" style="${css.itemBadge} background:${getBadgeColor('npc')};" title="Add Actor (from Selected Reference Token)">A</a>`] : []),
...
const renderItemsList = (css) =>
{
    ...
    const items = (st.items?.buttons || []).filter(btn =>
    {
        ...
        const typeMatch = activeFilter === 'all' ||
            ...
            (activeFilter === 'character' && (btn.type === 'variant' || btn.type === 'npc'));
    ...
    return items.map(btn =>
    {
        ...
        else if(btn.type === 'npc')
        {
            const refGraphic = btn.refId && getObj('graphic', btn.refId);
            const refCharId = refGraphic && refGraphic.get('represents');
            if(refGraphic && refCharId)
            {
                // Same behavior as Character/Variant: clicking the name opens the sheet the
                // reference token is linked to, not the token itself.
                action = `http://journal.roll20.net/character/${refCharId}`;
                tooltipAttr = ` title="Open character sheet"`;
            }
            else if(refGraphic)
            {
                action = 'javascript:void(0)';
                tooltipAttr = ` title="Actor reference token: ${(btn.name || '').replace(/"/g, '&quot;')} (not linked to a character sheet)"`;
            }
            else
            {
                action = 'javascript:void(0)';
                tooltipAttr = ` title="⚠ Actor reference token missing — was it deleted or moved? Placement will be skipped until it's restored."`;
            }
        }
...
// --- build edit controls ---
const isRedefinable = ['variant', 'npc', 'character', 'handout', 'macro', 'table', 'track'].includes(btn.type);
...
switch (btn.type) {
    case 'variant':
        ...
        break;
    case 'npc':
        // Use currently selected token as the new reference
        redefineCmd = `!director --redefine-item|${btn.id}|npc`;
        break;
    case 'character':
...
const editControls = isEditMode
    ? `
    <a href="!director --renameitem|${btn.id}|?{New display name|${sanitizeQueryDefault(btn.displayName || btn.name)}}"
    ...
    ${isRedefinable
        ? `<a href="${redefineCmd}"
             title="Redefine this ${typeDisplayName(btn.type)}"
             style="${css.utilityEditButton}; z-index: 9999;">${Pictos('R')}</a>`
        : ''}
    ${((btn.type === 'variant' || btn.type === 'character') && npcFeatureAvailable)
        ? `<a href="!director --convert-to-npc|${btn.id}"
             title="Convert this ${typeDisplayName(btn.type)} to an Actor — select the token to use as its reference first, then click this"
             style="${css.utilityEditButton}; z-index: 9999;">⇄</a>`
        : ''}
...
// --- GM Note button ---
// Which token ID Supernotes should look up depends on item type: NPC uses the permanent
// btn.refId reference token; Character/Variant uses btn.tokenId, which is only valid while
// this item's scene is the one live on the VTT (otherwise it's a stale ID from last placement).
// A dead ID crashes Supernotes, so the button is hidden when there's no live token.
let gmNoteHTML = '';
if(state.Supernotes)
{
    let noteTokenId = null;
    if(btn.type === 'npc')
    {
        if(btn.refId && getObj('graphic', btn.refId)) noteTokenId = btn.refId;
    }
    else if(btn.type === 'character' || btn.type === 'variant')
    {
        if(currentScene === st.lastSetScene && btn.tokenId && getObj('graphic', btn.tokenId)) noteTokenId = btn.tokenId;
    }

    if(noteTokenId)
    {
        gmNoteHTML = `
          <a href="!gmnote --id${noteTokenId} --card"
...
const updateHandout = () =>
{
    ...
    for(const actName of st.actsOrder)
    {
        ...
        const actControls = isEditMode ?
            `
<a href="!director --rename-act|${encodeURIComponent(actName)}|?{New Act Name}" title="Rename Act" style="${css.utilityEditButton}">${Pictos('p')}</a>
<a href="!director --delete-act-confirm|?{Are you sure you want to delete act &quot;${actName}&quot;? This will delete all scenes and assets within this act. This cannot be undone.|Delete|Cancel}|${encodeURIComponent(actName)}"
        ...
        if(expanded)
        {
            ...
            for(const sceneName of scenesOrder)
            {
                ...
                const sceneControls = isEditMode ?
                    `
            <a href="!director --rename-scene|${encodeURIComponent(actName)}|${encodeURIComponent(sceneName)}|?{New Scene Name}" title="Rename Scene" style="${css.utilityEditButton}">${Pictos('p')}</a>
<a href="!director --delete-scene-confirm|?{Are you sure you want to delete scene &quot;${sceneName}&quot; in act &quot;${actName}&quot;? This cannot be undone.|Delete|Cancel}|${encodeURIComponent(actName)}|${encodeURIComponent(sceneName)}"
    ...
    const imagesHTML = (() =>
    {
      ...
      <div style="${css.imageContainer}; " data-id="${img.id}">
        <div style="${scenesObj[activeScene]?.backdropId === img.id ? css.imageBoxBackdropWrapper : css.imageBoxWrapper}; position: relative;">
          <a href="!director --set-image-title|${img.id}|?{New title:|${sanitizeQueryDefault(img.title)}}" style="${css.imageTitleOverlay}">${img.title || 'Untitled'}</a>
          <div> 
...
const backupOptions = backupHandouts.length > 0
    ...
    const html = `
    <div style="width:auto;${st.settings.marginsOn ? '' : ' margin:-35px -30px;'}">
      <table style="width:100%; border-collapse:collapse;">
        <tr>
          <td colspan="3" style="${css.header}">
            Director
...
<div style="${css.utilityContainer}">
  Margins
  <a href="!director --margins|on" style="${css.utilitySubButton}${st.settings.marginsOn ? `; border: 1px solid #44aa44;` : ''}">On</a>
  <a href="!director --margins|off" style="${css.utilitySubButton}${!st.settings.marginsOn ? `; border: 1px solid #44aa44;` : ''}">Off</a>
</div>
...
// === Shared step logic for --set / --reveal ===
// Pulled out of the matching standalone commands so --set and --reveal can call the same
// logic instead of a duplicate copy. Standalone commands validate input, then delegate here.

const doExpandActOnly = (actName) =>
{
    const st = getState();
...
const doSetActiveScene = (sceneName) =>
{
    state[stateName].activeScene = sceneName;
    updateHandout();
};

const doWipeScene = (playerid) =>
{
    const st = getState();
    ...
    const pageId = getPageForPlayer(playerid) || Campaign().get('playerpageid');
    wipeScene(currentScene, playerid);
...
// Core of "Set Backdrop" — shared by the standalone --set-backdrop command and the --set/
// --reveal composite commands, which resolve a backdrop by name to an image id first and then
// hand off to this exact same logic.
const doSetBackdrop = (imageId, playerid) =>
{
    const st = getState();
    const currentScene = st.activeScene;
    if(!currentScene) return;
    ...
    // Only the scene that's actually live on the VTT (st.lastSetScene) may touch the VTT page,
    // jukebox, captions, or star highlights. Clicking a backdrop while browsing a different
    // scene just changes what that scene will show next time it's set.
    const isLiveScene = currentScene === st.lastSetScene;
    let pid = null;

    if(isLiveScene)
    {
        // Stop track for the current backdrop if it has one — must read
        // scene.backdropId BEFORE it's overwritten further down.
        ...
        if(!isDirectorPageName(page.get('name')))
        {
            sendStyledMessage('Set Backdrop', `Current page "${page.get('name')}" cannot be used as a Director page. The page title must contain one of these words:<br><i>stage, scene, theater, or theatre</i>.<br><br>Skipping backdrop change.`);
            return;
        }
        ...
        // Crossfade in the new image rather than mutating targetGraphic's imgsrc directly.
        // If there's no existing backdrop graphic, this just fades the new one in from nothing.
        ...
        tagGraphicAsDirector(newGraphic);
        toFront(newGraphic);

        startBackdropFade(pid, targetGraphic, newGraphic);
        ...
        // Update caption for new backdrop — uses getActiveAct() rather than the
        // separately-tracked st.activeAct, which isn't guaranteed to match.
        createOrUpdateCaption(
            getActiveAct(),
            currentScene,
    ...
    // Set the new backdrop — state and handout always reflect the click regardless of
    // whether this scene is live. Must happen before highlightStarredTokens below, which
    // reads scene.backdropId to decide which starred list applies.
    ...
    for(const part of parts)
    {
        ...
        switch(cmd.trim())
        {
            ...
            case 'add-item':
            {
                ...
                const validTypes = ['handout', 'character', 'track', 'macro', 'action', 'table', 'variant', 'npc'];
...
if (type === 'track') {
    ...
    break;
                ...
                // === Actor (internal type: npc): create one button per selected token, storing
                // only a reference to the token itself (no property snapshot) — createCopy()
                // re-derives everything live from that token at placement time. ===
                if(type === 'npc')
                {
                    if(!npcFeatureAvailable)
                    {
                        sendStyledMessage('Director', 'Actor items require Roll20\'s v1.5+ API sandbox (this campaign is on the classic sandbox). Use Variant instead, or ask Roll20 support about upgrading.');
                        break;
                    }
                    if(!msg.selected || !msg.selected.length)
                    {
                        sendStyledMessage('Director', 'You must select one or more reference tokens to define as Actors.');
                        break;
                    }
                    ...
                    for(const sel of msg.selected)
                    {
                        ...
                        // Refuse tokens sitting on a Director page (stage/scene/theater/
                        // theatre) — see isDirectorPageName for why: they're routinely wiped.
                        const tokenPage = getObj('page', token.get('_pageid'));
                        if(isDirectorPageName(tokenPage?.get('name')))
                        {
                            blocked.push(token.get('name') || 'Unnamed token');
                            continue;
                        }
                        ...
                        st.items.buttons.push(
                        {
                            id: generateUUID(),
                            type: 'npc',
                            name: token.get('name') || 'New Actor',
                            refId: token.id,
                            scene: activeScene,
                        });

                        created.push(token.get('name') || 'New Actor');
                    }
                    ...
                    if(blocked.length)
                    {
                        sendStyledMessage('Director', `Skipped ${blocked.length} token${blocked.length > 1 ? 's' : ''} on a Director stage/scene page — these get wiped on the next scene change, so they can't be used as Actor references: ${blocked.join(', ')}. Move ${blocked.length > 1 ? 'them' : 'it'} to another page first (e.g. your Token Page), then try again.`, 'warning');
                    }
...
case 'redefine':
{
    ...
    // === NPC/Actor: point the reference at a newly-selected token (no snapshot stored) ===
    if (type === 'npc')
    {
        if (!msg.selected || !msg.selected.length)
        {
            sendStyledMessage('Redefine Actor', 'Select a token to use as this Actor\'s new reference.');
            break;
        }
        ...
        if (!token)
        {
            sendStyledMessage('Redefine Actor', 'Invalid token selection.');
            break;
        }
        ...
        if (isDirectorPageName(tokenPage?.get('name')))
        {
            sendStyledMessage('Redefine Actor', `"${token.get('name') || 'That token'}" is on a Director stage/scene page — it gets wiped on the next scene change, so it can't be used as an Actor reference. Move it to another page first (e.g. your Token Page), then try again.`, 'warning');
            break;
        }

        btn.name = token.get('name') || 'New Actor';
        btn.refId = token.id;
        btn.scene = activeScene;

        sendStyledMessage('Actor Redefined', `Actor "${btn.name}" now references the selected token.`);
        updateHandout();
        break;
    }
    ...
    if (roll20Type)
    {
        ...
        if (!objs.length)
        {
            sendStyledMessage('Redefine', `No ${typeDisplayName(type).toLowerCase()}s found in the game.`);
            break;
        }
        ...
        sendStyledMessage(
            'Redefine',
            `[Select new ${typeDisplayName(type)}](~!director --redefine-assign|${btnId}|?{Select ${typeDisplayName(type)}|${options}})`
        );
        break;
    }
...
case 'redefine-assign':
{
    ...
    sendStyledMessage('Redefine', `${typeDisplayName(btn.type)} updated to "${newName}".`);
    break;
}
...
case 'redefine-item': {
    ...
    if (['character','handout','macro','table'].includes(type)) {
        ...
        if (!obj) {
            sendStyledMessage('Redefine', `Invalid selection for ${typeDisplayName(type)}.`);
            break;
        }
        btn.refId = obj.id;
        ...
        sendStyledMessage('Redefine', `${typeDisplayName(type)} updated to "${obj.get('name')}".`);
        updateHandout();
        break;
    }
    ...
    break;
            ...
            case 'convert-to-npc':
            {
                ...
                if(!btn)
                {
                    sendStyledMessage('Convert to Actor', `No item found with ID ${btnId}.`);
                    break;
                }
                if(btn.type !== 'variant' && btn.type !== 'character')
                {
                    sendStyledMessage('Convert to Actor', `"${btn.name}" is a ${typeDisplayName(btn.type)} — only Variant and Character items can be converted.`);
                    break;
                }
                if(!npcFeatureAvailable)
                {
                    sendStyledMessage('Convert to Actor', 'Actor items require Roll20\'s v1.5+ API sandbox (this campaign is on the classic sandbox).');
                    break;
                }

                // Requires exactly one selected token to use as the reference, same as Add Actor.
                if(!msg.selected || !msg.selected.length)
                {
                    sendStyledMessage('Convert to Actor', `Select the token to use as the reference for "${btn.name}", then try again.`);
                    break;
                }
                if(msg.selected.length > 1)
                {
                    sendStyledMessage('Convert to Actor', `Select only one token to use as the reference for "${btn.name}" — ${msg.selected.length} are currently selected.`);
                    break;
                }

                const refGraphic = getObj('graphic', msg.selected[0]._id);
                if(!refGraphic)
                {
                    sendStyledMessage('Convert to Actor', 'The selected token could not be found. It may have been deleted — reselect and try again.');
                    break;
                }

                // Same volatility risk Add Actor and Redefine Actor already guard against: a token
                // sitting on a Director stage/scene/theater/theatre page gets wiped on the next
                // scene change (Wipe Scene runs automatically), which would orphan this Actor and
                // every Variant bulk-matched onto it.
                const refPage = getObj('page', refGraphic.get('_pageid'));
                if(isDirectorPageName(refPage?.get('name')))
                {
                    sendStyledMessage('Convert to Actor', `"${refPage.get('name')}" is a Director stage/scene page — tokens there get wiped on the next scene change, so it can't be used as a reference. Move the token to another page (e.g. your Token Page) first, then try again.`);
                    break;
                }

                const oldName = btn.name;
                const matchType = btn.type; // 'variant' or 'character', captured before mutation below

                // Finds other Variant/Character items representing the same thing, to repoint them
                // at the same reference token. Matches on character sheet + name, plus image when
                // both sides are Variants (Characters have no stored image, so name+sheet suffices).
                const matchRefId = btn.refId;
                const matchNameLower = (btn.name || '').toLowerCase();
                const matchImgKey = matchType === 'variant' ? imgKeyForMatch(btn.tokenProps?.imgsrc) : null;

                btn.type = 'npc';
                btn.refId = refGraphic.id;
                delete btn.tokenProps;

                // An item with no linked character sheet has no reliable match signal, so it
                // converts alone, never in bulk.
                let bulkNote = '';
                if(!matchRefId)
                {
                    bulkNote = ` This ${typeDisplayName(matchType)} has no linked character sheet, so it was converted on its own — bulk matching requires one.`;
                }
                else
                {
                    const bulkMatches = (st.items?.buttons || []).filter(other =>
                    {
                        if(other === btn) return false;
                        if(other.type !== 'variant' && other.type !== 'character') return false;
                        if(other.refId !== matchRefId) return false;
                        if((other.name || '').toLowerCase() !== matchNameLower) return false;
                        // Only require the image to match when BOTH sides are Variants — that's
                        // the only pairing where per-instance art can actually diverge.
                        if(matchType === 'variant' && other.type === 'variant')
                        {
                            return imgKeyForMatch(other.tokenProps?.imgsrc) === matchImgKey;
                        }
                        return true;
                    });

                    bulkMatches.forEach(other =>
                    {
                        other.type = 'npc';
                        other.refId = refGraphic.id;
                        delete other.tokenProps;
                    });

                    if(bulkMatches.length > 0)
                    {
                        const scenes = [...new Set(bulkMatches.map(b => b.scene).filter(Boolean))];
                        const criteriaDesc = matchType === 'variant' ? 'same character sheet and name — plus matching art, for other Variants' : 'same character sheet and name';
                        bulkNote = ` Also found ${bulkMatches.length} matching item${bulkMatches.length === 1 ? '' : 's'} (${criteriaDesc}) elsewhere${scenes.length ? ` in ${scenes.join(', ')}` : ''} and converted ${bulkMatches.length === 1 ? 'it' : 'them'} to point at this same reference token.`;
                    }
                }
                ...
                sendStyledMessage('Convert to Actor', `"${oldName}" converted to Actor, using the selected token${refPage ? ` on "${refPage.get('name')}"` : ''} as its reference.${bulkNote}`);
                break;
            }
...
case 'roll-table': {
    ...
    break;
            ...
            // --set|Act|Scene|Backdrop Name    == wipe-scene, expand-act-only, set-active-scene,
            //                                     set-backdrop, set-scene
            // --reveal|Act|Scene|Backdrop Name == expand-act-only, set-active-scene, set-backdrop
            //                                     (no wipe, no final set-scene)
            // Both match names case-insensitively and validate the entire chain before touching
            // any state; on failure nothing changes. Both delegate to the same shared step
            // functions used by the standalone commands above.
            case 'set':
            case 'reveal':
            {
                const isSet = (cmd.trim() === 'set');
                const commandLabel = isSet ? 'Set' : 'Reveal';
                const [rawAct, rawScene, rawImageName] = params.map(p => decodeURIComponent((p || '').trim()));

                if(!rawAct || !rawScene || !rawImageName)
                {
                    sendStyledMessage(commandLabel, `Usage: <b>--${isSet ? 'set' : 'reveal'}|Act|Scene|Backdrop Name</b> — all three must be specified.`);
                    break;
                }

                const st = getState();
                const bad = (name) => `<b style="color:crimson;">"${name}"</b>`;
                const good = (name) => `"${name}"`;

                const actKey = findActKeyCI(st, rawAct);
                const act = actKey ? st.acts[actKey] : null;
                const sceneKey = act ? findSceneKeyCI(act, rawScene) : null;
                const scene = sceneKey ? act.scenes[sceneKey] : null;
                const image = scene ? findImageByTitleCI(scene, rawImageName) : null;

                if(!actKey || !sceneKey || !image)
                {
                    const chain = [
                        actKey ? good(rawAct) : bad(rawAct),
                        sceneKey ? good(rawScene) : bad(rawScene),
                        image ? good(rawImageName) : bad(rawImageName)
                    ].join(' &rarr; ');

                    sendStyledMessage(
                        commandLabel,
                        `Could not find: ${chain}<br><br>The highlighted name(s) above could not be found. Check that they exist and are spelled correctly. Nothing was changed.`
                    );
                    break;
                }
                ...
                if(isSet)
                {
                    doWipeScene(playerid);
                }
                doExpandActOnly(actKey);
                doSetActiveScene(sceneKey);
                doSetBackdrop(image.id, playerid);
                if(isSet)
                {
                    handleSetScene(playerid);
                }
                break;
            }
            ...
            case 'item-placeholder':
            {
                ...
                if(btn)
                {
                    sendStyledMessage('Not Implemented', `This is a placeholder for a <b>${typeDisplayName(btn.type)}</b> item.`, 'warning');
                }
                break;
            }
...
case 'captions-on':
{
    ...
    break;
            ...
            case 'toggle-help-more':
            {
                const st = getState();
                st.helpMoreExpanded = !st.helpMoreExpanded;
                updateState(st);
                updateHandout();
                // Also refresh the exported Help handout, if one exists, so clicking this
                // link from inside THAT handout updates what the GM is actually looking at
                // instead of only the live interface handout.
                refreshHelpHandoutIfExists();
                break;
            }
            ...
            case 'rename-act':
            {
                const oldName = params[0] ? decodeURIComponent(params[0]).trim() : '';
            ...
            case 'rename-scene':
            {
                const actName = params[0] ? decodeURIComponent(params[0]).trim() : '';
                const oldSceneName = params[1] ? decodeURIComponent(params[1]).trim() : '';
            ...
            {
                const [choice, encodedActName, encodedSceneName] = val.split('|');
                const actName = encodedActName ? decodeURIComponent(encodedActName) : encodedActName;
                const sceneName = encodedSceneName ? decodeURIComponent(encodedSceneName) : encodedSceneName;
                if(choice === 'Cancel')
                {
            ...
            {
                const [choice, encodedActName] = val.split('|');
                const actName = encodedActName ? decodeURIComponent(encodedActName) : encodedActName;
                if(choice === 'Cancel')
                {
...
case 'renameitem':
{
    ...
    break;
            ...
            case 'margins':
            {
                if(val !== 'on' && val !== 'off')
                {
                    sendStyledMessage('Director', 'Margins must be "on" or "off".');
                    break;
                }
                st.settings.marginsOn = (val === 'on');
                updateHandout();
                break;
            }
            ...
            case 'make-help-handout':
            {
                ...
                // Header-jump links need this handout's own id to build correct
                // http://journal.roll20.net/handout/<id>/#<header text> links, so the handout
                // has to exist (even briefly empty) before renderHelpHtml runs.
                if(!handout)
                {
