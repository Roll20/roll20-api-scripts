...
const Fetch = (() => { //eslint-disable-line no-unused-vars
    ...
    const vd = new Date(1784330806533);
    const versionInfo = () => {
    ...
    let initialization; // see ready event handling
    ...
    const getChar = ({ query: query = '', msg: msg } = {}) => {
        let character = query;
        if (typeof query !== 'string') return (character && character.id && initialization) ? simpleObj(character) : character;
    ...
    const getHandout = ({ query: query = '', msg: msg } = {}) => {
        let handout = query;
        if (typeof query !== 'string') return (handout && handout.id && initialization) ? simpleObj(handout) : handout;
        let qrx = new RegExp(escapeRegExp(query), 'i');
    ...
    const getPin = ({ query: query = '', msg: msg } = {}) => {
        let pin = query;
        if (typeof query !== 'string') return (pin && pin.id && initialization) ? simpleObj(pin) : pin;
    ...
    const getStatus = ({ source: source = '', query: query = '', msg: msg = {}, msgId: msgId = generateUUID() } = {}) => {
        ...
        token = typeof source === 'string'
            ? getGraphic({ query: source, msg: msg/*, pageid: getPageForPlayer(msg.playerid) */ })
            : source;
        if (!token) return;
    ...
    const getRollableTable = ({ query: query = '', msg: msg } = {}) => {
        let table = query;
        if (typeof query !== 'string') return (table && table.id && initialization) ? simpleObj(table) : table;
        let qrx = new RegExp(escapeRegExp(query), 'i');
        ...
        return (table && table.id) ? simpleObj(table) : undefined;
    };
    const getTableItems = ({ query: query = '', tbl: tbl = '', msg: msg } = {}) => {
        let item = query;
        if (typeof query !== 'string') return (item && item.id && initialization) ? simpleObj(item) : item;
        item = getObjOrNull('tableitem', query);
        let table;
        if (tbl) {
    ...
    const getFirstObjectOfType = (type = '') => {
        ...
        let lib = {
            campaign: () => Campaign(),
            // card
            character: () => getChar({ query: o }),
            custfx: () => getCustFx({ query: o.id }),
            // deck
            // door
            handout: () => getHandout({ query: o }),
            ...
            pin: () => getPin({ query: o }),
            // player
            rollabletable: () => getRollableTable({ query: o }),
            status: () => {
                ...
                return getStatus({ source, query });
            },
            tableitem: () => {
                return getTableItems({ query: o })
            },
            tag: () => {
                return { type: 'tag', is: 'no', count: 0 };
                // o = findObjs({ type: 'character' }).filter(c => c.get('tags').length)[0] ||
                //     findObjs({ type: 'handout' }).filter(h => h.get('tags').length)[0];
                // return o
                //     ? getTag({ oid: o.id, otype: o.get('type'), query: o.get('tags')[0], pid: getFirstGM().id })
                //     : { type: 'tag', is: 'no', count: 0 };
