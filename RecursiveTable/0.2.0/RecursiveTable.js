	...
	parseInlines = function(inlines,opts){
		return new Promise((returnSubs)=>{
			...
			_.each(inlines,(rollRecord,msgIdx)=>{
				...
				_.each(rollRecord.results.rolls,(roll,idx)=>{
					if(_.has(roll,'table')){
			...
			if(promises.length){
				Promise.all(promises)
                    ...
                    .catch((e)=>{
                        ...
                        sendChat(`RecursiveTables`,`/w "${opts.who}" <div>An Error occurred: <code>${die.tableItem.name}</code></div><div>Error: <code>${e.message}</code></div>`);
                    });
			} else {
				returnSubs(subs);
			}
    ...
    parseMessage = function(msg, opts){
        ...
        parseInlines(msg.inlinerolls, opts)
            ...
            .catch((e)=>{
                ...
                sendChat(`RecursiveTables`,`/w "${opts.who}" <div>An Error occured with this message: <code>${die.tableItem.name}</code></div><div>Error: <code>${e.message}</code></div>`);
            });
    },
