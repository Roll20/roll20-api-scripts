...
const TableToTable = (() => { // eslint-disable-line no-unused-vars
    ...
    const vd = new Date(1755832397041);
    const versionInfo = () => {
    ...
    //		UTILITIES
    // ==================================================
    RegExp.prototype.test0 = function (s) {
        this.lastIndex = 0;
        return this.test(s);
    };
    const libPrefix = 'https://'; //'https://s3.amazonaws.com/files.d20.io/images/';
    const storageRx = /https:\/\/(?:s3\.amazonaws\.com\/)?files\.d20\.io\/images\/[^/]+\/[^/]+\/(thumb|original|max)\.(?:jpg|png|gif)(?:\?\d+)?/gi
    const generateUUID = (() => {
        let a = 0;
        let b = [];
    ...
    const highlightLibrary = async (argObj) => {
        ...
        let newnotes = buildLibImage(argObj, library.filter(u => storageRx.test0(u))).join('');
                    ...
                    return m.replace(/(^.*?)(<tr.*?<\/tr>)(<\/table>.*)/i, (m, opening, rows, ending) => { // get rows of table
                        ...
                        if (newrows[0].length > 4) { // currently wrapped (user sees image, buttons)
                            ...
                            newrows = newrows.map((r,i) => {
                                ...
                                r[2] = r[2].replace(storagerx, m => html.img(m, '', localCSS.rowImage));
        ...
        switch(argObj.type.toLowerCase()) {
            ...
            case 'lib': // setup library
                ...
                if (argObj.confirm && validateBoolean(argObj.confirm)) {
                    ...
                    workingnotes.replace(/(?:\r\n|\r|\n)/g, '').replace(storageRx, m => {
                        library.push(m);
                        return m;
                    });
                    newnotes = buildLibImage(argObj, library.filter(u => storageRx.test0(u))).join('');
                    ho.set({ notes: newnotes });
