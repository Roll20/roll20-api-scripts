...
const Inspector = (() => { // eslint-disable-line no-unused-vars
    ...
    const vd = new Date(1766852906054);
    const versionInfo = () => {
    ...
    const getTipFromObjForID = (obj) => {
        let o = simpleObj(obj);
        let contents = (validTypes[o._type] || validTypes.default)(o);
    ...
    const escapePreserveLineBreaks = (s) => {
        if (s && s.length) {
            ...
            return HE(s.replace(/\n/g, aboutuuid))
                .replace(new RegExp(aboutuuid, 'g'), '<br>');
        }
        return s;
    };
    const validTypes = {
        'door': (o) => {
            return {
                ...
                Locked: o.isLocked,
                Secret: o.isSecret,
                Position: `(${Math.round(o.path.handle0.x)}, ${Math.round(o.path.handle0.y)}), (${Math.round(o.path.handle1.x)}, ${Math.round(o.path.handle1.y)})`,
            }
        },
        'window': (o) => {
            return {
        ...
        'default': (o) => {
            return {
                header: `UNKOWN TYPE: ${o._type || 'missing'}`,
                ID: o._id,
                Props: Object.keys(o).join(', ')
            }
        }
    };
    ...
    const libButtonsForRelatedChildren = {
        page: {
            ...
            door: (p) => Messenger.Button({ type: '!', elem: `!about --typefor type=door for=${p.name}`, label: html.tip('h', 'Doors'), css: { ...localCSS.relatedLink, ...{ 'font-family': 'Pictos Three' } } }),
            window: (p) => Messenger.Button({ type: '!', elem: `!about --typefor type=window for=${p.name}`, label: html.tip('t', 'Windows'), css: { ...localCSS.relatedLink, ...{ 'font-family': 'Pictos Custom' } } })
        },
        rollabletable: {
    ...
    const libRelatedChildren = { // p will be a simpleObj
        page: {
            ...
            door: (p) => findObjs({ type: 'door' }).filter(c => c.get('pageid') === p._id),
            window: (p) => findObjs({ type: 'window' }).filter(c => c.get('pageid') === p._id)
        },
        rollabletable: {
    ...
    // ==================================================
    const apihandles = {
        about: /^!(?:about|inspect)\b/i,
        aboutfirst: /^!(?:about|inspect)first\b/i,
        aboutconfig: /^!(?:about|inspect)config\b/i
    };
    ...
    const handleInput = (msg) => {
        ...
        let wto = /^![^\s\(]+\(chat\)/.test(msg.content) ? '' : msg.who.replace(/\s\(gm\)$/i, '');
