...
//			tokenObj.set('imgsrc',getCleanImgsrc(closedImg[0]));
	...
	var evalAttr = function(v) { 
		...
		try {
			return (!v || !v.length) ? '' : v.replace(/;/g,',')
											 .replace(/\^\(/g,'Math.max(')
											 .replace(/v\(/g,'Math.min(')
											 .replace(/\-\-/g,'+')
											 .replace(/\+\-/g,'-')
											 .replace(/\([\d\+\-\*\/]+?\)/g,eval)
											 .replace(/\d+\:\d+/g,calcAttr)
											 .replace(/(\d+)d(\d+)(?:r(\d+))?/ig,reRoll)
											 .replace(/\-\-/g,'+')
											 .replace(/\+\-/g,'-')
											 .replace(/[Mathmxin\.\,\(\)\d\+\-\*\/]+/,eval);
		} catch {
			sendError('Invalid attribute value given: calculating "'+orig+'" but only **\'+ - &#42; / ( ) : d ^ v ,\'** can be used. See **[CommandMaster Help]('+fields.journalURL+handoutIDs.CommandMasterHelp+')** for allowed Creature attribute specification formats.');
			return v;
		};
	};
	...
	async function handleAddAllPowers( args, type, selected, senderId, silent=false ) {
		...
		for (const t of selected) {
			...
			if (type === 'AB') {
				...
				_.each (_.uniq(powers[1].join('|').split('|').filter(ab=>!!ab)),(ab,i) => {
	...
	async function handleClassSelection( args, selected, senderId, isGM ) {
		...
		try {
			...
			async function classAndRace( cmd, args, token, value, senderId, selected, msg, qualifier ) {
				...
				if (selected.length === 1) makeClassMenu( args, selected, senderId, true, msg );
			};
