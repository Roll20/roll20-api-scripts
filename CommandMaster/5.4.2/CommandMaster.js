 ...
 * v4.0.2  02/04/2025  Made maths for Drag & Drop creature item quantities more flexible. Fixed bug in
 *                     makeConvertSpellsMenu() which used an invalid field name. 
 * v5.0.0  25/05/2025  Fixed issues with 'items:' equipment tag in class/race definitions. Added
 *                     RemoveBlankLines() to tidy tables on creation or overwriting a new NPC.
 *                     Added money to Drag & Drop attributes. Added weapon proficiency slot management,
 *                     and proficiency training sequence support. Added support for Drag & Drop traders.
 *                     Extended item conversion system to convert legacy Items table to multi-table
 *                     approach and make better use of character sheet, and allow money management.
 ...
 *                     DM macro button.
 * v5.3.1  30/04/2026  Corrected --add-treasure command display when None defined.  On first character sheet
 *                     token drop ask GM what type of token it should be.
 * v5.4.0  31/05/2026  Added shots: creature data tag to support called shots and body part AC. Added 
 *                     Surprise GM macro.
 * v5.4.2  20/07/2026  No changes, just keeping in step with RPGM versions.
 */
...
//		grey_action: '&lt;span style="display: inline-block; background-color: lightgrey; border: 1px solid black; padding: 4px; color: dimgrey; font-weight: extra-light;"&gt;$1$2&lt;/span&gt;$4',
	...
	CommandMaster_Help:	{name:'CommandMaster Help',
						 ...
						 bio:'<div style="font-weight: bold; text-align: center; border-bottom: 2px solid black;">'
							...
							+'<p>If the weapon specified is not currently Proficient (has the <i>expert</i> tick-box set in the proficiencies table on the character sheet) and the character has enough slots remaining to gain the proficiency, the weapon will be added and, if checks are enabled, the proficiency level set to Proficient regardless of the level requested. Further requests to set proficiency levels for this weapon will only work one level of proficiency improvement at a time, Proficient > Specialist > Mastery, unless the tilde is used.</p>'
							+'<p>The proficiencies can include proficiencies in Fighting Styles, such as Two Weapon Style or Bowyer Style, including becoming Specialist or gaining Mastery of a fighting style. These fighting styles at higher levels of proficiency bestow advantages to the character\'s attacks. Certain classes are limited in which fighting styles are available to them, and how many styles they can gain Specialism in. See <i>The Complete Fighter\'s Handbook</i> for more information on fighting styles.</p>'
							+'<p><b>Note:</b> this does more than just entering the weapon in the proficiency table.  It adds the weapon group that the weapon belongs to as a field to the table (see weapon database help handouts for details), which is then used by the AttackMaster API to manage related weapon attacks and give the correct proficiency bonuses or penalties for the class and weapon used, and for fighting styles updates additional hidden tables that control attack bonuses.</p>'
	...
	const messages = Object.freeze({
		...
		initMsg:'&{template:'+fields.menuTemplate+'} {{name=Initialisation Complete}}{{desc=Initialisation complete.  **New** Command macros added.  Go to Collections tab (next to the cog at the top of the Chat window), and select them to show In Bar, and turn on the Macro Quick Bar.  Then start by dragging some characters on to the map to create tokens, and use Token-setup and Add-Items on each}}',
		...
		notRetail: '&{template:'+fields.warningTemplate+'}{{name=Not a Retailer}}{{desc=This token does not represent a retailer who can restock their inventory. Try a different one.}}',
		noToken:'No token selected, or the selected token does not represent a valid character sheet. Please select a valid token before proceeding.',
	});
	...
	const reEquipSpecs = {
		prime:		{field:'prime',def:'',re:/[\[,\s]prime:(.+?)[,\]]/i},
		offhand:	{field:'offhand',def:'',re:/[\[,\s]offhand:(.+?)[,\]]/i},
		both:		{field:'both',def:'',re:/[\[,\s]both:(.+?)[,\]]/i},
		others:		{field:'other',def:'',re:/[\[,\s]others:(.+?)[,\]]/i},
		items:		{field:'items',def:'',re:/[\[,\s]items:(.+?)[,\]]/i},
		...
		qty:		{field:'qty',def:1,re:/[\[,\s]qty:([\w\d\+\-\*\/\^\.\(\)]+?)[,\s\]]/i},
...
//				state.CommandMaster.cmds[1] = 'Bar|Add inactive bar as an action button|money| |';
			...
			reNPCThiefSpecs = RPGMap.reNPCThiefSpecs;
...
/*	 
	...
	const weaponLookup = function( charCS ) {
		...
		do {
			item = tableGroupLookup( MagicItems, 'name', i, false );
			if (_.isUndefined(item)) {break;}
			itemDef = abilityLookup( fields.WeaponDB, item, charCS );
			if (itemDef.obj) {
				weaponList = listWeaponTypes( itemDef, weaponList );
			}
	...
	/*
	 * Calculate how many weapon proficiency slots an item consumes
	...
	/*
	 * Count the number of weapon proficiency slots already used and 
	...
	const countUsedSlots = function( charCS, ProfTable, totalSlots ) {
		...
		for (let i=0; i < ProfTable.sortKeys.length; i++) {
			...
			if (proficiency && proficiency != '-') {
				...
				usedSlots += parseInt(slots);
			};
		};
		setAttr( charCS, fields.WeapProfSlotsRemain, totalSlots-usedSlots );
		return usedSlots;
	};
	...
	const getProfButtons = function( selected, isDisplay ) {
		...
		for (row=ProfTable.table[1]; !_.isUndefined(wpName = ProfTable.tableLookup( fields.WP_name, row, false )); row++) {
			if (wpName && wpName != '-') {
				wpType = ProfTable.tableLookup( fields.WP_type, row );
				if (wpType && wpType.length) types.push( wpType.dispName() );
				...
				if (wpType.dbName() !== 'fightingstyle') {
					profs.push( [wpName.dispName(),wpProf] );
				} else {
					styles.push([wpName.dispName(),wpProf]);
				};
			}
		}
		...
		profs= _.chain(profs)
				...
				.uniq(true)
				.map( item => (isDisplay ? ('<span style=' + design.grey_button + '>'+item[0]+item[1]+'</span>') : ('['+item[0]+item[1]+'](!cmd --button CHOOSE_PROF|'+item[0]+')')))
				.join('')
				.value();
...
//										.map(s => s.substring(0,80).trim());
	...
	const setVars = function(charCS,data,field,prefix,total,change=false) {
		...
		for (i=1; !_.isUndefined(data[field+i]) && i <= 9; i++) {
			if (change) {
				...
				setAttr( charCS, [(prefix+i),'current'], v[0] );
				setAttr( charCS, fields.NPCqualifier, v.join('%%') );
				setAttr( charCS, fields.Monster_hitDice, (parseInt(v[1]) || ''));
			};
		};
	...
	const setImgs = function(tokenObj,charCS,parsedData,change=false) {
		...
		if (change) {
			...
			setAttr( charCS, fields.Token_closedImgW, ((closedImg[1] || 70)/(parsedData.cimg ? pixelRatio: 1)) );
			setAttr( charCS, fields.Token_closedImgH, ((closedImg[2] || closedImg[1] || 70)/(parsedData.cimg ? pixelRatio: 1)) );
			setAttr( charCS, fields.Token_openImg, getCleanImgsrc(openImg[0]) );
			setAttr( charCS, fields.Token_openImgW, ((openImg[1] || 70)/(parsedData.oimg ? pixelRatio: 1)) );
			setAttr( charCS, fields.Token_openImgH, ((openImg[2] || openImg[1] || 70)/(parsedData.oimg ? pixelRatio: 1)) );
		} else {
		...
		var createBio = function( charCS, bioObj, partName, onlyGM=false ) {
			...
			if (descObj) {
				...
				if (!onlyGM) {
					...
					for (let i=1; i<=9; ++i) {
						if (!_.isUndefined(descObj['desc'+i])) content += '<p>'+descObj['desc'+i]+'</p>';
						if (!_.isUndefined(descObj['hide'+i])) content += '<p>'+descObj['hide'+i]+'</p>';
					};
				};
				_.each(descObj,(t,k) => {
					descObj[k] = t;
					...
					if (!onlyGM && k.toLowerCase().startsWith('section')) {
						...
						content += (['prefix','name','title','subtitle'].includes(k.toLowerCase().replace(/\s/g,'')) || k.toLowerCase().startsWith('desc') || k.toLowerCase().startsWith('hide')) ? '' : '<p><b>' + k + '</b>: ' + t + '</p>';
					}
				});
				content = content.replace(/\*\*\*(.*?)\*\*\*/img,'<b><i>$1</i></b>')
								 .replace(/\*\*(.*?)\*\*/img,'<b>$1</b>')
								 .replace(/\*(.*?)\*/img,'<i>$1</i>')
								 .replace(/$/img,'<br>');
				GMcontent = GMcontent.replace(/\*\*\*(.*?)\*\*\*/img,'<b><i>$1</i></b>')
									 .replace(/\*\*(.*?)\*\*/img,'<b>$1</b>')
									 .replace(/\*(.*?)\*/img,'<i>$1</i>')
									 .replace(/$/img,'<br>');
			};
	...
	/*
	 * Set NPC thieving skill levels using relative percentages,
	...
	const handleNPCthiefSkillPoints = function( charCS, senderId ) {
		...
		_.each(rogueSkills,skill => keys.push(skill.factors[6]));
			
		raceData = _.pick(raceData, (val,key) => keys.includes(key));
	...
	const setCreatureAttrs = function( cmd, charCS, senderId, creature, token, qualifier=[] ) {
		...
		async function addPowersAndItems( charCS, token, isCreature, senderId, qualifier ) {
			...
			blankItems( charCS );
			let content = ((await handleAddAllItems( token, charCS, senderId, 'wp', qualifier )
		...
		if (creature && creature.trim().length && isCreature) {
			...
			setAttr( charCS, fields.CalledShots, attrData.shots )+'/Disarm:-1:-4/Parry:0:0';
			setAttr( charCS, fields.SavedShots, attrData.shots )+'/Disarm:-1:-4/Parry:0:0';
			...
			setAttr( charCS, fields.Strength_hit, evalAttr(attrData.tohit,charCS) );
			setAttr( charCS, fields.Strength_dmg, evalAttr(attrData.dmg,charCS) );
			setAttr( charCS, fields.Dex_acBonus, evalAttr(attrData.dexdef,charCS) );
			setAttr( charCS, fields.MonsterCritHit, evalAttr(attrData.crith,charCS) );
			setAttr( charCS, fields.MonsterCritMiss, evalAttr(attrData.critm,charCS) );
			...
			setAttr( charCS, fields.SpellSpeedOR, evalAttr(attrData.spellspeed,charCS) );
			setAttr( charCS, fields.Regenerate, evalAttr(attrData.regen,charCS) );
			...
			setAttr( charCS, fields.Strength, str || (!attrRoll ? '' : evalAttr((attrRestrict ? '8:15' : '3d6'),charCS)));
			setAttr( charCS, fields.Constitution, evalAttr((attrData.con || (!attrRoll ? '' : (attrRestrict ? '7:14' : '3d6'))),charCS));
			setAttr( charCS, fields.Dexterity, evalAttr((attrData.dex || (!attrRoll ? '' : (attrRestrict ? '7:14' : '3d6'))),charCS));
			setAttr( charCS, fields.Intelligence, (attrData.int ? evalAttr(attrData.int) : (monInt || evalAttr((!attrRoll ? '' : (attrRestrict ? '7:15' : '3d6')),charCS))));
			setAttr( charCS, fields.Wisdom, evalAttr((attrData.wis || (!attrRoll ? '' : (attrRestrict ? '7:15' : '3d6'))),charCS));
			...
			if (qualifier && qualifier.length && (raceData.buy || raceData.sell)) {
				let NPCdata = [];
				_.each( ['buy','sell','nobuy','nosell','tobuy','tosell'], e => {
					if (!!raceData[e]) NPCdata.push(e+':'+raceData[e]);
				});
				setAttr( charCS, fields.AttrNotes, 'NPCdata=['+NPCdata.join(',')+']' );
				setAttr( charCS, fields.ItemContainerHide, 0 );
				setAttr( charCS, fields.ItemContainerType, 7 );
			};
...
//			setAttr( charCS, fields.Race, raceData.name );
			...
			if (attrData.lv) {
				...
				_.each( classData, (c,k) => {
					...
					setAttr( charCS, levelField, evalAttr(levels[k],charCS));
					isCaster = isCaster || (caster( charCS, 'MU' ).lv > 0) || (caster( charCS, 'PR' ).lv > 0);
				});
				handleAddAllPRspells( ['',BT.ALL_PRSPELLS,0], [token], senderId );
				handleNPCthiefSkillPoints( charCS, senderId );
				if (isCaster) {
			...
			handleSetAbility( ['',BT.AB_SILENT,'Items menu',std.mi_menu.api,std.mi_menu.action,'3.Items Menu','replace'], [token] );
			...
			handleSetAbility( ['',BT.AB_SILENT,'Rest',std.rest.api,std.rest.action,'6.Rest','replace'], [token] );
			...
			if (creature && creature.trim().length) {
				...
				setImgs(tokenObj,charCS,raceData,!isReset);
			}
		...
		if (creature && creature.trim().length || isReset) {
			addPowersAndItems( charCS, token, isCreature, senderId, qualifier );
			handleCheckThiefMods( [tokenObj.id], senderId, true );
...
// ---------------------------------------------------- Make Menus ---------------------------------------------------------
	...
	async function makeSpellsMenu( args, selected, senderId, msg ) {
		...
		try {
			...
			content += '}}{{desc1=2. Add '+(spell ? ('['+spellName) : ('<span style=' + design.grey_button + '>the '+word))
					+  ' to '+(isPower ? 'Powers' : ('level '+level+(isMU ? ' MU' : ' PR')+' spellbook'))+(spell ? ('](!cmd --button ADD_'+cmdStr+')') : '</span>')
					+  (isPower ? '}}' : '<br>or Add '+(spell ? ('['+spellName) : '<span style=' + design.grey_button + '>the spell')+' as a Power'+(spell ? ('](!cmd --button ADD_POWERS|1|'+pwrPrefix+spell+')') : ('</span>')) + '}}')
	...
	async function makeProficienciesMenu( args, selected, senderId, msg ) {
		...
		try {
			...
			if (state.attackMaster.weapRules.slots) {
				...
				setAttr( charCS, fields.WeapProfSlotTotal, totalSlots );
			} else {
				totalSlots = attrLookup( charCS, fields.WeapProfSlotTotal );
			};
			countUsedSlots( charCS, getTable( charCS, fieldGroups.WPROF ), totalSlots );
			
			if (isDisplay) {
				content += '{{name=Weapon Proficiencies';
			} else {
				content += '{{name=Grant Weapon Proficiencies}}{{ ='+(msg||'')+'}}'
						+ '{{  =['+((weapon && !style) ? weapon : 'Choose Weapon')+'](!cmd --button CHOOSE_PROF|&#63;{Choose which Weapon?|'+weapons+'})'
						+ 'or ['+((weapon && style) ? weapon : 'Choose Style')+'](!cmd --button CHOOSE_PROF|&#63;{Choose which Style?|'+styles+'})'
			...
			content += '}}{{Section1=**Weapon Proficiencies**\n'
					+  (buttons.profs || 'None') + '}}'
					+  '{{Section2=**Fighting Styles**\n'
					+  (buttons.styles || 'None') + '}}'
					+  '{{Section3=**Related Weapon Types**\n'
					+  (buttons.types || 'None') + '}}'
					+  '{{Section4=Total Slots [['+attrLookup(charCS,fields.WeapProfSlotTotal)+']], Free Slots [['+attrLookup(charCS,fields.WeapProfSlotsRemain)+']]}}';
			if (isDisplay) {
				content += '{{desc3=To gain new proficiencies or improve existing ones, find and buy training. Alternatively, the GM can grant proficiencies.}}'
				sendResponse(charCS,content,senderId,flags.feedbackName,flags.feedbackImg);
			} else {
				content += '{{desc3=[Return to Token Menu](!cmd --abilities) or just do something else}}';
				sendFeedback(content,flags.feedbackName,flags.feedbackImg);
			};
		} catch (e) {
			sendCatchError('CommandMaster',msg_orig[senderId],e);
	...
	async function makeClassMenu( args, selected, senderId, isGM, msg='' ) {
		...
		try {
			...
			const content = '&{template:'+fields.menuTemplate+'}{{name=Review & Set\nRace and Classes}}'
						...
						+ '<table><tr><td>[Race](!cmd --button '+BT.RACE+'|&#63;{Which Race?|'+races+'})</td>'+(isGM ? ('<td>[Creature](!cmd --button '+BT.CREATURE+'|&#63;{Which Creature?|'+creatures+'})</td><td>[NPC](!cmd --button '+BT.NPC+'|&#63;{Which NPC?|'+npcs+'})</td><td>[Service](!cmd --button '+BT.SERVICE+'|&#63;{Which Service?|'+services+'})</td><td>[Container](!cmd --button '+BT.CONTAINER+'|&#63;{Which Container?|'+containers+'})</td></tr></table>') : '')+'}}'
						+ '{{desc2=<table>'
		...
		try {
			...
			const updateItemAttr = function( charCS, item, f='', t='' ) {
				...
				filterObjs( obj => {
					...
					} else if (attrName.includes(item)) {
						if (attrName.endsWith('-')) attrName = attrName.substring(0,attrName.length-1);
						if (attrName.startsWith(fields.ItemMUspellsList[0]) || attrName.startsWith(fields.ItemMUspellValues[0]) || attrName.startsWith(fields.ItemPRspellsList[0]) || attrName.startsWith(fields.ItemPRspellValues[0]) || attrName.startsWith(fields.ItemPowersList[0]) || attrName.startsWith(fields.ItemPowerValues[0]) || attrName.startsWith(fields.MIspellPrefix[0]) || attrName.startsWith(fields.MIpowerPrefix[0])) {
							if (attrName.endsWith(item)) {
								obj.set('name',attrName+'+'+t);
							} else if (f !== t && new RegExp('('+item+'.*?\\+)'+f,'i').test(attrName)) {
								obj.set('name',attrName.replace(new RegExp('('+item+'.*?)\\+'+f+'(.*?)$','i'),'$1$2+'+t));
							} else if (f !== t && new RegExp('('+item+'-[^\\+]*?)$','i').test(attrName)) {
								obj.set('name',attrName.replace(new RegExp('('+item+'-[^\\+]*?)$','i'),'$1+'+t));
							}
						}
					};
					return true;
			...
			const storeItem = function( Items, from, fromName, fromTable, toName, speed, qty ) {
				...
				do {
					...
					if (_.isUndefined(to)) to = Items[toTable].addTableRow().sortKeys.length - 1;
					if (!_.isUndefined(to)) {
						let toIndex = indexTableGroup( Items, toTable, to ),
							toTrueName = _.isUndefined(from) ? toName : (Items[fromTable].tableLookup( fields.Items_trueName, from, false ) || toName),
							toTrueSpeed = speed,
							toTrueQty = qty,
							toP = Items[toTable].fieldGroup,
							fromP = Items[fromTable].fieldGroup,
							toValLine = (p,t,v) => {toValues[fields[p+t][0]][fields[p+t][1]] = v},
							fromValLine = (p,t) => fromValues[fields[p+t][0]][fields[p+t][1]];
						if (fromTable !== toTable) {
							toValues = Items[toTable].copyValues();
							if (fromName === toName) {
								fromValues = Items[fromTable].copyRow(from);
								toTrueName = fromValLine(fromP,'trueName');
								toTrueSpeed = fromValLine(fromP,'trueSpeed');
								toTrueQty = fromValLine(fromP,'trueQty');
								toValLine(toP,'cost',fromValLine(fromP,'cost'));
								toValLine(toP,'type',fromValLine(fromP,'type'));
								toValLine(toP,'reveal',fromValLine(fromP,'reveal'));
								toValLine(toP,'trueType',fromValLine(fromP,'trueType'));
								toValLine(toP,'carried',fromValLine(fromP,'carried'));
//								toValLine(toP,'weight',fromValLine(fromP,'weight'));
							};
						} else {
							toValues = Items[toTable].copyRow(to);
						};
						if (toObj) toValLine(toP,'cost',resolveData( toName, fields.MagicItemDB, reItemData, charCS ).parsed.cost || 0);
						toValLine(toP,'name',toName);
						toValLine(toP,'trueName',toTrueName);
						toValLine(toP,'speed',speed);
						toValLine(toP,'trueSpeed',toTrueSpeed);
						toValLine(toP,'qty',qty);
						toValLine(toP,'trueQty',toTrueQty);
						Items[toTable] = Items[toTable].addTableRow( to, toValues );
						updateItemAttr( Items[toTable].character, fromName, from, Items[toTable].rowID(to) );
						updateItemAttr( Items[toTable].character, fromName, Items[fromTable].rowID(from), Items[toTable].rowID(to) );
						if (from !== to || fromTable !== toTable) {
							Items[fromTable] = Items[fromTable].addTableRow(from);
						}
					} else {
						toTable = fromTable;
					};
					if (fromName !== toName) [from,fromTable] = tableGroupFind( Items, 'name', fromName );
				} while ((fromName !== toName) && !_.isUndefined(from));
			...
			_.each( selected, e => {
				...
				if (toConvert.length && replaceWith.length && cmd === BT.STORE_ITEM) {
					...
					sendAPI( '!attk --nowaitmsg --check-ac '+tokenID+'|silent' );
				};
			});
			if (selected.length) {
			...
			if (!cmd.length || cmd === BT.SORT_ITEMS) {
				...
				for (const e of selected) {
					...
					setTimeout( handleCheckWeapons, 1000, tokenID, charCS );
				};
			};
...
/
	...
	const makeAddTreasure = function( tokenID, charCS, retMenu, addedTreasure, senderId ) {
		...
		const creatureName = (attrLookup( charCS, fields.Race) || 'creature'),
			  ...
			  dispTres = tresTypes & tresTypes !== 'None' ? tresTypes.replace(/([\d\.]*)([A-Za-z])/g,(m,p1,p2) => ((p1.length ? p1+'x' : '')+p2+',')).split(',').filter(t=>!!t).join(', ') : tresTypes,
		...
		const content = '&{template:'+fields.menuTemplate+'}{{name=Add Treasure by Tables}}'
					  ...
					  + ' using the standard random treasure Table 84 given in the Dungeon Master\'s Guide. Lair types normally are added to a container in the lair}}'
...
//						sendError('Class or Race "'+name+'" not found when assigning items to a *Drag & Drop* NPC or Creature');
				...
				if (!noOtherWeaps) [weapData,noOtherWeaps] = getWeaps( charCS, fields.RaceDB, weapData, attrLookup( charCS, fields.Race ), type );
	...
	/*
	 * Restock selected retail outlets
	 */
	...
	async function handleDoRestock( args, selected, senderId ) {
		...
		try {
			...
			for (const t of selected) {
				charCS = getCharacter(t._id);
				if (!charCS) {
					sendError('No token selected, or the token does not represent a character',msg_orig[senderId]);
					return;
				};
				if ((attrLookup( charCS, fields.Gender ) || '').dbName() !== 'creature') {
					sendResponse( charCS, messages.notRetail );
					return;
				};
...
//			Add "magic items" of the type and quantity specified
	...
	const handleChooseProf = function( args, selected, senderId ) {
		...
		makeProficienciesMenu( ['',weapProf,mastery,weapType], selected, senderId, 'Chosen '+weapClass+' proficiency \n**'+weapProf+'**\n of type '+weapType );
	}
	...
	var handleAddProf = function( args, selected, senderId, silent=false ) {
		...
		_.each( selected, e => {
			...
			if (charCS) {
				...
				if (weapLevel == 'NOT-PROF' && !isTraining) {
					ProfTable.addTableRow( row );
					if (isStyle && !_.isUndefined(row = StyleTable.tableFind( fields.Style_name, weapon ))) {
					...
					if (!isStyle && !classAllowedItem( charCS, weapon, weapon, weapType, '', 'weaps' )) {
						failTrainingReason = charName + ' is of a class that cannot use a ' + weapon + ', so I can\'t train you to use it. No refunds!';
					} else if (isStyle && !classAllowedItem( charCS, weapon, weapon, weapType, '', 'styles' )) {
						failTrainingReason = charName + ' is of a class that cannot adopt the ' + weapon + ' style of fighting, so I can\'t train you to fight that way. No refunds!';
					} else {
						switch (curProf) {
						case 'NOT-PROF':
							if (weapLevel !== 'PROFICIENT') {
								if (isStyle) {
									failTrainingReason = charName+' has to be proficient in '+weapon+' style in order to train to be specialist, and certain classes may be restricted in which styles they can have. Sorry, no refunds!';
								} else {
									msg = charName + ' needs to be Proficient in '+weapText+' first, so that is the level I have trained them to. Sorry, no refunds!';
									weapLevel = 'PROFICIENT';
								}
								levelText = 'proficiency';
							};
							break;
						case 'PROFICIENT':
							switch (weapLevel) {
							case 'NOT-PROF':
								failTrainingReason = 'Once trained, '+charName+' can\'t forget their training! '+charName+' remains proficient in '+weapText;
								break;
							case 'PROFICIENT':
							...
							default:
								...
								weapLevel = 'SPECIALIST';
								break;
							};
							levelText = 'specialism';
							break;
						case 'SPECIALIST':
							if (weapLevel !== 'MASTERY') {
								failTrainingReason = charName + ' is already a Specialist in '+weapText+' so they need to purchase Mastery training. Sorry, no refunds!';
								break;
							} else if (isStyle && resolveData( weapon, fields.StylesDB, reItemData ).raw.length < 3) {
								failTrainingReason = weapText + ' fighting style cannot be trained to any higher level than Specialist. Sorry, you\'ve wasted your money!';
								break;
							}
							levelText = 'mastery';
							break;
						case 'MASTERY':
							failTrainingReason = charName + ' is already as skilled as you can be in '+weapText+'. I can\'t teach '+charName+' any higher skill. Sorry, no refunds!';
							break;
						default:
							failTrainingReason = weapLevel + ' is some level of proficiency in '+weapon+' that I\'ve never heard of so sorry, I can\'t train '+charName+' to that level.  No refunds!';
							break;
						};
					};
					if (!failTrainingReason) {
						...
						if (neededSlots > (totalSlots-usedSlots)) {
							...
							setAttr( charCS, fields.WeapProfSlotsRemain, totalSlots-usedSlots-neededSlots );
						};
					};
					if (failTrainingReason) {
						sendResponse( charCS, '&{template:'+fields.warningTemplate+'}{{name=Training Failed}}{{desc='+failTrainingReason+'}}', senderId );
						return;
					} else if (!msg) {
						msg = '&{template:'+fields.defaultTemplate+'}{{name=Successful Training}}{{desc='+charName + ' has learned well, and has now gained '+levelText+' in '+weapText+'. Your records have been updated.}}';
					} else {
						msg = '&{template:'+fields.warningTemplate+'}{{name=Training Incomplete}}{{desc='+msg+'}}';
					}
				...
				if (isStyle && weapLevel === 'MASTERY' && resolveData( weapon, fields.StylesDB, reItemData ).raw.length < 3) weapLevel = 'SPECIALIST';

				weapProf[fields.WP_name[0]][fields.WP_name[1]] = weapon;
				weapProf[fields.WP_type[0]][fields.WP_type[1]] = weapType;
				weapProf[fields.WP_slots[0]][fields.WP_slots[1]] = weapSlots;
				weapProf[fields.WP_expert[0]][fields.WP_expert[1]] = '1';
				weapProf[fields.WP_specialist[0]][fields.WP_specialist[1]] = (weapLevel == 'SPECIALIST' || weapLevel == 'MASTERY') ? '1' : '0';
				weapProf[fields.WP_mastery[0]][fields.WP_mastery[1]] = (weapLevel == 'MASTERY') ? '1' : '0';
		...
		try {
			...
			_.each( selected, e => {
				...
				if (charCS) {
					...
					weapons.push({weapon:'Single-Weapon-Style',weapType:'Fighting-Style'});
					weapons.push({weapon:'Two-Hander-Style',weapType:'Fighting-Style'});
					if (!isRogue && !isWizard) weapons.push({weapon:'Weapon-and-Shield-Style',weapType:'Fighting-Style'});
					if (!isPriest && !isWizard) weapons.push({weapon:'Two-Weapon-Style',weapType:'Fighting-Style'});
					if (isCreature) weapons.push({weapon:'Thrown-Weapon-Style',weapType:'Fighting-Style'});
					if (isCreature) weapons.push({weapon:'Bowyer-Style',weapType:'Fighting-Style'});

					for (let row = 0; row < weapons.length; row++) {
						handleAddProf( ['~PROFICIENT', weapons[row].weapon, weapons[row].weapType], [e], null, silent || row < (weapons.length-1) );
						msg = 'Set all weapons as proficient';
					}
				}
			});
			if (!silent) makeProficienciesMenu( [''], selected, senderId, msg );
		} catch (e) {
			sendCatchError('CommandMaster',msg_orig[senderId],e);
		}
	}
	...
	/*
	 * Check with GM if a freshly dropped token represents
	 * a PC, NPC or Solo Creature (which should have linked
	 * default tokens) or a Mob Creature which should not
	 * have linked bars
	 */
	 
	const tokenType = function( tokenID, delay=false ) {
		const charCS = getCharacter( tokenID );
		if (!charCS) return;
		
		const curToken = getObj('graphic',tokenID);
		if (curToken.get('isdrawing')) return;
		
		const bar1val = curToken.get('bar1_value'),
			  bar2val = curToken.get('bar2_value'),
			  bar3val = curToken.get('bar3_value'),
			  dtExists = String(bar1val) + String(bar2val) + String(bar3val);
		
		if (dtExists  && dtExists.trim().length) return;
		const content = '&{template:'+fields.defaultTemplate+'}{{title=Character Type}}'
				+ '{{desc=Select the type of Character from the following:\n[Player Character (PC)](!cmd --nowaitmsg --button '+BT.AB_TOKEN+'||) [Non-Player Character (NPC)](!cmd --nowaitmsg --button '+BT.AB_TOKEN+'||) [Solo Creature](!cmd --nowaitmsg --button '+BT.AB_TOKEN+'||) [Creature Mob](!cmd --nowaitmsg --button '+BT.AB_TOKEN_NONE+'||) [None of these](!magic --message '+tokenID+'|Setting Character Type|The token has not been changed.  Use the GM\'s *Token Setup > Manage Token Bars* to change token bar settings.)}}'
				+ '{{hide1=**PC or NPC:** These characters have tokens that are fully linked to their character sheet, with values changed on one mirrored on the other.}}'
				+ '{{hide2=**Solo Creature:** A creature that has a fully linked token and has a one to one relationship token to character sheet.}}'
				+ '{{hide3=**Creature Mob:** One character sheet that has multiple tokens on the same page. The token bars are not fully linked to the character sheet so that each token can have different values.}}'
				+ '{{hide4=**None of these:** Does not set a default token}}';
		if (delay) {
			setTimeout( () => sendFeedback(content), 1000 );
		} else {
			sendFeedback(content);
		};
		return;
...
//							setDefaultTokenForCharacter( charCS, getObj('graphic',token._id) );
						...
						if (cmd !== BT.CONTAINER && cmd !== BT.CONTAINER_CKD) {
							if (![BT.CREATURE,BT.NPC,BT.SERVICE,BT.CREATURE_CKD].includes(cmd)) {
...
//				if (!errFlag && selected.length === 1) makeClassMenu( args, selected, senderId, true, msg );
			...
			if (tokenID) {
				...
				for(const token of selected) {
					setTimeout( classAndRace, (1000*t++), cmd, args, token, value, senderId, selected, msg, qualifier );
				};
			}
		} catch (e) {
			sendCatchError('CommandMaster',msg_orig[senderId],e);
	...
	var handleSetTokenBars = function( args, selected, senderId, silent=false ) {  	// toLowerCase
		...
		silent = silent || ((args[2] || '').toLowerCase().trim() === 'silent');
				...
				try {
					...
					if (charCS) {
						...
						if (thac0) {
							...
							thac0 = createObj('attribute', {characterid:charCS.id, name:fields.Thac0_base[0], current:thac0val, max:''});
						}
		...
		if (!silent) {
			...
			if (cmd === BT.AB_TOKEN_NONE) {
				content += '{{desc=Bar 1, 2 & 3 links set to \"None\"\n'
						...
						+  '{{desc1=This is best for multi-token Character Sheets e.g. Creature mobs. Area effect magical effects *should* work on mobs where part of the mob is in the area of effect, as long as spell effects in the Effects-DB have been created using the correct RPGMaster API commands.}}';
			} else {
	...
	const handleChangeImages = function( args, selected, senderId ) {
		...
		if (imgType.includes('img')) {
			...
			setAttr( charCS, fields.NPCqualifier, qualifier );
		};
...
// ------------------------------------------------------------- Command Action Functions ---------------------------------------------
	...
	var doInitialise = function(senderID) {
		...
		macro = findObjs({ type: 'macro', name: 'Surprise'},{caseInsensitive:true});
		if (!macro || !macro.length || _.isUndefined(macro[0])) {
			newMacro = createObj('macro',{name:'Surprise',action:'!init --surprise',playerid:senderID});
		}
		if (!!newMacro) {
	...
	const doSetProf = function(args, selected, senderId) {
		if ((!args[4] || !args[4].length) && (!selected || !selected.length)) {
			...
			handleAddProf(args, selected, senderId);
		}
		return;
	}
	...
	/*
	 * Restock the items held by a charater, generally used to restock
	...
	async function doCheckCharSetup(senderId) {
		...
		try {
			...
			_.each( dmCtrl.sort( (a,b) => {return (a[0].toLowerCase() === b[0].toLowerCase()) ? 0 : (a[0].toLowerCase() < b[0].toLowerCase() ? -1 : 1 );}), name => content += makeButton(name[0],'!cmd --button '+BT.PLAYER_CTRL+'|'+name[1]+'|&#63;{Which Player should control '+name[0]+'?|'+players.join('|')+'}'));
		...
		const doCommandCmd = function( e, selected, senderId, isGM ) {
			...
			try {
				...
				case 'setallprof':
					...
					break;
				case 'restock':
					if (isGM) doRestock(arg,senderId);
					break;
				case 'convspells':
...
//					log('commandMaster handleChatMessage: processing a --conv-items command');
			...
			if (!!state.CommandMaster.debug) reportTimes('!cmd --'+e);
		}
...
// -------------------------------------------------------------- Register the API -------------------------------------------
	...
	/*
	 * Look out for a new dropped token. If with as yet 
	 * undefined class or level or specs for a creature
	 * then pop up the class/race/creature dialog.
	 * Otherwise, if no default token, ask if a mob and
	 * set the default token appropriately.
	 */
