...
//					log('listProblemNames: legal = '+legal+', errMsg = '+msg);
...
//					+ '{{Section2=**Speaking As Names**\n'+illegalSpeakers+'}}'
					...
					+ (msg && msg.trim().length ? ('{{Section5=**To correct the selected name:** '+parseStr(msg)+'}}') : '{{desc=Select any name button to get recommendations as to how to rectify the issue.}}');
...
//			weaponSpecs = weaponSpecs ? [...('['+weaponSpecs[0]+']').matchAll(/\[\s*?(\w[\s\|\w\-]*?)\s*?,\s*?(\w[-\s\w\|]*?\w)\s*?,\s*?(\w[\s\w]*?\w)\s*?,\s*?(\w[\s\|\w\-]*?\w)\s*?\]/g)] : [];
	...
	var handleClassSelection = function( args, selected, isGM ) {
		...
		_.each( selected, token => {
			...
			if (cmd !== BT.CREATURE) handleCheckWeapons( token._id, charCS );
		});
...
/*	 
	var handleCheckWeapons = function( tokenID, charCS ) {
		...
		if (itemIndex.length && !isNaN(itemIndex)) {
			sendAPI('!attk --button '+BT.BOTH+'|'+tokenID+'|'+itemIndex+'|2||silent');
		}
		
		for (let r=3; !_.isUndefined(itemIndex = InHand.tableLookup( fields.InHand_index, r, false )); r++) {
			if (itemIndex.length && !isNaN(itemIndex)) {
				sendAPI('!attk --button '+BT.HAND+'|'+tokenID+'|'+itemIndex+'|'+r+'||silent');
			}
		}
		return;
	}
