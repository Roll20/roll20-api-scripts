...
//	const stdDB = ['MU_Spells_DB','PR_Spells_DB','Powers_DB','MI_DB','MI_DB_Ammo','MI_DB_Armour','MI_DB_Light','MI_DB_Potions','MI_DB_Rings','MI_DB_Scrolls_Books','MI_DB_Wands_Staves_Rods','MI_DB_Weapons','Attacks_DB','Class_DB','Race-DB','Race-DB-Creatures'];
	...
	CommandMaster_Help:	{name:'CommandMaster Help',
						 ...
						 bio:'<div style="font-weight: bold; text-align: center; border-bottom: 2px solid black;">'
							+'<span style="font-weight: bold; font-size: 125%">CommandMaster Help v2.04</span>'
							+'</div>'
...
/* ------------------------------------------------ Utility Functions -------------------------------------------- */
	...
	var setCreatureAttrs = function( charCS, creature, selected ) {
		...
		var parseDesc = function( charCS, creature ) {
			var content = '',
				GMcontent = '',
				creatureObj = getAbility( fields.RaceDB, creature, charCS );
				
			if (!creatureObj.obj) return;
			var descObj = _.object([...creatureObj.obj[1].body.replace(/[\r\n]/g,'').matchAll(/\{\{([^{}]+?)=(.*?)\}\}/g)].map(v => v.slice(1)));

			if (descObj) {
				content += '<h2>'+(descObj.title || '')+' '+(descObj.name || '')+'</h2>'
				_.each(descObj,(t,k) => {
					...
					if (k.toLowerCase().startsWith('section')) {
						if (t.endsWith('</b>')) {
							content += '<h3>' + t + '</h3>';
						} else {
							content += '<p>' + t + '</p>';
				...
				charCS.set( "bio", content );
				charCS.set( "gmnotes", GMcontent );
			}
		}
		...
		setTimeout( () => {
			...
			let content = ((handleAddAllItems( selected[0]._id, charCS, 'wp' ) + handleAddAllItems( selected[0]._id, charCS, 'ac' )) || '').trim();
...
// ---------------------------------------------------- Make Menus ---------------------------------------------------------
	...
	async function makeSpellsMenu( args, selected, msg ) {
	...
	async function makeProficienciesMenu( args, selected, msg ) {
		...
		var	weapon = args[1] || '',
			...
			buttons, weapObj,
			weapons = getMagicList( fields.WeaponDB, miTypeLists, 'weapon' ),
			styles = getMagicList( fields.StylesDB, miTypeLists, 'style' ),
	...
	async function makeClassMenu( args, selected, isGM, msg='' ) {
...
//			race_def = abilityLookup( fields.RaceDB, race, charCS, true );
	...
	async function makeConvertItemsMenu( args, selected ) {
...
//				sendAPI( '!attk --check-ac '+tokenID+'|' );
		...
		if (!miList.length) {
			...
			let weapons = getMagicList(fields.MagicItemDB,miTypeLists,'weapon'),
				ammo = getMagicList(fields.MagicItemDB,miTypeLists,'ammo'),
				armour = getMagicList(fields.MagicItemDB,miTypeLists,'armour'),
				potions = getMagicList(fields.MagicItemDB,miTypeLists,'potion'),
				scrolls = getMagicList(fields.MagicItemDB,miTypeLists,'scroll'),
				rods = getMagicList(fields.MagicItemDB,miTypeLists,'rod'),
				rings = getMagicList(fields.MagicItemDB,miTypeLists,'ring'),
				misc = getMagicList(fields.MagicItemDB,miTypeLists,'miscellaneous');
	...
	async function makeConvertSpellsMenu( args, selected ) {
		...
		muSpells = getMagicList( fields.MU_SpellsDB, spTypeLists, 'muspelll'+toLevel ),
		prSpells = getMagicList( fields.PR_SpellsDB, spTypeLists, 'prspelll'+toLevel ),
		powers = getMagicList( fields.PowersDB, spTypeLists, 'power' );
...
// --------------------------------------------------------------- Button press Handlers ----------------------------------------------
	...
	async function handleAddSpell( args, selected ) {
	...
	async function handleAddAllPRspells( args, selected ) {
...
//			weaponSpecs = weaponSpecs ? [...('['+weaponSpecs[0]+']').matchAll(/\[\s*?(\w[\s\|\w\-]*?)\s*?,\s*?(\w[-\s\w\|]*?\w)\s*?,\s*?(\w[\s\w]*?\w)\s*?,\s*?(\w[\s\|\w\-]*?\w)\s*?\]/g)] : [];
	...
	async function handleAddAllProfs( args, selected ) {
	...
	async function handleClassSelection( args, selected, isGM ) {
	...
	async function handleSetSaves( args, selected ) {
	...
	async function handleSetDefaultBars( args, selected ) {
...
//	var doCheckCharSetupDelayed = function() {
	async function doCheckCharSetup() {
		...
		_.each(args, function(e) {
			setTimeout( () => {
				...
				try {
					...
					sendError('CommandMaster JavaScript '+err.name+': '+err.message);
				}
			}, (1*t++), e);
		});
	};
	...
	var handleNewToken = function(obj,prev) {
		...
		if (obj.get('_subtype') == 'token' && !obj.get('isdrawing')) {
			...
			if (charCS) {
				...
				} else if ((race || '').toLowerCase() === 'magic item') {
					handleSetTokenBars( [BT.AB_TOKEN], [obj], true );
