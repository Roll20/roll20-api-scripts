...
//	const stdDB = ['MU_Spells_DB','PR_Spells_DB','Powers_DB','MI_DB','MI_DB_Ammo','MI_DB_Armour','MI_DB_Light','MI_DB_Potions','MI_DB_Rings','MI_DB_Scrolls_Books','MI_DB_Wands_Staves_Rods','MI_DB_Weapons','Attacks_DB','Class_DB','Race-DB','Race-DB-Creatures'];
	...
	const reAttr = Object.freeze ({
		...
		cac:		{field:'cac',def:'',re:/[:\s\|]?ac\s*=\s*([\s\d]+?):?([\s\d]*?)[\|,\]]/i},
		mov:		{field:'mov',def:'',re:/[:\s\|]?mov\s*=\s*(.+?):?([\s\d]*?)[\|,\]]/i},
		fly:		{field:'fly',def:'',re:/[:\s\|]?fly\s*=\s*(.+?)[\|,\]]/i},
		hd:			{field:'hd',def:'',re:/[:\s\|]?hd\s*=\s*([d\d\+\-]+?)[\|,\]]/i},
...
/* ------------------------------------------------ Utility Functions -------------------------------------------- */
	...
	var calcAttr = function( attrRange='3:18' ) {
		...
		log('calcAttr: attrRange = '+attrRange+', low = '+low+', high = '+high);
		if (high && !isNaN(high)) {
...
// --------------------------------------------------------------- Button press Handlers ----------------------------------------------
	...
	var handleAddAllPowers = function( args, selected ) {
		...
		var charCS, powers, content, classes, race, raceObj, spellSet;
		...
		_.each( selected, t => {
			...
			if (raceObj.obj) {
				...
				if (raceType && raceType[0][4]) {
					raceObj = abilityLookup( fields.RaceDB, raceType[0][4] );
					if (raceObj.obj) {
						spellSet = addMIspells( charCS, raceObj.obj[1] );
						if (spellSet.PW[0].length) {
							let list = spellSet.PW[0].join('|');
