...
//			tokenObj.set('imgsrc',getCleanImgsrc(closedImg[0]));
	...
	var parseDesc = function( charCS, senderId ) {
		...
		var createBio = function( charCS, bioObj, partName, onlyGM=false ) {
			...
			var descObj = _.object([...bioObj.obj[1].body.replace(/[\r\n]/g,'').matchAll(/\{\{([^{}]+?)=(.*?)\}\}/g)].map(v => v.slice(1))),
				...
				GMcontent = '';
			if (descObj) {
				content += '<h3>'+partName+': '+(onlyGM ? bioObj.obj[1].name : ((descObj.title || '')+' '+(descObj.name || ''))) + '</h3>';
				_.each(descObj,(t,k) => {
					...
					if (!onlyGM && k.toLowerCase().startsWith('section')) {
						...
						content += (['name','title','subtitle'].includes(k.toLowerCase().replace(/\s/g,'')) || k.toLowerCase().startsWith('desc')) ? '' : '<p><b>' + k + '</b>: ' + t + '</p>';
					}
				});
				if (!onlyGM) {
	...
	var setCreatureAttrs = function( cmd, charCS, senderId, creature, token ) { 
		...
		async function addPowersAndItems( charCS, token, isCreature, senderId ) {
			if (handleAddAllPowers( [BT.RACE], 'PW', [token], senderId )) {
			...
			if (!isCreature) handleAddAllPowers( [BT.RACE], 'AB', [token], senderId );
...
// ---------------------------------------------------- Make Menus ---------------------------------------------------------
	...
	async function makeClassMenu( args, selected, senderId, isGM, msg='' ) {
		...
		try {
			...
			var	fighter_classes = getMagicList( fields.ClassDB, clTypeLists, 'warrior', senderId, 'Warrior|Fighter|Paladin|Ranger', true, 'Specify class' ),
				...
				races = getMagicList( fields.RaceDB, clTypeLists, 'humanoid', senderId, 'Human|Dwarf|Elf|Gnome|Half-Elf|Halfling|Half-Orc', true, 'Specify race' ),
	...
	async function makeConvertSpellsMenu( args, selected, senderId ) {
		...
		try {
			...
			muSpells = getMagicList( fields.MU_SpellsDB, spTypeLists, 'muspelll'+toLevel, senderId ),
			...
			content += '}}{{Section5=**Spell Database Lists**\n'
					...
					+  '[Wizard L'+toLevel+'](!cmd --button '+BT.TOSPELL+'|'+caster+'|'+fromLevel+'|'+fromName+'|mu|'+toLevel+'|&#63;{Which spell is a substitute for '+fromName+'?&#124;'+muSpells+'|Level1|Level2|Level3|Level4|Level5|Level6|Level7|Level8|Level9})'
					+  '[Priest L'+toLevel+'](!cmd --button '+BT.TOSPELL+'|'+caster+'|'+fromLevel+'|'+fromName+'|pr|'+toLevel+'|&#63;{Which spell is a substitute for '+fromName+'?&#124;'+prSpells+'|Level1|Level2|Level3|Level4|Level5|Level6|Level7})'
					+  '[Powers](!cmd --button '+BT.TOSPELL+'|'+caster+'|'+fromLevel+'|'+fromName+'|pw|'+toLevel+'|&#63;{Which spell is a substitute for '+fromName+'?&#124;'+powers+'})'
...
// --------------------------------------------------------------- Button press Handlers ----------------------------------------------
	...
	var handleAddAllPowers = function( args, type, selected, senderId, silent=false ) {
...
//		silent = silent || (type === 'AB');
		
		var tokenID, tokenObj, charCS, powers=[], content,
			classes, race, raceObj, spellSet, parsedData, parsedAttr, raceData,
			racePowers = [], abAction = [];
		...
		_.each( selected, t => {
			...
			if (!silent) content = '&{template:'+fields.defaultTemplate+'}{{name='+charCS.get('name')+'\'s Powers}}'
			...
			for (const c of classes) {
				if (c.obj && c.dB.dbName().startsWith(fields.ClassDB.dbName())) {
					...
					if (spellSet[type][0].length) {
						powers.push(spellSet[type][0].join('|'));
					}
					if (!silent) content += '{{'+className+'='+(spellSet[type][0].join('|').replace(/\|/g,', ') || 'None')+'}}';
				}
			}
			...
			if (type === 'PW') {
				...
				if (lock) {
					let lockObj = abilityLookup( fields.AbilitiesDB, roller+lock, charCS, true );
					if (!lockObj.obj) {
						lockObj = abilityLookup( fields.AbilitiesDB, lock, charCS );
						roller = '';
					};
					if (lockObj.obj) {
						[parsedData,parsedAttr,raceData] = resolveData( roller+lock, fields.AbilitiesDB, /abilitydata=([^\{]*)/im );
				...
				if (trap) {
					let trapObj = abilityLookup( fields.AbilitiesDB, roller+trap, charCS, true );
					if (!trapObj.obj) {
				...
				_.each (_.uniq(powers.join('|').split('|').filter(ab=>!!ab)),(ab,i) => {
					let abObj = abilityLookup( fields.AbilitiesDB, ab, null, true, false );
					if (!abObj.obj) {sendError('Specified ability macro "'+ab+'" does not exist in the databases',msg_orig[senderId]);return;}
			...
			if (!silent && powers.length) {
				content += '{{'+race+'='+(_.uniq(racePowers.join('|').split('|')).join(', ') || 'None')+'}}';
				sendFeedback( content,flags.feedbackName,flags.feedbackImg );
			}
		});
	...
	var handleChangeLockOrTrap = function( args, senderId ) {
		...
		if (!abObj.obj) {
			...
			handleAddAllPowers(args, 'AB', [{_id:tokenID}], senderId);
...
//			weaponSpecs = weaponSpecs ? [...('['+weaponSpecs[0]+']').matchAll(/\[\s*?(\w[\s\|\w\-]*?)\s*?,\s*?(\w[-\s\w\|]*?\w)\s*?,\s*?(\w[\s\w]*?\w)\s*?,\s*?(\w[\s\|\w\-]*?\w)\s*?\]/g)] : [];
	...
	async function handleClassSelection( args, selected, senderId, isGM ) {
		...
		try {
			...
			var setClassAndRace = function( cmd, args, token, value, senderId ) {
				...
				return new Promise(resolve => {
					try {
						...
						case BT.RESET_CONTAINER:
							log('setClassAndRace: resetting container');
						...
						case BT.CONTAINER:
							...
							setCreatureAttrs( cmd, charCS, senderId, value, token );
			...
			async function classAndRace( cmd, args, token, value, senderId ) {
				return await setClassAndRace( cmd, args, token, value, senderId );
			}
			...
			let t=0;
			if (tokenID) {
				errFlag = await setClassAndRace( cmd, args, {_id:tokenID}, value, senderId );
			} else {
				for( const token of selected) {
					setTimeout( classAndRace, (10000*t++), cmd, args, token, value, senderId );
				};
	...
	var handleChangeImages = function( args, selected, senderId ) {
		...
		args.shift();
		makeChangeImagesMenu( args, senderId, 'The stored '+imgType+' for the trapped / locked container '+charCS.get('name')+' has been successfully changed');
		return;
	}
