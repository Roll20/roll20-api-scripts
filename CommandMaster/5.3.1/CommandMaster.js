 ...
 *                     DM macro button.
 * v5.3.1  30/05/2026  Corrected --add-treasure command display when None defined.  On first character sheet
...
//																								  .replace(/\*\*(.*?)\*\*/img,'<b>$1</b>')
//																								  .replace(/\*(.*?)\*/img,'<i>$1</i>'))+'</p>';
//						if (!_.isUndefined(descObj['hide'+i])) content += '<p>'+(descObj['hide'+i].replace(/\*\*\*(.*?)\*\*\*/img,'<b><i>$1</i></b>')
//																								  .replace(/\*\*(.*?)\*\*/img,'<b>$1</b>')
//																								  .replace(/\*(.*?)\*/img,'<i>$1</i>'))+'</p>';
					};
				};
...
//		log('handleNPCthiefSkillPoints: returned from resolveData() '+attrLookup( charCS, fields.Race )+' raceData = '+_.pairs(raceData));
		
		_.each(rogueSkills,skill => keys.push(skill.factors[6]));
			
		raceData = _.pick(raceData, (val,key) => keys.includes(key));
			
//		log('handleNPCthiefSkillPoints: then after pick, '+attrLookup( charCS, fields.Race )+' raceData = '+_.pairs(raceData));
...
//		log('tokenType: default token = '+dtExists+', length = '+dtExists.trim().length);
