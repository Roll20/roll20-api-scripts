var CustomStatusMarkers = (() => {
  ...
  class Commands {
     ...
     */
    static importState(msg) {
      let argv = msg.content.split(' ');
      let json = argv.slice(1).join(' ');
      importState(msg.playerid, json);
    }
  ...
  class Menu {
    ...
    static show(playerId) {
      ...
      if(playerIsGM(playerId)) {
        ...
        actionsHtml += '<div style="text-align: center;">[⏏ Export State](' + EXPORT_STATE_CMD + ')</div>';
        actionsHtml += '<div style="text-align: center;">[⚠ Import State](' + IMPORT_STATE_CMD + ' ?{Paste exported state JSON here:})</div>';
    ...
    /**
     * Produces HTML content in a bordered panel.
     * @param {string} header
  ...
  function exportState(playerId) {
    ...
    let html = Menu.showPanel('Export Custom Status Markers', content);
    _whisper(playerId, html);
   ...
   * @param {string} json
   */
  function importState(playerId, json) {
    let scriptState = getState();
  ...
  on('ready', () => {
    ...
    _.each(players, player => {
      _installMacro(player, 'CustomStatusMarkersMenu', MENU_CMD);
      _installMacro(player, 'CustomStatusMarkersToggle', SET_MARKER_CMD + ' ' + getEffectNamePrompt());
      _installMacro(player, 'CustomStatusMarkersToggleCount', SET_MARKER_COUNT_CMD + ' ' + getEffectNamePrompt() + ' ?{count}');
      _installMacro(player, 'CustomStatusMarkersToggleTint', SET_MARKER_TINT_CMD + ' ' + getEffectNamePrompt() + ' ?{color}');
    });
