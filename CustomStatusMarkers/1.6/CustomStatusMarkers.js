...
CustomStatusMarkers = (function() {
    ...
    /**
     * Checks if a token has a specified custom status marker.
     * @param  {(Graphic|String)}  token
     *         The token or its ID.
     * @param  {String}  statusName
     *         The name of the custom status.
     * @return {Boolean}
     *         True iff the token currently has a custom status marker for
     *         statusName.
     */
    function hasStatus(token, statusName) {
        if(_.isString(token)) {
          token = findObjs({
            _type: 'graphic',
            _id: token
